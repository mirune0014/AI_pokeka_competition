...
_turn_end_prize_floor_latch = {}
_last_decision_signature = None
_last_decision_action = None

_TURN_END_PRIZE_FLOOR_VERSION = "turn_end_exact_prize_floor_v1"
...
def _clear_turn_end_prize_floor_latch() -> None:
    _turn_end_prize_floor_latch.clear()
...
def _clear_emergency_state(*, clear_cache: bool = False) -> None:
    ...
    _clear_turn_end_prize_floor_latch()
    if clear_cache:
...
def _decision_signature(obs: Observation) -> tuple:
    ...
    turn_end_guard_signature = None
    if _turn_end_prize_floor_latch:
        turn_end_guard_signature = (
            _turn_end_prize_floor_latch.get("stage"),
            _turn_end_prize_floor_latch.get("next_action"),
...
def _turn_end_cards_fingerprint(cards, expected_player: int | None) -> tuple | None:
    """Freeze one fully public card zone in serial order."""
    if cards is None:
        return None
    rows = []
    for card in cards:
        if (
            ...
            or (
                ...
                and getattr(card, "playerIndex", None) != expected_player
            )
        ):
            return None
...
def _turn_end_pokemon_group_fingerprint(
    state, pokemon_group, expected_player: int
) -> tuple | None:
    rows = []
    serials = []
    for pokemon in pokemon_group:
        if not _bridge_pokemon_is_publicly_complete(pokemon, expected_player):
            return None
        rows.append(_bridge_pokemon_fingerprint(pokemon))
        serials.extend(_bridge_pokemon_component_serials(pokemon))
    if not _bridge_protected_serials_are_unique(state, serials):
        return None
    return tuple(sorted(rows, key=lambda row: row[1]))


def _turn_end_status_fingerprint(player_state) -> tuple:
    return (
        player_state.poisoned,
        player_state.burned,
        player_state.asleep,
...
def _turn_end_public_snapshot(state, my_index: int) -> dict | None:
    """Return the exact public state needed by one frozen Prize route."""
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    if mine.hand is None or len(mine.hand) != mine.handCount:
        return None
    zones = {
        "own_hand": _turn_end_cards_fingerprint(mine.hand, my_index),
        "own_discard": _turn_end_cards_fingerprint(mine.discard, my_index),
        "opponent_discard": _turn_end_cards_fingerprint(
            theirs.discard, 1 - my_index
        ),
        "stadium": _turn_end_cards_fingerprint(state.stadium, None),
        "own_active": _turn_end_pokemon_group_fingerprint(
            state,
            [pokemon for pokemon in mine.active if pokemon is not None],
            my_index,
        ),
        "own_bench": _turn_end_pokemon_group_fingerprint(
            state, list(mine.bench), my_index
        ),
        "opponent_active": _turn_end_pokemon_group_fingerprint(
            state,
            [pokemon for pokemon in theirs.active if pokemon is not None],
            1 - my_index,
        ),
        "opponent_bench": _turn_end_pokemon_group_fingerprint(
            state, list(theirs.bench), 1 - my_index
        ),
    }
    if any(value is None for value in zones.values()):
        return None
    visible_serials = _bridge_public_serials(state)
    if len(visible_serials) != len(set(visible_serials)):
        return None
    opponent_field = tuple(
        sorted(
            zones["opponent_active"] + zones["opponent_bench"],
    ...
    return {
        **zones,
        ...
        "own_status": _turn_end_status_fingerprint(mine),
        "opponent_status": _turn_end_status_fingerprint(theirs),
...
def _turn_end_normalized_text(text: str) -> str:
    return " ".join(
        _normalized_skill_text(text)
        .replace("é", "e")
        .replace("’", "'")
...
def _turn_end_telepath_text_certified() -> bool:
    """Certify exact card 19 and its optional zero-card Bench search."""
    ...
    if (
        ...
        return False
    text = _turn_end_normalized_text(data.skills[0].text)
    return text == (
...
def _turn_end_evolution_text_certified(card_id: int) -> bool:
    expected = {
        Kadabra: (
    ...
    return bool(
        ...
        and _turn_end_normalized_text(data.skills[0].text) == expected
    )


def _turn_end_attack_metadata_certified(attack_id: int) -> bool:
    attack = attack_table.get(attack_id)
    if attack is None or tuple(int(unit) for unit in attack.energies) != (
        int(EnergyType.PSYCHIC),
    ):
        return False
    text = _turn_end_normalized_text(attack.text)
    if attack_id == ATTACK_TELEPORTATION:
        return (
            attack.damage == 10
            and text
    ...
    if attack_id == ATTACK_SUPER_PSY_BOLT:
        return attack.damage == 30 and not text
    if attack_id == ATTACK_POWERFUL_HAND:
        return (
            attack.damage == 0
            and "place 2 damage counters" in text
            and "for each card in your hand" in text
        )
    return False


def _turn_end_powerful_hand_target_clear(state, target: Pokemon) -> bool:
    """Target-local Powerful Hand certificate, including safe Spiky Energy."""
    data = card_table.get(target.id)
    if data is None or data.resistance == EnergyType.PSYCHIC:
        return False
    energy_ids = {card.id for card in target.energyCards}
    ...
    if (
        ...
        return False
    known_nondefensive_special = {
        12,  # Legacy Energy: prize_count handles it.
        14,  # Spiky Energy triggers on attack damage; 1072 places counters.
        Enriching_Energy,
        Telepath_Psychic_Energy,
        Rock_Fighting_Energy,
    }
...
def _turn_end_visible_attack_effects_clear(
    state, target: Pokemon, attack_id: int
) -> bool:
    """Resolve protection only on the fixed target, never another lane."""
    if attack_id == ATTACK_POWERFUL_HAND:
        return _turn_end_powerful_hand_target_clear(state, target)
    return _powerful_hand_target_is_publicly_clear(state, target)


def _turn_end_attack_profile(
    state,
    attacker_id: int,
    target: Pokemon,
    hand_count: int,
) -> dict | None:
    """Certify exact damage and Prize value without future information."""
    if (
        hand_count < 0
        or attacker_id not in ABRA_LINE
        or not _bridge_pokemon_is_publicly_complete(
            target, 1 - state.yourIndex
        )
        or target.hp <= 0
    ):
        return None
    attack_id = {
        Abra: ATTACK_TELEPORTATION,
        Kadabra: ATTACK_SUPER_PSY_BOLT,
        Alakazam: ATTACK_POWERFUL_HAND,
    }[attacker_id]
    if not _turn_end_attack_metadata_certified(attack_id):
        return None
    if not _turn_end_visible_attack_effects_clear(state, target, attack_id):
        return None
    if attack_id == ATTACK_POWERFUL_HAND:
        ...
        if target_data is None:
            return None
        damage = int(attack.damage)
        if target_data.weakness == EnergyType.PSYCHIC:
            damage *= 2
        if target_data.resistance == EnergyType.PSYCHIC:
            damage = max(0, damage - 30)
    target_prizes = prize_count(target)
    takes_prize = damage >= target.hp and target_prizes > 0
    return {
        "attack_id": attack_id,
        "damage": damage,
        "target_prizes": target_prizes if takes_prize else 0,
        "target_fingerprint": _bridge_pokemon_fingerprint(target),
    }


def _turn_end_active_energy_units(active: Pokemon) -> tuple[int, ...] | None:
    units = _bridge_retaliation_energy_units(active)
    if units is None:
        return None
    payable = _bridge_retaliation_can_pay(
        units, (int(EnergyType.PSYCHIC),)
    )
    return units if payable is not None else None


def _turn_end_hand_card(
    obs: Observation, option, my_index: int
) -> Card | None:
    mine = obs.current.players[my_index]
    if (
        option.area not in (None, AreaType.HAND)
    ...
    if (
        ...
        or card.id <= 0
        or card.serial <= 0
        or getattr(card, "playerIndex", None) != my_index
        or card_table.get(card.id) is None
    ):
        return None
    return card
...
def _turn_end_evolution_candidates(
    obs: Observation, active: Pokemon, my_index: int
) -> list[dict]:
    expected = {Abra: Kadabra, Kadabra: Alakazam}.get(active.id)
    if expected is None:
        return []
    candidates = []
    for option_index, option in enumerate(obs.select.option):
        if (
            ...
            continue
        card = _turn_end_hand_card(obs, option, my_index)
        if (
            card is None
            or card.id != expected
            or not _turn_end_evolution_text_certified(card.id)
        ):
            continue
        candidates.append(
            {
                ...
                "card_fingerprint": _bridge_card_fingerprint(card),
                "active_serial": active.serial,
    ...
    return sorted(
        candidates,
        key=lambda row: (row["card_serial"], row["option_index"]),
    )


def _turn_end_attach_candidates(
    obs: Observation, active: Pokemon, my_index: int
) -> list[dict]:
    candidates = []
    for option_index, option in enumerate(obs.select.option):
        if (
            ...
            continue
        card = _turn_end_hand_card(obs, option, my_index)
        if card is None or card.id not in PSYCHIC_ENERGY_IDS:
            continue
        if (
            card.id == Telepath_Psychic_Energy
            and not _turn_end_telepath_text_certified()
        ):
            continue
        candidates.append(
            {
...
def _turn_end_boss_candidates(obs: Observation, my_index: int) -> list[dict]:
    candidates = []
    ...
    if (
        data is None
        or data.cardType != CardType.SUPPORTER
        or len(data.skills or []) != 1
        or _turn_end_normalized_text(data.skills[0].text)
    ...
    for option_index, option in enumerate(obs.select.option):
        if option.type != OptionType.PLAY:
            continue
        card = _turn_end_hand_card(obs, option, my_index)
        if card is None or card.id != Boss_Orders:
            continue
        candidates.append(
            {
                "kind": "boss",
...
def _turn_end_attack_action(
    attack_id: int, target: Pokemon
) -> dict:
    return {
        "kind": "attack",
        "attack_id": attack_id,
...
def _turn_end_route_sort_key(route: dict) -> tuple:
    card_serials = tuple(
        action["card_serial"]
        for action in route["actions"]
        if action["kind"] in ("evolve", "attach", "boss")
    )
...
def _turn_end_exact_prize_routes(obs: Observation) -> list[dict]:
    """Enumerate only complete public two-to-four-action Prize routes."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    opponent_active = theirs.active[0] if len(theirs.active) == 1 else None
    snapshot = _turn_end_public_snapshot(state, my_index)
    if (
        ...
        or snapshot is None
        or active is None
        or active.id not in ABRA_LINE
        or opponent_active is None
        or not _bridge_pokemon_is_publicly_complete(active, my_index)
        or _turn_end_active_energy_units(active) is None
        or any(_turn_end_status_fingerprint(mine)[index] for index in (2, 3, 4))
        or len(mine.prize) <= 0
    ):
        return []

    energy_units = _turn_end_active_energy_units(active)
    can_pay_now = _bridge_retaliation_can_pay(
        energy_units, (int(EnergyType.PSYCHIC),)
    )
    if can_pay_now is None:
        return []
    evolution_rows = [None, *_turn_end_evolution_candidates(obs, active, my_index)]
    attach_rows = _turn_end_attach_candidates(obs, active, my_index)
    boss_rows = _turn_end_boss_candidates(obs, my_index)
    routes = []

    for evolution in evolution_rows:
        attacker_id = evolution["card_id"] if evolution else active.id
        attacker_serial = (
            evolution["card_serial"] if evolution else active.serial
        )
        prefix = [] if evolution is None else [evolution]
        if can_pay_now:
            lane_variants = [(list(prefix), None)]
        elif not state.energyAttached:
            lane_variants = [
                (list(prefix) + [attachment], attachment)
                for attachment in attach_rows
            ]
        else:
            lane_variants = []

        for lane_actions, attachment in lane_variants:
            hand_after_lane = mine.handCount - len(lane_actions)
            if hand_after_lane < 0:
                continue
            active_profile = _turn_end_attack_profile(
                state, attacker_id, opponent_active, hand_after_lane
            )
            active_gain = (
                active_profile["target_prizes"]
                if active_profile is not None
                else 0
            )
            if active_gain > 0 and lane_actions:
                post_ko = max(0, len(mine.prize) - active_gain)
                if not (
                    attachment is not None
                    and attachment["card_id"] == Telepath_Psychic_Energy
                    and post_ko > 0
                    and mine.deckCount <= post_ko
                ):
                    actions = lane_actions + [
                        _turn_end_attack_action(
                            active_profile["attack_id"], opponent_active
                        )
                    ]
                    if 2 <= len(actions) <= 4:
                        routes.append(
                            {
                                "actions": actions,
                                "active_serial": attacker_serial,
                                "attacker_id": attacker_id,
                                "target_serial": opponent_active.serial,
                                "target_fingerprint": active_profile[
                                    "target_fingerprint"
                                ],
                                "target_hp": opponent_active.hp,
                                "target_prizes": active_gain,
                                "post_ko_prizes": post_ko,
                                "damage": active_profile["damage"],
                                "attack_id": active_profile["attack_id"],
                                "has_boss": False,
                                "final_prize": active_gain >= len(mine.prize),
                                "telepath": bool(
                                    attachment is not None
                                    and attachment["card_id"]
                                    == Telepath_Psychic_Energy
                                ),
                            }
                        )
            ...
            for boss in boss_rows:
                hand_after_boss = hand_after_lane - 1
                if hand_after_boss < 0:
                    continue
                for target in theirs.bench:
                    profile = _turn_end_attack_profile(
                        state, attacker_id, target, hand_after_boss
                    )
                    if profile is None or profile["target_prizes"] <= 0:
                        continue
                    target_gain = profile["target_prizes"]
                    final_prize = target_gain >= len(mine.prize)
                    if not (
                        target_gain > active_gain
                        or (final_prize and active_gain < len(mine.prize))
                    ):
                        continue
                    post_ko = max(0, len(mine.prize) - target_gain)
                    if (
                        attachment is not None
                        and attachment["card_id"] == Telepath_Psychic_Energy
                        and post_ko > 0
                        ...
                        continue
                    boss_action = dict(boss)
                    boss_action["target_serial"] = target.serial
                    boss_action["target_fingerprint"] = (
                        _bridge_pokemon_fingerprint(target)
                    )
                    actions = lane_actions + [
                        boss_action,
                        _turn_end_attack_action(profile["attack_id"], target),
                    ]
                    if 2 <= len(actions) <= 4:
                        routes.append(
                            {
                                ...
                                "attacker_id": attacker_id,
                                "target_serial": target.serial,
                                "target_fingerprint": profile[
                                    "target_fingerprint"
                                ],
                                "target_hp": target.hp,
                                "target_prizes": target_gain,
                                "post_ko_prizes": post_ko,
    ...
    return sorted(routes, key=_turn_end_route_sort_key)


def _turn_end_remove_hand_cards(
    hand_fingerprint: tuple, actions, completed_actions: int
) -> tuple | None:
    remaining = list(hand_fingerprint)
    for action in actions[:completed_actions]:
        if action["kind"] not in ("evolve", "attach", "boss"):
            continue
        fingerprint = action.get("card_fingerprint")
        matches = [
            index for index, row in enumerate(remaining) if row == fingerprint
        ]
        if len(matches) != 1:
            return None
        remaining.pop(matches[0])
    return tuple(sorted(remaining, key=lambda row: row[1]))


def _turn_end_expected_active_fingerprint(
    latch: dict, completed_actions: int
) -> tuple | None:
    initial = latch.get("initial_active_fingerprint")
    actions = latch.get("actions") or ()
    if not isinstance(initial, tuple) or len(initial) != 10:
        return None
    (
        card_id,
        serial,
    ...
    for action in actions[:completed_actions]:
        if action["kind"] == "evolve":
            data = card_table.get(action["card_id"])
            if data is None or data.hp <= max_hp:
                return None
            pre_evolution = pre_evolution + (
                (card_id, serial, latch.get("player")),
            )
            hp = hp + data.hp - max_hp
            max_hp = data.hp
            card_id = action["card_id"]
            serial = action["card_serial"]
            appear_this_turn = True
        elif action["kind"] == "attach":
            energies = energies + (int(EnergyType.PSYCHIC),)
            energy_cards = energy_cards + (
                action["card_fingerprint"],
            )
    return (
        card_id,
        serial,
...
def _turn_end_prefix_matches(
    ...
    *,
    ...
    state = obs.current
    my_index = state.yourIndex
    current = _turn_end_public_snapshot(state, my_index)
    initial = latch.get("initial_snapshot")
    actions = latch.get("actions") or ()
    if (
        current is None
        ...
        or my_index != latch.get("player")
        or completed_actions < 0
        or completed_actions > len(actions)
    ):
        return False
    expected_hand = _turn_end_remove_hand_cards(
        initial["own_hand"], actions, completed_actions
    )
    expected_active = _turn_end_expected_active_fingerprint(
        latch, completed_actions
    )
    if expected_hand is None or expected_active is None:
        return False

    completed = actions[:completed_actions]
    attachment_done = any(
        action["kind"] == "attach" for action in completed
    )
    boss_actions = [
        action for action in completed if action["kind"] == "boss"
    ]
    if len(boss_actions) > 1:
        return False
    boss_done = bool(boss_actions)
    expected_discard = initial["own_discard"]
    if boss_done and boss_switched:
        expected_discard = tuple(
            sorted(
                expected_discard
                + (boss_actions[0]["card_fingerprint"],),
                key=lambda row: row[1],
            )
        )
    ...
    if boss_switched:
        if not boss_done:
            return False
        target_fingerprint = latch.get("target_fingerprint")
        old_active = initial["opponent_active"]
        expected_opponent_active = (target_fingerprint,)
        expected_opponent_bench = list(initial["opponent_bench"])
        matches = [
            index
            for index, row in enumerate(expected_opponent_bench)
            if row == target_fingerprint
        ]
        if len(matches) != 1 or len(old_active) != 1:
            return False
        expected_opponent_bench.pop(matches[0])
        expected_opponent_bench.extend(old_active)
        expected_opponent_bench = tuple(
            sorted(expected_opponent_bench, key=lambda row: row[1])
        )
        expected_opponent_status = (False, False, False, False, False)
    else:
        expected_opponent_active = initial["opponent_active"]
        expected_opponent_bench = initial["opponent_bench"]
        expected_opponent_status = initial["opponent_status"]

    return (
        current["own_hand"] == expected_hand
        and current["own_discard"] == expected_discard
        and current["opponent_discard"] == initial["opponent_discard"]
        and current["stadium"] == initial["stadium"]
        and current["own_active"] == (expected_active,)
        and current["own_bench"] == initial["own_bench"]
        and current["opponent_field"] == initial["opponent_field"]
        and current["opponent_active"] == expected_opponent_active
        and current["opponent_bench"] == expected_opponent_bench
        and current["own_status"] == initial["own_status"]
        and current["opponent_status"] == expected_opponent_status
        and current["own_deck_count"] == initial["own_deck_count"]
        and current["opponent_deck_count"]
        == initial["opponent_deck_count"]
        and current["own_prize_count"] == initial["own_prize_count"]
        and current["opponent_prize_count"]
        == initial["opponent_prize_count"]
        and current["opponent_hand_count"]
        == initial["opponent_hand_count"]
        and current["supporter_played"]
        == (initial["supporter_played"] or boss_done)
        and current["energy_attached"]
        == (initial["energy_attached"] or attachment_done)
        and current["retreated"] == initial["retreated"]
    )


def _turn_end_match_action_option(
    obs: Observation, action: dict
) -> int | None:
    select = obs.select
    ...
    for option_index, option in enumerate(select.option):
        if action["kind"] == "attack":
            if (
                ...
                and option.attackId == action["attack_id"]
            ):
                matches.append(option_index)
            continue
        if action["kind"] == "evolve":
        ...
        if (
            card is not None
            and _bridge_card_fingerprint(card)
            == action.get("card_fingerprint")
        ):
            matches.append(option_index)
    if (
        len(matches) != 1
...
def _turn_end_final_attack_certified(
    obs: Observation, latch: dict, completed_actions: int
) -> int | None:
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    ...
    action = (latch.get("actions") or ())[completed_actions]
    if (
        select.context != SelectContext.MAIN
        or active is None
        or target is None
        or active.id != latch.get("attacker_id")
        or active.serial != latch.get("active_serial")
        ...
        or _bridge_retaliation_can_pay(
            _turn_end_active_energy_units(active),
            (int(EnergyType.PSYCHIC),),
        )
        is not True
    ):
        return None
    profile = _turn_end_attack_profile(
        state, active.id, target, mine.handCount
    )
    if (
        profile is None
        or profile["attack_id"] != action.get("attack_id")
        or profile["attack_id"] != latch.get("attack_id")
        or profile["damage"] != latch.get("damage")
        or profile["target_prizes"] != latch.get("target_prizes")
        or profile["target_prizes"] <= 0
        or max(0, len(mine.prize) - profile["target_prizes"])
        != latch.get("post_ko_prizes")
        or (
            latch.get("telepath")
            and latch.get("post_ko_prizes", 0) > 0
            and mine.deckCount <= latch.get("post_ko_prizes")
        )
    ):
        return None
    return _turn_end_match_action_option(obs, action)


def _turn_end_issue_route_action(
    obs: Observation, latch: dict, action_index: int
) -> list[int] | None:
    actions = latch.get("actions") or ()
    ...
    if action["kind"] == "attack":
        option_index = _turn_end_final_attack_certified(
            obs, latch, action_index
        )
    else:
        option_index = _turn_end_match_action_option(obs, action)
    if option_index is None:
        _clear_turn_end_prize_floor_latch()
        return None
    latch["next_action"] = action_index + 1
    latch["stage"] = {
        "evolve": "await_evolution_choice",
        "attach": "await_attachment_main",
        "boss": "await_boss_target",
        "attack": "await_resolution",
    }[action["kind"]]
    return [option_index]


def _turn_end_optional_telepath_selection_is_exact(
    obs: Observation, latch: dict
) -> bool:
    select = obs.select
    my_index = obs.current.yourIndex
    actions = latch.get("actions") or ()
    action_index = latch.get("next_action", 0) - 1
    if action_index not in range(len(actions)):
        return False
    attachment = actions[action_index]
    effect = select.effect
    context_card = select.contextCard
    effect_fingerprint = (
        _bridge_card_fingerprint(effect) if effect is not None else None
    )
    context_fingerprint = (
        _bridge_card_fingerprint(context_card)
        if context_card is not None
        else None
    )
    if (
        attachment["kind"] != "attach"
        or attachment["card_id"] != Telepath_Psychic_Energy
        or select.context != SelectContext.TO_BENCH
        or attachment["card_fingerprint"]
        not in (effect_fingerprint, context_fingerprint)
    ...
    for option in select.option:
        if (
            ...
            or option.index < 0
            or option.index >= len(obs.select.deck)
        ):
            return False
...
def _turn_end_advance_route(obs: Observation, latch: dict) -> list[int] | None:
    next_action = latch.get("next_action")
    actions = latch.get("actions") or ()
    if not isinstance(next_action, int) or next_action not in range(len(actions)):
        _clear_turn_end_prize_floor_latch()
        return None
    return _turn_end_issue_route_action(obs, latch, next_action)


def _turn_end_exact_prize_floor_overlay(
    obs: Observation,
) -> list[int] | None:
    if not _turn_end_prize_floor_latch:
        return None
    latch = _turn_end_prize_floor_latch
    state = obs.current
    select = obs.select
    if (
        latch.get("version") != _TURN_END_PRIZE_FLOOR_VERSION
        or state.turn != latch.get("turn")
        ...
        return None
    stage = latch.get("stage")
    completed = latch.get("next_action")
    ...
    if stage == "await_resolution":
        _clear_turn_end_prize_floor_latch()
        return None
    if stage == "await_evolution_choice":
        if (
            select.context != SelectContext.ACTIVATE
            or not _turn_end_prefix_matches(
                obs,
                latch,
        ...
        if (
            len(no_matches) != 1
            or len(yes_matches) != 1
            or len(select.option) != 2
        ):
            _clear_turn_end_prize_floor_latch()
            return None
        latch["stage"] = "await_evolution_main"
        return [no_matches[0]]
    if stage == "await_evolution_main":
        if (
            select.context != SelectContext.MAIN
            or not _turn_end_prefix_matches(
                obs,
            ...
            _clear_turn_end_prize_floor_latch()
            return None
        return _turn_end_advance_route(obs, latch)
    if stage == "await_attachment_main":
        if not _turn_end_prefix_matches(
            obs,
            latch,
            completed_actions=completed,
            boss_switched=False,
        ...
        if select.context == SelectContext.TO_BENCH:
            if not _turn_end_optional_telepath_selection_is_exact(obs, latch):
                _clear_turn_end_prize_floor_latch()
                return None
            latch["stage"] = "await_attachment_after_optional"
            return []
        if select.context != SelectContext.MAIN:
            _clear_turn_end_prize_floor_latch()
            return None
        return _turn_end_advance_route(obs, latch)
    if stage == "await_attachment_after_optional":
        if (
            select.context != SelectContext.MAIN
            or not _turn_end_prefix_matches(
                obs,
    ...
    if stage == "await_boss_target":
        if (
            select.context not in (SelectContext.SWITCH, SelectContext.TO_ACTIVE)
            or not _turn_end_prefix_matches(
                obs,
                latch,
                completed_actions=completed,
            ...
            _clear_turn_end_prize_floor_latch()
            return None
        boss = (latch.get("actions") or ())[completed - 1]
        effect = select.effect
        if (
            effect is None
            or _bridge_card_fingerprint(effect)
            != boss.get("card_fingerprint")
        ):
            _clear_turn_end_prize_floor_latch()
            return None
        ...
        for option_index, option in enumerate(select.option):
            if (
                ...
                or option.playerIndex != 1 - state.yourIndex
                or not isinstance(option.index, int)
                or option.index < 0
                or option.index >= len(theirs.bench)
            ):
                _clear_turn_end_prize_floor_latch()
                return None
            target = theirs.bench[option.index]
            if (
                target.serial == latch.get("target_serial")
                and _bridge_pokemon_fingerprint(target)
                == latch.get("target_fingerprint")
            ):
                matches.append(option_index)
        if len(matches) != 1:
            _clear_turn_end_prize_floor_latch()
            return None
        latch["stage"] = "await_boss_main"
        return [matches[0]]
    if stage == "await_boss_main":
        if (
            select.context != SelectContext.MAIN
            or not _turn_end_prefix_matches(
                obs,
...
def _turn_end_parent_attack_is_zero_prize(
    ...
    return bool(
        profile is not None
        and profile["attack_id"] == option.attackId
        and profile["target_prizes"] == 0
    )


def _turn_end_start_latch(
    obs: Observation, route: dict
) -> list[int] | None:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if snapshot is None or active is None:
        return None
    _turn_end_prize_floor_latch.update(
        version=_TURN_END_PRIZE_FLOOR_VERSION,
        stage="starting",
        turn=state.turn,
        player=my_index,
        actions=tuple(dict(action) for action in route["actions"]),
        next_action=0,
        initial_snapshot=snapshot,
        initial_active_fingerprint=_bridge_pokemon_fingerprint(active),
        active_serial=route["active_serial"],
        attacker_id=route["attacker_id"],
        target_serial=route["target_serial"],
        target_fingerprint=route["target_fingerprint"],
        target_hp=route["target_hp"],
        target_prizes=route["target_prizes"],
        post_ko_prizes=route["post_ko_prizes"],
        damage=route["damage"],
        attack_id=route["attack_id"],
        telepath=route["telepath"],
        final_prize=route["final_prize"],
    )
    action = _turn_end_issue_route_action(
        obs, _turn_end_prize_floor_latch, 0
    )
    if action is None:
        _clear_turn_end_prize_floor_latch()
    return action
...
def _start_turn_end_exact_prize_floor(
    obs: Observation,
    parent_top_option,
    parent_top_index: int | None,
) -> list[int] | None:
    """Start only at the frozen turn-ending/resource-destruction boundary."""
    if (
        _turn_end_prize_floor_latch
        ...
        or parent_top_index is None
    ):
        return None
    routes = _turn_end_exact_prize_routes(obs)
    if not routes:
        return None
    route = routes[0]
    first_option_index = _turn_end_match_action_option(
        obs, route["actions"][0]
    )
    if first_option_index is None:
        return None
    parent_is_first = parent_top_index == first_option_index
    start = parent_is_first or parent_top_option.type == OptionType.END

    if (
        not start
        and parent_top_option.type == OptionType.ATTACK
        and _turn_end_parent_attack_is_zero_prize(obs, parent_top_option)
    ):
        start = True
    if not start and parent_top_option.type == OptionType.RETREAT:
        start = True

    route_attachment = next(
        (
            action
            for action in route["actions"]
    ...
    if (
        not start
        and route_attachment is not None
        and parent_top_option.type == OptionType.ATTACH
    ):
        start = True

    route_evolution = next(
        (
            action
            for action in route["actions"]
            if action["kind"] == "evolve"
    ...
    if (
        not start
        and route_evolution is not None
        and parent_top_option.type == OptionType.EVOLVE
    ):
        parent_card = _turn_end_hand_card(
            obs, parent_top_option, obs.current.yourIndex
        )
        if (
            parent_card is not None
            and parent_card.serial == route_evolution["card_serial"]
        ):
            start = True

    if not start and parent_top_option.type == OptionType.PLAY:
        parent_card = _turn_end_hand_card(
            obs, parent_top_option, obs.current.yourIndex
        )
        route_has_boss = any(
            action["kind"] == "boss" for action in route["actions"]
        )
        if parent_card is not None:
            parent_data = card_table.get(parent_card.id)
            # Preserve every inherited Boss decision unless this exact Boss
            # is already the certified route's first action.
            if parent_card.id == Boss_Orders:
                start = parent_is_first
            elif (
                route_has_boss
                and parent_data is not None
                and parent_data.cardType == CardType.SUPPORTER
            ):
                start = True

    return _turn_end_start_latch(obs, route) if start else None
...
def _start_fez_ko_bridge(
    ...
    if (
        ...
        or state.retreated
        or _fez_ko_bridge_latch
        or _turn_end_prize_floor_latch
        or _hilda_source_latch
        or _enriching_reserve_latch
        or parent_top_option is None
...
def agent(obs_dict: dict) -> list[int]:
    ...
    emergency_action = _turn_end_exact_prize_floor_overlay(obs)
    if emergency_action is not None:
        return _remember_action(_decision_signature(obs), emergency_action)
    ...
    turn_end_action = _start_turn_end_exact_prize_floor(
        obs,
        select.option[desc_indices[0]] if desc_indices else None,
        desc_indices[0] if desc_indices else None,
    )
    if turn_end_action is not None:
        return _remember_action(_decision_signature(obs), turn_end_action)
    ...
    if (
        ...
        and not _turn_end_prize_floor_latch
    ):
