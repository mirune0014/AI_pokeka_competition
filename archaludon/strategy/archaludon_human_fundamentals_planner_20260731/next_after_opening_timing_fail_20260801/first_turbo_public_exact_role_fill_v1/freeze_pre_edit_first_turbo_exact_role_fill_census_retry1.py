...
ATTEMPT1_RUNNER = HERE / "freeze_pre_edit_first_turbo_exact_role_fill_census.py"
ATTEMPT1_SPEC = HERE / "EXECUTION_SPEC_FIRST_TURBO_PUBLIC_EXACT_ROLE_FILL_V1.md"
ATTEMPT1_FAILURE = HERE / "ROOT_EXECUTION_ATTEMPT1_FAILURE.md"
OUTPUT = HERE / "pre_edit_first_turbo_exact_role_fill_census_retry1_raw"
...
EXPECTED_HASHES = {
    ...
    "attempt1_runner": "42243ED174DFF40D80BD6DBD7004E81B339FBE22576CFAE7ED2D408E20B075F1",
    "attempt1_spec": "3DE92E1B0461E4396D2FE5B502FCBD7D945B727E48D3553715BC7CE66F18DF21",
    "attempt1_failure": "320EA49356A071C32F788C38097AE264BB356E0EF61DA51B9E702087513A2E6C",
}
...
def main():
    ...
    actual_hashes = {
        ...
        "attempt1_runner": sha256(ATTEMPT1_RUNNER),
        "attempt1_spec": sha256(ATTEMPT1_SPEC),
        "attempt1_failure": sha256(ATTEMPT1_FAILURE),
    }
        ...
        for seat in entry["target_seats"]:
            ...
            for step_index, step in enumerate(replay["steps"]):
                ...
                if starts:
                    ...
                    if callback_key in callback_by_key:
                        raise SystemExit("duplicate semantic first-Turbo callback key")
                    callback_by_key[callback_key] = row
                ...
                prior_row = callback_by_key.get(callback_key)
                if prior_row is not None:
                    signature_fields = (
                        "context_energy_serial", "parent_semantic",
                        "contract_semantic", "parent_valid", "contract_valid",
                        "pre_owners", "post_owners", "classification",
                        "direction", "predicted_difference", "error",
                    )
                    if any(prior_row[field] != row[field] for field in signature_fields):
                        raise SystemExit(
                            "non-identical semantic first-Turbo callback retry"
                        )
                    duplicate_callback_retries += 1
                    continue
                callback_by_key[callback_key] = row
    ...
    summary = {
        ...
        "integrity": {
            ...
            "unique_semantic_callback_keys": len(callback_by_key),
            "identical_callback_retries_collapsed": duplicate_callback_retries,
