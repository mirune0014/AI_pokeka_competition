...
def needs_lillie_bench_rescue(obs) -> bool:
    return bool(
        active_pokemon(obs)
        and not any(me(obs).bench or [])
        and not any(cid in CYNTHIA_BASICS for cid in hand_ids(obs))
        and any(p.id in ALAKAZAM_LINE for p in opponent_pokemon(obs))
    )
...
def score_play(obs, opt) -> tuple[int, str]:
    ...
    if cid == LILLIE:
        if needs_lillie_bench_rescue(obs):
            return 50000, "Lillie for lone-board basic"
