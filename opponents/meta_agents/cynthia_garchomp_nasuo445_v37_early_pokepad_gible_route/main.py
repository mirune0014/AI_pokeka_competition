...
def effect_source_is_poke_pad(obs, opt=None) -> bool:
    """Identify a public effect prompt by its stable source-card marker."""
    select = obs.select
    markers = [
        getattr(select, name, None)
        for name in (
            "effectSourceId", "effect_source_id", "effectCardId", "effect_card_id",
            "fromCardId", "from_card_id", "effect", "contextCard", "context_card",
    ...
    return any(getattr(marker, "id", marker) == POKE_PAD for marker in markers)


def early_pokepad_before_energy_attach_index(obs, scored: list[tuple[int, int, str]]) -> int | None:
    """On turns one and two, search a plausible missing Gible before the top Energy attach."""
    if (
        obs.select.context != SelectContext.MAIN
        or getattr(obs.current, "turn", 0) not in {1, 2}
        or sum(1 for pokemon in my_pokemon(obs) if pokemon.id in MAIN_LINE) >= 3
        or not gible_remains(obs)
    ):
        return None
    _score, selected_index, _reason = max(scored, key=lambda row: (row[0], -row[1]))
    selected = obs.select.option[selected_index]
    if (
        selected.type != OptionType.ATTACH
        or getattr(option_card(obs, selected), "id", None) not in ENERGIES
    ):
        return None
    pokepad_actions = [
        (score, index) for score, index, _reason in scored
        if (
            obs.select.option[index].type == OptionType.PLAY
            and getattr(option_card(obs, obs.select.option[index]), "id", None) == POKE_PAD
        )
    ]
    return max(pokepad_actions, key=lambda row: (row[0], -row[1]))[1] if pokepad_actions else None


def early_pokepad_gible_target_index(obs, scored: list[tuple[int, int, str]]) -> int | None:
    """Prefer Gible to Gabite only in an explicitly marked Poke Pad resolution."""
    if (
        ...
        return None
    _score, selected_index, _reason = max(scored, key=lambda row: (row[0], -row[1]))
    if getattr(option_card(obs, obs.select.option[selected_index]), "id", None) != GABITE:
        return None
    gible_actions = [
        (score, index) for score, index, _reason in scored
        if (
            getattr(option_card(obs, obs.select.option[index]), "id", None) == GIBLE
            and effect_source_is_poke_pad(obs, obs.select.option[index])
        )
    ]
    return max(gible_actions, key=lambda row: (row[0], -row[1]))[1] if gible_actions else None
    ...
    if backup_index is not None:
        ...
        for row_index, (score, index, reason) in enumerate(scored):
            if index == backup_index:
                ...
                break
    pokepad_index = early_pokepad_before_energy_attach_index(obs, scored)
    if pokepad_index is not None:
        selected_attach_score = max(scored, key=lambda row: (row[0], -row[1]))[0]
        for row_index, (score, index, reason) in enumerate(scored):
            if index == pokepad_index:
                scored[row_index] = (
                    selected_attach_score + 1,
                    index,
                    f"{reason} before early Energy attachment",
                )
                break
    pokepad_gible_index = early_pokepad_gible_target_index(obs, scored)
    if pokepad_gible_index is not None:
        selected_gabite_score = max(scored, key=lambda row: (row[0], -row[1]))[0]
        for row_index, (score, index, reason) in enumerate(scored):
            if index == pokepad_gible_index:
                scored[row_index] = (
                    selected_gabite_score + 1,
                    index,
                    f"{reason} from Poke Pad",
                )
                break
