...
def _two_prize_basic_play_lethal_conversion_unsafe(
    obs_dict: dict,
    obs: Observation,
    ...
    parent_index = chosen_action[0]
    if not 0 <= parent_index < len(select.option):
        return None
    parent_option = select.option[parent_index]
    if (
        not isinstance(parent_option.index, int)
        or isinstance(parent_option.index, bool)
        or not _two_prize_option_is_exact(
            parent_option, OptionType.PLAY, index=parent_option.index
        )
        or not 0 <= parent_option.index < len(mine.hand)
        or len(mine.bench) >= mine.benchMax
    ):
        return None
    played = mine.hand[parent_option.index]
    played_data = card_table.get(played.id)
    if (
        played_data is None
        or played_data.cardType != CardType.POKEMON
        or not played_data.basic
        or played_data.stage1
        or played_data.stage2
        or played_data.evolvesFrom is not None
        or played_data.skills
    ):
        return None
    same_card_plays = []
    for option_index, option in enumerate(select.option):
        if (
            ...
            and 0 <= option.index < len(mine.hand)
            and mine.hand[option.index].serial == played.serial
        ):
            same_card_plays.append(option_index)
    if same_card_plays != [parent_index]:
        return None
    ...
    if (
        target is None
        or not _two_prize_opponent_pokemon_is_clear(
            state, target, 1 - my_index
        )
    ...
    hand_count = mine.handCount
    if not (
        20 * hand_count >= target.hp > 20 * (hand_count - 1)
        and hand_count == (target.hp + 19) // 20
    ):
        return None
    successors = [
        pokemon
    ...
    for option in select.option:
        ...
        if (
            ...
            return None
        if mine.hand[option.index].id != Boss_Orders:
            continue
        if not _two_prize_option_is_exact(
            option, OptionType.PLAY, index=option.index
        ):
            return None
        legal_boss.append(option)
    if legal_boss:
        boss_damage = 20 * (hand_count - 1)
        for pokemon in theirs.bench:
            prizes = prize_count(pokemon)
            if boss_damage >= pokemon.hp and (
                prizes > 2 or prizes >= len(mine.prize)
            ):
                return None
    return [attack_options[0]]


def _two_prize_basic_play_lethal_conversion(
    obs_dict: dict,
    obs: Observation,
    chosen_action: list[int],
    inherited_owner_at_entry: bool,
) -> list[int] | None:
    """Atomic fail-closed replacement of one optional Basic PLAY."""
    before = _two_prize_policy_state_snapshot()
    try:
        action = _two_prize_basic_play_lethal_conversion_unsafe(
...
def agent(obs_dict: dict) -> list[int]:
    ...
    two_prize_lethal_action = _two_prize_basic_play_lethal_conversion(
        obs_dict,
        obs,
        chosen_action,
        inherited_owner_at_entry,
    )
    if two_prize_lethal_action is not None:
        return _remember_action(decision_signature, two_prize_lethal_action)
