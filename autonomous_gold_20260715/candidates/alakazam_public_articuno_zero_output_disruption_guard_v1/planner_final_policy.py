...
import planner_articuno_zero_output_guard as articuno_guard
...
def _arbitrate_new_plan(parent: Any, obs: Any, snap: Any, parent_action: list[int], parent_pre: dict[str, Any], parent_post: dict[str, Any]):
    ...
    if not owners:
        articuno_certificate = articuno_guard.certificate(
            parent, obs, snap, parent_action, parent_pre, parent_post
        )
        if articuno_certificate is not None:
            articuno_data, articuno_action = articuno_certificate
            articuno = articuno_guard.build_plan(
                parent,
                obs,
                snap,
                parent_action,
...
def _advance_transaction(parent: Any, obs: Any):
    transaction = core.INTEGRATED_TRANSACTION
    if transaction is not None and transaction.get("kind") == articuno_guard.KIND:
        return articuno_guard.advance(parent, obs, transaction)
...
def agent(parent: Any, parent_agent: Any, obs_dict: dict) -> list[int]:
    ...
    articuno_guard.finalize_return(action)
    return action
