...
def alakazam_rock_active_guard_index(obs) -> int | None:
    """Return the legal Rock attachment to an exposed main-line Active."""
    active = active_pokemon(obs)
    target = opponent_active(obs)
    if (
        obs.select.context != SelectContext.MAIN
        or getattr(active, "id", None) not in MAIN_LINE
        or getattr(target, "id", None) not in {KADABRA, ALAKAZAM}
        or not lacks_rock_energy(active)
    ):
        return None
    ...
    for index, option in enumerate(getattr(obs.select, "option", None) or []):
        if (
            ...
            and getattr(option_card(obs, option), "id", None) == ROCK_FIGHTING
...
def is_generic_bench_energy_preload(obs, option, reason: str) -> bool:
    """Identify only the ordinary v75 Bench-preload choices this guard may replace."""
    return bool(
        option.type == OptionType.ATTACH
        and option.inPlayArea == AreaType.BENCH
        and getattr(option_card(obs, option), "id", None) in ENERGIES
        and getattr(option_target(obs, option), "id", None) in MAIN_LINE
        and reason in {
            "preload bench Garchomp",
            "preload bench Gabite",
            "preload bench Gible",
            "preload Gabite",
            "preload Gible",
        }
...
def choose_options(obs):
    ...
    if route_index is not None:
        return [route_index]
    rock_guard_index = alakazam_rock_active_guard_index(obs)
    ...
    if obs.select.context == SelectContext.MAIN:
        ...
        if bridge_index is not None:
            return [bridge_index]
        if rock_guard_index is not None:
            _score, normal_index, normal_reason = max(scored, key=lambda row: (row[0], -row[1]))
            if is_generic_bench_energy_preload(obs, obs.select.option[normal_index], normal_reason):
                return [rock_guard_index]
