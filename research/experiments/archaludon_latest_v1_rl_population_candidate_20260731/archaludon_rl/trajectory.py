...
TRAJECTORY_SCHEMA_VERSION = "trajectory-v3"
MANIFEST_SCHEMA_VERSION = "run-manifest-v3"
...
class RunManifest:
    ...
    def finalize(
        ...
        dataset_hash = dataset_sha256(self.collection_spec_sha256, ordered_rows)
        return replace(
            self,
    ...
    return tuple(
        {
            ...
            "legal_option_identities": [
                option.get("identity")
                for option in row.get("legal_semantic_options", ())
            ],
...
def compare_duplicate_traces(
    ...
    return {
        "equal": identity_equal and not mismatches,
