"""Exact Historical-Silver plus isolated Rules 1, 4, 5, and 9.
...
_RULE9_ID = "PARENT_GEAR_EXACT_LAST_PRIZE_BOSS_CONTINUATION_V1"
_DURALUDON = 169
_ARCHALUDON_EX = 190
_CINDERACE = 666
_ARCHALUDON = 840
...
def _card_metadata_exact(card_id):
    ...
    if card_id == _GEAR:
        return bool(
            getattr(data, "name", None) == "Pokégear 3.0"
            and getattr(data, "cardType", None) == _CardType.ITEM
            and len(skills) == 1
            and getattr(skills[0], "name", None) == "Pokégear 3.0"
            and getattr(skills[0], "text", None)
    ...
    if card_id == _EXPLORER:
        return bool(
            getattr(data, "name", None) == "Explorer’s Guidance"
            and getattr(data, "cardType", None) == _CardType.SUPPORTER
            and len(skills) == 1
            and getattr(skills[0], "name", None) == "Explorer’s Guidance"
            and getattr(skills[0], "text", None)
...
def _owner_view(owner):
    ...
    return {
        ...
        "gear_serial": (
            owner.get("gear_ref", (None, None, None))[1]
            if isinstance(owner.get("gear_ref"), tuple)
            else None
        ),
        "boss_serial": (
            owner.get("boss_ref", (None, None, None))[1]
            if isinstance(owner.get("boss_ref"), tuple)
            else None
        ),
...
def _with_ref(refs, selected):
    if not isinstance(refs, tuple) or not isinstance(selected, tuple):
        return None
    return tuple(sorted(refs + (selected,)))


def _rule9_proposal(action, purpose, proof, owner):
    return _make_proposal(
        action,
        purpose,
        proof,
        _owner_view(owner),
        rule_id=_RULE9_ID,
        category="EXACT_GEAR_BOSS_TRANSACTION",
    )


def _single_parent_gear(obs, parent_action, rows):
    if (
        not isinstance(parent_action, list)
        or len(parent_action) != 1
        or not _is_exact_int(parent_action[0])
        ...
        or parent_action[0] >= len(rows)
        or not all(
            _card_metadata_exact(card_id)
            for card_id in (_GEAR, _BOSS, _EXPLORER, _LILLIE)
        )
    ):
        return None
    selected = rows[parent_action[0]]
    ...
    seat = obs.current.yourIndex
    if (
        selected[1].type != _OptionType.PLAY
        or ref is None
        or ref[0] != _GEAR
        or ref[2] != seat
    ):
    ...
    matches = [
        row
        for row in rows
        if row[1].type == _OptionType.PLAY and row[2][1] == ref
    ]
    if hand_refs.count(ref) != 1 or len(matches) != 1:
        return None
    return selected


def _rule9_attack_row(obs, rows):
    attack_rows = [row for row in rows if row[1].type == _OptionType.ATTACK]
    registered = list(_registered_attack_rows(obs, rows))
    if len(attack_rows) != 1 or len(registered) != 1 or attack_rows[0] != registered[0]:
        return None
    selected = registered[0]
    if sum(row[2] == selected[2] for row in rows) != 1:
        return None
    seat = obs.current.yourIndex
    attacker = obs.current.players[seat].active[0]
    energy_refs = _exact_metal_energy_refs(attacker, seat)
    payable = (
        None
        if energy_refs is None
        else _printed_attack_payable_with_metals(attacker.id, len(energy_refs))
    )
    if payable is None or selected[2][5] not in payable:
        return None
    return selected


def _rule9_certificate(obs, rows):
    attack = _rule9_attack_row(obs, rows)
    if attack is None:
        return None
    seat = obs.current.yourIndex
    mine = obs.current.players[seat]
    theirs = obs.current.players[1 - seat]
    attacker = mine.active[0]
    attack_id = attack[2][5]
    current = _exact_damage_and_take(obs, attacker, theirs.active[0], attack_id)
    if current is None:
        return None
    remaining = len(mine.prize)
    if current[2] == remaining:
        return None
    qualifying = []
    bench_results = []
    for index, target in enumerate(theirs.bench):
        result = _exact_damage_and_take(obs, attacker, target, attack_id)
        if result is None:
            return None
        bench_results.append((index, target, result))
        if result[2] == remaining:
            qualifying.append((index, target, result))
    if len(qualifying) != 1:
        return None
    index, target, result = qualifying[0]
    return {
        "attack_row": attack,
        "attack_id": attack_id,
        "attacker": _pokemon_fingerprint(attacker, seat),
        "current_target": _pokemon_fingerprint(theirs.active[0], 1 - seat),
        "current_damage": current[0],
        "current_take": current[2],
        "target_index": index,
        "target": _pokemon_fingerprint(target, 1 - seat),
        "target_serial": target.serial,
        "target_damage": result[0],
        "target_take": result[2],
        "remaining_prize": remaining,
        "bench_results": tuple(
            (bench_index, pokemon.serial, value[0], value[2])
            for bench_index, pokemon, value in bench_results
        ),
    }


def _start_rule9_transaction(obs, parent_action):
    global _materialization_owner
    gates = {
        "normal_main": _exact_main(obs),
        "owner_empty": _materialization_owner is None,
        "setup_inactive": _setup_ledger is None,
        "supporter_unused": getattr(obs.current, "supporterPlayed", None) is False,
    }
    if not all(gates.values()):
        return None, "rule9_entry_gate_failed", gates, False, False
    rows = _option_rows(obs)
    snapshot = _state_snapshot(obs)
    gates["option_bindings_exact"] = rows is not None
    gates["snapshot_exact"] = snapshot is not None
    if rows is None or snapshot is None:
        return None, "rule9_public_binding_failed", gates, False, False
    gear = _single_parent_gear(obs, parent_action, rows)
    gates["parent_exact_bound_gear"] = gear is not None
    if gear is None:
        return None, "rule9_parent_not_exact_bound_gear", gates, False, False
    seat = obs.current.yourIndex
    mine = obs.current.players[seat]
    boss_in_hand = any(card.id == _BOSS for card in mine.hand)
    deck_count = getattr(mine, "deckCount", None)
    gates["boss_absent_from_hand"] = not boss_in_hand
    gates["deck_positive"] = _is_exact_int(deck_count) and deck_count > 0
    if boss_in_hand or not gates["deck_positive"]:
        return None, "rule9_access_or_deck_gate_failed", gates, False, False
    certificate = _rule9_certificate(obs, rows)
    gates["unique_exact_terminal_bench_certificate"] = certificate is not None
    if certificate is None:
        return None, "rule9_terminal_certificate_failed", gates, False, False
    prompt = _prompt_fingerprint(obs)
    if prompt is None:
        return None, "rule9_entry_prompt_inexact", gates, False, False
    gear_ref = gear[2][1]
    owner = {
        "owner": _RULE9_ID,
        "stage": "GEAR_PLAY_EMITTED",
        "route_kind": "PARENT_GEAR_EXACT_LAST_PRIZE_BOSS_CONTINUATION",
        "seat": seat,
        "turn": obs.current.turn,
        "action_count": obs.current.turnActionCount,
        "gear_ref": gear_ref,
        "entry_deck_count": deck_count,
        "reveal_size": min(7, deck_count),
        "attack_id": certificate["attack_id"],
        "attacker": certificate["attacker"],
        "current_target": certificate["current_target"],
        "current_damage": certificate["current_damage"],
        "current_take": certificate["current_take"],
        "target_index": certificate["target_index"],
        "target": certificate["target"],
        "target_serial": certificate["target_serial"],
        "target_damage": certificate["target_damage"],
        "target_take": certificate["target_take"],
        "remaining_prize": certificate["remaining_prize"],
        "bench_results": certificate["bench_results"],
        "start_snapshot": snapshot,
        "last_role": gear[2],
        "last_prompt": prompt,
        "last_order": tuple(row[2] for row in rows),
    ...
    proof = {
        ...
        "current_damage": owner["current_damage"],
        "current_take": owner["current_take"],
        "target": owner["target"],
        "target_damage": owner["target_damage"],
        "target_take": owner["target_take"],
        "remaining_prize": owner["remaining_prize"],
        "entry_deck_count": deck_count,
        "gates": dict(gates),
    }
    return _rule9_proposal(
        parent_action,
...
def _rule9_expected_snapshot(owner, **updates):
    expected = dict(owner["start_snapshot"])
    expected.update(updates)
    return expected


def _rule9_log_moves_exact(logs, expected):
    if len(logs) != len(expected):
        return False
    remaining = list(expected)
    for log in logs:
        matches = []
        for position, (ref, from_area, to_area) in enumerate(remaining):
            if _log_exact(
                log,
                _LogType.MOVE_CARD,
                {
...
def _rule9_reveal(obs, owner):
    seat = owner["seat"]
    start = owner["start_snapshot"]
    base_hand = _without_ref(start["own_hand"], owner["gear_ref"])
    expected = _rule9_expected_snapshot(
        owner,
        action_count=owner["action_count"] + 1,
        own_hand=base_hand,
        own_deck=owner["entry_deck_count"] - owner["reveal_size"],
    )
    snapshot = _state_snapshot(obs)
    effect_ref = _exact_card_ref(getattr(obs.select, "effect", None), seat)
    looking = getattr(obs.current, "looking", None)
    if (
        base_hand is None
        or snapshot is None
        or snapshot != expected
        ...
        or obs.select.maxCount != 1
        or effect_ref != owner["gear_ref"]
        or obs.select.contextCard is not None
        or obs.select.deck is not None
        or not isinstance(looking, list)
        or len(looking) != owner["reveal_size"]
        or len(obs.logs) != owner["reveal_size"] + 1
        or not _log_exact(
            obs.logs[0],
            _LogType.PLAY,
            {
                "playerIndex": seat,
                "cardId": _GEAR,
                "serial": owner["gear_ref"][1],
            },
        )
    ):
        return None
    refs = tuple(_exact_card_ref(card, seat) for card in looking)
    if (
        any(ref is None or not _known_card_metadata(ref[0]) for ref in refs)
        or len(refs) != len(set(refs))
        or not _rule9_log_moves_exact(
            obs.logs[1:],
            tuple((ref, _AreaType.DECK, _AreaType.LOOKING) for ref in refs),
        )
    ):
        return None
    supporters = []
    for ref in refs:
        data = _parent.CARD_DB[ref[0]]
        if data.cardType == _CardType.SUPPORTER:
            if ref[0] not in {_BOSS, _EXPLORER, _LILLIE} or not _card_metadata_exact(
                ref[0]
            ):
                return None
    ...
    option_refs = []
    option_indices = []
    for _position, option, role in rows:
        index = getattr(option, "index", None)
        if (
            option.type != _OptionType.CARD
            or option.area != _AreaType.LOOKING
            or option.playerIndex != seat
            or not _is_exact_int(index)
            or index < 0
            or index >= len(looking)
            or role[1] != refs[index]
        ):
            return None
        option_refs.append(role[1])
        option_indices.append(index)
    if (
        len(option_refs) != len(set(option_refs))
        or len(option_indices) != len(set(option_indices))
        or set(option_refs) != set(supporters)
    ):
        return None
    bosses = sorted((ref for ref in supporters if ref[0] == _BOSS), key=lambda ref: ref[1])
    boss_ref = bosses[0] if bosses else None
    selected = None
    if boss_ref is not None:
        matches = [row for row in rows if row[2][1] == boss_ref]
        if len(matches) != 1:
            return None
        selected = matches[0]
    prompt = _prompt_fingerprint(obs)
...
def _rule9_resolution_logs_exact(obs, owner, boss_ref):
    refs = owner["reveal_refs"]
    moves = []
    for ref in refs:
        destination = _AreaType.HAND if ref == boss_ref else _AreaType.DECK
        moves.append((ref, _AreaType.LOOKING, destination))
    shuffles = [
        log
        for log in obs.logs
        if _log_exact(log, _LogType.SHUFFLE, {"playerIndex": owner["seat"]})
    ]
    movement = [log for log in obs.logs if getattr(log, "type", None) == _LogType.MOVE_CARD]
    return bool(
        len(obs.logs) == len(refs) + 1
        and len(shuffles) == 1
        and len(movement) == len(refs)
        and _rule9_log_moves_exact(movement, tuple(moves))
    )


def _rule9_certificate_matches(obs, owner, rows):
    certificate = _rule9_certificate(obs, rows)
    return bool(
        certificate is not None
        and certificate["attack_id"] == owner["attack_id"]
        and certificate["attacker"] == owner["attacker"]
        and certificate["current_target"] == owner["current_target"]
        and certificate["current_damage"] == owner["current_damage"]
        and certificate["current_take"] == owner["current_take"]
        and certificate["target_index"] == owner["target_index"]
        and certificate["target"] == owner["target"]
        and certificate["target_serial"] == owner["target_serial"]
        and certificate["target_damage"] == owner["target_damage"]
        and certificate["target_take"] == owner["target_take"]
        and certificate["remaining_prize"] == owner["remaining_prize"]
        and certificate["bench_results"] == owner["bench_results"]
    )


def _rule9_post_reveal(obs, owner, boss_ref):
    start = owner["start_snapshot"]
    hand = _without_ref(start["own_hand"], owner["gear_ref"])
    if boss_ref is not None:
        hand = _with_ref(hand, boss_ref)
    discard = start["own_discard"] + (owner["gear_ref"],)
    expected = _rule9_expected_snapshot(
        owner,
        action_count=owner["action_count"] + 1,
        own_hand=hand,
        own_discard=discard,
        own_deck=owner["entry_deck_count"] - (1 if boss_ref is not None else 0),
    )
    snapshot = _state_snapshot(obs)
    if (
        hand is None
        ...
        or snapshot != expected
        or not _exact_main(obs)
        or not _rule9_resolution_logs_exact(obs, owner, boss_ref)
    ):
        return None
    rows = _option_rows(obs)
    if rows is None:
        return None
    if boss_ref is None:
        return {"snapshot": snapshot, "rows": rows, "boss": None}
    if not _rule9_certificate_matches(obs, owner, rows):
        return None
    matches = [
        row
        for row in rows
        if row[1].type == _OptionType.PLAY and row[2][1] == boss_ref
    ]
    if len(matches) != 1 or sum(row[2] == matches[0][2] for row in rows) != 1:
        return None
    return {"snapshot": snapshot, "rows": rows, "boss": matches[0]}


def _rule9_target_prompt(obs, owner):
    post = owner["post_reveal_snapshot"]
    hand = _without_ref(post["own_hand"], owner["boss_ref"])
    expected = dict(post)
    expected.update(
        action_count=owner["action_count"] + 2,
        supporter_played=True,
    ...
    if rows is None:
        return None
    bench_refs = {
        (pokemon.id, pokemon.serial, 1 - seat)
        for pokemon in obs.current.players[1 - seat].bench
    }
    option_refs = []
    matches = []
    for row in rows:
    ...
    if (
        len(option_refs) != len(set(option_refs))
        or set(option_refs) != bench_refs
        or len(matches) != 1
    ):
        return None
    return matches[0], rows, snapshot


def _rule9_attack_prompt(obs, owner):
    post = owner["post_reveal_snapshot"]
    target_index = owner["target_index"]
    bench = list(post["opponent_bench"])
    if target_index < 0 or target_index >= len(bench):
        return None
    bench[target_index] = post["opponent_active"][0]
    hand = _without_ref(post["own_hand"], owner["boss_ref"])
    expected = dict(post)
    expected.update(
        action_count=owner["action_count"] + 3,
        supporter_played=True,
        own_hand=hand,
        own_discard=post["own_discard"] + (owner["boss_ref"],),
        opponent_active=(owner["target"],),
    ...
    if (
        ...
        return None
    rows = _option_rows(obs)
    attack = _rule9_attack_row(obs, rows) if rows is not None else None
    if attack is None or attack[2][5] != owner["attack_id"]:
        return None
    attacker = obs.current.players[seat].active[0]
    target = obs.current.players[1 - seat].active[0]
    result = _exact_damage_and_take(obs, attacker, target, owner["attack_id"])
    if (
        _pokemon_fingerprint(attacker, seat) != owner["attacker"]
        or _pokemon_fingerprint(target, 1 - seat) != owner["target"]
        or result is None
        or result[0] != owner["target_damage"]
        or result[2] != owner["target_take"]
        or result[2] != owner["remaining_prize"]
    ):
        return None
    return attack, rows


def _rule9_abort(reason, proof=None, duplicate=False):
    global _materialization_owner
    owner = _materialization_owner
    details = {} if proof is None else dict(proof)
    details["aborted_stage"] = owner.get("stage") if isinstance(owner, dict) else None
    details["irreversible_abort"] = True
    _materialization_owner = None
    return (
        None,
        "rule9_irreversible_abort:" + reason,
        details,
        duplicate,
        False,
    )
...
def _rule9_retry(obs, owner, purpose):
    prompt = _prompt_fingerprint(obs)
    if prompt is None or prompt != owner.get("last_prompt"):
        return None
    ...
    proof = {
        "attack_id": owner["attack_id"],
        "gear_ref": owner["gear_ref"],
        "boss_ref": owner.get("boss_ref"),
        "target_serial": owner["target_serial"],
        "duplicate_rebind": True,
    }
    return (
        _rule9_proposal(action, purpose, proof, owner),
        None,
        proof,
        True,
        tuple(row[2] for row in rows) != owner.get("last_order"),
...
def _resume_rule9(obs):
    global _materialization_owner
    owner = _materialization_owner
    if not isinstance(owner, dict) or owner.get("owner") != _RULE9_ID:
        return _rule9_abort("owner_conflict")
    if not all(
        _card_metadata_exact(card_id)
        for card_id in (_GEAR, _BOSS, _EXPLORER, _LILLIE)
    ):
        return _rule9_abort("metadata_drift")
    stage = owner.get("stage")
    retry_purposes = {
        "GEAR_PLAY_EMITTED": "RULE9_PARENT_GEAR_ENTRY_SAME_ACTION",
        "BOSS_PLAY_EMITTED": "RULE9_POST_ACQUISITION_BOSS_PLAY",
        "BOSS_TARGET_EMITTED": "RULE9_BOUND_BOSS_TARGET",
        "ATTACK_EMITTED": "RULE9_BOUND_SAME_ATTACK",
    }
    if stage in retry_purposes:
        retry = _rule9_retry(obs, owner, retry_purposes[stage])
        if retry is False:
            return _rule9_abort("duplicate_semantic_role", duplicate=True)
        if retry is not None:
            return retry
    if stage == "GEAR_PLAY_EMITTED":
        reveal = _rule9_reveal(obs, owner)
        if reveal is None:
            return _rule9_abort("gear_reveal_confirmation_failed")
        owner["reveal_refs"] = reveal["refs"]
        owner["reveal_supporters"] = reveal["supporters"]
        owner["reveal_prompt"] = reveal["prompt"]
        owner["last_prompt"] = reveal["prompt"]
        owner["last_order"] = tuple(row[2] for row in reveal["rows"])
        owner["boss_ref"] = reveal["boss_ref"]
        if reveal["boss_ref"] is None:
            owner["stage"] = "MISS_EMPTY_EMITTED"
            owner["last_role"] = None
            action = []
            purpose = "RULE9_REVEAL_UNSUPPORTED_EMPTY"
        else:
            owner["stage"] = "GEAR_HIT_EMITTED"
            owner["last_role"] = reveal["selected"][2]
            action = [reveal["selected"][0]]
            purpose = "RULE9_REVEAL_BOUND_BOSS"
        proof = {
            "gear_ref": owner["gear_ref"],
            "reveal_refs": owner["reveal_refs"],
            "reveal_supporters": owner["reveal_supporters"],
            "boss_ref": owner["boss_ref"],
            "attack_id": owner["attack_id"],
            "target_serial": owner["target_serial"],
        }
        return _rule9_proposal(action, purpose, proof, owner), None, proof, False, False
    if stage in {"GEAR_HIT_EMITTED", "MISS_EMPTY_EMITTED"}:
        reveal = _rule9_reveal(obs, owner)
        if reveal is not None and reveal["prompt"] == owner.get("reveal_prompt"):
            if (
                reveal["refs"] != owner.get("reveal_refs")
                and tuple(sorted(reveal["refs"]))
                != tuple(sorted(owner.get("reveal_refs", ())))
            ):
                return _rule9_abort("reveal_identity_drift", duplicate=True)
            if reveal["boss_ref"] != owner.get("boss_ref"):
                return _rule9_abort("reveal_boss_drift", duplicate=True)
            if stage == "MISS_EMPTY_EMITTED":
                action = []
                purpose = "RULE9_REVEAL_UNSUPPORTED_EMPTY"
            else:
                selected = reveal["selected"]
                if selected is None or selected[2] != owner.get("last_role"):
                    return _rule9_abort("reveal_duplicate_rebind_failed", duplicate=True)
                action = [selected[0]]
                purpose = "RULE9_REVEAL_BOUND_BOSS"
            permuted = tuple(row[2] for row in reveal["rows"]) != owner.get("last_order")
            proof = {
                "gear_ref": owner["gear_ref"],
                "reveal_refs": owner["reveal_refs"],
                "boss_ref": owner.get("boss_ref"),
            ...
            return _rule9_proposal(action, purpose, proof, owner), None, proof, True, permuted
        post = _rule9_post_reveal(obs, owner, owner.get("boss_ref"))
        if post is None:
            return _rule9_abort("post_reveal_ledger_failed")
        if stage == "MISS_EMPTY_EMITTED":
            _materialization_owner = None
            proof = {
                "gear_ref": owner["gear_ref"],
            ...
            return None, "rule9_miss_confirmed", proof, False, False
        boss = post["boss"]
        owner["post_reveal_snapshot"] = post["snapshot"]
        owner["stage"] = "BOSS_PLAY_EMITTED"
        owner["last_role"] = boss[2]
        owner["last_prompt"] = _prompt_fingerprint(obs)
        owner["last_order"] = tuple(row[2] for row in post["rows"])
        proof = {
            "gear_ref": owner["gear_ref"],
            "boss_ref": owner["boss_ref"],
            "attack_id": owner["attack_id"],
            "target_serial": owner["target_serial"],
            "post_reveal_ledger": True,
        }
        return _rule9_proposal(
            [boss[0]],
    ...
    if stage == "BOSS_PLAY_EMITTED":
        target = _rule9_target_prompt(obs, owner)
        if target is None:
            return _rule9_abort("boss_play_confirmation_failed")
        selected, rows, _snapshot = target
        owner["stage"] = "BOSS_TARGET_EMITTED"
        owner["last_role"] = selected[2]
        owner["last_prompt"] = _prompt_fingerprint(obs)
        owner["last_order"] = tuple(row[2] for row in rows)
        proof = {
        ...
        return _rule9_proposal(
            [selected[0]], "RULE9_BOUND_BOSS_TARGET", proof, owner
        ), None, proof, False, False
    if stage == "BOSS_TARGET_EMITTED":
        attack = _rule9_attack_prompt(obs, owner)
        if attack is None:
            return _rule9_abort("boss_target_confirmation_failed")
        selected, rows = attack
        ...
        proof = {
            ...
            "attacker": owner["attacker"],
            "target": owner["target"],
            "damage": owner["target_damage"],
            "take": owner["target_take"],
            "remaining_prize": owner["remaining_prize"],
        }
        return _rule9_proposal(
            [selected[0]], "RULE9_BOUND_SAME_ATTACK", proof, owner
        ), None, proof, False, False
    if stage == "ATTACK_EMITTED":
        ...
        if (
            ...
            return _rule9_abort("terminal_attack_confirmation_failed")
        _materialization_owner = None
        return None, "rule9_attack_dispatched", {"matching_attack": True}, False, False
    return _rule9_abort("owner_stage_unknown")
...
def _resolve(obs, parent_action):
    ...
    if _materialization_owner is not None:
        ...
        if (
            isinstance(_materialization_owner, dict)
            and _materialization_owner.get("owner") == _RULE9_ID
        ):
            return _resume_rule9(obs)
    ...
    if context == _SelectContext.MAIN:
        ...
        if boss[0] is not None:
            return boss
        gear = _start_rule9_transaction(obs, parent_action)
        if gear[0] is not None:
            return gear
        return gear
...
def agent(obs_dict):
    """Call exact Silver once, then resolve Rule 1, Rule 4, Rule 5, or Rule 9."""
