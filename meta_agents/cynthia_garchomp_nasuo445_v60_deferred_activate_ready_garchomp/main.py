...
def activate_ready_garchomp_index(obs) -> int | None:
    """Move an energized benched Garchomp active from a support-role Active."""
    if obs.select.context != SelectContext.MAIN:
        return None

    active = active_pokemon(obs)
    target = opponent_active(obs)
    if getattr(active, "id", None) not in {ROSELIA, ROSERADE, SPIRITOMB} or target is None:
        return None
    if not any(
        getattr(pokemon, "id", None) == GARCHOMP_EX and energy_count(pokemon) >= 1
        for pokemon in (getattr(me(obs), "bench", None) or [])
        if pokemon
    ):
        return None
    if any(
        ...
        and best_damage_for_active(obs, getattr(option, "attackId", None)) >= hp(target)
        for option in (getattr(obs.select, "option", None) or [])
    ):
        return None
    ...
    for index, option in enumerate(options):
        if (
            ...
            and getattr(option_card(obs, option), "id", None) == SURFER
        ):
            return index
    for index, option in enumerate(options):
    ...
    card_data = CARD_DB.get(getattr(active, "id", None))
    raw_retreat_cost = getattr(card_data, "retreatCost", getattr(active, "retreatCost", None))
    try:
        retreat_cost = int(raw_retreat_cost)
    except (TypeError, ValueError):
        return None
    attached = energy_count(active)
    if not attached < retreat_cost <= attached + 1:
        return None

    eligible = []
    for index, option in enumerate(options):
        if option.type != OptionType.ATTACH or option.inPlayArea != AreaType.ACTIVE:
            continue
        energy_id = getattr(option_card(obs, option), "id", None)
        if energy_id in ENERGIES:
            eligible.append((0 if energy_id == BASIC_FIGHTING else 1, index))
    return min(eligible)[1] if eligible else None


def choose_options(obs):
    activation_index = activate_ready_garchomp_index(obs)
