...
MODEL_SCHEMA_VERSION = "residual-actor-critic-v3"
...
def save_checkpoint(
    ...
    *,
    ...
    # Passing a random temporary pathname directly to torch.save leaks that
    # basename into the ZIP member prefix.  A file-like target always uses the
    # stable "archive" prefix, so identical semantic payloads are byte-identical.
...
def _validate_metadata(
    ...
    *,
    ...
    if not isinstance(behavior_receipt, Mapping):
        raise ValueError("checkpoint behavior-policy receipt missing")
    validate_behavior_policy_identity(
        behavior_receipt,
        metadata.get("behavior_policy_schema_sha256"),
    )
