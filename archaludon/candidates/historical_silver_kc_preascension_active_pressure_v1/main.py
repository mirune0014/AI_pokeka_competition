...
def _kc_preascension_active_pressure(obs, opt):
    if detect_matchup(obs) != "crustle" or obs.select.context != SelectContext.MAIN:
        return False
    stadium = list(obs.current.stadium or [])
    if len(stadium) != 1 or stadium[0].id != FULL_METAL_LAB:
        return False
    opponent = opp_active_pokemon(obs)
    if (
        opponent is None
        ...
        or not any(
            card and card.id == 14
            for card in (getattr(opponent, "energyCards", None) or [])
        )
    ):
        return False
    opposing_bench = opp_bench_pokemon(obs)
    if not any(
        pokemon.id == 344
        and pokemon.hp > 0
        and effective_damage(120, pokemon) >= pokemon.hp
        for pokemon in opposing_bench
    ):
        return False
    active = active_pokemon(obs)
    if (
        ...
        or getattr(active, "appearThisTurn", True)
        or getattr(active, "maxHp", None) != 130
    ):
        return False
    active_energy = energy_count(active)
    if (
        active_energy not in {0, 1, 2}
        or damage_on(active) != 20 * active_energy
        or active.hp != 130 - 20 * active_energy
    ):
        return False
    own_bench = [pokemon for pokemon in my_state(obs).bench if pokemon]
    if not any(
        pokemon.id == DURALUDON
        and not getattr(pokemon, "appearThisTurn", True)
        and getattr(pokemon, "maxHp", None) == 130
        and energy_count(pokemon) == 1
        and pokemon.hp == 110
        for pokemon in own_bench
    ):
        return False
    if any(
        pokemon.id in {DURALUDON, ARCHALUDON, ARCHALUDON_EX}
        and energy_count(pokemon) >= 3
        for pokemon in own_bench
    ):
        return False
    if has_in_play(obs, CINDERACE):
        return False
    ids = hand_ids(obs)
    if ARCHALUDON not in ids or BOSS not in ids:
        return False
    legal_attacks = [
    ...
    if any(
        effective_damage(best_attack_damage(obs, attack.attackId), target)
        >= target.hp
        for attack in legal_attacks
        for target in [opponent] + opposing_bench
    ):
        return False
    ...
    return (
        ...
        and getattr(opt, "inPlayIndex", None) == 0
...
def apply_overrides(obs, opt, score, reason):
    ...
    if opt.type == OptionType.ATTACH:
        ...
        if _kc_preascension_active_pressure(obs, opt):
            return 30000, "KC: rebuild Active for pre-Ascension Coated line"
