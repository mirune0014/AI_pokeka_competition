...
def _observation_fingerprint(raw: dict[str, Any]) -> str | None:
    """Fingerprint only callback-visible, C2-permitted state."""
    try:
        current = raw["current"]
    ...
    *,
    ...
    return {
        ...
        "observation_fingerprint": (
            _observation_fingerprint(raw) if isinstance(raw, dict) else None
        ),
        "route_rows": [],
...
def analyze(
    ...
    *,
    ...
    try:
        ...
        transaction_reasons = []
        if isinstance(transaction_state, dict) and (
            transaction_state.get("integrated_transaction_active")
            or transaction_state.get("v1_transaction_active")
        ):
