...
def order_champions_call_before_attacks(obs, scored: list[tuple[int, int, str]]) -> list[tuple[int, int, str]]:
    """Move only attacks that precede the next legal Call behind that Call."""
    if obs.select.context != SelectContext.MAIN or deck_count(obs) < 7:
        return scored
    call_position = next(
        (
            position
            for position, (_score, index, _reason) in enumerate(scored)
            if obs.select.option[index].type == OptionType.ABILITY
            and getattr(option_card(obs, obs.select.option[index]), "id", None) == GABITE
        ),
        None,
    )
    if call_position is None:
        return scored
    attacks_before_call = [
        row for row in scored[:call_position]
        if obs.select.option[row[1]].type == OptionType.ATTACK
    ]
    if not attacks_before_call:
        return scored
    non_attacks_before_call = [
        row for row in scored[:call_position]
        if obs.select.option[row[1]].type != OptionType.ATTACK
    ]
    return (
        non_attacks_before_call
        + [scored[call_position]]
        + attacks_before_call
        + scored[call_position + 1:]
    )
...
def choose_options(obs):
    ...
    scored = order_champions_call_before_attacks(obs, scored)
