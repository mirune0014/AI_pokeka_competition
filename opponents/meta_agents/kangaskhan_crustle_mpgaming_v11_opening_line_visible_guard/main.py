...
def opening_line_visible_guard(opp):
    """Block the proactive opening attach for measured public matchup markers."""
    opponent_active = active(opp)
    visible_ids = {p.id for p in field(opp)}
    return bool(visible_ids.intersection((305, 741, 675))
                or (opponent_active is not None and opponent_active.id == 116))
...
def opening_line_attach_score(obs, me, opp, opt):
    ...
    if not (len(me.prize) == len(opp.prize) == 6
            ...
            and not attacks
            and not opening_line_visible_guard(opp)
            and bench_line
            and target):
        return 0
