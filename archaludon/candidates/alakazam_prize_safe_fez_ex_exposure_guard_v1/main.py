...
def _parent_agent(obs_dict: dict) -> list[int]:
...
# Isolated child overlay: never expose a fresh two-Prize Fezandipiti ex when
# the opponent's complete public Active board already certifies a payable
# fixed 210+ attack.  The exact frozen parent above always acts first.
_FEZ_EXPOSURE_GUARD_VERSION = "prize_safe_fez_exposure_guard_v1"
_MAXIMUM_BELT = 1158
_fez_exposure_last_observation = None
_fez_exposure_last_action = None


def _clear_fez_exposure_cache() -> None:
    global _fez_exposure_last_observation, _fez_exposure_last_action
    _fez_exposure_last_observation = None
    _fez_exposure_last_action = None


def _remember_fez_exposure_action(
    observation: dict, action: list[int]
) -> list[int]:
    global _fez_exposure_last_observation, _fez_exposure_last_action
    _fez_exposure_last_observation = copy.deepcopy(observation)
    _fez_exposure_last_action = tuple(action)
    return list(action)


def _fez_complete_public_board(obs: Observation) -> dict | None:
    """Certify the exact public in-play rows used by this one guard."""
    state = obs.current
    if (
        state is None
        or not isinstance(state.yourIndex, int)
    ...
    for owner, player in enumerate(state.players):
        if (
            len(player.active) != 1
            or player.active[0] is None
            or not isinstance(player.benchMax, int)
            or player.benchMax < 0
            or len(player.bench) > player.benchMax
        ):
            return None
        pokemon_rows = list(player.active) + list(player.bench)
        for pokemon in pokemon_rows:
            ...
            if len(pokemon.tools) > 1:
                return None
            for cards, allowed in (
                (pokemon.preEvolution, {CardType.POKEMON}),
                (
                    pokemon.energyCards,
                    {CardType.BASIC_ENERGY, CardType.SPECIAL_ENERGY},
                ),
                (pokemon.tools, {CardType.TOOL}),
            ):
                for card in cards:
                    card_data = card_table.get(card.id)
                    if card_data is None or card_data.cardType not in allowed:
                        return None
            components = _bridge_pokemon_component_serials(pokemon)
            protected_serials.extend(components)
            rows.append((owner, pokemon, data))
    ...
    for stadium in state.stadium:
        ...
        if (
            ...
            or stadium.id <= 0
            or stadium.serial <= 0
            or getattr(stadium, "playerIndex", None) not in (0, 1)
        ):
            return None
        protected_serials.append(stadium.serial)

    if (
        len(protected_serials) != len(set(protected_serials))
        or not _bridge_protected_serials_are_unique(
            state, protected_serials
        )
    ):
        return None

    own_rows = [pokemon for owner, pokemon, _ in rows if owner == my_index]
    if not own_rows or any(
        pokemon.id == Fezandipiti_ex or prize_count(pokemon) != 1
        for pokemon in own_rows
    ):
        return None

    opponent_active = theirs.active[0]
    return {
        "version": _FEZ_EXPOSURE_GUARD_VERSION,
        "player": my_index,
        "opponent": 1 - my_index,
        "opponent_active": opponent_active,
        "rows": tuple(rows),
        "statuses": statuses,
        "protected_serials": tuple(protected_serials),
    }


def _fez_visible_modifier_bonus(
    obs: Observation,
    *,
    attacker: Pokemon,
    attacker_owner: int,
    ...
    """Return recognized before-W/R damage, otherwise fail closed."""
    state = obs.current
    bonus = 0
    for owner, player in enumerate(state.players):
        for pokemon in list(player.active) + list(player.bench):
            ...
            for energy in pokemon.energyCards:
                ...
                if (
                    energy_data is None
                    or _bridge_retaliation_energy_unit(energy) is None
                    or not all(
                        _bridge_retaliation_skill_is_certified_irrelevant(
                            skill.text, source_kind="energy"
                        )
            ...
            for tool in pokemon.tools:
                ...
                if tool_data is None:
                    return None
                if tool.id == _MAXIMUM_BELT:
                    if (
                        owner == attacker_owner
                        and pokemon.serial == attacker.serial
                        and bool(target_data.ex)
                    ):
                        bonus += 50
                    continue
                if not all(
                    _bridge_retaliation_skill_is_certified_irrelevant(
                        skill.text, source_kind="tool"
                    )
                    for skill in (tool_data.skills or [])
                ):
                    return None
...
def _fez_fixed_attack_damage(
    ...
    *,
    ...
    """Certify fixed immediate damage with one recognized visible modifier."""
    attacker_data = card_table.get(attacker.id)
    if attacker_data is None:
        return None
    base_profile, failure = _bridge_retaliation_attack_damage(
        attacker_data, attack, target_data
    )
    if base_profile is None or failure is not None:
        return None
    bonus = _fez_visible_modifier_bonus(
        obs,
        attacker=attacker,
    ...
    damage = int(attack.damage) + bonus
    steps = [("printed", int(attack.damage))]
    if bonus:
        steps.append(("maximum_belt_before_wr", damage))
    attack_type = int(attacker_data.energyType)
    weakness = (
        None if target_data.weakness is None else int(target_data.weakness)
    )
    resistance = (
        None
        if target_data.resistance is None
        else int(target_data.resistance)
    )
    if not base_profile["ignores_weakness_resistance"]:
        if weakness == attack_type:
            damage *= 2
            steps.append(("weakness_x2", damage))
        if resistance == attack_type:
    ...
    return {
        "printed_damage": int(attack.damage),
        "recognized_before_wr_bonus": bonus,
        "attack_type": attack_type,
        "weakness": weakness,
        "resistance": resistance,
        "ignores_weakness_resistance": base_profile[
...
def _fez_exposure_certificate(obs: Observation) -> dict | None:
    """Certify a status-ready payable fixed 210+ hit into bare Fez ex."""
    board = _fez_complete_public_board(obs)
    if board is None:
        return None
    state = obs.current
    opponent = state.players[board["opponent"]]
    if opponent.asleep or opponent.paralyzed or opponent.confused:
        return None
    attacker = board["opponent_active"]
    attacker_data = card_table.get(attacker.id)
    fez_data = card_table.get(Fezandipiti_ex)
    if (
        attacker_data is None
        or fez_data is None
        or fez_data.cardType != CardType.POKEMON
        or not fez_data.basic
        or not fez_data.ex
        or fez_data.hp != 210
        or not isinstance(attacker_data.attacks, list)
        or not attacker_data.attacks
        or len(attacker_data.attacks) != len(set(attacker_data.attacks))
    ):
        return None
    energy_units = _bridge_retaliation_energy_units(attacker)
    ...
    for attack_id in attacker_data.attacks:
        ...
        if not payable:
            continue
        damage = _fez_fixed_attack_damage(
            obs,
            attacker=attacker,
            attacker_owner=board["opponent"],
            attack=attack,
            target_data=fez_data,
        )
        # One currently payable variable/conditional or modifier-ambiguous
        # attack makes the whole public threat state ambiguous.
        if damage is None:
            return None
        payable_profiles.append((attack_id, cost, damage))
    threats = [
        profile
        for profile in payable_profiles
        if profile[2]["certified_damage"] >= fez_data.hp
    ]
    if not threats:
        return None
    return {
        **board,
        "fez_hp": fez_data.hp,
        "payable_attacks": tuple(payable_profiles),
        "threats": tuple(threats),
    }


def _fez_option_signature(option) -> tuple:
    return (
        int(option.type),
        option.number,
        int(option.area) if option.area is not None else None,
...
def _fez_maskable_indices(obs: Observation) -> list[int] | None:
    """Return only legal Fez search/PLAY indices; None means ambiguity."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    signatures = [_fez_option_signature(option) for option in select.option]
    if len(signatures) != len(set(signatures)):
        return None

    if select.context in (SelectContext.TO_HAND, SelectContext.TO_BENCH):
        ...
        if (
            ...
            or effect.id <= 0
            or effect.serial <= 0
            or effect.playerIndex != my_index
        ):
            return [] if select.minCount != 0 else None
        indices = []
        option_card_indices = []
        option_card_serials = []
        for option_index, option in enumerate(select.option):
            if (
                ...
                return None
            card = _safe_get_card(
                obs, AreaType.DECK, option.index, my_index
            )
            if (
                card is None
                or card.id <= 0
                ...
                return None
            option_card_indices.append(option.index)
            option_card_serials.append(card.serial)
            if card.id == Fezandipiti_ex:
                indices.append(option_index)
        if (
            len(option_card_indices) != len(set(option_card_indices))
            or len(option_card_serials) != len(set(option_card_serials))
        ):
            return None
        return indices
    ...
    for option_index, option in enumerate(select.option):
        ...
        if (
            ...
            return None
        play_hand_indices.append(option.index)
        if card.id == Fezandipiti_ex:
            fez_indices.append(option_index)
    if len(play_hand_indices) != len(set(play_hand_indices)):
        return None
    return fez_indices


def _fez_enabled_terminal_certificate(obs: Observation) -> dict | None:
    """No exemption: Fez Ability eligibility is not fully public here."""
    # A currently legal lethal attack does not make PLAY-Fez necessary and is
    # not a Fez-enabled transaction.  Flip the Script eligibility and the
    # complete PLAY -> Ability -> attack sequence are not certified by one
    # observation callback, so v1 intentionally has no terminal exemption.
    return None


def _fez_terminal_looking_ambiguous(obs: Observation) -> bool:
    """Catch Fez-draw Powerful Hand wins that this overlay cannot prove."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if (
        ...
        or mine.confused
        or mine.deckCount <= 0
        or _reserve_attack_is_payable(mine.active[0]) is not True
    ):
        return False
    target = theirs.active[0]
    try:
        if prize_count(target) < len(mine.prize):
            return False
    ...
    projected = (mine.handCount + contribution) * 20
    ...
    if target_data.resistance == EnergyType.PSYCHIC:
        projected = max(0, projected - 30)
    return projected >= target.hp


def _rerun_parent_without_fez(
    obs_dict: dict, masked_indices: list[int]
) -> list[int] | None:
    """Re-run the frozen parent with only the certified Fez options absent."""
    ...
    if not isinstance(raw_options, list):
        return None
    index_map = [
        index for index in range(len(raw_options)) if index not in masked
    ]
    raw_select["option"] = [raw_options[index] for index in index_map]
    original_min = raw_select.get("minCount")
    original_max = raw_select.get("maxCount")
    if not isinstance(original_min, int) or not isinstance(original_max, int):
        return None
    raw_select["maxCount"] = min(original_max, len(index_map))
    if raw_select["maxCount"] < original_min:
        return None

    reranked_action = _parent_agent(reranked)
    if (
        not isinstance(reranked_action, list)
        or any(
            not isinstance(index, int)
            or index < 0
            or index >= len(index_map)
            for index in reranked_action
        )
        or len(reranked_action) < original_min
        or len(reranked_action) > original_max
    ):
        return None
    mapped = [index_map[index] for index in reranked_action]
    if any(index in masked for index in mapped):
        return None
    # Optional selection may return zero only when Fez was the sole legal
    # option left after masking.
    if not mapped and index_map:
        return None
    return mapped


def _snapshot_parent_policy_state() -> dict:
    """Copy every mutable exact-parent policy global before an overlay run."""
    return {
        "pre_turn": pre_turn,
        ...
        "fez_bridge": copy.deepcopy(_fez_ko_bridge_latch),
        "active_psychic": copy.deepcopy(_active_psychic_ko_latch),
        "stranded": copy.deepcopy(_stranded_retreat_ko_latch),
        "resource_first": copy.deepcopy(_kadabra_resource_first_latch),
        "terminal": copy.deepcopy(_reserve_terminal_win_latch),
        "decision_signature": copy.deepcopy(_last_decision_signature),
        "decision_action": copy.deepcopy(_last_decision_action),
    }


def _restore_parent_policy_state(snapshot: dict) -> None:
    """Restore one exact-parent snapshot without replacing latch objects."""
    global pre_turn, ability_used_dudunsparce, ability_used_fezandipiti
    global _last_decision_signature, _last_decision_action
    ...
    for latch, key in (
        ...
        (_fez_ko_bridge_latch, "fez_bridge"),
        (_active_psychic_ko_latch, "active_psychic"),
        (_stranded_retreat_ko_latch, "stranded"),
        (_kadabra_resource_first_latch, "resource_first"),
        (_reserve_terminal_win_latch, "terminal"),
    ):
        latch.clear()
...
def agent(obs_dict: dict) -> list[int]:
    """Exact parent first, then the isolated public Fez exposure overlay."""
    if (
        _fez_exposure_last_observation is not None
        and obs_dict == _fez_exposure_last_observation
        and _fez_exposure_last_action is not None
    ):
        return list(_fez_exposure_last_action)

    before_parent = _snapshot_parent_policy_state()
    parent_action = _parent_agent(copy.deepcopy(obs_dict))
    after_parent = _snapshot_parent_policy_state()
    if not isinstance(obs_dict, dict) or obs_dict.get("select") is None:
        return _remember_fez_exposure_action(obs_dict, parent_action)
    try:
        obs = to_observation_class(copy.deepcopy(obs_dict))
        certificate = _fez_exposure_certificate(obs)
        masked_indices = (
            _fez_maskable_indices(obs) if certificate is not None else []
        )
    except (AttributeError, IndexError, KeyError, TypeError, ValueError):
        return _remember_fez_exposure_action(obs_dict, parent_action)
    if masked_indices is None or not masked_indices:
        return _remember_fez_exposure_action(obs_dict, parent_action)
    if (
        any(
            not isinstance(index, int)
        ...
        or not any(index in set(masked_indices) for index in parent_action)
    ):
        return _remember_fez_exposure_action(obs_dict, parent_action)

    # Never suppress an already certified final attack, and delegate all
    # Powerful-Hand terminal-looking cases whose Fez Ability route cannot be
    # completely certified from this callback's public state.
    if (
        _fez_enabled_terminal_certificate(obs) is not None
        or _fez_terminal_looking_ambiguous(obs)
    ):
        return _remember_fez_exposure_action(obs_dict, parent_action)

    # The masked run must behave as if the exact parent saw only the masked
    # legal-option set once.  Remove every state mutation from its first Fez
    # choice before reranking.
    _restore_parent_policy_state(before_parent)
    replacement = _rerun_parent_without_fez(obs_dict, masked_indices)
    if replacement is None:
        # Failed/ambiguous overlay: return both the exact action and the exact
        # post-action parent state from the first, unmasked computation.
        _restore_parent_policy_state(after_parent)
        return _remember_fez_exposure_action(obs_dict, parent_action)
    return _remember_fez_exposure_action(obs_dict, replacement)
