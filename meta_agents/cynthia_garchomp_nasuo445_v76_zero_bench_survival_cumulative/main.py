...
def zero_bench_search_survival_index(obs) -> int | None:
    """Select a benchable Basic when a guarded search is the only board out."""
    effect_id = getattr(getattr(obs.select, "effect", None), "id", None)
    if (
        obs.select.context != SelectContext.TO_HAND
        or obs.select.maxCount != 1
        or effect_id not in {POKE_PAD, GABITE}
    ):
        return None
    ...
    if (
        ...
        or bench is None
        or hand is None
        or not isinstance(bench_max, int)
    ):
        return None

    occupied_active = [pokemon for pokemon in active if pokemon]
    occupied_bench = [pokemon for pokemon in bench if pokemon]
    if (
        len(occupied_active) != 1
        or occupied_bench
        or len(occupied_bench) >= bench_max
        or any(getattr(card, "id", None) in CYNTHIA_BASICS for card in hand if card)
    ):
        return None
    ...
    if effect_id == GABITE:
        secured_ids = {
            ...
            for card in [*hand, *occupied_active, *occupied_bench]
            if card
        }
        if GARCHOMP_EX not in secured_ids:
            return None

    for basic_id in (GIBLE, ROSELIA, SPIRITOMB):
        for index, option in enumerate(getattr(obs.select, "option", None) or []):
            if (
                option.type == OptionType.CARD
                and getattr(option_card(obs, option), "id", None) == basic_id
            ):
                return index
    return None


def zero_bench_direct_play_survival_index(
    obs,
    scored: list[tuple[int, int, str]],
) -> int | None:
    """Bench a legal Basic before a visible non-KO top attack on an empty board."""
    options = getattr(obs.select, "option", None)
    if obs.select.context != SelectContext.MAIN or not options or not scored:
        return None
    if len(options) != len(scored) or any(reason.startswith("error ") for _score, _index, reason in scored):
        return None
    ...
    bench = getattr(player, "bench", None)
    bench_max = getattr(player, "benchMax", None)
    prizes = getattr(opponent(obs), "prize", None)
    target = opponent_active(obs)
    ...
    legal_attacks = [option for option in options if option.type == OptionType.ATTACK]
    for option in legal_attacks:
        attack_id = getattr(option, "attackId", None)
        if attack_id is None:
            return None
        try:
            attack_damage = best_damage_for_active(obs, attack_id)
        except Exception:
            return None
...
def choose_options(obs):
    ...
    if obs.select.context == SelectContext.MAIN:
        ...
        if development_index is None:
            survival_index = zero_bench_direct_play_survival_index(obs, scored)
            if survival_index is not None:
                return [survival_index]
