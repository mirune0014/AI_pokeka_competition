...
def champions_call_route_index(obs) -> int | None:
    ...
    known_garchomp_count = (
        sum(1 for card in player.hand if card and card.id == GARCHOMP_EX)
        + sum(1 for pokemon in my_pokemon(obs) if pokemon.id == GARCHOMP_EX)
    )
    if (
        known_garchomp_count >= 2
        and not roserade_in_hand
        and not roserade_in_play
