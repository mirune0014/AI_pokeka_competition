...
_sacred_ash_hold = {}
_sacred_ash_release_block = {}
_last_decision_signature = None
_last_decision_action = None

_SACRED_ASH_NAME = "Sacred Ash"
_SACRED_ASH_SKILL_TEXT = (
...
def _clear_sacred_ash_hold() -> None:
    _sacred_ash_hold.clear()


def _clear_sacred_ash_release_block() -> None:
    _sacred_ash_release_block.clear()


def _release_sacred_ash_hold(obs: Observation) -> None:
    """Clear a hold and make its first release turn a true delegation turn."""
    if _sacred_ash_hold:
        owner = _sacred_ash_hold.get("player")
        held_turn = _sacred_ash_hold.get("turn")
        _sacred_ash_release_block.update(
            player=owner,
            held_turn=held_turn,
            block_turn=(
                obs.current.turn
                if obs.current.yourIndex == owner
                else None
            ),
        )
    _clear_sacred_ash_hold()
...
def _clear_emergency_state(*, clear_cache: bool = False) -> None:
    ...
    _clear_sacred_ash_hold()
    _clear_sacred_ash_release_block()
    if clear_cache:
        _clear_decision_cache()


def _parent_transaction_active() -> bool:
    return bool(
        _hilda_source_latch
...
def _sacred_ash_metadata_certified() -> bool:
    data = card_table.get(Sacred_Ash)
    if (
        data is None
        or data.cardType != CardType.ITEM
        or data.name != _SACRED_ASH_NAME
        or len(data.skills) != 1
    ):
        return False
    skill = data.skills[0]
    return (
        getattr(skill, "name", None) == _SACRED_ASH_NAME
        and getattr(skill, "text", None) == _SACRED_ASH_SKILL_TEXT
    )


def _sacred_ash_play_options(obs: Observation) -> list[tuple[int, Card]] | None:
    """Resolve every Ash PLAY by public hand serial, failing closed."""
    result = []
    for option_index, option in enumerate(obs.select.option):
        ...
        try:
            card = get_card(
                obs, AreaType.HAND, option.index, obs.current.yourIndex
            )
        ...
        if card.id == Sacred_Ash:
            if not isinstance(card.serial, int):
                return None
            result.append((option_index, card))
    return result


def _sacred_ash_public_serials_complete(mine) -> bool:
    """Require one unambiguous serial for every visible own-zone card."""
    seen = set()
    ...
    def visit(card) -> bool:
        ...
        if (
            card_table.get(card_id) is None
            or not isinstance(serial, int)
            or serial in seen
        ):
            return False
        seen.add(serial)
        for attribute in ("preEvolution", "energyCards", "tools"):
            for child in getattr(card, attribute, ()) or ():
                if not visit(child):
                    return False
        return True

    roots = []
    for zone_name in ("hand", "discard", "active", "bench"):
        roots.extend(getattr(mine, zone_name, ()) or ())
    return all(visit(card) for card in roots)


def _sacred_ash_public_certificate(
    obs: Observation,
    ash_serial: int,
    *,
    final_prize_route: bool,
    board_out_route: bool,
) -> dict | None:
    """Return the complete public hold certificate, or fail closed."""
    state = obs.current
    select = obs.select
    try:
        ordinary_select = int(select.type) == 0
    except (TypeError, ValueError):
        ordinary_select = False
    if (
        select.context != SelectContext.MAIN
        or not ordinary_select
        ...
        or state.turn < 0
        or state.result not in (None, -1)
        or _parent_transaction_active()
        or final_prize_route
        or board_out_route
        or not _sacred_ash_metadata_certified()
    ):
        return None
    ...
    if (
        ...
        or not _sacred_ash_public_serials_complete(mine)
    ):
        return None

    # The one visible Ash and every returnable Pokemon must have an exact,
    # player-owned serial mapping. Unknown discard metadata is relevant because
    # it makes the legal returnable set unknowable.
    visible_ash = []
    visible_alakazam = False
    for card in mine.hand:
        data = card_table.get(card.id)
        if data is None or not isinstance(card.serial, int):
            return None
        if card.id == Sacred_Ash:
            if card.playerIndex != state.yourIndex:
                return None
            visible_ash.append(card)
        if card.id == Alakazam:
            visible_alakazam = True
    if (
        len(visible_ash) != 1
        or visible_ash[0].serial != ash_serial
    ):
        return None
    ...
    for card in mine.discard:
        ...
        if data.cardType != CardType.POKEMON:
            continue
        if (
            not isinstance(card.serial, int)
            or card.serial in returnable_serials
            or card.playerIndex != state.yourIndex
        ):
            return None
        returnable_serials.add(card.serial)

    board = []
    for card in mine.active:
        if card is not None:
            board.append(card)
    ...
    for pokemon in board:
        ...
        if (
            ...
            or pokemon.serial in board_serials
        ):
            return None
        board_serials.add(pokemon.serial)
        if pokemon.id == Alakazam:
            visible_alakazam = True
        if pokemon.id not in ABRA_LINE:
            continue
        abra_line_bodies += 1
        for energy in pokemon.energyCards:
            if (
                card_table.get(energy.id) is None
                or not isinstance(energy.serial, int)
            ):
                return None
            if energy.id in PSYCHIC_ENERGY_IDS:
                psychic_attached = True

    deck_count = mine.deckCount
    prize_count_ = len(mine.prize)
    reserve = deck_count - prize_count_ - 1
    recovery = min(5, len(returnable_serials))
    if (
        recovery not in (1, 2)
        or reserve <= 5
        or abra_line_bodies < 3
        or not psychic_attached
        or not visible_alakazam
    ):
        return None

    ash_options = _sacred_ash_play_options(obs)
    if ash_options is None or len(ash_options) != 1:
        return None
    option_index, option_card = ash_options[0]
    if option_card.serial != ash_serial:
        return None
    return {
        "player": state.yourIndex,
        "turn": state.turn,
        "ash_serial": ash_serial,
        "ash_option_index": option_index,
        "deck_count": deck_count,
        "prizes": prize_count_,
        "reserve": reserve,
        "returnable_count": len(returnable_serials),
        "recovery": recovery,
        "returnable_serials": tuple(sorted(returnable_serials)),
        "abra_line_bodies": abra_line_bodies,
    }


def _sacred_ash_callback_expected(obs: Observation) -> bool:
    context = obs.select.context
    allowed = _sacred_ash_hold.get("expected_contexts", ())
    if int(context) not in allowed:
        return False
    expected_ids = _sacred_ash_hold.get("expected_card_ids", ())
    expected_serials = _sacred_ash_hold.get("expected_serials", ())
    effect = obs.select.effect
    context_card = obs.select.contextCard
    return bool(
        (effect is not None and effect.id in expected_ids)
        or (
            context_card is not None
            and (
                context_card.id in expected_ids
                or context_card.serial in expected_serials
            )
        )
    )


def _prepare_sacred_ash_hold(obs: Observation) -> None:
    """Clear stale holds before the parent's cached-decision shortcut."""
    if not _sacred_ash_hold:
        return
    state = obs.current
    if (
        state.yourIndex != _sacred_ash_hold.get("player")
        or state.turn != _sacred_ash_hold.get("turn")
    ):
        _release_sacred_ash_hold(obs)
        return
    mine = state.players[state.yourIndex]
    matches = [
        card
        ...
        if card.id == Sacred_Ash
        and card.serial == _sacred_ash_hold.get("ash_serial")
    ]
    if len(matches) != 1:
        _release_sacred_ash_hold(obs)
        return
    if obs.select.context == SelectContext.MAIN:
        if _parent_transaction_active():
            _release_sacred_ash_hold(obs)
        return
    if not _sacred_ash_callback_expected(obs):
        _release_sacred_ash_hold(obs)


def _sacred_ash_filter_final_order(
    obs: Observation,
    ...
    *,
    ...
    """Remove only the certified Ash serial from exact-v3's final order."""
    if obs.select.context != SelectContext.MAIN or not desc_indices:
        return desc_indices
    state = obs.current
    if _sacred_ash_release_block:
        if state.yourIndex == _sacred_ash_release_block.get("player"):
            block_turn = _sacred_ash_release_block.get("block_turn")
            if block_turn is None:
                _sacred_ash_release_block["block_turn"] = state.turn
                return desc_indices
            if block_turn == state.turn:
                return desc_indices
            _clear_sacred_ash_release_block()
    ash_options = _sacred_ash_play_options(obs)
    if ash_options is None or len(ash_options) != 1:
        _release_sacred_ash_hold(obs)
        return desc_indices
    ash_option_index, ash_card = ash_options[0]

    if not _sacred_ash_hold:
        if desc_indices[0] != ash_option_index:
            return desc_indices
        certificate = _sacred_ash_public_certificate(
            obs,
            ash_card.serial,
            final_prize_route=final_prize_route,
            board_out_route=board_out_route,
        )
        if certificate is None:
            return desc_indices
        _sacred_ash_hold.update(certificate)
    else:
        certificate = _sacred_ash_public_certificate(
            obs,
            _sacred_ash_hold.get("ash_serial"),
            final_prize_route=final_prize_route,
            board_out_route=board_out_route,
        )
        if certificate is None:
    ...
    filtered = [index for index in desc_indices if index != ash_option_index]
    if not filtered:
        _release_sacred_ash_hold(obs)
        return desc_indices
    return filtered
...
def _note_sacred_ash_main_choice(
    obs: Observation, action: list[int]
) -> None:
    """Record only the next action's public mandatory-callback identity."""
    if not _sacred_ash_hold or obs.select.context != SelectContext.MAIN:
        return
    ...
    if option.type == OptionType.PLAY:
        ...
        card = get_card(
            obs, option.area, option.index, obs.current.yourIndex
        )
    elif option.type == OptionType.RETREAT:
        expected_contexts.add(int(SelectContext.TO_ACTIVE))

    _sacred_ash_hold["expected_contexts"] = tuple(sorted(expected_contexts))
    _sacred_ash_hold["expected_card_ids"] = tuple(
        sorted({value.id for value in (card, target) if value is not None})
    )
    _sacred_ash_hold["expected_serials"] = tuple(
        sorted(
            {
                value.serial
                for value in (card, target)
                if value is not None and isinstance(value.serial, int)
            }
        )
    )
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # Isolated public Sacred Ash reserve timing. Exact-v3's full stable order is
    # complete above; only its one certified Ash serial can be removed from
    # that order for this player/turn. All downstream parent finalization
    # (transactions, flags, callbacks, and effect choices) remains unchanged.
    opponent_board_count = len(
        [pokemon for pokemon in op_state.active if pokemon is not None]
        + [pokemon for pokemon in op_state.bench if pokemon is not None]
    )
    public_board_out_route = bool(
        target_can_kill and target_idx == 0 and opponent_board_count == 1
    )
    desc_indices = _sacred_ash_filter_final_order(
        obs,
        desc_indices,
        final_prize_route=can_win_this_turn,
        board_out_route=public_board_out_route,
    )
    ...
    if fez_bridge_action is not None:
        _note_sacred_ash_main_choice(obs, fez_bridge_action)
    ...
    if (
        ...
        if transaction_action is not None:
            ...
            _note_sacred_ash_main_choice(obs, transaction_action)
    ...
    if (
        ...
        _note_sacred_ash_main_choice(obs, chosen_action)
