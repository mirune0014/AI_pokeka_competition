...
POFFIN, POKEGEAR, HILDA, LILLIE, XEROSIC, BOSS, SWITCH, HERO_CAPE = 1086, 1122, 1225, 1227, 1197, 1182, 1123, 1159
...
def kang_finish_attach(obs, opt, me):
    if obs.current.supporterPlayed:
        return False
    energy = card(obs, opt.area, opt.index, obs.current.yourIndex)
    target = card(obs, opt.inPlayArea, opt.inPlayIndex, obs.current.yourIndex)
    a = active(me)
    if not (energy and energy.id in ENERGY and target is a and a and a.id == KANGASKHAN and ex(a)):
        return False
    if len(a.energies) != 2 or any(c.id == POFFIN for c in me.hand):
        return False
    if any(
        option.type == OptionType.PLAY
        and (cape := card(obs, AreaType.HAND, option.index, obs.current.yourIndex))
        and cape.id == HERO_CAPE
        for option in obs.select.option
    ):
        return False
    return not (a.hp >= a.maxHp - 10 and count(me, CRUSTLE) == 0)
    ...
    try:
        ...
        for opt in sel.option:
            ...
            if sel.context == SelectContext.MAIN:
                ...
                elif opt.type == OptionType.ATTACH: score = 8001 if kang_finish_attach(obs, opt, me) else 7000
                elif opt.type == OptionType.PLAY:
                    c = card(obs, AreaType.HAND, opt.index, obs.current.yourIndex)
                    score = {POFFIN: 8500, HILDA: 8000, LILLIE: 7000, POKEGEAR: 6000, XEROSIC: 5000, BOSS: 3000}.get(c.id if c else -1, 1000)
