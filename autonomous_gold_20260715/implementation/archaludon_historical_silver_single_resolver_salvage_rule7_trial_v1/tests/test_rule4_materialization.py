...
class Rule4MaterializationTests(unittest.TestCase):
    ...
    def test_rule7_zero_release_does_not_suppress_rule4_same_callback(self):
        pre, _post, expected, _serial = route_fixture(main._ROUTE_DURALUDON, 0)
        main._materialization_owner = {
            "owner": main._RULE7_ID,
            "stage": "ZERO_EMITTED",
            "last_prompt": ("different",),
        }
        self.assertEqual(self.call(pre, [0]), [expected])
        self.assertEqual(main._last_proposal["rule_id"], main._RULE4_ID)
        self.assertEqual(main._materialization_owner["owner"], main._RULE4_ID)
