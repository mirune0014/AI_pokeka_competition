...
# H4_V2_PARENT_ATTACK_ADMISSIBLE_UNIQUE_HIGHER_PRIZE_BOSS_KO transaction.
# It contains only public, serial-bound facts and is reset on every boundary
# or mismatch.  The inherited historical-Silver Attack is only an arming
# witness; H4 retains its own conservative attack arbitration.
...
def _h4_parent_attack_witness(obs, parent_choice):
    """Resolve one cached historical-Silver choice to a public Attack."""
    if (
...
def _h4_legal_attack_options(obs, attack_id=None):
    choices = []
    for position, option in enumerate(obs.select.option):
        if (
            option.type == OptionType.ATTACK
            and option.attackId in _H4_SAFE_ATTACKS
...
def _h4_build_certificate(obs, parent_attack_witness):
    ...
    legal_attacks = _h4_legal_attack_options(obs)
    if (
        attacker is None
    ...
    distinct_attack_ids = sorted({attack_id for _, attack_id in legal_attacks})
    for attack_id in distinct_attack_ids:
        active_damage = _h4_exact_damage(obs, attack_id, opposing_active)
        ...
        for target in opp.bench:
            damage = _h4_exact_damage(obs, attack_id, target)
    ...
    target_candidates = [
        ...
        and item["target_prize"] == maximum_prize
    ]
    chosen = min(
        target_candidates,
        key=lambda item: (-item["damage"], item["attack_id"]),
    )
    ...
    return {
        ...
        "original_active_yield": chosen["current_yield"],
...
def _h4_post_target_valid(obs, transaction):
    ...
    if (
        ...
        or not any(
            card.serial == transaction["original_active"][1]
            for card in (opp.bench or ())
        )
...
def _historical_silver_choose_options(obs):
    scored = []
...
def choose_options(obs):
    # An active transaction has precedence.  If it completes or rolls back,
    # delegate once from the actual observation and never re-arm in the same
    # callback.
    if _h4_transaction is not None:
        h4_choice = _h4_safe_choose(obs)
        if h4_choice is not None:
            return h4_choice
        return _historical_silver_choose_options(obs)

    # Compute the exact inherited action once.  Failure to resolve it as one
    # ordinary MAIN Attack returns this cached choice unchanged.
    parent_choice = _historical_silver_choose_options(obs)
    parent_attack_witness = _h4_safe_parent_attack_witness(obs, parent_choice)
    if parent_attack_witness is None:
        return parent_choice

    h4_choice = _h4_safe_choose(obs, parent_attack_witness)
    if h4_choice is not None:
        return h4_choice
    return parent_choice
