...
_recycle_pad_alakazam_latch = {}
_recycle_pad_blocked_turn = None
...
def _clear_recycle_pad_alakazam_latch() -> None:
    _recycle_pad_alakazam_latch.clear()
...
def _clear_emergency_state(*, clear_cache: bool = False) -> None:
    global _recycle_pad_blocked_turn
    ...
    _clear_recycle_pad_alakazam_latch()
    _recycle_pad_blocked_turn = None
    if clear_cache:
...
def _decision_signature(obs: Observation) -> tuple:
    ...
    recycle_guard_signature = None
    if _recycle_pad_alakazam_latch:
        recycle_guard_signature = (
            _recycle_pad_alakazam_latch.get("stage"),
...
def _prepare_emergency_state(obs: Observation) -> None:
    ...
    for latch, clear in (
        ...
        if latch and (
            ...
            clear()
    if (
        _recycle_pad_blocked_turn is not None
        and _recycle_pad_blocked_turn != (state.turn, state.yourIndex)
    ):
        _recycle_pad_blocked_turn = None
...
def _start_active_psychic_immediate_ko(
    ...
    if (
        ...
        or _recycle_pad_alakazam_latch
        or active is None
...
def _recycle_card_group_fingerprint(
    cards, expected_player: int | None = None
) -> tuple | None:
    rows = []
    for card in cards:
        if (
...
def _recycle_same_cards(left: tuple | None, right: tuple | None) -> bool:
    if left is None or right is None or len(left) != len(right):
        return False
    return tuple(sorted(left, key=lambda row: row[1])) == tuple(
        sorted(right, key=lambda row: row[1])
    )


def _recycle_remove_cards(source: tuple, removed: tuple) -> tuple | None:
    remaining = list(source)
    for fingerprint in removed:
        if remaining.count(fingerprint) != 1:
            return None
        remaining.remove(fingerprint)
    return tuple(remaining)


def _recycle_board_rows(player, player_index: int) -> tuple | None:
    rows = []
    ...
    serials = [row[2] for row in rows]
    if len(serials) != len(set(serials)):
        return None
    return tuple(rows)


def _recycle_bench_rows(player, player_index: int) -> tuple | None:
    rows = _recycle_board_rows(player, player_index)
    if rows is None:
        return None
    return tuple(row for row in rows if row[0] == int(AreaType.BENCH))


def _recycle_status_fingerprint(state) -> tuple:
    return tuple(
        (
            player.poisoned,
            player.burned,
...
def _recycle_all_visible_serials_unique(state) -> bool:
    serials = _bridge_public_serials(state)
    return (
        all(isinstance(serial, int) and serial > 0 for serial in serials)
        and len(serials) == len(set(serials))
    )


def _recycle_psychic_draw_is_certified() -> bool:
    ...
    if (
        ...
        or data.basic
        or data.stage1
        or data.evolvesFrom != card_table[Kadabra].name
        or data.hp != 140
    ...
    skill = data.skills[0]
    return (
        " ".join(_normalized_skill_text(skill.name).split())
        == "psychic draw"
        and " ".join(_normalized_skill_text(skill.text).split())
        == (
...
def _recycle_evolved_active_is_exact(mine, latch: dict) -> Pokemon | None:
    player_index = latch.get("player")
    active = mine.active[0] if len(mine.active) == 1 else None
    expected_pre = tuple(latch.get("source_pre_evolution") or ()) + (
        (Kadabra, latch.get("source_serial"), player_index),
    )
    if (
        active is None
        or active.id != Alakazam
        or active.serial != latch.get("target_alakazam_serial")
        or active.hp != latch.get("expected_evolved_hp")
        or active.maxHp != card_table[Alakazam].hp
        or not active.appearThisTurn
        or not _bridge_pokemon_is_publicly_complete(active, player_index)
        or tuple(int(unit) for unit in active.energies)
        != latch.get("source_energy_units")
        or tuple(
            _bridge_card_fingerprint(card) for card in active.energyCards
        )
        != latch.get("source_energy_cards")
        or tuple(_bridge_card_fingerprint(card) for card in active.tools)
...
def _recycle_future_deck_is_safe(latch: dict) -> bool:
    post_draw = latch.get("expected_post_draw_deck")
    prizes = latch.get("own_prize_count")
    return (
        isinstance(post_draw, int)
        and isinstance(prizes, int)
        and post_draw - 1 > prizes
    )


def _recycle_common_is_same(
    obs: Observation,
    latch: dict,
    *,
    expected_turn_actions: int,
    ...
    own_discard = _recycle_card_group_fingerprint(mine.discard, my_index)
    opponent_discard = _recycle_card_group_fingerprint(
        theirs.discard, 1 - my_index
    )
    stadium = _recycle_card_group_fingerprint(state.stadium)
    own_bench = _recycle_bench_rows(mine, my_index)
    opponent_board = _recycle_board_rows(theirs, 1 - my_index)
    current_active = mine.active[0] if len(mine.active) == 1 else None
    active_ok = (
        _recycle_evolved_active_is_exact(mine, latch) is not None
        if evolved
        else (
            current_active is not None
            and _bridge_pokemon_is_publicly_complete(
                current_active, my_index
            )
            and _bridge_pokemon_fingerprint(current_active)
    ...
    return (
        ...
        and state.turnActionCount == expected_turn_actions
        ...
        and mine.benchMax == latch.get("own_bench_max")
        and theirs.benchMax == latch.get("opponent_bench_max")
        and mine.deckCount == expected_deck_count
        and theirs.deckCount == latch.get("opponent_deck_count")
        and theirs.handCount == latch.get("opponent_hand_count")
        and hand is not None
        and len(hand) == mine.handCount
        and _recycle_same_cards(own_discard, expected_discard)
        and _recycle_same_cards(
            opponent_discard, latch.get("opponent_discard")
        )
        and _recycle_same_cards(stadium, latch.get("stadium"))
        and own_bench == latch.get("own_bench")
        and opponent_board == latch.get("opponent_board")
        and _recycle_status_fingerprint(state) == latch.get("status")
        and active_ok
        and _recycle_all_visible_serials_unique(state)
        and _recycle_future_deck_is_safe(latch)
    )


def _recycle_exact_hand(
    mine, player_index: int, expected: tuple
) -> bool:
    current = _active_psychic_hand_fingerprint(mine, player_index)
    return _recycle_same_cards(current, expected)


def _recycle_hand_with_unknown_draws(
    mine,
    player_index: int,
    frozen: tuple,
    draw_count: int,
    ...
    current = _active_psychic_hand_fingerprint(mine, player_index)
    if current is None or len(current) != len(frozen) + draw_count:
        return False
    if any(current.count(card) != 1 for card in frozen):
        return False
    frozen_serials = {card[1] for card in frozen}
    unknown = [card for card in current if card[1] not in frozen_serials]
    return len(unknown) == draw_count


def _recycle_matching_main_options(
    obs: Observation, card_id: int, serial: int, option_type: OptionType
) -> list[int]:
    ...
    for option_index, option in enumerate(obs.select.option):
        ...
        if card.id == card_id and card.serial == serial:
            matches.append(option_index)
    return matches


def _recycle_block_and_clear(obs: Observation) -> None:
    global _recycle_pad_blocked_turn
    _recycle_pad_blocked_turn = (obs.current.turn, obs.current.yourIndex)
    _clear_recycle_pad_alakazam_latch()


def _start_recycle_pad_alakazam_attack_line(
    obs: Observation, parent_option_index: int
) -> list[int] | None:
    """Replace only finalized Poké Pad with Ash -> Pad -> Active Alakazam."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if (
        ...
        or not all_latches_clear
        or _recycle_pad_blocked_turn == (state.turn, my_index)
        or not isinstance(parent_option_index, int)
        or parent_option_index < 0
        or parent_option_index >= len(select.option)
        or active is None
        ...
        or not any(
            ...
            and int(unit) == int(EnergyType.PSYCHIC)
            for unit, card in zip(active.energies, active.energyCards)
        )
        or any(
            pokemon is not None and pokemon.id == Alakazam
            for pokemon in list(mine.active) + list(mine.bench)
        )
        or not (0 < len(mine.prize) <= 4 and 0 < len(theirs.prize) <= 4)
        or not _recycle_psychic_draw_is_certified()
        or not _recycle_all_visible_serials_unique(state)
    ):
        return None

    hand = _active_psychic_hand_fingerprint(mine, my_index)
    discard = _recycle_card_group_fingerprint(mine.discard, my_index)
    own_bench = _recycle_bench_rows(mine, my_index)
    opponent_board = _recycle_board_rows(theirs, 1 - my_index)
    opponent_discard = _recycle_card_group_fingerprint(
        theirs.discard, 1 - my_index
    ...
    parent_option = select.option[parent_option_index]
    if parent_option.type != OptionType.PLAY:
        return None
    if (
        not isinstance(parent_option.index, int)
        or parent_option.index < 0
        or parent_option.index >= len(mine.hand)
    ):
        return None
    pad = mine.hand[parent_option.index]
    if (
        pad.id != Poke_Pad
        or getattr(pad, "playerIndex", None) != my_index
        or _recycle_matching_main_options(
            obs, Poke_Pad, pad.serial, OptionType.PLAY
        )
        != [parent_option_index]
    ):
        return None
    ...
    for option_index, option in enumerate(select.option):
        ...
        if card.id == Sacred_Ash:
            ash_matches.append((option_index, card))
    if len(ash_matches) != 1:
        return None
    ash_option_index, ash = ash_matches[0]
    if getattr(ash, "playerIndex", None) != my_index:
        return None
    ...
    alakazam = [card for card in eligible if card.id == Alakazam]
    if not alakazam:
        return None
    target = alakazam[0]
    expected_return_count = min(5, len(eligible))
    expected_post_draw_deck = (
        mine.deckCount + expected_return_count - 1 - 3
    )
    if expected_post_draw_deck - 1 <= len(mine.prize):
        return None

    ash_fp = _bridge_card_fingerprint(ash)
    pad_fp = _bridge_card_fingerprint(pad)
    target_fp = _bridge_card_fingerprint(target)
    hand_after_ash = _recycle_remove_cards(hand, (ash_fp,))
    hand_after_pad = _recycle_remove_cards(hand_after_ash or (), (pad_fp,))
    if hand_after_ash is None or hand_after_pad is None:
        return None
    expected_evolved_hp = (
        active.hp + card_table[Alakazam].hp - active.maxHp
    )
    if expected_evolved_hp <= 0 or expected_evolved_hp > card_table[Alakazam].hp:
        return None

    _recycle_pad_alakazam_latch.update(
        stage="await_to_deck",
        turn=state.turn,
        player=my_index,
        start_turn_actions=state.turnActionCount,
        ...
        start_hand=hand,
        hand_after_ash=hand_after_ash,
        hand_after_pad=hand_after_pad,
        start_discard=discard,
        opponent_discard=opponent_discard,
        stadium=stadium,
        status=_recycle_status_fingerprint(state),
        own_bench=own_bench,
        ...
        ash_serial=ash.serial,
        ash_fingerprint=ash_fp,
        pad_serial=pad.serial,
        pad_fingerprint=pad_fp,
        target_alakazam_serial=target.serial,
        target_alakazam_fingerprint=target_fp,
        expected_return_count=expected_return_count,
        expected_post_draw_deck=expected_post_draw_deck,
    )
    return [ash_option_index]


def _recycle_pad_alakazam_overlay(obs: Observation) -> list[int] | None:
    """Advance only the frozen recycle/refill Active-Kadabra transaction."""
    if not _recycle_pad_alakazam_latch:
        return None
    latch = _recycle_pad_alakazam_latch
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    stage = latch.get("stage")
    start_actions = latch.get("start_turn_actions")
    start_deck = latch.get("start_deck_count")

    def abort() -> None:
        _recycle_block_and_clear(obs)

    if stage == "await_to_deck":
        if (
            select.context != SelectContext.TO_DECK
            or int(select.type) != 1
            or select.contextCard is not None
            or select.effect is None
            or _bridge_card_fingerprint(select.effect)
            != latch.get("ash_fingerprint")
            or select.deck is not None
            or not _recycle_common_is_same(
                obs,
                latch,
                expected_turn_actions=start_actions + 1,
                expected_deck_count=start_deck,
                expected_discard=latch.get("start_discard"),
                evolved=False,
            )
            or not _recycle_exact_hand(
                mine, my_index, latch.get("hand_after_ash")
            )
            or select.maxCount != latch.get("expected_return_count")
            or select.maxCount > 5
            or select.minCount < 0
            or select.minCount > select.maxCount
        ):
            abort()
            return None
        resolved = []
        ...
        for option_index, option in enumerate(select.option):
            ...
            frozen_first = (
                0
                if card.serial == latch.get("target_alakazam_serial")
                else 1
            )
            resolved.append(
                (
        ...
        if (
            len(resolved) != len(eligible)
            or len(resolved) < select.maxCount
            or sum(
                row[3] == latch.get("target_alakazam_fingerprint")
                for row in resolved
            )
            != 1
        ):
            abort()
            return None
        selected = sorted(resolved, key=lambda row: row[:3])[
            : select.maxCount
        ]
        selected_fingerprints = tuple(row[3] for row in selected)
        if latch.get("target_alakazam_fingerprint") not in selected_fingerprints:
            abort()
            return None
        discard_after = _recycle_remove_cards(
            latch.get("start_discard"), selected_fingerprints
        )
        if discard_after is None:
            abort()
            return None
        latch["selected_return_fingerprints"] = selected_fingerprints
        latch["discard_after_ash"] = discard_after + (
            latch.get("ash_fingerprint"),
        )
        latch["stage"] = "await_pad_play"
        return [row[2] for row in selected]

    if stage == "await_pad_play":
        if (
            select.context != SelectContext.MAIN
            or int(select.type) != 0
            or select.minCount != 1
            ...
            or not _recycle_common_is_same(
                obs,
                latch,
                expected_turn_actions=start_actions + 2,
                expected_deck_count=(
                    start_deck + latch.get("expected_return_count")
                ),
                expected_discard=latch.get("discard_after_ash"),
                evolved=False,
            )
            or not _recycle_exact_hand(
                mine, my_index, latch.get("hand_after_ash")
            ...
            abort()
            return None
        matches = _recycle_matching_main_options(
            obs,
            Poke_Pad,
            latch.get("pad_serial"),
            OptionType.PLAY,
        )
        if len(matches) != 1:
            abort()
            return None
        latch["stage"] = "await_pad_search"
        return [matches[0]]

    if stage == "await_pad_search":
        expected_discard = latch.get("discard_after_ash")
        if (
            ...
            or not _recycle_common_is_same(
                obs,
                latch,
                expected_turn_actions=start_actions + 3,
                expected_deck_count=(
                    start_deck + latch.get("expected_return_count")
                ),
            ...
            or not _recycle_exact_hand(
                mine, my_index, latch.get("hand_after_pad")
            )
        ):
            abort()
            return None
        matches = []
        seen = set()
        public_serials = set(_bridge_public_serials(state))
        for option_index, option in enumerate(select.option):
            if (
                option.type != OptionType.CARD
                or option.area != AreaType.DECK
                ...
                or not isinstance(option.index, int)
                or option.index < 0
                or option.index >= len(select.deck)
            ):
                abort()
                return None
            ...
            if (
                ...
                or card_table.get(card.id) is None
                or card_table[card.id].cardType != CardType.POKEMON
                or card.serial in public_serials
            ):
                abort()
                return None
            fingerprint = _bridge_card_fingerprint(card)
            if card.serial in seen:
                abort()
                return None
            seen.add(card.serial)
            if fingerprint == latch.get("target_alakazam_fingerprint"):
                matches.append(option_index)
        if len(matches) != 1:
            abort()
            return None
        latch["stage"] = "await_evolve_play"
        return [matches[0]]

    if stage == "await_evolve_play":
        discard_after_pad = latch.get("discard_after_ash") + (
            latch.get("pad_fingerprint"),
        )
        hand_with_target = latch.get("hand_after_pad") + (
            latch.get("target_alakazam_fingerprint"),
        )
        if (
            select.context != SelectContext.MAIN
            or int(select.type) != 0
            ...
            or not _recycle_common_is_same(
                obs,
                latch,
                expected_turn_actions=start_actions + 4,
                expected_deck_count=(
                    start_deck
                    + latch.get("expected_return_count")
                    - 1
                ),
                expected_discard=discard_after_pad,
                evolved=False,
            )
            or not _recycle_exact_hand(mine, my_index, hand_with_target)
        ):
            abort()
            return None
        matches = []
        ...
        if len(matches) != 1:
            abort()
            return None
        latch["discard_after_pad"] = discard_after_pad
        latch["stage"] = "await_draw_offer"
        return [matches[0]]

    if stage == "await_draw_offer":
        ...
        if (
            ...
            or int(select.type) != 9
            or select.effect is not None
            or context_card is None
            or _bridge_card_fingerprint(context_card)
            != latch.get("target_alakazam_fingerprint")
            or select.minCount != 1
            or select.maxCount != 1
            ...
            or not _recycle_common_is_same(
                obs,
                latch,
                expected_turn_actions=start_actions + 5,
                expected_deck_count=(
                    start_deck
                    + latch.get("expected_return_count")
                    - 1
                ),
                expected_discard=latch.get("discard_after_pad"),
                evolved=True,
            )
            or not _recycle_exact_hand(
                mine, my_index, latch.get("hand_after_pad")
            ...
            abort()
            return None
        latch["stage"] = "await_draw_resolution"
        return [yes[0]]

    if stage == "await_draw_resolution":
        if (
            select.context != SelectContext.MAIN
            or not _recycle_common_is_same(
                obs,
                latch,
                expected_turn_actions=start_actions + 6,
                expected_deck_count=latch.get("expected_post_draw_deck"),
                expected_discard=latch.get("discard_after_pad"),
                evolved=True,
            )
            or not _recycle_hand_with_unknown_draws(
                mine,
                my_index,
                latch.get("hand_after_pad"),
                3,
            )
        ):
            abort()
...
def agent(obs_dict: dict) -> list[int]:
    ...
    transaction_action = _recycle_pad_alakazam_overlay(obs)
    if transaction_action is not None:
        return _remember_action(decision_signature, transaction_action)
    ...
    if (
        ...
        and not _recycle_pad_alakazam_latch
    ):
    ...
    # The exact v3 scorer has finalized Poké Pad.  Only this isolated recovery
    # transaction may replace that exact serial, and only after every inherited
    # overlay has declined the state.
    if (
        context == SelectContext.MAIN
        and len(chosen_action) == 1
        and not _hilda_source_latch
        ...
        transaction_action = _start_recycle_pad_alakazam_attack_line(
            obs, chosen_action[0]
        )
        if transaction_action is not None:
