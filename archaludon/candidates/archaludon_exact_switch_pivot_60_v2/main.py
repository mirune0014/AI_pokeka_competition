...
# ---------------------------------------------------------------------------
# Candidate E: exact Switch pivot (independent child of the accepted parent)
# ---------------------------------------------------------------------------
# This section is the only child-owned resolver.  It deliberately does not
# copy or alter the parent scorer.  The rule admits one public, same-turn
# pivot only when the current Active has no known attack route and one Bench
# Pokémon is already attack-ready without another action.
import copy as _switch_copy
import hashlib as _switch_hashlib
import json as _switch_json
import unicodedata as _switch_unicodedata
import re as _switch_re


_SWITCH_CARD_ID = 1123
_SWITCH_EXACT_NAME = "Switch"
_SWITCH_EXACT_TEXT = (
    "Switch your Active Pokémon with 1 of your Benched Pokémon."
)
_SWITCH_EXACT_OBJECT_SHA256 = (
    "AB6D96C6C566C72AFEC8D8C072249C53960058DBF34A305B4EBEDB70B802A200"
)
_SWITCH_OWNER = "EXACT_SWITCH_PIVOT"
_SWITCH_STAGES = (
    "IDLE",
    "SWITCH_PLAY_BOUND",
    "WAIT_DESTINATION",
    "WAIT_MAIN_AFTER_SWITCH",
    "WAIT_ATTACK_RESOLUTION",
    "COMPLETE",
)
_SWITCH_CALLBACK_LIMIT = 4
_SWITCH_COMMITMENT = None
_SWITCH_TELEMETRY = {
    "rule_id": _SWITCH_OWNER,
    "selected_source": "HISTORICAL_SILVER_PARENT",
    "switch_drawn_games": 0,
    "switch_legal_games": 0,
    "switch_trigger_eligible_games": 0,
    "switch_played_games": 0,
    "same_turn_attack_games": 0,
    "switch_to_terminal_win_games": 0,
    "switch_to_ko_games": 0,
    "switch_to_positive_non_ko_games": 0,
    "transaction_aborts": 0,
    "parent_commitment_rollbacks": 0,
    "outside_trigger_switch_suppressions": 0,
    "rejection_reason": "not_called",
    "parent_call_count": 0,
}

_SWITCH_SUPPORTED_ATTACKS = frozenset({223, 224, 253, 965, 1072, 1212})
_SWITCH_BENIGN_SKILLS = frozenset({120, 140, 184, 265, 269, 648, 743})


def _switch_object_hash(card):
    try:
        payload = _switch_json.dumps(
            getattr(card, "__dict__", None),
            ensure_ascii=False,
            sort_keys=True,
            default=str,
        ).encode("utf-8")
        return _switch_hashlib.sha256(payload).hexdigest().upper()
    except Exception:
        return None


def _switch_registry_exact():
    """Require the exact local CARD_DB Switch and Item effect contract."""
    try:
        card = _parent.CARD_DB.get(_SWITCH_CARD_ID)
        skills = getattr(card, "skills", None)
        return bool(
            card is not None
            and getattr(card, "cardId", None) == _SWITCH_CARD_ID
            and getattr(card, "name", None) == _SWITCH_EXACT_NAME
            and getattr(card, "cardType", None) == _CardType.ITEM
            and isinstance(skills, list)
            and len(skills) == 1
            and getattr(skills[0], "name", None) == _SWITCH_EXACT_NAME
            and getattr(skills[0], "text", None) == _SWITCH_EXACT_TEXT
            and getattr(card, "attacks", None) == []
            and _switch_object_hash(card) == _SWITCH_EXACT_OBJECT_SHA256
        )
    except Exception:
        return False


def _switch_snapshot_parent_commitment():
    """Snapshot only the audited candidate-parent commitment field."""
    return {"_materialization_owner": _switch_copy.deepcopy(_materialization_owner)}


def _switch_restore_parent_commitment(snapshot):
    global _materialization_owner
    if not isinstance(snapshot, dict) or "_materialization_owner" not in snapshot:
        return False
    _materialization_owner = _switch_copy.deepcopy(snapshot["_materialization_owner"])
    return True


def _switch_rows(obs):
    try:
        return _option_rows(obs)
    except Exception:
        return None


def _switch_card_ref_from_row(obs, row):
    try:
        ref = row[2][1]
    except Exception:
        return None
    return ref if isinstance(ref, tuple) and len(ref) == 3 else None


def _switch_play_options(obs, rows):
    if rows is None or not _switch_registry_exact():
        return ()
    seat = getattr(obs.current, "yourIndex", None)
    found = []
    for row in rows:
        option = row[1]
        ref = _switch_card_ref_from_row(obs, row)
        if (
            getattr(option, "type", None) == _OptionType.PLAY
            and ref is not None
            and ref[0] == _SWITCH_CARD_ID
            and ref[2] == seat
        ):
            found.append(row)
    return tuple(found)
...
def _switch_target_options(obs, rows, commitment=None):
    if rows is None or getattr(obs.select, "context", None) not in {
        _SelectContext.SWITCH,
        _SelectContext.TO_ACTIVE,
    }:
        return ()
    seat = getattr(obs.current, "yourIndex", None)
    bound_serial = commitment.get("destination_serial") if isinstance(commitment, dict) else None
    found = []
    for row in rows:
        option = row[1]
        ref = _switch_card_ref_from_row(obs, row)
        if (
            getattr(option, "type", None) == _OptionType.CARD
            and getattr(option, "area", None) == _AreaType.BENCH
            and getattr(option, "playerIndex", None) == seat
            and ref is not None
            and ref[2] == seat
            and getattr(_parent.CARD_DB.get(ref[0]), "cardType", None) == _CardType.POKEMON
            and (bound_serial is None or ref[1] == bound_serial)
        ):
            found.append(row)
    return tuple(found)


def _switch_exact_status(player):
    values = tuple(
        getattr(player, name, None)
        for name in ("poisoned", "burned", "asleep", "paralyzed", "confused")
    )
...
def _switch_energy_payable(pokemon, attack):
    energies = getattr(pokemon, "energies", None)
    required = getattr(attack, "energies", None)
    if not isinstance(energies, list) or not isinstance(required, list):
        return None
    try:
        units = [int(value) for value in energies]
        cost = [int(value) for value in required]
    except (TypeError, ValueError):
        return None
    valid = {int(value) for value in _EnergyType}
    if any(value not in valid for value in units + cost):
        return None
    colored = [value for value in cost if value != int(_EnergyType.COLORLESS)]
    if any(units.count(value) < colored.count(value) for value in set(colored)):
        return False
    return len(units) - len(colored) >= cost.count(int(_EnergyType.COLORLESS))


def _switch_effective_damage(obs, attacker, target, attack_id):
    """Exact public damage for the small printed attack surface in this deck."""
    if attack_id not in _SWITCH_SUPPORTED_ATTACKS:
        return None
    attack = getattr(_parent, "ALL_ATTACKS", {}).get(attack_id)
    ...
    if attack is None or attacker_data is None or target_data is None:
        return None
    if getattr(attacker_data, "cardType", None) != _CardType.POKEMON:
        return None
    if _switch_exact_status(obs.current.players[obs.current.yourIndex]) != (False,) * 5:
        return None
    if _switch_exact_status(obs.current.players[1 - obs.current.yourIndex]) != (False,) * 5:
        return None
    if getattr(attacker, "tools", None) or getattr(target, "tools", None):
        return None
    ...
    if stadium:
        stadium_ref = _exact_card_ref(stadium[0])
        if stadium_ref is None or stadium_ref[0] != _FULL_METAL_LAB:
            return None
    base = getattr(attack, "damage", None)
    if not _is_exact_int(base) or base < 0:
        return None
    if attack_id == 1072:
        hand = getattr(obs.current.players[obs.current.yourIndex], "hand", None)
        ...
        if not _is_exact_int(hp) or not _is_exact_int(max_hp) or max_hp < hp:
            return None
        damage_taken = max_hp - hp
        if damage_taken % 10:
            return None
        base += damage_taken
    elif attack_id not in {223, 253, 965, 1212}:
        return None
    skills = getattr(target_data, "skills", None)
    if skills and getattr(target, "id", None) not in _SWITCH_BENIGN_SKILLS:
        # Crustle's printed prevention is exact for a Basic attacker.
        if getattr(target, "id", None) == 345:
            if not isinstance(getattr(attacker_data, "basic", None), bool):
                return None
            if attacker_data.basic:
                return 0
        else:
            return None
    ...
    if resistance is not None and attacker_type == resistance:
        damage -= 30
    if stadium:
        if getattr(target_data, "energyType", None) == _EnergyType.METAL:
            damage -= 30
    return max(0, damage)


def _switch_prize_yield(pokemon):
    data = getattr(_parent, "CARD_DB", {}).get(getattr(pokemon, "id", None))
    if data is None:
        return None
    ex = getattr(data, "ex", None)
    mega = getattr(data, "megaEx", None)
    if not isinstance(ex, bool) or not isinstance(mega, bool) or (ex and mega):
        return None
    return 3 if mega else 2 if ex else 1


def _switch_attack_certificate(obs, attacker, target, attack_id):
    attack = getattr(_parent, "ALL_ATTACKS", {}).get(attack_id)
    payable = _switch_energy_payable(attacker, attack) if attack is not None else None
    damage = _switch_effective_damage(obs, attacker, target, attack_id)
    prize = _switch_prize_yield(target)
    remaining = getattr(obs.current.players[obs.current.yourIndex], "prize", None)
    if payable is not True or damage is None or prize is None or not isinstance(remaining, list):
        return None
    take = min(prize, len(remaining)) if damage >= getattr(target, "hp", 10**9) else 0
    return {
        "attack_id": attack_id,
        "damage": damage,
        "prize": prize,
        "take": take,
        "wins_game_now": bool(take >= len(remaining)),
    }


def _switch_bench_certificates(obs):
    try:
        seat = obs.current.yourIndex
        mine = obs.current.players[seat]
        opponent = obs.current.players[1 - seat]
        ...
        return None
    if not isinstance(bench, list) or target is None:
        return None
    candidates = []
    for pokemon in bench:
        if pokemon is None:
            continue
        if _pokemon_fingerprint(pokemon, seat) is None:
            return None
        data = getattr(_parent, "CARD_DB", {}).get(getattr(pokemon, "id", None))
        attacks = getattr(data, "attacks", None) if data is not None else None
        if not isinstance(attacks, list):
            return None
        rows = []
        for attack_id in attacks:
            attack = getattr(_parent, "ALL_ATTACKS", {}).get(attack_id)
            if attack is None or _switch_energy_payable(pokemon, attack) is None:
                return None
            if _switch_energy_payable(pokemon, attack) is True:
                certificate = _switch_attack_certificate(obs, pokemon, target, attack_id)
                if certificate is None:
                    return None
                rows.append(certificate)
        if rows:
            best = max(rows, key=lambda row: (
                int(row["wins_game_now"]), row["take"], row["damage"]
            ))
            candidates.append({
                "pokemon": pokemon,
...
def _switch_active_attack_state(obs, rows):
    try:
        seat = obs.current.yourIndex
        active = obs.current.players[seat].active[0]
        target = obs.current.players[1 - seat].active[0]
        ...
        return None, None
    attack_rows = [row for row in (rows or ()) if getattr(row[1], "type", None) == _OptionType.ATTACK]
    if not attack_rows:
        data = getattr(_parent, "CARD_DB", {}).get(getattr(active, "id", None))
        return (False, None) if isinstance(getattr(data, "attacks", None), list) else (None, None)
    certificates = []
    for row in attack_rows:
        attack_id = getattr(row[1], "attackId", None)
        certificate = _switch_attack_certificate(obs, active, target, attack_id)
        if certificate is None:
            return None, None
        certificates.append(certificate)
    return any(row["damage"] > 0 for row in certificates), certificates


def _switch_active_action_path_exists(obs, rows, active):
    active_ref = _exact_card_ref(active, obs.current.yourIndex)
    if active_ref is None or rows is None:
        return None
    for row in rows:
        option = row[1]
        if getattr(option, "type", None) not in {_OptionType.ATTACH, _OptionType.EVOLVE}:
            continue
        if row[2][4] == active_ref:
            return True
    return False


def _switch_parent_kind(obs, parent_action, rows):
    ...
    if getattr(option, "type", None) == _OptionType.END:
        return "END"
    if getattr(option, "type", None) == _OptionType.ATTACK:
        return "ATTACK"
    if getattr(option, "type", None) == _OptionType.PLAY:
        ref = _switch_card_ref_from_row(obs, rows[position])
        if ref is not None and ref[0] == _LILLIE:
            return "LILLIE"
    return "OTHER"


def _switch_trigger(obs, parent_action, rows):
    if (
        rows is None
        or getattr(obs.select, "type", None) != _SelectType.MAIN
        or getattr(obs.select, "context", None) != _SelectContext.MAIN
        or getattr(obs.current, "result", None) != -1
        or getattr(obs.current, "supporterPlayed", None) is not False
        or _SWITCH_COMMITMENT is not None
        or _materialization_owner is not None
        or _setup_ledger is not None
    ):
        return None, "main_or_owner_gate_failed"
    kind = _switch_parent_kind(obs, parent_action, rows)
    if kind == "LILLIE" or kind == "UNKNOWN":
        return None, "parent_safety_or_unknown"
    # The accepted parent remains authoritative whenever it selected a
    # certified safety/continuity rule.  A stale or unknown source marker is
    # not evidence that a Switch pivot is safe; fail closed to the parent.
    parent_source = _last_telemetry.get("selected_source")
    if parent_source not in {None, "HISTORICAL_SILVER_PARENT"}:
        return None, "parent_rule_priority"
    play_options = _switch_play_options(obs, rows)
    if not play_options:
        return None, "switch_not_legal"
    seat = obs.current.yourIndex
    active = obs.current.players[seat].active[0]
    if active is None:
        return None, "active_unknown"
    active_state, _ = _switch_active_attack_state(obs, rows)
    if active_state is not False:
        return None, "active_attack_present_or_unknown"
    path = _switch_active_action_path_exists(obs, rows, active)
    if path is not False:
        return None, "active_attach_or_evolve_path"
    candidates = _switch_bench_certificates(obs)
    if not candidates:
        return None, "no_ready_bench_attacker"
    best_key = max(
        (int(row["wins_game_now"]), row["take"], row["damage"])
    ...
    if len(best) != 1:
        return None, "bench_candidate_tie"
    candidate = best[0]
    switch_row = min(play_options, key=lambda row: (row[0], row[2][1][1]))
    return {
...
def _switch_attack_row(obs, rows, commitment):
    ...
    for row in rows:
        if getattr(row[1], "type", None) != _OptionType.ATTACK:
            continue
        if getattr(row[1], "attackId", None) != commitment.get("attack_id"):
            continue
        matches.append(row)
    return matches[0] if len(matches) == 1 else None


def _switch_log_confirmed(obs, commitment):
    try:
        return any(
            _log_exact(
                log,
                _LogType.SWITCH,
                {
                    "playerIndex": commitment["player_index"],
                    "cardIdActive": commitment["active_card_id"],
                    "serialActive": commitment["active_serial"],
                    "cardIdBench": commitment["destination_card_id"],
                    "serialBench": commitment["destination_serial"],
                },
            )
            for log in getattr(obs, "logs", ())
        )
...
def _switch_release(reason, aborted=False):
    global _SWITCH_COMMITMENT
    if aborted:
        _SWITCH_TELEMETRY["transaction_aborts"] += 1
    _SWITCH_TELEMETRY["rejection_reason"] = reason
    _SWITCH_COMMITMENT = None


def _switch_resume(obs, parent_action, rows):
    global _SWITCH_COMMITMENT
    commitment = _SWITCH_COMMITMENT
    if not isinstance(commitment, dict):
        return None
    commitment["callback_count"] += 1
    if commitment["callback_count"] > _SWITCH_CALLBACK_LIMIT:
        _switch_restore_parent_commitment(commitment["parent_commitment_before"])
        _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
        _switch_release("callback_budget_exceeded", aborted=True)
        return None
    current = obs.current
    if (
        current.result != -1
        or current.yourIndex != commitment["player_index"]
        or current.turn != commitment["turn"]
    ):
        _switch_restore_parent_commitment(commitment["parent_commitment_before"])
        _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
        _switch_release("turn_or_result_changed", aborted=True)
        return None
    stage = commitment["stage"]
    if stage == "WAIT_DESTINATION":
        if getattr(obs.select, "context", None) not in {_SelectContext.SWITCH, _SelectContext.TO_ACTIVE}:
            _switch_restore_parent_commitment(commitment["parent_commitment_before"])
            _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
            _switch_release("switch_target_context_unknown", aborted=True)
            return None
        targets = _switch_target_options(obs, rows, commitment)
        if len(targets) != 1:
            _switch_restore_parent_commitment(commitment["parent_commitment_before"])
            _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
            _switch_release("switch_target_binding_failed", aborted=True)
            return None
        commitment["stage"] = "WAIT_MAIN_AFTER_SWITCH"
        return [targets[0][0]]
    if stage == "WAIT_MAIN_AFTER_SWITCH":
        if (
            getattr(obs.select, "type", None) != _SelectType.MAIN
            or getattr(obs.select, "context", None) != _SelectContext.MAIN
        ):
            _switch_restore_parent_commitment(commitment["parent_commitment_before"])
            _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
            _switch_release("post_switch_main_unknown", aborted=True)
            return None
        active = obs.current.players[commitment["player_index"]].active[0]
        opponent = obs.current.players[1 - commitment["player_index"]].active[0]
        if (
            active is None
            or opponent is None
            or active.serial != commitment["destination_serial"]
            or opponent.serial != commitment["opponent_active_serial"]
        ):
            _switch_restore_parent_commitment(commitment["parent_commitment_before"])
            _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
            _switch_release("post_switch_identity_failed", aborted=True)
            return None
        row = _switch_attack_row(obs, rows, commitment)
        if row is None:
            _switch_restore_parent_commitment(commitment["parent_commitment_before"])
            _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
            _switch_release("bound_attack_missing", aborted=True)
            return None
        certificate = _switch_attack_certificate(obs, active, opponent, commitment["attack_id"])
        if certificate is None or certificate["damage"] < 1:
            _switch_restore_parent_commitment(commitment["parent_commitment_before"])
            _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
            _switch_release("post_switch_damage_unknown", aborted=True)
            return None
        _switch_restore_parent_commitment(commitment["parent_commitment_before"])
        _SWITCH_TELEMETRY["parent_commitment_rollbacks"] += 1
        commitment["stage"] = "WAIT_ATTACK_RESOLUTION"
        _SWITCH_TELEMETRY["same_turn_attack_games"] += 1
        if certificate["take"] > 0:
            _SWITCH_TELEMETRY["switch_to_ko_games"] += 1
        if certificate["wins_game_now"]:
            _SWITCH_TELEMETRY["switch_to_terminal_win_games"] += 1
        else:
            _SWITCH_TELEMETRY["switch_to_positive_non_ko_games"] += 1
        return [row[0]]
    if stage == "WAIT_ATTACK_RESOLUTION":
        if _switch_log_confirmed(obs, commitment) or getattr(obs.current, "result", None) != -1:
            _switch_release("attack_resolution_confirmed")
        else:
            _switch_release("attack_resolution_unknown")
        return None
    _switch_release("stage_unknown", aborted=True)
    return None


def get_switch_telemetry():
    return dict(_SWITCH_TELEMETRY)


_ACCEPTED_PARENT_AGENT_SWITCH = agent


def _switch_final_agent(obs_dict):
    global _SWITCH_COMMITMENT
    parent_before = _switch_snapshot_parent_commitment()
    parent_action = _ACCEPTED_PARENT_AGENT_SWITCH(obs_dict)
    _SWITCH_TELEMETRY.update({
        "selected_source": "HISTORICAL_SILVER_PARENT",
    ...
    try:
        obs = _to_observation_class(obs_dict)
        rows = _switch_rows(obs)
        if rows is None or obs.select is None or obs.current is None:
            _switch_release("rows_or_observation_unknown", aborted=True)
            return parent_action
        if _SWITCH_COMMITMENT is not None:
            resumed = _switch_resume(obs, parent_action, rows)
            if resumed is not None:
                _SWITCH_TELEMETRY["selected_source"] = _SWITCH_OWNER
                return resumed
            if _SWITCH_COMMITMENT is not None:
                return parent_action
        if getattr(obs.select, "context", None) != _SelectContext.MAIN:
            return parent_action
        eligible, reason = _switch_trigger(obs, parent_action, rows)
        if eligible is None:
            if reason not in {"main_or_owner_gate_failed", "switch_not_legal"}:
                _SWITCH_TELEMETRY["outside_trigger_switch_suppressions"] += 1
            _SWITCH_TELEMETRY["rejection_reason"] = reason
            return parent_action
        _SWITCH_TELEMETRY["switch_drawn_games"] += 1
        _SWITCH_TELEMETRY["switch_legal_games"] += 1
        _SWITCH_TELEMETRY["switch_trigger_eligible_games"] += 1
        candidate = eligible["candidate"]
        switch_row = eligible["switch_row"]
        switch_ref = _switch_card_ref_from_row(obs, switch_row)
        active = obs.current.players[obs.current.yourIndex].active[0]
        opponent = obs.current.players[1 - obs.current.yourIndex].active[0]
        if switch_ref is None or active is None or opponent is None:
            _switch_release("switch_binding_unknown", aborted=True)
            return parent_action
        _switch_restore_parent_commitment(parent_before)
        _SWITCH_COMMITMENT = {
            "owner": _SWITCH_OWNER,
            ...
            "callback_count": 0,
            "switch_serial": switch_ref[1],
            "destination_card_id": candidate["card_id"],
            "destination_serial": candidate["serial"],
            "attack_id": candidate["attack_id"],
            "active_card_id": active.id,
            "active_serial": active.serial,
            "opponent_active_serial": opponent.serial,
            "expected_damage": candidate["damage"],
            "expected_prize": candidate["take"],
            "parent_commitment_before": parent_before,
        }
        _SWITCH_TELEMETRY.update({
            "selected_source": _SWITCH_OWNER,
            "rejection_reason": reason,
            "parent_commitment_rollbacks": _SWITCH_TELEMETRY.get("parent_commitment_rollbacks", 0) + 1,
        })
        _SWITCH_TELEMETRY["switch_played_games"] += 1
        return [switch_row[0]]
    except Exception as exc:
        _switch_release("wrapper_exception:" + type(exc).__name__, aborted=True)
        return parent_action


# Loader contract: this is the only final callable insertion for Candidate E.
agent = _switch_final_agent
