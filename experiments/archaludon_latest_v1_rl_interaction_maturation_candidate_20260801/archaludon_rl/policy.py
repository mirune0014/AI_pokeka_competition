...
POLICY_SCHEMA_VERSION = "residual-policy-v4"
...
class PolicyDecision:
    ...
    behavior_formula_id: str
    behavior_schema_version: str
    behavior_schema_sha256: str
    behavior_temperature: float
    behavior_support_mixture: float
    behavior_option_order: tuple[int, ...]
    behavior_action_order_sha256: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.behavior_formula_id != BEHAVIOR_FORMULA_ID
            or self.behavior_schema_version != BEHAVIOR_POLICY_SCHEMA_VERSION
            or self.behavior_schema_sha256
            != behavior_policy_sha256(behavior_receipt)
            or self.behavior_temperature != BEHAVIOR_TEMPERATURE
            or self.behavior_support_mixture != BEHAVIOR_SUPPORT_MIXTURE
        ):
            raise ValueError("policy decision behavior-policy identity mismatch")
        protected = bool(
        ...
        if self.ppo_eligible:
            ...
            if (
                ...
                or len(self.residuals) != self.legal_option_count
                or len(self.final_probabilities) != self.legal_option_count
                or self.behavior_option_order
                != tuple(range(self.legal_option_count))
                or self.behavior_action_order_sha256
                != behavior_action_order_sha256(
                    _ordered_action_receipt(self.semantic_options)
                )
            ...
            recomputed = ReferencePolicy().distribution(
                self.legal_option_count,
                teacher_index,
                self.residuals,
            )
            if self.final_probabilities != recomputed.probabilities:
                raise ValueError("eligible behavior probabilities are not canonical")
            if (
                len(self.action) != 1
                or self.behavior_logprob is None
                or not math.isclose(
                    float(self.behavior_logprob),
                    math.log(self.final_probabilities[self.action[0]]),
                    rel_tol=0.0,
                    abs_tol=1e-15,
                )
            ):
                raise ValueError("eligible behavior logprob is not canonical")


def _ordered_action_receipt(
...
class ResidualPolicy:
    def __init__(
        ...
        *,
        ...
        self.behavior_receipt = canonical_behavior_policy_receipt()
        self.behavior_schema_sha256 = behavior_policy_sha256(
            self.behavior_receipt
        )
    ...
    def decide(self, observation: Any) -> PolicyDecision:
        ...
        if select is None:
            ...
            return PolicyDecision(
                ...
                behavior_formula_id=BEHAVIOR_FORMULA_ID,
                behavior_schema_version=BEHAVIOR_POLICY_SCHEMA_VERSION,
                behavior_schema_sha256=self.behavior_schema_sha256,
                behavior_temperature=BEHAVIOR_TEMPERATURE,
                behavior_support_mixture=BEHAVIOR_SUPPORT_MIXTURE,
                behavior_option_order=(),
                behavior_action_order_sha256=behavior_action_order_sha256(()),
            )
        ...
        return PolicyDecision(
            ...
            behavior_temperature=BEHAVIOR_TEMPERATURE,
            behavior_support_mixture=BEHAVIOR_SUPPORT_MIXTURE,
            behavior_option_order=tuple(
                option.engine_index for option in options
            ),
            behavior_action_order_sha256=behavior_action_order_sha256(
                _ordered_action_receipt(options)
            ),
