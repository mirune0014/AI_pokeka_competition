...
def _missing_energy_units(attached, required) -> int:
    """Minimum additional single-energy attachments for a public attack cost."""
    available = [int(energy) for energy in attached]
    typed = [int(energy) for energy in required if int(energy) != int(EnergyType.COLORLESS)]
    missing = 0
    for needed in typed:
        if needed in available:
            available.remove(needed)
        elif int(EnergyType.RAINBOW) in available:
            available.remove(int(EnergyType.RAINBOW))
        else:
            missing += 1
    colorless = sum(int(energy) == int(EnergyType.COLORLESS) for energy in required)
    return missing + max(0, colorless - len(available))


def _certified_fixed_ko_attacks(attacker: Pokemon, defender: Pokemon, *, max_missing: int):
    """Public, fail-closed KO certificates for fixed-text-free attacks."""
    ...
    for attack_id in attacker_data.attacks:
        ...
        if attack is None or attack.text.strip() or attack.damage <= 0:
            continue
        if _missing_energy_units(attacker.energies, attack.energies) > max_missing:
            continue
        damage = int(attack.damage)
        # Ignore Weakness (a conservative lower bound) and apply public Resistance.
        if (defender_data.resistance is not None
                and int(attacker_data.energyType) == int(defender_data.resistance)):
            damage = max(0, damage - 30)
        if damage >= defender.hp:
            certified.append(attack)
    return certified


def _acerola_immediate_damage(*, hand_size: int, defense_energy_count: int,
                               enhanced_hammer_count: int) -> int:
    """Immediate Powerful Hand damage after Acerola and required Hammers."""
    if enhanced_hammer_count < defense_energy_count:
        return -1
    return max(0, hand_size - 1 - defense_energy_count) * 20


def _acerola_final_bridge(*, acerola_in_hand: bool, supporter_unused: bool,
                           opponent_prizes: int, active_is_alakazam: bool,
                           active_has_psychic: bool, powerful_hand_legal: bool,
                           opponent_active_is_ex: bool, opponent_ex_ready_ko: bool,
                           ready_non_ex_ko: bool, immediate_winning_ko: bool,
                           projected_damage: int, opponent_active_hp: int,
                           ex_takes_all_prizes: bool,
                           bench_has_two_prize_rule_box: bool,
                           bench_empty: bool) -> bool:
    if not (acerola_in_hand and supporter_unused and opponent_prizes in (1, 2)):
        return False
    if not (active_is_alakazam and active_has_psychic and powerful_hand_legal):
        return False
    if not (opponent_active_is_ex and opponent_ex_ready_ko) or ready_non_ex_ko:
        return False
    if immediate_winning_ko or not ex_takes_all_prizes or projected_damage <= 0:
        return False
    if not (projected_damage < opponent_active_hp <= 2 * projected_damage):
        return False
    if opponent_prizes == 2 and bench_has_two_prize_rule_box:
        return False
    if opponent_prizes == 1 and not bench_empty:
        return False
    return True


def _selection_effect_id(select) -> int:
    for source in (select.effect, select.contextCard):
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # ---- Acerola final-prize bridge (public state only) ----
    active_defense_energy_count = (
        count_special_defense_energies(op_active) if op_active is not None else 0
    )
    acerola_projected_damage = _acerola_immediate_damage(
        hand_size=hand_size,
        defense_energy_count=active_defense_energy_count,
        enhanced_hammer_count=hand_counts[Enhanced_Hammer],
    )
    powerful_hand_legal = any(
        option.type == OptionType.ATTACK and option.attackId == ATTACK_POWERFUL_HAND
        for option in select.option
    )
    opponent_active_is_ex = op_active is not None and _has_rule_box(op_active.id)
    opponent_ex_ready_ko = bool(
        opponent_active_is_ex
        and _certified_fixed_ko_attacks(op_active, active_pokemon, max_missing=1)
    ) if active_pokemon is not None else False
    ready_non_ex_ko = any(
        not _has_rule_box(pokemon.id)
        and bool(_certified_fixed_ko_attacks(pokemon, active_pokemon, max_missing=0))
        for pokemon in op_all_pokemon
    ) if active_pokemon is not None else False
    immediate_winning_ko = target_can_kill and my_prize_count <= target_prize_gain
    bench_has_two_prize_rule_box = any(
        _has_rule_box(pokemon.id) and prize_count(pokemon) >= 2
        for pokemon in my_state.bench
    )
    acerola_final_bridge = _acerola_final_bridge(
        acerola_in_hand=hand_counts[Acerola_Mischief] > 0,
        supporter_unused=not state.supporterPlayed,
        opponent_prizes=len(op_state.prize),
        active_is_alakazam=active_id == Alakazam,
        active_has_psychic=active_has_psychic,
        powerful_hand_legal=powerful_hand_legal,
        opponent_active_is_ex=opponent_active_is_ex,
        opponent_ex_ready_ko=opponent_ex_ready_ko,
        ready_non_ex_ko=ready_non_ex_ko,
        immediate_winning_ko=immediate_winning_ko,
        projected_damage=acerola_projected_damage,
        opponent_active_hp=op_active.hp if op_active is not None else 0,
        ex_takes_all_prizes=(
            op_active is not None and my_prize_count <= prize_count(op_active)
        ),
        bench_has_two_prize_rule_box=bench_has_two_prize_rule_box,
        bench_empty=len(my_state.bench) == 0,
    )
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if _selection_effect_id(select) == Acerola_Mischief:
                if (o.playerIndex == my_index and o.area == AreaType.ACTIVE
                        and o.index == 0 and card.id == Alakazam):
                    score = 40000
                else:
                    score = -1
            ...
            if data.cardType == CardType.POKEMON:
                ...
                elif card.id == Acerola_Mischief:
                    score = 3300 if acerola_final_bridge else -1
