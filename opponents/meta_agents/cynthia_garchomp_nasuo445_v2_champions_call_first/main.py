...
def is_champions_call(obs, opt) -> bool:
    return opt.type == OptionType.ABILITY and getattr(option_card(obs, opt), "id", None) == GABITE


def is_game_winning_active_ko(obs, opt) -> bool:
    """True only for a legal attack that visibly takes the final prizes."""
...
def choose_options(obs):
    champions_call_available = any(is_champions_call(obs, opt) for opt in obs.select.option)
    scored: list[tuple[int, int, str]] = []
    for i, opt in enumerate(obs.select.option):
        try:
            score, reason = score_option(obs, opt)
            if champions_call_available:
                if is_game_winning_active_ko(obs, opt):
                    score, reason = 110000, "game-winning active KO before Champion's Call"
                elif is_champions_call(obs, opt):
                    score, reason = 100000, "Champion's Call before voluntary actions"
