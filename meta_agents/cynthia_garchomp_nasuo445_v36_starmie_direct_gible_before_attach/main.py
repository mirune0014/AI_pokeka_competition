...
def is_direct_gible_bench_action(obs, opt) -> bool:
    """Identify a currently legal, direct Gible bench action."""
    return bool(
        opt.type == OptionType.PLAY
        and getattr(option_card(obs, opt), "id", None) == GIBLE
        and bench_has_space(obs)
    )


def starmie_direct_gible_before_attach_index(obs, scored: list[tuple[int, int, str]]) -> int | None:
    """Against a visible Starmie board early, widen before attaching to the sole body."""
    if (
        obs.select.context != SelectContext.MAIN
        or getattr(obs.current, "turn", 0) not in {1, 2}
        or sum(1 for pokemon in my_pokemon(obs) if pokemon.id in MAIN_LINE) != 1
        or not any(pokemon.id in {STARYU, MEGA_STARMIE_EX} for pokemon in opponent_pokemon(obs))
    ):
        return None
    ...
    if (
        ...
        or getattr(option_target(obs, selected), "id", None) not in MAIN_LINE
    ):
        return None
    direct_gible_actions = [
        (score, index) for score, index, _reason in scored
        if is_direct_gible_bench_action(obs, obs.select.option[index])
    ]
    return max(direct_gible_actions, key=lambda row: (row[0], -row[1]))[1] if direct_gible_actions else None
    ...
    if backup_index is not None:
        ...
        for row_index, (score, index, reason) in enumerate(scored):
            if index == backup_index:
                ...
                break
    starmie_gible_index = starmie_direct_gible_before_attach_index(obs, scored)
    if starmie_gible_index is not None:
        selected_attach_score = max(scored, key=lambda row: (row[0], -row[1]))[0]
        for row_index, (score, index, reason) in enumerate(scored):
            if index == starmie_gible_index:
                scored[row_index] = (
                    selected_attach_score + 1,
                    index,
                    f"{reason} before sole-body attachment versus visible Starmie",
                )
                break
