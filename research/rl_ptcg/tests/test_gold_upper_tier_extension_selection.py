...
from research.rl_ptcg.gold_oracle_states import canonical_sha256, file_sha256
from research.rl_ptcg.gold_upper_tier_extension_selection import (
    _self_hash,
...
class UpperTierExtensionSelectionTests(unittest.TestCase):
    ...
    def write(self):
        with patch("research.rl_ptcg.gold_upper_tier_extension_selection.verify_allroots_audit", return_value={"verified": True}):
    ...
    def test_uses_the_bound_allroots_audit_cli(self):
        with patch(
            "research.rl_ptcg.gold_upper_tier_extension_selection.verify_allroots_audit",
            return_value={"verified": True},
        ) as verify:
            write_upper_tier_extension_selection(self.audit, "target", self.output, self.workspace)
        self.assertEqual(
            (self.workspace / "infrastructure" / "tools" / "build_gold_upper_tier_allroots_audit.py").resolve(),
