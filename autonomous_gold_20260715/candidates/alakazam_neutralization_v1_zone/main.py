...
Dawn = 1231              # x4
Battle_Cage = 1264       # x4
Neutralization_Zone = 1247  # x1  (ACE SPEC)
...
def _missing_energy_units(attached, required) -> int:
    ...
    available = [int(e) for e in attached]
    typed = [int(e) for e in required if int(e) != int(EnergyType.COLORLESS)]
    missing = 0
    for need in typed:
        if need in available:
            available.remove(need)
        elif int(EnergyType.RAINBOW) in available:
            available.remove(int(EnergyType.RAINBOW))
        else:
            missing += 1
    colorless = sum(int(e) == int(EnergyType.COLORLESS) for e in required)
    missing += max(0, colorless - len(available))
    return missing


def _public_ready_attacks(pokemon: Pokemon):
    data = card_table.get(pokemon.id)
    if data is None:
        return []
    return [
        attack_table[attack_id]
        for attack_id in data.attacks
        if attack_id in attack_table
        and _missing_energy_units(pokemon.energies, attack_table[attack_id].energies) <= 1
    ]


def _should_play_zone(*, visible_ready_ex: bool, active_non_rule_box: bool,
                      zone_active: bool, immediate_active_ko_before: bool,
                      immediate_active_ko_after: bool, final_prize_ex_ko_threat: bool,
                      ready_non_ex_attacker: bool, boss_ko_ready: bool) -> bool:
    """Pure public-state gate for spending Neutralization Zone from hand."""
    if zone_active or not visible_ready_ex or not active_non_rule_box:
        return False
    emergency = final_prize_ex_ko_threat
    if immediate_active_ko_before and not immediate_active_ko_after and not emergency:
        return False
    if boss_ko_ready and not emergency:
        return False
    if ready_non_ex_attacker and immediate_active_ko_before and not emergency:
        return False
    return True


def _should_play_battle_cage(*, stadium_id: int, dragapult_pressure: bool,
                             zone_play_available: bool) -> bool:
    """Battle Cage is a fallback and never replaces an active Zone."""
    return (stadium_id != Neutralization_Zone
            and dragapult_pressure
            and not zone_play_available)
...
def agent(obs_dict: dict) -> list[int]:
    ...
    # ---- Neutralization Zone public-state gate ----
    ready_ex = []
    ready_non_ex = []
    for pkmn in op_all_pokemon:
        attacks = _public_ready_attacks(pkmn)
        if not attacks:
            continue
        if _has_rule_box(pkmn.id):
            ready_ex.append((pkmn, attacks))
        else:
            ready_non_ex.append((pkmn, attacks))

    active_non_rule_box = active_pokemon is not None and not _has_rule_box(active_id)
    active_hammer_needed = count_special_defense_energies(op_active) if op_active is not None else 0
    active_ko_energy_ok = hand_counts[Enhanced_Hammer] >= active_hammer_needed
    active_hand_after_costs = max_hand_size - active_hammer_needed
    powerful_hand_ready = active_id == Alakazam and active_has_psychic and state.turn >= 2
    immediate_active_ko_before = bool(
        powerful_hand_ready and op_active is not None and active_ko_energy_ok
        and active_hand_after_costs * 20 >= op_active.hp
    )
    immediate_active_ko_after = bool(
        powerful_hand_ready and op_active is not None and active_ko_energy_ok
        and max(0, active_hand_after_costs - 1) * 20 >= op_active.hp
    )
    op_prize_count = len(op_state.prize)
    final_prize_ex_ko_threat = False
    if active_pokemon is not None and op_prize_count <= prize_count(active_pokemon):
        final_prize_ex_ko_threat = any(
            any(attack.damage >= active_pokemon.hp for attack in attacks)
            for _, attacks in ready_ex
        )

    zone_condition = _should_play_zone(
        visible_ready_ex=bool(ready_ex),
        active_non_rule_box=active_non_rule_box,
        zone_active=stadium_id == Neutralization_Zone,
        immediate_active_ko_before=immediate_active_ko_before,
        immediate_active_ko_after=immediate_active_ko_after,
        final_prize_ex_ko_threat=final_prize_ex_ko_threat,
        ready_non_ex_attacker=bool(ready_non_ex),
        boss_ko_ready=target_use_boss and target_can_kill,
    )
    zone_play_available = hand_counts[Neutralization_Zone] > 0 and zone_condition
        ...
        if o.type == OptionType.NUMBER:
            ...
            if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE:
                ...
                if card.id == Dudunsparce:
                    ...
                    score += 80 if zone_condition else -80
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if card.id == Buddy_Buddy_Poffin:
                    ...
                    if _should_play_battle_cage(
                        stadium_id=stadium_id,
                        dragapult_pressure=op_has_dragapult_line,
                        zone_play_available=zone_play_available,
                    ):
                        score = 19000
                    else:
                        score = -1
                    ...
                    score = 19500 if zone_condition else -1
