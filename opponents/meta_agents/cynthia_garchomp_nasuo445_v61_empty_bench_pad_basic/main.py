...
def empty_bench_poke_pad_basic_index(obs) -> int | None:
    """Select a benchable Basic from Poke Pad when only the Active is in play."""
    if (
        obs.select.context != SelectContext.TO_HAND
        or getattr(getattr(obs.select, "effect", None), "id", None) != POKE_PAD
    ):
        return None
    ...
    if (
        ...
        or bench is None
        or len(bench) != 0
        or bench_max is None
        or len(bench) >= bench_max
        or hand is None
    ):
        return None

    priorities = {GIBLE: 0, ROSELIA: 1, SPIRITOMB: 2}
    if any(getattr(card, "id", None) in priorities for card in hand if card):
        return None
    ...
    for index, option in enumerate(getattr(obs.select, "option", None) or []):
        ...
        if card_id in priorities:
            eligible.append((priorities[card_id], index))
