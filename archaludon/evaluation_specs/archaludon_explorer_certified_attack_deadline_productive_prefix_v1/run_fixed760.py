...
def panel_command(spec: dict, panel: dict) -> list[str]:
    ...
    for opponent in panel["opponents"]:
        command.extend(
            ["--opponent", f"{opponent['label']}={ROOT / opponent['path']}"]
        )
    command.extend(
        [
            "--games-per-seat",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Frozen trace-retained Explorer-deadline fixed-760 executor."
    )
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Run the frozen schedule after successful preflight.",
    )
