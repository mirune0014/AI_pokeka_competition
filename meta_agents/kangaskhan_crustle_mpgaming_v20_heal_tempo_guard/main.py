...
def jumbo_ice_cream_tempo_guard(me, opp, options):
    """Keep the proactive heal only when public tempo does not favor attacking."""
    attacks = [opt for opt in options if opt.type == OptionType.ATTACK]
    if not attacks or len(me.prize) < len(opp.prize):
        return False
    opp_active = active(opp)
    if opp.deckCount <= 3 or opp_active is None:
        return True
    for opt in attacks:
        attack = ATTACK_DB.get(opt.attackId)
        damage = getattr(attack, "damage", None)
        if type(damage) is int and damage >= opp_active.hp:
            return False
    return True
...
def agent(obs_dict, configuration=None):
    try:
        ...
        jumbo_ice_cream_allowed = jumbo_ice_cream_tempo_guard(me, opp, sel.option)
        scores = []
            ...
            if sel.context == SelectContext.MAIN:
                if opt.type == OptionType.ATTACK:
                    ...
                    score = (11000 if (c is not None and c.id == JUMBO_ICE_CREAM
                                       and jumbo_ice_cream_allowed and me_active is not None
                                       and active_missing_hp >= 80)
