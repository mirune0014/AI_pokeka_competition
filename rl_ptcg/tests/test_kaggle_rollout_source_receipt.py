...
from rl_ptcg.kaggle_rollout_source_receipt import (
...
class KaggleRolloutSourceReceiptTests(unittest.TestCase):
    def _fixture(self, root: Path) -> tuple[Path, Path, Path, dict]:
        ...
        receipt = root / "analysis_outputs" / "gold_replay_phase3" / "receipt.json"
    ...
    def _build(self, execution_path: Path, log: Path, receipt: Path, local: dict) -> dict:
        with patch("rl_ptcg.kaggle_rollout_source_receipt.verify_kaggle_rollout_execution", return_value=local):
    ...
    def test_rejects_receipt_input_drift(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("rl_ptcg.kaggle_rollout_source_receipt.verify_kaggle_rollout_execution", return_value=self._local(execution)):
