...
def _visible_effects_are_clear(parent: Any, state: Any) -> bool:
    dangerous = ("prevent", "protect", "less damage", "takes no damage", "isn't affected", "is not affected", "weakness", "resistance", "can't be knocked out", "cannot be knocked out", "redirect", "damage counter")
    sources = []
        ...
        for skill in data.skills or ():
            ...
            for marker in dangerous:
                if marker not in text:
                    continue
                if marker == "damage counter" and "once during your turn" in text:
                    continue
                return False
    return True
