...
class TrajectoryModelTrainingTests(unittest.TestCase):
    ...
    def _refresh_manifest_contract_hashes(
        ...
        payload["dataset_sha256"] = dataset_sha256(spec_hash, receipts)
    ...
    def test_trainer_rejects_legacy_ambiguous_manifest_and_trajectory(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(ValueError, "unsafe prior run manifest"):
                self._train_fixture(fixture)

        with tempfile.TemporaryDirectory() as temporary:
            fixture = self._make_run(
                Path(temporary),
                run_id="legacy-trajectory",
            )
            episode = json.loads(
                fixture["episode_path"].read_text(encoding="utf-8")
            )
            episode["schema_version"] = "trajectory-v2"
            del episode["opponent_id"]
            fixture["episode_path"].write_text(
                json.dumps(episode),
                encoding="utf-8",
            )
            self._refresh_manifest_episode(fixture)
            with self.assertRaisesRegex(ValueError, "unsafe prior trajectory"):
                self._train_fixture(fixture)
