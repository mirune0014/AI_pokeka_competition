...
# Strict public prize exchange.  This deliberately supports only the one
# fixed-damage attack used by the frozen Archaludon plan.  Any modifier that
# is not explicitly modeled returns to the unchanged legacy Boss policy.
_ALAKAZAM_EXCHANGE_ATTACK_IDS = {METAL_DEFENDER}
_ALAKAZAM_EXCHANGE_SAFE_SKILL_TERMS = ("once during your turn", "draw")
_ALAKAZAM_EXCHANGE_UNSAFE_SKILL_TERMS = (
    "damage done",
    "prevent",
    "takes less damage",
    "isn't affected by",
...
def _alakazam_exchange_target_is_exact(obs, target):
    if target is None or target.hp is None:
        return False
    target_data = CARD_DB.get(target.id)
    if target_data is None:
        return False
    if getattr(target, "tools", None) or []:
        return False
    if any(
        card and card.id not in set(range(1, 10))
        for card in (getattr(target, "energyCards", None) or [])
    ):
        return False
    for skill in getattr(target_data, "skills", None) or []:
        text = (getattr(skill, "text", None) or "").lower()
        if any(term in text for term in _ALAKAZAM_EXCHANGE_UNSAFE_SKILL_TERMS):
            return False
        if text and not all(
            term in text for term in _ALAKAZAM_EXCHANGE_SAFE_SKILL_TERMS
        ):
            return False

    stadium_ids = [card.id for card in (obs.current.stadium or []) if card]
    if any(card_id != FULL_METAL_LAB for card_id in stadium_ids):
        return False
    return True


def _alakazam_exchange_attacks(obs, require_main_option):
    attacker = active_pokemon(obs)
    if attacker is None:
        return []
    mine = my_state(obs)
    if any(bool(getattr(mine, name, False)) for name in ("asleep", "paralyzed", "confused")):
        return []

    if require_main_option:
        attack_ids = {
        ...
        if not attack_ids:
            return []
        attacks = [ALL_ATTACKS.get(attack_id) for attack_id in attack_ids]
    else:
        attacks = _ready_beneficial_attacks(attacker)
    return [
        attack
        for attack in attacks
        if attack is not None and attack.attackId in _ALAKAZAM_EXCHANGE_ATTACK_IDS
    ]


def _alakazam_exchange_damage(obs, attacker, attack, target):
    if not _alakazam_exchange_target_is_exact(obs, target):
        return None
    damage = _checked_attack_damage(attacker, attack, target)
    stadium_ids = [card.id for card in (obs.current.stadium or []) if card]
    if FULL_METAL_LAB in stadium_ids:
        target_data = CARD_DB.get(target.id)
        energy_type = getattr(target_data, "energyType", None)
        energy_type = int(getattr(energy_type, "value", energy_type or 0))
        if energy_type == METAL_ENERGY:
            damage = max(0, damage - 30)
    return damage


def _alakazam_strict_prize_exchange(obs, require_main_option):
    if detect_matchup(obs) != "alakazam":
        return None
    attacker = active_pokemon(obs)
    opponent_active = opp_active_pokemon(obs)
    if attacker is None or opponent_active is None:
        return None
    attacks = _alakazam_exchange_attacks(obs, require_main_option)
    if not attacks:
        return None

    active_ko = any(
        (damage := _alakazam_exchange_damage(obs, attacker, attack, opponent_active))
        is not None
        and damage >= opponent_active.hp
        for attack in attacks
    )
    if not active_ko:
        return None
    ...
    if remaining <= 0:
        return None
    active_prizes = min(prize_value(opponent_active), remaining)
    if active_prizes >= remaining:
        return None

    certified = []
    for target in opp_bench_pokemon(obs):
        target_ko = any(
            (damage := _alakazam_exchange_damage(obs, attacker, attack, target))
            is not None
        ...
        if not target_ko:
            continue
        effective_prizes = min(prize_value(target), remaining)
        if effective_prizes > active_prizes:
            certified.append((target, effective_prizes))
    if not certified:
        return None
    return {
        "active_prizes": active_prizes,
        "best_bench_prizes": max(value for _, value in certified),
        "certified": certified,
    }
...
def score_play(obs, opt):
    ...
    if cid == BOSS:
        if obs.current.supporterPlayed:
            return -1000, "Supporter already used"
        exchange = _alakazam_strict_prize_exchange(obs, True)
        if exchange is not None:
            return (
                32000 + exchange["best_bench_prizes"] * 100,
                "Alakazam exchange: Boss higher-prize exact KO",
            )
        if detect_matchup(obs) == "ogerpon":
            opp_act = opp_active_pokemon(obs)
            attacks = planned_archaludon_attacks(obs)
...
def _bench_ex_continuity_option(obs, opt):
    """Certify one deterministic bench evolution as the next attack backup."""
    try:
        if obs.select.context != SelectContext.MAIN or opt.type != OptionType.EVOLVE:
            return False
        ...
        mine = my_state(obs)
        active = active_pokemon(obs)
        if active is None or active.id != ARCHALUDON_EX:
            return False
        if sum(p.id == ARCHALUDON_EX for p in all_my_pokemon(obs)) != 1:
            return False

        # Enumerate only legal MAIN options.  Max Energy wins; bench order and
        # serials make both the target and duplicate hand-card options unique.
        legal = []
        for option_order, candidate in enumerate(obs.select.option):
            if candidate.type != OptionType.EVOLVE:
                continue
            if candidate.inPlayArea != AreaType.BENCH:
                continue
            card = option_card(obs, candidate)
            target = option_target(obs, candidate)
            if card is None or card.id != ARCHALUDON_EX:
                continue
            if target is None or target.id != DURALUDON:
                continue
            if candidate.inPlayIndex is None:
                return False
            if getattr(target, "serial", None) is None or getattr(card, "serial", None) is None:
                return False
            if getattr(target, "energyCards", None) is None:
                return False
            legal.append((
                (
                    -len(target.energyCards),
                    int(candidate.inPlayIndex),
                    int(target.serial),
                    int(card.serial),
                    option_order,
                ),
        ...
        if not legal:
            return False
        _, winner, target = min(legal, key=lambda entry: entry[0])
        if winner is not opt:
            return False

        bench_line = [
            pokemon
            for pokemon in (mine.bench or [])
            if pokemon and pokemon.id in {DURALUDON, ARCHALUDON, ARCHALUDON_EX}
        ]
        if len(bench_line) != 1 and len(target.energyCards) < 2:
            return False

        opponent = opp_state(obs)
        if opponent.prize is None or len(opponent.prize) < 3:
            return False

        # Metal Defender takes only the Active Pokemon's printed prizes.  If
        # that value is below our remaining-prize count, no current attack can
        # be an immediate prize win; all less certain cases fail closed.
        opponent_active = opp_active_pokemon(obs)
        if opponent_active is None or CARD_DB.get(opponent_active.id) is None:
            return False
        if mine.prize is None or len(mine.prize) <= prize_value(opponent_active):
            return False

        if CORNERSTONE_OGERPON_EX in opp_visible_card_ids(obs):
            return False
        if detect_matchup(obs) == "crustle":
            return False
...
def score_evolve(obs, opt):
    ...
    if cid == ARCHALUDON_EX and tid == DURALUDON:
        ...
        if _bench_ex_continuity_option(obs, opt):
            return 19000, "continuity: evolve certified bench backup"
...
def score_target(obs, opt):
    ...
    if ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
        ...
        if pi != yi and card:
            exchange = _alakazam_strict_prize_exchange(obs, False)
            if exchange is not None:
                card_serial = getattr(card, "serial", None)
                certified_prizes = next(
                    (
                        effective_prizes
                        for target, effective_prizes in exchange["certified"]
                        if card_serial is not None
                        and getattr(target, "serial", None) == card_serial
                        and target.id == card.id
                        and target.hp == card.hp
                    ),
                    None,
                )
                if certified_prizes == exchange["best_bench_prizes"]:
                    parent_score = (
                        20000
                        + prize_value(card) * 3000
                        + energy_count(card) * 100
                    )
                    return (
                        40000 + parent_score,
                        "Alakazam exchange: target higher-prize exact KO",
                    )
