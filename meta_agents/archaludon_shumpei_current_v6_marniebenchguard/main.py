...
ARTICUNO = 414
RELICANTH = 57
# Removed-card sentinels keep inherited branches unreachable without changing
# the route-aware helpers that still share their structure.
CINDERACE = -666
EXPLORER = -1185
...
_SETUP_ACTIVE_PRIORITY = {
    DURALUDON: (30000, "Active: Duraludon"),
    ARTICUNO: (20000, "Active fallback: Articuno"),
    RELICANTH: (10000, "Active fallback: Relicanth"),
}

ALWAYS_SAFE_DISCARD = {METAL_ENERGY}
...
def mirror_alloy_search_ready(obs):
    """Ultra Ball search only: race a visible, charged mirror Archaludon ex."""
    active = active_pokemon(obs)
    if not (active and active.id == DURALUDON):
        return False
    if metal_in_discard(obs) < 2 or has_in_play(obs, ARCHALUDON_EX):
        return False
    opp_pokemon = ([opp_active_pokemon(obs)] if opp_active_pokemon(obs) else []) + opp_bench_pokemon(obs)
    return any(
        p.id == ARCHALUDON_EX and energy_count(p) >= 3
        for p in opp_pokemon
    )


def mirror_bench_reserve_evolution_ready(obs, target):
    """Bank a second ex only against the visible charged Cinderace mirror."""
    active = active_pokemon(obs)
    if not (
        active and active.id == ARCHALUDON_EX and energy_count(active) >= 3
        and active.hp <= 220
        and not obs.current.energyAttached
        and getattr(obs.current, "turnActionCount", 0) <= 1
        and can_evolve_to_archaludon_now(target, obs)
    ):
        return False
    opp_pokemon = ([opp_active_pokemon(obs)] if opp_active_pokemon(obs) else []) + opp_bench_pokemon(obs)
    return (
        any(p.id == 666 for p in opp_pokemon)
        and any(p.id == ARCHALUDON_EX and energy_count(p) >= 3 for p in opp_pokemon)
    )
...
def safe_discard_count(obs):
    ...
    for cid in ids:
        if cid == METAL_ENERGY and mt + safe < 2:
            safe += 1
    draw_in_hand = sum(1 for c in ids if c in (LILLIE, CARMINE, JUDGE, XEROSIC))
    if draw_in_hand >= 2:
        safe += draw_in_hand - 1
    return safe
...
def duraludon_line_count(obs):
    """Visible and held Duraludon/Archaludon pieces for bench discipline."""
    return (count_in_play(obs, DURALUDON) + count_in_play(obs, ARCHALUDON_EX)
            + sum(1 for cid in hand_ids(obs) if cid in {DURALUDON, ARCHALUDON_EX}))


def has_real_boss_ko(obs):
    """Do not spend a draw/disruption Supporter when Boss converts to a KO."""
    attacks = planned_archaludon_attacks(obs)
    return any(effective_damage(atk["damage"], target) >= target.hp
               for target in opp_bench_pokemon(obs) for atk in attacks)


def relevant_disruption(obs):
    """Xerosic is for a visible resource-rich or oversized opposing hand."""
    opp = opp_state(obs)
    opp_pokemon = [p for p in (opp.active + opp.bench) if p]
    loaded = any(energy_count(p) >= 3 or has_tool(p) for p in opp_pokemon)
    return opp.handCount >= my_state(obs).handCount + 3 or loaded
...
ITEMS = {POKE_PAD, ULTRA_BALL, POKEGEAR, NIGHT_STRETCHER, HERO_CAPE}
...
def score_play(obs, opt):
    ...
    if cid == ARTICUNO:
        if duraludon_line_count(obs) < 2:
            return -1000, "save Articuno: establish two Duraludon lines"
        visible_opp_ids = {p.id for p in ([opp_active_pokemon(obs)] if opp_active_pokemon(obs) else []) + opp_bench_pokemon(obs)}
        core_in_play = count_in_play(obs, DURALUDON) + count_in_play(obs, ARCHALUDON_EX)
        if core_in_play >= 2 and visible_opp_ids & {646, 647, 648} and archaludon_ex_attack_route(obs):
            return -1000, "save Articuno: Marnie line while core can attack"
        return 2500, "optional Articuno after two-line core"
    if cid == RELICANTH:
        if has_in_play(obs, RELICANTH):
            return -1000, "save duplicate Relicanth"
        if not any(p.id == ARCHALUDON_EX for p in all_my_pokemon(obs)):
            return 1500, "defer Relicanth until evolution is live"
        return 3500, "optional first Relicanth"
    ...
    if cid == CARMINE:
        ...
        if has_real_boss_ko(obs):
            return -1000, "save Carmine: real Boss KO available"
        line_missing = need_archaludon(obs) or need_duraludon(obs)
        no_metal = METAL_ENERGY not in ids and not obs.current.energyAttached
        return (9500, "Carmine: repair line/energy") if (line_missing or no_metal) else (1500, "Carmine: refresh")

    if cid == JUDGE:
        if obs.current.supporterPlayed:
            return -1000, "Supporter already used"
        viable_line = duraludon_line_count(obs) >= 2 or any(
            p.id in {DURALUDON, ARCHALUDON_EX} for p in all_my_pokemon(obs))
        if opp_state(obs).handCount >= my_state(obs).handCount + 3 and viable_line and not has_real_boss_ko(obs):
            return 7000, "Judge: materially larger opposing hand"
        return -1000, "save Judge: no favorable disruption"

    if cid == XEROSIC:
        if obs.current.supporterPlayed:
            return -1000, "Supporter already used"
        if relevant_disruption(obs) and not has_real_boss_ko(obs):
            return 6000, "Xerosic: relevant disruption"
        return -1000, "save Xerosic: no disruption target"
...
def score_evolve(obs, opt):
    ...
    if cid == ARCHALUDON_EX and tid == DURALUDON:
        ...
        if mirror_bench_reserve_evolution_ready(obs, target):
            return 30000, "Mirror: bank bench Archaludon ex before KO"
        if mc >= 2:
...
def score_option(obs, opt):
    ...
    if opt.type in {OptionType.YES, OptionType.NO}:
        ...
        if ctx == SelectContext.ACTIVATE:
            return (1, "activate") if opt.type == OptionType.YES else (0, "decline")
...
def score_to_hand(obs, opt):
    ...
    if effect_id == ULTRA_BALL and cid == ARCHALUDON_EX and mirror_alloy_search_ready(obs):
        return 23000, "Ultra Ball: mirror Alloy race"
    ...
    if cid == ARTICUNO:
        return (2500, "take optional Articuno") if duraludon_line_count(obs) >= 2 else (-1000, "skip Articuno before core")
    if cid == RELICANTH and not has_in_play(obs, RELICANTH) and any(
            p.id == ARCHALUDON_EX for p in all_my_pokemon(obs)):
        return 4000, "take first Relicanth"
    if cid == METAL_ENERGY:
        return 8000, "take Metal Energy"
    if cid == CARMINE and not obs.current.supporterPlayed:
        return 7000, "take Carmine"
    if cid == JUDGE and not obs.current.supporterPlayed:
        return 4500, "take Judge"
    if cid == XEROSIC and not obs.current.supporterPlayed:
        return 3000, "take Xerosic"
...
def score_discard(obs, opt):
    ...
    if effect_id == ULTRA_BALL:
        ...
        draw_count = ids.count(LILLIE) + ids.count(CARMINE) + ids.count(JUDGE) + ids.count(XEROSIC)
        if cid in (LILLIE, CARMINE, JUDGE, XEROSIC) and draw_count >= 2:
            return 11000, "UB: surplus supporter"
        if cid == ULTRA_BALL and ids.count(ULTRA_BALL) > 1:
            return 10000, "UB: duplicate"
        if cid in (LILLIE, CARMINE, JUDGE, XEROSIC) and draw_count <= 1:
    ...
    if cid == ARTICUNO:
        return 7000, "discard optional Articuno"
    if cid in {BOSS, FULL_METAL_LAB, POKEGEAR}:
        return 8500, "discard utility"
    if cid in {LILLIE, CARMINE, JUDGE, XEROSIC} and ids.count(cid) > 1:
