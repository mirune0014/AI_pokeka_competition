...
# H5_INHERITED_ATTACK_NONEX_120_KO transaction.  This is an unstacked,
# direct-historical-Silver overlay: it may replace only an inherited Raging
# Hammer with one serial-bound non-ex Archaludon evolution and its immediately
# confirmed Coated Attack.
_h5_transaction = None
...
# ── H5 inherited-Attack non-ex 120 KO transaction ──

_H5_SAFE_STADIUMS = {
    FULL_METAL_LAB,
    1252,  # Gravity Mountain: public HP already reflects its effect.
    1264,  # Battle Cage: does not alter Active attack damage.
    1266,  # Nighttime Mine: Archaludon 840 is not a Tera Pokemon.
}
_H5_SAFE_ATTACHED_ENERGY_IDS = set(range(1, 10)) | {
...
_H5_ALTERNATE_EVOLUTION_ATTACKS = {
    ARCHALUDON_EX: (METAL_DEFENDER, 220),
}


def _h5_reset():
    global _h5_transaction
    _h5_transaction = None
...
def _h5_card_fingerprint(card):
    if card is None:
        return None
    return (getattr(card, "id", None), _h5_serial(card))


def _h5_pokemon_fingerprint(pokemon):
    if pokemon is None:
        return None
    return (
        pokemon.id,
...
def _h5_board_fingerprint(player):
    pokemon = ([player.active[0]] if player.active else []) + list(
        player.bench or ()
    )
    return tuple(sorted(
        (_h5_pokemon_fingerprint(card) for card in pokemon if card),
        key=repr,
    ))


def _h5_bench_fingerprint(player):
    return tuple(
        _h5_pokemon_fingerprint(card)
        for card in (player.bench or ())
        if card
    )


def _h5_hand_fingerprint(player):
    return tuple(sorted(
        (
            _h5_card_fingerprint(card)
...
def _h5_stadium_fingerprint(obs):
    return tuple(
        _h5_card_fingerprint(card)
...
def _h5_public_modifiers(obs):
    return (
        _h5_stadium_fingerprint(obs),
        _h5_conditions(my_state(obs)),
        _h5_conditions(opp_state(obs)),
        _opp_last_attack_id,
    )


def _h5_visible_cards(obs):
...
def _h5_visible_serial_is_unique(obs, serial):
    if serial is None:
        return False
    return sum(
        1 for card in _h5_visible_cards(obs)
        if _h5_serial(card) == serial
    ) == 1


def _h5_known_tools_only(pokemon):
    # Hero's Cape changes only the already-public hp/maxHp for this policy.
    return all(
...
def _h5_safe_attached_energy_only(pokemon):
    return all(
        getattr(card, "id", None) in _H5_SAFE_ATTACHED_ENERGY_IDS
        and _h5_serial(card) is not None
        for card in (getattr(pokemon, "energyCards", None) or ())
    )


def _h5_exact_three_basic_metal(pokemon):
    cards = list(getattr(pokemon, "energyCards", None) or ())
    energy_types = tuple(getattr(pokemon, "energies", None) or ())
    serials = [_h5_serial(card) for card in cards]
    return (
        len(cards) == 3
        and energy_types == (METAL_ENERGY, METAL_ENERGY, METAL_ENERGY)
        and all(card.id == METAL_ENERGY for card in cards)
        and all(serial is not None for serial in serials)
        and len(set(serials)) == 3
    )


def _h5_static_cards_supported():
...
def _h5_local_only_modifier(text):
    # A local defense on some other Pokemon cannot increase an alternative
    # route's certainty.  Treating that route as a KO can only veto H5.
    return (
...
def _h5_public_board_supported(obs, target):
    target_serial = _h5_serial(target)
    for player in obs.current.players:
        pokemon = ([player.active[0]] if player.active else []) + list(
            player.bench or ()
        )
        for card in pokemon:
            if (
                ...
                or not _h5_known_tools_only(card)
                or not _h5_safe_attached_energy_only(card)
            ):
                return False
            texts = _h5_skill_texts(card)
            if texts is None:
                return False
            for text in texts:
                if "prize" in text:
                ...
                if not changes_combat:
                    continue
                if _h5_serial(card) == target_serial:
                    return False
                if not _h5_local_only_modifier(text):
                    return False

    stadium = list(obs.current.stadium or ())
    if (
        len(stadium) > 1
        or (stadium and stadium[0].id not in _H5_SAFE_STADIUMS)
    ):
        return False
    for card in stadium:
        texts = _h5_skill_texts(card)
...
def _h5_persistent_effects_supported():
...
def _h5_prize_value(pokemon):
    data = CARD_DB.get(pokemon.id) if pokemon else None
    texts = _h5_skill_texts(pokemon)
    if (
        data is None
        or texts is None
        or any("prize" in text for text in texts)
        or not _h5_known_tools_only(pokemon)
        or not _h5_safe_attached_energy_only(pokemon)
    ):
        return None
...
def _h5_exact_damage(obs, attacker_id, attack_id, target, damage_counters=0):
    ...
    if (
        ...
        or not _h5_public_board_supported(obs, target)
        or not _h5_persistent_effects_supported()
    ):
        return None

    if attack_id == RAGING_HAMMER:
        base = 80 + 10 * damage_counters
    elif attack_id in _H5_DIRECT_ATTACKS:
        base = _H5_DIRECT_ATTACKS[attack_id]
...
def _h5_parent_is_inherited_raging(obs, parent_choice):
    if (
...
def _h5_evolution_options(obs, card_id, target_serial):
    choices = []
    yi = obs.current.yourIndex
    for position, option in enumerate(obs.select.option):
        ...
        if (
            ...
            and _h5_serial(card) is not None
            and target
            and _h5_serial(target) == target_serial
...
def _h5_attack_options(obs, attack_id=None):
    choices = []
    for position, option in enumerate(obs.select.option):
        if (
            option.type == OptionType.ATTACK
...
def _h5_boss_options(obs):
    choices = []
    for position, option in enumerate(obs.select.option):
        if option.type != OptionType.PLAY:
            continue
        card = option_card(obs, option)
        if card and card.id == BOSS and _h5_serial(card) is not None:
            choices.append((position, card.serial))
    return choices


def _h5_route_attacks_before_evolution(obs, active, target):
    routes = []
    attack_options = _h5_attack_options(obs)
    if not attack_options:
        return None
    distinct = sorted({attack_id for _, attack_id in attack_options})
    if any(attack_id not in _H5_DIRECT_ATTACKS for attack_id in distinct):
        return None
    counters = damage_on(active) // 10
    for attack_id in distinct:
        damage = _h5_exact_damage(
    ...
    for option in obs.select.option:
        ...
        if (
            ...
            or _h5_serial(evolution_target) != _h5_serial(active)
            or option.inPlayArea != AreaType.ACTIVE
        ):
            continue
        if evolution.id == ARCHALUDON:
            continue
        if evolution.id not in _H5_ALTERNATE_EVOLUTION_ATTACKS:
            return None
        attack_id, _ = _H5_ALTERNATE_EVOLUTION_ATTACKS[evolution.id]
        attack = ALL_ATTACKS.get(attack_id)
        if (
            attack is None
...
def _h5_no_equal_or_higher_route(obs, active, target, target_prize):
    routes = _h5_route_attacks_before_evolution(obs, active, target)
    if routes is None:
        return False
    if any(damage >= target.hp for _, _, damage in routes):
        return False

    if not _h5_boss_options(obs):
        return True
    for bench_target in opp_bench_pokemon(obs):
        bench_prize = _h5_prize_value(bench_target)
...
def _h5_no_post_evolution_equal_or_higher_route(obs, target_prize):
    legal_attacks = _h5_attack_options(obs)
    if (
        not legal_attacks
        or any(
...
def _h5_build_certificate(obs, parent_choice):
    if (
        ...
        # H5 owns an evolution-to-attack conversion that is already complete
        # without this turn's attachment.  Once attachment has been consumed,
        # the state belongs to the separately reserved resource/promotion
        # hypotheses, so fail closed.
        or obs.current.energyAttached
        or not _h5_static_cards_supported()
        or not _h5_parent_is_inherited_raging(obs, parent_choice)
    ...
    if (
        ...
        or _h5_serial(active) is None
        or _h5_serial(target) is None
        or not _h5_visible_serial_is_unique(obs, active.serial)
        or not _h5_visible_serial_is_unique(obs, target.serial)
        or not _h5_exact_three_basic_metal(active)
        or not _h5_known_tools_only(active)
        or any(_h5_conditions(mine))
        or not _h5_public_board_supported(obs, target)
        or not _h5_persistent_effects_supported()
    ):
        return None

    evolution_options = _h5_evolution_options(
        obs, ARCHALUDON, active.serial
    )
    if not evolution_options:
        return None
    evolution_serial = min(serial for _, serial in evolution_options)
    if not _h5_visible_serial_is_unique(obs, evolution_serial):
        return None
    ...
    active_prize = _h5_prize_value(active)
    evolution_prize = _h5_prize_value(evolution_card)
    target_prize = _h5_prize_value(target)
    our_prizes = len(mine.prize or ())
    if (
        active_prize != 1
        or evolution_prize != 1
        or target_prize != 1
        or our_prizes <= target_prize
    ):
        return None

    inherited_damage = _h5_exact_damage(
        obs,
        DURALUDON,
        RAGING_HAMMER,
        target,
    ...
    coated_damage = _h5_exact_damage(
    ...
    if (
        ...
        or not _h5_no_equal_or_higher_route(
    ...
    expected_hand = list(_h5_hand_fingerprint(mine))
    ...
    return {
        ...
        "active": _h5_pokemon_fingerprint(active),
        ...
        "target": _h5_pokemon_fingerprint(target),
        ...
        "public_modifiers": _h5_public_modifiers(obs),
        "opponent_board": _h5_board_fingerprint(opp),
        "mine_bench": _h5_bench_fingerprint(mine),
        "hand": _h5_hand_fingerprint(mine),
...
def _h5_pre_evolution_valid(obs, parent_choice, transaction):
    mine = my_state(obs)
    opp = opp_state(obs)
    active = active_pokemon(obs)
    target = opp_active_pokemon(obs)
    choices = _h5_evolution_options(
    ...
    return (
        ...
        and (len(mine.prize or ()), len(opp.prize or ()))
        == transaction["prizes"]
        and _h5_pokemon_fingerprint(active) == transaction["active"]
        and _h5_pokemon_fingerprint(target) == transaction["target"]
        and _h5_board_fingerprint(opp)
        == transaction["opponent_board"]
        and _h5_bench_fingerprint(mine) == transaction["mine_bench"]
        and _h5_hand_fingerprint(mine) == transaction["hand"]
        and _h5_public_modifiers(obs)
        ...
        and _h5_parent_is_inherited_raging(obs, parent_choice)
        and any(
...
def _h5_evolution_confirmed(obs, transaction):
    mine = my_state(obs)
    opp = opp_state(obs)
    active = active_pokemon(obs)
    target = opp_active_pokemon(obs)
    if (
        ...
        or _h5_serial(active) != transaction["evolution_serial"]
        or obs.current.turnActionCount
        != transaction["turn_action_count"] + 1
        or (len(mine.prize or ()), len(opp.prize or ()))
        != transaction["prizes"]
        or _h5_pokemon_fingerprint(target) != transaction["target"]
        or _h5_board_fingerprint(opp)
        != transaction["opponent_board"]
        or _h5_bench_fingerprint(mine) != transaction["mine_bench"]
        or _h5_hand_fingerprint(mine)
        != transaction["expected_post_evolution_hand"]
        or _h5_public_modifiers(obs)
        != transaction["public_modifiers"]
        ...
        or not any(
            prior.id == DURALUDON
            and _h5_serial(prior) == transaction["active_serial"]
            for prior in (getattr(active, "preEvolution", None) or ())
        )
        or not _h5_visible_serial_is_unique(
...
def _h5_post_evolution_valid(obs, transaction):
    active = active_pokemon(obs)
    target = opp_active_pokemon(obs)
    choices = _h5_attack_options(obs, COATED_ATTACK)
    if (
        obs.select.context not in {
            SelectContext.MAIN,
        ...
        or active is None
        or target is None
        or _h5_prize_value(active) != 1
        or _h5_prize_value(target) != transaction["target_prize"]
        or len(my_state(obs).prize or ()) <= transaction["target_prize"]
    ):
        return False
    damage = _h5_exact_damage(
        obs, ARCHALUDON, COATED_ATTACK, target
    ...
    return (
        ...
        and _h5_no_post_evolution_equal_or_higher_route(
            obs, transaction["target_prize"]
        )
    )


def _h5_attack_confirmed(obs, transaction):
    return any(
...
def _h5_choose(obs, parent_choice):
    global _h5_transaction
    transaction = _h5_transaction
    if transaction is None:
        certificate = _h5_build_certificate(obs, parent_choice)
        if certificate is None:
            return None
        _h5_transaction = certificate
    ...
    if (
        ...
        _h5_reset()
        return None
    if _h5_attack_confirmed(obs, transaction):
    ...
    if transaction["stage"] == "ARMED":
        ...
        if (
            ...
            and _h5_serial(active) == transaction["evolution_serial"]
        ):
            if not _h5_evolution_confirmed(obs, transaction):
                _h5_reset()
                return None
            transaction["stage"] = "EVOLVED"
        else:
            if not _h5_pre_evolution_valid(
    ...
    if transaction["stage"] in {"EVOLVED", "ATTACK_READY"}:
        if (
            not _h5_evolution_confirmed(obs, transaction)
            or not _h5_post_evolution_valid(obs, transaction)
        ):
            _h5_reset()
            return None
        choices = _h5_attack_options(obs, COATED_ATTACK)
...
def _h5_safe_choose(obs, parent_choice):
    try:
        return _h5_choose(obs, parent_choice)
    except Exception:
        _h5_reset()
        return None
...
def choose_options(obs):
    ...
    h5_choice = _h5_safe_choose(obs, parent_choice)
    return parent_choice if h5_choice is None else h5_choice
