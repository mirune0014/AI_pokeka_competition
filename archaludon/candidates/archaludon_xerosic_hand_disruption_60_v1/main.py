...
# ---------------------------------------------------------------------------
# Candidate D: Xerosic hand-disruption overlay
#
# This section is deliberately appended after the accepted-parent callable.
# The copied parent remains byte-for-byte intact; the final callable below
# calls it exactly once and only emits a certified Xerosic PLAY/card choice.
# Every uncertainty falls back to the accepted parent action.
# ---------------------------------------------------------------------------
import hashlib as _hashlib
import json as _json


_XEROSIC_CARD_ID = 1197
_XEROSIC_EXACT_NAME = "Xerosic’s Machinations"
_XEROSIC_EXACT_TEXT = (
    "Your opponent discards cards from their hand until they have 3 cards in their hand."
)
_XEROSIC_EXACT_OBJECT_SHA256 = (
    "4DE75F769AD237BD3A0E5BA6F386FFD1DD57B7CCB884B19B6B42428C75300F9B"
)
_XEROSIC = "XEROSIC_HAND_DISRUPTION_V1"
_XEROSIC_MODES = ("ALAKAZAM_SAFETY", "LARGE_HAND")
_XEROSIC_TELEMETRY = {
    "rule_id": _XEROSIC,
    "selected_source": "HISTORICAL_SILVER_PARENT",
    "mode": None,
    ...
    "safety_plays": 0,
    "large_hand_plays": 0,
    "pokégear_to_xerosic_plays": 0,
    "post_effect_opponent_hand_count": None,
    "rejection_reason": "not_called",
    "parent_call_count": 0,
}
_XEROSIC_PENDING = None
_ACCEPTED_PARENT_AGENT = agent


def _xerosic_registry_exact():
    """Verify the local CARD_DB and engine-visible effect contract exactly."""
    try:
        card = _parent.CARD_DB.get(_XEROSIC_CARD_ID)
        ...
        object_hash = _hashlib.sha256(payload).hexdigest().upper()
        return bool(
            card is not None
            and getattr(card, "cardId", None) == _XEROSIC_CARD_ID
            and getattr(card, "name", None) == _XEROSIC_EXACT_NAME
            and getattr(card, "cardType", None) == _CardType.SUPPORTER
            and isinstance(skills, list)
            and len(skills) == 1
            and getattr(skills[0], "name", None) == _XEROSIC_EXACT_NAME
            and getattr(skills[0], "text", None) == _XEROSIC_EXACT_TEXT
            and getattr(card, "attacks", None) == []
            and object_hash == _XEROSIC_EXACT_OBJECT_SHA256
        )
    except Exception:
        return False
...
def exact_xerosic_post_hand_count(opponent_hand_count):
    """Return the exact post-effect hand count, or None when unprovable."""
    if not _xerosic_registry_exact():
        return None
    if not _is_exact_int(opponent_hand_count) or opponent_hand_count < 0:
        return None
    # The local CARD_DB text says “until they have 3”; discard is automatic.
...
def _d_xerosic_options(obs, rows):
    if rows is None or not _xerosic_registry_exact():
        return ()
    found = []
    for position, option, role in rows:
        ...
        if (
            card is not None
            and getattr(card, "id", None) == _XEROSIC_CARD_ID
...
def _d_exact_current_ko(obs, rows):
    """Return True/False only when every legal attack is exactly classified."""
    try:
        seat = obs.current.yourIndex
        mine = obs.current.players[seat]
        theirs = obs.current.players[1 - seat]
        attacker = mine.active[0] if isinstance(mine.active, list) and mine.active else None
        target = theirs.active[0] if isinstance(theirs.active, list) and theirs.active else None
    except Exception:
        return None
    if attacker is None or target is None or rows is None:
        return None
    attack_rows = [row for row in rows if getattr(row[1], "type", None) == _OptionType.ATTACK]
    if not attack_rows:
        return False
    unknown = False
    for row in attack_rows:
        attack_id = getattr(row[1], "attackId", None)
        try:
            result = _exact_damage_and_take(obs, attacker, target, attack_id)
        except Exception:
            result = None
...
def _d_powerful_hand_ready(obs):
    try:
...
def _d_trigger_mode(obs, rows):
    """Return SAFETY/LARGE_HAND only for a public, exact proof."""
    if not _xerosic_registry_exact() or rows is None:
        return None, "registry_or_rows_unknown", None
    current = getattr(obs, "current", None)
    select = getattr(obs, "select", None)
    if (
        current is None
        or select is None
        or getattr(select, "context", None) != _SelectContext.MAIN
        or getattr(select, "type", None) != _SelectType.MAIN
        or getattr(current, "result", None) != -1
        or getattr(current, "supporterPlayed", None) is not False
    ):
        return None, "not_exact_main", None
    try:
        ...
        return None, "board_unknown", None
    if (
        own_active is None
        or opp_active is None
        or getattr(opp_active, "id", None) != 743
        or not isinstance(own_bench, list)
        or not isinstance(opp_bench, list)
        or own_bench
        or not any(p is not None for p in opp_bench)
        or not _is_exact_int(own_prizes)
        or not 1 <= own_prizes <= 6
        or not _is_exact_int(opp_hand_count)
        or opp_hand_count < 0
        or not _is_exact_int(own_hp)
        or own_hp <= 0
    ):
        return None, "public_gate_failed", None
    exact_ko = _d_exact_current_ko(obs, rows)
    if exact_ko is not False:
        return None, "current_ko_unknown_or_present", exact_ko
    post_hand = exact_xerosic_post_hand_count(opp_hand_count)
    if post_hand is None:
        return None, "post_effect_unknown", exact_ko
    minimum_next_hand = post_hand + 1
    minimum_powerful_hand = minimum_next_hand * 20
    safety = (
        own_prizes in {4, 5, 6}
        and _d_powerful_hand_ready(obs)
        and minimum_powerful_hand < own_hp
    )
    large_hand = (
        own_prizes >= 2
        and opp_hand_count >= 8
        and opp_hand_count - post_hand >= 5
    )
    if safety:
        return "ALAKAZAM_SAFETY", "exact_alakazam_loss_avoided", exact_ko
    if large_hand:
        return "LARGE_HAND", "large_hand_reduction_at_least_five", exact_ko
    return None, "trigger_not_met", exact_ko


def _d_parent_kind(obs, parent_action, rows):
    """Classify parent action; unknown/setup/transaction is fail-closed."""
    if rows is None or not isinstance(parent_action, list) or len(parent_action) != 1:
        return "UNKNOWN"
    position = parent_action[0]
    if not _is_exact_int(position) or position < 0 or position >= len(rows):
        return "UNKNOWN"
    option = rows[position][1]
    option_type = getattr(option, "type", None)
    ...
    if option_type == _OptionType.PLAY:
        try:
            ...
            card = None
        return "LILLIE" if getattr(card, "id", None) == _LILLIE else "SETUP"
    return "SETUP"


def _d_safe_to_override_parent(obs, parent_action, rows, mode):
    if _materialization_owner is not None or _setup_ledger is not None:
        return False
    kind = _d_parent_kind(obs, parent_action, rows)
    if mode == "ALAKAZAM_SAFETY":
        return kind in {"ATTACK", "END", "LILLIE"}
    return kind in {"ATTACK", "END"}


def _d_update_telemetry(**updates):
    global _XEROSIC_TELEMETRY
    state = dict(_XEROSIC_TELEMETRY)
    state.update(updates)
    _XEROSIC_TELEMETRY = state


def get_xerosic_telemetry():
    return dict(_XEROSIC_TELEMETRY)


def _d_action_is_legal(obs, action):
    """Check the parent result before handing it to the engine.

    The accepted parent is called exactly once, but its older transaction
    layer can occasionally return a stale one-item selection after the deck
    composition changes.  Returning that stale index is a mechanical engine
    fault, not a strategic fallback.  This narrow check never changes a
    legal parent action.
    """
    try:
        options = getattr(getattr(obs, "select", None), "option", None)
        minimum = getattr(obs.select, "minCount", None)
        maximum = getattr(obs.select, "maxCount", None)
        if not isinstance(options, list) or not isinstance(action, list):
            return False
        if not _is_exact_int(minimum) or not _is_exact_int(maximum):
            return False
        if len(action) < minimum or len(action) > maximum:
            return False
        if len(set(action)) != len(action):
            return False
        return all(_is_exact_int(index) and 0 <= index < len(options) for index in action)
    except Exception:
        return False


def _d_repair_invalid_parent(obs, parent_action):
    """Recover only a mechanically invalid parent selection.

    Prefer the accepted parent's own chooser over inventing a strategic
    action.  If that chooser is unavailable, select the first legal minimum
    options as a final validity guard; this branch is telemetry-visible and
    is never used for a legal parent result.
    """
    if _d_action_is_legal(obs, parent_action):
        return parent_action, False
    try:
        repaired = _parent.choose_options(obs)
    except Exception:
        repaired = None
    if _d_action_is_legal(obs, repaired):
        return repaired, True
    try:
        options = getattr(obs.select, "option", None)
        minimum = int(obs.select.minCount)
        if isinstance(options, list) and minimum >= 0 and minimum <= len(options):
            fallback = list(range(minimum))
            if _d_action_is_legal(obs, fallback):
                return fallback, True
    except Exception:
        pass
    return parent_action, True


def agent(obs_dict):
    """Accepted parent once, then one exact Xerosic resolver."""
    global _XEROSIC_PENDING
    parent_action = _ACCEPTED_PARENT_AGENT(obs_dict)
    _d_update_telemetry(
        selected_source="HISTORICAL_SILVER_PARENT",
        mode=None,
        registry_exact=False,
    ...
    try:
        obs = _to_observation_class(obs_dict)
        parent_action, parent_repaired = _d_repair_invalid_parent(obs, parent_action)
        if parent_repaired:
            _d_update_telemetry(
                selected_source="PARENT_MECHANICAL_REPAIR",
                rejection_reason="invalid_parent_action_repaired",
                parent_repaired=True,
            )
        registry_exact = _xerosic_registry_exact()
        _d_update_telemetry(registry_exact=registry_exact)
        if not registry_exact or obs.select is None or obs.current is None:
            _XEROSIC_PENDING = None
            return parent_action
        rows = _d_rows(obs)
        x_options = _d_xerosic_options(obs, rows)
        if x_options:
            _d_update_telemetry(drawn=True, legal=True)
        # A Pokégear/search card prompt may expose Xerosic without being MAIN.
        # Reuse the same public trigger proof; no hidden hand contents are read.
        if getattr(obs.select, "context", None) != _SelectContext.MAIN:
            if x_options and _XEROSIC_PENDING in _XEROSIC_MODES:
                position = min(row[0] for row in x_options)
                _d_update_telemetry(
                    selected_source=_XEROSIC,
                    mode=_XEROSIC_PENDING,
                    played=True,
                    pokégear_to_xerosic_plays=(_XEROSIC_TELEMETRY.get("pokégear_to_xerosic_plays", 0) + 1),
                    post_effect_opponent_hand_count=(
                        exact_xerosic_post_hand_count(obs.current.players[1 - obs.current.yourIndex].handCount)
                    ),
                    rejection_reason="pokégear_xerosic_selected",
                )
                _XEROSIC_PENDING = None
                return [position]
            return parent_action
        mode, reason, exact_ko = _d_trigger_mode(obs, rows)
        if mode is None:
            _XEROSIC_PENDING = None
            _d_update_telemetry(rejection_reason=reason)
            return parent_action
        if not x_options:
            # A direct hand play is unavailable.  Keep the mode for an
            # immediately-following Pokégear card prompt, otherwise parent.
            _XEROSIC_PENDING = mode
            _d_update_telemetry(mode=mode, rejection_reason="awaiting_search_option")
            return parent_action
        if not _d_safe_to_override_parent(obs, parent_action, rows, mode):
            _XEROSIC_PENDING = None
            _d_update_telemetry(mode=mode, rejection_reason="setup_or_transaction_priority")
            return parent_action
        position = min(row[0] for row in x_options)
        post_hand = exact_xerosic_post_hand_count(
            obs.current.players[1 - obs.current.yourIndex].handCount
        )
        update = {
            "selected_source": _XEROSIC,
            "mode": mode,
            "played": True,
            "post_effect_opponent_hand_count": post_hand,
            "rejection_reason": reason,
        }
        if mode == "ALAKAZAM_SAFETY":
            update["safety_plays"] = _XEROSIC_TELEMETRY.get("safety_plays", 0) + 1
        else:
            update["large_hand_plays"] = _XEROSIC_TELEMETRY.get("large_hand_plays", 0) + 1
        _d_update_telemetry(**update)
        _XEROSIC_PENDING = None
        return [position]
    except Exception as exc:
        _XEROSIC_PENDING = None
        _d_update_telemetry(rejection_reason="wrapper_exception:" + type(exc).__name__)
        return parent_action


# ---------------------------------------------------------------------------
# Candidate D continuation repair: Pokégear -> bound Xerosic transaction.
#
# This is the final resolver.  The earlier D resolver is retained as a
# historical implementation record, but is intentionally shadowed here so
# that only one callable is exported.  The continuation owns no hidden hand
# or deck state and never deep-copies the parent module.
# ---------------------------------------------------------------------------
_POKEGEAR_CARD_ID = 1122
_POKEGEAR_EXACT_NAME = "Pokégear 3.0"
_POKEGEAR_EXACT_TEXT = (
...
_POKEGEAR_EXACT_OBJECT_SHA256 = (
    "F9AA7371702F114FE4FE3B61815609C5C9DE7730A04250BBCDFEFF87801E6D6C"
)
_POKEGEAR_XEROSIC_OWNER = "POKEGEAR_XEROSIC_COMMITMENT"
_POKEGEAR_STAGES = (
    "IDLE",
...
_POKEGEAR_CALLBACK_LIMIT = 3
_POKEGEAR_COMMITMENT = None
_PARENT_COMMITMENT_STATE_FIELDS = ("_materialization_owner",)


def _d_object_sha(card):
    try:
        payload = _json.dumps(
            getattr(card, "__dict__", None),
            ensure_ascii=False,
        ...
        return _hashlib.sha256(payload).hexdigest().upper()
    except Exception:
        return None


def _pokegear_registry_exact():
    try:
        card = _parent.CARD_DB.get(_POKEGEAR_CARD_ID)
        skills = getattr(card, "skills", None)
        return bool(
            card is not None
            and getattr(card, "cardId", None) == _POKEGEAR_CARD_ID
            and getattr(card, "name", None) == _POKEGEAR_EXACT_NAME
            and getattr(card, "cardType", None) == _CardType.ITEM
            and isinstance(skills, list)
            and len(skills) == 1
            and getattr(skills[0], "name", None) == _POKEGEAR_EXACT_NAME
            and getattr(skills[0], "text", None) == _POKEGEAR_EXACT_TEXT
            and getattr(card, "attacks", None) == []
            and _d_object_sha(card) == _POKEGEAR_EXACT_OBJECT_SHA256
        )
    except Exception:
        return False


def _snapshot_parent_commitment_state():
    """Snapshot only the audited action-commitment allowlist.

    `_setup_ledger` is a setup transaction and is deliberately not touched;
    `_parent` turn/log fields are also not guessed or copied.  The only state
    that the Xerosic override may roll back is this module's materialization
    owner, which can otherwise retain a parent action commitment after the
    action is replaced.
    """
    return {"_materialization_owner": _copy.deepcopy(_materialization_owner)}


def _restore_parent_commitment_state(snapshot):
    global _materialization_owner
    if not isinstance(snapshot, dict) or "_materialization_owner" not in snapshot:
        return False
    _materialization_owner = _copy.deepcopy(snapshot["_materialization_owner"])
    return True


def _d_action_role(obs, rows, action):
...
def _d_action_card_id(obs, rows, action):
    role = _d_action_role(obs, rows, action)
    if role is None or role[1] is None:
        return None
    return role[1][0]


def _d_parent_pokegear_ref(obs, rows, parent_action):
    if _d_action_card_id(obs, rows, parent_action) != _POKEGEAR_CARD_ID:
        return None
    role = _d_action_role(obs, rows, parent_action)
    option = rows[parent_action[0]][1]
    if role is None or role[1] is None or getattr(option, "type", None) != _OptionType.PLAY:
        return None
    return role[1]


def _d_pokegear_prompt_exact(obs, rows):
    if not _pokegear_registry_exact() or rows is None:
        return False
    select = getattr(obs, "select", None)
    ...
    return bool(
        select is not None
        and getattr(select, "type", None) == _SelectType.CARD
        and getattr(select, "context", None) == _SelectContext.TO_HAND
        and getattr(select, "minCount", None) == 0
        and getattr(select, "maxCount", None) == 1
        and effect_ref is not None
        and effect_ref[0] == _POKEGEAR_CARD_ID
        and getattr(select, "contextCard", None) is None
        and getattr(select, "deck", None) is None
    )


def _d_bound_xerosic_play(obs, rows, commitment):
    if rows is None or not isinstance(commitment, dict):
        return None
    bound = commitment.get("xerosic_serial")
    seat = commitment.get("player_index")
    matches = []
    for row in rows:
        pos, option, role = row
        ref = role[1] if role is not None else None
        if (
            getattr(option, "type", None) == _OptionType.PLAY
            and ref == (_XEROSIC_CARD_ID, bound, seat)
        ):
            matches.append(row)
    return matches[0] if len(matches) == 1 else None


def _d_xerosic_bound_in_hand(obs, commitment):
    try:
        seat = commitment["player_index"]
        hand = obs.current.players[seat].hand
        refs = tuple(_exact_card_ref(card, seat) for card in hand)
        return commitment.get("xerosic_serial") in {ref[1] for ref in refs if ref is not None}
    except Exception:
        return None


def _d_bind_state_matches(obs, commitment):
    try:
        current = obs.current
        seat = commitment["player_index"]
        if (
            current.yourIndex != seat
            or current.turn != commitment["turn"]
            or current.result != -1
            or current.players[seat].active[0].serial != commitment["own_active_serial"]
            or current.players[1 - seat].active[0].serial != commitment["opponent_active_serial"]
            or len(current.players[seat].bench) != 0
            or len([p for p in current.players[1 - seat].bench if p is not None]) == 0
            or len(current.players[seat].prize) != commitment["own_prize_count"]
            or current.players[1 - seat].handCount != commitment["opponent_hand_count_at_bind"]
        ):
            return False
        return True
    except Exception:
...
def _d_commitment_from_pending(obs, pending, rows, x_options):
    if not isinstance(pending, dict) or pending.get("mode") not in _XEROSIC_MODES:
        return None
    if not _d_pokegear_prompt_exact(obs, rows) or not x_options:
        return None
    current = obs.current
    seat = current.yourIndex
    try:
        active = current.players[seat].active[0]
        opponent_active = current.players[1 - seat].active[0]
        selected = min(x_options, key=lambda row: (row[0], row[2][1][1]))
        serial = selected[2][1][1]
    except Exception:
        return None
    if not _d_bind_state_matches(
        obs,
        {
            "player_index": seat,
            "turn": pending.get("turn"),
            "own_active_serial": pending.get("own_active_serial"),
            "opponent_active_serial": pending.get("opponent_active_serial"),
            "own_prize_count": pending.get("own_prize_count"),
            "opponent_hand_count_at_bind": pending.get("opponent_hand_count_at_bind"),
        },
    ):
        return None
    return {
        "owner": _POKEGEAR_XEROSIC_OWNER,
        "stage": "WAIT_MAIN_FOR_XEROSIC",
        "player_index": seat,
        "turn": current.turn,
        "started_step": getattr(current, "turnActionCount", None),
        "callback_count": 0,
        "mode": pending["mode"],
        "pokegear_serial": pending.get("pokegear_serial"),
        "xerosic_serial": serial,
        "opponent_active_serial": opponent_active.serial,
        "own_active_serial": active.serial,
        "opponent_hand_count_at_bind": opponent_active and current.players[1 - seat].handCount,
        "own_prize_count": len(current.players[seat].prize),
        "expected_xerosic_card_id": _XEROSIC_CARD_ID,
        "expected_xerosic_name": _XEROSIC_EXACT_NAME,
        "expected_xerosic_text_hash": _hashlib.sha256(
            _XEROSIC_EXACT_TEXT.encode("utf-8")
        ).hexdigest().upper(),
        "expected_xerosic_object_sha256": _XEROSIC_EXACT_OBJECT_SHA256,
        "parent_commitment_before": pending.get("parent_commitment_before"),
        "last_stage_reason": "gear_selection_bound",
    }


def _d_release_commitment(reason):
    global _POKEGEAR_COMMITMENT, _XEROSIC_PENDING
    old = _POKEGEAR_COMMITMENT
    _POKEGEAR_COMMITMENT = None
    _XEROSIC_PENDING = None
    _d_update_telemetry(rejection_reason=reason, commitment_stage="IDLE")
    return old


def _d_confirm_xerosic_play(obs, commitment):
    try:
        seat = commitment["player_index"]
        current = obs.current
        supporter_done = current.yourIndex == seat and current.supporterPlayed is True
        hand_gone = _d_xerosic_bound_in_hand(obs, commitment) is False
        played_log = any(
            _log_exact(
                log,
                _LogType.PLAY,
                {
                    "playerIndex": seat,
                    "cardId": _XEROSIC_CARD_ID,
                    "serial": commitment["xerosic_serial"],
                },
            )
            for log in getattr(obs, "logs", ())
        )
        return bool(supporter_done and hand_gone and played_log)
    except Exception:
        return False


def _d_continuation(obs, parent_action, parent_before, parent_after, rows, x_options):
    """Resolve one bound Pokégear continuation after the parent call."""
    global _POKEGEAR_COMMITMENT
    commitment = _POKEGEAR_COMMITMENT
    if not isinstance(commitment, dict):
        return None
    commitment["callback_count"] = commitment.get("callback_count", 0) + 1
    if commitment["callback_count"] > 4:
        _restore_parent_commitment_state(
            commitment.get("parent_commitment_before")
        )
        _d_release_commitment("continuation_callback_budget_exceeded")
        return None
    current = obs.current
    if current.result != -1 or current.yourIndex != commitment["player_index"]:
        _restore_parent_commitment_state(
            commitment.get("parent_commitment_before")
        )
        _d_release_commitment("continuation_turn_or_result_changed")
        return None
    stage = commitment.get("stage")
    if stage == "WAIT_MAIN_FOR_XEROSIC":
        if (
            commitment["callback_count"] > _POKEGEAR_CALLBACK_LIMIT
            or
            current.turn != commitment["turn"]
            or getattr(obs.select, "type", None) != _SelectType.MAIN
            or getattr(obs.select, "context", None) != _SelectContext.MAIN
            or current.supporterPlayed is not False
            or not _d_bind_state_matches(obs, commitment)
        ):
            _restore_parent_commitment_state(
                commitment.get("parent_commitment_before")
            )
            _d_release_commitment("continuation_main_prompt_not_exact")
            return None
        mode, reason, _ko = _d_trigger_mode(obs, rows)
        bound = _d_bound_xerosic_play(obs, rows, commitment)
        if mode != commitment.get("mode") or bound is None:
            _restore_parent_commitment_state(
                commitment.get("parent_commitment_before")
            )
            _d_release_commitment("continuation_trigger_or_bound_serial_failed")
            return None
        _restore_parent_commitment_state(commitment.get("parent_commitment_before"))
        commitment["stage"] = "WAIT_XEROSIC_RESOLUTION"
        commitment["last_stage_reason"] = reason
        _d_update_telemetry(
            selected_source=_XEROSIC,
            mode=commitment.get("mode"),
            played=True,
            pokégear_to_xerosic_plays=(
                _XEROSIC_TELEMETRY.get("pokégear_to_xerosic_plays", 0) + 1
            ),
            post_effect_opponent_hand_count=exact_xerosic_post_hand_count(
                current.players[1 - current.yourIndex].handCount
            ),
            rejection_reason="pokégear_xerosic_main_play",
            commitment_stage=commitment["stage"],
        )
        return [bound[0]]
    if stage == "WAIT_XEROSIC_RESOLUTION":
        if _d_confirm_xerosic_play(obs, commitment):
            _d_release_commitment("xerosic_play_confirmed")
            return None
        # The effect has no player-owned selection.  A different prompt,
        # opponent callback, or parent turn advance closes this owner.
        _d_release_commitment("xerosic_resolution_observed_or_unknown")
        return None
    _d_release_commitment("continuation_stage_unknown")
    return None


def _d_start_pokegear_pending(obs, rows, parent_action, parent_before, mode):
    global _XEROSIC_PENDING
    ref = _d_parent_pokegear_ref(obs, rows, parent_action)
    if ref is None:
        _XEROSIC_PENDING = None
        return False
    seat = obs.current.yourIndex
    _XEROSIC_PENDING = {
        ...
        "player_index": seat,
        "turn": obs.current.turn,
        "started_step": getattr(obs.current, "turnActionCount", None),
        "callback_count": 0,
...
def _d_final_agent(obs_dict):
    global _XEROSIC_PENDING, _POKEGEAR_COMMITMENT
    parent_before = _snapshot_parent_commitment_state()
    parent_action = _ACCEPTED_PARENT_AGENT(obs_dict)
    parent_after = _snapshot_parent_commitment_state()
    _d_update_telemetry(
        selected_source="HISTORICAL_SILVER_PARENT",
        mode=None,
        registry_exact=False,
        ...
        parent_call_count=1,
        commitment_stage=(
            _POKEGEAR_COMMITMENT.get("stage")
            if isinstance(_POKEGEAR_COMMITMENT, dict)
            else "IDLE"
        ),
    )
    try:
        ...
        if not _xerosic_registry_exact() or not _pokegear_registry_exact():
            _d_release_commitment("registry_unknown")
            return parent_action
        rows = _d_rows(obs)
        if rows is None or obs.select is None or obs.current is None:
            _d_release_commitment("rows_or_observation_unknown")
            return parent_action
        x_options = _d_xerosic_options(obs, rows)
        if x_options:
            _d_update_telemetry(drawn=True, legal=True)

        continuation_action = _d_continuation(
            obs, parent_action, parent_before, parent_after, rows, x_options
        )
        if continuation_action is not None:
            return continuation_action
        # If a continuation released above, do not let its old pending token
        # leak into a later prompt.
        if _POKEGEAR_COMMITMENT is not None:
            return parent_action

        if _d_pokegear_prompt_exact(obs, rows):
            if isinstance(_XEROSIC_PENDING, dict) and _d_bind_state_matches(
                obs, _XEROSIC_PENDING
            ):
                commitment = _d_commitment_from_pending(
                    obs, _XEROSIC_PENDING, rows, x_options
                )
                if commitment is not None:
                    _restore_parent_commitment_state(
                        commitment.get("parent_commitment_before")
                    )
                    _POKEGEAR_COMMITMENT = commitment
                    _XEROSIC_PENDING = None
                    position = min(
                        row[0]
                        for row in x_options
                        if row[2][1][1] == commitment["xerosic_serial"]
                    )
                    _d_update_telemetry(
                        selected_source=_XEROSIC,
                        ...
                        drawn=True,
                        legal=True,
                        rejection_reason="gear_selection_bound",
                        commitment_stage="WAIT_MAIN_FOR_XEROSIC",
                    )
                    return [position]
            _XEROSIC_PENDING = None
            _d_update_telemetry(rejection_reason="pokegear_proof_failed")
            return parent_action

        if getattr(obs.select, "context", None) != _SelectContext.MAIN:
            _XEROSIC_PENDING = None
            return parent_action
        mode, reason, _exact_ko = _d_trigger_mode(obs, rows)
        if mode is None:
            _XEROSIC_PENDING = None
            _d_update_telemetry(rejection_reason=reason)
            return parent_action
        if not x_options:
            if _d_start_pokegear_pending(
                obs, rows, parent_action, parent_before, mode
            ):
                _d_update_telemetry(
                    mode=mode,
                    rejection_reason="awaiting_pokegear_xerosic_option",
                )
            else:
                _XEROSIC_PENDING = None
                _d_update_telemetry(rejection_reason="no_direct_xerosic_or_pokegear")
            return parent_action
        if not _d_safe_to_override_parent(obs, parent_action, rows, mode):
            _XEROSIC_PENDING = None
            _d_update_telemetry(mode=mode, rejection_reason="setup_or_transaction_priority")
            return parent_action
        position = min(row[0] for row in x_options)
        _restore_parent_commitment_state(parent_before)
        post_hand = exact_xerosic_post_hand_count(
            obs.current.players[1 - obs.current.yourIndex].handCount
        )
        update = {
        ...
        _d_release_commitment("wrapper_exception:" + type(exc).__name__)
        return parent_action


# Loader contract: this must remain the final callable insertion.
agent = _d_final_agent
