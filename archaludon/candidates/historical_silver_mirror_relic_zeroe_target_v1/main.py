...
def score_target(obs, opt):
    ...
    if ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
        ...
        if pi != yi and card:
            ...
            if (
                detect_matchup(obs) == "archaludon"
                and cid == DURALUDON
                and energy_count(card) == 0
            ):
                attacks = planned_archaludon_attacks(obs)
                opponent_active = opp_active_pokemon(obs)
                active_ko = bool(opponent_active) and any(
                    effective_damage(attack["damage"], opponent_active)
                    >= opponent_active.hp
                    for attack in attacks
                )
                candidate_ko = any(
                    effective_damage(attack["damage"], card) >= card.hp
                    for attack in attacks
                )
                relicanth_ko = any(
                    target.id == RELICANTH
                    and any(
                        effective_damage(attack["damage"], target) >= target.hp
                        for attack in attacks
                    )
                    for target in opp_bench_pokemon(obs)
                )
                ...
                if (
                    ...
                    and candidate_ko
                    and relicanth_ko
                    and not lethal_bench
                ):
                    return 22400, "Boss: zero-E Duraludon below Relicanth"
            if detect_matchup(obs) == "ogerpon":
                if cid == 117:
