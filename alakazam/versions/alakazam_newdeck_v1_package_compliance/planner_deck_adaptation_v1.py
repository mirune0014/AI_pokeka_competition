...
def _powerful_hand_ko(parent, obs, hand_count):
    ...
    if (
        ...
        or not parent._powerful_hand_target_is_publicly_clear(obs.current, theirs.active[0])
...
def _base_transaction(rule, snap, obs, public, index, card, key):
    return {
        ...
        "stage": {RULE_BOSS: "await_boss_child", RULE_MINE: "await_mine_attack", RULE_ALAKAZAM: "await_alakazam_ability", RULE_XEROSIC: "await_xerosic_verify", RULE_LANA: "await_lana_child", RULE_HAMMER: "await_hammer_child"}[rule],
...
def _hammer_enables_current_ko(parent, obs, area, pokemon, energy_index):
    ...
    if (
        ...
        or virtual is None
        or parent._powerful_hand_target_is_publicly_clear(obs.current, pokemon)
        or not parent._powerful_hand_target_is_publicly_clear(obs.current, virtual)
...
def _candidate_hammer(parent, obs, snap, public):
    ...
    return [index], transaction
def _played_delta(parent, obs, transaction, opponent_changes=False):
    ...
    if current is None or expected_hand is None:
        return None
    unchanged = {"turn", "result", "stadium_played", "energy_attached", "retreated", "own_deck", "opponent_deck", "stadium", "fields"}
    if any(current[key] != start[key] for key in unchanged):
        return None
    if (
        current["action_count"] != start["action_count"] + 1
        or current["own_hand"] != expected_hand
        or current["own_hand_count"] != start["own_hand_count"] - 1
        or current["own_discard"] != start["own_discard"] + (transaction["card_row"],)
    ):
        return None
    if transaction["rule"] in (RULE_BOSS, RULE_BOSS_READY_STOP, RULE_XEROSIC, RULE_LANA):
    ...
    if not opponent_changes and (
        ...
        or current["opponent_discard"] != start["opponent_discard"]
    ):
        return None
    return current
...
def _advance_lana(parent, obs, transaction):
    ...
    if (
        _played_delta(parent, obs, transaction) is None or obs.current.yourIndex != owner
        or select.context != parent.SelectContext.TO_HAND or int(select.type) != 1
        or select.minCount != 1 or not 1 <= select.maxCount <= 3
        or _card_row(select.effect) != transaction["card_row"] or select.contextCard is not None
    ):
        return None
...
def _advance_hammer(parent, obs, transaction):
    ...
    if (
        _played_delta(parent, obs, transaction) is None or obs.current.yourIndex != owner
        or select.context != parent.SelectContext.DISCARD_ENERGY or int(select.type) != 4
    ...
    matches = []
    for option_index, option in enumerate(select.option):
        found = _find_pokemon(parent, obs, 1 - owner, transaction["pokemon_serial"])
        if (
            found is None or option.type != parent.OptionType.ENERGY
            or int(option.area) != transaction["energy_area"] or option.index != transaction["pokemon_index"]
            or option.playerIndex != 1 - owner or option.energyIndex != transaction["energy_index"]
            or option.count != 1
            or not _exact_option(option, parent.OptionType.ENERGY, area=parent.AreaType(transaction["energy_area"]), index=transaction["pokemon_index"], playerIndex=1 - owner, energyIndex=transaction["energy_index"], count=1)
            or found[0] != option.area or found[1] != option.index
            or not 0 <= option.energyIndex < len(found[2].energyCards)
            or _card_row(found[2].energyCards[option.energyIndex]) != transaction["energy_row"]
        ):
            continue
        matches.append((option_index, census[option_index]))
    if len(matches) != 1:
...
def _verify_lana(parent, obs, transaction):
    ...
    return bool(
        current is not None
        and current["own_hand_count"] == post["own_hand_count"] + transaction["recovery_count"]
        and sorted(current["own_hand"]) == sorted(post["own_hand"] + selected)
        and all(row not in current["own_discard"] for row in selected)
    )


def _verify_hammer(parent, obs, transaction):
    current = _public_state(parent, obs)
    found = _find_pokemon(parent, obs, 1 - transaction["owner"], transaction["pokemon_serial"])
    return bool(
        current is not None and found is not None
        and transaction["energy_row"] not in tuple(_card_row(card) for card in found[2].energyCards)
        and current["opponent_discard"].count(transaction["energy_row"]) == 1
    )


def _verify_xerosic(parent, obs, transaction):
    current = _played_delta(parent, obs, transaction, opponent_changes=True)
    return bool(
        current is not None and current["opponent_hand_count"] == 3
        and len(current["opponent_discard"]) == len(transaction["start"]["opponent_discard"]) + transaction["opponent_discards"]
    )
...
def agent(parent, v0_agent, obs_dict):
    """Call v0 exactly once unless rebinding an identical v1 callback."""
    global V1_TRANSACTION
    ...
    try:
        if not isinstance(obs_dict, dict) or obs_dict.get("select") is None:
            ...
            _trace(None, None, v0_action, None, "V0_FALLBACK")
            return v0_action
        ...
        if not runtime_model.raw_parsed_agree(obs_dict, obs):
            reset()
            v0_action = v0_agent(obs_dict)
            _trace(None, context, v0_action, None, "RAW_PARSED_MISMATCH", "V0_FALLBACK")
            return v0_action
        snap = runtime_model.public_snapshot(parent, obs)
        if snap is None or _option_census(parent, obs) is None:
            reset()
            v0_action = v0_agent(obs_dict)
            _trace(None, context, v0_action, None, "AMBIGUOUS_PUBLIC_METADATA", "V0_FALLBACK")
            return v0_action
        if snap.sha256 in core.INTEGRATED_DUPLICATE_CACHE:
            v0_action = v0_agent(obs_dict)
            _trace(snap.sha256, context, v0_action, None, "INHERITED_DUPLICATE_OWNER", "V0_FALLBACK")
            return v0_action
        if V1_TRANSACTION is None:
            V1_DUPLICATES.clear()
        duplicate = _rebind_duplicate(parent, obs, snap.sha256)
        if duplicate is not None:
            _trace(snap.sha256, context, duplicate, V1_TRANSACTION.get("rule") if V1_TRANSACTION else None, "V1_DUPLICATE_REBIND")
            return duplicate

        parent_pre = core.parent_state_snapshot(parent)
        had_parent_transaction = core.INTEGRATED_TRANSACTION is not None
        ...
        if removed_parent_transaction or removed is not None:
            ...
            return action
        inherited_owner = (
            had_parent_transaction or core.INTEGRATED_TRANSACTION is not None
            or core.parent_owner_active(parent_pre) or core.parent_owner_active(parent_post)
        )
        if inherited_owner:
            reset()
            _trace(snap.sha256, context, v0_action, None, "INHERITED_TRANSACTION_OWNER", "V0_FALLBACK")
            return v0_action
        ...
        if V1_TRANSACTION is not None:
            ...
            if stage == "await_boss_child":
                ...
                complete = _verify_added_attack(parent, obs, transaction)
            ...
            if action is not None and model.action_is_valid(obs, action):
                ...
                _trace(snap.sha256, context, action, transaction["rule"], "EXACT_CHILD_PROMPT", "SERIAL_REBOUND")
                return action
            abort_tag = _transaction_abort_tag(transaction) if not complete else None
            reset()
            tags = tuple(tag for tag in (abort_tag, "V1_TRANSACTION_COMPLETE" if complete else "V1_TRANSACTION_ABORT", "V0_FALLBACK") if tag is not None)
            _trace(snap.sha256, context, v0_action, None, *tags)
            return v0_action

        public = _main_envelope(parent, obs)
        if public is None:
            _trace(snap.sha256, context, v0_action, None, "MAIN_ENVELOPE_NOT_EXACT", "V0_FALLBACK")
            return v0_action
        _clear_compliance_block()
        if _v0_current_ko(parent, obs, v0_action):
            if _current_ko_is_terminal(parent, obs):
                _trace(snap.sha256, context, v0_action, None, "CURRENT_EXACT_TERMINAL_KO_PRECEDENCE", "V0_FALLBACK")
                return v0_action
            candidate = (
            ...
            if candidate is None:
                ...
                tags = tuple(tag for tag in (block_tag, "CURRENT_EXACT_KO_PRECEDENCE", "CURRENT_EXACT_NONTERMINAL_KO_PRESERVED", "V0_FALLBACK") if tag is not None)
                _trace(snap.sha256, context, v0_action, None, *tags)
                return v0_action
        else:
            candidate = _candidate_boss(parent, obs, snap, public) or _candidate_mine(parent, obs, snap, public) or _candidate_alakazam(parent, obs, snap, public) or _candidate_xerosic(parent, obs, snap, public) or _candidate_lana(parent, obs, snap, public) or _candidate_hammer(parent, obs, snap, public)
        if candidate is None:
            tags = ("UNKNOWN_NOT_IMPLEMENTED_FAIL_CLOSED", "V0_FALLBACK") if _unknown_present(parent, obs) else ("V0_NONFIRE_EXACT", "V0_FALLBACK")
            _trace(snap.sha256, context, v0_action, None, *tags)
            return v0_action
        action, transaction = candidate
        if not model.action_is_valid(obs, action):
            _trace(snap.sha256, context, v0_action, None, "V1_ACTION_INVALID", "V0_FALLBACK")
            return v0_action
        core.restore_parent_state(parent, parent_pre)
        V1_TRANSACTION = transaction
        _remember(parent, obs, snap.sha256, action)
        _trace(snap.sha256, context, action, transaction["rule"], *_candidate_trace_tags(transaction))
        return action
    except Exception as error:
        reset()
        if v0_action is None:
            v0_action = v0_agent(obs_dict)
        _trace(None, context, v0_action, None, f"V1_FAIL_CLOSED_{type(error).__name__}", "V0_FALLBACK")
        return v0_action
...
def _advance_boss_attack(parent, obs, transaction):
    ...
    if (
        ...
        or current["action_count"] != post["action_count"]
        or current["own_hand"] != post["own_hand"] or current["own_discard"] != post["own_discard"]
        ...
        return None
    action = [_attack_index(parent, obs)]
...
def _advance_alakazam_ability(parent, obs, transaction):
    ...
    if (
        ...
        return None
    census = _option_census(parent, obs)
    yes = [index for index, option in enumerate(select.option) if option.type == parent.OptionType.YES and _exact_option(option, parent.OptionType.YES)]
    if census is None or len(yes) != 1:
        return None
...
def _advance_alakazam_attack(parent, obs, transaction):
    ...
    if (
        ...
        or mine.deckCount != transaction["start_deck"] - 3 or mine.handCount != transaction["hand_after_draw"]
        ...
        or not set(post["own_hand"]).issubset(set(current["own_hand"])) or len(current["own_hand"]) - len(post["own_hand"]) != 3
        or len(mine.active) != 1 or mine.active[0].serial != transaction["attacker_serial"]
        or len(theirs.active) != 1 or theirs.active[0].serial != transaction["target_serial"]
        or not _powerful_hand_ko(parent, obs, mine.handCount)
    ):
...
def _verify_added_attack(parent, obs, transaction):
    found = _find_pokemon(parent, obs, 1 - transaction["owner"], transaction["target_serial"])
    return obs.current.result == transaction["owner"] or found is None or found[2].hp <= 0
...
def _advance_boss_attack(parent, obs, transaction):
    ...
    if (
        ...
        or not _boss_switch_public_delta_is_exact(current, post, transaction)
    ):
        return None
...
def _advance_backup_alakazam_attack(parent, obs, transaction):
    ...
    if (
        ...
        or current["energy_attached"] != post["energy_attached"]
        or current["retreated"] != post["retreated"]
        or current["own_hand_count"] != transaction["hand_after_draw"]
        or current["opponent_hand_count"] != post["opponent_hand_count"]
        or current["own_deck"] != transaction["start_deck"] - 3
        or current["opponent_deck"] != post["opponent_deck"]
        or current["own_discard"] != post["own_discard"]
        or current["opponent_discard"] != post["opponent_discard"]
        or current["stadium"] != post["stadium"]
        or current["fields"] != post["fields"]
        or not set(post["own_hand"]).issubset(set(current["own_hand"]))
        or len(current["own_hand"]) - len(post["own_hand"]) != 3
        or len(mine.active) != 1
        or mine.active[0].serial != transaction["attacker_serial"]
        or found is None
        or found[0] != parent.AreaType.BENCH
