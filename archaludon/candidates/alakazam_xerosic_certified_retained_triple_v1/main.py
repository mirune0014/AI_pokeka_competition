...
def _xerosic_has_psychic_energy(pokemon: Pokemon) -> bool:
    return any(
        energy.id in PSYCHIC_ENERGY_IDS for energy in pokemon.energyCards
    )


def _xerosic_next_turn_draw_is_safe(mine, draw_count: int) -> bool:
    """Certify a post-Xerosic draw after the mandatory next-turn draw."""
    return (
        isinstance(draw_count, int)
        and draw_count >= 0
        and mine.deckCount - 1 - draw_count > len(mine.prize)
    )


def _xerosic_attack_branch_score(
    mine,
    retained_cards: tuple[Card, ...],
    *,
    active_survives: bool,
) -> tuple[int, int, int, int]:
    """Score only publicly executable next-turn attack continuity.

    The four fields are ``route``, ``complete``,
    ``energized_kadabra_alakazam``, and ``energy_fills_route``.  The branch
    never assumes a hidden top-deck or a hidden prize identity.
    """
    retained_ids = [card.id for card in retained_cards]
    retained_alakazam = Alakazam in retained_ids
    retained_rare_candy = Rare_Candy in retained_ids
    retained_psychic = any(
        card.id in PSYCHIC_ENERGY_IDS for card in retained_cards
    )

    field = list(mine.bench)
    if active_survives:
        field = list(mine.active) + field
    field = [pokemon for pokemon in field if pokemon is not None]

    route = 0
    energized_kadabra_alakazam = 0
    energy_fills_route = 0

    for pokemon in field:
        energized = _xerosic_has_psychic_energy(pokemon)
        if pokemon.id == Alakazam:
            if energized:
                route = max(route, 6)
            elif retained_psychic:
                route = max(route, 5)
                energy_fills_route = 1
            else:
                route = max(route, 3)
    ...
    for pokemon in field:
        ...
        if pokemon.id == Kadabra:
            if retained_alakazam:
                if energized:
                    route = max(route, 6)
                    energized_kadabra_alakazam = 1
                elif retained_psychic:
                    route = max(route, 5)
                    energy_fills_route = 1
                else:
            ...
            if retained_alakazam and retained_rare_candy:
                if energized:
                    route = max(route, 6)
                elif retained_psychic:
                    route = max(route, 5)
                    ...
                    route = max(route, 4)
            elif energized or retained_psychic:
                route = max(route, 2)
                if not energized and retained_psychic:
                    energy_fills_route = 1
            else:
                route = max(route, 1)
    ...
    return (
        route,
        int(route >= 5),
        energized_kadabra_alakazam,
        energy_fills_route,
    )


def _xerosic_draw_route_score(
    mine,
    retained_cards: tuple[Card, ...],
    *,
    active_removed: bool,
) -> int:
    """Return the best certified public draw/search/recovery route."""
    retained_ids = [card.id for card in retained_cards]
    discard_ids = [card.id for card in mine.discard]
    bench_free_after_branch = mine.benchMax - len(mine.bench)
    if active_removed and mine.bench:
        # One Bench Pokemon must be promoted after the Active is removed.
        bench_free_after_branch += 1

    score = 0
    if Dawn in retained_ids and _xerosic_next_turn_draw_is_safe(mine, 3):
        score = max(score, 7)
    if Hilda in retained_ids and _xerosic_next_turn_draw_is_safe(mine, 2):
        score = max(score, 6)
    if (
        Night_Stretcher in retained_ids
        and any(
            card_id in ABRA_LINE
            or card_id in DUNSPARCE_LINE
            or card_id in PSYCHIC_ENERGY_IDS
    ...
    if (
        active_removed
        and Fezandipiti_ex in retained_ids
        and bench_free_after_branch > 0
        and _xerosic_next_turn_draw_is_safe(mine, 3)
    ):
        score = max(score, 5)
    if Poke_Pad in retained_ids and _xerosic_next_turn_draw_is_safe(mine, 1):
        score = max(score, 5)
    if (
        Buddy_Buddy_Poffin in retained_ids
        and bench_free_after_branch > 0
        and _xerosic_next_turn_draw_is_safe(mine, 2)
    ):
        score = max(score, 5)
    if (
        Sacred_Ash in retained_ids
        and sum(card_id in ABRA_LINE or card_id in DUNSPARCE_LINE
                for card_id in discard_ids) >= 2
    ):
        score = max(score, 5)
    if (
        Dudunsparce in retained_ids
        and any(pokemon.id == Dunsparce for pokemon in mine.bench)
        and _xerosic_next_turn_draw_is_safe(mine, 2)
    ):
        score = max(score, 4)
    return score
...
def _xerosic_disruption_score(state, retained_cards: tuple[Card, ...]) -> int:
    """Value disruption only when its public target is executable."""
    ...
    score = 0
    if Enhanced_Hammer in retained_ids:
        has_special_target = any(
            card_table.get(energy.id) is not None
            and card_table[energy.id].cardType == CardType.SPECIAL_ENERGY
            for pokemon in opponent_field
            for energy in pokemon.energyCards
        )
        if has_special_target:
            score = max(score, 3)
    if Boss_Orders in retained_ids and any(
        pokemon is not None for pokemon in theirs.bench
    ):
        score = max(score, 2)
    if Battle_Cage in retained_ids:
        current_stadium_id = state.stadium[0].id if state.stadium else None
        if current_stadium_id != Battle_Cage:
            score = max(score, 1)
    return score


def _xerosic_retained_triple_rank(
    obs: Observation,
    retained_cards: tuple[Card, ...],
) -> tuple:
    """Lexicographic, public-state-only retained-triple plan rank."""
    state = obs.current
    mine = state.players[state.yourIndex]
    retained_ids = [card.id for card in retained_cards]
    retained_counts = defaultdict(int)
    for card_id in retained_ids:
        retained_counts[card_id] += 1

    survive = _xerosic_attack_branch_score(
        mine, retained_cards, active_survives=True
    )
    removed = _xerosic_attack_branch_score(
        mine, retained_cards, active_survives=False
    )
    visible_kadabra = [
        pokemon
        for pokemon in mine.bench
        if pokemon is not None and pokemon.id == Kadabra
    ]
    alakazam_for_kadabra = int(
        Alakazam in retained_ids and bool(visible_kadabra)
    )
    energized_kadabra_alakazam = int(
        alakazam_for_kadabra
        and any(_xerosic_has_psychic_energy(pokemon)
                for pokemon in visible_kadabra)
    )

    dudunsparce_live = int(
        Dudunsparce in retained_ids
        and any(pokemon.id == Dunsparce for pokemon in mine.bench)
        and _xerosic_next_turn_draw_is_safe(mine, 2)
    )
    survive_draw = _xerosic_draw_route_score(
        mine, retained_cards, active_removed=False
    )
    removed_draw = _xerosic_draw_route_score(
        mine, retained_cards, active_removed=True
    )

    without_energy = tuple(
        card for card in retained_cards
        if card.id not in PSYCHIC_ENERGY_IDS
    )
    survive_without_energy = _xerosic_attack_branch_score(
        mine, without_energy, active_survives=True
    )
    removed_without_energy = _xerosic_attack_branch_score(
        mine, without_energy, active_survives=False
    )
    energy_fills_route = int(
        any(card.id in PSYCHIC_ENERGY_IDS for card in retained_cards)
        and (
            survive[0] > survive_without_energy[0]
            or removed[0] > removed_without_energy[0]
        )
    )
    ...
    rare_candy_live = (
        Rare_Candy in retained_ids
        and Alakazam in retained_ids
        and visible_abra
    )
    dead_rare_candy = (
        retained_counts[Rare_Candy] if not rare_candy_live else 0
    )
    irrelevant_psychic_energy = sum(
        card.id in PSYCHIC_ENERGY_IDS for card in retained_cards
    ) if not energy_fills_route else 0
    duplicate_cards = sum(
        max(0, count - 1) for count in retained_counts.values()
    )

    dead_stadium_or_tool = 0
    current_stadium_id = state.stadium[0].id if state.stadium else None
    if retained_counts[Battle_Cage] and current_stadium_id == Battle_Cage:
        dead_stadium_or_tool += retained_counts[Battle_Cage]
    if retained_counts[Lucky_Helmet]:
        has_tool_target = any(
            not pokemon.tools
            for pokemon in list(mine.active) + list(mine.bench)
            if pokemon is not None
        )
        if not has_tool_target:
            dead_stadium_or_tool += retained_counts[Lucky_Helmet]

    disruption = _xerosic_disruption_score(state, retained_cards)
    useful_roles = sum((
        removed[1],
        alakazam_for_kadabra,
        dudunsparce_live,
        int(max(survive_draw, removed_draw) > 0),
        energy_fills_route,
        int(disruption > 0),
    ))
    stable_fingerprints = tuple(
        sorted((int(card.id), int(card.serial)) for card in retained_cards)
    )
    stable_tie_break = tuple(
        -value for fingerprint in stable_fingerprints for value in fingerprint
    )
    ...
    return (
        ...
        alakazam_for_kadabra,
        dudunsparce_live,
        min(survive_draw, removed_draw),
        max(survive_draw, removed_draw),
        energy_fills_route,
        disruption,
        min(survive[0], removed[0]),
        removed[0],
        survive[0],
        -dead_rare_candy,
        -irrelevant_psychic_energy,
        -dead_stadium_or_tool,
        -duplicate_cards,
        useful_roles,
        stable_tie_break,
    )
...
def _xerosic_certified_retained_triple(
    obs: Observation,
) -> list[int] | None:
    """Return the discard complement for one fully certified Xerosic callback."""
    state = obs.current
    select = obs.select
    if select is None or select.context != SelectContext.DISCARD:
        return None
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if (
        effect is None
        or effect.id != Xerosics_Machinations
        or effect.playerIndex != 1 - my_index
        or not isinstance(effect.serial, int)
        or effect.serial <= 0
        or select.contextCard is not None
        or hand is None
        or mine.handCount != len(hand)
        or len(hand) <= 3
        or select.minCount != select.maxCount
        or select.maxCount != len(hand) - 3
        or len(select.option) != len(hand)
    ):
        return None

    entries = []
    seen_hand_indices = set()
    seen_serials = set()
    for option_index, option in enumerate(select.option):
        if (
            ...
            or option.index < 0
            or option.index >= len(hand)
            or option.index in seen_hand_indices
        ):
            return None
        ...
        if (
            ...
            or card.serial <= 0
            or card.serial in seen_serials
        ):
            return None
        seen_hand_indices.add(option.index)
        seen_serials.add(card.serial)
        entries.append((option_index, card))
    if seen_hand_indices != set(range(len(hand))):
        return None

    best_entries = max(
        combinations(entries, 3),
        key=lambda triple: _xerosic_retained_triple_rank(
            obs, tuple(entry[1] for entry in triple)
        ),
    )
    retained_option_indices = {entry[0] for entry in best_entries}
    discard_option_indices = [
        option_index
        for option_index in range(len(select.option))
        if option_index not in retained_option_indices
    ]
    if len(discard_option_indices) != select.maxCount:
        return None
    return discard_option_indices
...
def agent(obs_dict: dict) -> list[int]:
    ...
    xerosic_action = _xerosic_certified_retained_triple(obs)
    if xerosic_action is not None:
        return _remember_action(decision_signature, xerosic_action)
