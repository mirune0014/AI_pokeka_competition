...
SUPPORTERS = {BOSS, LANA_AID, XEROSIC, HILDA, DAWN}
ENERGIES = {PSYCHIC_ENERGY, ENRICHING_ENERGY, TELEPATH_PSYCHIC_ENERGY}

# This is deliberately per-turn and minimal: it only permits one visible
# engine action before a nonlethal Powerful Hand, then returns to baseline
# ranking for the rest of the turn.
ENGINEFLOW_TURN: int | None = None
ENGINEFLOW_USED = False
...
def hand_is_short(obs) -> bool:
    return len(hand_ids(obs)) <= 5


def alakazam_accessible(obs) -> bool:
    ids = hand_ids(obs)
    return ready_alakazam(obs) or (ALAKAZAM in ids and has_in_play(obs, ABRA))


def fez_shaymin_bench_allowed(obs) -> bool:
    """Keep a future second Abra slot while the Abra/Dunsparce core is incomplete."""
    pokemon_count = len(all_my_pokemon(obs))
    has_core = has_in_play(obs, ABRA) and has_in_play(obs, DUNSPARCE)
    second_abra_room = count_in_play(obs, ABRA) >= 2 or pokemon_count < 5
    return has_core and second_abra_room and (hand_is_short(obs) or alakazam_accessible(obs))
...
def setup_score(obs, opt) -> tuple[int, str]:
    ...
    if ctx == SelectContext.SETUP_BENCH_POKEMON:
        ...
        if cid == FEZANDIPITI_EX:
            return (900, "bench Fezandipiti after core setup") if fez_shaymin_bench_allowed(obs) else (-900, "hold Fezandipiti for Abra/Dunsparce core")
        if cid == SHAYMIN:
            return (500, "bench Shaymin after core setup") if fez_shaymin_bench_allowed(obs) else (-1000, "hold Shaymin for Abra/Dunsparce core")
    return 0, "setup"
...
def score_play(obs, opt) -> tuple[int, str]:
    ...
    if cid == FEZANDIPITI_EX:
        if not fez_shaymin_bench_allowed(obs):
            return -900, "hold Fezandipiti for second Abra/core room"
        return 2200 if len(my_state(obs).prize or []) < 6 else 600, "bench Fezandipiti after core setup"
    if cid == SHAYMIN:
        if not fez_shaymin_bench_allowed(obs):
            return -1000, "hold Shaymin for second Abra/core room"
        return 900, "bench Shaymin after core setup"
    if cid == BUDDY_POFFIN:
...
def discard_score(obs, card_id: int) -> tuple[int, str]:
    ...
    if hand.count(card_id) >= 2 and card_id in {HILDA, POKE_PAD, BUDDY_POFFIN, NIGHTTIME_MINE, DUNSPARCE}:
...
def score_to_hand(obs, opt) -> tuple[int, str]:
    ...
    if cid in {BUDDY_POFFIN, POKE_PAD, NIGHTTIME_MINE, NIGHT_STRETCHER, ENHANCED_HAMMER}:
...
def option_is_immediate_ko(obs, opt) -> bool:
    return opt.type == OptionType.ATTACK and can_ko_active(obs, best_attack_damage(obs, opt.attackId))


def setup_gate_reason(obs, opt) -> str | None:
    """Return a reason only for the requested no-Alakazam pre-attack routes."""
    card = option_card(obs, opt)
    cid = getattr(card, "id", None)
    if opt.type == OptionType.EVOLVE and cid == ALAKAZAM:
        return "engineflow: complete immediate Alakazam route"
    if opt.type == OptionType.PLAY and cid == RARE_CANDY and ALAKAZAM in hand_ids(obs) and has_in_play(obs, ABRA):
        return "engineflow: complete Rare Candy+Alakazam route"
    if count_in_play(obs, ABRA) < 2:
        if opt.type == OptionType.PLAY and cid == ABRA:
            return "engineflow: establish second Abra"
        if opt.type == OptionType.PLAY and cid == BUDDY_POFFIN and deck_count(obs) > 10:
            return "engineflow: Poffin for second Abra"
    if opt.type == OptionType.EVOLVE and cid == KADABRA:
        return "engineflow: staged Kadabra route"
    return None


def engine_advance_reason(obs, opt) -> str | None:
    """One safe, visible-information engine action before nonlethal Powerful Hand."""
    card = option_card(obs, opt)
    cid = getattr(card, "id", None)
    if opt.type == OptionType.PLAY and cid in {DAWN, HILDA}:
        if (cid == DAWN and (needs_setup(obs) or hand_is_short(obs))) or (cid == HILDA and hand_is_short(obs) and deck_count(obs) > 10):
            return f"engineflow: {card_name(cid)} hand/setup extension"
    if opt.type == OptionType.PLAY and cid == BUDDY_POFFIN and deck_count(obs) > 10:
        if count_in_play(obs, ABRA) < 2 or count_in_play(obs, DUNSPARCE) < 1:
            return "engineflow: Poffin for missing Abra/Dunsparce"
    if opt.type == OptionType.EVOLVE and cid == DUDUNSPARCE and deck_count(obs) > 10:
        return "engineflow: Dudunsparce draw loop"
    if opt.type == OptionType.ATTACH and getattr(option_target(obs, opt), "id", None) in ALAKAZAM_LINE:
        target = option_target(obs, opt)
        if energy_count(target) < 2:
            return "engineflow: attach toward Alakazam attack readiness"
    return None


def engineflow_bonus(obs, opt, options) -> tuple[int, str | None]:
    """Apply the Gold-positive ordering without changing KO, Boss, retreat, or deck guards."""
    global ENGINEFLOW_TURN, ENGINEFLOW_USED
    turn = int(getattr(obs.current, "turn", -1) or -1)
    if turn != ENGINEFLOW_TURN:
        ENGINEFLOW_TURN, ENGINEFLOW_USED = turn, False
    if any(option_is_immediate_ko(obs, candidate) for candidate in options):
        return 0, None
    if any(option_card(obs, candidate) and getattr(option_card(obs, candidate), "id", None) == BOSS for candidate in options):
        return 0, None
    has_nonko_attack = any(candidate.type == OptionType.ATTACK for candidate in options)
    if not ready_alakazam(obs) and has_nonko_attack:
        reason = setup_gate_reason(obs, opt)
        immediate_routes = [candidate for candidate in options if setup_gate_reason(obs, candidate) and "immediate" in setup_gate_reason(obs, candidate)]
        second_abra_routes = [candidate for candidate in options if setup_gate_reason(obs, candidate) and "second Abra" in setup_gate_reason(obs, candidate)]
        staged_routes = [candidate for candidate in options if setup_gate_reason(obs, candidate) == "engineflow: staged Kadabra route"]
        preferred = immediate_routes or second_abra_routes or staged_routes
        if reason and opt in preferred:
            return 40000, reason
        return 0, None
    has_nonlethal_powerful_hand = any(
        candidate.type == OptionType.ATTACK
        and candidate.attackId == POWERFUL_HAND
        and not option_is_immediate_ko(obs, candidate)
        for candidate in options
    )
    if ready_alakazam(obs) and has_nonlethal_powerful_hand and not ENGINEFLOW_USED:
        reason = engine_advance_reason(obs, opt)
        if reason:
            return 35000, reason
    return 0, None
...
def choose_options(obs):
    global ENGINEFLOW_USED
    scored: list[tuple[int, int, str]] = []
    for i, opt in enumerate(obs.select.option):
        try:
            score, reason = score_option(obs, opt)
            bonus, engine_reason = engineflow_bonus(obs, opt, obs.select.option)
            if engine_reason:
                score += bonus
    ...
    if selected:
        selected_options = [obs.select.option[i] for i in selected]
        if any(engine_advance_reason(obs, opt) for opt in selected_options):
            ENGINEFLOW_USED = True
    return selected
