...
def should_rotate_heavy_garchomp(obs) -> bool:
    ...
    return bool(
        ...
        and any(
            p.id == GARCHOMP_EX and energy_count(p) >= 2
...
def score_retreat(obs, _opt) -> tuple[int, str]:
    active = active_pokemon(obs)
    if should_rotate_heavy_garchomp(obs):
        return 19000, "rotate heavy Garchomp to ready backup"
