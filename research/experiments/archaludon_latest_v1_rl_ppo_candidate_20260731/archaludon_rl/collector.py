...
CHECKED_PAIRED_RUNNER = "infrastructure/tools/run_seeded_paired_suite.py"
...
def validate_duplicate_pair(
    ...
    for replica, episode in (("A", first), ("B", second)):
        failures = [
            {
            ...
            if row.get("model_failure_kind") is not None
            or row.get("model_timeout") is True
        ]
        if failures:
            raise ValueError(
                f"A/B duplicate replica {replica} contains model "
                f"timeout/fallback decisions: {failures}"
            )
