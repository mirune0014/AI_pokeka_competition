"""Exact Historical-Silver plus one isolated tempo-commitment overlay.

The imported accepted parent remains the complete policy.  This wrapper calls
it once and may emit one exact attack-commitment exception through the same
single resolver and transaction owner.
"""
...
_TEMPO_ID = "TEMPO_COMMITMENT_EXACT_POSITIVE_ATTACK_V1"
_DURALUDON = 169
_ARCHALUDON_EX = 190
_CINDERACE = 666
_ARCHALUDON = 840
...
def _tempo_attack_proposal(obs, parent_action):
    """Commit to a rigorously known positive attack before optional setup.

    This is deliberately narrower than a general attack scorer.  Every legal
    attack option must have exact local metadata and an exact public modifier
    surface.  If any option is unknown, or if the best value is tied without
    the accepted parent's attack breaking that tie, the parent action is kept.
    """
    gates = {
        "normal_main": _exact_main(obs),
        "owner_empty": _materialization_owner is None,
        "parent_action_list": isinstance(parent_action, list),
        "parent_called_once": isinstance(parent_action, list),
    }
    if not all(gates.values()):
        return None, "tempo_entry_gate_failed", gates, False, False
    if _parent.should_prioritize_lillie_against_alakazam(obs):
        gates["alakazam_lillie_safety"] = False
        return None, "tempo_lillie_safety_preserved", gates, False, False
    gates["alakazam_lillie_safety"] = True

    rows = _option_rows(obs)
    gates["option_bindings_exact"] = rows is not None
    if rows is None:
        return None, "tempo_option_binding_failed", gates, False, False
    seat = obs.current.yourIndex
    mine = obs.current.players[seat]
    theirs = obs.current.players[1 - seat]
    if (
        not isinstance(getattr(mine, "active", None), list)
        or len(mine.active) != 1
        or not isinstance(getattr(theirs, "active", None), list)
        or len(theirs.active) != 1
    ):
        return None, "tempo_active_binding_failed", gates, False, False
    attacker = mine.active[0]
    target = theirs.active[0]
    attack_rows = [
        row
        for row in rows
        if _attack_option_exact(row[1])
        and row[2][5] in _ATTACKER_ATTACKS.get(getattr(attacker, "id", None), ())
    ]
    gates["attack_options_present"] = bool(attack_rows)
    if not attack_rows:
        return None, "tempo_no_attack_option", gates, False, False

    # Deduplicate only semantically identical attack options.  Different
    # attack IDs or ambiguous bindings are never resolved by option order.
    by_attack = {}
    for row in attack_rows:
        attack_id = row[2][5]
        prior = by_attack.get(attack_id)
        if prior is not None and prior[2] != row[2]:
            return None, "tempo_duplicate_attack_binding_unknown", gates, False, False
        if prior is None or row[0] < prior[0]:
            by_attack[attack_id] = row
    evaluated = []
    for attack_id, row in sorted(by_attack.items()):
        result = _exact_damage_and_take(obs, attacker, target, attack_id)
        if result is None:
            return None, "tempo_modifier_or_attack_unknown", gates, False, False
        damage, printed_prize, take = result
        evaluated.append(
            {
                "attack_id": attack_id,
                ...
                "printed_prize": printed_prize,
                "take": take,
                "wins_game_now": take == len(mine.prize),
                "exact_ko": damage >= target.hp,
            }
        )
    positive = [item for item in evaluated if item["damage"] > 0]
    gates["all_attack_effects_exact"] = len(positive) == len(evaluated)
    if not positive:
        return None, "tempo_no_positive_damage", gates, False, False
    if len(positive) != len(evaluated):
        # A known 0-damage attack (for example Active Crustle) is not a
        # positive tempo commitment, and mixing it with another attack would
        # make the comparison depend on an incomplete effect model.
        return None, "tempo_zero_damage_present", gates, False, False

    def score(item):
        return (
            int(item["wins_game_now"]),
            int(item["exact_ko"]),
            int(item["take"]),
            int(item["damage"]),
        )

    best_score = max(score(item) for item in evaluated)
    best = [item for item in evaluated if score(item) == best_score]
    parent_item = None
    if (
        len(parent_action) == 1
        and _is_exact_int(parent_action[0])
        ...
        parent_role = rows[parent_action[0]][2]
        parent_attack_id = parent_role[5] if _attack_option_exact(rows[parent_action[0]][1]) else None
        parent_item = next(
            (item for item in evaluated if item["attack_id"] == parent_attack_id),
            None,
        )
    if len(best) != 1:
        if parent_item is None or score(parent_item) != best_score:
            return None, "tempo_attack_value_tie_parent_fallback", gates, False, False
        selected = parent_item
    else:
        selected = best[0]
    action = [selected["row"][0]]
    if action == parent_action:
        return None, "tempo_parent_already_selected", gates, False, False
    proof = {
        "attack_id": selected["attack_id"],
        "damage": selected["damage"],
        "printed_prize": selected["printed_prize"],
        "take": selected["take"],
        "wins_game_now": selected["wins_game_now"],
        "exact_ko": selected["exact_ko"],
        "score": best_score,
        "attacker": _pokemon_fingerprint(attacker, seat),
        "target": _pokemon_fingerprint(target, 1 - seat),
        "gates": dict(gates),
    ...
    return (
        _make_proposal(
            action,
            "TEMPO_COMMIT_POSITIVE_EXACT_ATTACK",
            proof,
            None,
            rule_id=_TEMPO_ID,
            category="TEMPO_ATTACK_COMMITMENT",
        ),
        None,
        proof,
        False,
...
def _resolve(obs, parent_action):
    ...
    if context == _SelectContext.MAIN:
        ...
        if direct[1] == "multiple_terminal_attack_ids":
            return direct
        tempo = _tempo_attack_proposal(obs, parent_action)
...
def _emit_telemetry(
    ...
    active_rule = proposal_rule or _RULE_ID
    _last_telemetry = {
        "rule_id": active_rule,
        "selected_source": (
...
def agent(obs_dict):
    """Call the accepted parent once, then resolve the isolated tempo overlay."""
