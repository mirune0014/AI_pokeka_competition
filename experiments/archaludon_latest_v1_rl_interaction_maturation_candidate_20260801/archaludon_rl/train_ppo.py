...
def load_manifest_dataset(
    ...
    *,
    ...
    expected_behavior_policy_receipt: Mapping[str, Any],
    expected_behavior_policy_schema_sha256: str,
) -> ManifestDataset:
    unresolved_path = (
        manifest_path
    ...
    manifest_behavior_receipt = payload.get("behavior_policy_receipt")
    manifest_behavior_hash = payload.get("behavior_policy_schema_sha256")
    if not isinstance(manifest_behavior_receipt, dict):
        raise ValueError("run manifest behavior-policy receipt missing")
    validate_behavior_policy_identity(
        manifest_behavior_receipt,
        manifest_behavior_hash,
    )
    if (
        manifest_behavior_receipt != dict(expected_behavior_policy_receipt)
        or manifest_behavior_hash
        != expected_behavior_policy_schema_sha256
    ):
        raise ValueError("run manifest/checkpoint behavior-policy mismatch")
    ...
    spec_hash = collection_spec_sha256(
        ...
        behavior_policy_receipt=dict(manifest_behavior_receipt),
        behavior_policy_schema_sha256=str(manifest_behavior_hash),
    ...
    if payload.get("dataset_sha256") != dataset_sha256(
        ...
        behavior_policy_receipt=manifest_behavior_receipt,
        behavior_policy_schema_sha256=manifest_behavior_hash,
    ...
    for receipt in receipts:
        ...
        if (
            ...
            != manifest_prior_hash
            or episode.get("behavior_policy_receipt")
            != manifest_behavior_receipt
            or episode.get("behavior_policy_schema_sha256")
            != manifest_behavior_hash
        ...
        if payload.get("duplicate_mode"):
            ...
            for replica_index, replica in enumerate(("a", "b")):
                ...
                if (
                    ...
                    or replica_episode.get("behavior_policy_receipt")
                    != episode.get("behavior_policy_receipt")
                    or replica_episode.get("behavior_policy_schema_sha256")
                    != episode.get("behavior_policy_schema_sha256")
...
def _validate_rows(
    ...
    *,
    ...
    canonical_behavior_receipt = canonical_behavior_policy_receipt()
    canonical_behavior_hash = behavior_policy_sha256(canonical_behavior_receipt)
    validate_behavior_policy_identity(
        behavior_policy_receipt,
        behavior_policy_schema_sha256,
    )
    if (
        dict(behavior_policy_receipt) != canonical_behavior_receipt
        or behavior_policy_schema_sha256 != canonical_behavior_hash
    ):
        raise ValueError("checkpoint behavior-policy provenance mismatch")
    ...
    for episode in episodes:
        ...
        episode_behavior_receipt = episode.get("behavior_policy_receipt")
        episode_behavior_hash = episode.get("behavior_policy_schema_sha256")
        if not isinstance(episode_behavior_receipt, dict):
            raise ValueError("episode behavior-policy receipt missing")
        validate_behavior_policy_identity(
            episode_behavior_receipt,
            episode_behavior_hash,
        )
        if (
            episode_behavior_receipt != dict(behavior_policy_receipt)
            or episode_behavior_hash != behavior_policy_schema_sha256
        ):
            raise ValueError("episode behavior-policy provenance mismatch")
        ...
        for row in episode.get("decisions") or ():
            ...
            if (
                row.get("behavior_formula_id") != BEHAVIOR_FORMULA_ID
                or row.get("behavior_schema_version")
                != BEHAVIOR_POLICY_SCHEMA_VERSION
                or row.get("behavior_schema_sha256")
                != canonical_behavior_hash
                or isinstance(row.get("behavior_temperature"), bool)
                or row.get("behavior_temperature") != BEHAVIOR_TEMPERATURE
                or isinstance(row.get("behavior_support_mixture"), bool)
                or row.get("behavior_support_mixture")
                != BEHAVIOR_SUPPORT_MIXTURE
            ):
                raise ValueError(
                    "decision behavior-policy formula/provenance mismatch"
                )
            if (
            ...
            behavior_option_order = row.get("behavior_option_order")
            if (
                not isinstance(behavior_option_order, (list, tuple))
                or any(
                    not isinstance(value, int) or isinstance(value, bool)
                    for value in behavior_option_order
                )
                or tuple(behavior_option_order)
                != tuple(range(legal_option_count))
                or tuple(
                    semantic_row.get("engine_index")
                    for semantic_row in semantic_rows
                )
                != tuple(behavior_option_order)
                or row.get("behavior_action_order_sha256")
                != behavior_action_order_sha256(semantic_rows)
            ):
                raise ValueError("decision behavior legal-action order mismatch")
            ...
            if any(
                float(value)
                <= BEHAVIOR_SUPPORT_MIXTURE / legal_option_count
                for value in probabilities
            ):
                raise ValueError("stored behavior distribution lost support floor")
            if not math.isclose(
            ...
            if any(
                ...
                raise ValueError("stored latest prior cannot be reproduced")
            if tuple(float(value) for value in probabilities) != (
                recomputed_distribution.probabilities
            ):
                raise ValueError(
...
def _torch_behavior_distribution(
    residuals: torch.Tensor,
    *,
    teacher_index: int,
    reference_config: ReferencePolicyConfig,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Reconstruct canonical iteration-004 mu with gradients intact."""

    option_count = int(residuals.numel())
    if option_count < 2 or not 0 <= teacher_index < option_count:
        raise ValueError("invalid behavior surface/teacher index")
    log_weights = torch.zeros_like(residuals)
    log_weights[teacher_index] = float(reference_config.teacher_margin)
    logits = (
        log_weights
    ...
    base_probabilities = torch.softmax(logits, dim=0)
    probabilities = (
        (1.0 - BEHAVIOR_SUPPORT_MIXTURE) * base_probabilities
        + BEHAVIOR_SUPPORT_MIXTURE / option_count
    )
    return probabilities, torch.log(probabilities)


def _torch_behavior_anchor_kl(
    residuals: torch.Tensor,
    *,
    teacher_index: int,
    reference_config: ReferencePolicyConfig,
) -> torch.Tensor:
    """KL(mu_current || detached mu_zero) on the identical legal surface."""
    ...
    anchor_probabilities, _ = _torch_behavior_distribution(
        torch.zeros_like(residuals),
        teacher_index=teacher_index,
        reference_config=reference_config,
    )
    anchor_probabilities = anchor_probabilities.detach()
    return (
        probabilities
        * (log_probabilities - torch.log(anchor_probabilities))
    ).sum()
...
def train(
    *,
    ...
    if not isinstance(behavior_receipt, dict) or not isinstance(
        behavior_hash, str
    ):
        raise ValueError("checkpoint behavior-policy identity missing after validation")
    validate_behavior_policy_identity(behavior_receipt, behavior_hash)
