...
def opening_width_before_first_resource_applies(obs) -> bool:
    """Delay Rock/Weight only while the acting player's main line is still bare and safe."""
    main_line = [pokemon for pokemon in my_pokemon(obs) if pokemon.id in MAIN_LINE]
    opposing_active = opponent_active(obs)
    opposing_active_is_energized_evolution = (
        bool(getattr(opposing_active, "preEvolution", None))
        and energy_count(opposing_active) > 0
    )
    return (
        int(getattr(obs.current, "turn", 0) or 0) <= 2
        and len(main_line) < 2
        and not any(energy_count(pokemon) for pokemon in main_line)
        and not opposing_active_is_energized_evolution
    )
