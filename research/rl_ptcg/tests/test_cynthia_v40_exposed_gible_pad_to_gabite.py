"""Focused public-state checks for the Cynthia Garchomp v40 Pad overlay."""
...
AGENT_DIR = ROOT / "opponents" / "meta_agents" / "cynthia_garchomp_nasuo445_v40_exposed_gible_pad_to_gabite"
...
def load_agent_module():
    ...
    spec = importlib.util.spec_from_file_location("cynthia_v40_main", AGENT_DIR / "main.py")
...
class ExposedGiblePadToGabiteTest(unittest.TestCase):
    ...
    def main_obs(self, *, hand_ids=(), active_id=None, active_energies=(), gible_appeared=False, target_hp=200, context=None, options=None):
        a = self.agent
        ...
        foe = types.SimpleNamespace(hand=[], active=[self.pokemon(999, hp=target_hp)], bench=[], discard=[], prize=[])
        return types.SimpleNamespace(
            current=types.SimpleNamespace(yourIndex=0, players=[mine, foe], looking=[], stadium=[]),
