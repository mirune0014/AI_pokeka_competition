...
_active_kadabra_stage_up_latch = {}
_active_kadabra_transient_guard = {}
...
def _clear_active_kadabra_stage_up_latch() -> None:
    _active_kadabra_stage_up_latch.clear()


def _clear_active_kadabra_transient_guard() -> None:
    _active_kadabra_transient_guard.clear()
...
def _clear_emergency_state(*, clear_cache: bool = False) -> None:
    ...
    _clear_active_kadabra_stage_up_latch()
    _clear_active_kadabra_transient_guard()
    if clear_cache:
...
def _decision_signature(obs: Observation) -> tuple:
    ...
    active_kadabra_guard_signature = None
    if _active_kadabra_stage_up_latch:
        # The private stage is deliberately excluded.  Once an action advances
        # the stage but the engine repeats the same public callback, the cache
        # must return the same option rather than consume the transaction twice.
        active_kadabra_guard_signature = (
            state.retreated,
...
def _start_active_psychic_immediate_ko(
    ...
    if (
        ...
        or _active_kadabra_stage_up_latch
        or active is None
...
def _stage_up_normalized_text(text: str) -> str:
    return " ".join(
        (text or "")
        .lower()
        .replace("pokémon", "pokemon")
...
def _stage_up_psychic_draw_is_certified() -> bool:
    """Certify the exact direct Alakazam evolution and optional draw-three."""
    ...
    skill = data.skills[0]
    return (
        _stage_up_normalized_text(skill.name) == "psychic draw"
        and _stage_up_normalized_text(skill.text)
        == (
...
def _stage_up_card_group_fingerprint(
    cards, player_index: int | None = None
) -> tuple | None:
    rows = []
    for card in cards:
...
def _stage_up_status_fingerprint(state) -> tuple:
    return tuple(
        (
            player.poisoned,
            player.burned,
...
def _stage_up_board_rows(player, player_index: int) -> tuple | None:
    rows = []
...
def _stage_up_freeze_common(
    ...
    own_discard = _stage_up_card_group_fingerprint(mine.discard, my_index)
    opponent_discard = _stage_up_card_group_fingerprint(
        theirs.discard, 1 - my_index
    )
    stadium = _stage_up_card_group_fingerprint(state.stadium)
    fixed_own = _stage_up_fixed_own_rows(
        mine, my_index, excluded_own_serials
    )
    opponent_board = _stage_up_board_rows(theirs, 1 - my_index)
    full_own = _stage_up_board_rows(mine, my_index)
    if any(
        value is None
        for value in (
            hand,
    ...
    return {
        ...
        "start_context": int(obs.select.context),
...
def _stage_up_common_is_same(
    ...
    *,
    ...
    return (
        ...
        and state.turnActionCount == turn_action_count
        ...
        and opponent_discard == latch.get("opponent_discard_fingerprint")
        and stadium == latch.get("stadium_fingerprint")
        and _stage_up_status_fingerprint(state)
...
def _stage_up_hand_without_selected(
...
def _stage_up_hand_has_frozen_cards(
    mine,
    *,
    player_index: int,
    frozen_cards: tuple,
    ...
    """Validate counts/serials only; drawn identities never enter a choice."""
    hand = _active_psychic_hand_fingerprint(mine, player_index)
    ...
    hand_set = set(hand)
    if len(frozen_set) != len(frozen_cards) or not frozen_set <= hand_set:
        return False
    old_serials = {row[1] for row in frozen_cards}
    new_rows = [row for row in hand if row not in frozen_set]
    return (
        len(new_rows) == new_count
        and len({row[1] for row in new_rows}) == new_count
        and all(row[1] not in old_serials for row in new_rows)
    )


def _stage_up_skill_is_yield_irrelevant(
    text: str,
    *,
    source_kind: str,
    owner: int | None,
...
def _stage_up_visible_effect_fingerprint(
    state, *, player_index: int, excluded_top_serials: set[int]
) -> tuple | None:
    """Certify every visible static source except the expected evolving top."""
    records = []
    protected = []
    excluded_found = set()
    for owner, player in enumerate(state.players):
        for area, pokemon_list in (
            ...
            for index, pokemon in enumerate(pokemon_list):
                ...
                if data is None or data.cardType != CardType.POKEMON:
                    return None
                for prior in pokemon.preEvolution:
                    prior_data = card_table.get(prior.id)
                    if prior_data is None or prior_data.cardType != CardType.POKEMON:
                        return None
                is_expected_evolving_top = (
                    pokemon.serial in excluded_top_serials
                )
                if is_expected_evolving_top:
                    excluded_found.add(pokemon.serial)
                elif not all(
                    _stage_up_skill_is_yield_irrelevant(
                        skill.text,
                        source_kind="pokemon",
                        owner=owner,
                        player_index=player_index,
                ...
                if not is_expected_evolving_top:
                    records.append(
                        (
                            owner,
                            int(area),
                ...
                for kind, cards, allowed in (
                    ...
                    for card in cards:
                        ...
                        if (
                            ...
                            or (
                                kind == "energy"
                                and _bridge_retaliation_energy_unit(card) is None
                            )
                            or not all(
                                _stage_up_skill_is_yield_irrelevant(
                                    skill.text,
    ...
    if excluded_found != excluded_top_serials:
        return None
    if (
...
def _stage_up_observe_transient_effects(obs: Observation) -> None:
    """Remember public next-turn attack prevention across shorter log batches."""
    state = obs.current
    my_index = state.yourIndex
    if (
        _active_kadabra_transient_guard.get("turn") != state.turn
        or _active_kadabra_transient_guard.get("player") != my_index
    ):
        _active_kadabra_transient_guard.clear()
        _active_kadabra_transient_guard.update(
            turn=state.turn,
            player=my_index,
            unknown=False,
            protected_serials=tuple(),
        )
    ...
    for log in obs.logs or []:
        ...
        if isinstance(attack_id, int) and attack_id > 0:
            attack = attack_table.get(attack_id)
            normalized = _stage_up_normalized_text(
                "" if attack is None else attack.text
            )
            if attack is None or any(
                ...
                if attack_id != 1266:
                    _active_kadabra_transient_guard["unknown"] = True
        head = getattr(log, "head", None)
        if head is not None and pending is not None:
            attack_id, source_serial, _ = pending
            if attack_id == 1266 and head is True:
                protected = set(
                    _active_kadabra_transient_guard.get(
                        "protected_serials", ()
                    )
                )
                if not isinstance(source_serial, int) or source_serial <= 0:
                    _active_kadabra_transient_guard["unknown"] = True
                else:
                    protected.add(source_serial)
                    _active_kadabra_transient_guard["protected_serials"] = (
...
def _stage_up_transient_certificate(target: Pokemon) -> tuple | None:
    if _active_kadabra_transient_guard.get("unknown"):
        return None
    protected = tuple(
        _active_kadabra_transient_guard.get("protected_serials") or ()
    )
    if target.serial in protected:
        return None
    return (
        _active_kadabra_transient_guard.get("turn"),
        _active_kadabra_transient_guard.get("player"),
        False,
        protected,
    )


def _stage_up_attack_certificate(
    state,
    *,
    attacker_id: int,
    attacker_top_serial: int,
    attack_id: int,
    ...
    if (
        ...
        return None
    effects = _stage_up_visible_effect_fingerprint(
        state,
        player_index=player_index,
        excluded_top_serials={attacker_top_serial},
    )
    if effects is None:
        return None
    ...
    if attack_id == ATTACK_SUPER_PSY_BOLT:
        ...
        normalized = _stage_up_normalized_text(attack.text)
        energy_ids = {card.id for card in target.energyCards}
        if (
            attack.damage != 0
            or "place 2 damage counters" not in normalized
            or "for each card in your hand" not in normalized
            or Mist_Energy in energy_ids
            or (
                Rock_Fighting_Energy in energy_ids
    ...
    return {
        ...
        "attacker_metadata_fingerprint": (
            attacker_data.cardId,
            attacker_data.name,
            attacker_data.hp,
            tuple(attacker_data.attacks or []),
            _bridge_metadata_skill_fingerprint(attacker_data),
        ),
    }


def _stage_up_active_evolution_options(
    obs: Observation, my_index: int
    ...
    for option_index, option in enumerate(obs.select.option):
        ...
        if encoding in encodings or card.serial in serials:
            return None
        encodings.add(encoding)
        serials.add(card.serial)
        candidates.append((option_index, card))
    return sorted(candidates, key=lambda row: row[0]) if candidates else None


def _stage_up_evolved_active_is_exact(
    mine, latch: dict, player_index: int
) -> Pokemon | None:
    ...
    if (
        active.id != Alakazam
        or active.serial != latch.get("selected_evolution_serial")
...
def _start_active_kadabra_stage_up(
    obs: Observation, parent_option_index: int
) -> list[int] | None:
    """Replace only finalized Super Psy Bolt with the certified stage-up KO."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if (
        ...
        or super_options != [parent_option_index]
        or active is None
        ...
        or not _bridge_pokemon_is_publicly_complete(target, 1 - my_index)
        or target.hp <= 0
        or not _stage_up_psychic_draw_is_certified()
    ):
        return None
    transient = _stage_up_transient_certificate(target)
    routes = _stage_up_active_evolution_options(obs, my_index)
    energy_units = _bridge_retaliation_energy_units(active)
    if transient is None or routes is None or energy_units is None:
        return None
    option_index, evolution = routes[0]
    projected_hand_count = mine.handCount - 1 + 3
    parent_certificate = _stage_up_attack_certificate(
        state,
        attacker_id=Kadabra,
        attacker_top_serial=active.serial,
    ...
    candidate_certificate = _stage_up_attack_certificate(
        state,
        attacker_id=Alakazam,
        attacker_top_serial=active.serial,
    ...
    if (
        parent_certificate is None
        or candidate_certificate is None
        or not candidate_certificate["ko"]
        or candidate_certificate["prize_yield"] <= 0
        or candidate_certificate["prize_yield"]
        <= parent_certificate["prize_yield"]
    ):
        return None
    final_prize = (
        candidate_certificate["prize_yield"] >= len(mine.prize)
    )
    if mine.deckCount - 3 <= 0 and not final_prize:
        return None
    common = _stage_up_freeze_common(
        obs, excluded_own_serials={active.serial, evolution.serial}
    )
    selected_fingerprint = _bridge_card_fingerprint(evolution)
    hand_after = _stage_up_hand_without_selected(
    ...
    if expected_evolved_hp <= 0 or expected_evolved_hp > card_table[Alakazam].hp:
        return None
    _active_kadabra_stage_up_latch.update(
        common,
        stage="await_draw_offer",
        selected_evolution_option=option_index,
        selected_evolution_id=Alakazam,
        selected_evolution_serial=evolution.serial,
        selected_evolution_fingerprint=selected_fingerprint,
        frozen_hand_after_evolution=hand_after,
        source_serial=active.serial,
        ...
        target_serial=target.serial,
        target_fingerprint=_bridge_target_fingerprint(target, theirs),
        transient_certificate=transient,
        expected_draw_count=3,
        expected_attack_id=ATTACK_POWERFUL_HAND,
        parent_certificate_core=(
            parent_certificate["attack_id"],
            parent_certificate["amount"],
            parent_certificate["prize_yield"],
            parent_certificate["target_serial"],
            parent_certificate["target_fingerprint"],
            parent_certificate["visible_effect_fingerprint"],
        ),
        candidate_certificate_core=(
            ...
            candidate_certificate["target_fingerprint"],
            candidate_certificate["visible_effect_fingerprint"],
            candidate_certificate["attacker_metadata_fingerprint"],
        ),
        expected_damage=candidate_certificate["amount"],
        expected_prize_yield=candidate_certificate["prize_yield"],
        expected_post_prize_count=max(
            0,
            len(mine.prize) - candidate_certificate["prize_yield"],
        ),
    )
    return [option_index]


def _active_kadabra_stage_up_overlay(
    obs: Observation,
) -> list[int] | None:
    if not _active_kadabra_stage_up_latch:
        return None
    latch = _active_kadabra_stage_up_latch
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if stage == "await_resolution":
        current_target = theirs.active[0] if theirs.active else None
        target_left = (
            current_target is None
            or current_target.serial != latch.get("target_serial")
            or current_target.hp <= 0
        )
        prize_delta = (
            len(mine.prize) == latch.get("expected_post_prize_count")
        )
        _clear_active_kadabra_stage_up_latch()
        # Both success and unexpected prevention delegate this callback.  The
        # frozen route never repairs or restarts on the same observation.
        return None if target_left or prize_delta else None

    evolved = _stage_up_evolved_active_is_exact(mine, latch, my_index)
    transient = (
        _stage_up_transient_certificate(theirs.active[0])
        if len(theirs.active) == 1
        else None
    )
    if (
        evolved is None
        or transient != latch.get("transient_certificate")
    ):
        _clear_active_kadabra_stage_up_latch()
        return None
    ...
    if stage == "await_draw_offer":
        ...
        options = tuple(option.type for option in select.option)
        yes = [
            index
            for index, option in enumerate(select.option)
        ...
        if (
            ...
            or select.type != OptionType.EVOLVE
            or select.effect is not None
            or context_card is None
            or _bridge_card_fingerprint(context_card) != selected
            or not _stage_up_common_is_same(
                obs,
                latch,
                hand_count=latch.get("start_hand_count") - 1,
                deck_count=latch.get("start_deck_count"),
                turn_action_count=latch.get("start_turn_action_count") + 1,
            )
            or not _stage_up_hand_has_frozen_cards(
                mine,
                player_index=my_index,
                frozen_cards=latch.get("frozen_hand_after_evolution"),
    ...
    if stage != "await_draw_resolution" or select.context != SelectContext.MAIN:
        _clear_active_kadabra_stage_up_latch()
        return None
    draw_count = latch.get("expected_draw_count")
    if (
        not _stage_up_common_is_same(
            obs,
            latch,
            hand_count=latch.get("start_hand_count") - 1 + draw_count,
            deck_count=latch.get("start_deck_count") - draw_count,
            turn_action_count=latch.get("start_turn_action_count") + 2,
        )
        or not _stage_up_hand_has_frozen_cards(
            mine,
            player_index=my_index,
    ...
    certificate = _stage_up_attack_certificate(
        state,
        attacker_id=Alakazam,
        attacker_top_serial=evolved.serial,
        attack_id=ATTACK_POWERFUL_HAND,
        energy_units=tuple(int(unit) for unit in evolved.energies),
        hand_count=mine.handCount,
        target=target,
        player_index=my_index,
    )
    ...
    if certificate is not None:
        core = (
            ...
            certificate["visible_effect_fingerprint"],
            certificate["attacker_metadata_fingerprint"],
        )
    if (
        certificate is None
        or not certificate["ko"]
        or core != latch.get("candidate_certificate_core")
        or len(matches) != 1
        or select.minCount != 1
        or select.maxCount != 1
    ):
...
def agent(obs_dict: dict) -> list[int]:
    ...
    _stage_up_observe_transient_effects(obs)
    ...
    had_active_kadabra_stage_up_latch = bool(
        _active_kadabra_stage_up_latch
    )
    transaction_action = _active_kadabra_stage_up_overlay(obs)
    if transaction_action is not None:
        return _remember_action(decision_signature, transaction_action)
    active_kadabra_stage_up_delegated = (
        had_active_kadabra_stage_up_latch
        and not _active_kadabra_stage_up_latch
    )
    ...
    # Exact v3 has finalized a unique Super Psy Bolt.  Only now may the
    # isolated Active-Kadabra transaction replace that terminal attack.
    if (
        not active_kadabra_stage_up_delegated
        and context == SelectContext.MAIN
        and len(chosen_action) == 1
        and not _hilda_source_latch
        and not _enriching_reserve_latch
        ...
        transaction_action = _start_active_kadabra_stage_up(
            obs, chosen_action[0]
        )
        if transaction_action is not None:
