...
try:
    import importlib.util as _residual_importlib
    import json as _residual_json
    import random as _residual_random
    from pathlib import Path as _ResidualPath
    _RESIDUAL_DIR = _ResidualPath(__file__).resolve().parent
    _RESIDUAL_SPEC = _residual_importlib.spec_from_file_location(
        "_ptcg_residual_policy_runtime", _RESIDUAL_DIR / "residual_policy.py")
    if _RESIDUAL_SPEC is None or _RESIDUAL_SPEC.loader is None:
        raise ImportError("could not load residual_policy.py")
    _RESIDUAL_MODULE = _residual_importlib.module_from_spec(_RESIDUAL_SPEC)
    _RESIDUAL_SPEC.loader.exec_module(_RESIDUAL_MODULE)
    _residual_choose = _RESIDUAL_MODULE.choose_residual
    _RESIDUAL_WEIGHTS = _residual_json.loads(
        (_RESIDUAL_DIR / "residual_weights.json").read_text(encoding="ascii"))
except Exception:
    _RESIDUAL_WEIGHTS = {}
...
def choose_options(obs):
    try:
        ...
        return _residual_choose(obs, score_option, option_card, option_target, detect_matchup,
                                choose_options_rule(obs), _RESIDUAL_WEIGHTS,
                                rng=_residual_random.Random(0), top_n=4, training=False,
                                residual_cap=0.35)[0]
    except Exception:
        return choose_options_rule(obs)
