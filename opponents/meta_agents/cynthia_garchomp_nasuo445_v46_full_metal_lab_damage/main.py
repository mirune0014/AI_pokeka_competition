...
def is_approved_buster_conversion(obs) -> bool:
    ...
    if not target:
        return False
    buster_ko = best_damage_for_active(obs, target, DRACONIC_BUSTER) >= hp(target)
    corkscrew_ko = best_damage_for_active(obs, target, CORKSCREW_DIVE) >= hp(target)
...
def best_damage_for_active(obs, target, attack_id: int | None = None) -> int:
    active = active_pokemon(obs)
    if not active:
        return 0
    ...
    if attack_id == DRACONIC_BUSTER or (
        ...
        damage = 260 + bonus
    elif attack_id == CORKSCREW_DIVE or active.id == GARCHOMP_EX:
        damage = 100 + bonus
    elif attack_id == LEAF_STEP or active.id == ROSERADE:
        damage = 80 + bonus
    elif attack_id == RAGING_CURSE or active.id == SPIRITOMB:
        damage = spiritomb_damage(obs) + bonus
    elif attack_id == DRAGONSLICE or active.id == GABITE:
        damage = 40 + bonus
    elif attack_id == ROCK_HURL or active.id == GIBLE:
        damage = 20 + bonus
    elif attack_id == SPIKE_STING or active.id == ROSELIA:
        damage = 20 + bonus
    else:
        attack = ATTACK_DB.get(attack_id)
        damage = int(getattr(attack, "damage", 0) or 0) if attack else 0

    target_card = CARD_DB.get(getattr(target, "id", None))
    target_energy_type = getattr(target_card, "energyType", None)
    target_energy_type = getattr(target_energy_type, "value", target_energy_type)
    stadiums = getattr(obs.current, "stadium", None) or []
    full_metal_lab_is_public = any(
        getattr(stadium, "id", getattr(stadium, "cardId", None)) == FULL_METAL_LAB
        for stadium in stadiums
        if stadium
    )
    if full_metal_lab_is_public and target_energy_type == METAL_ENERGY_TYPE:
...
def score_play(obs, opt) -> tuple[int, str]:
    ...
    if cid == BOSS:
        ...
        if target and best_damage_for_active(obs, target) >= hp(target):
...
def boss_target_score(obs, target) -> tuple[int, str]:
    damage = best_damage_for_active(obs, target)
...
def attack_score(obs, attack_id: int | None) -> tuple[int, str]:
    target = opponent_active(obs)
    damage = best_damage_for_active(obs, target, attack_id)
...
def is_immediate_corkscrew_ko(obs, opt) -> bool:
    ...
    return bool(
        ...
        and target
        and best_damage_for_active(obs, target, CORKSCREW_DIVE) >= hp(target)
    )
...
def score_target(obs, opt) -> tuple[int, str]:
    ...
    if ctx == SelectContext.DAMAGE:
        if best_damage_for_active(obs, card) >= hp(card):
