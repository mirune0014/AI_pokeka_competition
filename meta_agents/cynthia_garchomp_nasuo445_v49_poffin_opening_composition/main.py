...
def poffin_opening_composition_indices(obs) -> list[int] | None:
    if (
        obs.select.context != SelectContext.TO_BENCH
        or obs.select.minCount != 0
        or obs.select.maxCount < 2
        or not any(has_in_play(obs, card_id) for card_id in MAIN_LINE)
        or any(has_in_play(obs, card_id) for card_id in {ROSELIA, ROSERADE})
    ):
        return None
    gible_indices: list[int] = []
    roselia_indices: list[int] = []
    for index, option in enumerate(obs.select.option):
        card_id = getattr(option_card(obs, option), "id", None)
        if card_id == GIBLE:
            gible_indices.append(index)
        elif card_id == ROSELIA:
            roselia_indices.append(index)
    if not gible_indices or not roselia_indices:
        return None
    return sorted([gible_indices[0], roselia_indices[0]])


def choose_options(obs):
    opening_composition = poffin_opening_composition_indices(obs)
    if opening_composition is not None:
