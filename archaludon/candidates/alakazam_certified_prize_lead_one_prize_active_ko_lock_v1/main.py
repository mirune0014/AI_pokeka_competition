...
def _prize_lead_option_unused_fields_are_none(
    option, allowed: set[str]
) -> bool:
    fields = (
        "number",
        "area",
    ...
    return all(
        name in allowed or getattr(option, name, None) is None
        for name in fields
    )


def _prize_lead_card_is_exact(card: Card | None, owner: int) -> bool:
    return (
        card is not None
        and isinstance(card.id, int)
        and not isinstance(card.id, bool)
        and card.id > 0
        and isinstance(card.serial, int)
        and not isinstance(card.serial, bool)
        and card.serial > 0
        and getattr(card, "playerIndex", None) == owner
        and card_table.get(card.id) is not None
    )


def _prize_lead_known_stack_is_exact(
    pokemon: Pokemon | None, owner: int
) -> bool:
    if (
        pokemon is None
        or not isinstance(pokemon.id, int)
        or isinstance(pokemon.id, bool)
        or pokemon.id <= 0
        or not isinstance(pokemon.serial, int)
        or isinstance(pokemon.serial, bool)
        or pokemon.serial <= 0
        or not isinstance(pokemon.hp, int)
        or isinstance(pokemon.hp, bool)
        or not isinstance(pokemon.maxHp, int)
        or isinstance(pokemon.maxHp, bool)
        or pokemon.hp <= 0
        or pokemon.maxHp <= 0
        or pokemon.hp > pokemon.maxHp
    ...
    if data is None or data.cardType != CardType.POKEMON:
        return False
    if not all(
        _prize_lead_card_is_exact(card, owner)
        for card in (
            list(pokemon.energyCards)
            + list(pokemon.tools)
            + list(pokemon.preEvolution)
    ...
    if not all(
        ...
        return False
    if not all(
        card_table[card.id].cardType == CardType.TOOL
        for card in pokemon.tools
    ):
        return False
    if not all(
    ...
    if pokemon.id == Abra:
        lineage_is_exact = not pre_ids
    elif pokemon.id == Kadabra:
        lineage_is_exact = pre_ids == [Abra]
    elif pokemon.id == Alakazam:
        lineage_is_exact = pre_ids in ([Abra], [Abra, Kadabra])
    elif pokemon.id == Dunsparce:
        lineage_is_exact = not pre_ids
    elif pokemon.id == Dudunsparce:
        lineage_is_exact = pre_ids == [Dunsparce]
    elif data.basic:
        lineage_is_exact = not pre_ids
    elif data.stage1:
        lineage_is_exact = bool(pre_ids) and (
            card_table[pre_ids[-1]].name == data.evolvesFrom
        )
    elif data.stage2:
        lineage_is_exact = bool(pre_ids) and (
            card_table[pre_ids[-1]].name == data.evolvesFrom
            or (len(pre_ids) == 1 and card_table[pre_ids[0]].basic)
        )
    else:
        lineage_is_exact = False
    serials = _bridge_pokemon_component_serials(pokemon)
    return lineage_is_exact and len(serials) == len(set(serials))


def _prize_lead_raw_card_matches(raw, card: Card | None) -> bool:
    return (
        isinstance(raw, dict)
        and card is not None
        and raw.get("id") == card.id
        and raw.get("serial") == card.serial
        and raw.get("playerIndex") == getattr(card, "playerIndex", None)
    )


def _prize_lead_raw_card_list_matches(raw, cards) -> bool:
    return (
        isinstance(raw, list)
        and len(raw) == len(cards)
        and all(
            _prize_lead_raw_card_matches(raw_card, card)
            for raw_card, card in zip(raw, cards)
        )
    )


def _prize_lead_raw_pokemon_matches(
    raw, pokemon: Pokemon | None, owner: int
) -> bool:
    return (
        isinstance(raw, dict)
        and pokemon is not None
        and raw.get("id") == pokemon.id
        and raw.get("serial") == pokemon.serial
        and raw.get("playerIndex") == owner
        and raw.get("hp") == pokemon.hp
        and raw.get("maxHp") == pokemon.maxHp
        and raw.get("appearThisTurn") is pokemon.appearThisTurn
        and isinstance(raw.get("energies"), list)
        and tuple(raw.get("energies"))
        == tuple(int(unit) for unit in pokemon.energies)
        and _prize_lead_raw_card_list_matches(
            raw.get("energyCards"), pokemon.energyCards
        )
        and _prize_lead_raw_card_list_matches(raw.get("tools"), pokemon.tools)
        and _prize_lead_raw_card_list_matches(
            raw.get("preEvolution"), pokemon.preEvolution
        )
    )


def _prize_lead_raw_pokemon_list_matches(
    raw, pokemon_list, owner: int
) -> bool:
    return (
        isinstance(raw, list)
        and len(raw) == len(pokemon_list)
        and all(
            _prize_lead_raw_pokemon_matches(raw_pokemon, pokemon, owner)
            for raw_pokemon, pokemon in zip(raw, pokemon_list)
        )
    )


def _prize_lead_raw_player_matches(raw, player, owner: int) -> bool:
    ...
    if any(raw.get(name) != getattr(player, name) for name in scalar_fields):
        return False
    raw_hand = raw.get("hand")
    if player.hand is None:
        if raw_hand is not None:
            return False
    elif not _prize_lead_raw_card_list_matches(raw_hand, player.hand):
        return False
    raw_prize = raw.get("prize")
    if not isinstance(raw_prize, list) or len(raw_prize) != len(player.prize):
        return False
    if any(
        (raw_card is None) != (card is None)
        or (
            card is not None
            and not _prize_lead_raw_card_matches(raw_card, card)
        )
        for raw_card, card in zip(raw_prize, player.prize)
    ):
        return False
    return (
        _prize_lead_raw_card_list_matches(raw.get("discard"), player.discard)
        and _prize_lead_raw_pokemon_list_matches(
            raw.get("active"), player.active, owner
        )
        and _prize_lead_raw_pokemon_list_matches(
            raw.get("bench"), player.bench, owner
        )
    )


def _prize_lead_raw_option_matches(raw, option) -> bool:
    if not isinstance(raw, dict) or raw.get("type") != int(option.type):
        return False
    enum_fields = ("area", "inPlayArea", "specialConditionType")
    scalar_fields = (
        "number",
        "index",
        "playerIndex",
    ...
    for name in enum_fields:
        parsed = getattr(option, name, None)
        if raw.get(name) != (None if parsed is None else int(parsed)):
            return False
    return all(
        raw.get(name) == getattr(option, name, None) for name in scalar_fields
    )


def _prize_lead_raw_and_parsed_agree(
    ...
    state = obs.current
    select = obs.select
    raw_current = obs_dict.get("current")
    raw_select = obs_dict.get("select")
    raw_logs = obs_dict.get("logs")
    if (
        state is None
        or select is None
        or not isinstance(raw_current, dict)
        or not isinstance(raw_select, dict)
        or not isinstance(raw_logs, list)
        or len(raw_logs) != len(obs.logs)
    ):
        return False
    state_fields = (
        "turn",
    ...
    if any(
        raw_current.get(name) != getattr(state, name) for name in state_fields
    ):
        return False
    if raw_current.get("looking") is not None or state.looking is not None:
        return False
    raw_players = raw_current.get("players")
    if (
        not isinstance(raw_players, list)
        or len(raw_players) != 2
        or len(state.players) != 2
        or not all(
            _prize_lead_raw_player_matches(raw, player, owner)
            for owner, (raw, player) in enumerate(
                zip(raw_players, state.players)
            )
        )
        or not _prize_lead_raw_card_list_matches(
            raw_current.get("stadium"), state.stadium
        )
    ):
        return False
    select_enum_fields = ("type", "context")
    select_scalar_fields = (
        "minCount",
        "maxCount",
        "remainDamageCounter",
        "remainEnergyCost",
    ...
    if any(
        raw_select.get(name) != int(getattr(select, name))
        for name in select_enum_fields
    ) or any(
        raw_select.get(name) != getattr(select, name)
        for name in select_scalar_fields
    ):
        return False
    if (
        raw_select.get("deck") is not None
        or select.deck is not None
        or raw_select.get("contextCard") is not None
        or select.contextCard is not None
        or raw_select.get("effect") is not None
    ...
    if (
        ...
        or len(raw_options) != len(select.option)
        or not all(
            _prize_lead_raw_option_matches(raw, option)
            for raw, option in zip(raw_options, select.option)
        )
    ):
        return False
    log_fields = (
    ...
    for raw, log in zip(raw_logs, obs.logs):
        if not isinstance(raw, dict):
            return False
        for name in log_fields:
            parsed = getattr(log, name, None)
            if name in ("type", "fromArea", "toArea") and parsed is not None:
                parsed = int(parsed)
            if raw.get(name) != parsed:
                return False
    return True


def _prize_lead_public_state_is_complete(obs: Observation) -> bool:
    state = obs.current
    if (
        state is None
    ...
    for owner, player in enumerate(state.players):
        if (
            ...
            or not isinstance(player.prize, list)
            or any(
                not _prize_lead_known_stack_is_exact(pokemon, owner)
                for pokemon in list(player.active) + list(player.bench)
            )
            or any(
                not _prize_lead_card_is_exact(card, owner)
                for card in player.discard
            )
            or any(
                card is not None
                and not _prize_lead_card_is_exact(card, owner)
                for card in player.prize
            )
        ):
            return False
    if any(
        not _prize_lead_card_is_exact(card, card.playerIndex)
        or card.playerIndex not in (0, 1)
        or card_table[card.id].cardType != CardType.STADIUM
        for card in state.stadium
    ):
        return False
    if any(
        not _prize_lead_card_is_exact(card, state.yourIndex)
        for card in mine.hand
    ):
        return False
    public_serials = _bridge_public_serials(state)
...
def _prize_lead_local_energy_hp_delta(
...
def _prize_lead_local_tool_hp_delta(tool: Card) -> int | None:
...
def _prize_lead_max_hp_is_exact(state, pokemon: Pokemon) -> bool:
    data = card_table.get(pokemon.id)
    ...
    for player in state.players:
        for source in list(player.active) + list(player.bench):
            source_data = card_table.get(source.id)
            if source_data is None or any(
                " hp" in (" " + " ".join(_normalized_skill_text(skill.text).split()))
                for skill in (source_data.skills or [])
            ):
                return False
            for tool in source.tools:
                delta = _prize_lead_local_tool_hp_delta(tool)
                if delta is None:
                    return False
                if source.serial == pokemon.serial:
                    expected += delta
            for energy in source.energyCards:
                delta = _prize_lead_local_energy_hp_delta(energy, source)
                if delta is None:
                    return False
                if source.serial == pokemon.serial:
                    expected += delta
...
def _prize_lead_powerful_hand_metadata_is_exact() -> bool:
    powerful = attack_table.get(ATTACK_POWERFUL_HAND)
    alakazam = card_table.get(Alakazam)
    return (
        powerful is not None
        and alakazam is not None
        and alakazam.attacks == [ATTACK_POWERFUL_HAND]
...
def _prize_lead_attack_option_is_exact(option) -> bool:
    return (
        option.type == OptionType.ATTACK
        and option.attackId == ATTACK_POWERFUL_HAND
        and _prize_lead_option_unused_fields_are_none(option, {"attackId"})
    )


def _prize_lead_target_prize_is_unambiguous(target: Pokemon) -> bool:
    return (
        all(card.id != 12 for card in target.energyCards)
        and all(card.id != 1172 for card in target.tools)
        and prize_count(target) > 0
    )


def _prize_lead_visible_effects_are_clear(
    ...
    for owner, player in enumerate(state.players):
        for pokemon in list(player.active) + list(player.bench):
            if not _prize_lead_known_stack_is_exact(pokemon, owner):
                return False
            data = card_table.get(pokemon.id)
    ...
    for log in obs.logs:
        for name in (
            ...
            card_id = getattr(log, name, None)
            if card_id is not None and card_table.get(card_id) is None:
                return False
        if (
...
def _prize_lead_exact_play_card(
    ...
    if (
        ...
        or option.index < 0
        or not _prize_lead_option_unused_fields_are_none(option, {"index"})
    ):
        return None
    card = get_card(obs, AreaType.HAND, option.index, my_index)
    return card if _prize_lead_card_is_exact(card, my_index) else None


def _prize_lead_draw_ability_metadata_is_exact(card_id: int) -> bool:
    expected = {
        Kadabra: ("psychic draw", "draw 2 cards"),
        Alakazam: ("psychic draw", "draw 3 cards"),
        Dudunsparce: ("run away draw", "draw 3 cards"),
        Fezandipiti_ex: ("flip the script", "draw 3 cards"),
    }.get(card_id)
    data = card_table.get(card_id)
    if expected is None or data is None or len(data.skills or []) != 1:
        return False
    skill = data.skills[0]
    return (
        " ".join((skill.name or "").lower().split()) == expected[0]
        and expected[1] in " ".join(_normalized_skill_text(skill.text).split())
    )


def _prize_lead_parent_detour_is_exact(
    ...
    if (
        ...
        or isinstance(parent_action[0], bool)
        or not 0 <= parent_action[0] < len(select.option)
    ):
        return False
    option = select.option[parent_action[0]]
    if option.type == OptionType.PLAY:
        card = _prize_lead_exact_play_card(obs, option, my_index)
        return card is not None and card.id in _PRIZE_LEAD_DETOUR_PLAY_IDS
    if option.type == OptionType.ATTACH:
        if (
            ...
            or isinstance(option.inPlayIndex, bool)
            or option.inPlayIndex < 0
            or not _prize_lead_option_unused_fields_are_none(
        ...
        card = get_card(obs, AreaType.HAND, option.index, my_index)
        target = get_card(
            obs, option.inPlayArea, option.inPlayIndex, my_index
        )
        return (
            _prize_lead_card_is_exact(card, my_index)
            and card.id == Lucky_Helmet
            and card_table[card.id].cardType == CardType.TOOL
            and _prize_lead_known_stack_is_exact(target, my_index)
        )
    if option.type == OptionType.ABILITY:
        if (
            ...
            return False
        source = get_card(obs, option.area, option.index, my_index)
        return (
            _prize_lead_known_stack_is_exact(source, my_index)
            and source.id in _PRIZE_LEAD_DRAW_ABILITY_IDS
            and _prize_lead_draw_ability_metadata_is_exact(source.id)
        )
    return False


def _prize_lead_no_higher_boss_route(
    obs: Observation, hand_size: int, own_prizes: int
) -> bool:
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    ...
    for option_index, option in enumerate(select.option):
        ...
        if card.id == Boss_Orders:
            boss_options.append(option_index)

    target_rows = []
    for target in theirs.bench:
        if (
            not _prize_lead_known_stack_is_exact(target, 1 - my_index)
            or not _prize_lead_max_hp_is_exact(state, target)
            or not _prize_lead_target_prize_is_unambiguous(target)
            or not _prize_lead_visible_effects_are_clear(obs, target)
        ):
            return False
        target_rows.append((target, prize_count(target)))
    if not boss_options:
        return True
    if hand_size <= 1:
        return True
    post_boss_damage = 20 * (hand_size - 1)
    return not any(
        post_boss_damage >= target.hp
        and (target_prizes > 1 or target_prizes >= own_prizes)
        for target, target_prizes in target_rows
    )


def _prize_lead_successor_floor_certificate(
    obs: Observation,
) -> dict | None:
    """Prove a ready successor or two complete Benched Stage-2 lines."""
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    complete_alakazam = []
    for pokemon in mine.bench:
        if pokemon.id not in (Kadabra, Alakazam):
            continue
        if (
            pokemon.serial == active.serial
            or not _prize_lead_known_stack_is_exact(pokemon, my_index)
            or not _prize_lead_max_hp_is_exact(state, pokemon)
        ):
            return None
        if pokemon.id == Alakazam:
            complete_alakazam.append(pokemon)
        ...
        for attack_id in data.attacks:
            ...
            if attack is None or attack.attackId != attack_id:
                return None
            payment = _bridge_retaliation_can_pay(
                energy_units, attack.energies
            )
            if payment is None:
                return None
            payable = payable or payment
        if payable:
            ready_serials.append(pokemon.serial)

    alakazam_serials = [pokemon.serial for pokemon in complete_alakazam]
    double_alakazam = (
        len(alakazam_serials) >= 2
        and len(alakazam_serials) == len(set(alakazam_serials))
        and any(not pokemon.appearThisTurn for pokemon in complete_alakazam)
    )
    ready_successor = bool(ready_serials)
    if not (ready_successor or double_alakazam):
        return None
    return {
        "ready_successor": ready_successor,
        "double_alakazam": double_alakazam,
        "ready_serials": tuple(sorted(ready_serials)),
        "complete_alakazam_serials": tuple(sorted(alakazam_serials)),
    }


def _prize_lead_one_prize_active_ko_action(
    obs: Observation,
    obs_dict: dict,
    parent_action: list[int],
    owner_existed_at_entry: bool,
) -> list[int] | None:
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    ...
    if (
        ...
        or _stranded_retreat_ko_latch
        or _guarded_teleportation_latch
        or not _prize_lead_public_state_is_complete(obs)
        or not _prize_lead_raw_and_parsed_agree(obs, obs_dict)
        or not _prize_lead_powerful_hand_metadata_is_exact()
        or len(mine.active) != 1
        or len(theirs.active) != 1
        or not theirs.bench
        or any(
    ...
    if (
        ...
        or not _prize_lead_known_stack_is_exact(active, my_index)
        or not _prize_lead_max_hp_is_exact(state, active)
        or hand is None
        or len(hand) != mine.handCount
        or powerful is None
        or energy_units is None
        or _bridge_retaliation_can_pay(
            energy_units, powerful.energies
        )
        is not True
        or not _prize_lead_known_stack_is_exact(target, 1 - my_index)
        or not _prize_lead_max_hp_is_exact(state, target)
        or not _prize_lead_target_prize_is_unambiguous(target)
        or prize_count(target) != 1
        or not _prize_lead_visible_effects_are_clear(obs, target)
    ...
    for option_index, option in enumerate(select.option):
        if option.type != OptionType.ATTACK and option.attackId is None:
            continue
        if not _prize_lead_attack_option_is_exact(option):
            return None
        attack_options.append(option_index)
    if len(attack_options) != 1:
        return None
    if parent_action == [attack_options[0]]:
        return None
    ...
    if (
        own_prizes < 2
        or opponent_prizes < 1
        or opponent_prizes - own_prizes < 2
    ):
        return None

    if (
        _prize_lead_successor_floor_certificate(obs) is None
        or not _prize_lead_parent_detour_is_exact(obs, parent_action)
        or not _prize_lead_no_higher_boss_route(
            obs, len(hand), own_prizes
        )
    ):
        return None
...
def agent(obs_dict: dict) -> list[int]:
    ...
    select = obs.select
    context = select.context
    prize_lead_owner_existed_at_entry = bool(
        _hilda_source_latch
        or _enriching_reserve_latch
        or _fez_ko_bridge_latch
        or _active_psychic_ko_latch
    ...
    prize_lead_parent_action = desc_indices[:select.maxCount]
    try:
        prize_lead_action = _prize_lead_one_prize_active_ko_action(
            obs,
            obs_dict,
            prize_lead_parent_action,
            prize_lead_owner_existed_at_entry,
        )
    except (
        AttributeError,
        IndexError,
        ...
        prize_lead_action = None
    if prize_lead_action is not None:
        return _remember_action(decision_signature, prize_lead_action)
