...
def shaymin_access_ready(obs) -> bool:
    return ready_alakazam(obs) or ALAKAZAM in hand_ids(obs)
...
def crustletempo_blocks_engineflow(obs) -> bool:
    """Use only the opponent's visible board for the Crustle tempo exception."""
    visible = all_opp_pokemon(obs)
    crustles = [pokemon for pokemon in visible if pokemon.id == CRUSTLE]
    if not crustles:
        return False
    kangaskhans = [pokemon for pokemon in visible if pokemon.id == MEGA_KANGASKHAN_EX]
    full_wall = (
        len(kangaskhans) >= 2
        and len(crustles) >= 3
        and not any(pokemon.id == DWEBBLE for pokemon in visible)
        and any(hp(pokemon) < max_hp(pokemon) for pokemon in kangaskhans)
    )
    return not full_wall
...
def setup_score(obs, opt) -> tuple[int, str]:
    ...
    if cid == SHAYMIN and not shaymin_access_ready(obs):
        return -1000, "hold Shaymin for Alakazam"
...
def setup_gate_reason(obs, opt) -> str | None:
    ...
    if crustletempo_blocks_engineflow(obs):
        return None
    card = option_card(obs, opt)
    cid = getattr(card, "id", None)
