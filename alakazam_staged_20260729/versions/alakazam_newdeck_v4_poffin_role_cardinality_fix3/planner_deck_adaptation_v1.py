...
RULE_POFFIN_ROLE_CARDINALITY = "V4_POFFIN_ROLE_CARDINALITY"
...
LAST_V4_POFFIN_TRACE: dict[str, Any] | None = None
...
def _poffin_trace(details: dict[str, Any]) -> None:
    """Publish the C1 proof without removing any inherited trace fields."""
    global LAST_V4_POFFIN_TRACE
    LAST_V4_POFFIN_TRACE = copy.deepcopy(details)
    LAST_V1_PACKAGE_TRACE["v4_poffin_role_cardinality"] = copy.deepcopy(
        details
    )
...
def agent(parent, v0_agent, obs_dict):
    ...
    try:
        ...
        if not runtime_model.raw_parsed_agree(obs_dict, obs):
            if active_transaction is not None:
                if _v4_owned_optional_empty_abort(
                    active_transaction,
                    obs_dict,
                    obs,
                    context,
        ...
        if V1_TRANSACTION is not None:
            ...
            elif stage == "await_v4_poffin_child":
                action, complete = _advance_v4_poffin_child(
                    parent, obs, transaction
                )
            elif stage == "await_v4_poffin_complete":
                action = _v4_poffin_completion_replay(
                    parent, obs, transaction
                )
                complete = action is None
            elif stage == "await_v4_poffin_main_complete":
                complete = True
            if action is not None and model.action_is_valid(obs, action):
                ...
                _trace(
                    ...
                    *psychic_tags,
                    *(
                        (transaction["poffin_fail_reason"],)
                        if (
                            transaction["rule"]
                            == RULE_POFFIN_ROLE_CARDINALITY
                            and transaction.get("poffin_fail_reason")
                        )
                        else ()
                    ),
                )
                if transaction["rule"] == RULE_POFFIN_ROLE_CARDINALITY:
                    _poffin_trace(transaction["poffin_trace"])
                return action
            if complete:
                completed_rule = transaction["rule"]
                if (
                    completed_rule == RULE_POFFIN_ROLE_CARDINALITY
                    and stage in (
                        "await_v4_poffin_complete",
                        "await_v4_poffin_main_complete",
                    )
                ):
                    return _v4_reenter_after_completion(
                        parent,
                        v0_agent,
                        obs_dict,
                        transaction,
                ...
                if completed_rule == RULE_POFFIN_ROLE_CARDINALITY:
                    details = copy.deepcopy(
                        transaction.get("poffin_trace", {})
                    )
                    details["completion_parent_action"] = list(v0_action)
                    details["completion_applied_action"] = list(v0_action)
                    details["completion_selected_rule"] = None
                    if transaction.get("poffin_fail_reason") is not None:
                        details["completion_fail_closed_reason"] = (
                            transaction["poffin_fail_reason"]
                        )
                    _poffin_trace(details)
                return v0_action
            fault_action = _irreversible_fault_action(parent, obs)
        ...
        poffin_child = _v4_poffin_child_decision(
            parent, obs, v0_action
        )
        if poffin_child is not None:
            action, transaction = poffin_child
            if model.action_is_valid(obs, action):
                _restore_delegate_state(parent, delegate_pre)
                ...
                _trace(
                    ...
                    RULE_POFFIN_ROLE_CARDINALITY,
                    "EXACT_CHILD_PROMPT",
                    transaction["poffin_trace"]["classification"],
                )
                _poffin_trace(transaction["poffin_trace"])
                return action
        ...
        poffin_main = _v4_poffin_main_decision(
            parent, obs, obs_dict, v0_action, snap
        )
        public = _main_envelope(parent, obs)
        if public is None:
            if poffin_main is not None:
                return _apply_v4_poffin_main(
                    parent,
                    v0_agent,
                    obs_dict,
                    obs,
...
# --- v4 C1: exact Buddy-Buddy Poffin demand and 0/1/2 role cardinality. ---

def _v4_role_board(parent, obs):
    state = obs.current
    if (
        state is None
        or state.yourIndex not in (0, 1)
    ...
    if (
        ...
        return None
    field = list(mine.active) + list(mine.bench)
    serials = []
    for pokemon in field:
        if (
            ...
            return None
        serials.extend(parent._bridge_pokemon_component_serials(pokemon))
    if (
        any(type(serial) is not int or serial <= 0 for serial in serials)
        or len(serials) != len(set(serials))
    ):
        return None
    abra_count = sum(
        pokemon.id in (ABRA, KADABRA, ALAKAZAM) for pokemon in field
    )
    dunsparce_count = sum(
        pokemon.id in (DUNSPARCE, DUDUNSPARCE) for pokemon in field
    )
    free_bench = mine.benchMax - len(mine.bench)
    return {
        "owner": owner,
        "A": abra_count,
        "N": dunsparce_count,
...
def _v4_public_role_inventory(parent, obs, obs_dict):
    """Prove remaining role basics from the exact 60-card public partition."""
    state = obs.current
    if (
        state is None
        or state.yourIndex not in (0, 1)
...
def _v4_main_poffin_row(parent, obs, action):
    if (
    ...
    key = runtime_model.stable_option_key(parent, obs, option)
    if (
        row != (BUDDY_BUDDY_POFFIN, card.serial, owner)
        or key is None
    ):
        return None
    return option_index, option.index, card, key


def _v4_role_deficits(a_count, n_count):
    return {
        "A_PRIMARY": max(0, 1 - a_count),
        "N_PRIMARY": max(0, 1 - n_count),
        "A_BACKUP": max(0, 2 - a_count),
        "N_BACKUP": max(0, 2 - n_count),
    }
...
def _v4_trace_base(
    *,
    ...
    return {
        "rule": RULE_POFFIN_ROLE_CARDINALITY,
        "parent_action": list(parent_action),
        "applied_action": list(applied_action),
        "A": board["A"],
        "N": board["N"],
        "F": board["F"],
        "normal_capacity": normal_capacity,
        "abra_final_slot_exception": bool(exception),
        "role_deficits_before": copy.deepcopy(before),
        "role_deficits_after": copy.deepcopy(after),
        "resolved_candidates": copy.deepcopy(candidates),
        "selected_candidates": copy.deepcopy(selected),
...
def _v4_poffin_main_decision(
    parent, obs, obs_dict, parent_action, snap
):
    select = obs.select
    if (
        select is None
        ...
        return None
    row = _v4_main_poffin_row(parent, obs, parent_action)
    board = _v4_role_board(parent, obs)
    public = _public_state(parent, obs)
    ...
    if (
        ...
        return None
    option_index, hand_index, card, option_key = row
    ...
    fail_reason = None
    if board["F"] == 0:
        fail_reason = "NO_LEGAL_EMPTY_BENCH"
    elif unknown_abra == 0 and unknown_dunsparce == 0:
        fail_reason = "PUBLIC_ROLE_BASICS_DEPLETED"
    elif normal_capacity == 0 and not exception_possible:
        fail_reason = "ZERO_NORMAL_CAPACITY_NO_ABRA_EXCEPTION"
    elif board["A"] >= 2 and board["N"] >= 2:
        fail_reason = "ZERO_ROLE_DEMAND"
    classification = (
        "MAIN_VETO_ZERO_DEMAND"
        if fail_reason is not None
        else "MAIN_PRESERVE_PARENT_POFFIN"
    )
    details = _v4_trace_base(
        parent_action=parent_action,
        applied_action=parent_action,
        board=board,
        normal_capacity=normal_capacity,
        exception=exception_possible,
        before=_v4_role_deficits(board["A"], board["N"]),
        after=_v4_role_deficits(board["A"], board["N"]),
        candidates=[],
        selected=[],
        classification=classification,
    ...
    details.update(
        public_inventory_count=inventory["public_card_count"],
    ...
    hidden_role_target_expected = bool(
        fail_reason is None
        and (
            (board["A"] < 2 and unknown_abra >= 1)
            or (board["N"] < 2 and unknown_dunsparce >= 1)
        )
        and (normal_capacity >= 1 or exception_possible)
    )
    return {
        "classification": classification,
        "board": board,
        ...
        "inventory": inventory,
        "hidden_role_target_expected": hidden_role_target_expected,
        "trace": details,
        "snapshot_hash": snap.sha256,
    }
...
def _v4_filtered_parent_rerank(
    ...
    for option_index, option in enumerate(obs.select.option):
        if (
            option.type == parent.OptionType.PLAY
            and type(option.index) is int
            and 0 <= option.index < len(mine.hand or ())
            and mine.hand[option.index].id == BUDDY_BUDDY_POFFIN
            and _exact_option(
                option, parent.OptionType.PLAY, index=option.index
            )
        ):
            removed_indices.append(option_index)
    ...
    filtered_raw["select"]["option"] = [
        ...
        if index not in set(removed_indices)
    ]
    try:
        _restore_delegate_state(parent, delegate_pre)
        ...
        keys = tuple(
            runtime_model.stable_option_key(
                parent, filtered_obs, filtered_obs.select.option[index]
            )
        ...
        if (
            ...
            or not model.action_is_valid(obs, applied_action)
            or any(
                index in removed_indices for index in applied_action
            )
        ):
            raise ValueError("FILTERED_PARENT_ACTION_REBIND_FAILED")
...
def _apply_v4_poffin_main(
    ...
    global V1_TRANSACTION
    details = copy.deepcopy(decision["trace"])
    if decision["classification"] == "MAIN_VETO_ZERO_DEMAND":
        action = _v4_filtered_parent_rerank(
            parent, v0_agent, obs_dict, obs, delegate_pre
        )
        if action is None:
            details["classification"] = (
                "MAIN_RERANK_UNCERTIFIED_PARENT_PRESERVED"
            )
            details["fail_closed_reason"] = "PARENT_RERANK_NOT_CERTIFIED"
            _trace(
                snap.sha256,
                int(obs.select.context),
                parent_action,
                None,
                "V4_POFFIN_RERANK_FAIL_CLOSED",
                "V0_FALLBACK",
            )
            _poffin_trace(details)
            return list(parent_action)
        details["applied_action"] = list(action)
        rerank_state = _delegate_state_snapshot(parent)
        rerank_owner = (
            core.INTEGRATED_TRANSACTION is not None
            or core.parent_owner_active(rerank_state["parent"])
        )
        V1_TRANSACTION = {
            "rule": RULE_POFFIN_ROLE_CARDINALITY,
            "owner": decision["board"]["owner"],
            "stage": "await_v4_poffin_main_complete",
            "rerank_owner_established": rerank_owner,
            "poffin_trace": details,
        }
        _remember(parent, obs, snap.sha256, action)
        _trace(
            snap.sha256,
    ...
    V1_TRANSACTION = {
        "rule": RULE_POFFIN_ROLE_CARDINALITY,
        "owner": decision["board"]["owner"],
        "stage": "await_v4_poffin_child",
        "snapshot_hash": decision["snapshot_hash"],
        "start": decision["public"],
        "card_row": decision["card_row"],
        "card_serial": decision["card_serial"],
        "option_key": decision["option_key"],
        "action": tuple(action),
        "hidden_role_target_expected": decision[
            "hidden_role_target_expected"
        ],
        "poffin_trace": details,
    }
    _remember(parent, obs, snap.sha256, action)
...
def _v4_poffin_effect_row(obs):
    rows = [
        _card_row(card)
        for card in (obs.select.effect, obs.select.contextCard)
        if card is not None
    ]
    if (
        not rows
        or any(
            row is None or row[0] != BUDDY_BUDDY_POFFIN
            for row in rows
        )
        or len(set(rows)) != 1
    ):
...
def _v4_poffin_child_envelope(parent, obs, transaction=None):
    select = obs.select
    if (
        select is None
        ...
        or select.maxCount not in (1, 2)
        or select.remainDamageCounter != 0
        or select.remainEnergyCost != 0
        or not isinstance(select.deck, list)
    ):
        return None, "PROMPT_ENVELOPE_MISMATCH"
    effect_row = _v4_poffin_effect_row(obs)
    if (
        effect_row is None
        or effect_row[2] != obs.current.yourIndex
    ):
        return None, "EFFECT_MISMATCH"
    if (
        transaction is not None
        and effect_row != transaction.get("card_row")
    ):
        return None, "EFFECT_TRANSACTION_MISMATCH"
    census = _option_census(parent, obs)
    if census is None:
        return None, "OPTION_CENSUS_AMBIGUOUS"
    owner = obs.current.yourIndex
    candidates = []
    rows = []
    serials = []
    for option_index, (option, key) in enumerate(
        zip(select.option, census)
    ):
        card = core._option_card(parent, obs, option)
        row = _card_row(card)
        if (
            option.type != parent.OptionType.CARD
            or option.area != parent.AreaType.DECK
            or option.playerIndex not in (None, owner)
            or type(option.index) is not int
            or not 0 <= option.index < len(select.deck)
            or row is None
            or row[2] != owner
            or select.deck[option.index].serial != row[1]
            or not _exact_option(
                option,
                parent.OptionType.CARD,
            ...
            return None, "OPTION_SOURCE_NOT_EXACT_OWN_DECK_CARD"
        rows.append(row)
    ...
    if (
        ...
        return None, "DUPLICATE_PHYSICAL_CARD_OR_SERIAL"
    return {
...
def _v4_select_poffin_roles(parent, obs, parent_action, transaction=None):
    envelope, fail_reason = _v4_poffin_child_envelope(
        parent, obs, transaction
    )
    if envelope is None:
    ...
    board = _v4_role_board(parent, obs)
    if board is None:
        return None, "ROLE_BOARD_AMBIGUOUS"
    candidates = sorted(
        envelope["candidates"],
        key=lambda row: (
            row["card_id"],
    ...
    normal_capacity = min(
        obs.select.maxCount, 2, max(0, board["F"] - 1)
    )
    has_abra = any(row["card_id"] == ABRA for row in candidates)
    exception = (
        normal_capacity == 0
        and board["A"] == 0
        and board["F"] == 1
    ...
    capacity = 1 if exception else normal_capacity
    projected_a, projected_n = board["A"], board["N"]
    selected = []
    used_serials = set()
    ...
    def choose(card_id):
        return next(
            (
                ...
                and row["card_serial"] not in used_serials
            ),
            None,
        )

    priorities = (
        ("A_PRIMARY", ABRA, lambda: projected_a < 1),
        ("N_PRIMARY", DUNSPARCE, lambda: projected_n < 1),
        ("A_BACKUP", ABRA, lambda: projected_a < 2),
        ("N_BACKUP", DUNSPARCE, lambda: projected_n < 2),
    )
    for role, card_id, needed in priorities:
        if len(selected) >= capacity or not needed():
            continue
        chosen = choose(card_id)
        if chosen is None:
            continue
        chosen = copy.deepcopy(chosen)
        chosen["role"] = role
        selected.append(chosen)
        used_serials.add(chosen["card_serial"])
        if card_id == ABRA:
            projected_a += 1
        else:
            projected_n += 1

    action = [row["option_index"] for row in selected]
    if not model.action_is_valid(obs, action):
        return None, "SELECTED_CARDINALITY_NOT_LEGAL"
    public_candidates = [
    ...
    classification = f"CHILD_SELECT_{len(action)}"
    details = _v4_trace_base(
        parent_action=parent_action,
        ...
        exception=exception,
        before=_v4_role_deficits(board["A"], board["N"]),
        after=_v4_role_deficits(projected_a, projected_n),
        candidates=public_candidates,
        selected=public_selected,
        classification=classification,
        fail_reason=None,
    )
    return (action, details), None


def _v4_poffin_child_decision(parent, obs, parent_action):
    result, _ = _v4_select_poffin_roles(
        parent, obs, parent_action, None
    )
    if result is None:
        return None
    ...
    transaction = {
        "rule": RULE_POFFIN_ROLE_CARDINALITY,
        "owner": obs.current.yourIndex,
        "stage": "await_v4_poffin_complete",
        "poffin_trace": details,
    }
    _v4_arm_child_replay(parent, obs, action, transaction)
    return action, transaction


def _v4_owned_play_delta(parent, obs, transaction):
    return (
        _played_delta(
            parent,
            obs,
...
def _advance_v4_poffin_child(parent, obs, transaction):
    envelope, envelope_reason = _v4_poffin_child_envelope(
        parent, obs, transaction
    )
    if envelope is None:
        transaction["poffin_fail_reason"] = envelope_reason
        return None, True
    if _v4_owned_play_delta(parent, obs, transaction) is None:
        transaction["poffin_fail_reason"] = "STALE_POST_PLAY_BOARD_DELTA"
        return None, True
    parent_action = _certify_delegate_action(
        obs, _deterministic_legal_action(obs, prefer_nonempty=False)
    )
    result, fail_reason = _v4_select_poffin_roles(
        parent, obs, parent_action or [], transaction
    )
    if result is None:
        transaction["poffin_fail_reason"] = fail_reason
        if obs.select.minCount == 0 and model.action_is_valid(obs, []):
            action = []
            details = copy.deepcopy(transaction["poffin_trace"])
            details.update(
                parent_action=list(parent_action or []),
                applied_action=[],
                selected_cardinality=0,
                classification="CHILD_SELECT_0",
                fail_closed_reason=fail_reason,
            )
            transaction["poffin_trace"] = details
            transaction["stage"] = "await_v4_poffin_complete"
            _v4_arm_child_replay(parent, obs, action, transaction)
            return action, False
        return None, True
    action, details = result
    if (
        not action
        and transaction.get("hidden_role_target_expected") is True
    ):
        details["fail_closed_reason"] = "HIDDEN_ZONE_TARGET_WHIFF"
        transaction["poffin_fail_reason"] = "HIDDEN_ZONE_TARGET_WHIFF"
    transaction["poffin_trace"] = details
    transaction["stage"] = "await_v4_poffin_complete"
    _v4_arm_child_replay(parent, obs, action, transaction)
    return action, False
...
def _v4_arm_child_replay(parent, obs, action, transaction):
    envelope, _ = _v4_poffin_child_envelope(parent, obs, None)
    board = _v4_role_board(parent, obs)
    if envelope is None or board is None:
        return
    transaction["selected_keys"] = tuple(
        runtime_model.stable_option_key(
            parent, obs, obs.select.option[index]
        )
        for index in action
    )
    transaction["child_candidate_keys"] = tuple(
        sorted(
            (
                row["semantic_key"]
    ...
    transaction["child_effect_row"] = envelope["effect_row"]
    transaction["child_fields"] = board["fields"]
    transaction["child_counts"] = (
        board["A"],
        board["N"],
        board["F"],
    )
...
def _v4_poffin_completion_replay(parent, obs, transaction):
    envelope, _ = _v4_poffin_child_envelope(parent, obs, None)
    board = _v4_role_board(parent, obs)
    keys = transaction.get("selected_keys")
    if (
        envelope is None
        or board is None
        or not isinstance(keys, tuple)
        or envelope["effect_row"] != transaction.get("child_effect_row")
        or board["fields"] != transaction.get("child_fields")
        or (board["A"], board["N"], board["F"])
        != transaction.get("child_counts")
        or tuple(
            sorted(
                row["semantic_key"]
                for row in envelope["candidates"]
        ...
        != transaction.get("child_candidate_keys")
    ):
        return None
    mapping = {}
    for index, option in enumerate(obs.select.option):
...
def _v4_reenter_after_completion(
    parent,
    v0_agent,
    obs_dict,
    transaction,
):
    """Release C1 and arbitrate this new callback exactly once end to end."""
    original_details = copy.deepcopy(
        transaction.get("poffin_trace", {})
    )
    completion_parent_actions = []

    def capture_parent(raw):
        action = v0_agent(raw)
        completion_parent_actions.append(
            list(action) if isinstance(action, (list, tuple)) else []
        )
        return action

    completion_fail_reason = transaction.get("poffin_fail_reason")
    reset()
    applied_action = agent(parent, capture_parent, obs_dict)
    completion_trace = copy.deepcopy(LAST_V1_PACKAGE_TRACE)
    details = original_details
    details["completion_parent_action"] = (
        completion_parent_actions[0]
        if completion_parent_actions
        else []
    )
    details["completion_applied_action"] = list(applied_action)
    details["completion_selected_rule"] = completion_trace.get(
        "selected_rule"
    )
    if completion_fail_reason is not None:
        details["completion_fail_closed_reason"] = completion_fail_reason
    _poffin_trace(details)
    return applied_action


def _v4_owned_optional_empty_abort(
    transaction,
    obs_dict,
    obs,
    context,
    ...
    if (
        transaction.get("rule") != RULE_POFFIN_ROLE_CARDINALITY
        or transaction.get("stage") != "await_v4_poffin_child"
        or obs is None
        or obs.select is None
        or obs.select.minCount != 0
        or not model.action_is_valid(obs, [])
        or _certify_raw_action(obs_dict, []) != []
    ):
        return False
    details = copy.deepcopy(transaction.get("poffin_trace", {}))
    details.update(
        parent_action=[],
        applied_action=[],
        selected_cardinality=0,
    ...
    _trace(
        ...
        [],
        RULE_POFFIN_ROLE_CARDINALITY,
        "V4_IRREVERSIBLE_OPTIONAL_EMPTY_ABORT",
        reason,
    )
    _poffin_trace(details)
    return True
