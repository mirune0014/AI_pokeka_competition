...
# Deck-only causal control.  This is intentionally appended after the formal
# parent callable and does not import Candidate F or any of its transaction
# state.  The only policy difference is that an Xerosic option is never
# selected; every other parent decision is returned byte-for-byte.
_CONTROL_XEROSIC_ID = 1197
_CONTROL_LILLIE_ID = 1227
_CONTROL_POKEGEAR_ID = 1122
_CONTROL_PARENT_AGENT = agent
_CONTROL_TELEMETRY = {
    "parent_call_count": 0,
    "xerosic_options_seen": 0,
    "xerosic_suppressions": 0,
    "lillie_fallbacks": 0,
    "no_alternative_fallbacks": 0,
    "observation_unknown": 0,
}


def _control_option_card_id(obs, option):
    try:
        card = _parent.option_card(obs, option)
    except Exception:
        card = None
    if card is None:
        return None
    value = getattr(card, "id", getattr(card, "cardId", None))
    return value if isinstance(value, int) and not isinstance(value, bool) else None


def _control_option_rows(obs):
    options = getattr(getattr(obs, "select", None), "option", None)
    if not isinstance(options, list):
        return None
    rows = []
    for position, option in enumerate(options):
        option_type = getattr(option, "type", None)
        card_id = _control_option_card_id(obs, option)
        rows.append((position, option, option_type, card_id))
    return tuple(rows)


def _control_xerosic_positions(obs, rows):
    if rows is None:
        return ()
    # An opponent's Xerosic effect asks us to choose cards to DISCARD.  Those
    # hand-card options are not a Xerosic PLAY/CARD choice and must remain in
    # the legal discard set; suppressing one there would make a required
    # multi-card selection shorter and produce an invalid action.
    context = getattr(getattr(obs, "select", None), "context", None)
    if context not in {_SelectContext.MAIN, _SelectContext.TO_HAND}:
        return ()
    context = getattr(getattr(obs, "select", None), "context", None)
    effect = getattr(getattr(obs, "select", None), "effect", None)
    effect_id = getattr(effect, "id", None)
    if context == _SelectContext.MAIN:
        allowed = {_OptionType.PLAY}
    elif context == _SelectContext.TO_HAND and effect_id == _CONTROL_POKEGEAR_ID:
        allowed = {_OptionType.CARD}
    else:
        return ()
    return tuple(
        position
        for position, _option, option_type, card_id in rows
        if card_id == _CONTROL_XEROSIC_ID and option_type in allowed
    )


def _control_safe_parent_action(obs, parent_action, rows):
    """Return parent action unless it selects Xerosic, then choose a legal alternative."""
    if not isinstance(parent_action, list) or not parent_action:
        return parent_action, "parent_action_not_xerosic"
    x_positions = set(_control_xerosic_positions(obs, rows))
    if not x_positions or not any(position in x_positions for position in parent_action):
        return parent_action, "parent_action_not_xerosic"
    alternatives = [
        position
        for position, _option, _option_type, card_id in (rows or ())
        if position not in x_positions
    ]
    if not alternatives:
        _CONTROL_TELEMETRY["no_alternative_fallbacks"] += 1
        return parent_action, "xerosic_only_legal_option"
    lillie = [
        position
        for position, _option, _option_type, card_id in (rows or ())
        if position in alternatives and card_id == _CONTROL_LILLIE_ID
    ]
    if len(parent_action) == 1:
        chosen = lillie[0] if len(lillie) == 1 else alternatives[0]
        if chosen in set(lillie):
            _CONTROL_TELEMETRY["lillie_fallbacks"] += 1
        return [chosen], "xerosic_suppressed_lillie" if chosen in set(lillie) else "xerosic_suppressed_alternative"
    filtered = [position for position in parent_action if position not in x_positions]
    if filtered:
        return filtered, "xerosic_suppressed_multiselect"
    return [alternatives[0]], "xerosic_suppressed_alternative"


def _control_restore_parent_state(snapshot):
    """Restore only formal-parent commitment fields when suppression changed one."""
    self_owner, runtime_owner = snapshot
    globals()["_materialization_owner"] = self_owner
    if hasattr(_parent, "_materialization_owner"):
        _parent._materialization_owner = runtime_owner


def _control_raw_context(obs_dict):
...
def _control_final_agent(obs_dict):
    import copy as _control_copy

    raw_context = _control_raw_context(obs_dict)
    if raw_context is not None and raw_context not in {
        int(_SelectContext.MAIN),
        int(_SelectContext.TO_HAND),
    }:
        parent_action = _CONTROL_PARENT_AGENT(obs_dict)
        _CONTROL_TELEMETRY["parent_call_count"] = 1
        return parent_action

    self_before = _control_copy.deepcopy(globals().get("_materialization_owner"))
    runtime_before = _control_copy.deepcopy(getattr(_parent, "_materialization_owner", None))
    parent_action = _CONTROL_PARENT_AGENT(obs_dict)
    _CONTROL_TELEMETRY["parent_call_count"] = 1
    try:
        obs = _to_observation_class(obs_dict)
        context = getattr(getattr(obs, "select", None), "context", None)
        # Do not inspect or rewrite non-policy selection prompts.  In
        # particular, an opponent Xerosic discard prompt can expose hand
        # options whose option_card lookup is not a PLAY/CARD policy surface;
        # the formal parent action must pass through byte-for-byte.
        if context not in {_SelectContext.MAIN, _SelectContext.TO_HAND}:
            return parent_action
        rows = _control_option_rows(obs)
        x_seen = _control_xerosic_positions(obs, rows)
        _CONTROL_TELEMETRY["xerosic_options_seen"] += len(x_seen)
        action, reason = _control_safe_parent_action(obs, parent_action, rows)
        if reason != "parent_action_not_xerosic":
            _control_restore_parent_state((self_before, runtime_before))
            _CONTROL_TELEMETRY["xerosic_suppressions"] += 1
        return action
    except Exception:
        _CONTROL_TELEMETRY["observation_unknown"] += 1
        return parent_action


def get_control_telemetry():
    return dict(_CONTROL_TELEMETRY)


# Loader contract: the control is the only final exported callable.
agent = _control_final_agent
