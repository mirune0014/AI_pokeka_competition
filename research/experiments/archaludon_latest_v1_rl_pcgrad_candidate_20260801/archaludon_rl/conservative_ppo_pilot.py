...
def _expected_correction_spec() -> dict[str, Any]:
    return {
        ...
        "source_implementation_snapshot": {
            "path": (
                "research/experiments/archaludon_latest_v1_rl_temperature_candidate_20260731"
            ),
...
class _CheckpointPublicationHandoffError(RuntimeError):
    """Transfer an identity-verified public checkpoint and its held inode."""
    ...
    def __init__(
        ...
        *,
        ...
        checkpoint_guard: _StableFileGuard,
        checkpoint_readback: bytes,
    ):
        super().__init__(
            "checkpoint publication failed after public identity verification: "
        ...
        self.checkpoint_readback = checkpoint_readback


def _after_checkpoint_public_identity_verified() -> None:
    """No-op fault-injection seam after ownership becomes transferable."""
...
def _publish_checkpoint_exclusive(
    ...
    *,
    ...
    try:
        ...
        if public_identity_verified and staging is not None:
            if staging_hash is None or serialized_readback is None:
                raise RuntimeError(
                    "verified checkpoint lost its hash or readback evidence"
                ) from error
            protection = staging
            staging = None
            raise _CheckpointPublicationHandoffError(
                cause=error,
                checkpoint_path=checkpoint_path,
                checkpoint_sha256=staging_hash,
                checkpoint_guard=protection,
                checkpoint_readback=serialized_readback,
            ) from error
        raise
    finally:
