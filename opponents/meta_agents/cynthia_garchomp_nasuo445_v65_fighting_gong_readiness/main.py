...
def fighting_gong_readiness_index(
    obs,
    scored: list[tuple[int, int, str]],
) -> int | None:
    """Return Basic Fighting only for the exact public Gong readiness state."""
    try:
        ...
        if (
            ...
            or getattr(selection, "minCount", None) != 1
        ...
        if options is None or deck is None or len(scored) != len(options) or not scored:
            return None
        if sorted(index for _score, index, _reason in scored) != list(range(len(options))):
            return None

        _baseline_score, baseline_index, _baseline_reason = max(
            scored,
        ...
        for index, option in enumerate(options):
            if (
                getattr(option, "type", None) != OptionType.CARD
                or getattr(option, "area", None) != AreaType.DECK
            ):
                continue
            ...
            if card is None or card_id is None:
                return None
            if card_id == GIBLE:
        ...
        if (
            baseline_index not in gible_indices
        ...
        if any(pokemon is None or getattr(pokemon, "id", None) is None for pokemon in in_play):
            return None
...
def choose_options(obs):
    ...
    gong_index = fighting_gong_readiness_index(obs, scored)
