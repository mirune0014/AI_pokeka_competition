...
def night_stretcher_continuity_reason(obs) -> str | None:
    """Return a reason only when Stretcher restores an immediately live line."""
    discarded = discard_ids(obs)
    if GARCHOMP_EX in discarded and has_in_play(obs, GABITE):
        return "Night Stretcher Garchomp continuity"
    if GABITE in discarded and has_in_play(obs, GIBLE):
        return "Night Stretcher Gabite continuity"
    player = me(obs)
    bench_open = len([p for p in (player.bench or []) if p]) < int(
        getattr(player, "benchMax", 0) or 0
    )
    if GIBLE in discarded and bench_open:
        return "Night Stretcher Gible continuity"
    return None
...
def score_play(obs, opt) -> tuple[int, str]:
    ...
    if cid == NIGHT_STRETCHER:
        continuity_reason = night_stretcher_continuity_reason(obs)
        if continuity_reason:
            return 18900, continuity_reason
