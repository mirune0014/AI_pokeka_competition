...
def choose_options(obs):
    ...
    # Seeded terminal-reward search found a small, repeatable mirror gain from
    # attaching before otherwise close Item plays in these two turn windows.
    turn = obs.current.turn
    if (obs.select.context == SelectContext.MAIN
            and detect_matchup(obs) == "archaludon"
            and (turn <= 4 or 7 <= turn <= 10)):
        safe = sorted((row for row in scored if row[0] >= 0),
                      key=lambda row: (row[0], -row[1]), reverse=True)
        if safe:
            spread = max(safe[0][0] - safe[-1][0], 1)
            residual_pool = {row[1] for row in safe[:4]}
            scored = [
                (score + 0.03 * spread, i, reason + "; RL mirror attach")
                if i in residual_pool and obs.select.option[i].type == OptionType.ATTACH
                else (score, i, reason)
