...
def _public_state(parent, obs):
    ...
    if mine.hand is None or len(mine.hand) != mine.handCount or theirs.hand is not None or state.looking is not None:
        return None
    ...
    for player_owner, player in enumerate(state.players):
        for pokemon in list(player.active) + list(player.bench):
            if not parent._bridge_pokemon_is_publicly_complete(pokemon, player_owner) or semantics.energy_units(parent, pokemon) is None:
                return None
            fields.append(parent._bridge_pokemon_fingerprint(pokemon))
    return {
        "turn": state.turn, "action_count": state.turnActionCount, "result": state.result,
...
def _played_delta(parent, obs, transaction, opponent_changes=False):
    ...
    if (
        ...
        return None
    if transaction["rule"] in (RULE_BOSS, RULE_XEROSIC, RULE_LANA):
    ...
    try:
        ...
        if V1_TRANSACTION is not None:
            ...
            if action is not None and model.action_is_valid(obs, action):
                ...
                return action
            reset()
            _trace(snap.sha256, context, v0_action, None, "V1_TRANSACTION_COMPLETE" if complete else "V1_TRANSACTION_ABORT", "V0_FALLBACK")
            return v0_action
        ...
        if _v0_current_ko(parent, obs, v0_action):
            _trace(snap.sha256, context, v0_action, None, "CURRENT_EXACT_KO_PRECEDENCE", "V0_FALLBACK")
            return v0_action
        ...
        _remember(parent, obs, snap.sha256, action)
        _trace(snap.sha256, context, action, transaction["rule"], "EXACT_PUBLIC_PROOF", "H_FLOOR_CERTIFIED")
        return action
...
def _advance_boss(parent, obs, transaction):
    ...
    if (
        post_play is None or obs.current.yourIndex != owner
        or select.context != parent.SelectContext.SWITCH or int(select.type) != 1
        or select.minCount != 1 or select.maxCount != 1
        or _card_row(select.effect) != transaction["card_row"] or select.contextCard is not None
    ):
        return None
