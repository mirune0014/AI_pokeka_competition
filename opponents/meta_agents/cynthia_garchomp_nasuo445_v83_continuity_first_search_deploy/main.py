...
TO_HAND_POKEMON_SEARCH_SOURCES = {FIGHTING_GONG, POKE_PAD, NIGHT_STRETCHER, GABITE}
...
def exact_card_identity(card) -> tuple[int, int] | None:
    card_id = getattr(card, "id", None)
    serial = getattr(card, "serial", None)
    if card_id is None or serial is None:
        return None
    return int(card_id), int(serial)


def continuity_active_is_only_pokemon(obs) -> bool:
    player = me(obs)
    active = active_pokemon(obs)
    occupied_bench = [pokemon for pokemon in (getattr(player, "bench", None) or []) if pokemon]
    return bool(
        exact_card_identity(active) is not None
        and not occupied_bench
        and len(occupied_bench) < int(getattr(player, "benchMax", 0) or 0)
    )


def continuity_line_signature(obs) -> tuple | None:
    """Describe only the legal attack, evolution, and gust lines."""
    if obs.select.context != SelectContext.MAIN:
        return None
    attacks = []
    evolutions = []
    ...
    for option in obs.select.option:
        if option.type == OptionType.ATTACK:
            if option.attackId is None:
                return None
            attacks.append(int(option.attackId))
        elif option.type == OptionType.EVOLVE:
            evolution = exact_card_identity(option_card(obs, option))
            target = exact_card_identity(option_target(obs, option))
            if evolution is None or target is None:
                return None
            evolutions.append((evolution, target))
        elif option.type == OptionType.PLAY:
            played = option_card(obs, option)
            if getattr(played, "id", None) == BOSS:
                identity = exact_card_identity(played)
                if identity is None:
                    return None
                gusts.append(identity)
    return tuple(sorted(attacks)), tuple(sorted(evolutions)), tuple(sorted(gusts))


def continuity_pokemon_signature(pokemon) -> tuple | None:
    identity = exact_card_identity(pokemon)
    if identity is None:
        return None
    return (
        identity,
        int(getattr(pokemon, "hp", 0) or 0),
        int(getattr(pokemon, "maxHp", 0) or 0),
        energy_count(pokemon),
        tuple(
            identity
            for identity in (
                exact_card_identity(card)
...
def continuity_board_signature(obs) -> tuple | None:
    """Capture board facts a search-to-hand effect must not alter."""
    player = me(obs)
    other = opponent(obs)
    active = continuity_pokemon_signature(active_pokemon(obs))
    opposing_active = continuity_pokemon_signature(opponent_active(obs))
    if active is None or opposing_active is None:
        return None
    own_bench = tuple(
        signature
        for signature in (
            continuity_pokemon_signature(pokemon)
            for pokemon in (getattr(player, "bench", None) or [])
            if pokemon
        )
        if signature is not None
    )
    opposing_bench = tuple(
        signature
        for signature in (
            continuity_pokemon_signature(pokemon)
            for pokemon in (getattr(other, "bench", None) or [])
            if pokemon
        )
        if signature is not None
    )
    flags = tuple(
        bool(getattr(obs.current, name, False))
        for name in ("supporterPlayed", "stadiumPlayed", "energyAttached", "retreated")
    )
    conditions = tuple(
    ...
    return (
        ...
        opposing_active,
        opposing_bench,
        len(getattr(player, "prize", None) or []),
        len(getattr(other, "prize", None) or []),
        flags,
        conditions,
    )


def continuity_has_immediate_ko(obs) -> bool:
    target = opponent_active(obs)
    if target is None:
        return False
    return any(
        option.type == OptionType.ATTACK
        and best_damage_for_active(obs, option.attackId) >= hp(target)
        for option in obs.select.option
    )


def continuity_exact_hand_count(obs, identity: tuple[int, int]) -> int:
    return sum(
        exact_card_identity(card) == identity
        for card in (getattr(me(obs), "hand", None) or [])
        if card
    )


def continuity_exact_bench_count(obs, identity: tuple[int, int]) -> int:
    return sum(
        exact_card_identity(card) == identity
        for card in (getattr(me(obs), "bench", None) or [])
        if card
    )


class ContinuityFirstSearchDeploy:
    """One-turn, fail-closed search-to-bench transaction around exact v80."""
    ...
    def reset(self) -> None:
        ...
        self.gible_was_deployable = False

    def in_scope(self, obs) -> bool:
        return (
            self.phase != "idle"
            and self.owner == obs.current.yourIndex
            and self.turn == obs.current.turn
        )

    def legal_gible_was_deployable(self, obs) -> bool:
        return any(
            option.type == OptionType.PLAY
            and getattr(option_card(obs, option), "id", None) == GIBLE
            for option in obs.select.option
        )

    def maybe_arm(self, obs, baseline: list[int]) -> None:
        if (
            obs.select.context != SelectContext.MAIN
            or len(baseline) != 1
            or not continuity_active_is_only_pokemon(obs)
            or continuity_has_immediate_ko(obs)
        ):
            return
        selected_index = baseline[0]
        if selected_index < 0 or selected_index >= len(obs.select.option):
            return
        selected = obs.select.option[selected_index]
        source = option_card(obs, selected)
        if (
            selected.type not in {OptionType.PLAY, OptionType.ABILITY}
            or getattr(source, "id", None) not in TO_HAND_POKEMON_SEARCH_SOURCES
        ):
            return
        source_identity = exact_card_identity(source)
        line_signature = continuity_line_signature(obs)
        board_signature = continuity_board_signature(obs)
        if source_identity is None or line_signature is None or board_signature is None:
            return
        self.phase = "await_search"
        self.owner = obs.current.yourIndex
        self.turn = obs.current.turn
        self.source_identity = source_identity
        self.line_signature = line_signature
        self.board_signature = board_signature
        self.gible_was_deployable = self.legal_gible_was_deployable(obs)

    def search_target_index(self, obs, baseline: list[int]) -> int | None:
        if (
            obs.select.context != SelectContext.TO_HAND
            or obs.select.maxCount != 1
            or getattr(obs.select, "minCount", 0) > 1
            or exact_card_identity(getattr(obs.select, "effect", None)) != self.source_identity
            or not continuity_active_is_only_pokemon(obs)
            or continuity_board_signature(obs) != self.board_signature
        ):
            return None
        candidates = []
        for index, option in enumerate(obs.select.option):
            ...
            if (
                option.type == OptionType.CARD
                and option.area in {AreaType.DECK, AreaType.DISCARD}
                and getattr(card, "id", None) in CYNTHIA_BASICS
        ...
        gibles = [row for row in candidates if row[1][0] == GIBLE]
        if gibles and not self.gible_was_deployable:
            chosen_index, chosen_identity = max(
                gibles,
                key=lambda row: (
                    score_option_with_champions_call_order(obs, obs.select.option[row[0]])[0],
                    -row[0],
                ),
            )
        else:
            ...
            retained = [row for row in candidates if row[0] == baseline_index]
            if len(retained) != 1:
                return None
            chosen_index, chosen_identity = retained[0]

        if sum(identity == chosen_identity for _index, identity in candidates) != 1:
            return None
        self.target_identity = chosen_identity
        self.phase = "await_deploy"
        return chosen_index

    def deployment_index(self, obs) -> int | None:
        identity = self.target_identity
        if (
            identity is None
            or identity[0] not in CYNTHIA_BASICS
            or obs.select.context != SelectContext.MAIN
            or not continuity_active_is_only_pokemon(obs)
            or continuity_board_signature(obs) != self.board_signature
            or continuity_line_signature(obs) != self.line_signature
            or continuity_exact_hand_count(obs, identity) != 1
        ):
            return None
        legal = [
            index
            for index, option in enumerate(obs.select.option)
            if option.type == OptionType.PLAY
            and exact_card_identity(option_card(obs, option)) == identity
        ]
        return legal[0] if len(legal) == 1 else None

    def choose(self, obs) -> list[int]:
        baseline = choose_options(obs)
        if self.phase != "idle" and not self.in_scope(obs):
            self.reset()

        if self.phase == "confirm_deploy":
            identity = self.target_identity
            if (
                identity is not None
                and continuity_exact_bench_count(obs, identity) == 1
                and continuity_exact_hand_count(obs, identity) == 0
            ):
                self.reset()
                return baseline
            self.reset()
        ...
        if self.phase == "await_deploy":
            if obs.select.context != SelectContext.MAIN:
                identity = self.target_identity
                if (
                    identity is not None
                    and identity[0] in CYNTHIA_BASICS
                    and continuity_active_is_only_pokemon(obs)
                    and continuity_board_signature(obs) == self.board_signature
                    and continuity_exact_hand_count(obs, identity) == 1
                ):
                    return baseline
                self.reset()
                return baseline
            index = self.deployment_index(obs)
            if index is None:
                self.reset()
                return baseline
            self.phase = "confirm_deploy"
            return [index]

        if self.phase == "await_search":
            index = self.search_target_index(obs, baseline)
            if index is None:
                self.reset()
                return baseline
            return [index]

        self.maybe_arm(obs, baseline)
        return baseline


_CONTINUITY_FIRST_AGENT = ContinuityFirstSearchDeploy()


def reset_continuity_first_state() -> None:
    _CONTINUITY_FIRST_AGENT.reset()


def agent(observation: dict[str, Any]) -> list[int]:
    if observation.get("select") is None:
        reset_continuity_first_state()
        return read_deck_csv()
    obs = to_observation_class(observation)
    return _CONTINUITY_FIRST_AGENT.choose(obs)
