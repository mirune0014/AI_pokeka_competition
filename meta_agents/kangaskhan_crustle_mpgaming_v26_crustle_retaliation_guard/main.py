...
def crustle_retaliation_guard(obs, sel, me, opp, me_active):
    """Spend a legal Jumbo before a nonlethal Spiky Crustle trade that loses us."""
    opp_active = active(opp)
    if (me_active is None or me_active.id != CRUSTLE
            or opp_active is None or opp_active.id != CRUSTLE):
        return False
    jumbo_legal = any(
        opt.type == OptionType.PLAY
        and (candidate := card(obs, AreaType.HAND, opt.index, obs.current.yourIndex)) is not None
        and candidate.id == JUMBO_ICE_CREAM
        for opt in sel.option
    )
    if not jumbo_legal:
        return False
    attached = list(getattr(opp_active, "energyCards", None) or opp_active.energies)
    has_spiky = any(getattr(energy, "id", energy) == 14 for energy in attached)
    retaliation_legal = (len(attached) >= 3
                          and any(getattr(energy, "id", energy) == 1 for energy in attached))
    if not (has_spiky and retaliation_legal):
        return False
    for opt in sel.option:
        if opt.type != OptionType.ATTACK:
            continue
        attack = ATTACK_DB.get(opt.attackId)
        if (attack is not None and attack.damage == 120
                and opp_active.hp > attack.damage
                and me_active.hp - 20 <= 120):
            return True
    return False
...
def agent(obs_dict, configuration=None):
    try:
        ...
        retaliation_guard = crustle_retaliation_guard(obs, sel, me, opp, me_active)
        heal_first = retaliation_guard or (attacks_available and me_active is not None and active_missing_hp >= 80
