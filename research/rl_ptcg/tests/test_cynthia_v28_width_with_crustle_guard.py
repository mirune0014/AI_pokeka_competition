"""Focused Crustle matchup guard checks for Cynthia Garchomp v28."""
import csv
...
AGENT_PATH = ROOT / "opponents" / "meta_agents" / "cynthia_garchomp_nasuo445_v28_width_with_crustle_guard" / "main.py"
V24_DECK_PATH = ROOT / "opponents" / "meta_agents" / "cynthia_garchomp_nasuo445_v24_width_before_first_resource" / "deck.csv"
V28_DECK_PATH = AGENT_PATH.parent / "deck.csv"
...
def load_agent_module():
    ...
    spec = importlib.util.spec_from_file_location("cynthia_v28_main", AGENT_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class WidthWithCrustleGuardTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_visible_344_and_345_disable_with_or_without_energy(self):
        a = self.agent
        energy = types.SimpleNamespace(id=a.BASIC_FIGHTING)
        for card_id in (344, 345):
            for energies in ([], [energy]):
                with self.subTest(card_id=card_id, energized=bool(energies)):
                    if card_id == 344:
                        obs = self.observation(opponent_bench=[self.pokemon(card_id, energies=energies)])
                    else:
                        obs = self.observation(opponent_active=self.pokemon(card_id, energies=energies))
    ...
    def test_energized_non_crustle_opponent_retains_width(self):
        a = self.agent
    ...
    def test_deck_is_unchanged_and_has_sixty_cards(self):
        with V24_DECK_PATH.open(newline="") as v24_file, V28_DECK_PATH.open(newline="") as v28_file:
            v24_cards = [row[0] for row in csv.reader(v24_file) if row]
            v28_cards = [row[0] for row in csv.reader(v28_file) if row]
        self.assertEqual(len(v28_cards), 60)
        self.assertEqual(v28_cards, v24_cards)
