...
def has_rock_ready_main_attacker_route(obs) -> bool:
    active = active_pokemon(obs)
    if active and active.id == GARCHOMP_EX and damage_on(active) > 0:
        return False
    return has_in_play(obs, GARCHOMP_EX) or (
        has_in_play(obs, GABITE) and GARCHOMP_EX in hand_ids(obs)
    )
...
def score_attach(obs, opt) -> tuple[int, str]:
    ...
    rock_bonus = (
        180
        if cid == ROCK_FIGHTING
        and tid in MAIN_LINE
        and lacks_rock_energy(target)
        and has_rock_ready_main_attacker_route(obs)
        else 0
    )
    if tid == GARCHOMP_EX:
