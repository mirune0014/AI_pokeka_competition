"""Exact Historical-Silver plus isolated Rules 1, 4, 5, and 6.
...
_RULE6_ID = "PARENT_POKE_PAD_EMPTY_BENCH_DURALUDON_ONE_METAL_READY_SUCCESSOR_TRANSACTION_V1"
_DURALUDON = 169
_ARCHALUDON_EX = 190
_CINDERACE = 666
_ARCHALUDON = 840
...
_RULE6_ROLE = "DURALUDON_ONE_METAL_READY_SUCCESSOR"
_RULE6_PAD_START = "POKE_PAD_DURALUDON_START"
_RULE6_PAD_TARGET = "POKE_PAD_DURALUDON_TARGET"
_RULE6_DURALUDON_BENCH = "POKE_PAD_DURALUDON_BENCH"
_RULE6_READY_ATTACH = "POKE_PAD_DURALUDON_READY_ATTACH"
_RULE6_WHIFF_EMPTY = "POKE_PAD_DURALUDON_WHIFF_EMPTY"
...
def _card_metadata_exact(card_id):
    ...
    if card_id == _POKE_PAD:
        return bool(
            getattr(data, "name", None) == "Poké Pad"
            and getattr(data, "cardType", None) == _CardType.ITEM
            and len(skills) == 1
            and getattr(skills[0], "name", None) == "Poké Pad"
            and getattr(skills[0], "text", None)
...
def _owner_view(owner):
    ...
    return {
        ...
        "pad_serial": (
            owner.get("pad_ref", (None, None, None))[1]
            if isinstance(owner.get("pad_ref"), tuple)
            else None
        ),
        "metal_serial": (
            owner.get("metal_ref", (None, None, None))[1]
            if isinstance(owner.get("metal_ref"), tuple)
            else None
        ),
    }
...
def _rule6_proposal(action, purpose, proof, owner):
    return _make_proposal(
        action,
        purpose,
        proof,
        _owner_view(owner),
        rule_id=_RULE6_ID,
        category="POKE_PAD_READY_SUCCESSOR_TRANSACTION",
    )


def _add_ref(refs, selected):
    if not isinstance(refs, tuple) or not isinstance(selected, tuple):
        return None
    return tuple(sorted(refs + (selected,)))


def _snapshot_matches(obs, owner, **changes):
    snapshot = _state_snapshot(obs)
    expected = dict(owner["start_snapshot"])
    expected.update(changes)
    return snapshot is not None and snapshot == expected


def _play_option_surface_exact(option):
    if getattr(option, "type", None) != _OptionType.PLAY:
        return False
    return all(
        key in {"type", "index", "playerIndex"} or value is None
        for key, value in vars(option).items()
    )


def _card_option_surface_exact(option):
    if getattr(option, "type", None) != _OptionType.CARD:
        return False
    return all(
        key in {"type", "area", "index", "playerIndex"} or value is None
        for key, value in vars(option).items()
    )


def _attach_option_surface_exact(option):
    if getattr(option, "type", None) != _OptionType.ATTACH:
        return False
    return all(
        key
        in {
...
def _rule6_current_attack_exact(obs, owner):
    ...
    if (
        ...
        or _pokemon_fingerprint(mine.active[0], seat) != owner["active"]
    ):
        return False
    registered = _registered_attack_rows(obs, rows)
    attack_ids = {row[2][5] for row in registered}
    if attack_ids != {owner["attack_id"]}:
        return False
    result = _exact_damage_and_take(
        obs, mine.active[0], theirs.active[0], owner["attack_id"]
    )
    return bool(
        result is not None
        and result[2] != len(mine.prize)
        and result == owner["attack_proof"]
    )


def _single_parent_pad(obs, parent_action, rows, snapshot):
    if (
        not isinstance(parent_action, list)
        or len(parent_action) != 1
        or not _is_exact_int(parent_action[0])
        or parent_action[0] < 0
        or parent_action[0] >= len(rows)
        or not _card_metadata_exact(_POKE_PAD)
    ):
        return None
    selected = rows[parent_action[0]]
    ref = selected[2][1]
    if (
        not _play_option_surface_exact(selected[1])
        or ref is None
        or ref[0] != _POKE_PAD
        or ref[2] != obs.current.yourIndex
        or snapshot["own_hand"].count(ref) != 1
    ):
        return None
    return selected


def _start_rule6(obs, parent_action):
    global _materialization_owner
    gates = {
        "normal_main": _exact_main(obs),
        "owner_empty": _materialization_owner is None,
        "energy_unused": getattr(obs.current, "energyAttached", None) is False,
    }
    if not all(gates.values()):
        return None, "rule6_entry_gate_failed", gates, False, False
    rows = _option_rows(obs)
    snapshot = _state_snapshot(obs)
    gates["option_bindings_exact"] = rows is not None
    gates["snapshot_exact"] = snapshot is not None
    if rows is None or snapshot is None:
        return None, "rule6_public_binding_failed", gates, False, False
    pad = _single_parent_pad(obs, parent_action, rows, snapshot)
    gates["parent_exact_pad"] = pad is not None
    if pad is None:
        return None, "parent_not_exact_poke_pad", gates, False, False
    seat = obs.current.yourIndex
    mine = obs.current.players[seat]
    gates["deck_count_exact"] = bool(
        _is_exact_int(snapshot["own_deck"])
        and 0 <= snapshot["own_deck"] <= 60
    )
    gates["bench_exactly_empty"] = mine.bench == []
    gates["bench_slot_available"] = bool(
        _is_exact_int(mine.benchMax) and mine.benchMax > 0
    )
    gates["duraludon_absent_from_hand"] = not any(
        ref[0] == _DURALUDON for ref in snapshot["own_hand"]
    )
    metal_refs = tuple(
        ref for ref in snapshot["own_hand"] if ref[0] == _METAL_ENERGY
    )
    gates["visible_basic_metal"] = bool(
        metal_refs and _card_metadata_exact(_METAL_ENERGY)
    )
    gates["duraludon_hammer_metadata_exact"] = bool(
        _card_metadata_exact(_DURALUDON) and _attack_metadata_exact(223)
    )
    if not all(gates.values()):
        return None, "rule6_role_gate_failed", gates, False, False
    registered = _registered_attack_rows(obs, rows)
    attack_ids = {row[2][5] for row in registered}
    gates["one_distinct_registered_attack"] = len(attack_ids) == 1
    if len(attack_ids) != 1:
        return None, "rule6_current_attack_not_unique", gates, False, False
    attack_id = next(iter(attack_ids))
    active = mine.active[0]
    target = obs.current.players[1 - seat].active[0]
    attack_proof = _exact_damage_and_take(obs, active, target, attack_id)
    gates["terminal_proof_exact"] = attack_proof is not None
    gates["terminal_proof_false"] = bool(
        attack_proof is not None and attack_proof[2] != len(mine.prize)
    )
    prompt = _prompt_fingerprint(obs)
    gates["prompt_exact"] = prompt is not None
    if not all(gates.values()):
        return None, "rule6_terminal_or_prompt_unknown", gates, False, False
    pad_ref = pad[2][1]
    owner = {
        "owner": _RULE6_ID,
        "stage": "PAD_PLAY_EMITTED",
        "route_kind": _RULE6_ROLE,
        "seat": seat,
        "turn": obs.current.turn,
        "action_count": obs.current.turnActionCount,
        "pad_ref": pad_ref,
        "pad_role": pad[2],
        "metal_ref": min(metal_refs, key=lambda ref: ref[1]),
        "active": _pokemon_fingerprint(active, seat),
        "attack_id": attack_id,
    ...
    proof = {
        "role": _RULE6_ROLE,
        "pad_ref": pad_ref,
        "metal_ref": owner["metal_ref"],
        "attack_id": attack_id,
        "gates": dict(gates),
    }
    return _rule6_proposal(
        parent_action, _RULE6_PAD_START, proof, owner
    ), None, proof, False, False


def _rule6_retry(obs, owner):
    prompt = _prompt_fingerprint(obs)
    if prompt is None or prompt != owner.get("last_prompt"):
        return None
    stage = owner.get("stage")
    if stage == "PAD_WHIFF_EMITTED":
        action = []
        purpose = _RULE6_WHIFF_EMPTY
    else:
        action = _bind_first_role(obs, owner.get("last_role"))
        if action is None:
            return False
        purpose = {
            "PAD_PLAY_EMITTED": _RULE6_PAD_START,
            "PAD_TARGET_EMITTED": _RULE6_PAD_TARGET,
            "DURALUDON_BENCH_EMITTED": _RULE6_DURALUDON_BENCH,
            "METAL_ATTACH_EMITTED": _RULE6_READY_ATTACH,
        }.get(stage)
        if purpose is None:
            return False
    rows = _option_rows(obs)
    proof = {
        "role": _RULE6_ROLE,
        "pad_ref": owner["pad_ref"],
        "selected_ref": owner.get("selected_ref"),
        "metal_ref": owner["metal_ref"],
        "attack_id": owner["attack_id"],
        "duplicate_rebind": True,
    }
    return (
        _rule6_proposal(action, purpose, proof, owner),
        None,
        proof,
        True,
        tuple(row[2] for row in rows) != owner.get("last_order"),
...
def _pad_target_rows(obs, owner):
    seat = owner["seat"]
    ...
    if (
        ...
        or len(deck_refs) != len(set(deck_refs))
        or len(deck_refs) != owner["start_snapshot"]["own_deck"]
    ):
        return None
    rows = _option_rows(obs)
    if rows is None:
    ...
    for row in rows:
        option = row[1]
        ref = row[2][1]
        data = None if ref is None else getattr(_parent, "CARD_DB", {}).get(ref[0])
        if (
            not _card_option_surface_exact(option)
            or option.area != _AreaType.DECK
            or option.playerIndex != seat
            or not _is_exact_int(option.index)
            or option.index < 0
            or option.index >= len(deck_refs)
            or ref != deck_refs[option.index]
            or data is None
            or data.cardType != _CardType.POKEMON
            or bool(getattr(data, "ex", False))
            or bool(getattr(data, "megaEx", False))
        ):
            return None
    meanings = {}
    for row in rows:
        ref = row[2][1]
        meanings.setdefault(ref[1], set()).add(ref)
    if any(len(values) != 1 for values in meanings.values()):
        return None
    return rows


def _rule6_pad_target_action(obs, owner):
    start = owner["start_snapshot"]
    hand_after_pad = _without_ref(start["own_hand"], owner["pad_ref"])
    effect_ref = _exact_card_ref(getattr(obs.select, "effect", None), owner["seat"])
    if (
        hand_after_pad is None
        or not _snapshot_matches(
            obs,
            owner,
            action_count=owner["action_count"] + 1,
            own_hand=hand_after_pad,
        )
        or obs.select.type != _SelectType.CARD
        or obs.select.context != _SelectContext.TO_HAND
        or obs.select.minCount != 0
        or obs.select.maxCount != 1
        or effect_ref != owner["pad_ref"]
        or obs.select.contextCard is not None
        or obs.current.looking is not None
        or not _card_metadata_exact(_POKE_PAD)
        or len(obs.logs) != 1
        or not _log_exact(
            ...
            _LogType.PLAY,
            {
                "playerIndex": owner["seat"],
                "cardId": _POKE_PAD,
                "serial": owner["pad_ref"][1],
            },
        )
    ):
        return None
    rows = _pad_target_rows(obs, owner)
    if rows is None:
        return None
    targets = [row for row in rows if row[2][1][0] == _DURALUDON]
    if not targets:
        return (), rows
    minimum_serial = min(row[2][1][1] for row in targets)
    same_physical = [
        row for row in targets if row[2][1][1] == minimum_serial
    ]
    return min(same_physical, key=lambda row: row[0]), rows


def _rule6_emit_pad_target(obs, owner):
    target = _rule6_pad_target_action(obs, owner)
    if target is None:
        return None
    ...
    if selected == ():
        ...
        return _rule6_proposal([], _RULE6_WHIFF_EMPTY, proof, owner), proof
    owner["stage"] = "PAD_TARGET_EMITTED"
    owner["selected_ref"] = selected[2][1]
    owner["last_role"] = selected[2]
    proof = {
        "role": _RULE6_ROLE,
        "pad_ref": owner["pad_ref"],
    ...
    return _rule6_proposal(
        [selected[0]], _RULE6_PAD_TARGET, proof, owner
    ), proof


def _rule6_duraludon_play_action(obs, owner):
    start = owner["start_snapshot"]
    hand_after_pad = _without_ref(start["own_hand"], owner["pad_ref"])
    hand_with_target = _add_ref(hand_after_pad, owner["selected_ref"])
    expected_discard = start["own_discard"] + (owner["pad_ref"],)
    if (
        hand_after_pad is None
        or hand_with_target is None
        or start["own_deck"] <= 0
        or not _snapshot_matches(
            obs,
            owner,
            action_count=owner["action_count"] + 2,
            own_hand=hand_with_target,
            own_discard=expected_discard,
            own_deck=start["own_deck"] - 1,
        )
        or not _rule6_current_attack_exact(obs, owner)
        or len(obs.logs) != 2
        or not _log_exact(
            obs.logs[0],
            _LogType.MOVE_CARD,
            {
                "playerIndex": owner["seat"],
                "cardId": _DURALUDON,
                "serial": owner["selected_ref"][1],
                "fromArea": int(_AreaType.DECK),
                "toArea": int(_AreaType.HAND),
            },
        )
        or not _log_exact(
            obs.logs[1], _LogType.SHUFFLE, {"playerIndex": owner["seat"]}
        )
    ):
        return None
    rows = _option_rows(obs)
    ...
    matches = [
        row
        for row in rows
        if _play_option_surface_exact(row[1])
        and row[2][1] == owner["selected_ref"]
    ]
    return (min(matches, key=lambda row: row[0]), rows) if matches else None


def _rule6_emit_duraludon_bench(obs, owner):
    result = _rule6_duraludon_play_action(obs, owner)
    if result is None:
        return None
    selected, rows = result
    owner["stage"] = "DURALUDON_BENCH_EMITTED"
    owner["last_role"] = selected[2]
    owner["last_prompt"] = _prompt_fingerprint(obs)
    owner["last_order"] = tuple(row[2] for row in rows)
    proof = {
    ...
    return _rule6_proposal(
        [selected[0]], _RULE6_DURALUDON_BENCH, proof, owner
    ), proof


def _rule6_empty_bench_duraludon(obs, owner, *, metal_attached):
    seat = owner["seat"]
    mine = obs.current.players[seat]
    if len(mine.bench) != 1:
        return None
    pokemon = mine.bench[0]
    fp = _pokemon_fingerprint(pokemon, seat)
    if (
        fp is None
        or fp[0:2] != owner["selected_ref"][0:2]
        or fp[2] != _parent.CARD_DB[_DURALUDON].hp
        or fp[3] != _parent.CARD_DB[_DURALUDON].hp
        or fp[4] is not True
        or fp[7] != ()
        or fp[8] != ()
    ):
        return None
    expected_refs = (owner["metal_ref"],) if metal_attached else ()
    expected_units = (int(_EnergyType.METAL),) if metal_attached else ()
    if fp[5] != expected_units or fp[6] != expected_refs:
        return None
    return fp


def _rule6_metal_attach_action(obs, owner):
    start = owner["start_snapshot"]
    hand_after_pad = _without_ref(start["own_hand"], owner["pad_ref"])
    bench_fp = _rule6_empty_bench_duraludon(obs, owner, metal_attached=False)
    if (
        hand_after_pad is None
        or bench_fp is None
        or not _snapshot_matches(
            ...
            own_hand=hand_after_pad,
            own_discard=start["own_discard"] + (owner["pad_ref"],),
            own_bench=(bench_fp,),
            own_deck=start["own_deck"] - 1,
        )
        or not _rule6_current_attack_exact(obs, owner)
    ...
    if rows is None:
        return None
    target_ref = (_DURALUDON, owner["selected_ref"][1], owner["seat"])
    matches = [
        row
        for row in rows
        if _attach_option_surface_exact(row[1])
        and row[1].area == _AreaType.HAND
        and row[1].inPlayArea == _AreaType.BENCH
        and row[1].inPlayIndex == 0
        and row[2][1] == owner["metal_ref"]
        and row[2][4] == target_ref
    ]
    return (min(matches, key=lambda row: row[0]), rows) if matches else None


def _rule6_emit_metal_attach(obs, owner):
    result = _rule6_metal_attach_action(obs, owner)
    if result is None:
        return None
    selected, rows = result
    owner["stage"] = "METAL_ATTACH_EMITTED"
    owner["last_role"] = selected[2]
    owner["last_prompt"] = _prompt_fingerprint(obs)
    owner["last_order"] = tuple(row[2] for row in rows)
    proof = {
    ...
    return _rule6_proposal(
        [selected[0]], _RULE6_READY_ATTACH, proof, owner
    ), proof


def _rule6_ready_confirmed(obs, owner):
    start = owner["start_snapshot"]
    hand_after_pad = _without_ref(start["own_hand"], owner["pad_ref"])
    hand_after_metal = _without_ref(hand_after_pad, owner["metal_ref"])
    bench_fp = _rule6_empty_bench_duraludon(obs, owner, metal_attached=True)
    return bool(
        hand_after_pad is not None
        and hand_after_metal is not None
        and bench_fp is not None
        and _snapshot_matches(
            obs,
            owner,
            action_count=owner["action_count"] + 4,
            energy_attached=True,
            own_hand=hand_after_metal,
            own_discard=start["own_discard"] + (owner["pad_ref"],),
            own_bench=(bench_fp,),
            own_deck=start["own_deck"] - 1,
        )
        and _rule6_current_attack_exact(obs, owner)
        and len(obs.logs) == 1
        and _log_exact(
            obs.logs[0],
            _LogType.ATTACH,
            {
                ...
                "serial": owner["metal_ref"][1],
                "cardIdTarget": _DURALUDON,
                "serialTarget": owner["selected_ref"][1],
            },
        )
        and _exact_metal_energy_refs(
            obs.current.players[owner["seat"]].bench[0], owner["seat"]
        )
        == (owner["metal_ref"],)
        and _printed_attack_payable_with_metals(_DURALUDON, 1) == (223,)
    )


def _rule6_whiff_confirmed(obs, owner):
    start = owner["start_snapshot"]
    hand_after_pad = _without_ref(start["own_hand"], owner["pad_ref"])
    return bool(
        hand_after_pad is not None
        ...
        and len(obs.logs) == 1
        and _log_exact(
            obs.logs[0], _LogType.SHUFFLE, {"playerIndex": owner["seat"]}
        )
    )


def _resume_rule6(obs):
    global _materialization_owner
    owner = _materialization_owner
    if not isinstance(owner, dict) or owner.get("owner") != _RULE6_ID:
        _materialization_owner = None
        return None, "rule6_owner_conflict", {}, False, False
    retry = _rule6_retry(obs, owner)
    if retry is False:
        _materialization_owner = None
        return None, "rule6_duplicate_rebind_failed", {}, True, False
    if retry is not None:
        return retry
    stage = owner.get("stage")
    if (
        ...
        _materialization_owner = None
        return None, "rule6_identity_discontinuity", {}, False, False
    if stage == "PAD_PLAY_EMITTED":
        emitted = _rule6_emit_pad_target(obs, owner)
        if emitted is None:
            _materialization_owner = None
            return None, "rule6_pad_callback_failed", {}, False, False
        proposal, proof = emitted
        return proposal, None, proof, False, False
    if stage == "PAD_TARGET_EMITTED":
        emitted = _rule6_emit_duraludon_bench(obs, owner)
        if emitted is None:
            _materialization_owner = None
            return None, "rule6_target_movement_failed", {}, False, False
        proposal, proof = emitted
        return proposal, None, proof, False, False
    if stage == "DURALUDON_BENCH_EMITTED":
        emitted = _rule6_emit_metal_attach(obs, owner)
        if emitted is None:
            _materialization_owner = None
            return None, "rule6_bench_confirmation_failed", {}, False, False
        proposal, proof = emitted
        return proposal, None, proof, False, False
    if stage == "METAL_ATTACH_EMITTED":
        confirmed = _rule6_ready_confirmed(obs, owner)
        proof = {
            "role": _RULE6_ROLE,
            "pad_ref": owner["pad_ref"],
            "selected_ref": owner["selected_ref"],
        ...
        _materialization_owner = None
        return None, (
            "rule6_ready_complete" if confirmed else "rule6_attachment_failed"
        ), proof, False, False
    if stage == "PAD_WHIFF_EMITTED":
        confirmed = _rule6_whiff_confirmed(obs, owner)
        _materialization_owner = None
        return None, (
            "rule6_whiff_complete" if confirmed else "rule6_whiff_recovery_failed"
        ), {"role": _RULE6_ROLE, "whiff_confirmed": confirmed}, False, False
    _materialization_owner = None
    return None, "rule6_owner_conflict", {}, False, False
...
def _resolve(obs, parent_action):
    ...
    if _materialization_owner is not None:
        if (
            ...
            and _materialization_owner.get("owner") == _RULE6_ID
        ):
            return _resume_rule6(obs)
        if (
            isinstance(_materialization_owner, dict)
    ...
    if context == _SelectContext.MAIN:
        ...
        if boss[0] is not None:
            return boss
        rule6 = _start_rule6(obs, parent_action)
        if rule6[0] is not None:
            return rule6
        return rule6
...
def agent(obs_dict):
    """Call exact Silver once, then resolve Rule 1, Rule 4, Rule 5, or Rule 6."""
