...
from rl_ptcg.gold_teacher_labels import build_teacher_labels
...
class GoldTeacherLabelsTest(unittest.TestCase):
    ...
    def _build(self, corpus: Path, oracle: Path, output: Path, *, oracle_verified=None, receipt_result=None, **kwargs) -> dict:
        ...
        with patch("rl_ptcg.gold_teacher_labels.verify_oracle_output", return_value=verified), \
             patch("rl_ptcg.gold_teacher_labels.verify_gold_oracle_states", return_value={}), \
             patch("rl_ptcg.gold_teacher_labels.verify_kaggle_rollout_source_receipt", return_value=receipt_result), \
             patch("rl_ptcg.gold_teacher_labels.verify_teacher_labels", return_value={"labels": 1}):
    ...
    def test_nested_source_workspace_is_bound_separately_from_audit_workspace(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = workspace / "analysis_outputs" / "labels"
            with patch(
                "rl_ptcg.gold_teacher_labels.verify_oracle_output",
                return_value={"complete": True, "report_recomputed": True},
            ) as verify_oracle, patch(
                "rl_ptcg.gold_teacher_labels.verify_gold_oracle_states",
                return_value={},
            ) as verify_states, patch(
                "rl_ptcg.gold_teacher_labels.verify_teacher_labels",
    ...
    def test_teacher_split_overrides_source_split_and_binds_state(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "rl_ptcg.gold_teacher_labels.verify_teacher_state_split",
    ...
    def test_cli_is_available_as_a_subprocess(self):
        ...
        result = subprocess.run([sys.executable, "tools/build_gold_teacher_labels.py", "--help"], cwd=root,
