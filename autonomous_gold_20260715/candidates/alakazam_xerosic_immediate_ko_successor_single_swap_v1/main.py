...
def _exact_v3_parent_action(obs_dict: dict) -> list[int]:
...
def _xerosic_normalized_text(text: str) -> str:
    return (
        (text or "")
        .lower()
        .replace("’", "'")
...
def _xerosic_powerful_hand_metadata_is_exact() -> bool:
    """Pin the public KO proof to the checked engine's printed attack."""
    attacker_data = card_table.get(Alakazam)
    attack = attack_table.get(ATTACK_POWERFUL_HAND)
    rare_candy_data = card_table.get(Rare_Candy)
    abra_data = card_table.get(Abra)
    kadabra_data = card_table.get(Kadabra)
    if (
        attacker_data is None
        or attack is None
        or rare_candy_data is None
        or abra_data is None
        or kadabra_data is None
        or attacker_data.name != "Alakazam"
        or attacker_data.cardType != CardType.POKEMON
        or not attacker_data.stage2
        or attacker_data.evolvesFrom != "Kadabra"
        or list(attacker_data.attacks or []) != [ATTACK_POWERFUL_HAND]
        or attack.name != "Powerful Hand"
        or attack.damage != 0
        or tuple(int(unit) for unit in attack.energies)
        != (int(EnergyType.PSYCHIC),)
        or _xerosic_normalized_text(attack.text)
        != "place 2 damage counters on your opponent's active pokemon for each card in your hand."
        or rare_candy_data.name != "Rare Candy"
        or rare_candy_data.cardType != CardType.ITEM
        or len(rare_candy_data.skills or []) != 1
        or "stage 2 card in your hand"
        not in _xerosic_normalized_text(rare_candy_data.skills[0].text)
        or not abra_data.basic
        or abra_data.name != "Abra"
        or abra_data.evolvesFrom is not None
        or not kadabra_data.stage1
        or kadabra_data.name != "Kadabra"
        or kadabra_data.evolvesFrom != "Abra"
    ):
        return False
    return True


def _xerosic_visible_skill_is_irrelevant(
    card_id: int,
    text: str,
    *,
    source_kind: str,
    ...
    """Accept only a proven irrelevant visible modifier."""
    normalized = _xerosic_normalized_text(text)
    if "prevent all effects of attacks" in normalized:
        return False
    ...
    return (
        source_kind == "stadium"
        and card_id == Nighttime_Mine
        and not attacker_data.tera
        and normalized
        == "attacks used by each tera pokemon in play (both yours and your opponent's) cost {c} more."
    )


def _xerosic_visible_modifiers_are_clear(
    state, attacker: Pokemon, target: Pokemon
) -> bool:
    """Fail closed on every visible in-play source that may alter the KO."""
    attacker_data = card_table.get(attacker.id)
    ...
    for owner, player in enumerate(state.players):
        for pokemon in list(player.active) + list(player.bench):
            ...
            for pre_evolution in pokemon.preEvolution:
                ...
                if pre_data is None or pre_data.cardType != CardType.POKEMON:
                    return False
            for skill in data.skills or []:
                if not _xerosic_visible_skill_is_irrelevant(
                    pokemon.id,
                    skill.text,
                    source_kind="pokemon",
                    ...
                    return False
            for kind, cards, expected_types in (
                (
                    "energy",
                    pokemon.energyCards,
                    {CardType.BASIC_ENERGY, CardType.SPECIAL_ENERGY},
                ...
                for card in cards:
                    ...
                    if (
                        card_data is None
                        or card_data.cardType not in expected_types
                        or card.serial <= 0
                        or getattr(card, "playerIndex", None) != owner
                    ):
                        return False
    ...
    if (
        attacker.serial not in protected_serials
        or target.serial not in protected_serials
    ):
        return False
    return _bridge_protected_serials_are_unique(state, protected_serials)


def _xerosic_opponent_powerful_hand_ko_is_certified(
    obs: Observation,
) -> bool:
    state = obs.current
    my_index = state.yourIndex
    ...
    theirs = state.players[1 - my_index]
    if (
        not _xerosic_powerful_hand_metadata_is_exact()
    ...
    if (
        ...
        or not _bridge_pokemon_is_publicly_complete(target, my_index)
        or not _bridge_pokemon_is_publicly_complete(attacker, 1 - my_index)
        or any(
            getattr(theirs, status, None) is not False
            for status in ("asleep", "paralyzed", "confused")
        )
        or not isinstance(theirs.handCount, int)
        or theirs.handCount < 0
        or (
            theirs.hand is not None
            and len(theirs.hand) != theirs.handCount
        )
        or 20 * theirs.handCount < target.hp
    ):
        return False
    energy_units = _bridge_retaliation_energy_units(attacker)
    attack = attack_table[ATTACK_POWERFUL_HAND]
    if (
        energy_units is None
        or _bridge_retaliation_can_pay(energy_units, attack.energies) is not True
    ):
        return False
    return _xerosic_visible_modifiers_are_clear(state, attacker, target)


def _xerosic_kadabra_is_publicly_exact(
    pokemon: Pokemon, player_index: int
) -> bool:
    data = card_table.get(Kadabra)
    abra_data = card_table.get(Abra)
    return (
        data is not None
        and abra_data is not None
        and _bridge_kadabra_has_complete_evolution_fingerprint(
            pokemon, player_index
        )
        and pokemon.maxHp == data.hp
        and pokemon.preEvolution[0].id == Abra
        and abra_data.basic
        and abra_data.evolvesFrom is None
    )


def _xerosic_only_energized_kadabra_successor(
    obs: Observation,
) -> Pokemon | None:
    state = obs.current
    my_index = state.yourIndex
    ...
    for pokemon in mine.bench:
        if pokemon.id == Kadabra:
            if not _xerosic_kadabra_is_publicly_exact(pokemon, my_index):
                return None
            energy_units = _bridge_retaliation_energy_units(pokemon)
            if energy_units is None:
                return None
            if (
                pokemon.appearThisTurn is False
                and int(EnergyType.PSYCHIC) in energy_units
                and _bridge_retaliation_can_pay(
            ...
            if energy_units is None:
                return None
            if _bridge_retaliation_can_pay(
                energy_units, attack.energies
            ) is True:
                return None
...
def _xerosic_rare_candy_has_no_public_abra_target(
    obs: Observation,
) -> bool:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    abra_data = card_table.get(Abra)
    if abra_data is None or not abra_data.basic:
        return False
    for pokemon in list(mine.active) + list(mine.bench):
        if pokemon.id != Abra:
            continue
        if (
            not _bridge_pokemon_is_publicly_complete(pokemon, my_index)
            or pokemon.preEvolution
            or pokemon.maxHp != abra_data.hp
        ):
            return False
        # Every currently surviving visible Abra is evolution-ready by the
        # next own turn, including one that entered play on the prior turn.
        return False
    return True


def _xerosic_callback_entries(
    ...
    if (
        ...
        or len(parent_action) != select.maxCount
...
def _xerosic_immediate_ko_successor_single_swap(
    obs: Observation, parent_action: list[int]
) -> list[int] | None:
    """Transform only the exact-v3 discard action by one certified swap."""
    entries = _xerosic_callback_entries(obs, parent_action)
    if entries is None:
        return None
    discarded = set(parent_action)
    hand_alakazams = [
        entry for entry in entries if entry[1].id == Alakazam
    ]
    discarded_alakazams = [
        entry for entry in hand_alakazams if entry[0] in discarded
    ]
    retained_candies = [
        entry
        for entry in entries
        if entry[1].id == Rare_Candy and entry[0] not in discarded
    ]
    if (
        len(hand_alakazams) != 1
        or len(discarded_alakazams) != 1
        or len(retained_candies) != 1
        or not _xerosic_opponent_powerful_hand_ko_is_certified(obs)
        or _xerosic_only_energized_kadabra_successor(obs) is None
        or not _xerosic_rare_candy_has_no_public_abra_target(obs)
    ):
        return None

    victim_option = discarded_alakazams[0][0]
    candidate_option = retained_candies[0][0]
    repaired = list(parent_action)
    repaired[repaired.index(victim_option)] = candidate_option
    if (
        len(repaired) != len(parent_action)
        or len(repaired) != len(set(repaired))
        or set(repaired)
        != (set(parent_action) - {victim_option}) | {candidate_option}
    ):
        return None
    return repaired
...
def agent(obs_dict: dict) -> list[int]:
    """Run exact-v3 first, then repair one fully certified Xerosic action."""
    parent_action = _exact_v3_parent_action(obs_dict)
    try:
        obs = to_observation_class(obs_dict)
        repaired = _xerosic_immediate_ko_successor_single_swap(
            obs, parent_action
        )
    ...
    # Replace the exact-v3 cache entry with the repaired action.  On an
    # identical callback exact-v3 therefore performs no side effect twice and
    # the wrapper cannot leak the cached pre-swap discard.
    return _remember_action(_decision_signature(obs), repaired)
