...
def _iono_voltorb_coated_conversion(obs, opt):
    def basic_energy_count(pokemon, card_id, energy_value):
        cards = list(getattr(pokemon, "energyCards", None) or [])
        values = list(getattr(pokemon, "energies", None) or [])
        if len(cards) != len(values) or any(card is None for card in cards):
            return None
        return sum(
            card.id == card_id
            and int(getattr(value, "value", value)) == energy_value
    ...
    if (
        card is None
        or card.id != ARCHALUDON
        or target is None
        or target.id != DURALUDON
        or opt.inPlayArea != AreaType.BENCH
        or not isinstance(opt.inPlayIndex, int)
    ):
        return False
    ...
    if (
        ...
        or obs.current.retreated
        or retreat_cost(active) != 0
        or not any(option.type == OptionType.RETREAT for option in obs.select.option)
    ):
        return False
    target_basic_metal = basic_energy_count(target, METAL_ENERGY, METAL_ENERGY)
    if (
        getattr(target, "appearThisTurn", True)
        or target.hp <= 0
        or target_basic_metal != 3
        or len(getattr(target, "energyCards", None) or []) != 3
        or len(getattr(target, "energies", None) or []) != 3
    ):
        return False
    damage = max(0, target.maxHp - target.hp)
    bonus = max(0, target.maxHp - 130)
    if 180 + bonus - damage <= 0:
        return False
    opponent = opp_active_pokemon(obs)
    opponent_data = CARD_DB.get(opponent.id) if opponent else None
    opponent_lightning = (
        basic_energy_count(opponent, 4, 4) if opponent is not None else None
    )
    if (
        opponent is None
        or opponent.id != 265
        or opponent_data is None
        or not getattr(opponent_data, "basic", False)
        or opponent.hp <= 0
        or effective_damage(120, opponent) < opponent.hp
        or opponent_lightning is None
        or opponent_lightning < 1
    ):
        return False
    same_target_ex = any(
        ...
        and option.inPlayArea == opt.inPlayArea
        and option.inPlayIndex == opt.inPlayIndex
        for option in obs.select.option
    )
    if not same_target_ex:
        return False
    if any(
        ...
        return False
    lightning_total = 0
    for pokemon in [opp_active_pokemon(obs)] + opp_bench_pokemon(obs):
        if pokemon is None or pokemon.hp <= 0 or pokemon.id not in IONO_LINE:
            continue
        count = basic_energy_count(pokemon, 4, 4)
        if count is None:
            return False
        lightning_total += count
    raw_voltaic = 20 + 20 * lightning_total
    stadium = list(obs.current.stadium or [])
    reduction = 30 if (
        len(stadium) == 1 and stadium[0] is not None
        and stadium[0].id == FULL_METAL_LAB
    ) else 0
    ex_damage = max(0, raw_voltaic - reduction)
    ex_hp = 300 + bonus - damage
    direct_danger = ex_damage >= ex_hp
    other_one_prize = False
    own = my_state(obs)
    for pokemon in own.active:
        if pokemon and pokemon.hp > 0 and pokemon.id in {DURALUDON, ARCHALUDON}:
            other_one_prize = True
    for index, pokemon in enumerate(own.bench):
        if index == opt.inPlayIndex:
            continue
        if pokemon and pokemon.hp > 0 and pokemon.id in {DURALUDON, ARCHALUDON}:
            other_one_prize = True
    prize_danger = len(opp_state(obs).prize or []) <= 2 and not other_one_prize
    return direct_danger or prize_danger
...
def score_evolve(obs, opt):
    ...
    if cid == ARCHALUDON and tid == DURALUDON:
        if _iono_voltorb_coated_conversion(obs, opt):
            return 34000, "Iono Voltorb: ready non-ex Coated prize wall"
