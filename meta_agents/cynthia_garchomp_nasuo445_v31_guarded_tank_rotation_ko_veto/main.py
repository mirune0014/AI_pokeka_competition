...
def should_rotate_guarded_tank_garchomp(obs) -> bool:
    """Rotate only a heavily damaged, public-board-ready Garchomp tank."""
    active = active_pokemon(obs)
    active_energy = energy_count(active)
    return bool(
        active
        and active.id == GARCHOMP_EX
        and not has_legal_immediate_corkscrew_ko(obs)
        and damage_on(active) >= 200
        and (damage_on(active) >= 300 or active_energy >= 4)
        and any(
            pokemon.id == GARCHOMP_EX and energy_count(pokemon) >= 2
...
def score_retreat(obs, _opt) -> tuple[int, str]:
    active = active_pokemon(obs)
    if should_rotate_guarded_tank_garchomp(obs):
        return 19000, "rotate guarded Garchomp to ready backup"
...
def has_legal_immediate_corkscrew_ko(obs) -> bool:
    """Whether any currently legal option immediately KOs with Corkscrew Dive."""
    return any(is_immediate_corkscrew_ko(obs, opt) for opt in obs.select.option)
