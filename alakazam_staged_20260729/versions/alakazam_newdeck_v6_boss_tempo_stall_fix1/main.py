...
import planner_public_survival_bench0 as _c3_survival
import planner_public_tactical_monotonicity as _fix9
import planner_boss_powerful_hand_exact_ko_reservation as _boss_reservation
import planner_boss_tempo_stall as _boss_tempo
...
def _complete_parent_agent(obs_dict: dict) -> list[int]:
...
def _trace_surface_snapshot() -> dict:
    return {
        "LAST_V0_PORT_TRACE": LAST_V0_PORT_TRACE,
        "LAST_V1_PACKAGE_TRACE": LAST_V1_PACKAGE_TRACE,
        "LAST_STAGED_POLICY_TRACE": LAST_STAGED_POLICY_TRACE,
    }


def _trace_surface_restore(snapshot: dict) -> None:
    global LAST_V0_PORT_TRACE, LAST_V1_PACKAGE_TRACE
    global LAST_STAGED_POLICY_TRACE
    LAST_V0_PORT_TRACE = snapshot.get("LAST_V0_PORT_TRACE")
    LAST_V1_PACKAGE_TRACE = snapshot.get("LAST_V1_PACKAGE_TRACE")
    LAST_STAGED_POLICY_TRACE = snapshot.get("LAST_STAGED_POLICY_TRACE")


def _trace_surface_publish(trace: dict, parent_surface: dict) -> None:
    global LAST_V0_PORT_TRACE, LAST_V1_PACKAGE_TRACE
    global LAST_STAGED_POLICY_TRACE
    if isinstance(parent_surface, dict):
        LAST_V0_PORT_TRACE = parent_surface.get("LAST_V0_PORT_TRACE")
        LAST_V1_PACKAGE_TRACE = parent_surface.get(
            "LAST_V1_PACKAGE_TRACE"
        )
    LAST_STAGED_POLICY_TRACE = trace


def _complete_survival_agent(obs_dict: dict) -> list[int]:
    return _c3_survival.agent(
        obs_dict,
        _complete_parent_agent,
        parent=_parent,
        trace_snapshot=_trace_surface_snapshot,
        trace_restore=_trace_surface_restore,
        trace_publish=_trace_surface_publish,
    )


def _complete_fix9_agent(obs_dict: dict) -> list[int]:
    return _fix9.agent(
        obs_dict,
        _complete_survival_agent,
        parent=_parent,
        trace_snapshot=_trace_surface_snapshot,
        trace_restore=_trace_surface_restore,
        trace_publish=_trace_surface_publish,
...
def _complete_fix10_agent(obs_dict: dict) -> list[int]:
    return _boss_reservation.agent(
        obs_dict,
...
def agent(obs_dict: dict) -> list[int]:
    return _boss_tempo.agent(
        obs_dict,
        _complete_fix10_agent,
        parent=_parent,
        trace_snapshot=_trace_surface_snapshot,
        trace_restore=_trace_surface_restore,
        trace_publish=_trace_surface_publish,
