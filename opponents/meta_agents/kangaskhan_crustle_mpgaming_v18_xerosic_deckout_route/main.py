...
def deckout_route_gate(obs, me, opp):
    """Public late-game Crustle stall state used by the Gold deckout route."""
    attacks = any(opt.type == OptionType.ATTACK for opt in obs.select.option)
    current_active = active(me)
    return (len(me.prize) == 6
            and len(opp.prize) == 5
            and current_active is not None and current_active.id == CRUSTLE
            and not current_active.energies
            and not attacks
            and count(me, DWEBBLE) >= 1
            and opp.deckCount <= 18)


def deckout_spiky_kang_score(obs, me, opt):
    """Advance the Bench Kangaskhan only in the public deckout route."""
    if opt.type != OptionType.ATTACH or opt.inPlayArea != AreaType.BENCH:
        return 0
    source = card(obs, opt.area, opt.index, obs.current.yourIndex)
    target = card(obs, opt.inPlayArea, opt.inPlayIndex, obs.current.yourIndex)
    return 8600 if (source is not None and source.id == 14
                    and target is not None and target.id == KANGASKHAN
                    and not target.energies) else 0
...
def agent(obs_dict, configuration=None):
    try:
        ...
        deckout_gate = sel.context == SelectContext.MAIN and deckout_route_gate(obs, me, opp)
        scores = []
        for opt in sel.option:
            ...
            if sel.context == SelectContext.MAIN:
                if opt.type == OptionType.ATTACK:
                    ...
                    score = (deckout_spiky_kang_score(obs, me, opt) if deckout_gate else 0) or backup_kang_grow_line_score(obs, me, opp, opt) or 7000
                elif opt.type == OptionType.PLAY:
                    c = card(obs, AreaType.HAND, opt.index, obs.current.yourIndex)
                    score = {POFFIN: 8500, HILDA: 8000, LILLIE: 7000, POKEGEAR: 6000,
                             XEROSIC: 8800 if deckout_gate and opp.handCount >= 4 else (8250 if xerosic_gate else 5000),
                             BOSS: 3000}.get(c.id if c else -1, 1000)
