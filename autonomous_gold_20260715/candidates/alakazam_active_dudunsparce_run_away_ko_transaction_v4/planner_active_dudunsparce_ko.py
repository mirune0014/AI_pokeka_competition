...
TELEPATH_TEXT = (
    ...
    "when you attach this card from your hand to a {p} pokemon, search your deck for up to 2 basic {p} "
    "pokemon and put them onto your bench. then, shuffle your deck."
)
BATTLE_CAGE_TEXT = (
    "prevent all damage counters from being placed on benched pokemon (both yours and your opponent's) "
    "by effects of attacks and abilities from the opponent's pokemon. (damage from attacks is still taken.)"
)
...
def _fixed_nonpokemon_metadata(parent: Any, data: Any, card_id: int, name: str, card_type: Any, energy_type: int) -> bool:
    return (
        data is not None and type(data.cardId) is int and data.cardId == card_id
        and data.name == name and data.cardType == card_type
        and data.retreatCost == 0 and data.hp == 0
        and data.weakness is None and data.resistance is None
        and data.energyType == energy_type and data.basic is False
        and data.stage1 is False and data.stage2 is False
        and data.ex is False and data.megaEx is False and data.tera is False
        and data.aceSpec is False and data.evolvesFrom is None
        and isinstance(data.skills, list) and isinstance(data.attacks, list)
        and not data.attacks
    )


def _basic_energy_is_exact_and_inert(parent: Any, card_id: int) -> bool:
    expected = BASIC_ENERGY_METADATA.get(card_id)
    data = parent.card_table.get(card_id)
    return (
        expected is not None
        and _fixed_nonpokemon_metadata(parent, data, card_id, expected[0], parent.CardType.BASIC_ENERGY, expected[1])
        and not data.skills
    )


def _telepath_psychic_is_exact(parent: Any, card_id: int) -> bool:
    data = parent.card_table.get(card_id)
    if not _fixed_nonpokemon_metadata(parent, data, TELEPATH_PSYCHIC, "Telepath Psychic Energy", parent.CardType.SPECIAL_ENERGY, int(parent.EnergyType.PSYCHIC)):
        return False
    if card_id != TELEPATH_PSYCHIC or len(data.skills) != 1:
        return False
    skill = data.skills[0]
    return (
        isinstance(skill.name, str) and skill.name == "Telepath Psychic Energy"
        and isinstance(skill.text, str)
        and parent._normalized_skill_text(skill.text) == TELEPATH_TEXT
    )


def _battle_cage_is_exact(parent: Any, card_id: int) -> bool:
    data = parent.card_table.get(card_id)
    if not _fixed_nonpokemon_metadata(parent, data, BATTLE_CAGE, "Battle Cage", parent.CardType.STADIUM, 0):
        return False
    if card_id != BATTLE_CAGE or len(data.skills) != 1:
        return False
    skill = data.skills[0]
    return (
        isinstance(skill.name, str) and skill.name == "Battle Cage"
        and isinstance(skill.text, str)
        and parent._normalized_skill_text(skill.text) == BATTLE_CAGE_TEXT
    )


def _opponent_pokemon_skills_are_inert(parent: Any, pokemon: Any) -> bool:
    data = parent.card_table.get(pokemon.id)
    if (
        data is None or type(data.cardId) is not int or data.cardId != pokemon.id
        or data.cardType != parent.CardType.POKEMON
        or not isinstance(data.skills, list) or not isinstance(data.attacks, list)
    ):
        return False
    for skill in data.skills:
        if not isinstance(skill.name, str) or not skill.name.strip() or not isinstance(skill.text, str):
            return False
        text = parent._normalized_skill_text(skill.text)
        if (
            "once during your turn" not in text
            or "damaged by an attack" in text
            or "knocked out by damage from an attack" in text
        ):
            return False
    return True


def _opponent_energy_is_inert(parent: Any, card_id: int) -> bool:
    return _basic_energy_is_exact_and_inert(parent, card_id) or _telepath_psychic_is_exact(parent, card_id)


def _attacker_attachments_are_clear(parent: Any, pokemon: Any) -> bool:
    if pokemon.tools or len(pokemon.energyCards) != 1:
        return False
    card_id = pokemon.energyCards[0].id
    return (
        (card_id == 5 and _basic_energy_is_exact_and_inert(parent, card_id))
        or _telepath_psychic_is_exact(parent, card_id)
    )


def _visible_effects_are_clear(parent: Any, state: Any) -> bool:
    if state.yourIndex not in (0, 1) or len(state.players) != 2:
        return False
    opponent = state.players[1 - state.yourIndex]
    if not isinstance(opponent.discard, list) or any(card.id == WALREIN for card in opponent.discard):
        return False
    for pokemon in list(opponent.active) + list(opponent.bench):
        if (
            pokemon.id == SLOWKING
            or not _opponent_pokemon_skills_are_inert(parent, pokemon)
            or pokemon.tools
            or any(not _opponent_energy_is_inert(parent, card.id) for card in pokemon.energyCards)
        ):
            return False
    if not isinstance(state.stadium, list) or len(state.stadium) > 1:
        return False
    return not state.stadium or _battle_cage_is_exact(parent, state.stadium[0].id)
...
def _target_type_is_exact_for_attack(parent: Any, target: Any, attack_id: int) -> bool:
    data = parent.card_table.get(getattr(target, "id", None))
    valid_types = {int(value) for value in parent.EnergyType}
    if (
        data is None or type(data.cardId) is not int or data.cardId != target.id
        or data.cardType != parent.CardType.POKEMON
        or type(data.energyType) is not int or data.energyType not in valid_types
    ):
        return False
    return attack_id != SUPER_PSY_BOLT or data.energyType != int(parent.EnergyType.METAL)
...
def _attacker_certificate(parent: Any, obs: Any, pokemon: Any, target: Any, hand_count: int):
    ...
    if attack_id is None or data is None or attack is None or attack.attackId != attack_id or tuple(data.attacks or ()) != (attack_id,) or tuple(int(value) for value in (attack.energies or ())) != (int(parent.EnergyType.PSYCHIC),) or line is None or not _one_prize_exact(parent, pokemon, owner) or not _target_type_is_exact_for_attack(parent, target, attack_id) or not _attacker_attachments_are_clear(parent, pokemon) or units is None or semantics.missing_energy(parent, units, attack.energies) or not _target_ko_effects_are_clear(parent, target) or not _lingering_opponent_attacks_are_clear(parent, obs.current, owner):
        return None
...
def _promotion_action(parent: Any, obs: Any, data: dict):
    ...
    if (
        ...
        or state.turnActionCount != data["start_action_count"] + 1 or not _visible_effects_are_clear(parent, state) or snapshot["own_active"] != ()
