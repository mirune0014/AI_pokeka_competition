...
class KaggleRolloutExecutionTests(unittest.TestCase):
    ...
    def test_verifies_all_bound_artifacts(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                patch(
                    "research.rl_ptcg.kaggle_rollout_execution.verify_rollout_payload",
                    return_value={"manifest_sha256": "asset"},
                ),
                patch(
                    "research.rl_ptcg.kaggle_rollout_execution.verify_seeded_engine_linux",
                    return_value={"manifest_sha256": "engine"},
                ),
                patch(
                    "research.rl_ptcg.kaggle_rollout_execution.verify_oracle_output",
                    return_value={
