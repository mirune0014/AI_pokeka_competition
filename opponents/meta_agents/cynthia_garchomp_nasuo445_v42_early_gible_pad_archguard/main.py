...
def opponent_shows_archaludon_line(obs) -> bool:
    """Use only publicly visible opposing Pokemon, never inferred deck identity."""
    return any(p.id in {DURALUDON, ARCHALUDON_EX} for p in opponent_pokemon(obs))
...
def bench_has_space(obs) -> bool:
    return len(getattr(me(obs), "bench", None) or []) < 5


def gible_remains(obs) -> bool:
    """Use the fixed public deck list and visible zones; never inspect deck order."""
    known_gible = count_in_play(obs, GIBLE) + hand_ids(obs).count(GIBLE) + discard_ids(obs).count(GIBLE)
    return known_gible < read_deck_csv().count(GIBLE)


def is_game_winning_attack(obs, opt) -> bool:
    """True only for a visible active KO that takes every remaining prize."""
    target = opponent_active(obs)
    remaining_prizes = len(getattr(me(obs), "prize", []) or [])
    return bool(
        opt.type == OptionType.ATTACK
        and target
        and remaining_prizes > 0
        and best_damage_for_active(obs, opt.attackId) >= hp(target)
        and prize_value(target) >= remaining_prizes
    )


def is_immediate_active_ko(obs, opt) -> bool:
    """True when the selected attack visibly KOs the opposing active."""
    target = opponent_active(obs)
    return bool(
        opt.type == OptionType.ATTACK
        and target
...
def is_direct_backup_action(obs, opt) -> bool:
    """Identify the reliability-filtered public, same-turn Garchomp backup routes."""
    card = option_card(obs, opt)
    cid = getattr(card, "id", None)
    target = option_target(obs, opt)
    tid = getattr(target, "id", None)
    if opt.type == OptionType.PLAY:
        if cid == NIGHT_STRETCHER:
            return bench_has_space(obs) and GIBLE in discard_ids(obs)
        if cid == FIGHTING_GONG:
            return (
                bench_has_space(obs)
                and gible_remains(obs)
                and len(getattr(me(obs), "prize", []) or []) in {4, 5}
                and not (getattr(me(obs), "bench", None) or [])
            )
    if opt.type == OptionType.EVOLVE:
        return (cid == GABITE and tid == GIBLE) or (cid == GARCHOMP_EX and tid == GABITE)
    return bool(
        opt.type == OptionType.ATTACH
        and cid in ENERGIES
        and opt.inPlayArea == AreaType.BENCH
        and tid in MAIN_LINE
    )


def backup_readiness_before_attack_index(obs, scored: list[tuple[int, int, str]]) -> int | None:
    """Return the best direct backup action only when v23 currently selects an attack."""
    if (
        obs.select.context != SelectContext.MAIN
        or getattr(active_pokemon(obs), "id", None) != GARCHOMP_EX
        or energy_count(active_pokemon(obs)) != 1
        or opponent_has_no_bench(obs)
        or any(
            p.id == GARCHOMP_EX and energy_count(p) >= 1
            for p in (getattr(me(obs), "bench", None) or [])
            if p
        )
    ):
        return None
    _selected_score, selected_index, _reason = max(scored, key=lambda row: (row[0], -row[1]))
    selected = obs.select.option[selected_index]
    if selected.type != OptionType.ATTACK or is_immediate_active_ko(obs, selected):
        return None
    direct_actions = [
        (score, index) for score, index, _reason in scored
        if is_direct_backup_action(obs, obs.select.option[index])
    ]
    if not direct_actions:
        return None
    return max(direct_actions, key=lambda row: (row[0], -row[1]))[1]


def has_public_evolution_eligible_gible(obs) -> bool:
    """Use only the engine's public same-turn marker; do not infer play order."""
    return any(
        pokemon.id == GIBLE and getattr(pokemon, "appearThisTurn", None) is False
        for pokemon in my_pokemon(obs)
    )


def is_garchomp_completion_attach(obs, opt) -> bool:
    """Keep an Energy that immediately readies the Active Garchomp above this detour."""
    target = option_target(obs, opt)
    return bool(
        opt.type == OptionType.ATTACH
        and getattr(option_card(obs, opt), "id", None) in ENERGIES
        and opt.inPlayArea == AreaType.ACTIVE
        and getattr(target, "id", None) == GARCHOMP_EX
        and energy_count(target) == 1
    )


def exposed_gible_pokepad_index(obs, scored: list[tuple[int, int, str]]) -> int | None:
    """Narrow early MAIN-only Pad detour while a public, older Gible can evolve."""
    if (
        obs.select.context != SelectContext.MAIN
        or int(getattr(obs.current, "turn", 0) or 0) > 3
        or opponent_shows_archaludon_line(obs)
        or not has_public_evolution_eligible_gible(obs)
        or GABITE in hand_ids(obs)
        or any(is_immediate_active_ko(obs, opt) for opt in obs.select.option if opt.type == OptionType.ATTACK)
    ):
        return None
    pad_indices = [
        index for index, opt in enumerate(obs.select.option)
        if opt.type == OptionType.PLAY and getattr(option_card(obs, opt), "id", None) == POKE_PAD
    ]
    if not pad_indices:
        return None
    selected_score, selected_index, _reason = max(scored, key=lambda row: (row[0], -row[1]))
    selected = obs.select.option[selected_index]
    selected_card_id = getattr(option_card(obs, selected), "id", None)
    selected_is_low_value = selected_score < 9000 and selected.type in {OptionType.ATTACK, OptionType.PLAY, OptionType.ATTACH}
    if (
        selected_index in pad_indices
        or not selected_is_low_value
        or selected.type == OptionType.EVOLVE
        or selected_card_id == BOSS
        or is_direct_backup_action(obs, selected)
        or is_garchomp_completion_attach(obs, selected)
    ):
        return None
    pad_index = max(
        pad_indices,
        key=lambda index: next(score for score, row_index, _reason in scored if row_index == index),
    )
    pad_score = next(score for score, index, _reason in scored if index == pad_index)
    return pad_index if pad_score < selected_score else None
    ...
    if development_index is not None:
        ...
        for row_index, (score, index, reason) in enumerate(scored):
            if index == development_index:
                ...
                break
    backup_index = backup_readiness_before_attack_index(obs, scored)
    if backup_index is not None:
        selected_attack_score = max(scored, key=lambda row: (row[0], -row[1]))[0]
        for row_index, (score, index, reason) in enumerate(scored):
            if index == backup_index:
                scored[row_index] = (
                    selected_attack_score + 1,
                    index,
                    f"{reason} before non-winning attack",
                )
                break
    pokepad_index = exposed_gible_pokepad_index(obs, scored)
    if pokepad_index is not None:
        selected_v35_score = max(scored, key=lambda row: (row[0], -row[1]))[0]
        for row_index, (score, index, reason) in enumerate(scored):
            if index == pokepad_index:
                scored[row_index] = (
                    selected_v35_score + 1,
                    index,
                    f"{reason} before exposed Gible",
                )
                break
