...
class V1RuntimeCompletionTests(unittest.TestCase):
    ...
    def boss_live_marnie_stack_sequence(
        self, target_id, target_serial, pre_evolution, energy_serials, tools=()
    ):
        start, child, main = self.fx.boss_ready_full_sequence()
        target = Pokemon(
            target_id,
            target_serial,
            100,
            self.policy.card_table[target_id].hp,
            True,
            [EnergyType.DARKNESS] * len(energy_serials),
            [self.fx.card(7, 1, serial) for serial in energy_serials],
            [self.fx.card(card_id, 1, serial) for card_id, serial in tools],
            [
                self.fx.card(card_id, 1, serial)
                for card_id, serial in pre_evolution
            ],
        )
        old_active = Pokemon(743, 20, 140, 140, True, [], [], [], [])
        start.current.players[1].active[0] = copy.deepcopy(old_active)
        ...
        for observation in (start, child, main):
            ...
            mine.prize = [None] * self.policy.prize_count(target)
            if mine.active[0].energyCards[0].serial == target_serial:
                mine.active[0].energyCards[0].serial = 911
        return start, child, main

    def ko_verification(
        self, main, target_serial, *, reverse_pre_evolution=True,
        reverse_energy=True, reverse_tools=False
    ):
        ...
        energies = (
            reversed(target.energyCards)
            if reverse_energy
            else target.energyCards
        )
        tools = reversed(target.tools) if reverse_tools else target.tools
        target_moves = (
            (target.id, target.serial, AreaType.ACTIVE),
            *((card.id, card.serial, AreaType.PRE_EVOLUTION) for card in pre_evolutions),
            *((card.id, card.serial, AreaType.ENERGY) for card in energies),
            *((card.id, card.serial, AreaType.TOOL) for card in tools),
        )
        theirs.active = []
    ...
    def grow_grass_hammer_verification(
        ...
        *,
        ...
        target_before = post_play[area_key][0]
        target_expected_after = v1._fingerprint_without_energy(
            target_before,
            energy_index,
            (selected.id, selected.serial, selected.playerIndex),
            grow_grass_target=True,
        )
        self.assertIsNotNone(target_expected_after)
        transaction = {
            "owner": 0,
    ...
    def test_hammer_non_grow_energy_preserves_hp_exactly(self):
        ...
        after = v1._fingerprint_without_energy(
            before,
            0,
            (energy.id, energy.serial, energy.playerIndex),
            grow_grass_target=True,
        )
    ...
    def test_hammer_grow_grass_multiple_energy_binds_index_and_serial(self):
        ...
        after = v1._fingerprint_without_energy(
            fingerprint, 1, grow_row, grow_grass_target=True
        )
        ...
        self.assertIsNone(
            v1._fingerprint_without_energy(
                fingerprint, 0, grow_row, grow_grass_target=True
            )
        )
        self.assertIsNone(
            v1._fingerprint_without_energy(
                fingerprint,
                1,
                (grow.id, grow.serial + 1, grow.playerIndex),
                grow_grass_target=True,
            )
        )
    ...
    def test_live_six_energy_ko_uses_reverse_engine_order(self):
        energy_serials = (58, 57, 54, 60, 62, 59)
        start, child, main = self.boss_live_marnie_stack_sequence(
            648, 11, ((646, 52), (647, 53)), energy_serials
        )
        self.assertEqual(self.invoke(start, [1], 1), [0])
        self.assertEqual(self.invoke(child, [0], 0), [0])
        self.assertEqual(self.invoke(main, [0], 0), [1])
        final = self.ko_verification(main, 11)
        expected_serials = (11, 53, 52, 59, 62, 60, 54, 57, 58)
        self.assertEqual(
            tuple(
                card.serial
                for card in final.current.players[1].discard[-len(expected_serials):]
            ),
            expected_serials,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            tuple(
                log.serial
                for log in final.logs[2:]
                if log.fromArea == AreaType.ENERGY
            ),
            tuple(reversed(energy_serials)),
        )
        self.assertEqual(self.invoke(final, [0, 1], 1), [0, 1])
        self.assert_complete([0, 1])

    def test_live_two_energy_ko_accepts_reverse_and_rejects_forward_order(self):
        energy_serials = (113, 120)
        start, child, main = self.boss_live_marnie_stack_sequence(
            647, 68, ((646, 112),), energy_serials
        )
        forward_start, forward_child, forward_main = copy.deepcopy(
            (start, child, main)
        )
        self.assertEqual(self.invoke(start, [1], 1), [0])
        self.assertEqual(self.invoke(child, [0], 0), [0])
        self.assertEqual(self.invoke(main, [0], 0), [1])
        final = self.ko_verification(main, 68)
        expected_serials = (68, 112, 120, 113)
        self.assertEqual(
            tuple(card.serial for card in final.current.players[1].discard[-4:]),
            expected_serials,
        )
        ...
        v1.reset()
        self.assertEqual(self.invoke(forward_start, [1], 1), [0])
        self.assertEqual(self.invoke(forward_child, [0], 0), [0])
        self.assertEqual(self.invoke(forward_main, [0], 0), [1])
        forward = self.ko_verification(
            forward_main, 68, reverse_energy=False
        )
        self.assertEqual(
            tuple(card.serial for card in forward.current.players[1].discard[-4:]),
            (68, 112, 113, 120),
    ...
    def test_attack_ko_preserves_tool_order_and_rejects_tool_mutation(self):
        tools = ((1173, 121), (1174, 122))
        start, child, main = self.boss_live_marnie_stack_sequence(
            647, 68, ((646, 112),), (113,), tools
        )
        mutated_start, mutated_child, mutated_main = copy.deepcopy(
            (start, child, main)
        )
        self.assertEqual(self.invoke(start, [1], 1), [0])
        self.assertEqual(self.invoke(child, [0], 0), [0])
        self.assertEqual(self.invoke(main, [0], 0), [1])
        final = self.ko_verification(main, 68)
        expected_serials = (68, 112, 113, 121, 122)
        self.assertEqual(
            tuple(card.serial for card in final.current.players[1].discard[-5:]),
            expected_serials,
        )
        self.assertEqual(
            tuple(log.serial for log in final.logs[2:]), expected_serials
        ...
        v1.reset()
        self.assertEqual(self.invoke(mutated_start, [1], 1), [0])
        self.assertEqual(self.invoke(mutated_child, [0], 0), [0])
        self.assertEqual(self.invoke(mutated_main, [0], 0), [1])
        mutated = self.ko_verification(
            mutated_main, 68, reverse_tools=True
        )
        self.assertEqual(
            tuple(card.serial for card in mutated.current.players[1].discard[-5:]),
            (68, 112, 113, 122, 121),
        )
        action = self.invoke(mutated, [0], 0)
        parsed = self.policy.to_observation_class(self.fx.raw(mutated))
        self.assertTrue(v1.model.action_is_valid(parsed, action))
        self.assert_fault()
