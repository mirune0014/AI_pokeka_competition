"""Stateful outer transaction wrapper for the Fix7 END->Shaymin emergency."""
...
RULE_VERSION = "BENCH0_END_SHAYMIN_EMERGENCY_FIX7"
...
def policy_closure_sha256() -> str:
    """Hash the narrow rule source and its direct runtime constants."""
    payload = {
        "rule_version": RULE_VERSION,
        "shaymin_card_id": SHAYMIN_CARD_ID,
        "active_prize_liability": ACTIVE_PRIZE_LIABILITY,
...
def _delegate_matches(
    parent: Any,
    snapshot: dict[str, Any],
    trace_snapshot: Callable[[], Any],
) -> bool:
    """Check exact restoration of state owned by the complete parent."""
    return (
        core.parent_state_snapshot(parent) == snapshot["parent"]
        and core.INTEGRATED_TRANSACTION
        == snapshot["integrated_transaction"]
        and core.INTEGRATED_DUPLICATE_CACHE
        == snapshot["integrated_duplicate_cache"]
        and core._DUPLICATE_ORDER
        == snapshot["integrated_duplicate_order"]
        and core.INTEGRATED_TRACE_LOG
        == snapshot["integrated_trace_log"]
        and core.INTEGRATED_LATEST_TRACE
        == snapshot["integrated_latest_trace"]
        and deck_v1.V1_TRANSACTION == snapshot["v1_transaction"]
        and deck_v1.V1_DUPLICATES == snapshot["v1_duplicates"]
        and deck_v1.REMOVED_RULE_HITS == snapshot["removed_rule_hits"]
        and deck_v1.LAST_V1_PACKAGE_TRACE
        == snapshot["last_v1_package_trace"]
        and deck_v1.COMPLIANCE_BLOCK_TAG
        == snapshot["compliance_block_tag"]
        and trace_snapshot() == snapshot["trace_surface"]
    )
...
def _merge_trace(
    ...
    *,
    ...
    merged["candidate_closure_sha256"] = policy_closure_sha256()
...
def _rebind_basic(
    ...
    for option_index, option in enumerate(options):
        ...
        if (
            ...
            and _serial(card) == serial
            and (
                option.get("cardId") is None
                or _int(option.get("cardId")) == card_id
            )
            and (
                option.get("serial") is None
                or _int(option.get("serial")) == serial
            )
            and (
                option.get("playerIndex") is None
                or _int(option.get("playerIndex")) == owner
            )
        ):
...
def _evaluate_end_shaymin_emergency(
    obs: dict[str, Any],
    parent_action: Any,
) -> dict[str, Any]:
    """Evaluate only the Prize-effective parent-END Shaymin emergency."""
    select = obs.get("select") if isinstance(obs, dict) else None
    current = obs.get("current") if isinstance(obs, dict) else None
    if not isinstance(select, dict) or not isinstance(current, dict):
        return _no_action_decision(parent_action, "NOT_LIVE_NORMAL_MAIN")
    if (
        _int(select.get("context")) != damage.MAIN
        or _int(select.get("type")) != 0
        or _int(current.get("result")) != -1
    ):
        return _no_action_decision(parent_action, "NOT_LIVE_NORMAL_MAIN")
    ...
    if (
        state is None
        or state.get("turn") is None
        or state.get("action_count") is None
    ):
        return _no_action_decision(parent_action, "PUBLIC_STATE_INVALID")
    owner = state["owner"]
    mine = state["players"][owner]
    if len(mine["active"]) != 1 or mine["active"][0] is None:
        return _no_action_decision(parent_action, "OWN_ACTIVE_NOT_EXACTLY_ONE")
    if mine["bench"] != []:
        return _no_action_decision(parent_action, "OWN_BENCH_NOT_EMPTY")

    parent_semantic = damage.semantic_action(obs, parent_action)
    if parent_semantic != ("END", None):
        return _no_action_decision(
            parent_action,
            "PARENT_SEMANTIC_NOT_END",
            parent_semantic=list(parent_semantic)
            if parent_semantic is not None
            else None,
        )

    players = current.get("players")
    if not isinstance(players, list) or owner not in (0, 1):
        return _no_action_decision(parent_action, "PUBLIC_STATE_INVALID")
    hand = players[owner].get("hand")
    options = select.get("option")
    if not isinstance(hand, list) or not isinstance(options, list):
        return _no_action_decision(parent_action, "SHAYMIN_HAND_OR_OPTIONS_INVALID")
    physical = [
        (index, card)
        for index, card in enumerate(hand)
        if _card_id(card) == SHAYMIN_CARD_ID
    ]
    if len(physical) != 1:
        return _no_action_decision(
            parent_action,
            ...
            if len(physical) > 1
            else "SHAYMIN_NOT_IN_HAND",
            physical_shaymin_count=len(physical),
        )
    hand_index, shaymin = physical[0]
    shaymin_serial = _serial(shaymin)
    if (
        shaymin_serial is None
        or _int(shaymin.get("playerIndex")) != owner
    ):
        return _no_action_decision(
            parent_action, "SHAYMIN_PHYSICAL_IDENTITY_INVALID"
        )
    ...
    for option_index, option in enumerate(options):
        if not isinstance(option, dict) or _int(option.get("type")) != damage.PLAY:
            continue
        option_hand_index = _int(option.get("index"))
        if option_hand_index is None or not 0 <= option_hand_index < len(hand):
            continue
        card = hand[option_hand_index]
        if _card_id(card) != SHAYMIN_CARD_ID:
            continue
        if (
            option_hand_index != hand_index
            or _serial(card) != shaymin_serial
            or (
                option.get("cardId") is not None
                and _int(option.get("cardId")) != SHAYMIN_CARD_ID
            )
            or (
                option.get("serial") is not None
                and _int(option.get("serial")) != shaymin_serial
            )
            or (
                option.get("playerIndex") is not None
                and _int(option.get("playerIndex")) != owner
            )
        ):
            continue
        legal_matches.append(option_index)
    if len(legal_matches) != 1:
        return _no_action_decision(
            parent_action,
            "SHAYMIN_PLAY_OPTION_AMBIGUITY"
            if len(legal_matches) > 1
            else "SHAYMIN_PLAY_OPTION_NOT_EXACT",
            legal_shaymin_play_count=len(legal_matches),
        )

    prize_counts = [
        len(player["prize"])
        if isinstance(player, dict) and isinstance(player.get("prize"), list)
        else None
        for player in players
    ]
    if any(
        count is None or not 1 <= count <= 6
        for count in prize_counts
    ):
        return _no_action_decision(
            parent_action,
            "PUBLIC_PRIZE_COUNTS_INVALID",
            public_prize_counts=prize_counts,
        )
    active_id = mine["active"][0]["id"]
    liability = ACTIVE_PRIZE_LIABILITY.get(active_id)
    if liability is None:
        return _no_action_decision(
            parent_action,
            "OWN_ACTIVE_PRIZE_LIABILITY_UNKNOWN",
            own_active_id=active_id,
            public_prize_counts=prize_counts,
        )
    opponent = 1 - owner
    opponent_remaining_prizes = prize_counts[opponent]
    if opponent_remaining_prizes <= liability:
        return _no_action_decision(
            parent_action,
            "PRIZE_FUTILE",
            own_active_id=active_id,
            current_active_public_prize_liability=liability,
            opponent_remaining_prizes=opponent_remaining_prizes,
            public_prize_counts=prize_counts,
        )

    selected = {
        "card_id": SHAYMIN_CARD_ID,
        "serial": shaymin_serial,
        "hand_index": hand_index,
        "option_index": legal_matches[0],
    }
    decision_id = _hash(
        {
            "rule_version": RULE_VERSION,
    ...
    return {
        ...
        "candidate_closure_sha256": policy_closure_sha256(),
        "raw_parent_action": parent_action,
        "proposed_action": [legal_matches[0]],
        "applied_action": [legal_matches[0]],
        "guard_class": "BENCH0_END_SHAYMIN_EMERGENCY",
        "guard_failure": None,
        "transaction_stage": "PROPOSED",
        "selected_basic": selected,
        "decision_id": decision_id,
        "parent_semantic": ["END", None],
        "own_active_id": active_id,
        "current_active_public_prize_liability": liability,
        "opponent_remaining_prizes": opponent_remaining_prizes,
        "public_prize_counts": prize_counts,
        "broad_c3_action_path_enabled": False,
    }
...
def _abort_transaction(
    ...
    C3_DUPLICATES.clear()
    C3_DUPLICATE_ORDER.clear()
    # A malformed callback or failed movement check must never use an index or
    # semantic saved from the stale transaction. Re-run the complete parent
    # from its exact pre-call snapshot and return that current legal action.
    _restore_delegate(
        parent,
        transaction["pre"],
        trace_restore,
...
def agent(
    ...
    *,
    ...
    if transaction is not None:
        ...
        C3_DUPLICATES.clear()
        C3_DUPLICATE_ORDER.clear()
        reentry_pre_transaction = (
            core.INTEGRATED_TRANSACTION is not None
            or deck_v1.V1_TRANSACTION is not None
        )
        action = complete_parent(obs)
        fresh_post = _delegate_snapshot(parent, trace_snapshot)
        fresh_surface = fresh_post["trace_surface"]
        reentry_post_transaction = (
            fresh_post["integrated_transaction"] is not None
            or fresh_post["v1_transaction"] is not None
        )
        if reentry_pre_transaction or reentry_post_transaction:
            parent_trace = (
            ...
            trace = _merge_trace(
                ...
                failure="REENTRY_PARENT_TRANSACTION_IN_PROGRESS",
            )
            _publish(trace_publish, trace, fresh_surface)
            return action
        ...
        if semantic != tuple(transaction["parent_semantic"]):
            ...
            if rebound is not None:
                ...
                if _delegate_matches(
                    parent, transaction["original_post"], trace_snapshot
                ):
                    action = rebound
                    ...
                    failure = "REENTRY_SAVED_END_STATE_RESTORE_MISMATCH"
            else:
                parent_surface = fresh_surface
                parent_trace = (
                    fresh_surface.get("LAST_STAGED_POLICY_TRACE")
                ...
                failure = "REENTRY_SAVED_END_NOT_LEGAL"
            trace = _merge_trace(
                parent_trace,
                transaction["decision"],
                applied_action=action,
    ...
    try:
        decision = _evaluate_end_shaymin_emergency(obs, parent_action)
