...
def is_immediate_roserade_buster_breakpoint(obs) -> bool:
    """Identify the visible one-Roserade Buster KO setup available this turn."""
    if obs.select.context != SelectContext.TO_HAND:
        return False
    active = active_pokemon(obs)
    target = opponent_active(obs)
    if not active or active.id != GARCHOMP_EX or not target:
        return False
    if not any(
        pokemon.id == ROSELIA and not getattr(pokemon, "appearThisTurn", False)
        for pokemon in my_pokemon(obs)
    ):
        return False
    buster_damage = 260 + 30 * count_in_play(obs, ROSERADE)
    if not (buster_damage < hp(target) <= buster_damage + 30):
        return False
    remaining_prizes = len(getattr(me(obs), "prize", []) or [])
    if prize_value(target) < 2 and prize_value(target) < remaining_prizes:
        return False
    energies = energy_count(active)
    return energies >= 2 or (
        energies == 1
        and not getattr(obs.current, "energyAttached", False)
        and any(card_id in ENERGIES for card_id in hand_ids(obs))
    )
...
def score_to_hand(obs, opt, allow_support_pivot: bool = False) -> tuple[int, str]:
    ...
    if cid == ROSERADE:
        if allow_support_pivot and is_immediate_roserade_buster_breakpoint(obs):
            return 17000, "take Roserade for immediate Draconic Buster breakpoint"
