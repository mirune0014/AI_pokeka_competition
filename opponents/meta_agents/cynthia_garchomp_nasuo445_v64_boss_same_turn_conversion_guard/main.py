...
def public_energy_types(pokemon) -> list[int] | None:
    """Return currently provided public Energy units, or None if any are unknown."""
...
def attached_card_energy_type(card) -> int | None:
    """Return the exact published type of a legal manual Energy attachment."""
    card_id = getattr(card, "id", None)
    if card_id not in ENERGIES:
        return None
    return normalized_energy_type(getattr(CARD_DB.get(card_id), "energyType", None))


def attack_cost_is_payable(active, attack_id: int, extra_energy_type: int | None = None) -> bool:
    """Conservatively match typed costs, with Colorless paid by any remaining unit."""
    attack = ATTACK_DB.get(attack_id)
    published_cost = getattr(attack, "energies", None)
    available = public_energy_types(active)
    if published_cost is None or available is None:
        return False
    costs = [normalized_energy_type(energy) for energy in published_cost]
    if any(energy is None for energy in costs):
        return False
    if extra_energy_type is not None:
        available.append(extra_energy_type)

    for required in (energy for energy in costs if energy != 0):
        matching_index = next(
            (index for index, provided in enumerate(available) if provided == required),
            None,
        )
    ...
    return len(available) >= sum(energy == 0 for energy in costs)


def attack_is_published_for_active(obs, attack_id: int) -> bool:
    """Require the attack to belong to the visible Active Cynthia's Pokemon."""
    active = active_pokemon(obs)
    expected_attacks = {
        ROSELIA: {SPIKE_STING},
        ROSERADE: {LEAF_STEP},
        GIBLE: {ROCK_HURL},
        GABITE: {DRAGONSLICE},
        GARCHOMP_EX: {CORKSCREW_DIVE, DRACONIC_BUSTER},
        SPIRITOMB: {RAGING_CURSE},
    }
    if active is None or attack_id not in expected_attacks.get(active.id, set()):
        return False
    published = getattr(CARD_DB.get(active.id), "attacks", None)
    return published is not None and attack_id in published and attack_id in ATTACK_DB
...
def visible_damage_state_is_deterministic(obs, target) -> bool:
    """Fail closed on visible status, unresolved effects, or damage prevention."""
    player = me(obs)
    if any(
...
def conservative_public_damage(obs, target, attack_id: int) -> int | None:
    """Return a lower bound that ignores Weakness and includes public Resistance."""
    if (
    ...
    if attack_id != ROCK_HURL and attack_type is not None and resistance == attack_type:
        damage = max(0, damage - 30)
    return damage


def buster_guard_passes_for_target(obs, target) -> bool:
    """Apply v63's existing Buster continuity/prize guard to a prospective target."""
    buster_damage = conservative_public_damage(obs, target, DRACONIC_BUSTER)
    corkscrew_damage = conservative_public_damage(obs, target, CORKSCREW_DIVE)
    if buster_damage is None or corkscrew_damage is None:
        return False
    remaining_prizes = len(getattr(me(obs), "prize", []) or [])
    return bool(
        buster_damage >= hp(target)
        and corkscrew_damage < hp(target)
        and (
            prize_value(target) >= remaining_prizes
            or opponent_has_no_bench(obs)
            or prize_value(target) >= 2
...
def attack_certifies_target(
    obs,
    target,
    attack_id: int,
    extra_energy_type: int | None = None,
) -> bool:
    active = active_pokemon(obs)
    if (
        active is None
        or not attack_cost_is_payable(active, attack_id, extra_energy_type)
    ):
        return False
    damage = conservative_public_damage(obs, target, attack_id)
    if damage is None or damage < hp(target):
        return False
    return attack_id != DRACONIC_BUSTER or buster_guard_passes_for_target(obs, target)
...
def certified_bench_targets(
    ...
    return [
        target
        for target in (getattr(opponent(obs), "bench", None) or [])
        if target
        and any(
            attack_certifies_target(obs, target, attack_id, extra_energy_type)
            for attack_id in attack_ids
        )
    ]


def boss_same_turn_conversion_guard(
    obs,
    scored: list[tuple[int, int, str]],
) -> tuple[int | None, bool]:
    """Return an isolated replacement and whether the false Boss must be held."""
    if obs.select.context != SelectContext.MAIN or not scored:
        return None, False
    _top_score, top_index, top_reason = max(scored, key=lambda row: (row[0], -row[1]))
    top_option = obs.select.option[top_index]
    if not (
        top_reason.startswith("Boss for KO")
        and top_option.type == OptionType.PLAY
        and getattr(option_card(obs, top_option), "id", None) == BOSS
    ):
        return None, False
    ...
    if certified_bench_targets(obs, exposed_attack_ids):
    ...
    if getattr(obs.current, "energyAttached", None) is False:
        ...
        active = active_pokemon(obs)
        active_attacks = list(
            getattr(CARD_DB.get(getattr(active, "id", None)), "attacks", None) or []
        )
        for index, option in enumerate(obs.select.option):
            if (
                option.type != OptionType.ATTACH
                ...
                or option_target(obs, option) is not active
            ):
                continue
            energy_type = attached_card_energy_type(option_card(obs, option))
            if energy_type is None:
                continue
            if certified_bench_targets(obs, active_attacks, energy_type):
                projected_attaches.append((score_attach(obs, option)[0], index))
        if projected_attaches:
            return max(projected_attaches, key=lambda row: (row[0], -row[1]))[1], False

    return None, True


def certified_boss_target_index(obs) -> int | None:
    """Restrict a certified Boss conversion while retaining v63 target ordering."""
    if (
        obs.select.context != SelectContext.SWITCH
        or getattr(getattr(obs.select, "effect", None), "id", None) != BOSS
        or not active_is_main_attacker(obs)
    ):
        return None
    theoretical_target = best_boss_target(obs)
    if theoretical_target is None or best_damage_for_active(obs) < hp(theoretical_target):
        return None
    ...
    for index, option in enumerate(obs.select.option):
        ...
        if (
            ...
            or not any(
                attack_certifies_target(obs, target, attack_id)
                for attack_id in attack_ids
            )
        ):
            continue
        eligible.append((boss_target_score(obs, target)[0], index))
    return max(eligible, key=lambda row: (row[0], -row[1]))[1] if eligible else None
...
def choose_options(obs):
    ...
    if route_index is not None:
        return [route_index]
    boss_target_index = certified_boss_target_index(obs)
    if boss_target_index is not None:
        return [boss_target_index]
    scored: list[tuple[int, int, str]] = []
    ...
    if obs.select.context == SelectContext.MAIN:
        ...
        if bridge_index is not None:
            return [bridge_index]
        conversion_index, hold_false_boss = boss_same_turn_conversion_guard(obs, scored)
        if conversion_index is not None:
            return [conversion_index]
        if hold_false_boss:
            scored = [
                (-400, index, "hold unconverted Boss KO")
                if reason.startswith("Boss for KO")
                and obs.select.option[index].type == OptionType.PLAY
                and getattr(option_card(obs, obs.select.option[index]), "id", None) == BOSS
                else (score, index, reason)
                for score, index, reason in scored
            ]
