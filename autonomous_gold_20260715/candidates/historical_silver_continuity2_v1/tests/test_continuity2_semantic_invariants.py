...
class Continuity2SemanticInvariantTests(unittest.TestCase):
    ...
    def test_turbo_reuses_safe_h0_target_set_for_future_h1(self):
        ...
        self.assertEqual(main.CONTINUITY_LATEST_TRACE["choice"]["exact_count"], 3)
        self.assertEqual(
            main.CONTINUITY_LATEST_TRACE["choice"]["kind"], "TURBO_EXACT_BOUND_H1"
        )
        role = main.CONTINUITY_LATEST_TRACE["H1_after_KO"]
        envelope = main.CONTINUITY_LATEST_TRACE["response_envelope"]
        self.assertEqual(role["readiness"], "READY_AFTER_TURBO")
        self.assertEqual(role["response_envelope"], envelope)
        self.assertEqual(role["h1_primary_gate"]["state"], "READY")
        self.assertEqual(
            {
                row["target_serial"]
                for row in role["h1_primary_gate"]["target_results"]
                if not row["terminal"]
            },
            {
                row["serial"]
                for row in envelope["post_response_active_candidates"]
                if not row["terminal"]
            },
        )
