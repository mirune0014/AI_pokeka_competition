...
baseline = load(BASELINE, "rule6_shadow_rule5")
sys.modules.pop("_historical_silver_parent", None)
candidate = load(CANDIDATE, "rule6_shadow_candidate")
            ...
            for step, obs, _recorded in decisions:
                ...
                if not valid_action(obs, left):
                    invalid_actions.append(
                        {"replay": replay_path.name, "seat": seat, "step": step, "side": "rule5", "action": left}
                    )
                if not valid_action(obs, right):
                    invalid_actions.append(
                        {"replay": replay_path.name, "seat": seat, "step": step, "side": "rule6", "action": right}
                    )
                ...
                if (
                    ...
                    and proposal.get("rule_id") == candidate._RULE6_ID
                    and (proposal.get("transaction") or {}).get("stage") == "PAD_PLAY_EMITTED"
                ):
                    natural_starts += 1
                if (
                    proposal is not None
                    and proposal.get("rule_id") == candidate._RULE6_ID
                    and proposal.get("purpose") == candidate._RULE6_WHIFF_EMPTY
                    and not telemetry.get("duplicate_retry")
                ):
                    whiff_emissions += 1
                if telemetry.get("rejection_reason") == "rule6_ready_complete":
                    ready_completions += 1
                if telemetry.get("rejection_reason") == "rule6_whiff_complete":
                    whiff_completions += 1
                reason = str(telemetry.get("rejection_reason", ""))
                if (
                    (telemetry.get("owner_before") or {}).get("owner")
                    == candidate._RULE6_ID
                    and reason.startswith("rule6_")
                    and reason
                    not in {"rule6_ready_complete", "rule6_whiff_complete"}
                    and (
                        reason.endswith("failed")
                        or "conflict" in reason
                        or "discontinuity" in reason
                    )
                ):
                    owned_fail_closed.append(
                        {"replay": replay_path.name, "seat": seat, "step": step, "reason": reason}
                    )
                if left == right:
                    continue
...
summary = {
    ...
    "ready_completions": ready_completions,
    "whiff_emissions": whiff_emissions,
    "whiff_completions": whiff_completions,
    "owned_fail_closed_count": len(owned_fail_closed),
    "owned_fail_closed": owned_fail_closed,
    ...
    "all_differences_rule6": all(row["classification"] in allowed for row in differences),
    "invalid_actions": invalid_actions,
    "exceptions": exceptions,
    "fault_count": len(invalid_actions) + len(exceptions),
}
