...
def _starmie_shaymin_is_rule_box(card_id: int) -> bool:
    """Return the checked public rule-box marker for a Pokemon card."""
    data = card_table.get(card_id)
    return bool(data is not None and (data.ex or data.megaEx))


def _starmie_shaymin_has_fragile_bench(mine) -> bool:
    """Whether direct 50 Bench damage can KO a public non-rule-box target."""
    for pokemon in mine.bench:
        if pokemon is None or pokemon.hp <= 0 or pokemon.hp > 50:
            continue
        data = card_table.get(pokemon.id)
        if (
            data is not None
            and data.cardType == CardType.POKEMON
            and not _starmie_shaymin_is_rule_box(pokemon.id)
        ):
            return True
    return False


def _starmie_shaymin_on_field(mine) -> bool:
    return any(
        pokemon is not None and pokemon.id == Shaymin
        for pokemon in list(mine.active) + list(mine.bench)
    )


def _starmie_shaymin_dawn_action(obs: Observation) -> list[int] | None:
    """Prefer Dawn's lowest legal Shaymin only under the frozen public guard."""
    state = obs.current
    select = obs.select
    if (
        select.context != SelectContext.TO_HAND
        or getattr(select.effect, "id", None) != Dawn
        or select.maxCount < 1
    ):
        return None
    ...
    if not any(
        pokemon.id in (Staryu, Mega_Starmie_ex)
        for pokemon in opponent_field
    ):
        return None
    if _starmie_shaymin_on_field(mine) or any(
        card.id == Shaymin for card in (mine.hand or [])
    ):
        return None

    abra_line_bodies = sum(
        pokemon is not None and pokemon.id in ABRA_LINE
        for pokemon in list(mine.active) + list(mine.bench)
    ) + sum(card.id in ABRA_LINE for card in (mine.hand or []))
    if abra_line_bodies < 2 or not _starmie_shaymin_has_fragile_bench(mine):
        return None
    ...
    for option_index, option in enumerate(select.option):
        ...
        if (
            card is not None
            and card.id == Shaymin
            and card.playerIndex == my_index
        ):
            shaymin_options.append(option_index)
    return [min(shaymin_options)] if shaymin_options else None


def _starmie_shaymin_has_certified_immediate_ko(obs: Observation) -> bool:
    """Keep a public, already legal immediate Active KO above Bench protection."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if active is None or target is None or target.hp <= 0:
        return False
    ...
    boss_legal = any(
        ...
        and (card := _turn_guard_hand_card(mine, option.index, my_index))
        is not None
    ...
    if (
        ...
        and _two_prize_alakazam_is_ready(active, my_index)
    ):
        boss_damage = 20 * (len(mine.hand or []) - 1)
        for pokemon in theirs.bench:
            if (
                _two_prize_opponent_pokemon_is_clear(
                    state, pokemon, 1 - my_index
                )
                and pokemon.hp > 0
                and boss_damage >= pokemon.hp
            ):
                return True
    ...
    for option in select.option:
        ...
        if option.attackId == ATTACK_POWERFUL_HAND:
            damage = 20 * len(mine.hand or [])
        else:
            attack = attack_table.get(option.attackId)
            if attack is None:
                continue
            # Use the direct-damage floor after the largest printed
            # resistance; do not project weakness or hidden effects.
            damage = max(0, attack.damage - 30)
        if damage >= target.hp:
            return True
    return False


def _starmie_shaymin_main_action(obs: Observation) -> list[int] | None:
    """Deploy held Shaymin under ready Jetting Blow after inherited routes."""
    state = obs.current
    select = obs.select
    ...
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    opponent = theirs.active[0] if len(theirs.active) == 1 else None
    if (
        opponent is None
        or opponent.id != Mega_Starmie_ex
        or not any(card.id == Basic_Water_Energy for card in opponent.energyCards)
        or _starmie_shaymin_on_field(mine)
        or not _starmie_shaymin_has_fragile_bench(mine)
    ):
        return None
    ...
    for option_index, option in enumerate(select.option):
        if option.type != OptionType.PLAY:
            continue
        card = get_card(obs, AreaType.HAND, option.index, my_index)
        if card is not None and card.id == Shaymin:
            shaymin_options.append(option_index)
    if not shaymin_options or _starmie_shaymin_has_certified_immediate_ko(obs):
        return None
    return [min(shaymin_options)]
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if (
        ...
        dawn_shaymin_action = _starmie_shaymin_dawn_action(obs)
        if dawn_shaymin_action is not None:
            return _remember_action(decision_signature, dawn_shaymin_action)
    ...
    # All inherited certified wins, KOs, enabling routes, and abort boundaries
    # above retain first refusal.  This is the isolated ordinary-MAIN overlay.
    if (
        not prepare_boundary_delegated
        and not turn_objective_recovery_delegated
        and not guarded_teleportation_delegated
        ...
        shaymin_guard_action = _starmie_shaymin_main_action(obs)
        if shaymin_guard_action is not None:
            return _remember_action(decision_signature, shaymin_guard_action)
    if (
        context == SelectContext.MAIN
        and chosen_action
        and not _hilda_source_latch
