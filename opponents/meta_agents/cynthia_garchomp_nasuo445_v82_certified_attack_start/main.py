...
def attack_energy_is_paid(pokemon, attack_id: int, extra_types: tuple[int, ...] = ()) -> bool:
    """Certify an attack cost from only the Energy types already in public state."""
    attack = ATTACK_DB.get(attack_id)
    if pokemon is None or attack is None:
        return False
    attached = [int(getattr(value, "value", value)) for value in (getattr(pokemon, "energies", None) or [])]
    attached.extend(extra_types)
    required = [int(getattr(value, "value", value)) for value in (getattr(attack, "energies", None) or [])]
    colorless = required.count(0)
    for energy_type in (value for value in required if value != 0):
        try:
            index = attached.index(energy_type)
        except ValueError:
            try:
                index = attached.index(10)  # Rainbow Energy.
            except ValueError:
                return False
        attached.pop(index)
    return len(attached) >= colorless


def guaranteed_attack_damage(obs, attacker, attack_id: int, target) -> int:
    """Return only printed, public-state damage that the route can guarantee."""
    attack = ATTACK_DB.get(attack_id)
    attacker_data = CARD_DB.get(getattr(attacker, "id", None))
    target_data = CARD_DB.get(getattr(target, "id", None))
    if attack is None or attacker_data is None or target_data is None or target is None:
        return 0
    if getattr(target, "preventDamage", False):
        return 0
    damage = int(getattr(attack, "damage", 0) or 0)
    attacker_serial = getattr(attacker, "serial", None)
    if (
        getattr(attacker, "id", None) in CYNTHIA_LINE
        and attacker_serial is not None
        and any(getattr(pokemon, "serial", None) == attacker_serial for pokemon in my_pokemon(obs))
    ):
        damage += roserade_bonus(obs)
    attack_type = int(getattr(getattr(attacker_data, "energyType", None), "value", getattr(attacker_data, "energyType", -1)))
    weakness = getattr(target_data, "weakness", None)
    resistance = getattr(target_data, "resistance", None)
    if weakness is not None and int(getattr(weakness, "value", weakness)) == attack_type:
        damage *= 2
    if (
        attack_id != ROCK_HURL
        and resistance is not None
        and int(getattr(resistance, "value", resistance)) == attack_type
    ):
        damage = max(0, damage - 30)
    return damage


def attack_start_attacks(pokemon) -> list[int]:
    expected = {GIBLE: ROCK_HURL, GABITE: DRAGONSLICE}.get(getattr(pokemon, "id", None))
    data = CARD_DB.get(getattr(pokemon, "id", None))
    if expected is None or data is None or expected not in (getattr(data, "attacks", None) or []):
        return []
    return [expected] if attack_energy_is_paid(pokemon, expected) else []


def certified_next_turn_prize_liability(obs, pokemon) -> int:
    """Count prizes only when a currently paid opposing attack visibly guarantees a KO."""
    attacker = opponent_active(obs)
    data = CARD_DB.get(getattr(attacker, "id", None))
    if pokemon is None or attacker is None or data is None:
        return 0
    for attack_id in (getattr(data, "attacks", None) or []):
        if (
            attack_energy_is_paid(attacker, attack_id)
            and guaranteed_attack_damage(obs, attacker, attack_id, pokemon) >= hp(pokemon)
        ):
            return prize_value(pokemon)
    return 0


def attack_start_attack_key(obs, attacker, attack_id: int, stable_index: int) -> tuple[int, int, int, int]:
    target = opponent_active(obs)
    damage = guaranteed_attack_damage(obs, attacker, attack_id, target)
    knockout = bool(target and damage >= hp(target))
    remaining = len(getattr(me(obs), "prize", []) or [])
    game_win = bool(knockout and remaining > 0 and prize_value(target) >= remaining)
    return int(game_win), int(knockout), damage, -stable_index


def certified_attack_start_candidate(obs, pokemon, support, stable_index: int):
    if pokemon is None or prize_value(pokemon) != 1:
        return None
    attacks = attack_start_attacks(pokemon)
    if not attacks or not attack_energy_is_paid(pokemon, CORKSCREW_DIVE):
        return None
    ranked = [
        (attack_start_attack_key(obs, pokemon, attack_id, index), attack_id)
        for index, attack_id in enumerate(attacks)
    ]
    attack_key, attack_id = max(ranked, key=lambda row: row[0])
    immediate_prize = bool(attack_key[1])
    if (
        certified_next_turn_prize_liability(obs, pokemon)
        > certified_next_turn_prize_liability(obs, support)
        and not immediate_prize
    ):
        return None
    return attack_key + (-stable_index,), attack_id


def certified_attack_start_target(obs, support):
    eligible = []
    for bench_index, pokemon in enumerate(getattr(me(obs), "bench", None) or []):
        route = certified_attack_start_candidate(obs, pokemon, support, bench_index)
        if route is not None:
            key, attack_id = route
            eligible.append((key, pokemon, attack_id))
    return max(eligible, key=lambda row: row[0]) if eligible else None


def support_can_take_current_prize(obs, support, extra_basic: bool = False) -> bool:
    target = opponent_active(obs)
    data = CARD_DB.get(getattr(support, "id", None))
    if target is None or data is None:
        return False
    extra = (BASIC_FIGHTING,) if extra_basic else ()
    return any(
        attack_energy_is_paid(support, attack_id, extra)
        and guaranteed_attack_damage(obs, support, attack_id, target) >= hp(target)
        for attack_id in (getattr(data, "attacks", None) or [])
    )


def printed_retreat_cost(pokemon) -> int:
    data = CARD_DB.get(getattr(pokemon, "id", None))
    return int(getattr(data, "retreatCost", 0) or 0) if data else 0


def is_basic_attach_to_active(obs, option, active) -> bool:
    target = option_target(obs, option)
    return bool(
        option.type == OptionType.ATTACH
        and getattr(option_card(obs, option), "id", None) == BASIC_FIGHTING
        and option.inPlayArea == AreaType.ACTIVE
        and getattr(target, "serial", None) is not None
        and getattr(target, "serial", None) == getattr(active, "serial", None)
    )


def attached_basic_to_current_active(obs, active) -> bool:
    your_index = obs.current.yourIndex
    matching = [
        log for log in (getattr(obs, "logs", None) or [])
        if int(getattr(getattr(log, "type", None), "value", getattr(log, "type", -1))) == 11
        and getattr(log, "playerIndex", None) == your_index
        and getattr(log, "cardId", None) == BASIC_FIGHTING
        and getattr(log, "cardIdTarget", None) == getattr(active, "id", None)
        and getattr(log, "serialTarget", None) == getattr(active, "serial", None)
    ]
    return len(matching) == 1


def certified_attack_start_route_index(obs) -> int | None:
    """Advance only a complete attach-retreat-promote-attack support transaction."""
    options = getattr(obs.select, "option", None) or []
    context = obs.select.context
    active = active_pokemon(obs)

    if context == SelectContext.MAIN and getattr(active, "id", None) in {ROSELIA, ROSERADE}:
        retreat_cost = printed_retreat_cost(active)
        if (
            getattr(obs.current, "retreated", None) is not False
            or retreat_cost <= 0
            or support_can_take_current_prize(obs, active, extra_basic=True)
            or certified_attack_start_target(obs, active) is None
        ):
            return None

        if getattr(obs.current, "energyAttached", None) is False:
            if energy_count(active) + 1 != retreat_cost or BASIC_FIGHTING not in hand_ids(obs):
                return None
            attach_indices = [
                index for index, option in enumerate(options)
                if is_basic_attach_to_active(obs, option, active)
            ]
            return attach_indices[0] if attach_indices else None

        if (
            getattr(obs.current, "energyAttached", None) is True
            and energy_count(active) == retreat_cost
            and has_energy_id(active, BASIC_FIGHTING)
            and attached_basic_to_current_active(obs, active)
        ):
            retreat_indices = [
                index for index, option in enumerate(options)
                if option.type == OptionType.RETREAT
            ]
            return retreat_indices[0] if retreat_indices else None
        return None

    if context == SelectContext.SWITCH and getattr(active, "id", None) in {ROSELIA, ROSERADE}:
        paid_basic = [
            log for log in (getattr(obs, "logs", None) or [])
            if int(getattr(getattr(log, "type", None), "value", getattr(log, "type", -1))) == 6
            and getattr(log, "playerIndex", None) == obs.current.yourIndex
            and getattr(log, "cardId", None) == BASIC_FIGHTING
            and getattr(log, "fromArea", None) == AreaType.ACTIVE
            and getattr(log, "toArea", None) == AreaType.DISCARD
        ]
        if (
            getattr(obs.current, "energyAttached", None) is not True
            or getattr(obs.current, "retreated", None) is not True
            or getattr(obs.select, "effect", None) is not None
            or energy_count(active) != 0
            or len(paid_basic) != 1
            or support_can_take_current_prize(obs, active)
        ):
            return None
        target_route = certified_attack_start_target(obs, active)
        if target_route is None:
            return None
        _key, intended, _attack_id = target_route
        intended_serial = getattr(intended, "serial", None)
        matching = [
            index for index, option in enumerate(options)
            if option.type == OptionType.CARD
            and getattr(option_card(obs, option), "serial", None) == intended_serial
        ]
        return matching[0] if len(matching) == 1 else None

    if (
        context == SelectContext.MAIN
        and getattr(active, "id", None) in {GIBLE, GABITE}
        and getattr(obs.current, "energyAttached", None) is True
        and getattr(obs.current, "retreated", None) is True
    ):
        your_index = obs.current.yourIndex
        switch_logs = [
            log for log in (getattr(obs, "logs", None) or [])
            if int(getattr(getattr(log, "type", None), "value", getattr(log, "type", -1))) == 8
            and getattr(log, "playerIndex", None) == your_index
            and getattr(log, "cardIdActive", None) in {ROSELIA, ROSERADE}
            and getattr(log, "cardIdBench", None) == getattr(active, "id", None)
            and getattr(log, "serialBench", None) == getattr(active, "serial", None)
        ]
        if len(switch_logs) != 1:
            return None
        support_serial = getattr(switch_logs[0], "serialActive", None)
        supports = [
            pokemon for pokemon in (getattr(me(obs), "bench", None) or [])
            if getattr(pokemon, "serial", None) == support_serial
            and getattr(pokemon, "id", None) in {ROSELIA, ROSERADE}
        ]
        if len(supports) != 1 or energy_count(supports[0]) != 0:
            return None
        route = certified_attack_start_candidate(obs, active, supports[0], 0)
        if route is None:
            return None
        _key, intended_attack = route
        legal_attacks = [
            (attack_start_attack_key(obs, active, option.attackId, index), index)
            for index, option in enumerate(options)
            if option.type == OptionType.ATTACK
            and option.attackId == intended_attack
            and attack_energy_is_paid(active, option.attackId)
        ]
        return max(legal_attacks, key=lambda row: row[0])[1] if legal_attacks else None
    return None
...
def choose_options(obs):
    attack_start_index = certified_attack_start_route_index(obs)
    if attack_start_index is not None:
        return [attack_start_index]
