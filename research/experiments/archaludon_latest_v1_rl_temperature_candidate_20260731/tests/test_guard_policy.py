...
class GuardPolicyTests(unittest.TestCase):
    ...
    def test_train_deploy_eligibility_identical(self):
        ...
        self.assertEqual(training.legal_option_count, 2)
        self.assertEqual(len(training.q_latest), training.legal_option_count)
        self.assertEqual(training.teacher_probability, training.q_latest[0])
        self.assertEqual(
            training.prior_schema_version,
            REFERENCE_PRIOR_SCHEMA_VERSION,
        )
        self.assertEqual(training.prior_schema_sha256, reference_prior_sha256())
