...
def _select_envelope_mismatch_reason(
    ...
    *,
    ...
    checks = (
        ...
        (type(select.remainEnergyCost) is int and select.remainEnergyCost == 0, "SELECT_REMAIN_ENERGY"),
...
def _owned_stage_rejection_reason(parent, obs, transaction):
    ...
    specifications = {
        "await_boss_child": (1, parent.SelectContext.SWITCH, None, transaction["card_row"], 1, 1),
        "await_boss_attack": (0, parent.SelectContext.MAIN, None, None, 1, 1),
        "await_hammer_child": (4, parent.SelectContext.DISCARD_ENERGY, None, transaction["card_row"], 1, 1),
        "await_lana_child": (1, parent.SelectContext.TO_HAND, None, transaction["card_row"], 1, getattr(obs.select, "maxCount", None)),
        "await_xerosic_verify": (0, parent.SelectContext.MAIN, None, None, 1, 1),
        "await_mine_attack": (0, parent.SelectContext.MAIN, None, None, 1, 1),
        "await_alakazam_ability": (9, parent.SelectContext.ACTIVATE, transaction["card_row"], None, 1, 1),
        "await_alakazam_attack": (0, parent.SelectContext.MAIN, None, None, 1, 1),
        "await_backup_alakazam_ability": (9, parent.SelectContext.ACTIVATE, transaction["card_row"], None, 1, 1),
        "await_backup_alakazam_attack": (0, parent.SelectContext.MAIN, None, None, 1, 1),
    }
...
def _attack_target_moves(parent, target, opponent):
    rows = (
        ...
        *((_card_row(card), int(parent.AreaType.PRE_EVOLUTION)) for card in target.preEvolution),
        *((_card_row(card), int(parent.AreaType.ENERGY)) for card in target.energyCards),
...
def _fingerprint_without_energy(fingerprint, energy_index, energy_row):
    if (
        fingerprint is None
        or not 0 <= energy_index < len(fingerprint[6])
        or not 0 <= energy_index < len(fingerprint[7])
...
def _verify_hammer(parent, obs, transaction):
    ...
    before_rows = post[area_key]
    before_matches = [
        (index, row)
        for index, row in enumerate(before_rows)
        if row[1] == transaction["pokemon_serial"]
    ]
    expected_rows = None
    if len(before_matches) == 1:
        index, before = before_matches[0]
        after = _fingerprint_without_energy(
            before, transaction["energy_index"], transaction["energy_row"]
        )
        ...
        if V1_TRANSACTION is not None:
            ...
            if stage == "await_boss_child":
                ...
                if transaction["rule"] in (RULE_BOSS, RULE_BOSS_READY_STOP):
                    ...
                    RULE_ALAKAZAM, RULE_ALAKAZAM_READY_BENCH
                ):
            ...
            if action is not None and model.action_is_valid(obs, action):
                ...
                _trace(
                    ...
                    "EXACT_CHILD_PROMPT", "SERIAL_REBOUND",
                )
                return action
            if complete:
