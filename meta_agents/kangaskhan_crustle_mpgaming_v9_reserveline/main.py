...
def reserve_line_attach_score(obs, me, opp, opt):
    """Return a MAIN-only Scissors-overriding score for a ready reserve body."""
    if opt.type != OptionType.ATTACH or opt.inPlayArea != AreaType.BENCH:
        return 0
    a, target = active(me), card(obs, opt.inPlayArea, opt.inPlayIndex, obs.current.yourIndex)
    attacks = {choice.attackId for choice in obs.select.option if choice.type == OptionType.ATTACK}
    opponent_active = active(opp)
    if not (a and a.id == CRUSTLE and len(a.energies) >= 3
            and SCISSORS in attacks and opponent_active and opponent_active.hp > 120
            and sum(count(me, cid) for cid in (DWEBBLE, CRUSTLE)) >= 2
            and target and target.id in (DWEBBLE, CRUSTLE)
            and not target.appearThisTurn and len(target.energies) < 2):
        return 0
    if target.id == CRUSTLE and len(target.energies) == 1:
        return 13000
    if target.id == DWEBBLE and len(target.energies) == 0:
        return 12900
    if target.id == CRUSTLE and len(target.energies) == 0:
        return 12800
    return 0
    ...
    try:
        ...
        for opt in sel.option:
            ...
            if sel.context == SelectContext.MAIN:
                ...
                elif opt.type == OptionType.ATTACH: score = reserve_line_attach_score(obs, me, opp, opt) or 7000
