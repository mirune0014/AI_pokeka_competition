...
import planner_active_dudunsparce_ko as active_run
...
ACTIVE_DUDUNSPARCE_KO_KIND = "ACTIVE_DUDUNSPARCE_RUN_AWAY_KO_TRANSACTION_V1"
DUNSPARCE = 305
DUDUNSPARCE = 66
SUPER_PSY_BOLT = 1071
_PSYCHIC_ATTACKS = {
...
def _add_missing_role_for_reservations(parent: Any, obs: Any, plan: Any):
    ...
    public = [
        pokemon
        for pokemon in list(obs.current.players[owner].active)
        + list(obs.current.players[owner].bench)
        if pokemon.id not in semantics.REMOVED_OWN_CARD_IDS
    ]
...
def _certify_candidate(parent: Any, obs: Any, candidate: Any):
    ...
    if dict(plan.metadata).get("kind") == "HANDHELD_FAN_RESPONSE":
        return None
...
def _arbitrate_new_plan(parent: Any, obs: Any, snap: Any, parent_action: list[int], parent_pre: dict[str, Any], parent_post: dict[str, Any]):
    ...
    if not owners:
        active_certificate = active_run.certificate(parent, obs, snap, parent_action)
        if active_certificate is not None:
            active_data, active_action = active_certificate
            active = active_run.build_plan(
                parent, obs, snap, parent_action, active_data, active_action
            )
...
def _advance_transaction(parent: Any, obs: Any):
    transaction = core.INTEGRATED_TRANSACTION
    if transaction is not None and transaction.get("kind") == ACTIVE_DUDUNSPARCE_KO_KIND:
        return active_run.advance(parent, obs, transaction)
