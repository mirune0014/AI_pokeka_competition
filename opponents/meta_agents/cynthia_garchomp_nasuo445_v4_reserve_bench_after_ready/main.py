...
def bench_count(obs) -> int:
    return sum(1 for pokemon in (me(obs).bench or []) if pokemon)


def has_legal_garchomp_ex_evolution(obs) -> bool:
    """Check current own legal actions for a Gabite-to-Garchomp ex evolution."""
    for evolve in obs.select.option:
        if evolve.type != OptionType.EVOLVE:
            continue
        player_index = getattr(evolve, "playerIndex", None)
        if player_index is not None and player_index != obs.current.yourIndex:
            continue
        if (
            getattr(option_card(obs, evolve), "id", None) == GARCHOMP_EX
            and getattr(option_target(obs, evolve), "id", None) == GABITE
        ):
            return True
    return False


def reserve_bench_after_main_line_ready(obs) -> bool:
    return bench_count(obs) >= 4 and (
        has_in_play(obs, GARCHOMP_EX) or has_legal_garchomp_ex_evolution(obs)
    )


def reserve_optional_basic_play(obs, card_id: int | None) -> bool:
    if not reserve_bench_after_main_line_ready(obs):
        return False
    return card_id in {ROSELIA, SPIRITOMB} or (
        card_id == GIBLE and count_in_play(obs, GIBLE) >= 2
    )
...
def score_play(obs, opt) -> tuple[int, str]:
    ...
    if reserve_optional_basic_play(obs, cid):
        return -9000, "reserve bench after main line ready"
    deck = deck_count(obs)
    if cid == GIBLE:
