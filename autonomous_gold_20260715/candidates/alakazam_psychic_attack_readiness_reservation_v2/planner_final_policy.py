...
def _advance_psychic_transaction(parent: Any, obs: Any, transaction: dict[str, Any]):
    ...
    if stage in ("await_attach_resolution", "optional_telepath_prompt"):
        ...
        if obs.select.context == parent.SelectContext.TO_BENCH:
            ...
            _psychic_trace(
                ...
                parent_action,
                [],
                "OPTIONAL_TELEPATH_SEARCH_SUPPRESSED",
                stage="optional_telepath_prompt",
            )
            return "override", [], "OPTIONAL_TELEPATH_SEARCH_SUPPRESSED"
        if obs.select.context != parent.SelectContext.MAIN:
            return abort("POST_ATTACH_CONTEXT_UNEXPECTED")
        if data["role"] == "H0":
            action = _exact_attack_action(parent, obs, data)
            if action is None:
            ...
            _psychic_trace(
                ...
                parent_action,
                action,
                "H0_EXACT_POSITIVE_ATTACK",
                stage="await_attack",
            )
            return "override", action, "H0_EXACT_POSITIVE_ATTACK"
    ...
    if stage == "reserved_until_exposure":
        ...
        if found is not None and found[0] == parent.AreaType.ACTIVE:
            ...
            action = _exact_attack_action(parent, obs, data)
            if action is None:
                return abort("PROMOTED_H0_ATTACK_UNAVAILABLE")
            _set_psychic_stage(transaction, "await_resolution")
            _psychic_trace(
                "PSYCHIC_READINESS_ATTACK",
                transaction,
                ...
                parent_action,
                action,
                "PROMOTED_H0_EXACT_POSITIVE_ATTACK",
                stage="await_attack",
            )
            return "override", action, "PROMOTED_H0_EXACT_POSITIVE_ATTACK"
        if obs.select.context in (parent.SelectContext.SWITCH, parent.SelectContext.TO_ACTIVE):
            if _original_active_is_gone(parent, obs, data):
                ...
                data["role"] = "H0"
                _set_psychic_stage(transaction, "await_promoted_main")
                _psychic_trace(
            ...
            _psychic_trace(
                ...
                parent_action,
                parent_action,
                "ORDINARY_SWITCH_REMAINS_PARENT_OWNED",
                stage=stage,
            )
            return "pass", None, "ORDINARY_SWITCH_REMAINS_PARENT_OWNED"
        if obs.select.context == parent.SelectContext.MAIN:
            later_turn = (
    ...
    if stage == "await_promoted_main":
        if not _reservation_is_exact(parent, obs, data):
            return abort("PROMOTED_H0_STALE")
        action = _exact_attack_action(parent, obs, data)
        if action is None:
            return abort("PROMOTED_NEXT_MAIN_ATTACK_UNAVAILABLE")
        _set_psychic_stage(transaction, "await_resolution")
        _psychic_trace(
            "PSYCHIC_READINESS_ATTACK",
            transaction,
            ...
            parent_action,
            action,
            "PROMOTED_NEXT_MAIN_EXACT_ATTACK",
            stage="await_attack",
        )
        return "override", action, "PROMOTED_NEXT_MAIN_EXACT_ATTACK"
...
def agent(parent: Any, parent_agent: Any, obs_dict: dict) -> list[int]:
    ...
    if not runtime_model.raw_parsed_agree(obs_dict, obs):
        core.reset_integrated_state()
        return parent_agent(obs_dict)
    action = integrated.agent(parent, parent_agent, obs_dict)
