...
def ready_hariyama(obs) -> bool:
    return any(p.id == HARIYAMA and energy_count(p) >= 2 for p in my_pokemon(obs))
...
def active_is_main_attacker(obs) -> bool:
    ...
    return bool(active and active.id in {MEGA_LUCARIO_EX, HARIYAMA, SOLROCK, LUNATONE})
...
def best_damage_for_active(obs, attack_id: int | None = None) -> int:
    ...
    if attack_id == POWER_GEM or active.id == LUNATONE:
        return 50
    if attack_id == WILD_PRESS or active.id == HARIYAMA:
        return 120
    if attack_id == CORKSCREW_PUNCH or active.id == MAKUHITA:
        return 30
...
def setup_score(obs, opt) -> tuple[int, str]:
    ...
    if ctx == SelectContext.SETUP_ACTIVE_POKEMON:
        scores = {RIOLU: 9500, SOLROCK: 6400, MAKUHITA: 5200, LUNATONE: 3600, MEGA_LUCARIO_EX: 2500}
    ...
    if ctx == SelectContext.SETUP_BENCH_POKEMON:
        ...
        if cid == MAKUHITA:
            return 5000 if count_in_play(obs, MAKUHITA) < 1 else 800, "setup bench Makuhita"
    return 0, "setup"
...
def score_play(obs, opt) -> tuple[int, str]:
    ...
    if cid == MAKUHITA:
        return 5200 if count_in_play(obs, MAKUHITA) < 1 else 700, "bench Makuhita"
    ...
    if cid == VS_SEEKER:
        return 4200 if active_is_main_attacker(obs) else 1200, "VS Seeker"
    if cid == HERO_CAPE:
...
def score_evolve(obs, opt) -> tuple[int, str]:
    ...
    if getattr(card, "id", None) == HARIYAMA and getattr(target, "id", None) == MAKUHITA:
        return 12000 + energy_count(target) * 800, "evolve Hariyama"
    return 1000, "evolve"
...
def score_attach(obs, opt) -> tuple[int, str]:
    ...
    if tid == HARIYAMA:
        return 6500 + max(0, 2 - energy_count(target)) * 800, "attach Hariyama"
    if tid == MAKUHITA:
        return 5200 + max(0, 2 - energy_count(target)) * 600, "preload Makuhita"
...
def score_retreat(obs, _opt) -> tuple[int, str]:
    ...
    if ready_hariyama(obs):
        return 8200, "retreat to Hariyama"
    return 600, "retreat"
...
def attack_score(obs, attack_id: int | None) -> tuple[int, str]:
    ...
    elif attack_id == WILD_PRESS:
        score, reason = 10500, "Wild Press"
    elif attack_id in {CORKSCREW_PUNCH, CONFRONT}:
        score, reason = 3600 + damage, "Makuhita attack"
    else:
...
def discard_score(obs, cid: int | None) -> tuple[int, str]:
    ...
    if cid in {MAKUHITA, HARIYAMA} and not ready_hariyama(obs):
        return -4200, f"keep Hariyama line {card_name(cid)}"
...
def score_to_hand(obs, opt) -> tuple[int, str]:
    ...
    if cid == HARIYAMA:
        return 8200 if has_in_play(obs, MAKUHITA) else 2500, "take Hariyama"
    if cid == MAKUHITA:
        return 6400 if count_in_play(obs, MAKUHITA) < 1 else 1200, "take Makuhita"
    if cid in ENERGIES:
    ...
    if cid in {FIGHTING_GONG, DUSK_BALL, POKE_PAD, LILLIE, CARMINE, WALLY, SWITCH, VS_SEEKER}:
...
def score_target(obs, opt) -> tuple[int, str]:
    ...
    if ctx in {SelectContext.SWITCH, SelectContext.TO_ACTIVE}:
        ...
        if cid in {RIOLU, SOLROCK, LUNATONE, HARIYAMA, MAKUHITA}:
    ...
    if ctx in {SelectContext.ATTACH_TO, SelectContext.ATTACH_FROM}:
        ...
        if cid in {HARIYAMA, MAKUHITA}:
            return 5400, "effect to Hariyama line"
