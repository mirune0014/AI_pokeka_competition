...
def _decision_signature(obs: Observation) -> tuple:
    ...
    if _certified_turn_plan_latch:
        # The stage is intentionally excluded.  Selecting an option may advance
        # the private stage before the engine advances the public observation;
        # keeping this signature purely public makes repeated callbacks return
        # the exact cached action instead of consuming a transaction twice.
...
def _turn_plan_common_is_same(
    ...
    *,
    ...
    retreated: bool,
    own_discard_fingerprint: tuple | None = None,
) -> bool:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    expected_discard = (
        ...
        if own_discard_fingerprint is None
        else own_discard_fingerprint
    )
    return (
        state.turn == latch.get("turn")
        ...
        and state.energyAttached == energy_attached
        and state.retreated == retreated
        and own_discard == expected_discard
...
def _turn_plan_hand_has_frozen_prefix(
    ...
    *,
    ...
    return (
        ...
        and all(row[1] not in {old[1] for old in frozen_cards}
                for row in new_rows)
    )
...
def _turn_plan_skill_is_yield_irrelevant(
    ...
    *,
    ...
    # and Rock Fighting Energy therefore block it even though their reminder
    # text correctly says ordinary damage is not an effect.
    if (
    ...
    if _bridge_retaliation_skill_is_certified_irrelevant(
        ...
        return True
    # Memory Dive changes the owner's available outgoing attack set; it does
    # not modify damage or effects received from the certified printed attack.
    if (
        ...
        return True
    # An opponent's Maximum Belt-style Tool only modifies that opponent's
    # outgoing attack.  It cannot alter our attack's yield on their Active.
    if (
        owner is not None
...
def _turn_plan_attack_certificate(
    ...
    *,
    ...
    if attack_id == ATTACK_SUPER_PSY_BOLT:
        ...
        amount = 30
        attack_type = int(attacker_data.energyType)
        if target_data.weakness is not None and int(target_data.weakness) == attack_type:
            amount *= 2
        if target_data.resistance is not None and int(target_data.resistance) == attack_type:
        ...
        if (
            ...
            return None
        # Damage counters are an effect, so Weakness and Resistance do not
        # change the exact amount.  Visible prevention was rejected above.
...
def _turn_plan_attack_for_pokemon(
    ...
    *,
    ...
    if units is None:
        return None
    if add_basic_psychic:
        units = tuple(units) + (int(EnergyType.PSYCHIC),)
...
def _start_buffer_ready_reserve(
    ...
    if (
        ...
        or parent_option.type != OptionType.ATTACH
        or parent_option.inPlayArea != AreaType.ACTIVE
        or parent_option.inPlayIndex != 0
        or active is None
        or active.id in ABRA_LINE
        ...
        return None
    energy = _turn_plan_selected_hand_card(obs, parent_option, my_index)
    if (
        energy is None
        or energy.id != Basic_Psychic_Energy
        or energy.serial <= 0
        or getattr(energy, "playerIndex", None) != my_index
        or sum(card.id == Basic_Psychic_Energy for card in mine.hand) != 1
    ):
        return None

    # Preserve a Basic-Psychic-as-retreat-payment line when an attacker is
    # already ready on the Bench.  Redirecting that attachment would erase an
    # exact current-turn retreat conversion (for example a Shaymin buffer).
    if (
        _bridge_retreat_cost_is_publicly_exact(state, active)
        and len(active.energies) + 1 >= int(card_table[active.id].retreatCost)
        and any(
            _turn_plan_attack_for_pokemon(
                state,
                pokemon,
                hand_count=mine.handCount - 1,
                target=target,
                player_index=my_index,
            )
            is not None
    ...
    for option_index, option in enumerate(select.option):
        if (
            option.type != OptionType.ATTACH
            or option.area != AreaType.HAND
            or option.index != parent_option.index
            or option.inPlayArea != AreaType.BENCH
            or option.playerIndex not in (None, my_index)
            ...
            continue
        pokemon = mine.bench[option.inPlayIndex]
        if (
            pokemon.id not in (Kadabra, Alakazam)
            or pokemon.energyCards
            or not _bridge_pokemon_is_publicly_complete(pokemon, my_index)
            or prize_count(pokemon) > prize_count(active)
        ):
            continue
        certificate = _turn_plan_attack_for_pokemon(
            ...
            target=target,
            player_index=my_index,
            add_basic_psychic=True,
        )
        if certificate is None or certificate["prize_yield"] <= 0:
            continue
        candidates.append(
            (
    ...
    if len(candidates) > 1 and best[:2] == candidates[1][:2]:
        return None
    _, _, _, option_index, destination, certificate = best
    common = _turn_plan_freeze_common(
        obs, excluded_own_serials={destination.serial}
    )
    hand_after = _turn_plan_hand_without_selected(
        mine, my_index, _bridge_card_fingerprint(energy)
    )
    if common is None or hand_after is None:
        return None
    protected = [
        *_bridge_pokemon_component_serials(active),
        *_bridge_pokemon_component_serials(destination),
    ...
    _certified_turn_plan_latch.update(
        common,
        stage="await_attach_effect",
        branch="BUFFER_READY_RESERVE",
        excluded_own_serials=(destination.serial,),
        selected_card_fingerprint=_bridge_card_fingerprint(energy),
        frozen_hand_after_action=hand_after,
        destination_serial=destination.serial,
        destination_area=int(AreaType.BENCH),
        destination_index=next(
            index
            for index, pokemon in enumerate(mine.bench)
            if pokemon.serial == destination.serial
        ),
        destination_static_fingerprint=_active_psychic_static_fingerprint(
            destination
        ),
        destination_energy_fingerprints=tuple(
            _bridge_card_fingerprint(card)
            for card in destination.energyCards
        ),
        destination_energy_units=tuple(int(unit) for unit in destination.energies),
        certificate_core=(
...
def _start_evolve_active_ready(
    ...
    for option_index, option in enumerate(select.option):
        if (
            ...
            active_options.append(option_index)
    ...
    parent_amount = 0 if parent_certificate is None else parent_certificate["amount"]
    if (
    ...
    # Do not steal an evolution from an already energized Bench line: that
    # would reduce the parent's visible ready-reserve count.
...
def _turn_plan_ready_destinations(
    state, mine, target: Pokemon, player_index: int
) -> list[tuple[int, Pokemon, dict]]:
    candidates = []
    for bench_index, pokemon in enumerate(mine.bench):
        if (
            pokemon.id != Alakazam
        ...
        if certificate is not None and certificate["ko"]:
            candidates.append((bench_index, pokemon, certificate))
    return candidates


def _start_retreat_convert_now(
    obs: Observation, parent_option
) -> list[int] | None:
    state = obs.current
    select = obs.select
    ...
    if (
        ...
        or state.retreated
        or parent_option is None
        or parent_option.type != OptionType.END
        or active is None
        or active.id != Alakazam
        or not _bridge_pokemon_is_publicly_complete(active, my_index)
        or any(card.id in PSYCHIC_ENERGY_IDS for card in active.energyCards)
        or len(active.energyCards) != 1
        or active.energyCards[0].id != Enriching_Energy
        or target is None
        or not _bridge_pokemon_is_publicly_complete(target, 1 - my_index)
        or any(option.type == OptionType.ATTACK for option in select.option)
        or not _bridge_retreat_cost_is_publicly_exact(state, active)
        or int(card_table[active.id].retreatCost) != 1
        or select.minCount != 1
        or select.maxCount != 1
    ):
        return None
    ...
    destinations = _turn_plan_ready_destinations(
        state, mine, target, my_index
    )
    if len(retreat_options) != 1 or len(destinations) != 1:
        return None
    destination_index, destination, certificate = destinations[0]
    target_prizes = prize_count(target)
    post_ko_prizes = len(mine.prize) - target_prizes
    if (
        target_prizes != 1
        or certificate["prize_yield"] != 1
        or post_ko_prizes < 0
        or (post_ko_prizes > 0 and mine.deckCount <= post_ko_prizes)
    ):
        return None
    source_rows = _stranded_retreat_source_energy_rows(active)
    if (
        source_rows is None
        or len(source_rows) != 1
        or source_rows[0][0]
        != _bridge_card_fingerprint(active.energyCards[0])
    ):
        return None
    common = _turn_plan_freeze_common(
        obs,
        excluded_own_serials={active.serial, destination.serial},
    )
    if common is None:
        return None
    payment_fingerprint = _bridge_card_fingerprint(active.energyCards[0])
    post_payment_discard = tuple(common["own_discard_fingerprint"]) + (
        payment_fingerprint,
    )
    protected = [
        *_bridge_pokemon_component_serials(active),
    ...
    _certified_turn_plan_latch.update(
        common,
        stage="await_payment",
        branch="RETREAT_CONVERT_NOW",
        excluded_own_serials=(active.serial, destination.serial),
        source_serial=active.serial,
        source_static_fingerprint=_active_psychic_static_fingerprint(active),
        source_energy_rows=source_rows,
        payment_serial=active.energyCards[0].serial,
        payment_fingerprint=payment_fingerprint,
        post_payment_discard_fingerprint=post_payment_discard,
        destination_serial=destination.serial,
        destination_start_index=destination_index,
        destination_fingerprint=_bridge_pokemon_fingerprint(destination),
        target_serial=target.serial,
        target_fingerprint=_bridge_target_fingerprint(target, theirs),
        target_prizes=target_prizes,
        post_ko_prizes=post_ko_prizes,
        expected_attack_id=ATTACK_POWERFUL_HAND,
        expected_amount=certificate["amount"],
        expected_prize_yield=certificate["prize_yield"],
    )
    return [retreat_options[0]]


def _start_certified_turn_plan(
    obs: Observation, parent_option
) -> list[int] | None:
    if (
        _certified_turn_plan_latch
    ...
    if parent_option is None:
        return None
    if parent_option.type == OptionType.ATTACH:
        return _start_buffer_ready_reserve(obs, parent_option)
    if parent_option.type == OptionType.EVOLVE:
        return _start_evolve_active_ready(obs, parent_option)
    if parent_option.type == OptionType.END:
        return _start_retreat_convert_now(obs, parent_option)
    return None


def _turn_plan_target_is_same(obs: Observation, latch: dict) -> bool:
    state = obs.current
    my_index = state.yourIndex
    theirs = state.players[1 - my_index]
    target = theirs.active[0] if theirs.active else None
    return (
        ...
        and _bridge_pokemon_is_publicly_complete(target, 1 - my_index)
        and _bridge_target_fingerprint(target, theirs)
        == latch.get("target_fingerprint")
    )


def _turn_plan_buffer_overlay(
    obs: Observation, latch: dict
) -> list[int] | None:
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    found = _bridge_find_pokemon(mine, latch.get("destination_serial"))
    if (
        latch.get("stage") != "await_attach_effect"
        ...
        or not _turn_plan_common_is_same(
            ...
            retreated=latch.get("start_retreated"),
        )
        or not _turn_plan_hand_has_frozen_prefix(
            mine,
            player_index=my_index,
            frozen_cards=latch.get("frozen_hand_after_action"),
            selected_fingerprint=latch.get("selected_card_fingerprint"),
            new_count=0,
        )
    ):
        _clear_certified_turn_plan_latch()
    ...
    expected_energy = tuple(
        latch.get("destination_energy_fingerprints") or ()
    ) + (latch.get("selected_card_fingerprint"),)
    current_energy = tuple(
        _bridge_card_fingerprint(card) for card in destination.energyCards
    )
    ...
    if (
        _active_psychic_static_fingerprint(destination)
        != latch.get("destination_static_fingerprint")
        or current_energy != expected_energy
        or tuple(int(unit) for unit in destination.energies)
        != tuple(latch.get("destination_energy_units") or ())
        + (int(EnergyType.PSYCHIC),)
...
def _turn_plan_evolution_overlay(
    ...
    if latch.get("stage") != "await_optional_draw":
        _clear_certified_turn_plan_latch()
        return None
    ...
    if select.context == SelectContext.ACTIVATE:
        ...
        if (
            source is None
            or source[0] != AreaType.ACTIVE
            or _bridge_pokemon_fingerprint(source[2])
            ...
            or not _turn_plan_common_is_same(
                ...
                hand_count=latch.get("start_hand_count") - 1,
                deck_count=latch.get("start_deck_count"),
                energy_attached=latch.get("start_energy_attached"),
                retreated=latch.get("start_retreated"),
            )
            or not _turn_plan_hand_has_frozen_prefix(
                mine,
    ...
    evolved = next(
        (
            (area, index, pokemon)
            for area, pokemon_list in (
                (AreaType.ACTIVE, mine.active),
                (AreaType.BENCH, mine.bench),
            )
            for index, pokemon in enumerate(pokemon_list)
            if pokemon is not None
    ...
    ) + ((latch.get("source_fingerprint")[0], latch.get("source_serial"), my_index),)
    if (
        pokemon.id != latch.get("expected_evolution_id")
        or not _bridge_pokemon_is_publicly_complete(pokemon, my_index)
        or tuple(
...
def _turn_plan_retreat_overlay(
    obs: Observation, latch: dict
) -> list[int] | None:
    state = obs.current
    select = obs.select
    ...
    if stage == "await_resolution":
        ...
        prize_delta = (
            len(mine.prize)
            == latch.get("own_prize_count")
            - latch.get("expected_prize_yield")
        )
        _clear_certified_turn_plan_latch()
        if target_left or prize_delta:
            return None
        return None
    ...
    if source is None or destination is None or not _turn_plan_target_is_same(obs, latch):
        _clear_certified_turn_plan_latch()
        return None

    if stage == "await_payment" and select.context == SelectContext.DISCARD_ENERGY:
        if (
            source[0] != AreaType.ACTIVE
            or destination[0] != AreaType.BENCH
            or _active_psychic_static_fingerprint(source[2])
            != latch.get("source_static_fingerprint")
            or _stranded_retreat_source_energy_rows(source[2])
            != latch.get("source_energy_rows")
            or _bridge_pokemon_fingerprint(destination[2])
            != latch.get("destination_fingerprint")
            or not _turn_plan_common_is_same(
                obs,
                latch,
                hand_count=latch.get("start_hand_count"),
                deck_count=latch.get("start_deck_count"),
                energy_attached=latch.get("start_energy_attached"),
                retreated=False,
            )
            or select.remainEnergyCost != 1
        ):
            _clear_certified_turn_plan_latch()
            return None
        matches = [
            ...
            and _bridge_retreat_energy_serial(obs, option)
            == latch.get("payment_serial")
        ]
        if len(matches) != 1 or select.minCount != 1 or select.maxCount != 1:
            _clear_certified_turn_plan_latch()
            return None
        latch["stage"] = "await_promotion"
        return [matches[0]]

    if stage == "await_promotion" and select.context in (
        ...
        if (
            ...
            or _active_psychic_static_fingerprint(source[2])
            != latch.get("source_static_fingerprint")
            or source[2].energyCards
            or source[2].energies
            or _bridge_pokemon_fingerprint(destination[2])
            != latch.get("destination_fingerprint")
            or not _turn_plan_common_is_same(
                obs,
                ...
                energy_attached=latch.get("start_energy_attached"),
                retreated=False,
                own_discard_fingerprint=latch.get(
                    "post_payment_discard_fingerprint"
                ),
            )
        ):
            _clear_certified_turn_plan_latch()
    ...
    if stage == "await_attack" and select.context == SelectContext.MAIN:
        target = theirs.active[0]
        if (
            source[0] != AreaType.BENCH
            or destination[0] != AreaType.ACTIVE
            or _active_psychic_static_fingerprint(source[2])
            != latch.get("source_static_fingerprint")
            or source[2].energyCards
            or source[2].energies
        ...
        if (
            ...
            or certificate["amount"] != latch.get("expected_amount")
            or certificate["prize_yield"]
            != latch.get("expected_prize_yield")
            or len(matches) != 1
            or select.minCount != 1
            or select.maxCount != 1
...
def _certified_turn_plan_overlay(
    ...
    if not _certified_turn_plan_latch:
        return None
    latch = _certified_turn_plan_latch
    branch = latch.get("branch")
    if branch == "BUFFER_READY_RESERVE":
        return _turn_plan_buffer_overlay(obs, latch)
    if branch == "EVOLVE_ACTIVE_READY":
        return _turn_plan_evolution_overlay(obs, latch)
    if branch == "RETREAT_CONVERT_NOW":
        return _turn_plan_retreat_overlay(obs, latch)
    _clear_certified_turn_plan_latch()
    return None
...
def agent(obs_dict: dict) -> list[int]:
    ...
    if transaction_action is not None:
        return _remember_action(decision_signature, transaction_action)
    # A completed or failed frozen plan delegates this entire observation to
    # exact v3.  It cannot immediately arm a second continuity transaction.
    turn_plan_delegated = (
        had_turn_plan_latch and not _certified_turn_plan_latch
    )
    ...
    # Exact v3 has now finalized its ordinary winner and received first claim
    # on every END state.  The isolated planner may replace only one finalized
    # ATTACH, EVOLVE, or otherwise-declined END under its public certificate.
    if (
        ...
        transaction_action = _start_certified_turn_plan(
