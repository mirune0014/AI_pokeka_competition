...
def load_manifest_dataset(
    ...
    *,
    ...
    if manifest_schema != MANIFEST_SCHEMA_VERSION:
        if manifest_schema in ("run-manifest-v1", "run-manifest-v2"):
            raise ValueError(
                "unsafe prior run manifest schema lacks unambiguous opponent identity"
            )
    ...
    if payload.get("dataset_sha256") != dataset_sha256(spec_hash, receipts):
    ...
    for receipt in receipts:
        ...
        if trajectory_schema != TRAJECTORY_SCHEMA_VERSION:
            if trajectory_schema in ("trajectory-v1", "trajectory-v2"):
                raise ValueError(
                    "unsafe prior trajectory schema lacks unambiguous opponent identity"
                )
    ...
    for episode in episodes:
        ...
        for row in episode.get("decisions") or ():
            ...
            if row.get("policy_schema_version") != POLICY_SCHEMA_VERSION:
                raise ValueError("PPO row policy schema mismatch")
            ...
            if (
                ...
                or int(select.get("option_count") or 0) < 2
            ):
            ...
            if len(q_latest) != len(actions) or any(float(q) <= 0 for q in q_latest):
            ...
            if not math.isclose(
                sum(float(q) for q in q_latest), 1.0, rel_tol=0.0, abs_tol=1e-6
            ):
            ...
            if len(semantic_rows) != len(actions) or len(effect_rows) != len(actions):
                raise ValueError("semantic/effect payload dimension mismatch")
            ...
            recomputed_distribution = ReferencePolicy().distribution(
                len(actions),
                int(teacher_action[0]),
                predicted_residuals,
            )
            if any(
    ...
    *,
    ...
    with torch.no_grad():
        for _, row in rows:
            ...
            logits = torch.log(q_latest) + 2.0 * torch.tanh(
                torch.clamp(residuals, -3.0, 3.0)
            )
...
def train(
    *,
    ...
    for epoch in range(cfg.epochs):
        ...
        optimizer.step()
        measured_kl = _mean_anchor_kl(model, rows, device=device)
