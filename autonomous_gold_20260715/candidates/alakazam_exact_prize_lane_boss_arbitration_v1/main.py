...
# ---------------------------------------------------------------------------
# EXACT_PRIZE_LANE_BOSS_ARBITRATION_V1
# The exact live parent above always runs first.  This is the only/last
# callable insertion, and it owns only the frozen Prize-lane transaction.
# ---------------------------------------------------------------------------

_exact_prize_lane_parent_agent = agent
_exact_prize_lane_boss_latch = {}
_exact_prize_lane_last_observation = None
_exact_prize_lane_last_action = None
_EXACT_PRIZE_LANE_VERSION = "exact_prize_lane_boss_arbitration_v1"


def _exact_prize_lane_clear(*, clear_cache: bool = False) -> None:
    global _exact_prize_lane_last_observation
    global _exact_prize_lane_last_action
...
def _exact_prize_lane_remember(signature: tuple, action: list[int]) -> list[int]:
    global _exact_prize_lane_last_observation
    global _exact_prize_lane_last_action
    _exact_prize_lane_last_observation = signature
    _exact_prize_lane_last_action = tuple(action)
    return list(action)
...
def _exact_prize_lane_field_rows(group, owner: int) -> tuple | None:
    if any(
        ...
        return None
    return tuple(_bridge_pokemon_fingerprint(pokemon) for pokemon in group)
...
def _exact_prize_lane_snapshot(obs: Observation) -> dict | None:
    ...
    if (
        ...
        return None
    (
        own_hand, own_discard, opponent_discard, stadium,
        own_prize, opponent_prize, own_active, own_bench,
        opponent_active, opponent_bench,
    ) = values
    return {
        "turn": state.turn,
...
def _exact_prize_lane_prize_value(pokemon: Pokemon, owner: int) -> int | None:
    ...
    for card in list(pokemon.energyCards) + list(pokemon.tools):
        metadata = card_table.get(card.id)
        if metadata is None:
            return None
        attached.append(metadata)
    # Decline every attached Prize modifier instead of guessing its value.
    if any(
        "prize" in _normalized_skill_text(skill.text)
        for metadata in (data, *attached)
        for skill in (metadata.skills or [])
    ):
        return None
    return base if prize_count(pokemon) == base else None


def _exact_prize_lane_attacker_is_ready(
    ...
    if (
        ...
        or not _bridge_pokemon_is_publicly_complete(attacker, obs.current.yourIndex)
        ...
        return False
    units = _bridge_retaliation_energy_units(attacker)
    return units is not None and _bridge_retaliation_can_pay(
        units, attack_table[ATTACK_POWERFUL_HAND].energies
    ) is True
...
def _exact_prize_lane_boss_options(obs: Observation) -> list[tuple[int, Card]] | None:
    ...
    if mine.hand is None or not _exact_prize_lane_boss_metadata_is_exact():
        return None
    hand_bosses = [card for card in mine.hand if card.id == Boss_Orders]
    options = []
    for option_index, option in enumerate(obs.select.option):
        if option.type != OptionType.PLAY:
            continue
        ...
        if (
            not _two_prize_option_is_exact(option, OptionType.PLAY, index=option.index)
            or card.serial <= 0
            or card.playerIndex != owner
    ...
    if (
        not hand_bosses
        or len(options) != len(hand_bosses)
        or {card.serial for _, card in options} != {card.serial for card in hand_bosses}
    ):
        return None
...
def _exact_prize_lane_target_lane(
    ...
    prizes = _exact_prize_lane_prize_value(target, 1 - obs.current.yourIndex)
    if (
        prizes is None
        or hand_count < 0
    ...
    return {
        ...
        "lethal": lethal,
        "terminal": lethal
        and prizes >= len(obs.current.players[obs.current.yourIndex].prize),
    }
...
def _exact_prize_lane_target_moves(target: Pokemon) -> tuple:
    return (
        ...
        *((card.id, card.serial, int(AreaType.PRE_EVOLUTION)) for card in target.preEvolution),
        *((card.id, card.serial, int(AreaType.ENERGY)) for card in target.energyCards),
        *((card.id, card.serial, int(AreaType.TOOL)) for card in target.tools),
    )


def _exact_prize_lane_start(
    raw: dict,
    obs: Observation,
    ...
    if (
        ...
        or not _exact_prize_lane_attacker_is_ready(obs, attacker)
        or current is None
        or not theirs.bench
        or not _two_prize_stadium_is_clear(state)
    ...
    terminals.extend(
        ("boss", lane, index)
        for index, lane in enumerate(boss_lanes)
        if lane["terminal"]
    )
    if len(terminals) > 1:
        return None
    if len(terminals) == 1:
        ...
        post_hand = tuple(
            row for row in snapshot["own_hand"] if row != boss_fingerprint
        )
        if (
            ...
            or not _bridge_protected_serials_are_unique(
                ...
                (
                    ...
                    *_bridge_pokemon_component_serials(current),
        ...
        _exact_prize_lane_boss_latch.update(
            ...
            current_target_serial=current.serial,
            current_target_fingerprint=_bridge_pokemon_fingerprint(current),
            target_index=target_index,
...
def _exact_prize_lane_target_action(obs: Observation, latch: dict) -> list[int] | None:
    state, select = obs.current, obs.select
    ...
    if (
        ...
        or not select.minCount == select.maxCount == 1
        or select.remainDamageCounter != 0
        or select.remainEnergyCost != 0
        or select.deck is not None
        or select.contextCard is not None
        or select.effect is None
        or _bridge_card_fingerprint(select.effect) != latch.get("boss_fingerprint")
        or len(obs.logs) != 1
    ...
    expected_options = tuple(
        (AreaType.BENCH, index, 1 - player) for index in range(len(theirs.bench))
    )
...
def _exact_prize_lane_attack_action(obs: Observation, latch: dict) -> list[int] | None:
    state, select = obs.current, obs.select
    owner = state.yourIndex
    mine, theirs = state.players[owner], state.players[1 - owner]
    attacker = mine.active[0] if len(mine.active) == 1 else None
    ...
    if (
        ...
        or _bridge_pokemon_fingerprint(attacker) != latch.get("attacker_fingerprint")
        or target is None
        or target.serial != latch.get("target_serial")
        or _bridge_pokemon_fingerprint(target) != latch.get("target_fingerprint")
        or _exact_prize_lane_prize_value(target, 1 - owner) != latch.get("target_prizes")
        or not _powerful_hand_target_is_publicly_clear(state, target)
...
def _exact_prize_lane_resolution_is_exact(obs: Observation, latch: dict) -> bool:
    state, select = obs.current, obs.select
    owner = state.yourIndex
    if owner not in (0, 1) or len(state.players) != 2:
        return False
    ...
    expected_taken = min(latch.get("target_prizes"), len(expected.get("own_prize")))
    if (
        state.turn != latch.get("turn")
        ...
        or _exact_prize_lane_card_rows(state.stadium, None) != expected.get("stadium")
        or int(select.type) != 1
...
def _exact_prize_lane_advance(obs: Observation) -> list[int] | None:
    latch = _exact_prize_lane_boss_latch
    if not latch:
        return None
    if (
        latch.get("version") != _EXACT_PRIZE_LANE_VERSION
        or obs.current.turn != latch.get("turn")
        or obs.current.yourIndex != latch.get("player")
    ):
        _exact_prize_lane_clear()
        return None
    stage = latch.get("stage")
    ...
    if stage == "await_attack":
        ...
        return action
    if stage == "await_resolution":
        _exact_prize_lane_resolution_is_exact(obs, latch)
        _exact_prize_lane_clear()
        return None
    _exact_prize_lane_clear()
    return None
...
def agent(obs_dict: dict) -> list[int]:
    """Run the exact live parent, then apply only exact Prize arbitration."""
    inherited_owner_at_entry = _two_prize_inherited_owner_active()
    had_transaction = bool(_exact_prize_lane_boss_latch)
    parent_action = _exact_prize_lane_parent_agent(obs_dict)
    try:
        signature = _two_prize_freeze_raw(obs_dict)
        if (
            _exact_prize_lane_last_observation == signature
            and _exact_prize_lane_last_action is not None
        ):
            return list(_exact_prize_lane_last_action)
        if not isinstance(obs_dict, dict) or obs_dict.get("select") is None:
            return _exact_prize_lane_remember(signature, parent_action)
        obs = to_observation_class(obs_dict)
        if (
            obs.current is None
            or obs.select is None
            or not _exact_prize_lane_action_is_valid(obs, parent_action)
            or not _two_prize_raw_parsed_agree(obs_dict, obs)
        ):
            _exact_prize_lane_clear()
            return _exact_prize_lane_remember(signature, parent_action)
        if had_transaction:
            if _two_prize_inherited_owner_active():
                _exact_prize_lane_clear()
                return _exact_prize_lane_remember(signature, parent_action)
            transaction_action = _exact_prize_lane_advance(obs)
            if (
                transaction_action is not None
                and _exact_prize_lane_action_is_valid(obs, transaction_action)
            ):
                return _exact_prize_lane_remember(signature, transaction_action)
            return _exact_prize_lane_remember(signature, parent_action)
        transaction_action = _exact_prize_lane_start(
            obs_dict,
            obs,
            parent_action,
            inherited_owner_at_entry,
