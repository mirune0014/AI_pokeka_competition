...
_ALAKAZAM_SURVIVAL_LILLIE_SCORE = 7000
RELICANTH = 57
...
def active_is_crustle_damage_blocker(obs):
    active = opp_active_pokemon(obs)
    return bool(active is not None and active.id == 345)
...
ALAKAZAM_LINE = {741, 742, ALAKAZAM}
...
def _powerful_hand_is_ready(obs):
    """Return True only for an exact, immediately attack-ready Alakazam."""
    opp = opp_state(obs)
    active = opp_active_pokemon(obs)
    if active is None or active.id != ALAKAZAM:
        return False
    card = CARD_DB.get(ALAKAZAM)
    attack = ALL_ATTACKS.get(POWERFUL_HAND)
    if card is None or attack is None:
        return False
    attacks = getattr(card, "attacks", None)
    if not isinstance(attacks, list) or tuple(attacks) != (POWERFUL_HAND,):
        return False
    if getattr(attack, "name", None) != "Powerful Hand":
        return False
    try:
        required_energy_types = tuple(int(value) for value in attack.energies)
        attached_energy_types = tuple(int(value) for value in active.energies)
    except (AttributeError, TypeError, ValueError):
        return False
    if required_energy_types != (PSYCHIC_ENERGY_TYPE,):
        return False
    if PSYCHIC_ENERGY_TYPE not in attached_energy_types:
        return False
    if any(
        getattr(opp, status, None) is not False
        for status in ("asleep", "paralyzed", "confused")
    ):
        return False
    deck_count = getattr(opp, "deckCount", None)
    return _is_strict_int(deck_count) and deck_count > 0


def should_prioritize_lillie_against_alakazam(obs):
    current = getattr(obs, "current", None)
    select = getattr(obs, "select", None)
    if current is None or select is None:
        return False
    if getattr(current, "result", None) != -1:
        return False
    if getattr(select, "context", None) != SelectContext.MAIN:
        return False
    if getattr(current, "supporterPlayed", None) is not False:
        return False
    mine = my_state(obs)
    opp = opp_state(obs)
    own_active = active_pokemon(obs)
    opp_active = opp_active_pokemon(obs)
    if mine is None or opp is None or own_active is None or opp_active is None:
        return False
    if own_active.id not in {CINDERACE, DURALUDON, ARCHALUDON, ARCHALUDON_EX}:
        return False
    own_bench = getattr(mine, "bench", None)
    opp_bench = getattr(opp, "bench", None)
    hand = getattr(mine, "hand", None)
    prize = getattr(mine, "prize", None)
    if not isinstance(own_bench, list) or own_bench:
        return False
    if not isinstance(opp_bench, list) or not any(p for p in opp_bench):
        return False
    hand_count = getattr(mine, "handCount", None)
    deck_count = getattr(mine, "deckCount", None)
    if not _is_strict_int(hand_count) or hand_count != len(hand or []):
        return False
    if not _is_strict_int(deck_count) or deck_count < 0:
        return False
    if LILLIE not in hand_ids(obs):
        return False
    remaining_prizes = len(prize or [])
    if remaining_prizes not in {4, 5, 6}:
        return False
    if not _powerful_hand_is_ready(obs):
        return False
    opponent_hand_count = getattr(opp, "handCount", None)
    if not _is_strict_int(opponent_hand_count) or opponent_hand_count < 0:
        return False
    own_hp = getattr(own_active, "hp", None)
    opponent_hp = getattr(opp_active, "hp", None)
    if not _is_strict_int(own_hp) or own_hp <= 0:
        return False
    if not _is_strict_int(opponent_hp) or opponent_hp <= 0:
        return False
    minimum_next_hand = opponent_hand_count + 1
    minimum_powerful_hand_counters = minimum_next_hand * 20
    floor, _ceiling, _ceiling_with_boss = _estimate_alakazam(obs)
    if floor != minimum_powerful_hand_counters:
        return False
    if floor < own_hp:
        return False
    attacks = planned_archaludon_attacks(obs)
    for planned in attacks:
        damage = planned.get("damage") if isinstance(planned, dict) else None
        if not _is_strict_int(damage):
            continue
        if effective_damage(damage, opp_active) >= opponent_hp:
            return False
    draw_count = 8 if remaining_prizes == 6 else 6
    available_after_play = deck_count + hand_count - 1
    return available_after_play >= draw_count
...
def apply_overrides(obs, opt, score, reason):
    ...
    # Crustle overrides
    crustle_active = active_is_crustle_damage_blocker(obs)
    card = option_card(obs, opt)
    cid = card.id if card else getattr(opt, 'cardId', None)
    ctx = obs.select.context
    ...
    if (
        crustle_active
        and opt.type == OptionType.EVOLVE
        and cid == ARCHALUDON_EX
    ):
        return -10000, "Crustle active: don't evolve to ex"
    ...
    if opt.type == OptionType.ATTACK:
        ...
        if crustle_active and aid == METAL_DEFENDER:
            return -5000, "Crustle active: Metal Defender does 0"
    ...
    if (
        crustle_active
        and ctx == SelectContext.TO_HAND
        and opt.type == OptionType.CARD
        and cid == ARCHALUDON_EX
    ):
        return -3000, "Crustle active: skip Archaludon ex"

    if ctx in {SelectContext.DISCARD, SelectContext.DISCARD_CARD_OR_ATTACHED_CARD}:
        if crustle_active and cid == ARCHALUDON_EX and score < 0:
            return 9000, "Crustle active: discard Archaludon ex"
...
def score_play(obs, opt):
    ...
    if cid == LILLIE:
        if obs.current.supporterPlayed:
            return -1000, "Supporter already used"
        if should_prioritize_lillie_against_alakazam(obs):
            return (
                _ALAKAZAM_SURVIVAL_LILLIE_SCORE,
                "Alakazam: lone Active floor KO, play Lillie",
            )
    ...
    if cid == BOSS:
        if obs.current.supporterPlayed:
            return -1000, "Supporter already used"
        if should_prioritize_lillie_against_alakazam(obs):
            return -2000, "Alakazam: suppress nonlethal Boss, preserve Lillie"
        if detect_matchup(obs) == "ogerpon":
            opp_act = opp_active_pokemon(obs)
            attacks = planned_archaludon_attacks(obs)
