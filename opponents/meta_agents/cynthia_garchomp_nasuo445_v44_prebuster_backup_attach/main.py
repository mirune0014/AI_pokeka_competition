...
def prebuster_backup_attach_index(obs, scored: list[tuple[int, int, str]]) -> int | None:
    """Find the best ordinary bench attachment before a nonterminal Buster KO."""
    if (
        obs.select.context != SelectContext.MAIN
        or getattr(active_pokemon(obs), "id", None) != GARCHOMP_EX
        or not scored
        ...
        return None
    ordinary_top = max(scored, key=lambda row: (row[0], -row[1]))
    buster = obs.select.option[ordinary_top[1]]
    target = opponent_active(obs)
    remaining_prizes = len(getattr(me(obs), "prize", []) or [])
    if (
        buster.type != OptionType.ATTACK
        or buster.attackId != DRACONIC_BUSTER
        or not is_approved_buster_conversion(obs)
        or not target
        or best_damage_for_active(obs, DRACONIC_BUSTER) < hp(target)
        or prize_value(target) >= remaining_prizes
        or opponent_has_no_bench(obs)
    ):
        return None
    attachments = [
        ...
        if obs.select.option[index].type == OptionType.ATTACH
        and getattr(option_card(obs, obs.select.option[index]), "id", None) in ENERGIES
        and obs.select.option[index].inPlayArea == AreaType.BENCH
        and getattr(option_target(obs, obs.select.option[index]), "id", None) in MAIN_LINE
    ]
    return max(attachments, key=lambda row: (row[0], -row[1]))[1] if attachments else None
    ...
    if development_index is not None:
        ...
        for row_index, (score, index, reason) in enumerate(scored):
            if index == development_index:
                ...
                break
    attachment_index = prebuster_backup_attach_index(obs, scored)
    if attachment_index is not None:
        buster_score = max(scored, key=lambda row: (row[0], -row[1]))[0]
        for row_index, (_score, index, reason) in enumerate(scored):
            if index == attachment_index:
                scored[row_index] = (buster_score + 1, index, f"{reason} before Draconic Buster")
                break
