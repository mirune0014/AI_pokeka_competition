...
def _preko_parent_agent(obs_dict: dict) -> list[int]:
...
# ---------------------------------------------------------------------------
# CERTIFIED_THREE_PRIZE_PRE_KO_SETUP_TRANSACTION_V2
#
# One outer public transaction may preserve a currently certified three-Prize
# Powerful Hand KO while completing one deterministic setup/recovery route.
# Every other exact parent choice is replaced by the immediate attack.
# ---------------------------------------------------------------------------

_preko_transaction_latch = {}
_preko_last_observation = None
_preko_last_action = None


def _preko_clear(*, clear_cache: bool = False) -> None:
    global _preko_last_observation, _preko_last_action
    _preko_transaction_latch.clear()
    if clear_cache:
        _preko_last_observation = None
        _preko_last_action = None


def _preko_remember(observation: dict, action: list[int]) -> list[int]:
    global _preko_last_observation, _preko_last_action
    _preko_last_observation = copy.deepcopy(observation)
    _preko_last_action = tuple(action)
    return list(action)


def _preko_snapshot_parent_state() -> dict:
    return {
        "merge": _merge_snapshot_complete_state(),
        "exposed": copy.deepcopy(_exposed_dudunsparce_transaction_latch),
        "exposed_last_observation": copy.deepcopy(
            _exposed_dudunsparce_last_observation
        ),
        "exposed_last_action": copy.deepcopy(
            _exposed_dudunsparce_last_action
        ),
    }


def _preko_restore_parent_state(snapshot: dict) -> None:
    global _exposed_dudunsparce_last_observation
    global _exposed_dudunsparce_last_action
    _merge_restore_complete_state(snapshot["merge"])
    _exposed_dudunsparce_transaction_latch.clear()
    _exposed_dudunsparce_transaction_latch.update(
        copy.deepcopy(snapshot["exposed"])
    )
    _exposed_dudunsparce_last_observation = copy.deepcopy(
        snapshot["exposed_last_observation"]
    )
    _exposed_dudunsparce_last_action = copy.deepcopy(
        snapshot["exposed_last_action"]
    )


def _preko_parent_owner(snapshot: dict) -> bool:
    return bool(snapshot.get("exposed")) or _exposed_dudunsparce_parent_owner(
        snapshot["merge"]
    )


def _preko_local_tool_hp_delta(tool: Card) -> int | None:
...
def _preko_max_hp_is_exact(state, pokemon: Pokemon) -> bool:
    data = card_table.get(pokemon.id)
    if data is None or data.hp <= 0:
        return False
    expected = data.hp
    for player in state.players:
        for source in list(player.active) + list(player.bench):
            ...
            if any(
                ...
                return False
            for tool in source.tools:
                delta = _preko_local_tool_hp_delta(tool)
                if delta is None:
                    return False
                if source.serial == pokemon.serial:
                    expected += delta
...
def _preko_metadata_certified() -> bool:
    expected = {
        Night_Stretcher: (
            "Night Stretcher",
            CardType.ITEM,
    ...
    if not (
        ...
        return False
    for card_id, (name, card_type, text) in expected.items():
        data = card_table.get(card_id)
        if (
            data is None
            or data.name != name
            or data.cardType != card_type
            or len(data.skills or []) != 1
            or data.skills[0].text != text
        ):
            return False
    for card_id in (Abra, Dunsparce):
        data = card_table.get(card_id)
...
def _preko_raw_active_is_exact(raw: dict, pokemon: Pokemon, owner: int) -> bool:
    ...
    return (
        ...
        and active[0].get("playerIndex") == owner
        and active[0].get("id") == pokemon.id
        and active[0].get("serial") == pokemon.serial
        and active[0].get("hp") == pokemon.hp
        and active[0].get("maxHp") == pokemon.maxHp
    )


def _preko_combat_certificate(
    ...
    if (
        ...
        or _enriching_zero_boss_lucario_latch
        or _exposed_dudunsparce_transaction_latch
        or not _preko_metadata_certified()
        or not _draw_survival_public_state_is_complete(obs)
        or len(mine.active) != 1
        or len(theirs.active) != 1
        or not _exposed_dudunsparce_status_is_clear(mine)
    ...
    if (
        active.id != Alakazam
        or not _preko_raw_active_is_exact(obs_dict, active, my_index)
        or not _preko_raw_active_is_exact(obs_dict, target, 1 - my_index)
        or not _draw_survival_known_stack_is_exact(active, my_index)
        or not _preko_max_hp_is_exact(state, active)
        or not _draw_survival_known_stack_is_exact(target, 1 - my_index)
        or not _preko_max_hp_is_exact(state, target)
        ...
        return None
    powerful = attack_table.get(ATTACK_POWERFUL_HAND)
    payment = _bridge_retaliation_energy_units(active)
    if (
        powerful is None
        or payment is None
        or _bridge_retaliation_can_pay(payment, powerful.energies) is not True
    ):
        return None
    target_commitment = _bridge_target_commitment_fingerprint(
        state, target, 1 - my_index
    )
    ...
    target_certificate = (
        _bridge_pokemon_fingerprint(target),
        target_commitment,
        _bridge_target_fingerprint(target, theirs),
    )
    return {
        "hand": hand,
        "hand_count": len(hand),
        "required": (target.hp + 19) // 20,
        "attacker": _bridge_pokemon_fingerprint(active),
        "payment": payment,
        "target": _bridge_pokemon_fingerprint(target),
        "target_certificate": target_certificate,
        "target_moves": _enriching_zero_boss_lucario_target_moves(target),
        "target_prizes": 3,
        "own_prizes": len(mine.prize),
        "visible_effects": _turn_plan_visible_effect_fingerprint(state),
    }


def _preko_base_certificate(obs: Observation, obs_dict: dict) -> dict | None:
    if obs.select is None or not _draw_free_exact_main_envelope(obs.select):
        return None
    certificate = _preko_combat_certificate(obs, obs_dict)
    if certificate is None or certificate["hand_count"] < certificate["required"]:
        return None
    attacks = []
    for option_index, option in enumerate(obs.select.option):
        if option.type != OptionType.ATTACK and option.attackId is None:
    ...
    if len(attacks) != 1:
        return None
    certificate["attack_action"] = [attacks[0]]
    return certificate


def _preko_route_environment_clear(state) -> bool:
    if _bridge_public_ability_lock_present(state):
        return False
    cannot = "can" + chr(39) + "t"
    for player in state.players:
        for pokemon in list(player.active) + list(player.bench):
            ...
            for card_id in sources:
                ...
                for skill in data.skills or []:
                    ...
                    if (
                        cannot + " draw" in text
                        or "cannot draw" in text
                        or ("prevent" in text and "draw" in text)
                    ):
                        return False
    for stadium in state.stadium:
        data = card_table.get(stadium.id)
...
def _preko_option_fingerprint(option) -> tuple:
    names = (
        "type",
        "number",
        "area",
    ...
    return tuple(
        (name, int(value) if hasattr(value, "__int__") else value)
        for name in names
        if (value := getattr(option, name, None)) is not None
    )


def _preko_exact_evolution_option(option, player: int) -> bool:
    return (
        option.type == OptionType.EVOLVE
        and option.area == AreaType.HAND
        and option.inPlayArea == AreaType.BENCH
        and option.playerIndex in (None, player)
        and isinstance(option.index, int)
        and not isinstance(option.index, bool)
        and isinstance(option.inPlayIndex, int)
        and not isinstance(option.inPlayIndex, bool)
        and _draw_survival_option_unused_fields_are_none(
            option,
            {
                "area",
...
def _preko_exact_play_option(option) -> bool:
    return (
        option.type == OptionType.PLAY
        and isinstance(option.index, int)
        and not isinstance(option.index, bool)
...
def _preko_selected_main_card(obs: Observation, action: list[int]):
    if (
        len(action) != 1
        or not 0 <= action[0] < len(obs.select.option)
        or obs.current.players[obs.current.yourIndex].hand is None
    ):
        return None, None
    option = obs.select.option[action[0]]
    if option.type not in (OptionType.PLAY, OptionType.EVOLVE):
        return option, None
    mine = obs.current.players[obs.current.yourIndex]
    if (
        ...
        return option, None
    return option, mine.hand[option.index]


def _preko_legal_recovery_card(card, player: int) -> bool:
    if not _draw_survival_card_is_exact(card, player):
        return False
    data = card_table.get(card.id)
    return data is not None and data.cardType in (
        CardType.POKEMON,
        CardType.BASIC_ENERGY,
    )


def _preko_legal_ash_card(card, player: int) -> bool:
    if not _draw_survival_card_is_exact(card, player):
        return False
    data = card_table.get(card.id)
...
def _preko_remove_one(rows: tuple, row: tuple) -> tuple | None:
    matches = [index for index, value in enumerate(rows) if value == row]
    if len(matches) != 1:
        return None
    index = matches[0]
    return rows[:index] + rows[index + 1 :]
...
def _preko_start_latch(
    ...
    **extra,
    ...
    state = obs.current
    _preko_transaction_latch.update(
        transaction="CERTIFIED_THREE_PRIZE_PRE_KO_SETUP_TRANSACTION_V2",
        route=route,
        turn=state.turn,
        player=state.yourIndex,
        start_action_count=state.turnActionCount,
        pre_hand=certificate["hand"],
        pre_hand_count=certificate["hand_count"],
        required=certificate["required"],
        pre_deck=state.players[state.yourIndex].deckCount,
        own_prizes=certificate["own_prizes"],
        attacker=certificate["attacker"],
        payment=certificate["payment"],
        target=certificate["target"],
        target_certificate=certificate["target_certificate"],
        target_moves=certificate["target_moves"],
        target_prizes=certificate["target_prizes"],
        start_snapshot=snapshot,
        route_action=_preko_option_fingerprint(option),
        selected_card=_bridge_card_fingerprint(selected_card),
        visible_effects=certificate["visible_effects"],
        **extra,
    )


def _preko_classify_route(
    obs: Observation, parent_action: list[int], certificate: dict
) -> str | None:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    option, card = _preko_selected_main_card(obs, parent_action)
    snapshot = _exposed_dudunsparce_public_snapshot(obs)
    if (
        option is None
        or card is None
        or snapshot is None
        or not _preko_route_environment_clear(state)
        or not _draw_survival_card_is_exact(card, my_index)
    ):
        return None
    hand_count = certificate["hand_count"]
    required = certificate["required"]
    deck_count = mine.deckCount

    if option.type == OptionType.EVOLVE:
        if (
            not _preko_exact_evolution_option(option, my_index)
            or not 0 <= option.inPlayIndex < len(mine.bench)
        ):
            return None
        source = mine.bench[option.inPlayIndex]
        if (
            source.appearThisTurn
            or not _draw_survival_known_stack_is_exact(source, my_index)
            or not _preko_max_hp_is_exact(state, source)
            or option.cardId not in (None, card.id)
            or option.serial not in (None, card.serial)
            or not _bridge_protected_serials_are_unique(
                state,
                (
                    card.serial,
            ...
            return None
        if card.id == Dudunsparce and source.id == Dunsparce:
            returned = 2 + len(source.energyCards) + len(source.tools)
            draws = min(3, deck_count)
            if (
                deck_count <= 0
                or
                hand_count - 1 + draws < required
                or deck_count - draws + returned < 1
            ):
                return None
            _preko_start_latch(
                obs,
                ...
                card,
                snapshot,
                stage="await_evolution_resolution",
                source_serial=source.serial,
                source_fingerprint=_bridge_pokemon_fingerprint(source),
                source_bench_index=option.inPlayIndex,
                draw_count=draws,
                return_count=returned,
                expected_post_draw_hand=hand_count - 1 + draws,
                expected_post_draw_deck=deck_count - draws + returned,
            )
            return "dudunsparce"
        if card.id == Kadabra and source.id == Abra:
            draws = min(2, deck_count)
            if (
                hand_count - 1 + draws < required
                or deck_count - draws < 1
            ):
                return None
            _preko_start_latch(
                obs,
                ...
                card,
                snapshot,
                stage="await_evolution_activate",
                source_serial=source.serial,
                source_fingerprint=_bridge_pokemon_fingerprint(source),
                source_bench_index=option.inPlayIndex,
                draw_count=draws,
                expected_post_draw_hand=hand_count - 1 + draws,
                expected_post_draw_deck=deck_count - draws,
            )
            return "kadabra"
        return None

    if option.type != OptionType.PLAY or not _preko_exact_play_option(option):
        return None
    if card.id == Night_Stretcher:
        if not any(
            _preko_legal_recovery_card(discarded, my_index)
        ...
        _preko_start_latch(
            ...
            card,
            snapshot,
            stage="await_stretcher_target",
        )
        return "night_stretcher"
    if card.id == Sacred_Ash:
        if hand_count - 1 < required or not any(
            _preko_legal_ash_card(discarded, my_index)
            for discarded in mine.discard
        ):
            return None
        _preko_start_latch(
            ...
            card,
            snapshot,
            stage="await_ash_selection",
        )
        return "sacred_ash"
    if card.id in (Abra, Dunsparce):
        data = card_table.get(card.id)
        if (
            hand_count - 1 < required
        ...
        _preko_start_latch(
            ...
            card,
            snapshot,
            stage="await_basic_resolution",
        )
        return "basic"
    return None


def _preko_parent_selected_boss(
    obs: Observation, parent_action: list[int]
) -> bool:
    option, card = _preko_selected_main_card(obs, parent_action)
    data = None if card is None else card_table.get(card.id)
    return bool(
        option is not None
        and card is not None
        and card.id == Boss_Orders
        and _preko_exact_play_option(option)
        and _draw_survival_card_is_exact(card, obs.current.yourIndex)
...
def _preko_combat_matches(obs: Observation, obs_dict: dict, latch: dict) -> bool:
    certificate = _preko_combat_certificate(obs, obs_dict)
    return bool(
        certificate is not None
        and obs.current.turn == latch.get("turn")
        and obs.current.yourIndex == latch.get("player")
        and certificate["required"] == latch.get("required")
        and certificate["attacker"] == latch.get("attacker")
        and certificate["payment"] == latch.get("payment")
        and certificate["target"] == latch.get("target")
        and certificate["target_certificate"]
        == latch.get("target_certificate")
        and certificate["target_moves"] == latch.get("target_moves")
        and certificate["target_prizes"] == latch.get("target_prizes")
        and certificate["own_prizes"] == latch.get("own_prizes")
    )


def _preko_evolved_source(obs: Observation, latch: dict, expected_id: int):
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    selected = latch.get("selected_card")
    before = latch.get("source_fingerprint")
    if selected is None or before is None:
        return None
    matches = [
        pokemon
        for pokemon in mine.bench
        if pokemon.id == expected_id
        and pokemon.serial == selected[1]
        and _draw_survival_known_stack_is_exact(pokemon, my_index)
        and _preko_max_hp_is_exact(state, pokemon)
    ]
    if len(matches) != 1:
        return None
    evolved = matches[0]
    old_damage = before[3] - before[2]
    expected_pre = tuple(before[9]) + (
        (before[0], before[1], my_index),
    )
    if (
        evolved.hp != evolved.maxHp - old_damage
        or not evolved.appearThisTurn
        or tuple(int(unit) for unit in evolved.energies) != before[6]
        or tuple(
            _bridge_card_fingerprint(card) for card in evolved.energyCards
        )
        != before[7]
        or tuple(_bridge_card_fingerprint(card) for card in evolved.tools)
        != before[8]
        or tuple(
            _bridge_card_fingerprint(card) for card in evolved.preEvolution
        )
        != expected_pre
    ):
        return None
...
def _preko_evolution_resolution_exact(
    obs: Observation, obs_dict: dict, latch: dict, expected_id: int
):
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    start = latch.get("start_snapshot")
    selected = latch.get("selected_card")
    carried = _preko_remove_one(
        latch.get("pre_hand") or (), selected
    )
    evolved = _preko_evolved_source(obs, latch, expected_id)
    if (
        snapshot is None
        or start is None
        or carried is None
        or evolved is None
        or not _preko_combat_matches(obs, obs_dict, latch)
        or not _preko_route_environment_clear(state)
        or (
            expected_id == Dudunsparce
            and not _draw_free_exact_main_envelope(obs.select)
        )
        or state.turnActionCount != latch.get("start_action_count") + 1
        or snapshot.get("own_hand") != carried
        or snapshot.get("own_deck") != latch.get("pre_deck")
        or not _preko_snapshot_delta(
            snapshot,
            start,
            {"action_count", "own_hand", "own_bench"},
        )
        or len(obs.logs) != 1
        or not _enriching_zero_boss_lucario_log_is_exact(
            obs.logs[0],
            12,
            {
                ...
                "cardId": expected_id,
                "serial": selected[1],
                "cardIdTarget": latch.get("source_fingerprint")[0],
                "serialTarget": latch.get("source_serial"),
            },
        )
    ):
        return None
    expected_bench = list(start.get("own_bench") or ())
    source_matches = [
        index
        for index, row in enumerate(expected_bench)
        if row[1] == latch.get("source_serial")
    ]
    if len(source_matches) != 1:
        return None
    expected_bench[source_matches[0]] = _bridge_pokemon_fingerprint(evolved)
    if snapshot.get("own_bench") != tuple(expected_bench):
        return None
    latch["carried_hand"] = carried
    latch["evolved_fingerprint"] = _bridge_pokemon_fingerprint(evolved)
    latch["evolved_snapshot"] = snapshot
    return evolved


def _preko_unique_run_away(obs: Observation, source_serial: int) -> list[int] | None:
    mine = obs.current.players[obs.current.yourIndex]
    matches = []
    for option_index, option in enumerate(obs.select.option):
        ...
        if (
            ...
            or option.playerIndex not in (None, obs.current.yourIndex)
            or not isinstance(option.index, int)
            or isinstance(option.index, bool)
            or not 0 <= option.index < len(mine.bench)
            or not _draw_survival_option_unused_fields_are_none(
            ...
            return None
        source = mine.bench[option.index]
        if (
            option.cardId not in (None, source.id)
            or option.serial not in (None, source.serial)
        ):
            return None
        if source.id == Dudunsparce and source.serial == source_serial:
            matches.append(option_index)
    return [matches[0]] if len(matches) == 1 else None


def _preko_unique_yes(obs: Observation, source_serial: int) -> list[int] | None:
    context = obs.select.contextCard
    yes_no = _draw_survival_yes_no_indices(obs.select)
    if (
        obs.select.type != SelectType.YES_NO
        or obs.select.context != SelectContext.ACTIVATE
        or obs.select.deck is not None
        or obs.select.effect is not None
        or context is None
        or context.id != Kadabra
        or context.serial != source_serial
        or not _draw_survival_card_is_exact(context, obs.current.yourIndex)
        or yes_no is None
    ):
        return None
    return [yes_no[0]]
...
def _preko_draw_logs_exact(
    obs: Observation, latch: dict, new_hand: tuple, *, with_return: bool
) -> bool:
    my_index = latch["player"]
    draw_logs = [log for log in obs.logs if int(getattr(log, "type", -1)) == 4]
    move_logs = [log for log in obs.logs if int(getattr(log, "type", -1)) == 6]
    other_logs = [
        log for log in obs.logs if int(getattr(log, "type", -1)) not in (4, 6)
    ]
    if len(draw_logs) != latch.get("draw_count"):
        return False
    drawn = []
    for log in draw_logs:
        row = (
        ...
        if not _enriching_zero_boss_lucario_log_is_exact(
            log,
            4,
            {"playerIndex": my_index, "cardId": row[0], "serial": row[1]},
        ):
            return False
        drawn.append(row)
    if len(set(drawn)) != len(drawn) or set(drawn) != set(new_hand):
        return False
    if not with_return:
        return not move_logs and not other_logs
    source_moves = latch.get("source_moves") or ()
    if (
        len(move_logs) != latch.get("return_count")
        or len(other_logs) != 1
        or not _enriching_zero_boss_lucario_log_is_exact(
            other_logs[0], 0, {"playerIndex": my_index}
        )
    ...
    for log in move_logs:
        ...
        if not _enriching_zero_boss_lucario_log_is_exact(log, 6, expected):
            return False
        actual.append(
            (expected["cardId"], expected["serial"], int(expected["fromArea"]))
        )
    return len(set(actual)) == len(actual) and set(actual) == set(source_moves)


def _preko_selection_option_card(
    obs: Observation, option, *, ash: bool
):
    mine = obs.current.players[obs.current.yourIndex]
    if (
        option.type != OptionType.CARD
        or option.area != AreaType.DISCARD
        or option.playerIndex != obs.current.yourIndex
        or not isinstance(option.index, int)
        or isinstance(option.index, bool)
        or not 0 <= option.index < len(mine.discard)
        or not _draw_survival_option_unused_fields_are_none(
            option, {"area", "index", "playerIndex"}
        )
    ):
        return None
    card = mine.discard[option.index]
    legal = (
        _preko_legal_ash_card(card, obs.current.yourIndex)
        if ash
        else _preko_legal_recovery_card(card, obs.current.yourIndex)
    )
    return card if legal else None


def _preko_item_prompt_snapshot_exact(
    obs: Observation, obs_dict: dict, latch: dict
) -> dict | None:
    snapshot = _exposed_dudunsparce_public_snapshot(obs)
    start = latch.get("start_snapshot")
    carried = _preko_remove_one(
        latch.get("pre_hand") or (), latch.get("selected_card")
    )
    if (
        snapshot is None
        or start is None
        or carried is None
        or not _preko_combat_matches(obs, obs_dict, latch)
        or not _preko_route_environment_clear(obs.current)
        or obs.current.turnActionCount != latch.get("start_action_count") + 1
        or snapshot.get("own_hand") != carried
        or snapshot.get("own_deck") != latch.get("pre_deck")
        or not _preko_snapshot_delta(
            snapshot, start, {"action_count", "own_hand"}
        )
        or len(obs.logs) != 1
        or not _enriching_zero_boss_lucario_log_is_exact(
            obs.logs[0],
            ...
            {
                "playerIndex": latch.get("player"),
                "cardId": latch.get("selected_card")[0],
                "serial": latch.get("selected_card")[1],
            },
        )
    ):
        return None
    latch["carried_hand"] = carried
    latch["prompt_snapshot"] = snapshot
    return snapshot


def _preko_effect_card_is_exact(obs: Observation, latch: dict) -> bool:
    effect = obs.select.effect
    return (
        effect is not None
        and _draw_survival_card_is_exact(effect, latch.get("player"))
        and _bridge_card_fingerprint(effect) == latch.get("selected_card")
    )


def _preko_force_attack(
    obs: Observation, obs_dict: dict, latch: dict, snapshot: dict
) -> list[int] | None:
    certificate = _preko_base_certificate(obs, obs_dict)
    if (
        certificate is None
        or not _preko_combat_matches(obs, obs_dict, latch)
        or certificate["hand_count"] < latch.get("required")
    ):
        return None
    latch["pre_attack_snapshot"] = snapshot
    latch["attack_hand_count"] = certificate["hand_count"]
    latch["attack_damage"] = 20 * certificate["hand_count"]
    latch["attack_action_count"] = obs.current.turnActionCount
    latch["stage"] = "await_attack_resolution"
    return certificate["attack_action"]


def _preko_advance_dudunsparce(
    obs: Observation, obs_dict: dict, latch: dict
) -> tuple[str, list[int] | None]:
    stage = latch.get("stage")
    if stage == "await_evolution_resolution":
        evolved = _preko_evolution_resolution_exact(
            obs, obs_dict, latch, Dudunsparce
        )
        if evolved is None:
            return "fail", None
        source_moves = (
            (evolved.id, evolved.serial, int(AreaType.BENCH)),
            *(
                (card.id, card.serial, int(AreaType.PRE_EVOLUTION))
                for card in evolved.preEvolution
            ),
            *(
                (card.id, card.serial, int(AreaType.ENERGY))
                for card in evolved.energyCards
            ),
            *(
                (card.id, card.serial, int(AreaType.TOOL))
                for card in evolved.tools
            ),
        )
        if len(source_moves) != latch.get("return_count"):
            return "fail", None
        action = _preko_unique_run_away(obs, evolved.serial)
        if action is None:
            return "fail", None
        latch["source_moves"] = source_moves
        latch["source_component_serials"] = tuple(
            row[1] for row in source_moves
        )
        latch["stage"] = "await_run_away_main"
        latch["run_away_action"] = _preko_option_fingerprint(
            obs.select.option[action[0]]
        )
        latch["stage"] = "await_run_away_resolution"
        return "force", action

    if stage != "await_run_away_resolution":
        return "fail", None
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    snapshot = _exposed_dudunsparce_public_snapshot(obs)
    evolved_snapshot = latch.get("evolved_snapshot")
    hand = None if snapshot is None else snapshot.get("own_hand")
    carried = tuple(latch.get("carried_hand") or ())
    carried_set = set(carried)
    if hand is None or len(carried_set) != len(carried):
        return "fail", None
    new_hand = tuple(row for row in hand if row not in carried_set)
    expected_bench = []
    if evolved_snapshot is not None:
        expected_bench = [
            row
            for row in evolved_snapshot.get("own_bench") or ()
            if row[1] != latch.get("selected_card")[1]
        ]
    if (
        snapshot is None
        or evolved_snapshot is None
        or not _preko_combat_matches(obs, obs_dict, latch)
        or not _preko_route_environment_clear(state)
        or not _draw_free_exact_main_envelope(obs.select)
        or state.turnActionCount != latch.get("start_action_count") + 2
        or len(hand) != latch.get("expected_post_draw_hand")
        or not carried_set <= set(hand)
        or len(new_hand) != latch.get("draw_count")
        or snapshot.get("own_deck") != latch.get("expected_post_draw_deck")
        or snapshot.get("own_bench") != tuple(expected_bench)
        or not _preko_snapshot_delta(
            snapshot,
            evolved_snapshot,
            {"action_count", "own_hand", "own_deck", "own_bench"},
        )
        or any(
            serial in _bridge_public_serials(state)
            for serial in latch.get("source_component_serials") or ()
        )
        or not _preko_draw_logs_exact(
            obs, latch, new_hand, with_return=True
        )
    ):
        return "fail", None
    action = _preko_force_attack(obs, obs_dict, latch, snapshot)
    return ("force", action) if action is not None else ("fail", None)


def _preko_advance_kadabra(
    obs: Observation, obs_dict: dict, latch: dict
) -> tuple[str, list[int] | None]:
    stage = latch.get("stage")
    if stage == "await_evolution_activate":
        evolved = _preko_evolution_resolution_exact(
            obs, obs_dict, latch, Kadabra
        )
        if evolved is None:
            return "fail", None
        action = _preko_unique_yes(obs, evolved.serial)
        if action is None:
            return "fail", None
        latch["activate_action"] = _preko_option_fingerprint(
            obs.select.option[action[0]]
        )
        latch["stage"] = "await_psychic_draw_resolution"
        return "force", action
    if stage != "await_psychic_draw_resolution":
        return "fail", None
    state = obs.current
    snapshot = _exposed_dudunsparce_public_snapshot(obs)
    evolved_snapshot = latch.get("evolved_snapshot")
    ...
    if (
        ...
        or not _preko_snapshot_delta(
            snapshot,
            evolved_snapshot,
            {"action_count", "own_hand", "own_deck"},
        )
        or not _preko_draw_logs_exact(
            obs, latch, new_hand, with_return=False
        )
    ):
        return "fail", None
    action = _preko_force_attack(obs, obs_dict, latch, snapshot)
...
def _preko_advance_stretcher(
    ...
    stage = latch.get("stage")
    if stage == "await_stretcher_target":
        prompt = _preko_item_prompt_snapshot_exact(obs, obs_dict, latch)
        if (
            prompt is None
            or obs.select.type != SelectType.CARD
            ...
            or not _preko_effect_card_is_exact(obs, latch)
            or len(parent_action) != 1
            or not 0 <= parent_action[0] < len(obs.select.option)
        ):
            return "fail", None
        cards = [
            _preko_selection_option_card(obs, option, ash=False)
            for option in obs.select.option
        ]
        if any(card is None for card in cards):
            return "fail", None
        rows = [_bridge_card_fingerprint(card) for card in cards]
        if len(set(rows)) != len(rows):
            return "fail", None
        selected = cards[parent_action[0]]
        latch["recovered_card"] = _bridge_card_fingerprint(selected)
        latch["selection_action"] = tuple(parent_action)
        latch["stage"] = "await_stretcher_resolution"
        return "pass", parent_action
    if stage != "await_stretcher_resolution":
        return "fail", None
    state = obs.current
    my_index = state.yourIndex
    snapshot = _exposed_dudunsparce_public_snapshot(obs)
    prompt = latch.get("prompt_snapshot")
    recovered = latch.get("recovered_card")
    carried = tuple(latch.get("carried_hand") or ())
    expected_hand = carried + (recovered,)
    before_discard = tuple(latch.get("start_snapshot", {}).get("own_discard") or ())
    without = _preko_remove_one(before_discard, recovered)
    expected_discard = None if without is None else without + (
        latch.get("selected_card"),
    )
    if (
        snapshot is None
        or prompt is None
        or expected_discard is None
        or not _preko_combat_matches(obs, obs_dict, latch)
        or not _preko_route_environment_clear(state)
        or not _draw_free_exact_main_envelope(obs.select)
        or state.turnActionCount != latch.get("start_action_count") + 2
        or snapshot.get("own_deck") != latch.get("pre_deck")
        or set(snapshot.get("own_hand") or ()) != set(expected_hand)
        or len(snapshot.get("own_hand") or ()) != len(expected_hand)
        or snapshot.get("own_discard") != expected_discard
        or not _preko_snapshot_delta(
            snapshot,
            prompt,
            {"action_count", "own_hand", "own_discard"},
        )
        or len(obs.logs) != 1
        or not _enriching_zero_boss_lucario_log_is_exact(
            obs.logs[0],
...
def _preko_advance_ash(
    ...
    stage = latch.get("stage")
    if stage == "await_ash_selection":
        prompt = _preko_item_prompt_snapshot_exact(obs, obs_dict, latch)
        if (
            prompt is None
            or obs.select.type != SelectType.CARD
            or obs.select.context != SelectContext.TO_DECK
            or not 1 <= obs.select.minCount <= obs.select.maxCount <= 5
            or obs.select.deck is not None
            or obs.select.contextCard is not None
            or not _preko_effect_card_is_exact(obs, latch)
            or not _exposed_dudunsparce_action_is_valid(obs, parent_action)
            ...
            return "fail", None
        cards = [
            _preko_selection_option_card(obs, option, ash=True)
            for option in obs.select.option
        ]
        if any(card is None for card in cards):
            return "fail", None
        ...
        if len(set(rows)) != len(rows):
            return "fail", None
        selected = tuple(rows[index] for index in parent_action)
        if len(set(selected)) != len(selected):
            return "fail", None
        latch["ash_cards"] = selected
        latch["selection_action"] = tuple(parent_action)
        latch["stage"] = "await_ash_resolution"
        return "pass", parent_action
    if stage != "await_ash_resolution":
        return "fail", None
    state = obs.current
    my_index = state.yourIndex
    snapshot = _exposed_dudunsparce_public_snapshot(obs)
    prompt = latch.get("prompt_snapshot")
    selected = tuple(latch.get("ash_cards") or ())
    expected_discard = tuple(
        latch.get("start_snapshot", {}).get("own_discard") or ()
    )
    for row in selected:
        reduced = _preko_remove_one(expected_discard, row)
        if reduced is None:
            return "fail", None
        expected_discard = reduced
    expected_discard += (latch.get("selected_card"),)
    logs = list(obs.logs)
    move_logs = [log for log in logs if int(getattr(log, "type", -1)) == 6]
    other_logs = [log for log in logs if int(getattr(log, "type", -1)) != 6]
    actual = []
    for log in move_logs:
        expected = {
            "playerIndex": my_index,
        ...
        if not _enriching_zero_boss_lucario_log_is_exact(log, 6, expected):
            return "fail", None
        actual.append((expected["cardId"], expected["serial"], my_index))
    if (
        snapshot is None
        or prompt is None
        or not selected
        ...
        or not _draw_free_exact_main_envelope(obs.select)
        or state.turnActionCount != latch.get("start_action_count") + 2
        or snapshot.get("own_hand") != latch.get("carried_hand")
        or snapshot.get("own_deck") != latch.get("pre_deck") + len(selected)
        or snapshot.get("own_discard") != expected_discard
        or not _preko_snapshot_delta(
            snapshot,
            prompt,
            {"action_count", "own_deck", "own_discard"},
        )
        or len(move_logs) != len(selected)
        or len(set(actual)) != len(actual)
        or set(actual) != set(selected)
        or len(other_logs) != 1
        or not _enriching_zero_boss_lucario_log_is_exact(
            other_logs[0], 0, {"playerIndex": my_index}
        )
...
def _preko_advance_basic(
    obs: Observation, obs_dict: dict, latch: dict
) -> tuple[str, list[int] | None]:
    if latch.get("stage") != "await_basic_resolution":
        return "fail", None
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    start = latch.get("start_snapshot")
    selected = latch.get("selected_card")
    carried = _preko_remove_one(latch.get("pre_hand") or (), selected)
    matches = [
        pokemon
        for pokemon in mine.bench
        if pokemon.id == selected[0]
        and pokemon.serial == selected[1]
        and pokemon.appearThisTurn
        and not pokemon.preEvolution
        and not pokemon.energyCards
        and not pokemon.tools
        and not pokemon.energies
        and pokemon.hp == card_table[pokemon.id].hp
        and pokemon.maxHp == card_table[pokemon.id].hp
    ]
    if (
        snapshot is None
        or start is None
        ...
        or snapshot.get("own_hand") != carried
        or snapshot.get("own_deck") != latch.get("pre_deck")
        or snapshot.get("own_bench")
        != tuple(start.get("own_bench") or ())
        + (_bridge_pokemon_fingerprint(matches[0]),)
        or not _preko_snapshot_delta(
            snapshot,
            start,
            {"action_count", "own_hand", "own_bench"},
...
def _preko_attack_resolution_exact(obs: Observation, latch: dict) -> bool:
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    before = latch.get("pre_attack_snapshot")
    target_moves = tuple(latch.get("target_moves") or ())
    if (
        before is None
        or state.turn != latch.get("turn")
        or state.turnActionCount != latch.get("attack_action_count") + 1
        or state.result != -1
        or state.looking is not None
        or select.type != SelectType.CARD
        or select.context != SelectContext.TO_HAND
        ...
        or select.contextCard is not None
        or select.effect is not None
        or _draw_survival_exact_hand(mine, my_index) != before.get("own_hand")
        or mine.deckCount != before.get("own_deck")
        or tuple(_bridge_pokemon_fingerprint(p) for p in mine.active)
        != before.get("own_active")
        or tuple(_bridge_pokemon_fingerprint(p) for p in mine.bench)
        != before.get("own_bench")
        or _enriching_zero_boss_lucario_ordered_cards(mine.discard, my_index)
        != before.get("own_discard")
        or _draw_free_prize_rows(mine) != before.get("own_prize")
        or len(theirs.active) != 0
        or tuple(_bridge_pokemon_fingerprint(p) for p in theirs.bench)
        != before.get("opponent_bench")
        or theirs.deckCount != before.get("opponent_deck")
        or theirs.handCount != before.get("opponent_hand_count")
        or _draw_free_prize_rows(theirs) != before.get("opponent_prize")
        or _draw_free_status(mine) != before.get("own_status")
        or _draw_free_status(theirs) != before.get("opponent_status")
        or _enriching_zero_boss_lucario_ordered_cards(state.stadium, None)
        != before.get("stadium")
    ):
        return False
    before_discard = tuple(before.get("opponent_discard") or ())
    target_rows = tuple(
        (card_id, serial, 1 - my_index)
    ...
    if not _enriching_zero_boss_lucario_log_is_exact(
        ...
        16,
        {
            "playerIndex": 1 - my_index,
            "cardId": target_moves[0][0],
            "serial": latch.get("target")[1],
            "value": -latch.get("attack_damage"),
            "putDamageCounter": True,
        },
    ):
        return False
...
def _preko_advance(
    ...
    latch = _preko_transaction_latch
    if latch.get("stage") == "await_attack_resolution":
        if not _preko_attack_resolution_exact(obs, latch):
            return "fail", None
        _preko_clear()
        return "resolved", parent_action
    route = latch.get("route")
    if route == "dudunsparce":
        return _preko_advance_dudunsparce(obs, obs_dict, latch)
    if route == "kadabra":
        return _preko_advance_kadabra(obs, obs_dict, latch)
    if route == "night_stretcher":
        return _preko_advance_stretcher(
            obs, obs_dict, latch, parent_action
        )
    if route == "sacred_ash":
        return _preko_advance_ash(obs, obs_dict, latch, parent_action)
    if route == "basic":
        return _preko_advance_basic(obs, obs_dict, latch)
    return "fail", None


def _preko_prepare(obs_dict: dict) -> bool:
    if not _preko_transaction_latch:
        return False
    current = obs_dict.get("current") if isinstance(obs_dict, dict) else None
    select = obs_dict.get("select") if isinstance(obs_dict, dict) else None
    if not isinstance(current, dict) or not isinstance(select, dict):
        _preko_clear(clear_cache=True)
        return True
    if (
        current.get("turn") != _preko_transaction_latch.get("turn")
        or current.get("yourIndex")
        != _preko_transaction_latch.get("player")
        or current.get("result") != -1
    ):
        _preko_clear(clear_cache=True)
        return True
...
def _preko_rerun_parent(
    ...
    if (
        ...
        or any(
            not isinstance(index, int)
            or isinstance(index, bool)
            or not 0 <= index < len(raw_options)
            for index in desired_action
        )
        or raw_select.get("minCount") > len(desired_action)
        or raw_select.get("maxCount") < len(desired_action)
    ):
        return None
    reranked = copy.deepcopy(obs_dict)
    reranked["select"]["option"] = [
    ...
    _preko_restore_parent_state(before_parent)
    _merge_push_start_quarantine()
    try:
        action = _preko_parent_agent(reranked)
    finally:
        _merge_pop_start_quarantine()
    if (
        not isinstance(action, list)
        or len(action) != len(set(action))
        or not raw_select.get("minCount") <= len(action) <= raw_select.get("maxCount")
        or any(
            not isinstance(index, int)
            or isinstance(index, bool)
            or not 0 <= index < len(desired_action)
            for index in action
        )
    ):
        return None
    mapped = [desired_action[index] for index in action]
    return mapped if mapped == desired_action else None


def _preko_genuine_parent(
    obs_dict: dict, before_parent: dict
) -> list[int]:
    _preko_clear(clear_cache=True)
    _preko_restore_parent_state(before_parent)
    return _preko_parent_agent(copy.deepcopy(obs_dict))
...
def _preko_start(
    ...
    if (
        _preko_parent_owner(before_parent)
        or _preko_parent_owner(after_parent)
        or before_parent["merge"].get("merge_start_quarantine_depth") != 0
        or after_parent["merge"].get("merge_start_quarantine_depth") != 0
    ):
        return "identity", parent_action
    certificate = _preko_base_certificate(obs, obs_dict)
    if certificate is None:
        return "identity", parent_action
    attack = certificate["attack_action"]
    if (
        certificate["target_prizes"] >= certificate["own_prizes"]
        or _preko_parent_selected_boss(obs, parent_action)
    ):
        return "force", attack
    route = _preko_classify_route(obs, parent_action, certificate)
    if route is not None:
        return "route", parent_action
    return "force", attack


def agent(obs_dict: dict) -> list[int]:
    """Apply one certified setup/recovery route, then lock Powerful Hand."""
    global _preko_last_observation, _preko_last_action
    if (
        _preko_last_observation is not None
        and obs_dict == _preko_last_observation
        and _preko_last_action is not None
    ):
        return list(_preko_last_action)

    _preko_last_observation = None
    _preko_last_action = None
    stale_cleared = _preko_prepare(obs_dict)
    had_transaction = bool(_preko_transaction_latch)
    before_parent = _preko_snapshot_parent_state()
    if stale_cleared:
        fallback = _preko_genuine_parent(obs_dict, before_parent)
        return _preko_remember(obs_dict, fallback)

    block_lower_start = bool(had_transaction or _merge_start_quarantine_depth)
    if block_lower_start:
        _merge_push_start_quarantine()
    try:
        parent_action = _preko_parent_agent(copy.deepcopy(obs_dict))
    finally:
        if block_lower_start:
            _merge_pop_start_quarantine()
    after_parent = _preko_snapshot_parent_state()
    if not isinstance(obs_dict, dict) or obs_dict.get("select") is None:
        if had_transaction:
            fallback = _preko_genuine_parent(obs_dict, before_parent)
            return _preko_remember(obs_dict, fallback)
        return _preko_remember(obs_dict, parent_action)
    ...
    try:
        ...
        if had_transaction:
            if (
                ...
                fallback = _preko_genuine_parent(obs_dict, before_parent)
                return _preko_remember(obs_dict, fallback)
            disposition, desired = _preko_advance(
                obs, obs_dict, parent_action
            )
            if disposition == "fail" or desired is None:
                fallback = _preko_genuine_parent(obs_dict, before_parent)
                return _preko_remember(obs_dict, fallback)
            if disposition in ("pass", "resolved"):
                return _preko_remember(obs_dict, parent_action)
            if not _exposed_dudunsparce_action_is_valid(obs, desired):
                fallback = _preko_genuine_parent(obs_dict, before_parent)
                return _preko_remember(obs_dict, fallback)
            if desired != parent_action:
                replacement = _preko_rerun_parent(
                    obs_dict, before_parent, desired
                )
                if replacement != desired:
                    fallback = _preko_genuine_parent(
                        obs_dict, before_parent
                    )
                    return _preko_remember(obs_dict, fallback)
            return _preko_remember(obs_dict, desired)

        disposition, desired = _preko_start(
            obs, obs_dict, parent_action, before_parent, after_parent
        )
        if disposition in ("identity", "route") or desired is None:
            return _preko_remember(obs_dict, parent_action)
        if not _exposed_dudunsparce_action_is_valid(obs, desired):
            return _preko_remember(obs_dict, parent_action)
        if desired != parent_action:
            ...
            if replacement != desired:
                _preko_restore_parent_state(after_parent)
                _preko_clear()
                return _preko_remember(obs_dict, parent_action)
        return _preko_remember(obs_dict, desired)
    except (
