...
from archaludon_rl.train_ppo import PPOConfig, load_manifest_dataset, train
...
class TrajectoryModelTrainingTests(unittest.TestCase):
    ...
    def _make_run(
        ...
        *,
        ...
        behavior_receipt = dict(checkpoint_meta["behavior_policy_receipt"])
        behavior_hash = str(
            checkpoint_meta["behavior_policy_schema_sha256"]
        )
    ...
    def _refresh_manifest_episode(
        ...
        refreshed = replace(
            ...
            dataset_sha256=dataset_sha256(
                ...
                behavior_policy_receipt=manifest.behavior_policy_receipt,
                behavior_policy_schema_sha256=(
                    manifest.behavior_policy_schema_sha256
                ),
    ...
    def _refresh_manifest_contract_hashes(
        ...
        spec_hash = collection_spec_sha256(
            ...
            behavior_policy_receipt=dict(payload["behavior_policy_receipt"]),
            behavior_policy_schema_sha256=str(
                payload["behavior_policy_schema_sha256"]
            ),
        ...
        spec_hash = json_sha256(
            {
                ...
                "behavior_policy_receipt": dict(
                    payload["behavior_policy_receipt"]
                ),
                "behavior_policy_schema_sha256": str(
                    payload["behavior_policy_schema_sha256"]
                ),
        ...
        payload["dataset_sha256"] = json_sha256(
            {
                ...
                "behavior_policy_schema_version": payload[
    ...
    def test_terminal_reward_attaches_to_last_eligible_not_protected(self):
        ...
        builder = EpisodeBuilder(
            ...
            behavior_policy_receipt=canonical_behavior_policy_receipt(),
    ...
    def test_checkpoint_zero_residual_roundtrip(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                loaded_metadata["behavior_policy_receipt"],
                canonical_behavior_policy_receipt(),
            )
            self.assertEqual(
                loaded_metadata["behavior_policy_schema_sha256"],
                behavior_policy_sha256(),
            )
            with self.assertRaises(ValueError):
                load_checkpoint(path, expected_source_hashes={"main": "wrong"})

    def test_checkpoint_serialization_is_byte_identical_across_destinations(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            first_path = root / "first-name.pt"
            second_path = root / "nested" / "different-name.pt"
            model = ResidualActorCritic()
            optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
            residuals, value = model(
                torch.zeros(STATE_DIM),
                torch.zeros((2, ACTION_DIM)),
            )
            (residuals.sum() + value).backward()
            optimizer.step()
            metadata = checkpoint_metadata(
                source_hashes={"main": "hash"},
                training={"determinism_fixture": True},
            )
            first_hash = save_checkpoint(
                first_path,
                model,
                metadata,
                optimizer=optimizer,
            )
            second_hash = save_checkpoint(
                second_path,
                model,
                metadata,
                optimizer=optimizer,
            ...
            for path in (first_path, second_path):
                ...
                for name, value_ in model.state_dict().items():
                    self.assertTrue(
                        torch.equal(value_, loaded.state_dict()[name])
                    )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for label, mutate, expected in (
                ...
                (
                    "missing-behavior",
                    lambda metadata: (
                        metadata.pop("behavior_policy_receipt"),
                        metadata.pop("behavior_policy_schema_sha256"),
                    ),
                    "behavior-policy receipt missing",
                ),
                (
                    "tampered-behavior-temperature",
                    lambda metadata: (
                        metadata["behavior_policy_receipt"].update(
                            {"temperature": 0.66}
                        ),
                        metadata.update(
                            {
                                "behavior_policy_schema_sha256": (
                                    behavior_policy_sha256(
                                        metadata["behavior_policy_receipt"]
                                    )
                                )
                            }
                        ),
                    ),
                    "canonical formula/configuration",
                ),
            ):
                with self.subTest(label=label):
                    metadata = checkpoint_metadata(
    ...
    def test_duplicate_audit_replica_receipts_are_reproducible_and_tamper_evident(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                manifest["behavior_policy_receipt"],
                episode["behavior_policy_receipt"],
            )
            self.assertEqual(
                manifest["behavior_policy_schema_sha256"],
                episode["behavior_policy_schema_sha256"],
            )
            for receipt in replica_receipts:
                audit_path = fixture["manifest_path"].parent.joinpath(
                    *Path(receipt["path"]).parts
                )
                replica = json.loads(audit_path.read_text(encoding="utf-8"))
                self.assertEqual(
                    replica["behavior_policy_receipt"],
                    manifest["behavior_policy_receipt"],
                )
                self.assertEqual(
                    replica["behavior_policy_schema_sha256"],
                    manifest["behavior_policy_schema_sha256"],
                )
    ...
    def test_manifest_loader_rejects_shared_fake_behavior_receipt_across_retained_and_ab(self):
        with tempfile.TemporaryDirectory() as temporary:
            fixture = self._make_run(
                Path(temporary),
                run_id="duplicate-shared-fake-behavior",
                duplicate_mode=True,
            )
            retained = json.loads(
                fixture["episode_path"].read_text(encoding="utf-8")
            )
            fake_receipt = deepcopy(retained["behavior_policy_receipt"])
            fake_receipt["temperature"] = 0.66
            fake_hash = behavior_policy_sha256(fake_receipt)
            replica_receipts = retained["duplicate_audit"][
                "replica_receipts"
            ]
            for receipt in replica_receipts:
                ...
                replica = json.loads(audit_path.read_text(encoding="utf-8"))
                replica["behavior_policy_receipt"] = deepcopy(fake_receipt)
                replica["behavior_policy_schema_sha256"] = fake_hash
                publish_clean_episode(audit_path, replica)
                receipt["bytes"] = audit_path.stat().st_size
                receipt["sha256"] = sha256_file(audit_path)
            retained["behavior_policy_receipt"] = deepcopy(fake_receipt)
            retained["behavior_policy_schema_sha256"] = fake_hash
            retained["duplicate_audit"]["replica_receipts"] = (
                replica_receipts
            )
            publish_clean_episode(fixture["episode_path"], retained)
            self._refresh_manifest_episode(fixture)
            ...
            with self.assertRaisesRegex(
                ValueError,
                "episode header does not match its run manifest",
            ):
                load_manifest_dataset(
                    fixture["manifest_path"],
                    input_checkpoint_sha256=fixture["checkpoint_hash"],
                    expected_source_hashes=checkpoint_source_hashes(),
                    expected_reference_prior_receipt=(
                        canonical_reference_prior_receipt()
                    ),
                    expected_reference_prior_schema_sha256=(
                        reference_prior_sha256()
                    ),
                    expected_behavior_policy_receipt=(
                        canonical_behavior_policy_receipt()
                    ),
                    expected_behavior_policy_schema_sha256=(
                        behavior_policy_sha256()
                    ),
                )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            duplicate_manifest = replace(
                ...
                dataset_sha256=dataset_sha256(
                    ...
                    behavior_policy_receipt=(
                        duplicate_manifest.behavior_policy_receipt
                    ),
                    behavior_policy_schema_sha256=(
                        duplicate_manifest.behavior_policy_schema_sha256
                    ),
    ...
    def test_trainer_rejects_population_path_and_manifest_hash_mutation(self):
        ...
        with tempfile.TemporaryDirectory(dir=tests_root) as temporary:
            ...
            payload = json.loads(population_path.read_text(encoding="utf-8"))
            replacement = repo / "opponents" / "meta_agents" / "alakazam_psychic_public_simple"
    ...
    def test_trainer_rejects_behavior_formula_probability_and_order_tampering(self):
        row_mutations = (
            (
                "formula-id",
                lambda row: row.update({"behavior_formula_id": "wrong"}),
                "behavior-policy formula/provenance",
            ),
            (
                "schema-version",
                lambda row: row.update({"behavior_schema_version": "wrong"}),
                "behavior-policy formula/provenance",
            ),
            (
                "schema-hash",
                lambda row: row.update({"behavior_schema_sha256": "0" * 64}),
                "behavior-policy formula/provenance",
            ),
            (
                "temperature",
                lambda row: row.update({"behavior_temperature": 0.66}),
                "behavior-policy formula/provenance",
            ),
            (
                "support-mixture",
                lambda row: row.update({"behavior_support_mixture": 0.03}),
                "behavior-policy formula/provenance",
            ),
            (
                "option-order",
                lambda row: row.update({"behavior_option_order": [1, 0]}),
                "legal-action order",
            ),
            (
                "action-order-hash",
                lambda row: row.update(
                    {"behavior_action_order_sha256": "0" * 64}
                ),
                "legal-action order",
            ),
            (
                ...
                lambda row: row.update(
                    {"final_probabilities": row["final_probabilities"][:-1]}
                ),
            ...
            (
                "logprob",
                lambda row: row.update(
                    {"behavior_logprob": row["behavior_logprob"] + 1e-10}
                ),
                "behavior logprob",
            ),
        )
        for label, mutate, expected in row_mutations:
            with self.subTest(row=label), tempfile.TemporaryDirectory() as temporary:
                fixture = self._make_run(
                    Path(temporary),
                    run_id=f"behavior-row-{label}",
                    policy_rng_seed=31,
                )
                episode = json.loads(
                    fixture["episode_path"].read_text(encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            row = episode["decisions"][0]
            row["final_probabilities"][0] += 1e-10
            row["final_probabilities"][1] -= 1e-10
            selected = row["final_action"][0]
            row["behavior_logprob"] = math.log(
                row["final_probabilities"][selected]
            )
            publish_clean_episode(fixture["episode_path"], episode)
            self._refresh_manifest_episode(fixture)
            with self.assertRaisesRegex(
                ValueError, "behavior probabilities cannot be reproduced"
            ):
                self._train_fixture(fixture)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            fixture = self._make_run(
                Path(temporary),
                run_id="behavior-row-support-floor",
                policy_rng_seed=31,
            )
            episode = json.loads(
                fixture["episode_path"].read_text(encoding="utf-8")
            ...
            row = episode["decisions"][0]
            floor = row["behavior_support_mixture"] / row["legal_option_count"]
            row["final_probabilities"] = [floor, 1.0 - floor]
            selected = row["final_action"][0]
            row["behavior_logprob"] = math.log(
                row["final_probabilities"][selected]
            )
            publish_clean_episode(fixture["episode_path"], episode)
            self._refresh_manifest_episode(fixture)
            with self.assertRaisesRegex(ValueError, "lost support floor"):
                self._train_fixture(fixture)

        episode_mutations = (
            (
                "receipt-formula",
                lambda episode: episode["behavior_policy_receipt"].update(
                    {"formula_id": "wrong"}
                ),
            ),
            (
            ...
            (
                "receipt-anchor-direction",
                lambda episode: episode["behavior_policy_receipt"].update(
                    {"anchor_kl_direction": "wrong"}
                ),
            ),
            (
                "receipt-anchor-formula",
                lambda episode: episode["behavior_policy_receipt"].update(
                    {"anchor_reference_formula_id": "wrong"}
                ),
            ),
            (
                "receipt-anchor-surface",
                lambda episode: episode["behavior_policy_receipt"].update(
                    {"anchor_surface_identity": "wrong"}
                ),
            ),
            (
                "receipt-hash",
                lambda episode: episode.update(
                    {"behavior_policy_schema_sha256": "0" * 64}
                ),
            ),
        )
        for label, mutate in episode_mutations:
            with self.subTest(episode=label), tempfile.TemporaryDirectory() as temporary:
                fixture = self._make_run(
                    Path(temporary),
                    run_id=f"behavior-episode-{label}",
                )
                episode = json.loads(
                    fixture["episode_path"].read_text(encoding="utf-8")
                )
        ...
        for label, mutate in (
            (
                "self-consistent-wrong-receipt",
                lambda payload: (
                    payload["behavior_policy_receipt"].update(
                        {"temperature": 0.66}
                    ),
                    payload.update(
                        {
            ...
            with self.subTest(manifest=label), tempfile.TemporaryDirectory() as temporary:
                fixture = self._make_run(
                    Path(temporary),
                    run_id=f"behavior-manifest-{label}",
                )
                payload = json.loads(
                    fixture["manifest_path"].read_text(encoding="utf-8")
                )
    ...
    def test_trainer_rejects_legacy_ambiguous_manifest_and_trajectory(self):
        ...
        for legacy_policy_version in (1, 2, 3):
