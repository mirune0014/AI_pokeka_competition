...
Genesect = 142           # x0 (legacy public-opponent semantics)
Psyduck = 858            # x0 (legacy public-opponent semantics)
Shaymin = 343            # x1
Rare_Candy = 1079        # x3
...
Sacred_Ash = 1129        # x1
Poke_Pad = 1152          # x4
Lucky_Helmet = 1156      # x0 (legacy public-opponent semantics)
Boss_Orders = 1182       # x3
Lanas_Aid = 1184         # x1; v0 generic hold
Xerosics_Machinations = 1197  # x3; v0 generic hold
Hilda = 1225             # x4
Dawn = 1231              # x4
Battle_Cage = 1264       # x0 (legacy public-opponent semantics)
REMOVED_OWN_CARD_IDS = frozenset({142, 858, 1156, 1161, 1264})
Nighttime_Mine = 1266    # x2; v0 generic hold
...
RESERVE_BASIC_PRIORITY = (Abra, Dunsparce, Shaymin, Fezandipiti_ex)
POKE_PAD_BASIC_PRIORITY = (Abra, Dunsparce, Shaymin)
SINGLETON_LOSS_THREATS = {Solrock, Riolu, Duskull, Staryu}
V0_GENERIC_HOLD = frozenset(
    {Lanas_Aid, Xerosics_Machinations, Nighttime_Mine}
)


def _v0_generic_port_score(option_type, context, card_id):
    '''Return the explicit v0-port score and reason for newly added cards.'''
    if card_id not in V0_GENERIC_HOLD:
        return None
    if option_type == OptionType.PLAY and context == SelectContext.MAIN:
        return -1, 'V0_GENERIC_HOLD'
    if option_type == OptionType.CARD and context == SelectContext.DISCARD:
        return 1, 'V0_GENERIC_FORCED_DISCARD'
    return None
...
_STRANDED_RETREAT_START_MODE_FINALIZED_END = "finalized_end"
_STRANDED_RETREAT_START_MODE_FINALIZED_SUPER_PSY_BOLT = (
    "finalized_super_psy_bolt"
)
...
def _decision_signature(
    ...
    if _stranded_retreat_ko_latch:
        stranded_guard_signature = (
            _stranded_retreat_ko_latch.get("start_mode"),
            _stranded_retreat_ko_latch.get("finalized_attack_id"),
            _stranded_retreat_ko_latch.get("finalized_attack_option_index"),
            state.retreated,
            len(mine.prize),
            len(theirs.prize),
            mine.handCount,
...
def _start_guarded_teleportation_continuity(
    ...
    for option_index, option in enumerate(select.option):
        if option.type == OptionType.PLAY:
            # Deleted own cards must not certify a protected dead-option lane.
            return None
...
def _bridge_ready_alakazam_destinations(
    mine, player_index: int
) -> list[tuple[int, Pokemon]]:
    """Return every complete positive-HP Bench Alakazam ready to attack."""
    return [
        (bench_index, pokemon)
        for bench_index, pokemon in enumerate(mine.bench)
        if pokemon.id == Alakazam
        and pokemon.serial > 0
        and _stranded_retreat_pokemon_is_strictly_complete(
            pokemon, player_index
        )
        and any(
            card.id in PSYCHIC_ENERGY_IDS for card in pokemon.energyCards
...
def _stranded_retreat_pokemon_is_strictly_complete(
    pokemon: Pokemon | None, player_index: int
) -> bool:
    """Require every public component to have checked card metadata."""
    if not _bridge_pokemon_is_publicly_complete(pokemon, player_index):
        return False
    components = (
        list(pokemon.energyCards)
    ...
    return card_table.get(pokemon.id) is not None and all(
        card_table.get(card.id) is not None for card in components
    )
...
def _start_stranded_retreat_ko_bridge(
    obs: Observation,
    *,
    start_mode: str = _STRANDED_RETREAT_START_MODE_FINALIZED_END,
    finalized_action: list[int] | None = None,
) -> list[int] | None:
    ...
    finalized_attack_matches = [
        option_index
        for option_index, option in enumerate(select.option)
        if option.type == OptionType.ATTACK
        and option.attackId == ATTACK_SUPER_PSY_BOLT
    ]
    finalized_attack_index = (
        finalized_action[0]
        if isinstance(finalized_action, list) and len(finalized_action) == 1
        else None
    )
    if (
        start_mode
        not in (
            _STRANDED_RETREAT_START_MODE_FINALIZED_END,
            _STRANDED_RETREAT_START_MODE_FINALIZED_SUPER_PSY_BOLT,
        )
        ...
        or (
            ...
            and (
                ...
                or not _stranded_retreat_pokemon_is_strictly_complete(
                    active, my_index
                )
                or finalized_attack_index is None
                or finalized_attack_matches != [finalized_attack_index]
            )
        )
        or (
            active.id == Alakazam
        ...
        or (
            not finalized_super_psy_bolt
            and any(
                option.type == OptionType.ATTACK for option in select.option
            )
        )
        or target is None
        or not _bridge_pokemon_is_publicly_complete(
    ...
    if not retreat_options or (
        finalized_super_psy_bolt and len(retreat_options) != 1
    ):
        return None
    ...
    if (
        retreat_cost <= 0
        or (finalized_super_psy_bolt and retreat_cost != 1)
    ...
    if finalized_super_psy_bolt:
        destinations = _bridge_ready_alakazam_destinations(mine, my_index)
        if len(destinations) != 1:
            return None
        destination_index, destination_pokemon = destinations[0]
    else:
    ...
    _stranded_retreat_ko_latch.update(
        ...
        finalized_attack_id=(
            ATTACK_SUPER_PSY_BOLT if finalized_super_psy_bolt else None
        ),
        finalized_attack_option_index=(
            finalized_attack_index if finalized_super_psy_bolt else None
        ),
...
def _stranded_retreat_ko_overlay(
    ...
    start_mode = latch.get("start_mode")
    finalized_attack_id = latch.get("finalized_attack_id")
    finalized_attack_option_index = latch.get(
        "finalized_attack_option_index"
    )
    if (
        start_mode == _STRANDED_RETREAT_START_MODE_FINALIZED_END
        and (
            finalized_attack_id is not None
            or finalized_attack_option_index is not None
        )
    ) or (
        start_mode
        == _STRANDED_RETREAT_START_MODE_FINALIZED_SUPER_PSY_BOLT
        and (
            finalized_attack_id != ATTACK_SUPER_PSY_BOLT
            or not isinstance(finalized_attack_option_index, int)
            or finalized_attack_option_index < 0
        )
    ) or start_mode not in (
        _STRANDED_RETREAT_START_MODE_FINALIZED_END,
        _STRANDED_RETREAT_START_MODE_FINALIZED_SUPER_PSY_BOLT,
    ):
        _clear_stranded_retreat_ko_latch()
...
def _turn_guard_thin_deck_helmet_action(
    ...
    """Lucky Helmet is absent from the exact v1 deck and cannot drive policy."""
    return None
...
def _exact_prize_lane_live_parent_agent(obs_dict: dict) -> list[int]:
    ...
    prepare_boundary_delegated = had_latch_before_prepare and not any(
        (
            _hilda_source_latch,
            _enriching_reserve_latch,
            _fez_ko_bridge_latch,
    ...
    had_fez_ko_bridge_latch = bool(_fez_ko_bridge_latch)
    emergency_action = _fez_ko_bridge_overlay(obs)
    if emergency_action is not None:
        return _remember_action(decision_signature, emergency_action)
    fez_ko_bridge_delegated = (
        had_fez_ko_bridge_latch and not _fez_ko_bridge_latch
    )

    had_hilda_source_latch = bool(_hilda_source_latch)
    had_enriching_reserve_latch = bool(_enriching_reserve_latch)
    emergency_action = _enriching_reserve_overlay(obs)
    if emergency_action is not None:
        return _remember_action(decision_signature, emergency_action)
    hilda_source_delegated = (
        had_hilda_source_latch and not _hilda_source_latch
    )
    enriching_reserve_delegated = (
        had_enriching_reserve_latch and not _enriching_reserve_latch
    )

    had_active_psychic_ko_latch = bool(_active_psychic_ko_latch)
    ...
    active_psychic_ko_delegated = (
        had_active_psychic_ko_latch and not _active_psychic_ko_latch
    )
    ...
    for o in select.option:
        score = 0
        removed_own_source = None
        if o.type in (OptionType.PLAY, OptionType.ATTACH, OptionType.EVOLVE):
            removed_own_source = get_card(obs, AreaType.HAND, o.index, my_index)
        elif o.type in (OptionType.CARD, OptionType.ABILITY):
            candidate_owner = o.playerIndex if o.playerIndex is not None else my_index
            if candidate_owner == my_index:
                removed_own_source = get_card(obs, o.area, o.index, my_index)
        if (
            removed_own_source is not None
            and removed_own_source.id in REMOVED_OWN_CARD_IDS
        ):
            scores.append(-10**12)
            continue
        if o.type == OptionType.NUMBER:
            ...
            v0_generic = _v0_generic_port_score(o.type, context, card.id)

            if v0_generic is not None:
                score = v0_generic[0]
            ...
            elif active_id in (Abra, Dunsparce, Dudunsparce, Shaymin):
    ...
    # The parent pipeline is now fully finalized.  Replace only its unique
    # Super Psy Bolt with the strict public-clear retreat transaction, and
    # never claim an observation on which an inherited latch just aborted.
    if (
        not prepare_boundary_delegated
        and not turn_objective_recovery_delegated
        and not guarded_teleportation_delegated
        and not stranded_retreat_delegated
        and not fez_ko_bridge_delegated
        and not hilda_source_delegated
        and not enriching_reserve_delegated
        and not active_psychic_ko_delegated
        and context == SelectContext.MAIN
        and len(chosen_action) == 1
        and select.option[chosen_action[0]].type == OptionType.ATTACK
        and select.option[chosen_action[0]].attackId
        ...
        transaction_action = _start_stranded_retreat_ko_bridge(
            obs,
            start_mode=(
                _STRANDED_RETREAT_START_MODE_FINALIZED_SUPER_PSY_BOLT
            ),
            finalized_action=chosen_action,
        )
        if transaction_action is not None:
            return _remember_action(
                _decision_signature(obs, obs_dict), transaction_action
...
# ---------------------------------------------------------------------------
# EXACT_PRIZE_LANE_BOSS_ARBITRATION_ACTIVE_PSYCHIC_HANDOFF_V2
# The complete live parent above is preserved and invoked once for every
# first-seen callback.  This final wrapper owns only the certified Prize-lane
# decision and its exact Boss continuation.
# ---------------------------------------------------------------------------

_exact_prize_lane_parent_agent = _exact_prize_lane_live_parent_agent
_exact_prize_lane_boss_latch = {}
_exact_prize_lane_duplicate = {}
_EXACT_PRIZE_LANE_VERSION = (
    "exact_prize_lane_boss_arbitration_active_psychic_handoff_v2"
)
_EXACT_ACTIVE_PSYCHIC_KEYS = frozenset(
    {
        "stage",
        "turn",
        "player",
        "active_serial",
        "active_static_fingerprint",
        "active_energy_fingerprints",
        "active_energy_units",
        "target_serial",
        "target_fingerprint",
        "selected_energy_id",
        "selected_energy_serial",
        "selected_energy_fingerprint",
        "post_attach_hand_fingerprint",
        "own_prize_count",
        "target_prizes",
        "post_ko_prizes",
        "conservative_damage",
...
def _exact_prize_lane_clear(*, clear_duplicate: bool = False) -> None:
    _exact_prize_lane_boss_latch.clear()
    if clear_duplicate:
        _exact_prize_lane_duplicate.clear()


def _exact_prize_lane_parent_state() -> dict:
    """Deep-copy every mutable field owned by the exact live parent."""
    return {
        ...
        "teleportation": copy.deepcopy(_guarded_teleportation_latch),
        "recovery": copy.deepcopy(_turn_objective_recovery_latch),
        "semantic_failure": copy.deepcopy(
            _guarded_teleportation_semantic_failure
        ),
        "decision_signature": copy.deepcopy(_last_decision_signature),
        "decision_action": copy.deepcopy(_last_decision_action),
        "pre_turn": pre_turn,
        "ability_dudunsparce": ability_used_dudunsparce,
        "ability_fezandipiti": ability_used_fezandipiti,
    }


def _exact_prize_lane_restore_parent(snapshot: dict) -> None:
    """Restore one complete parent-policy snapshot without broad clearing."""
    global _last_decision_signature, _last_decision_action
    global pre_turn, ability_used_dudunsparce, ability_used_fezandipiti
    targets = (
        ...
        (_guarded_teleportation_latch, "teleportation"),
        (_turn_objective_recovery_latch, "recovery"),
        (_guarded_teleportation_semantic_failure, "semantic_failure"),
    )
    for target, key in targets:
        target.clear()
        target.update(copy.deepcopy(snapshot[key]))
    ...
    ability_used_dudunsparce = snapshot["ability_dudunsparce"]
    ability_used_fezandipiti = snapshot["ability_fezandipiti"]


def _exact_prize_lane_parent_frozen(snapshot: dict) -> tuple:
    return _two_prize_freeze_raw(snapshot)


def _exact_prize_lane_transaction_identity(latch: dict) -> tuple | None:
    if not latch:
        return None
    return (
        latch.get("version"),
        ...
        latch.get("turn"),
        latch.get("player"),
        latch.get("start_action_count"),
        latch.get("boss_fingerprint"),
        latch.get("target_serial"),
        latch.get("attacker_serial"),
    )


def _exact_prize_lane_duplicate_action(signature: tuple) -> list[int] | None:
    record = _exact_prize_lane_duplicate
    if not record or record.get("raw_signature") != signature:
        return None
    if record.get("transaction_identity") != (
        _exact_prize_lane_transaction_identity(_exact_prize_lane_boss_latch)
    ):
        return None
    if record.get("post_stage") != (
        _exact_prize_lane_boss_latch.get("stage")
        if _exact_prize_lane_boss_latch
        else None
    ):
        return None
    if record.get("parent_state") != _exact_prize_lane_parent_frozen(
        _exact_prize_lane_parent_state()
    ):
        return None
    action = record.get("action")
    return list(action) if isinstance(action, tuple) else None


def _exact_prize_lane_remember(
    signature: tuple, action: list[int]
) -> list[int]:
    _exact_prize_lane_duplicate.clear()
    _exact_prize_lane_duplicate.update(
        raw_signature=signature,
        transaction_identity=_exact_prize_lane_transaction_identity(
            _exact_prize_lane_boss_latch
        ),
        post_stage=(
            _exact_prize_lane_boss_latch.get("stage")
        ...
        parent_state=_exact_prize_lane_parent_frozen(
            _exact_prize_lane_parent_state()
        ),
        action=tuple(action),
    )
...
def _exact_prize_lane_action_is_valid(obs: Observation, action: list[int]) -> bool:
    select = obs.select
    return (
...
def _exact_prize_lane_main_envelope(select) -> bool:
    return (
        int(select.type) == 0
        and select.context == SelectContext.MAIN
        and select.minCount == select.maxCount == 1
        and select.remainDamageCounter == select.remainEnergyCost == 0
...
def _exact_prize_lane_card_rows(cards, owner: int | None) -> tuple | None:
    if cards is None:
        return None
    rows = []
    for card in cards:
        if (
            ...
            or card.serial <= 0
            or card_table.get(card.id) is None
            or (owner is not None and card.playerIndex != owner)
        ):
            return None
        rows.append(_bridge_card_fingerprint(card))
    if len({row[1] for row in rows}) != len(rows):
...
def _exact_prize_lane_prize_rows(player, owner: int) -> tuple | None:
    rows = []
...
def _exact_prize_lane_status(player) -> tuple:
    return (
        player.poisoned,
        player.burned,
        player.asleep,
...
def _exact_prize_lane_field_rows(group, owner: int) -> tuple | None:
    rows = []
    for pokemon in group:
        if not _two_prize_public_pokemon_is_complete(pokemon, owner) and not (
            pokemon.id == Alakazam
...
def _exact_prize_lane_snapshot(obs: Observation) -> dict | None:
    state = obs.current
    owner = state.yourIndex
    if owner not in (0, 1) or len(state.players) != 2:
        return None
    mine, theirs = state.players[owner], state.players[1 - owner]
    if mine.hand is None or len(mine.hand) != mine.handCount:
        return None
    values = (
        _exact_prize_lane_card_rows(mine.hand, owner),
        _exact_prize_lane_card_rows(mine.discard, owner),
        _exact_prize_lane_card_rows(theirs.discard, 1 - owner),
        _exact_prize_lane_card_rows(state.stadium, None),
        _exact_prize_lane_prize_rows(mine, owner),
        _exact_prize_lane_prize_rows(theirs, 1 - owner),
        _exact_prize_lane_field_rows(mine.active, owner),
        _exact_prize_lane_field_rows(mine.bench, owner),
        _exact_prize_lane_field_rows(theirs.active, 1 - owner),
        _exact_prize_lane_field_rows(theirs.bench, 1 - owner),
    )
    if any(value is None for value in values):
        return None
    serials = _bridge_public_serials(state)
    if (
        any(not isinstance(serial, int) or serial <= 0 for serial in serials)
    ...
    return {
        ...
        "own_status": _exact_prize_lane_status(mine),
        ...
        "opponent_status": _exact_prize_lane_status(theirs),
...
def _exact_prize_lane_visible_modifiers_are_clear(state) -> bool:
    metadata = []
    for player in state.players:
        for pokemon in list(player.active) + list(player.bench):
            data = card_table.get(pokemon.id)
...
def _exact_prize_lane_prize_value(pokemon: Pokemon, owner: int) -> int | None:
    if not _two_prize_public_pokemon_is_complete(pokemon, owner):
        return None
    data = card_table.get(pokemon.id)
    if (
        data is None
        or not isinstance(data.ex, bool)
        or not isinstance(data.megaEx, bool)
        or (data.ex and data.megaEx)
    ):
        return None
    base = 3 if data.megaEx else 2 if data.ex else 1
    attached_metadata = []
    for card in list(pokemon.energyCards) + list(pokemon.tools):
        item = card_table.get(card.id)
        if item is None:
            return None
        attached_metadata.append(item)
    if any(
        "prize" in _normalized_skill_text(skill.text)
        for item in (data, *attached_metadata)
        for skill in (item.skills or [])
    ):
        return None
    return base if prize_count(pokemon) == base else None


def _exact_prize_lane_attacker_ready(
    obs: Observation, attacker: Pokemon | None
) -> bool:
    mine = obs.current.players[obs.current.yourIndex]
    if (
        attacker is None
        or attacker.id != Alakazam
        or not _two_prize_powerful_hand_metadata_is_exact()
        or not _bridge_pokemon_is_publicly_complete(
            attacker, obs.current.yourIndex
        )
        or not isinstance(attacker.appearThisTurn, bool)
        or attacker.maxHp != card_table[Alakazam].hp
        or not 0 < attacker.hp <= attacker.maxHp
        or not _two_prize_alakazam_lineage_is_complete(
            attacker, obs.current.yourIndex
        )
        or _exact_prize_lane_status(mine) != (False,) * 5
    ):
        return False
    energy_units = _bridge_retaliation_energy_units(attacker)
    return energy_units is not None and _bridge_retaliation_can_pay(
        energy_units, attack_table[ATTACK_POWERFUL_HAND].energies
    ) is True


def _exact_prize_lane_attack_option(obs: Observation) -> int | None:
    matches = [
        index
        for index, option in enumerate(obs.select.option)
        if option.type == OptionType.ATTACK
        and option.attackId == ATTACK_POWERFUL_HAND
        and _two_prize_option_is_exact(
            option, OptionType.ATTACK, attackId=ATTACK_POWERFUL_HAND
        )
    ]
    return matches[0] if len(matches) == 1 else None


def _exact_prize_lane_boss_metadata_is_exact() -> bool:
    data = card_table.get(Boss_Orders)
    return (
        data is not None
        and data.name == "Boss’s Orders"
        and data.cardType == CardType.SUPPORTER
        and len(data.skills or []) == 1
        and data.skills[0].text
        == "Switch in 1 of your opponent’s Benched Pokémon to the Active Spot."
    )


def _exact_prize_lane_boss_options(obs: Observation) -> list[tuple[int, Card]] | None:
    owner = obs.current.yourIndex
    mine = obs.current.players[owner]
    if mine.hand is None or not _exact_prize_lane_boss_metadata_is_exact():
        return None
    physical = [card for card in mine.hand if card.id == Boss_Orders]
    options = []
    mapped_serials = []
    for option_index, option in enumerate(obs.select.option):
        if option.type != OptionType.PLAY:
        ...
        if (
            ...
            or mine.hand[option.index].id != Boss_Orders
        ):
            continue
        card = mine.hand[option.index]
        if (
            not _two_prize_option_is_exact(
                option, OptionType.PLAY, index=option.index
            )
            or card.serial <= 0
            or card.playerIndex != owner
            or sum(candidate.serial == card.serial for candidate in mine.hand) != 1
        ):
            return None
        options.append((option_index, card))
        mapped_serials.append(card.serial)
    if (
        not physical
        or len(options) != len(physical)
        or len(mapped_serials) != len(set(mapped_serials))
        or set(mapped_serials) != {card.serial for card in physical}
    ):
        return None
    return sorted(options, key=lambda row: row[0])


def _exact_prize_lane_target_lane(
    obs: Observation, target: Pokemon, hand_count: int
) -> dict | None:
    prize_value = _exact_prize_lane_prize_value(
        target, 1 - obs.current.yourIndex
    )
    if (
        prize_value is None
        or hand_count < 0
        or not _powerful_hand_target_is_publicly_clear(obs.current, target)
    ):
        return None
    ...
    return {
        "target": target,
        "fingerprint": _bridge_pokemon_fingerprint(target),
        "prizes": prize_value,
        "damage": damage,
        "lethal": lethal,
        "terminal": lethal
        and prize_value
        >= len(obs.current.players[obs.current.yourIndex].prize),
    }


def _exact_prize_lane_target_moves(target: Pokemon) -> tuple:
    return (
...
def _exact_prize_lane_handoff_entry(
    raw: dict, obs: Observation, parent_pre: dict, raw_signature: tuple
) -> dict | None:
    state = obs.current
    active_latch = parent_pre["active_psychic"]
    inherited = (
        parent_pre["hilda"],
        parent_pre["enriching"],
        parent_pre["fez"],
        parent_pre["stranded"],
        parent_pre["teleportation"],
        parent_pre["recovery"],
    )
    selected = active_latch.get("selected_energy_fingerprint")
    attack_option = _exact_prize_lane_attack_option(obs)
    if (
        _exact_prize_lane_boss_latch
        or any(inherited)
        or not active_latch
        or frozenset(active_latch) != _EXACT_ACTIVE_PSYCHIC_KEYS
        or active_latch.get("stage") != "await_attack"
        or not isinstance(selected, tuple)
        or len(selected) != 3
        or selected[0] != active_latch.get("selected_energy_id")
        or selected[1] != active_latch.get("selected_energy_serial")
        or selected[2] != active_latch.get("player")
        or selected[0] not in PSYCHIC_ENERGY_IDS
        or not isinstance(selected[1], int)
        or selected[1] <= 0
        or not _two_prize_raw_parsed_agree(raw, obs)
        or not _exact_prize_lane_main_envelope(obs.select)
        or state.result != -1
        or state.looking is not None
        or state.supporterPlayed
        or attack_option is None
        or not _active_psychic_post_attach_is_same(obs, active_latch)
    ):
        return None
    return {
        "raw_signature": raw_signature,
        "entry_latch": copy.deepcopy(active_latch),
        "entry_latch_frozen": _two_prize_freeze_raw(active_latch),
        "parent_pre": copy.deepcopy(parent_pre),
        "parent_pre_frozen": _exact_prize_lane_parent_frozen(parent_pre),
        "attack_option": attack_option,
        "expected_parent_signature": _decision_signature(obs, raw),
    }


def _exact_prize_lane_handoff_parent_transition(
    obs: Observation,
    parent_action: list[int],
    certificate: dict,
    parent_post: dict,
) -> bool:
    expected_active = copy.deepcopy(certificate["entry_latch"])
    expected_active["stage"] = "await_resolution"
    before = certificate["parent_pre"]
    unchanged_keys = (
        "hilda",
        "enriching",
        "fez",
    ...
    return (
        parent_action == [certificate["attack_option"]]
        and parent_post["active_psychic"] == expected_active
        and all(parent_post[key] == before[key] for key in unchanged_keys)
        and parent_post["decision_action"]
        == (certificate["attack_option"],)
        and parent_post["decision_signature"]
        == certificate["expected_parent_signature"]
        and parent_post["decision_signature"] != before["decision_signature"]
    )


def _exact_prize_lane_build_plan(
    raw: dict,
    obs: Observation,
    parent_action: list[int],
    *,
    origin: str,
    source_certificate: dict | None = None,
) -> dict | None:
    state = obs.current
    owner = state.yourIndex
    mine, theirs = state.players[owner], state.players[1 - owner]
    attacker = mine.active[0] if len(mine.active) == 1 else None
    current_target = theirs.active[0] if len(theirs.active) == 1 else None
    snapshot = _exact_prize_lane_snapshot(obs)
    if (
        _exact_prize_lane_boss_latch
        or not _two_prize_raw_parsed_agree(raw, obs)
        or not _exact_prize_lane_main_envelope(obs.select)
        ...
        or not _exact_prize_lane_attacker_ready(obs, attacker)
        or current_target is None
        or not theirs.bench
        or not _two_prize_stadium_is_clear(state)
        or not _exact_prize_lane_visible_modifiers_are_clear(state)
    ):
        return None
    attack_option = _exact_prize_lane_attack_option(obs)
    boss_options = _exact_prize_lane_boss_options(obs)
    if attack_option is None or boss_options is None or len(parent_action) != 1:
        return None
    if parent_action[0] == attack_option:
        parent_kind = "powerful_hand"
    elif parent_action[0] in {option for option, _ in boss_options}:
        parent_kind = "boss"
    else:
        return None
    if origin == "certified_active_psychic_handoff" and parent_kind != "powerful_hand":
        return None

    current_lane = _exact_prize_lane_target_lane(
        obs, current_target, mine.handCount
    )
    boss_lanes = [
        _exact_prize_lane_target_lane(obs, target, mine.handCount - 1)
        for target in theirs.bench
    ]
    if current_lane is None or any(lane is None for lane in boss_lanes):
        return None
    terminals = []
    if current_lane["terminal"]:
        terminals.append(("active", current_lane, None))
    terminals.extend(
        ("boss", lane, target_index)
        for target_index, lane in enumerate(boss_lanes)
        if lane["terminal"]
    )
    if len(terminals) != 1:
        return None
    kind, lane, target_index = terminals[0]
    if kind == "active":
        if origin == "certified_active_psychic_handoff":
            return None
        return {"action": [attack_option], "latch": None}

    boss_option, boss_card = boss_options[0]
    boss_fingerprint = _bridge_card_fingerprint(boss_card)
    boss_hand_index = next(
        (
            index
            for index, row in enumerate(snapshot["own_hand"])
            if row == boss_fingerprint
        ),
        None,
    )
    if boss_hand_index is None or target_index is None:
        return None
    post_hand = (
        snapshot["own_hand"][:boss_hand_index]
        + snapshot["own_hand"][boss_hand_index + 1 :]
    )
    if (
        len(post_hand) != mine.handCount - 1
        or not _bridge_protected_serials_are_unique(
            state,
            (
                boss_card.serial,
                ...
                *_bridge_pokemon_component_serials(lane["target"]),
            ),
        )
    ):
        return None
    expected_target_prompt = dict(snapshot)
    ...
    expected_attack = dict(expected_target_prompt)
    expected_attack.update(
        action_count=state.turnActionCount + 2,
        own_discard=snapshot["own_discard"] + (boss_fingerprint,),
        opponent_status=(False,) * 5,
        opponent_active=(lane["fingerprint"],),
        opponent_bench=tuple(post_bench),
    )
    latch = {
        "version": _EXACT_PRIZE_LANE_VERSION,
        "origin": origin,
        "source_certificate": copy.deepcopy(source_certificate),
        "start_raw_signature": _two_prize_freeze_raw(raw),
        "stage": "await_target",
        "turn": state.turn,
        "player": owner,
        "start_action_count": state.turnActionCount,
        ...
        "attacker_fingerprint": _bridge_pokemon_fingerprint(attacker),
        "current_target_serial": current_target.serial,
        "current_target_fingerprint": _bridge_pokemon_fingerprint(
            current_target
        ),
        "target_index": target_index,
        "target_serial": lane["target"].serial,
        "target_fingerprint": lane["fingerprint"],
        "target_prizes": lane["prizes"],
        "target_moves": _exact_prize_lane_target_moves(lane["target"]),
        "expected_damage": lane["damage"],
        "post_boss_hand": post_hand,
        "start_snapshot": snapshot,
        "expected_target_prompt": expected_target_prompt,
        "expected_attack": expected_attack,
    }
    return {"action": [boss_option], "latch": latch}


def _exact_prize_lane_active_preservation(
    raw: dict, obs: Observation, parent_action: list[int]
) -> list[int] | None:
    """Apply only the ordinary no-latch higher-Prize Active decision."""
    state = obs.current
    owner = state.yourIndex
    mine, theirs = state.players[owner], state.players[1 - owner]
    attacker = mine.active[0] if len(mine.active) == 1 else None
    current = theirs.active[0] if len(theirs.active) == 1 else None
    if (
        _two_prize_inherited_owner_active()
        or _exact_prize_lane_boss_latch
        or not _two_prize_raw_parsed_agree(raw, obs)
        or not _exact_prize_lane_main_envelope(obs.select)
        or state.result != -1
        or state.looking is not None
        or state.supporterPlayed
        or _exact_prize_lane_snapshot(obs) is None
        or not _exact_prize_lane_attacker_ready(obs, attacker)
        or current is None
        or not theirs.bench
        or not _two_prize_stadium_is_clear(state)
    ...
    boss_options = _exact_prize_lane_boss_options(obs)
    if (
        attack_option is None
        or boss_options is None
        or len(parent_action) != 1
        or parent_action[0] not in {option for option, _ in boss_options}
    ):
        return None
    current_lane = _exact_prize_lane_target_lane(obs, current, mine.handCount)
    boss_lanes = [
        _exact_prize_lane_target_lane(obs, target, mine.handCount - 1)
        for target in theirs.bench
    ]
    if current_lane is None or any(lane is None for lane in boss_lanes):
        return None
    terminal_count = int(current_lane["terminal"]) + sum(
        int(lane["terminal"]) for lane in boss_lanes
    )
    lethal_boss_lanes = [lane for lane in boss_lanes if lane["lethal"]]
    if (
        terminal_count == 0
        and current_lane["lethal"]
        and lethal_boss_lanes
        and all(
            current_lane["prizes"] > lane["prizes"]
            for lane in lethal_boss_lanes
        )
    ):
...
def _exact_prize_lane_log_is_exact(log, log_type: int, expected: dict) -> bool:
...
def _exact_prize_lane_target_action(
    obs: Observation, latch: dict
) -> list[int] | None:
    state, select = obs.current, obs.select
    player = latch.get("player")
    theirs = state.players[1 - player]
    if (
        _exact_prize_lane_snapshot(obs) != latch.get("expected_target_prompt")
        ...
        != latch.get("boss_fingerprint")
        or len(obs.logs) != 1
        or not _exact_prize_lane_log_is_exact(
            obs.logs[0],
            10,
            {
                "playerIndex": player,
    ...
    for option_index, option in enumerate(select.option):
        ...
        if (
            isinstance(option.index, int)
            and not isinstance(option.index, bool)
            and 0 <= option.index < len(theirs.bench)
...
def _exact_prize_lane_attack_action(
    obs: Observation, latch: dict
) -> list[int] | None:
    state, select = obs.current, obs.select
    owner = state.yourIndex
    ...
    attack_option = _exact_prize_lane_attack_option(obs)
    if (
        _exact_prize_lane_snapshot(obs) != latch.get("expected_attack")
        or not _exact_prize_lane_main_envelope(select)
        or not _exact_prize_lane_attacker_ready(obs, attacker)
        or attacker.serial != latch.get("attacker_serial")
        or _bridge_pokemon_fingerprint(attacker)
        != latch.get("attacker_fingerprint")
        or target is None
        or target.serial != latch.get("target_serial")
        or _bridge_pokemon_fingerprint(target) != latch.get("target_fingerprint")
        or _exact_prize_lane_prize_value(target, 1 - owner)
        != latch.get("target_prizes")
        or not _powerful_hand_target_is_publicly_clear(state, target)
        or attack_option is None
        ...
        or not _exact_prize_lane_log_is_exact(
            ...
            {
                "playerIndex": 1 - owner,
                "cardIdActive": latch.get("current_target_fingerprint")[0],
                "serialActive": latch.get("current_target_serial"),
                "cardIdBench": latch.get("target_fingerprint")[0],
...
def _exact_prize_lane_resolution_is_exact(
    obs: Observation, latch: dict
) -> bool:
    state, select = obs.current, obs.select
    owner = state.yourIndex
    ...
    if not isinstance(expected, dict):
        return False
    own_hand = _exact_prize_lane_card_rows(mine.hand, owner)
    own_discard = _exact_prize_lane_card_rows(mine.discard, owner)
    opponent_discard = _exact_prize_lane_card_rows(theirs.discard, 1 - owner)
    own_prize = _exact_prize_lane_prize_rows(mine, owner)
    opponent_prize = _exact_prize_lane_prize_rows(theirs, 1 - owner)
    own_active = _exact_prize_lane_field_rows(mine.active, owner)
    own_bench = _exact_prize_lane_field_rows(mine.bench, owner)
    opponent_bench = _exact_prize_lane_field_rows(theirs.bench, 1 - owner)
    target_prizes = latch.get("target_prizes")
    if not isinstance(target_prizes, int):
        return False
    expected_taken = min(target_prizes, len(expected.get("own_prize")))
    if (
        state.turn != latch.get("turn")
        ...
        or _exact_prize_lane_status(mine) != expected.get("own_status")
        or _exact_prize_lane_status(theirs) != (False,) * 5
        or own_hand != latch.get("post_boss_hand")
        or own_discard != expected.get("own_discard")
        or own_prize != expected.get("own_prize")
        or own_active != expected.get("own_active")
        or own_bench != expected.get("own_bench")
        or theirs.active
        or opponent_bench != expected.get("opponent_bench")
        or opponent_prize != expected.get("opponent_prize")
        or theirs.handCount != expected.get("opponent_hand_count")
        or opponent_discard
        != expected.get("opponent_discard")
        + tuple(
            (card_id, serial, 1 - owner)
            for card_id, serial, _ in latch.get("target_moves")
        )
        or _exact_prize_lane_card_rows(state.stadium, None)
    ...
    if len(obs.logs) != 2 + len(latch.get("target_moves")):
        return False
    if not _exact_prize_lane_log_is_exact(
        obs.logs[0],
        15,
        {
            "playerIndex": owner,
    ...
    if not _exact_prize_lane_log_is_exact(
        ...
        {
            "playerIndex": 1 - owner,
            "cardId": latch.get("target_fingerprint")[0],
        ...
        return False
    return all(
        _exact_prize_lane_log_is_exact(
            log,
            6,
            {
                "playerIndex": 1 - owner,
...
def _exact_prize_lane_advance(obs: Observation) -> tuple[str, list[int] | None]:
    latch = _exact_prize_lane_boss_latch
    if (
        not latch
        or latch.get("version") != _EXACT_PRIZE_LANE_VERSION
        or obs.current.turn != latch.get("turn")
        or obs.current.yourIndex != latch.get("player")
    ):
        return ("failure", None)
    stage = latch.get("stage")
    if stage == "await_target":
        action = _exact_prize_lane_target_action(obs, latch)
        if action is None:
            return ("failure", None)
        latch["stage"] = "await_attack"
        return ("override", action)
    if stage == "await_attack":
        action = _exact_prize_lane_attack_action(obs, latch)
        if action is None:
            return ("failure", None)
        latch["stage"] = "await_resolution"
        return ("override", action)
    if stage == "await_resolution":
        return (
            ("resolved", None)
            if _exact_prize_lane_resolution_is_exact(obs, latch)
            else ("failure", None)
        )
    return ("failure", None)


def _exact_prize_lane_atomic_handoff_commit(
    plan: dict, parent_post: dict
) -> bool:
    """Install the complete Boss transaction, then consume only its source."""
    latch = plan.get("latch")
    if not isinstance(latch, dict) or not latch:
        return False
    try:
        _exact_prize_lane_boss_latch.clear()
        _exact_prize_lane_boss_latch.update(copy.deepcopy(latch))
        _clear_active_psychic_ko_latch()
        _clear_decision_cache()
        return True
    except Exception:
        _exact_prize_lane_boss_latch.clear()
        _exact_prize_lane_restore_parent(parent_post)
        return False


def agent(obs_dict: dict) -> list[int]:
    """Final/only callable: exact parent plus one atomic Prize-lane overlay."""
    try:
        raw_signature = _two_prize_freeze_raw(obs_dict)
    except Exception:
        raw_signature = ("unfreezable",)
    duplicate = _exact_prize_lane_duplicate_action(raw_signature)
    if duplicate is not None:
        return duplicate

    parent_pre = _exact_prize_lane_parent_state()
    had_transaction = bool(_exact_prize_lane_boss_latch)
    parsed = None
    handoff_entry = None
    try:
        if isinstance(obs_dict, dict) and obs_dict.get("select") is not None:
            parsed = to_observation_class(obs_dict)
            if parsed.current is not None and parsed.select is not None:
                handoff_entry = _exact_prize_lane_handoff_entry(
                    obs_dict, parsed, parent_pre, raw_signature
                )
    except (AttributeError, IndexError, KeyError, TypeError, ValueError):
        parsed = None
        handoff_entry = None

    # Every first-seen callback reaches the exact live parent exactly once.
    parent_action = _exact_prize_lane_parent_agent(obs_dict)
    parent_post = _exact_prize_lane_parent_state()
    try:
        if parsed is None:
            ...
            parsed = to_observation_class(obs_dict)
        if (
            parsed.current is None
            or parsed.select is None
            or not _exact_prize_lane_action_is_valid(parsed, parent_action)
            or not _two_prize_raw_parsed_agree(obs_dict, parsed)
        ):
            if had_transaction:
                _exact_prize_lane_clear(clear_duplicate=True)
            return parent_action

        if had_transaction:
            outcome, override = _exact_prize_lane_advance(parsed)
            if outcome == "override" and override is not None and (
                _exact_prize_lane_action_is_valid(parsed, override)
            ):
                try:
                    _exact_prize_lane_restore_parent(parent_pre)
                except Exception:
                    _exact_prize_lane_clear(clear_duplicate=True)
                    _exact_prize_lane_restore_parent(parent_post)
                    return parent_action
                return _exact_prize_lane_remember(raw_signature, override)
            if outcome == "resolved":
                _exact_prize_lane_clear(clear_duplicate=True)
                return parent_action
            _exact_prize_lane_clear(clear_duplicate=True)
        ...
        if handoff_entry is not None and _exact_prize_lane_handoff_parent_transition(
            parsed, parent_action, handoff_entry, parent_post
        ):
            source_certificate = {
                "raw_signature": handoff_entry["raw_signature"],
                "entry_latch": handoff_entry["entry_latch_frozen"],
                "parent_pre": handoff_entry["parent_pre_frozen"],
                "parent_post": _exact_prize_lane_parent_frozen(parent_post),
            }
            plan = _exact_prize_lane_build_plan(
                obs_dict,
                parsed,
                parent_action,
                origin="certified_active_psychic_handoff",
                source_certificate=source_certificate,
            )
            if plan is not None and plan.get("latch") is not None:
                if _exact_prize_lane_atomic_handoff_commit(plan, parent_post):
                    action = plan["action"]
                    if _exact_prize_lane_action_is_valid(parsed, action):
                        return _exact_prize_lane_remember(raw_signature, action)
                _exact_prize_lane_clear(clear_duplicate=True)
                _exact_prize_lane_restore_parent(parent_post)
                return parent_action
            # A non-Boss handoff result keeps the legitimate parent attack,
            # await_resolution latch, and parent decision cache untouched.
            return parent_action

        # Ordinary ownership remains strictly no-inherited-latch at both entry
        # and post-parent boundaries.
        if not any(
            parent_pre[key]
            for key in (
                "hilda",
        ...
        ) and not _two_prize_inherited_owner_active():
            plan = _exact_prize_lane_build_plan(
                obs_dict,
                parsed,
                parent_action,
                origin="ordinary_no_inherited_latch",
            )
            if plan is not None:
                action = plan["action"]
                latch = plan.get("latch")
                ...
                if _exact_prize_lane_action_is_valid(parsed, action):
                    return _exact_prize_lane_remember(raw_signature, action)
                _exact_prize_lane_clear(clear_duplicate=True)
            preservation = _exact_prize_lane_active_preservation(
                obs_dict, parsed, parent_action
            )
            if preservation is not None and _exact_prize_lane_action_is_valid(
                parsed, preservation
            ):
                return _exact_prize_lane_remember(raw_signature, preservation)
        return parent_action
    except (AttributeError, IndexError, KeyError, TypeError, ValueError):
        if had_transaction:
            _exact_prize_lane_clear(clear_duplicate=True)
