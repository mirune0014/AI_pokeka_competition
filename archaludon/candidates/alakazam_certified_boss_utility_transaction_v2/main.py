...
BOSS_SINGLETON_LOSS_THREATS = SINGLETON_LOSS_THREATS | {Riolu_Mega_Lucario}
...
_boss_prize_conversion_latch = {}
_last_decision_signature = None
_last_decision_action = None

_BOSS_TRANSACTION_VERSION = "certified_boss_utility_transaction_v2"
...
def _clear_boss_prize_conversion_latch() -> None:
    _boss_prize_conversion_latch.clear()
...
def _clear_emergency_state(*, clear_cache: bool = False) -> None:
    ...
    _clear_boss_prize_conversion_latch()
    if clear_cache:
...
def _decision_signature(obs: Observation) -> tuple:
    ...
    boss_guard_signature = None
    if _boss_prize_conversion_latch:
        boss_guard_signature = (
            _boss_prize_conversion_latch.get("stage"),
            _boss_prize_conversion_latch.get("branch"),
...
def _boss_cards_fingerprint(cards, expected_player: int | None) -> tuple | None:
    """Return a canonical exact fingerprint for a fully public card zone."""
    if cards is None:
        return None
    fingerprints = []
    for card in cards:
        if (
            ...
            return None
        fingerprints.append(_bridge_card_fingerprint(card))
    if len({fingerprint[1] for fingerprint in fingerprints}) != len(
        fingerprints
    ):
        return None
    return tuple(sorted(fingerprints, key=lambda fingerprint: fingerprint[1]))


def _boss_pokemon_fingerprints(
    ...
    for pokemon in pokemon_group:
        if not _bridge_pokemon_is_publicly_complete(
            ...
            return None
        fingerprints.append(_bridge_pokemon_fingerprint(pokemon))
        protected_serials.extend(_bridge_pokemon_component_serials(pokemon))
    if not _bridge_protected_serials_are_unique(state, protected_serials):
        return None
    return tuple(sorted(fingerprints, key=lambda fingerprint: fingerprint[1]))


def _boss_status_fingerprint(player_state) -> tuple:
    return (
        player_state.poisoned,
        player_state.burned,
        player_state.asleep,
...
def _boss_public_snapshot(state, my_index: int) -> dict | None:
    """Freeze all public facts that Boss itself is allowed to change."""
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    if mine.hand is None or len(mine.hand) != mine.handCount:
        return None

    own_hand = _boss_cards_fingerprint(mine.hand, my_index)
    own_discard = _boss_cards_fingerprint(mine.discard, my_index)
    opponent_discard = _boss_cards_fingerprint(
        theirs.discard, 1 - my_index
    )
    stadium = _boss_cards_fingerprint(state.stadium, None)
    own_active = _boss_pokemon_fingerprints(
        state, [pokemon for pokemon in mine.active if pokemon is not None], my_index
    )
    own_bench = _boss_pokemon_fingerprints(state, list(mine.bench), my_index)
    opponent_active = _boss_pokemon_fingerprints(
        state,
        [pokemon for pokemon in theirs.active if pokemon is not None],
        1 - my_index,
    )
    opponent_bench = _boss_pokemon_fingerprints(
    ...
    protected_serials = [
        fingerprint[1]
        for group in (own_hand, own_discard, opponent_discard, stadium)
    ...
    opponent_field = tuple(
        sorted(
            ...
            key=lambda fingerprint: fingerprint[1],
        )
    )
    return {
        "own_hand": own_hand,
        ...
        "own_status": _boss_status_fingerprint(mine),
        "opponent_status": _boss_status_fingerprint(theirs),
...
def _boss_expected_without_card(
    hand_fingerprint: tuple, card_fingerprint: tuple
) -> tuple | None:
    matches = [
        fingerprint
        for fingerprint in hand_fingerprint
        if fingerprint == card_fingerprint
    ]
    if len(matches) != 1:
        return None
    return tuple(
...
def _boss_expected_discard(
    discard_fingerprint: tuple, card_fingerprint: tuple
) -> tuple:
    return tuple(
        sorted(
            discard_fingerprint + (card_fingerprint,),
            key=lambda fingerprint: fingerprint[1],
        )
    )


def _boss_unique_attack_option(select, attack_id: int) -> int | None:
    matches = [
        option_index
        for option_index, option in enumerate(select.option)
...
def _boss_exact_attack_route(
    obs: Observation,
    attacker: Pokemon,
    target: Pokemon,
    hand_count: int,
) -> dict | None:
    """Certify a legal, exact, current Psychic attack into one public target."""
    state = obs.current
    my_index = state.yourIndex
    if (
        not _bridge_pokemon_is_publicly_complete(attacker, my_index)
        or not _bridge_pokemon_is_publicly_complete(target, 1 - my_index)
        or not any(
            energy.id in PSYCHIC_ENERGY_IDS
            for energy in attacker.energyCards
        )
        or not _powerful_hand_target_is_publicly_clear(state, target)
    ):
        return None

    if attacker.id == Alakazam:
        attack_id = ATTACK_POWERFUL_HAND
        damage = 20 * hand_count
    elif attacker.id == Kadabra:
        attack_id = ATTACK_SUPER_PSY_BOLT
        damage = 30
    else:
        return None
    attack_option = _boss_unique_attack_option(obs.select, attack_id)
    if attack_option is None or damage < target.hp:
        return None
    return {
        "attack_id": attack_id,
...
def _boss_find_public_bench_target(
    theirs, serial: int, fingerprint: tuple
) -> tuple[int, Pokemon] | None:
    matches = [
        (bench_index, pokemon)
        for bench_index, pokemon in enumerate(theirs.bench)
        if pokemon.serial == serial
        and _bridge_pokemon_fingerprint(pokemon) == fingerprint
    ]
    return matches[0] if len(matches) == 1 else None


def _boss_post_boss_common_matches(
    obs: Observation, latch: dict, *, target_switched: bool
) -> bool:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    current = _boss_public_snapshot(state, my_index)
    frozen = latch.get("public_snapshot")
    if (
        current is None
        ...
        or current["own_hand"] != latch.get("expected_post_boss_hand")
        or current["stadium"] != latch.get("stadium_fingerprint")
        or current["own_active"] != frozen.get("own_active")
        or current["own_bench"] != frozen.get("own_bench")
        or current["opponent_field"] != frozen.get("opponent_field")
        or current["own_status"] != frozen.get("own_status")
        or current["own_deck_count"] != frozen.get("own_deck_count")
        or current["opponent_deck_count"]
        != frozen.get("opponent_deck_count")
        or current["own_prize_count"] != frozen.get("own_prize_count")
        or current["opponent_prize_count"]
        != frozen.get("opponent_prize_count")
        or current["opponent_hand_count"]
        != frozen.get("opponent_hand_count")
        or current["energy_attached"] != frozen.get("energy_attached")
        or current["retreated"] != frozen.get("retreated")
        or current["opponent_discard"] != frozen.get("opponent_discard")
    ):
        return False

    if target_switched:
        return (
            current["own_discard"] == latch.get("expected_post_boss_discard")
            and current["opponent_active"]
            == (latch.get("target_fingerprint"),)
            and current["opponent_status"] == (False, False, False, False, False)
            and mine.handCount == len(latch.get("expected_post_boss_hand") or ())
        )
    return (
        current["own_discard"] == frozen.get("own_discard")
        and current["opponent_active"] == frozen.get("opponent_active")
        and current["opponent_bench"] == frozen.get("opponent_bench")
        and current["opponent_status"] == frozen.get("opponent_status")
    )


def _boss_current_active_prize_gain(
    obs: Observation, attacker: Pokemon, target: Pokemon, hand_count: int
) -> int:
    route = _boss_exact_attack_route(obs, attacker, target, hand_count)
    return prize_count(target) if route is not None else 0


def _boss_certified_ko_candidates(
    obs: Observation, attacker: Pokemon, current_active_gain: int
) -> list[dict]:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    post_boss_hand_count = mine.handCount - 1
    if post_boss_hand_count < 0:
        return []

    candidates = []
    for bench_index, target in enumerate(theirs.bench):
        route = _boss_exact_attack_route(
            obs, attacker, target, post_boss_hand_count
        )
        if route is None:
            continue
        target_prizes = prize_count(target)
        if target_prizes <= 0 or target_prizes < current_active_gain:
            continue
        post_ko_prizes = max(0, len(mine.prize) - target_prizes)
        if post_ko_prizes != 0 and mine.deckCount <= post_ko_prizes:
            continue
        candidates.append(
            {
                **route,
                ...
                "post_ko_prizes": post_ko_prizes,
                "final_prize": target_prizes >= len(mine.prize),
            }
        )
    return sorted(
        candidates,
        key=lambda candidate: (
            0 if candidate["final_prize"] else 1,
            -candidate["target_prizes"],
            -candidate["target"].hp,
            candidate["bench_index"],
            candidate["target"].serial,
        ),
    )


def _boss_certified_ko_plan_for_target(
    obs: Observation,
    attacker: Pokemon,
    target: Pokemon,
    current_active_gain: int,
) -> dict | None:
    """Certify the inherited target itself without re-ranking the Bench."""
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    target_fingerprint = _bridge_pokemon_fingerprint(target)
    target_match = _boss_find_public_bench_target(
        theirs, target.serial, target_fingerprint
    )
    post_boss_hand_count = mine.handCount - 1
    if target_match is None or post_boss_hand_count < 0:
        return None
    route = _boss_exact_attack_route(
        obs, attacker, target, post_boss_hand_count
    )
    ...
    if (
        route is None
        or target_prizes <= 0
        or target_prizes < current_active_gain
        or (post_ko_prizes != 0 and mine.deckCount <= post_ko_prizes)
    ):
        return None
    bench_index, exact_target = target_match
    return {
        **route,
        "bench_index": bench_index,
        "target": exact_target,
...
def _boss_payability_skill_is_certified_irrelevant(text: str) -> bool:
    """Reject every visible static clause that can change attack payability."""
    normalized = _bridge_retaliation_normalized_text(text)
    if not normalized:
        return True
    markers = (
...
def _boss_resource_tax_text_has_free_self_mobility(text: str) -> bool:
    """Detect public effects that can bypass the certified retreat burden."""
    normalized = _bridge_retaliation_normalized_text(text)
    if not normalized:
        return False
    direct_markers = (
        ...
        "return this pokemon",
        "switch this pokemon with",
        "move this pokemon to your bench",
        "put this pokemon onto your bench",
        "shuffle your active pokemon",
        "return your active pokemon",
        "switch your active pokemon with",
        "move your active pokemon to your bench",
        "put your active pokemon onto your bench",
    )
    if any(marker in normalized for marker in direct_markers):
        return True
    self_destination = (
        "this pokemon" in normalized
        and any(
            marker in normalized
            for marker in (
                "into your deck",
    ...
    active_destination = (
        "your active pokemon" in normalized
        and any(
            marker in normalized
            for marker in (
                "into your deck",
    ...
    return self_destination or active_destination


def _boss_payability_static_fingerprint(state) -> tuple | None:
    """Freeze all in-play public metadata that can alter printed costs."""
    records = []
    protected_serials = []
    for owner, player in enumerate(state.players):
        for pokemon in list(player.active) + list(player.bench):
            ...
            if data is None or data.cardType != CardType.POKEMON:
                return None
            if not all(
                _boss_payability_skill_is_certified_irrelevant(skill.text)
                for skill in (data.skills or [])
            ):
                return None
            protected_serials.extend(
                _bridge_pokemon_component_serials(pokemon)
            )
            records.append(
                (
            ...
            for pre_evolution in pokemon.preEvolution:
                pre_data = card_table.get(pre_evolution.id)
                if (
                    pre_data is None
                    or pre_data.cardType != CardType.POKEMON
                    or getattr(pre_evolution, "playerIndex", None) != owner
                ):
                    return None
            energy_units = _bridge_retaliation_energy_units(pokemon)
            if energy_units is None:
                return None
            for unit, energy in zip(energy_units, pokemon.energyCards):
                energy_data = card_table.get(energy.id)
                if (
                    energy_data is None
                    or energy_data.cardType
                    not in (CardType.BASIC_ENERGY, CardType.SPECIAL_ENERGY)
                    or not all(
                        _boss_payability_skill_is_certified_irrelevant(
                            skill.text
                        )
                        for skill in (energy_data.skills or [])
                    )
                ):
                    return None
                records.append(
                    (
                        ...
                        _bridge_metadata_skill_fingerprint(energy_data),
                    )
                )
            for tool in pokemon.tools:
                ...
                if (
                    tool_data is None
                    or tool_data.cardType != CardType.TOOL
                    or not all(
                        _boss_payability_skill_is_certified_irrelevant(
                            skill.text
                        )
                ...
                records.append(
                    (
                        ...
                        "tool",
                        pokemon.serial,
                        _bridge_card_fingerprint(tool),
                        _bridge_metadata_skill_fingerprint(tool_data),
                    )
                )
    for stadium in state.stadium:
        data = card_table.get(stadium.id)
        if (
            ...
            return None
        protected_serials.append(stadium.serial)
        records.append(
            (
                None,
                "stadium",
...
def _boss_resource_tax_certificate(
    state, target: Pokemon, my_index: int
) -> dict | None:
    opponent_index = 1 - my_index
    if (
        not _bridge_pokemon_is_publicly_complete(target, opponent_index)
        or not _bridge_protected_serials_are_unique(
            state, _bridge_pokemon_component_serials(target)
        )
    ):
        return None
    data = card_table.get(target.id)
    if (
        ...
        or not isinstance(data.retreatCost, int)
        or data.retreatCost <= 0
        or not _bridge_retreat_cost_is_publicly_exact(state, target)
        or not isinstance(data.attacks, list)
        or len(data.attacks) != len(set(data.attacks))
        or any(
            _boss_resource_tax_text_has_free_self_mobility(skill.text)
            for skill in (data.skills or [])
        )
    ):
        return None
    energy_units = _bridge_retaliation_energy_units(target)
    static_fingerprint = _boss_payability_static_fingerprint(state)
    if energy_units is None or static_fingerprint is None:
        return None
    ...
    for attack_id in data.attacks:
        ...
        if (
            ...
            or _boss_resource_tax_text_has_free_self_mobility(
                attack.text
            )
        ):
            return None
    ...
    attached_energy_count = len(target.energyCards)
    if not (
        data.basic
        or data.retreatCost >= 2
        or attached_energy_count >= 1
    ):
        return None
    return {
        "version": _BOSS_TRANSACTION_VERSION,
        "target_fingerprint": _bridge_pokemon_fingerprint(target),
        "target_metadata": (
            target.id,
            bool(data.basic),
            int(data.retreatCost),
            tuple(data.attacks),
            _bridge_metadata_skill_fingerprint(data),
        ),
...
def _boss_higher_prize_near_ko_plan(
    obs: Observation,
    attacker: Pokemon,
    target: Pokemon,
    current_active_gain: int,
) -> dict | None:
    # Powerful Hand's counter placement cannot currently be certified against
    # every public weakness/resistance and static damage modifier.  A false
    # residual is worse than suppressing this optional branch, so fail closed.
    return None
    ...
    if (
        ...
        or not _bridge_pokemon_is_publicly_complete(attacker, my_index)
        or not _bridge_pokemon_is_publicly_complete(target, 1 - my_index)
        or not _powerful_hand_target_is_publicly_clear(state, target)
    ):
        return None
    attack_option = _boss_unique_attack_option(
        obs.select, ATTACK_POWERFUL_HAND
    )
    damage = 20 * (mine.handCount - 1)
    residual_hp = target.hp - damage
    target_prizes = prize_count(target)
    if (
        ...
        or target_prizes < 2
        or target_prizes <= current_active_gain
        or mine.deckCount <= len(mine.prize)
    ):
        return None
    return {
        "attack_id": ATTACK_POWERFUL_HAND,
...
def _boss_certified_threat_target(
    state, parent_target: Pokemon | None, my_index: int
) -> dict | None:
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    if (
        parent_target is None
        or parent_target.id not in BOSS_SINGLETON_LOSS_THREATS
        or not _bridge_pokemon_is_publicly_complete(
            parent_target, 1 - my_index
        )
        or mine.deckCount <= len(mine.prize)
    ):
        return None
    matches = [
        (bench_index, pokemon)
        for bench_index, pokemon in enumerate(theirs.bench)
        if pokemon.serial == parent_target.serial
        and _bridge_pokemon_fingerprint(pokemon)
        == _bridge_pokemon_fingerprint(parent_target)
    ]
    if len(matches) != 1:
        return None
    bench_index, target = matches[0]
    return {
        "bench_index": bench_index,
        "target": target,
        "target_fingerprint": _bridge_pokemon_fingerprint(target),
...
def _start_certified_boss_prize_conversion(
    obs: Observation,
    scores: list[int],
    desc_indices: list[int],
    parent_target: Pokemon | None,
) -> tuple[list[int] | None, bool]:
    """Start one certified Boss transaction or suppress every Boss copy."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    if select.context != SelectContext.MAIN or not desc_indices:
        return None, False

    top_option = select.option[desc_indices[0]]
    top_card = (
        get_card(obs, AreaType.HAND, top_option.index, my_index)
        if top_option.type == OptionType.PLAY
        and isinstance(top_option.index, int)
        and mine.hand is not None
        and 0 <= top_option.index < len(mine.hand)
        else None
    )
    if top_card is None or top_card.id != Boss_Orders:
        return None, False

    all_boss_option_indices = []
    boss_options = []
    for option_index, option in enumerate(select.option):
        if (
        ...
        if card is not None and card.id == Boss_Orders:
            all_boss_option_indices.append(option_index)
        if (
            card is not None
            and card.id == Boss_Orders
            and card.serial > 0
            and getattr(card, "playerIndex", None) == my_index
        ):
            boss_options.append((card.serial, option_index, card))

    snapshot = _boss_public_snapshot(state, my_index)
    active = mine.active[0] if len(mine.active) == 1 else None
    opponent_active = theirs.active[0] if len(theirs.active) == 1 else None
    parent_target_fingerprint = (
        _bridge_pokemon_fingerprint(parent_target)
        if parent_target is not None
        and _bridge_pokemon_is_publicly_complete(parent_target, 1 - my_index)
        else None
    )
    parent_target_is_bench = (
        parent_target is not None
        and parent_target_fingerprint is not None
        and _boss_find_public_bench_target(
            theirs, parent_target.serial, parent_target_fingerprint
        )
        is not None
    )
    start_is_publicly_exact = (
        not _boss_prize_conversion_latch
        ...
        and parent_target_is_bench
        and len(boss_options) >= 1
        and any(
            option_index == desc_indices[0]
            and card.serial == top_card.serial
            for _, option_index, card in boss_options
        )
    )

    plan = None
    branch = None
    current_active_gain = 0
    if start_is_publicly_exact:
        current_active_gain = _boss_current_active_prize_gain(
            obs, active, opponent_active, mine.handCount
        )
        direct_ko_plan = _boss_certified_ko_plan_for_target(
            obs, active, parent_target, current_active_gain
        )
        if direct_ko_plan is not None:
            plan = direct_ko_plan
            branch = "same_turn_ko"
        else:
            certified_ko_candidates = _boss_certified_ko_candidates(
                obs, active, current_active_gain
            )
            inherited_prize_tier = prize_count(parent_target)
            callback_tier_candidates = [
                candidate
                for candidate in certified_ko_candidates
                if candidate["target_prizes"] == inherited_prize_tier
            ]
            plan = (
                callback_tier_candidates[0]
                if len(callback_tier_candidates) >= 2
                else None
            )
            if plan is not None:
                branch = "same_turn_ko"
            else:
                plan = _boss_higher_prize_near_ko_plan(
                    obs,
                    active,
                    parent_target,
                    current_active_gain,
                )
            if plan is not None and branch is None:
                branch = "higher_prize_near_ko"
            elif plan is None:
                plan = _boss_certified_threat_target(
                    state, parent_target, my_index
                )
                if plan is not None:
                    branch = "singleton_threat_control"
                else:
                    resource_tax = (
                        _boss_resource_tax_certificate(
                            state, parent_target, my_index
                        )
                        if current_active_gain == 0
                        and mine.deckCount > len(mine.prize)
                        else None
                    )
                    if resource_tax is not None:
                        plan = {
                            "target": parent_target,
                            "target_fingerprint": parent_target_fingerprint,
                            "target_prizes": prize_count(parent_target),
                            "post_ko_prizes": None,
                            "final_prize": False,
                            "attack_id": None,
                            "damage": None,
                            "residual_hp": None,
                            "resource_tax_certificate": resource_tax,
                        }
                        branch = "resource_tax_control"

    if plan is None:
        for option_index in all_boss_option_indices:
            scores[option_index] = -1
        return None, True

    boss_option_index = desc_indices[0]
    boss_card = top_card
    boss_fingerprint = _bridge_card_fingerprint(boss_card)
    expected_hand = _boss_expected_without_card(
        snapshot["own_hand"], boss_fingerprint
    )
    if (
        expected_hand is None
        or not _bridge_protected_serials_are_unique(
            state,
            tuple(
                sorted(
                    {
                        boss_card.serial,
                        active.serial,
                        opponent_active.serial,
                        parent_target.serial,
                        plan["target"].serial,
                    }
                )
            ),
    ...
    _boss_prize_conversion_latch.update(
        version=_BOSS_TRANSACTION_VERSION,
        stage="await_target",
        branch=branch,
        turn=state.turn,
        player=my_index,
        boss_option_index=boss_option_index,
        boss_fingerprint=boss_fingerprint,
        pre_boss_hand=snapshot["own_hand"],
        expected_post_boss_hand=expected_hand,
        public_snapshot=snapshot,
        stadium_fingerprint=snapshot["stadium"],
        expected_post_boss_discard=_boss_expected_discard(
            snapshot["own_discard"], boss_fingerprint
        ),
        attacker_serial=active.serial,
        attacker_fingerprint=_bridge_pokemon_fingerprint(active),
        current_active_serial=opponent_active.serial,
        current_active_fingerprint=_bridge_pokemon_fingerprint(
            opponent_active
        ),
        current_active_prize_gain=current_active_gain,
        parent_target_serial=parent_target.serial,
        parent_target_fingerprint=parent_target_fingerprint,
        target_serial=plan["target"].serial,
        target_fingerprint=plan["target_fingerprint"],
        target_prizes=plan["target_prizes"],
        post_ko_prizes=plan["post_ko_prizes"],
        final_prize=plan["final_prize"],
        attack_id=plan["attack_id"],
        damage=plan["damage"],
        residual_hp=plan.get("residual_hp"),
        resource_tax_certificate=plan.get("resource_tax_certificate"),
    )
    return [boss_option_index], False


def _certified_boss_prize_conversion_overlay(
    obs: Observation,
) -> list[int] | None:
    if not _boss_prize_conversion_latch:
        return None
    latch = _boss_prize_conversion_latch
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    if (
        latch.get("version") != _BOSS_TRANSACTION_VERSION
        or state.turn != latch.get("turn")
        or my_index != latch.get("player")
    ):
        _clear_boss_prize_conversion_latch()
    ...
    if latch.get("stage") == "await_target":
        ...
        if (
            ...
            or select.minCount != 1
            or select.maxCount != 1
            or not _boss_post_boss_common_matches(
                obs, latch, target_switched=False
            )
        ):
            _clear_boss_prize_conversion_latch()
            return None
        ...
        for option_index, option in enumerate(select.option):
            if (
                ...
                or option.playerIndex != 1 - my_index
                or not isinstance(option.index, int)
                or not 0 <= option.index < len(theirs.bench)
            ):
                _clear_boss_prize_conversion_latch()
                return None
            target = get_card(
        ...
        if len(matches) != 1:
            _clear_boss_prize_conversion_latch()
            return None
        latch["stage"] = "await_post_switch"
        return [matches[0]]

    if latch.get("stage") == "await_post_switch":
        if (
            select.context != SelectContext.MAIN
            or not _boss_post_boss_common_matches(
                obs, latch, target_switched=True
            )
        ):
            _clear_boss_prize_conversion_latch()
            return None
        if latch.get("branch") == "singleton_threat_control":
            _clear_boss_prize_conversion_latch()
            return None

        if latch.get("branch") == "resource_tax_control":
            target = theirs.active[0] if len(theirs.active) == 1 else None
            certificate = (
                _boss_resource_tax_certificate(state, target, my_index)
                if target is not None
                else None
            )
            if certificate != latch.get("resource_tax_certificate"):
                _clear_boss_prize_conversion_latch()
                return None
            _clear_boss_prize_conversion_latch()
            return None
        ...
        if (
            ...
            _clear_boss_prize_conversion_latch()
            return None
        if latch.get("branch") == "higher_prize_near_ko":
            attack_option = _boss_unique_attack_option(
                select, ATTACK_POWERFUL_HAND
            )
            ...
            if (
                ...
                or not _powerful_hand_target_is_publicly_clear(state, target)
                or damage != latch.get("damage")
                or residual_hp != latch.get("residual_hp")
                or residual_hp <= 0
                or residual_hp > 20
                or target_prizes != latch.get("target_prizes")
                or target_prizes < 2
                or target_prizes
                <= latch.get("current_active_prize_gain", 0)
                or mine.deckCount <= len(mine.prize)
            ):
                _clear_boss_prize_conversion_latch()
                return None
        ...
        route = _boss_exact_attack_route(
            obs, attacker, target, mine.handCount
        )
        target_prizes = prize_count(target)
        post_ko_prizes = max(0, len(mine.prize) - target_prizes)
        if (
            route is None
            or route["attack_id"] != latch.get("attack_id")
            or route["damage"] != latch.get("damage")
            or target_prizes != latch.get("target_prizes")
            or target_prizes < latch.get("current_active_prize_gain", 0)
            or post_ko_prizes != latch.get("post_ko_prizes")
            or (post_ko_prizes != 0 and mine.deckCount <= post_ko_prizes)
        ):
            _clear_boss_prize_conversion_latch()
            return None
        latch["stage"] = "await_resolution"
        return [route["attack_option"]]
...
def _start_fez_ko_bridge(
    ...
    if (
        ...
        or _boss_prize_conversion_latch
...
def agent(obs_dict: dict) -> list[int]:
    ...
    emergency_action = _certified_boss_prize_conversion_overlay(obs)
    if emergency_action is not None:
    ...
    boss_action, boss_suppressed = _start_certified_boss_prize_conversion(
        obs,
        scores,
        desc_indices,
        target_pokemon,
    ...
    if boss_action is not None:
        return _remember_action(_decision_signature(obs), boss_action)
    if boss_suppressed:
        desc_indices = [
            i
            for i, _ in sorted(
    ...
    if (
        ...
        and not _enriching_reserve_latch
        and not _fez_ko_bridge_latch
        and not _boss_prize_conversion_latch
    ):
