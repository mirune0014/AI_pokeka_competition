...
# Candidate C is deliberately a single, narrow transaction.  It does not
# replace the accepted parent's scorer and it never guesses a hidden deck
# card.  Its only purpose is to ask whether a second Archaludon line can be
# made materially more developed with one exact public action before the
# current turn is closed.
_TWO_LINE_ID = "TWO_LINE_DEVELOPMENT_COMMITMENT_V2"
_TWO_LINE_OWNER = "TWO_LINE_DEVELOPMENT"


def _two_line_owner_view(owner):
    if not isinstance(owner, dict):
        return None
    target = owner.get("target_ref")
    return {
        "owner": owner.get("owner"),
        "stage": owner.get("stage"),
        "objective_type": owner.get("objective_type"),
        "target_serial": target[1] if isinstance(target, tuple) else None,
        "action_role": owner.get("action_role"),
    }


def _two_line_attackable_exact(obs, rows):
    """Return True only when the current Active has an exact public KO."""
    try:
        seat = obs.current.yourIndex
        mine = obs.current.players[seat]
        theirs = obs.current.players[1 - seat]
        if not isinstance(mine.active, list) or len(mine.active) != 1:
            return False
        if not isinstance(theirs.active, list) or len(theirs.active) != 1:
            return False
        attacker = mine.active[0]
...
def _two_line_developed(obs, rows):
    """Check the three public SECOND_LINE_DEVELOPED definitions."""
    try:
        seat = obs.current.yourIndex
        mine = obs.current.players[seat]
        bench = getattr(mine, "bench", None)
        ...
        for pokemon in bench:
            if pokemon is None:
                continue
            metal_refs = _exact_metal_energy_refs(pokemon, seat)
            if metal_refs is None:
                return False
            if pokemon.id in {_ARCHALUDON_EX, _ARCHALUDON} and len(metal_refs) >= 1:
                return True
            if pokemon.id == _DURALUDON and len(metal_refs) >= 1:
                for row in rows:
                    role = row[2]
                    if (
                        row[1].type == _OptionType.EVOLVE
                        and role[1] is not None
                        and role[1][0] in {_ARCHALUDON_EX, _ARCHALUDON}
                        and role[4] == (pokemon.id, pokemon.serial, seat)
                    ):
                        return True
            if (
                pokemon.id in {_ARCHALUDON_EX, _ARCHALUDON}
                and bool(getattr(pokemon, "appearThisTurn", False))
                and any(
                    getattr(base, "id", None) == _DURALUDON
                    for base in getattr(pokemon, "preEvolution", ())
                )
            ):
                return True
    except Exception:
...
def _two_line_candidates(obs, rows, target_serial=None):
    """Return at most one exact direct materialization action per priority."""
    seat = obs.current.yourIndex
    mine = obs.current.players[seat]
    bench = getattr(mine, "bench", None)
    if not isinstance(bench, list):
        return None, "invalid_bench"
    board_refs = {
        (pokemon.id, pokemon.serial, seat)
        for pokemon in bench
    ...
    arch_refs = {
        (pokemon.id, pokemon.serial, seat)
        for pokemon in bench
        if pokemon is not None and pokemon.id in {_ARCHALUDON_EX, _ARCHALUDON}
    }
    dura_refs = {
        (pokemon.id, pokemon.serial, seat)
        for pokemon in bench
    ...
    evolves = [
        ...
        if (
            row[1].type == _OptionType.EVOLVE
            and row[2][1] is not None
            and row[2][1][0] in {_ARCHALUDON_EX, _ARCHALUDON}
            and row[2][4] in dura_refs
        )
    ]
    if len(evolves) == 1:
        return evolves[0], "EVOLVE"
    if len(evolves) > 1:
        return None, "ambiguous_evolution"

    attaches = [
        row
        for row in rows
        if (
            row[1].type == _OptionType.ATTACH
            and row[2][1] is not None
            and row[2][1][0] == _METAL_ENERGY
            and row[2][4] in arch_refs
        )
    ]
    if len(attaches) == 1:
        return attaches[0], "ATTACH"
    if len(attaches) > 1:
        return None, "ambiguous_archaludon_attach"

    if len(bench) >= getattr(mine, "benchMax", 0):
        return None, "bench_full"
    plays = [
        row
        for row in rows
    ...
    if target_serial is not None:
        plays = [row for row in plays if row[2][1][1] == target_serial]
    if len(plays) == 1:
        return plays[0], "BENCH"
    if len(plays) > 1:
        return None, "ambiguous_duraludon_bench"
    # Poke Pad / Ultra Ball / Explorer are intentionally not started from a
    # mere card-id match.  Their target, discard cost, and post-search line
    # are hidden or multi-prompt; without an exact public continuation they
    # must remain the parent's decision.
    return None, "no_exact_direct_objective"


def _two_line_prompt(obs):
    return _prompt_fingerprint(obs)


def _two_line_proposal(obs, row, objective_type, proof, owner):
    action = [row[0]]
    owner["action_role"] = row[2]
    owner["last_prompt"] = _two_line_prompt(obs)
    owner["last_order"] = tuple(item[2] for item in _option_rows(obs) or ())
    owner["stage"] = "MATERIALIZE_SECOND_LINE"
    return _make_proposal(
        action,
        objective_type,
        proof,
        _two_line_owner_view(owner),
        rule_id=_TWO_LINE_ID,
        category="TWO_LINE_DEVELOPMENT",
    )


def _two_line_start(obs, parent_action):
    global _two_line_owner
    gates = {
        "main": _exact_main(obs),
        "result_open": getattr(obs.current, "result", None) == -1,
        "owner_empty": _two_line_owner is None,
        "remaining_prizes_at_least_two": False,
        "terminal_win_absent": True,
        "current_exact_ko_absent": False,
        "lillie_safety_absent": True,
        "second_line_not_developed": False,
    }
    if not gates["main"] or not gates["result_open"] or not gates["owner_empty"]:
        return None, "entry_gate_failed", gates, False, False
    seat = getattr(obs.current, "yourIndex", None)
    mine = getattr(obs.current, "players", [None, None])[seat]
    prizes = getattr(mine, "prize", None)
    gates["remaining_prizes_at_least_two"] = isinstance(prizes, list) and len(prizes) >= 2
    if not gates["remaining_prizes_at_least_two"]:
        return None, "one_prize_or_invalid", gates, False, False
    rows = _option_rows(obs)
    if rows is None:
        return None, "option_bindings_unknown", gates, False, False
    # The accepted parent's terminal resolver remains the authority for a
    # proven win.  A non-None result here is never replaced by development.
    direct = _start_direct_current_win(obs, parent_action)
    if direct[0] is not None or direct[1] == "multiple_terminal_attack_ids":
        gates["terminal_win_absent"] = False
        return None, "terminal_parent_priority", gates, False, False
    gates["current_exact_ko_absent"] = not _two_line_attackable_exact(obs, rows)
    if not gates["current_exact_ko_absent"]:
        return None, "current_exact_ko_priority", gates, False, False
    try:
        gates["lillie_safety_absent"] = not bool(
            _parent.should_prioritize_lillie_against_alakazam(obs)
        )
    except Exception:
        gates["lillie_safety_absent"] = False
    if not gates["lillie_safety_absent"]:
        return None, "lillie_safety_priority", gates, False, False
    gates["second_line_not_developed"] = not _two_line_developed(obs, rows)
    if not gates["second_line_not_developed"]:
        return None, "second_line_already_developed", gates, False, False
    row, reason = _two_line_candidates(obs, rows)
    if row is None:
        return None, reason, gates, False, False
    owner = {
        "owner": _TWO_LINE_OWNER,
        "stage": "BIND_OBJECTIVE",
        "seat": seat,
        "turn": obs.current.turn,
        "target_ref": row[2][4] or row[2][1],
        "objective_type": reason,
        "action_role": row[2],
        "last_prompt": None,
        "last_order": (),
    }
    _two_line_owner = owner
    proof = {
        "gates": dict(gates),
        "objective_type": reason,
        ...
        "search_routes": "fail_closed_without_exact_public_continuation",
    }
    return _two_line_proposal(obs, row, reason, proof, owner), None, proof, False, False


def _two_line_resume(obs, parent_action):
    global _two_line_owner
    owner = _two_line_owner
    if not isinstance(owner, dict) or owner.get("owner") != _TWO_LINE_OWNER:
        _two_line_owner = None
        return None, "owner_missing", {}, False, False
    prompt = _two_line_prompt(obs)
    if prompt is not None and prompt == owner.get("last_prompt"):
        action = _bind_role(obs, owner.get("action_role"))
        if action is None:
            _two_line_owner = None
            return None, "duplicate_rebind_failed", {}, True, False
        rows = _option_rows(obs) or ()
        permuted = tuple(item[2] for item in rows) != owner.get("last_order")
        proof = {
            "objective_type": owner.get("objective_type"),
            "duplicate_rebind": True,
    ...
    if getattr(obs.select, "context", None) != _SelectContext.MAIN:
        _two_line_owner = None
        return None, "objective_prompt_not_exact", {}, False, False
    rows = _option_rows(obs)
    if rows is None:
        _two_line_owner = None
        return None, "objective_options_unknown", {}, False, False
    if _two_line_developed(obs, rows):
        _two_line_owner = None
        return None, "second_line_developed_parent_fallback", {
            "stage": "ATTACK_OR_FALLBACK",
        }, False, False
    row, reason = _two_line_candidates(
        obs,
        rows,
        target_serial=(
            owner.get("target_ref", (None, None, None))[1]
            if isinstance(owner.get("target_ref"), tuple)
            else None
        ),
    )
    if row is None:
        _two_line_owner = None
        return None, "objective_unavailable_parent_fallback:" + reason, {}, False, False
    # A changed target is not a new search: keep the transaction only when
    # the same physical target remains visible.
    target_ref = owner.get("target_ref")
    if target_ref is not None and row[2][4] not in {None, target_ref} and row[2][1] != target_ref:
        _two_line_owner = None
        return None, "target_changed_parent_fallback", {}, False, False
    owner["objective_type"] = reason
    owner["target_ref"] = row[2][4] or row[2][1]
    proof = {
        "objective_type": reason,
        "continuation": True,
        "target_ref": owner["target_ref"],
...
def _resolve(obs, parent_action):
    global _setup_ledger, _materialization_owner, _two_line_owner
    if obs.select is None or obs.current is None:
        _setup_ledger = None
        _materialization_owner = None
        _two_line_owner = None
        return None, "deck_request", {}, False, False
    if _two_line_owner is not None:
        return _two_line_resume(obs, parent_action)
    ...
    if context == _SelectContext.MAIN:
        ...
        if direct[1] == "multiple_terminal_attack_ids":
            return direct
        two_line = _two_line_start(obs, parent_action)
        if two_line[0] is not None:
            return two_line
...
def _emit_telemetry(
    ...
    active_owner = _two_line_owner if isinstance(_two_line_owner, dict) else _materialization_owner
    ...
    _last_telemetry = {
        ...
        "owner_after": (
            _two_line_owner_view(active_owner)
            if active_owner is _two_line_owner and isinstance(active_owner, dict)
            else _owner_view(active_owner)
        ),
        "parent_call_count": 1,
    }
...
def agent(obs_dict):
    """Call exact Silver once, then resolve the accepted parent or Candidate C."""
    global _last_proposal, _materialization_owner, _two_line_owner
