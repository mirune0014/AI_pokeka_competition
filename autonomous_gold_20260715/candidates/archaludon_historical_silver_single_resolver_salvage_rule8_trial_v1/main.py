"""Exact Historical-Silver plus isolated Rules 1, 4, 5, and 8.
...
_RULE8_ID = "PUBLIC_EXACT_SAME_ACTIVE_ATTACK_DOMINANCE_V1"
_DURALUDON = 169
_ARCHALUDON_EX = 190
_CINDERACE = 666
_ARCHALUDON = 840
...
def _rule8_zero_consequences_exact():
    expected_fields = {"attackId", "name", "text", "damage", "energies"}
    attacks = getattr(_parent, "ALL_ATTACKS", {})
    for attack_id in (223, 224):
        attack = attacks.get(attack_id)
        if (
            attack is None
            or set(vars(attack)) != expected_fields
            or getattr(attack, "attackId", None) != attack_id
            or not isinstance(getattr(attack, "energies", None), list)
            or not _attack_metadata_exact(attack_id)
        ):
            return None
    return {
        "purpose": {223: "DAMAGE_ONLY", 224: "DAMAGE_ONLY"},
        "recoil": {223: 0, 224: 0},
        "self_damage": {223: 0, 224: 0},
        "energy_discard": {223: 0, 224: 0},
        "energy_consumption": {223: 0, 224: 0},
        "additional_side_effects": {223: (), 224: ()},
    }


def _rule8_public_serials_unique(obs):
    state = obs.current
    seat = state.yourIndex
    serials = []
    ...
    for card in state.players[seat].hand:
        if not add_ref(_exact_card_ref(card, seat)):
            return False
    for owner, player in ((seat, state.players[seat]), (1 - seat, state.players[1 - seat])):
        for card in player.discard:
            if not add_ref(_exact_card_ref(card, owner)):
                return False
        for pokemon in tuple(player.active) + tuple(player.bench):
            fingerprint = _pokemon_fingerprint(pokemon, owner)
            if fingerprint is None:
                return False
            serials.append(fingerprint[1])
            for ref in fingerprint[6] + fingerprint[7] + fingerprint[8]:
                if not add_ref(ref):
                    return False
    for card in state.stadium:
        if not add_ref(_exact_card_ref(card)):
            return False
    return len(serials) == len(set(serials))


def _rule8_target_fields_exact(target):
    data = getattr(_parent, "CARD_DB", {}).get(getattr(target, "id", None))
    valid_energy_values = {int(value) for value in _EnergyType}

    def exact_optional_energy(value):
        return value is None or (
            _is_exact_int(value) and value in valid_energy_values
        )

    return bool(
        data is not None
        and _is_exact_int(getattr(data, "hp", None))
        and data.hp > 0
        and _is_exact_int(getattr(data, "energyType", None))
        and data.energyType in valid_energy_values
        and exact_optional_energy(getattr(data, "weakness", None))
        and exact_optional_energy(getattr(data, "resistance", None))
        and isinstance(getattr(data, "skills", None), list)
        and isinstance(getattr(data, "attacks", None), list)
        and isinstance(getattr(data, "ex", None), bool)
        and isinstance(getattr(data, "megaEx", None), bool)
    )


def _rule8_pareto_proof(parent_result, candidate_result, target):
    if (
        not isinstance(parent_result, tuple)
        or len(parent_result) != 3
        or not isinstance(candidate_result, tuple)
        or len(candidate_result) != 3
        or any(
            not _is_exact_int(value) or value < 0
            for value in parent_result + candidate_result
        )
        or parent_result[1] != candidate_result[1]
        or not _is_exact_int(getattr(target, "hp", None))
        or target.hp <= 0
    ):
        return None
    parent_dimensions = (
        parent_result[0],
        int(parent_result[0] >= target.hp),
        parent_result[2],
    )
    candidate_dimensions = (
        candidate_result[0],
        int(candidate_result[0] >= target.hp),
        candidate_result[2],
    )
    if not all(
        candidate >= parent
        for parent, candidate in zip(parent_dimensions, candidate_dimensions)
    ) or not any(
        candidate > parent
        for parent, candidate in zip(parent_dimensions, candidate_dimensions)
    ):
        return None
    names = ("final_damage", "ko", "prizes_taken")
    return {
        "parent": dict(zip(names, parent_dimensions)),
        "candidate": dict(zip(names, candidate_dimensions)),
        "strict_dimensions": tuple(
            name
            for name, parent, candidate in zip(
                names, parent_dimensions, candidate_dimensions
            )
            if candidate > parent
        ),
        "printed_prize": parent_result[1],
    }


def _rule8_proposal(action, proof):
    return _make_proposal(
        action,
        "SAME_ACTIVE_EXACT_PARETO_ATTACK_DOMINANCE",
        proof,
        None,
        rule_id=_RULE8_ID,
        category="EXACT_SAME_ACTIVE_ATTACK_DOMINANCE",
    )
...
def _start_same_active_attack_dominance(obs, parent_action):
    gates = {
        "normal_main": _exact_main(obs),
        "owner_empty": _materialization_owner is None,
    }
    if not all(gates.values()):
        return None, "dominance_entry_gate_failed", gates, False, False
    rows = _option_rows(obs)
    snapshot = _state_snapshot(obs)
    gates["option_bindings_exact"] = rows is not None
    gates["public_board_exact"] = snapshot is not None
    if rows is None or snapshot is None:
        return None, "dominance_public_binding_failed", gates, False, False
    gates["public_serials_unique"] = _rule8_public_serials_unique(obs)
    if not gates["public_serials_unique"]:
        return None, "dominance_duplicate_public_serial", gates, False, False

    parent = _parent_registered_attack(obs, parent_action, rows)
    gates["parent_unique_hammer"] = bool(
        parent is not None and parent[2][5] == 223
    )
    if not gates["parent_unique_hammer"]:
        return None, "dominance_parent_not_unique_hammer", gates, False, False
    ...
    gates["same_unique_active_binding"] = bool(
        attacker_fp is not None
        and attacker_fp[0] == _DURALUDON
        and target_fp is not None
    )
    if not gates["same_unique_active_binding"]:
        return None, "dominance_active_binding_failed", gates, False, False

    canonical_roles = {
        attack_id: (
            int(_OptionType.ATTACK),
            None,
            None,
            None,
            None,
    ...
    attacks = {
        attack_id: [
            row
            for row in rows
            if row[2] == canonical_roles[attack_id]
            and _attack_option_exact(row[1])
        ]
        for attack_id in (223, 224)
    }
    gates["unique_legal_hammer"] = len(attacks[223]) == 1
    gates["unique_legal_raging_hammer"] = len(attacks[224]) == 1
    if not gates["unique_legal_hammer"] or not gates["unique_legal_raging_hammer"]:
        return None, "dominance_attack_options_not_unique", gates, False, False
    if attacks[223][0] != parent:
        return None, "dominance_parent_hammer_binding_mismatch", gates, False, False

    energies = _exact_metal_energy_refs(attacker, seat)
    payable = None
    if energies is not None:
        payable = _printed_attack_payable_with_metals(attacker.id, len(energies))
    gates["public_energy_exact"] = bool(
        energies is not None
        and payable is not None
        and 223 in payable
        and 224 in payable
    )
    if not gates["public_energy_exact"]:
        return None, "dominance_energy_or_cost_unknown", gates, False, False

    consequences = _rule8_zero_consequences_exact()
    gates["printed_pair_and_zero_consequences_exact"] = consequences is not None
    gates["target_fields_exact"] = _rule8_target_fields_exact(target)
    if consequences is None or not gates["target_fields_exact"]:
        return None, "dominance_metadata_or_consequence_unknown", gates, False, False

    parent_result = _exact_damage_and_take(obs, attacker, target, 223)
    candidate_result = _exact_damage_and_take(obs, attacker, target, 224)
    gates["exact_outcomes_known"] = bool(
        parent_result is not None and candidate_result is not None
    )
    if not gates["exact_outcomes_known"]:
        return None, "dominance_outcome_unknown", gates, False, False
    pareto = _rule8_pareto_proof(parent_result, candidate_result, target)
    gates["strict_pareto"] = pareto is not None
    if pareto is None:
        return None, "dominance_not_strict_pareto", gates, False, False
    ...
    selected_semantic = parent_semantic[:-1] + (224,)
    proof = {
        "seat": seat,
        "turn": state.turn,
        "action_count": state.turnActionCount,
        "attacker": attacker_fp,
        "target": target_fp,
        "parent_semantic": parent_semantic,
        "selected_semantic": selected_semantic,
        "energy_refs": energies,
        "printed_costs": {223: _EXPECTED_ATTACKS[223][3], 224: _EXPECTED_ATTACKS[224][3]},
        "zero_consequences": consequences,
        "outcomes": pareto,
        "gates": dict(gates),
    }
    action = [attacks[224][0][0]]
    return _rule8_proposal(action, proof), None, proof, False, False
...
def _resolve(obs, parent_action):
    ...
    if context == _SelectContext.MAIN:
        ...
        if boss[0] is not None:
            return boss
        dominance = _start_same_active_attack_dominance(obs, parent_action)
        if dominance[0] is not None:
            return dominance
        return dominance
...
def agent(obs_dict):
    """Call exact Silver once, then resolve Rule 1, Rule 4, Rule 5, or Rule 8."""
