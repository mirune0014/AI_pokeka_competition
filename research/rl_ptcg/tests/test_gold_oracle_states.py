...
from research.rl_ptcg.gold_oracle_states import (
    SCHEMA_VERSION,
    _resolve_bound_path,
    _selection_inputs,
    build_beliefs,
...
from infrastructure.tools.build_gold_oracle_states import parse_extra_decks
...
class GoldOracleStateTests(unittest.TestCase):
    ...
    def test_inventory_source_binding_accepts_cross_platform_legacy_path(self):
        bound = "_local_generated/analysis_outputs/inventory/all_seats.csv"
        stored = {
        ...
        broken = dict(
            stored,
            source_path="C:\\repo\\not_analysis_outputs\\inventory\\all_seats.csv",
        )
    ...
    def test_late_incompatible_same_archetype_donor_can_make_synthetic(self):
        ...
        with patch("research.rl_ptcg.gold_oracle_states.sample_search_guess"):
            belief = build_beliefs(
                [base, donor],
                "alakazam",
                Counter({99: 1}),
    ...
    def test_baseline_is_in_top3_and_diverse_candidate_is_tagged(self):
        ...
        with patch("research.rl_ptcg.gold_oracle_states.canonicalize_prompt_action") as canonicalize:
    ...
    def test_rank_target_uses_exact_4096_cap(self):
        with patch("research.rl_ptcg.gold_oracle_states.rank_complete_actions", return_value={}) as rank:
    ...
    def test_selection_hash_binds_all_extra_decks_and_required_inputs(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for path in (dataset, audit, baseline, engine / "cg", root / "infrastructure" / "tools"):
            ...
            with patch("research.rl_ptcg.gold_oracle_states.__file__", str(root / "collector.py")):
                (root / "collector.py").write_text("collector", encoding="ascii")
                (root / "infrastructure" / "tools" / "build_gold_oracle_states.py").write_text("cli", encoding="ascii")
