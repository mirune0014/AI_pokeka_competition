...
_BOSS_TRANSACTION_VERSION = "certified_boss_prize_conversion_transaction_v1"
...
def _start_certified_boss_prize_conversion(
    ...
    if start_is_publicly_exact:
        ...
        ko_candidates = _boss_certified_ko_candidates(
            obs, active, current_active_gain
        )
        if (
            ko_candidates
            and ko_candidates[0]["target"].serial == parent_target.serial
            and ko_candidates[0]["target_fingerprint"]
            == parent_target_fingerprint
        ):
            plan = ko_candidates[0]
            branch = "same_turn_ko"
        else:
    ...
    _, boss_option_index, boss_card = min(
        boss_options, key=lambda item: (item[0], item[1])
    )
