"""Focused public-state regression checks for Cynthia Garchomp v35."""
...
AGENT_DIR = ROOT / "opponents" / "meta_agents" / "cynthia_garchomp_nasuo445_v35_reliable_development_before_attack"
...
def load_agent_module():
    ...
    spec = importlib.util.spec_from_file_location("cynthia_v35_main", AGENT_DIR / "main.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class ReliableDevelopmentBeforeAttackTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_exact_60_card_deck_equality(self):
        v34 = ROOT / "opponents" / "meta_agents" / "cynthia_garchomp_nasuo445_v34_min_energy_development_before_attack" / "deck.csv"
        deck = (AGENT_DIR / "deck.csv").read_bytes()
        self.assertEqual(deck, v34.read_bytes())
    ...
    def test_gong_overrides_only_with_empty_bench_and_four_or_five_prizes(self):
        a = self.agent
        attack = self.option(a.OptionType.ATTACK, attack_id=a.CORKSCREW_DIVE)
        gong = self.option(a.OptionType.PLAY, area="hand", index=0)
        for prizes in (4, 5):
            obs = self.obs([gong, attack], [types.SimpleNamespace(id=a.FIGHTING_GONG)], active_energies=(a.BASIC_FIGHTING,), prizes=prizes)
            a.best_damage_for_active = lambda *_args: 0
            rows = self.scored(obs, 1, action_score=48600)
            self.assertTrue(a.is_direct_backup_action(obs, gong))
            self.assertEqual(a.backup_readiness_before_attack_index(obs, rows), 0)
            self.assertEqual(a.choose_options(obs), [0])

        for prizes, bench in ((6, ()), (2, ()), (4, (self.pokemon(a.ROSELIA),))):
            obs = self.obs([gong, attack], [types.SimpleNamespace(id=a.FIGHTING_GONG)], active_energies=(a.BASIC_FIGHTING,), prizes=prizes, bench=bench)
            a.best_damage_for_active = lambda *_args: 0
            rows = self.scored(obs, 1, action_score=48600)
            self.assertFalse(a.is_direct_backup_action(obs, gong))
            self.assertIsNone(a.backup_readiness_before_attack_index(obs, rows))
            self.assertEqual(a.choose_options(obs), [1])

    def test_direct_gible_is_not_forced_before_attack(self):
        a = self.agent
        gible = self.option(a.OptionType.PLAY, area="hand", index=0)
        attack = self.option(a.OptionType.ATTACK, attack_id=a.CORKSCREW_DIVE)
        obs = self.obs([gible, attack], [types.SimpleNamespace(id=a.GIBLE)], active_energies=(a.BASIC_FIGHTING,))
        a.best_damage_for_active = lambda *_args: 0
        rows = self.scored(obs, 1, action_score=48600)
        self.assertFalse(a.is_direct_backup_action(obs, gible))
        self.assertIsNone(a.backup_readiness_before_attack_index(obs, rows))
        self.assertEqual(a.choose_options(obs), [1])
    ...
    def test_non_one_energy_active_states_are_inactive(self):
        a = self.agent
        gong = self.option(a.OptionType.PLAY, area="hand", index=0)
        attack = self.option(a.OptionType.ATTACK, attack_id=a.CORKSCREW_DIVE)
