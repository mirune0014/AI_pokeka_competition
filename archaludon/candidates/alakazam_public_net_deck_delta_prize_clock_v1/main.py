...
def _exact_v3_parent_agent(obs_dict: dict) -> list[int]:
...
# ---------------------------------------------------------------------------
# Public net-deck-delta prize-clock overlay.
#
# The frozen exact-v3 policy above is intentionally unchanged apart from its
# private entry-point name.  The public ``agent`` below always computes that
# exact action first.  It may then replace the action in only one of three
# optional families: Alakazam Psychic Draw, exposed Lucky Helmet attachment,
# or serial-exact positive-net Dudunsparce Run Away Draw.
# ---------------------------------------------------------------------------

_net_clock_last_observation = None
_net_clock_last_action = None


def _net_clock_snapshot_parent_state() -> dict:
    """Copy every mutable exact-v3 policy global before a speculative run."""
    return {
        "pre_turn": pre_turn,
...
def _net_clock_restore_parent_state(snapshot: dict) -> None:
    """Restore one exact-v3 snapshot without replacing latch identities."""
    global pre_turn, ability_used_dudunsparce, ability_used_fezandipiti
    global _last_decision_signature, _last_decision_action
...
def _net_clock_remember(observation: dict, action: list[int]) -> list[int]:
    """Make an identical repeated callback idempotent at the wrapper boundary."""
    global _net_clock_last_observation, _net_clock_last_action
    _net_clock_last_observation = copy.deepcopy(observation)
    _net_clock_last_action = tuple(action)
    return list(action)


def _net_clock_card_is_exact(card, player_index: int) -> bool:
    return (
        card is not None
...
def _net_clock_pokemon_is_exact(pokemon, player_index: int, state) -> bool:
    if (
        pokemon is None
        # The checked API's Pokemon dataclass has no playerIndex field; its
        # ownership is established by the exact Active/Bench container.
    ...
    if not all(_net_clock_card_is_exact(card, player_index) for card in components):
        return False
    if not all(
        card_table[card.id].cardType
        in (CardType.BASIC_ENERGY, CardType.SPECIAL_ENERGY)
    ...
    serials = [pokemon.serial] + [card.serial for card in components]
    return (
        len(serials) == len(set(serials))
        and _bridge_protected_serials_are_unique(state, serials)
    )


def _net_clock_stage_stack_is_exact(pokemon, player_index: int, state) -> bool:
    """Validate the complete public evolution stack for this deck's lines."""
    if not _net_clock_pokemon_is_exact(pokemon, player_index, state):
        return False
    ...
    if pokemon.id == Kadabra:
        return pre_ids == [Abra]
    if pokemon.id == Alakazam:
        # Rare Candy legally exposes Abra directly under the Stage 2.
    ...
    return data is not None and (
        (data.basic and not pre_ids)
        or ((data.stage1 or data.stage2) and bool(pre_ids))
    )

def _net_clock_tight(mine) -> bool:
    prizes = len(mine.prize)
    return (
        isinstance(mine.deckCount, int)
        and mine.deckCount >= 0
        and 1 <= prizes <= 6
        and mine.deckCount <= 3 * prizes + 2
    )


def _net_clock_powerful_hand_ko(obs: Observation) -> dict | None:
    """Certify an immediate public Powerful Hand KO with no unknown draw."""
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    ...
    if (
        not _net_clock_stage_stack_is_exact(active, my_index, state)
        or active.id != Alakazam
        or ATTACK_POWERFUL_HAND not in (card_table[Alakazam].attacks or [])
        or not any(card.id in PSYCHIC_ENERGY_IDS for card in active.energyCards)
        or not _net_clock_stage_stack_is_exact(target, 1 - my_index, state)
        or not _powerful_hand_target_is_publicly_clear(state, target)
        or any(not _net_clock_card_is_exact(card, my_index) for card in mine.hand)
        or target.hp > 20 * mine.handCount
    ):
        return None
    hand_serials = [card.serial for card in mine.hand]
    if (
        len(hand_serials) != len(set(hand_serials))
        or not _bridge_protected_serials_are_unique(state, hand_serials)
    ):
        return None
    target_prizes = prize_count(target)
    if target_prizes <= 0:
...
def _net_clock_independent_backup(
    obs: Observation, *, primary_serial: int, excluded_serial: int | None = None
) -> dict | None:
    """Choose a distinct, public attacker-formation witness.

    An already evolved Alakazam is a complete backup formation.  A public
    Kadabra plus a serial-exact Alakazam in hand is the only allowed staged
    route.  No card identity is assigned to a future draw.
    """
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    hand_alakazams = []
    if mine.hand is not None and len(mine.hand) == mine.handCount:
        hand_alakazams = sorted(
            (
                card
                for card in mine.hand
                if card.id == Alakazam
                and _net_clock_card_is_exact(card, my_index)
            ),
            key=lambda card: card.serial,
        )

    candidates = []
    for area_rank, pokemon in (
        [(0, card) for card in mine.active if card is not None]
        + [(1, card) for card in mine.bench]
    ):
        if (
            pokemon.serial in (primary_serial, excluded_serial)
            or not _net_clock_stage_stack_is_exact(pokemon, my_index, state)
        ):
            continue
        if pokemon.id == Alakazam:
            rank = 0 if any(
            ...
            resource = None
        elif pokemon.id == Kadabra and hand_alakazams:
            rank = 2
            resource = hand_alakazams[0]
            if resource.serial in (primary_serial, excluded_serial, pokemon.serial):
                continue
        else:
            continue
        candidates.append((rank, area_rank, pokemon.serial, pokemon, resource))
    if not candidates:
        return None
    rank, _, _, pokemon, resource = min(candidates, key=lambda row: row[:3])
    return {
        "rank": rank,
        "serial": pokemon.serial,
        "fingerprint": _bridge_pokemon_fingerprint(pokemon),
        "resource": None if resource is None else _bridge_card_fingerprint(resource),
    }


def _net_clock_terminal_attack(obs: Observation) -> bool:
    certificate = _net_clock_powerful_hand_ko(obs)
    return certificate is not None and certificate["terminal"]


def _net_clock_psychic_draw_no(
    obs: Observation, parent_action: list[int]
) -> int | None:
    """Return the unique NO option for a certified optional Alakazam draw."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if (
        ...
        or context_card.id != Alakazam
        or not _net_clock_card_is_exact(context_card, my_index)
        or select.minCount != 1
        or select.maxCount != 1
        or len(select.option) != 2
        or not _net_clock_tight(mine)
    ):
        return None
    ...
    matching_sources = [
        ...
        if pokemon is not None
        and pokemon.id == Alakazam
        and pokemon.serial == context_card.serial
        and _net_clock_stage_stack_is_exact(pokemon, my_index, state)
    ]
    if len(matching_sources) != 1:
        return None
    attack = _net_clock_powerful_hand_ko(obs)
    if attack is None or attack["terminal"]:
        return None
    backup = _net_clock_independent_backup(
        obs, primary_serial=attack["active_serial"]
    )
    if backup is None:
        return None
    return no[0]
...
def _net_clock_dudunsparce_return_count(
    state, pokemon, player_index: int
) -> int | None:
    """Return serial-distinct full stack + Energy + Tool card count R."""
    if (
        pokemon.id != Dudunsparce
        or not _net_clock_stage_stack_is_exact(pokemon, player_index, state)
        or len(pokemon.preEvolution) != 1
        or pokemon.preEvolution[0].id != Dunsparce
    ):
        return None
...
def _net_clock_run_away_candidates(obs: Observation) -> list[tuple] | None:
    """Resolve every legal Dudunsparce Ability option without option-order ties."""
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    if select.context != SelectContext.MAIN or select.minCount != 1 or select.maxCount != 1:
        return None
    rows = []
    seen_sources = set()
    for option_index, option in enumerate(select.option):
        ...
        if (
            ...
            return None
        source_area = mine.active if option.area == AreaType.ACTIVE else mine.bench
        if option.index >= len(source_area):
            return None
        source = source_area[option.index]
        if source is None:
            return None
        if source.id != Dudunsparce:
            continue
        if (
            option.cardId not in (None, Dudunsparce)
            or option.serial not in (None, source.serial)
            or source.serial in seen_sources
        ):
            return None
        returned = _net_clock_dudunsparce_return_count(state, source, my_index)
        if returned is None:
            return None
        seen_sources.add(source.serial)
        rows.append(
            (
                returned,
                source.serial,
...
def _net_clock_positive_run_away(obs: Observation) -> int | None:
    """Choose the greatest-R legal positive-net Dudunsparce, if fully safe."""
    state = obs.current
    mine = state.players[state.yourIndex]
    if not _net_clock_tight(mine):
        return None
    attack = _net_clock_powerful_hand_ko(obs)
    if attack is None or attack["terminal"]:
        return None
    rows = _net_clock_run_away_candidates(obs)
    if not rows:
        return None
    pokemon_count = len([card for card in mine.active if card is not None]) + len(mine.bench)
    if pokemon_count <= 1:
        return None

    safe = []
    for returned, serial, area, source_index, option_index, source in rows:
        projected = mine.deckCount + returned - 3
        if (
            returned <= 3
            or projected <= len(mine.prize)
            or serial == attack["active_serial"]
        ):
            continue
        backup = _net_clock_independent_backup(
            obs,
            primary_serial=attack["active_serial"],
            excluded_serial=serial,
        )
        if backup is None:
            continue
        safe.append(
            (-returned, serial, area, source_index, option_index, source, backup)
        )
    if not safe:
        return None
    return min(safe, key=lambda row: row[:4])[4]


def _net_clock_lucky_option_rows(obs: Observation) -> list[tuple] | None:
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    seen = set()
    for option_index, option in enumerate(select.option):
        if option.type != OptionType.ATTACH or option.area != AreaType.HAND:
            continue
        if not isinstance(option.index, int) or option.index < 0 or option.index >= len(mine.hand):
            return None
        card = mine.hand[option.index]
        if card.id != Lucky_Helmet:
            continue
        if (
            not _net_clock_card_is_exact(card, my_index)
            or option.inPlayArea not in (AreaType.ACTIVE, AreaType.BENCH)
            or not isinstance(option.inPlayIndex, int)
            or option.inPlayIndex < 0
        ):
            return None
        area_cards = mine.active if option.inPlayArea == AreaType.ACTIVE else mine.bench
        if option.inPlayIndex >= len(area_cards):
            return None
        target = area_cards[option.inPlayIndex]
        if target is None or not _net_clock_stage_stack_is_exact(
            target, my_index, state
        ):
            return None
        key = (card.serial, target.serial)
        if key in seen:
            return None
        seen.add(key)
        rows.append((option_index, card, target, option.inPlayArea, option.inPlayIndex))
    return rows


def _net_clock_target_is_soon_active(obs: Observation, target) -> bool:
    """Certify only a promotion forced by this overlay's selected Run Away."""
    state = obs.current
    mine = state.players[state.yourIndex]
    chosen = _net_clock_positive_run_away(obs)
    if chosen is None:
        return False
    option = obs.select.option[chosen]
    if option.area != AreaType.ACTIVE or not mine.active:
        return False
    other = [
        pokemon
        for pokemon in list(mine.active) + list(mine.bench)
        if pokemon is not None and pokemon.serial != mine.active[0].serial
    ]
    return len(other) == 1 and other[0].serial == target.serial


def _net_clock_positive_run_away_after_helmet(obs: Observation, target) -> bool:
    state = obs.current
    mine = state.players[state.yourIndex]
    if mine.active and mine.active[0].serial == target.serial:
        # An Active Dudunsparce needs a separately certified promotion/attack
        # transaction.  This overlay does not infer one from a future prompt.
        return False
    returned = _net_clock_dudunsparce_return_count(
        state, target, state.yourIndex
    )
    if returned is None:
        return False
    returned += 1
    ...
    return (
        returned > 3
        and pokemon_count > 1
        and mine.deckCount + returned - 3 > len(mine.prize)
    )


def _net_clock_lucky_mask(
    obs: Observation, parent_action: list[int]
) -> set[int] | None:
    state = obs.current
    select = obs.select
    ...
    if (
        ...
        or select.maxCount != 1
        or not _net_clock_tight(mine)
        or _net_clock_terminal_attack(obs)
    ):
        return None
    rows = _net_clock_lucky_option_rows(obs)
    if rows is None:
        return None
    masked = set()
    for option_index, _, target, target_area, _ in rows:
        exposed = target_area == AreaType.ACTIVE or _net_clock_target_is_soon_active(
            obs, target
        )
        if not exposed:
            continue
        if target.id == Dudunsparce and _net_clock_positive_run_away_after_helmet(
            obs, target
        ):
            continue
        masked.add(option_index)
    if not masked or not any(index in masked for index in parent_action):
        return None
    return masked


def _net_clock_rerun_parent(
    obs_dict: dict,
    *,
    before_parent: dict,
    allowed_indices: list[int],
) -> list[int] | None:
    """Run exact-v3 once on a certified semantic subset and map indices back."""
    ...
    if (
        ...
        return None
    original_min = raw_select.get("minCount")
    original_max = raw_select.get("maxCount")
    if (
        not isinstance(original_min, int)
        or not isinstance(original_max, int)
        or original_min < 0
        or original_max < original_min
        or len(allowed_indices) < original_min
    ):
        return None
    raw_select["option"] = [raw_options[index] for index in allowed_indices]
    raw_select["maxCount"] = min(original_max, len(allowed_indices))
    _net_clock_restore_parent_state(before_parent)
    reranked_action = _exact_v3_parent_agent(reranked)
    if (
        not isinstance(reranked_action, list)
        or len(reranked_action) < original_min
        or len(reranked_action) > raw_select["maxCount"]
        or len(reranked_action) != len(set(reranked_action))
        or any(
            not isinstance(index, int)
            or index < 0
            or index >= len(allowed_indices)
            for index in reranked_action
        )
    ):
        return None
    return [allowed_indices[index] for index in reranked_action]


def agent(obs_dict: dict) -> list[int]:
    """Compute exact-v3 first, then apply one public prize-clock overlay."""
    if (
        _net_clock_last_observation is not None
        and obs_dict == _net_clock_last_observation
        and _net_clock_last_action is not None
    ):
        return list(_net_clock_last_action)

    before_parent = _net_clock_snapshot_parent_state()
    parent_action = _exact_v3_parent_agent(copy.deepcopy(obs_dict))
    after_parent = _net_clock_snapshot_parent_state()
    if not isinstance(obs_dict, dict) or obs_dict.get("select") is None:
        return _net_clock_remember(obs_dict, parent_action)
    ...
    try:
        ...
        psychic_no = _net_clock_psychic_draw_no(obs, parent_action)
        if psychic_no is not None:
            allowed = [psychic_no]
            replacement = _net_clock_rerun_parent(
                obs_dict, before_parent=before_parent, allowed_indices=allowed
            )
            if replacement == [psychic_no]:
                return _net_clock_remember(obs_dict, replacement)
            _net_clock_restore_parent_state(after_parent)
            return _net_clock_remember(obs_dict, parent_action)

        lucky_mask = _net_clock_lucky_mask(obs, parent_action)
        if lucky_mask:
            allowed = [
                index
                for index in range(len(obs.select.option))
                if index not in lucky_mask
            ]
            replacement = _net_clock_rerun_parent(
                obs_dict, before_parent=before_parent, allowed_indices=allowed
            )
            if replacement is not None and not any(
                index in lucky_mask for index in replacement
            ):
                return _net_clock_remember(obs_dict, replacement)
            _net_clock_restore_parent_state(after_parent)
            return _net_clock_remember(obs_dict, parent_action)

        run_away = _net_clock_positive_run_away(obs)
        if run_away is not None:
            replacement = _net_clock_rerun_parent(
                obs_dict,
                before_parent=before_parent,
                allowed_indices=[run_away],
            )
            if replacement == [run_away]:
                return _net_clock_remember(obs_dict, replacement)
            _net_clock_restore_parent_state(after_parent)
            return _net_clock_remember(obs_dict, parent_action)
