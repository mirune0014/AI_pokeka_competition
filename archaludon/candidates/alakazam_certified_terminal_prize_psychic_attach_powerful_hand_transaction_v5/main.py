...
_TERMINAL_SAFE_ARCHALUDON_PUBLIC_CARD_IDS = frozenset(
    (8, 57, 169, 190, 666, 1097, 1121, 1122, 1147, 1152, 1185, 1227, 1244)
)
_TERMINAL_SAFE_ARCHALUDON_POKEMON_ATTACKS = {
    57: (
        "Relicanth",
        ((61, "Razor Fin", 30, "", (6, 0)),),
    ),
    ...
    190: (
        ...
        (
            (
                ...
                "Metal Defender",
                220,
                "During your opponent’s next turn, this Pokémon has no "
                "Weakness.",
                (8, 8, 8),
            ),
        ),
...
def _terminal_exact_archaludon_attack_fingerprint(card_id: int) -> tuple | None:
    data = card_table.get(card_id)
    if (
        data is None
        or data.cardId != card_id
    ...
    for attack_id in data.attacks:
        ...
        if (
            ...
            or attack.attackId != attack_id
            or not isinstance(attack.name, str)
            or not isinstance(attack.damage, int)
            or not isinstance(attack.text, str)
            or not isinstance(attack.energies, list)
        ):
            return None
        attacks.append(
            (
                ...
                attack.damage,
                attack.text,
                tuple(int(unit) for unit in attack.energies),
            )
        )
    return data.name, tuple(attacks)


def _terminal_exact_archaludon_public_lane_is_clear(
    state, player_state, owner: int
) -> bool:
    lost = getattr(player_state, "lost", ())
    if not isinstance(lost, (list, tuple)) or len(lost) != 0:
        return False
    if state.stadium:
        if (
            len(state.stadium) != 1
            or state.stadium[0].id != Full_Metal_Lab
            or not _terminal_full_metal_lab_metadata_is_exact()
        ):
            return False
    ...
    for cards in zones:
        ...
        for card in cards:
            ...
            if (
                card is None
                or card.id not in _TERMINAL_SAFE_ARCHALUDON_PUBLIC_CARD_IDS
                or card.serial <= 0
                or card.playerIndex != owner
                or data is None
                or data.cardId != card.id
    ...
    return all(
        _terminal_exact_archaludon_attack_fingerprint(card_id)
        == _TERMINAL_SAFE_ARCHALUDON_POKEMON_ATTACKS.get(card_id)
        for card_id in visible_pokemon_ids
    )
...
def _start_terminal_prize_psychic_attach_unsafe(
    ...
    if (
        ...
        or not _terminal_exact_archaludon_public_lane_is_clear(
            state, theirs, 1 - owner
        )
    ):
        return None
    active = mine.active[0]
