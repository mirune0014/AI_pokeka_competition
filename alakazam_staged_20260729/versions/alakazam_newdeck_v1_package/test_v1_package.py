...
class V1PackageTests(unittest.TestCase):
    ...
    def test_lana_child_reorders_and_uses_only_allowlist(self):
        ...
        mine = self.player(
            ...
            discard=recovered + [lana], hand_count=3,
        )
    ...
    def test_hammer_child_binds_area_owner_index_energy_index_and_serial(self):
        ...
        self.assertEqual(self.call(obs, [2]), [0])
        hammer = hand[0]
        mine = self.player(active=[obs.current.players[0].active[0]], hand=hand[1:], discard=[hammer], hand_count=5)
    ...
    def test_child_serial_ambiguity_fails_closed(self):
        ...
        self.assertEqual(self.call(obs, [2]), [0])
        lana = hand[0]
        mine = self.player(active=[obs.current.players[0].active[0]], hand=hand[1:], discard=recovered + [lana], hand_count=3)
        ...
        child = Observation(select, [], state)
        fallback = [0]
        self.assertIs(self.call(child, fallback), fallback)
        self.assertIsNone(v1.V1_TRANSACTION)
    ...
    def test_snapshot_mutation_aborts_child(self):
        ...
        select = SelectData(SelectType.ENERGY, SelectContext.DISCARD_ENERGY, 1, 1, 0, 0, [option], None, None, hammer)
        fallback = [0]
        self.assertIs(self.call(Observation(select, [], state), fallback), fallback)
        self.assertIsNone(v1.V1_TRANSACTION)
...
def _test_boss_terminal_positive_child_reorder_and_verified_attack(self):
    ...
    boss = hand[0]
    mine = self.player(active=[obs.current.players[0].active[0]], hand=hand[1:], discard=[boss], hand_count=5); mine.prize = [None]
    ...
    main_state = State(4, 3, 0, 0, True, False, False, False, -1, [], None, [mine, switched])
...
def _test_boss_switch_mutation_aborts_before_attack(self):
    ...
    main_select = SelectData(SelectType.MAIN, SelectContext.MAIN, 1, 1, 0, 0, [Option(OptionType.ATTACK, attackId=1072), Option(OptionType.END)], None, None, None)
    fallback = [1]
    self.assertIs(self.call(Observation(main_select, [], state), fallback), fallback); self.assertIsNone(v1.V1_TRANSACTION)
...
def _test_mine_stadium_mutation_aborts(self):
    ...
    select = SelectData(SelectType.MAIN, SelectContext.MAIN, 1, 1, 0, 0, [Option(OptionType.ATTACK, attackId=1072), Option(OptionType.END)], None, None, None)
    fallback = [1]; self.assertIs(self.call(Observation(select, [], state), fallback), fallback); self.assertIsNone(v1.V1_TRANSACTION)
...
def _test_alakazam_positive_yes_reorder_draw_guard_and_attack(self):
    ...
    main_state = State(4, 3, 0, 0, False, False, False, False, -1, [], None, [own_drawn, obs.current.players[1]])
    select = SelectData(SelectType.MAIN, SelectContext.MAIN, 1, 1, 0, 0, [Option(OptionType.ATTACK, attackId=1072), Option(OptionType.END)], None, None, None)
...
def _test_alakazam_draw_snapshot_mutation_aborts(self):
    ...
    main_state = State(4, 3, 0, 0, False, False, False, False, -1, [], None, [mutated, obs.current.players[1]])
    select = SelectData(SelectType.MAIN, SelectContext.MAIN, 1, 1, 0, 0, [Option(OptionType.ATTACK, attackId=1072), Option(OptionType.END)], None, None, None)
    fallback = [1]; self.assertIs(self.call(Observation(select, [], main_state), fallback), fallback); self.assertIsNone(v1.V1_TRANSACTION)
