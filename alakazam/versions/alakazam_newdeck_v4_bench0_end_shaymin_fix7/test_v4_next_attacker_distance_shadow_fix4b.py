...
import planner_public_survival_bench0 as outer_rule
...
KNOWN_FIX4_CONFLICTING_FINGERPRINT = (
    "75F113FC56DF7F101F296913D90C90568C9FB8AE26338B84CBC9482FBFF9B86A"
)


class NextAttackerDistanceShadowFix4BTests(unittest.TestCase):
    def setUp(self):
        v1.reset()
    ...
    def setup_observation(self, context, active):
        raw = copy.deepcopy(self.base)
        owner = raw["current"]["yourIndex"]
        mine = self.mine(raw)
        mine["active"] = copy.deepcopy(active)
        mine["bench"] = []
        if context == 2:
            options = [
                {
    ...
    def entrypoint_with_parent_action(self, raw, action):
        ...
        main._deck_v1.agent = (
            lambda parent_module, delegate, observation: action
        )
        try:
            returned = main.agent(copy.deepcopy(raw))
            trace = copy.deepcopy(main.LAST_STAGED_POLICY_TRACE)
        finally:
    ...
    def test_empty_pre_setup_active_is_stable_unknown(self):
        raw = self.setup_observation(2, [])
        sentinel = (8, 5)
        first = self.analyze(raw, action=sentinel)
        second = self.analyze(raw, action=sentinel)
        returned, entrypoint_trace = self.entrypoint_with_parent_action(
            raw, sentinel
        )
        self.assertIs(returned, sentinel)
        self.assertIsNone(first["metric_exception"])
        self.assertEqual(
            first["best_primary_route"]["route_class"], "UNKNOWN"
        )
        self.assertIn(
        ...
        self.assertEqual(first, second)
        self.assertEqual(first["raw_parent_action"], sentinel)
        self.assertEqual(first["applied_action"], sentinel)
        self.assertEqual(
            entrypoint_trace["raw_parent_action"], sentinel
        )
        self.assertEqual(
            entrypoint_trace["applied_action"], sentinel
        )

    def test_face_down_active_in_both_setup_contexts_fails_closed(self):
        for context in (2, 38):
            with self.subTest(context=context):
                raw = self.setup_observation(context, [None])
                sentinel = (context, 7)
                returned, trace = self.entrypoint_with_parent_action(
                    raw, sentinel
                )
                self.assertIs(returned, sentinel)
                self.assertIsNone(trace["metric_exception"])
                ...
                self.assertEqual(
                    trace["best_fallback_route"]["route_class"],
                    "UNKNOWN",
                )
                self.assertIn(
                    "FACE_DOWN_IN_PLAY_IDENTITY_UNKNOWN",
                    trace["unsupported_reasons"],
                )
                self.assertEqual(trace["raw_parent_action"], sentinel)
                self.assertEqual(trace["applied_action"], sentinel)
                self.assertTrue(
                    all(trace["action_identity"].values())
                )

    def test_face_down_setup_duplicate_and_reordered_options_are_stable(self):
        raw = self.setup_observation(2, [None])
        sentinel = [4, 1]
        first = self.analyze(raw, action=sentinel)
        duplicate = self.analyze(raw, action=sentinel)
        reordered_raw = copy.deepcopy(raw)
        reordered_raw["select"]["option"].reverse()
        reordered = self.analyze(reordered_raw, action=sentinel)
        self.assertEqual(first, duplicate)
        self.assertEqual(
            first["observation_fingerprint"],
            reordered["observation_fingerprint"],
        )
        self.assertEqual(first["route_rows"], reordered["route_rows"])
        self.assertIsNone(reordered["metric_exception"])
        self.assertIn(
            "FACE_DOWN_IN_PLAY_IDENTITY_UNKNOWN",
            reordered["unsupported_reasons"],
        )
        self.assertEqual(reordered["raw_parent_action"], sentinel)
        self.assertEqual(reordered["applied_action"], sentinel)

    def test_transaction_fingerprint_absent_false_true_normalization(self):
        absent = self.analyze(self.base)
        empty = c2.analyze(
        ...
        false = c2.analyze(
            ...
            [0],
            transaction_state={
                "integrated_transaction_active": False,
                "v1_transaction_active": False,
                "integrated_transaction_kind": "IGNORED",
                "opponent": "IGNORED",
                "seat": 1,
                "seed": 999,
                "hidden": ["IGNORED"],
        ...
        fingerprints = {
            ...
            for trace in (absent, empty, false)
        }
        self.assertEqual(len(fingerprints), 1)
        self.assertEqual(absent["route_rows"], empty["route_rows"])
        self.assertEqual(absent["route_rows"], false["route_rows"])

        integrated = c2.analyze(
            copy.deepcopy(self.base),
            [0],
            transaction_state={
                "integrated_transaction_active": True,
                "v1_transaction_active": False,
            },
        )
        v1_active = c2.analyze(
            copy.deepcopy(self.base),
            [0],
            transaction_state={
                "integrated_transaction_active": False,
                "v1_transaction_active": True,
            },
        )
        self.assertNotEqual(
            integrated["observation_fingerprint"],
            absent["observation_fingerprint"],
        )
        self.assertNotEqual(
            v1_active["observation_fingerprint"],
            absent["observation_fingerprint"],
        )
        self.assertNotEqual(
            integrated["observation_fingerprint"],
        ...
        for trace in (integrated, v1_active):
            self.assertIsNone(trace["metric_exception"])
            self.assertEqual(
                trace["best_primary_route"]["route_class"], "UNKNOWN"
            )
    ...
    def test_known_fix4_conflict_class_and_option_reorder_are_separated(self):
        self.assertEqual(len(KNOWN_FIX4_CONFLICTING_FINGERPRINT), 64)
        inactive = c2.analyze(
            copy.deepcopy(self.base),
            [0],
            transaction_state={
        ...
        active_raw = copy.deepcopy(self.base)
        active_raw["select"]["option"].reverse()
        active = c2.analyze(
            active_raw,
            [0],
            transaction_state={
        ...
        self.assertNotEqual(
            inactive["observation_fingerprint"],
            active["observation_fingerprint"],
        )
        self.assertEqual(
            active["best_primary_route"]["route_class"], "UNKNOWN"
        )
        self.assertIn(
            "PARENT_TRANSACTION_IN_PROGRESS",
            active["unsupported_reasons"],
        )
