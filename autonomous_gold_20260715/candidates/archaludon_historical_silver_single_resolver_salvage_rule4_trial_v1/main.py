...
def _option_role(obs, option):
    ...
    if target is not None:
        target_fp = _pokemon_fingerprint(target, obs.current.yourIndex)
...
def _make_proposal(action, purpose, proof, transaction):
    return {
        "rule_id": _RULE4_ID,
        "action": action,
...
def _resolve(obs, parent_action):
    ...
    if context == _SelectContext.MAIN:
        return _start_materialization(obs, parent_action)
...
def agent(obs_dict):
    """Call exact Silver once, then resolve Rule 1 or Rule 4."""
