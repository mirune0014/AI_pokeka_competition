...
_CONTINUITY_VERSION = "continuity2-v1-public-combat-envelope-closure-v1"
...
def _continuity_trace(plan, event, chosen_key=None, reason=None,
    ...
    trace = {
        ...
        "pending_transaction": plan.get("pending_transaction"),
        "pending_event": plan.get("pending_event"),
...
def _continuity_clear_pending(reason, event="ABANDON"):
    ...
    if _CONTINUITY_PENDING is not None:
        _CONTINUITY_PENDING_EVENT = {
            "event": event,
            "reason": reason,
            "transaction": dict(_CONTINUITY_PENDING),
        }
    _CONTINUITY_PENDING = None
...
def _continuity_pending_for_observation(obs, slots):
    ...
    if (
        ...
        _continuity_clear_pending("turn/player mismatch")
        return None
    ...
    if context not in allowed:
        ...
        return None
    expected_effect = {"ALLOY": ARCHALUDON_EX, "TURBO": CINDERACE}.get(kind)
    if expected_effect is not None:
        visible_effect = effect_id
        visible_serial = getattr(getattr(obs.select, "effect", None), "serial", None)
        if context == SelectContext.ACTIVATE:
            context_card = getattr(obs.select, "contextCard", None)
            visible_effect = getattr(context_card, "id", None)
            visible_serial = getattr(context_card, "serial", None)
        if visible_effect != expected_effect:
            _continuity_clear_pending("callback effect mismatch")
            return None
        if (
            pending.get("effect_serial") is None
            or visible_serial != pending.get("effect_serial")
        ):
            _continuity_clear_pending("callback effect serial mismatch")
            return None
    return pending
...
def _continuity_certified_primary_line(obs, slot, *, acceleration_cap=0,
    ...
    if current_envelope is not None:
        carried_bench_threat = _continuity_bench_threat(
            obs, current_envelope, slot
        )
        if (
            current_envelope["unknown"]
            or carried_bench_threat >= slot["hp"]
        ):
            return None
        if carried_bench_threat > 0:
            execution_slot = _continuity_project_slot_state(
                slot, hp=slot["hp"] - carried_bench_threat
            )
...
def _continuity_turbo_target(obs, bench_slots, plan=None):
    """Return one successor that is both buildable and publicly safe."""
    active = active_pokemon(obs)
    if active is None or active.id != CINDERACE:
        return None, 0
    ...
    callback_exact = bool(
        ...
        and effect.serial == active.serial
        and obs.select.context == SelectContext.ATTACH_TO
    )
    if not turbo_rows and not callback_exact:
        return None, 0
    exposed_cards = [option_card(obs, row[2]) for row in exposed_rows]
    executable_cap = (
        3 if turbo_rows
        else min(3, len(exposed_cards), int(getattr(obs.select, "maxCount", 0) or 0))
    )
    active_execution_slot = _continuity_slot(
        AreaType.ACTIVE, 0, active, obs.current.yourIndex
    )
    current_envelope = continuity_response_envelope(
        obs, active, 965, h0_execution_slot=active_execution_slot
    )
    preferred = (
        (plan or {}).get("H1_after_KO", {}).get("identity", {}) or {}
    ).get("line_key")
    candidates = []
    for slot in bench_slots:
        ...
        if turbo_rows:
            # MAIN may bind a deterministic target, but hidden Deck Energy is
            # not positive safety evidence until ATTACH_TO exposes real cards.
            route = _continuity_attack_route(
                obs, slot, legal_attack_ids=None, primary_only=True
            )
            ...
            certificate = _continuity_certified_primary_line(
                obs,
                slot,
                acceleration_cap=executable_cap,
                current_envelope=current_envelope,
                acceleration_transaction=transition,
            )
            if certificate is None or certificate["deficit"] <= 0:
                continue
            certified_deficit = certificate["deficit"]
        candidates.append((
            0 if slot["line_key"] == preferred else 1,
            slot["line_key"],
            slot,
            certified_deficit,
        ))
    candidates.sort(key=lambda row: (row[0], row[1]))
    return (candidates[0][2], candidates[0][3]) if candidates else (None, 0)
...
def _continuity_meaningful_legal_attack(obs, active_slot, bench_slots):
    ...
    if 965 in legal and target is not None:
        ...
        if turbo and _continuity_outgoing_damage(
...
def _continuity_acceleration_queue(obs, plan, slots, transaction, cap):
    ...
    for line_key in ordered_keys:
        ...
        if is_h0:
            ...
            certificate = _continuity_certified_primary_line(
                ...
                current_envelope=plan.get("response_envelope"),
...
def _continuity_bound_target(obs, plan, transaction, kind):
    ...
    if assignment is None or not _continuity_bind_acceleration_resources(
        plan["ledger"], transaction
    ):
        reason = f"{kind} Energy serial is not transaction-bound"
    ...
    if not rows:
        reason = f"{kind} target option absent"
        _continuity_clear_pending(reason)
        plan["pending_event"] = _CONTINUITY_PENDING_EVENT or {
    ...
    plan["transaction_update"] = transaction
    return True


def _continuity_alloy_callback_legacy(obs, plan, slots, pending):
    context = obs.select.context
    transaction = pending or _continuity_recover_alloy_transaction(obs, slots)
    if transaction is None:
        plan.setdefault(
            "pending_event", {"event": "UNKNOWN", "reason": "unbound Alloy lineage"}
        )
        return False
    transaction = dict(transaction)
    slot = _continuity_slot_by_line(slots, transaction["line_key"])
    if slot is None:
        plan["pending_event"] = {"event": "ABANDON", "reason": "Alloy lineage absent"}
        return False
    queue = _continuity_acceleration_queue(obs, plan, slots, transaction, 2)
    transaction["target_queue"] = queue
    plan["pending_transaction"] = transaction

    if context == SelectContext.ACTIVATE:
        usable = min(2, sum(item["count"] for item in queue), metal_in_discard(obs))
        chosen_type = OptionType.YES if usable > 0 else OptionType.NO
        rows = _continuity_sorted_options(obs, lambda option: option.type == chosen_type)
        if not rows:
            return False
        selected = [rows[0][0]]
        all_keys = [continuity_option_key(obs, option) for option in obs.select.option]
        plan["choice"] = _continuity_choice(
            selected,
            "ALLOY_ACTIVATE" if usable else "ALLOY_DECLINE",
            violations=[key for key in all_keys if key not in selected],
        )
        plan["objective"] = "RECOVER_PRIMARY_ATTACK" if usable else "PRESERVE_RESOURCES"
        plan["reason"] = f"ALLOY_QUEUE_{usable}_H0_THEN_H1"
        plan["transaction_update"] = transaction
        return True
    ...
    if context == SelectContext.ATTACH_TO:
        ...
        wanted = min(2, sum(item["count"] for item in queue), len(rows), obs.select.maxCount)
        transaction = _continuity_prepare_assignments(obs, plan, transaction, rows, queue, wanted)
        if transaction is None:
            return False
        selected = [row[0] for row in rows[:wanted]]
        all_keys = [continuity_option_key(obs, option) for option in obs.select.option]
        plan["choice"] = _continuity_choice(
            selected, "ALLOY_EXACT_COUNT", exact_count=wanted,
            violations=[key for key in all_keys if key not in selected],
        )
        plan["objective"] = "RECOVER_PRIMARY_ATTACK" if wanted else "PRESERVE_RESOURCES"
        plan["reason"] = f"ALLOY_EXACT_{wanted}_H0_THEN_H1"
        plan["transaction_update"] = transaction
        return True
...
def _continuity_alloy_callback(obs, plan, slots, pending):
    context = obs.select.context
    transaction = dict(pending) if pending else _continuity_recover_alloy_transaction(obs, slots)
    if transaction is None:
        plan.setdefault(
            "pending_event", {"event": "UNKNOWN", "reason": "unbound Alloy lineage"}
        )
    ...
    if slot is None:
        if pending is not None:
            return _continuity_abandon_callback(plan, "Alloy lineage absent")
        plan["pending_event"] = {"event": "ABANDON", "reason": "Alloy lineage absent"}
        return False

    if pending is None:
        queue = _continuity_acceleration_queue(obs, plan, slots, transaction, 2)
        wanted = min(2, sum(item["count"] for item in queue))
        if context == SelectContext.ATTACH_TO:
            exposed = sorted(
                [
                    option_card(obs, option)
                    for option in obs.select.option
            ...
            exposed = sorted(
                [card for card in (my_state(obs).discard or [])
                 if card and card.id == METAL_ENERGY],
                key=lambda card: card.serial,
            )
        reserved = [card.serial for card in exposed[:wanted]]
        transaction["target_queue"] = queue
        transaction["reserved_energy_serials"] = reserved
        transaction["assigned_energy"] = _continuity_assign_reserved_serials(reserved, queue)
    else:
        queue = list(transaction.get("target_queue", []))
        reserved = list(transaction.get("reserved_energy_serials", []))

    wanted = sum(item.get("count", 0) for item in queue)
    if (
        wanted != len(reserved)
        or len(reserved) != len(set(reserved))
        or len(transaction.get("assigned_energy", [])) != len(reserved)
        or [item.get("serial") for item in transaction.get("assigned_energy", [])] != reserved
    ):
        if pending is not None:
            return _continuity_abandon_callback(plan, "Alloy reservation shape changed")
        queue = []
        reserved = []
        wanted = 0
        transaction["target_queue"] = []
        transaction["reserved_energy_serials"] = []
        transaction["assigned_energy"] = []
    ...
    if context == SelectContext.ACTIVATE:
        ...
        if any(serial not in visible for serial in reserved):
            return _continuity_abandon_callback(plan, "reserved Alloy Energy absent at activation")
        chosen_type = OptionType.YES if wanted else OptionType.NO
        rows = _continuity_sorted_options(obs, lambda option: option.type == chosen_type)
        if not rows:
            return False
        if wanted and not _continuity_bind_acceleration_resources(plan["ledger"], transaction):
            return _continuity_abandon_callback(plan, "Alloy ledger ownership conflict")
        selected = [rows[0][0]]
        all_keys = [continuity_option_key(obs, option) for option in obs.select.option]
        plan["choice"] = _continuity_choice(
            selected,
            "ALLOY_ACTIVATE" if wanted else "ALLOY_DECLINE",
            violations=[key for key in all_keys if key not in selected],
        )
        plan["objective"] = "RECOVER_PRIMARY_ATTACK" if wanted else "PRESERVE_RESOURCES"
        plan["reason"] = f"ALLOY_RESERVED_EXACT_{wanted}_H0_THEN_H1"
        plan["transaction_update"] = transaction
        return True
    ...
    if context == SelectContext.ATTACH_TO:
        ...
        if any(len(by_serial.get(serial, [])) != 1 for serial in reserved):
            if pending is not None:
                return _continuity_abandon_callback(plan, "exact reserved Alloy option absent")
            reserved = []
            wanted = 0
        if wanted > int(getattr(obs.select, "maxCount", 0) or 0):
            return _continuity_abandon_callback(plan, "Alloy reserved count exceeds callback maximum")
        if wanted < int(getattr(obs.select, "minCount", 0) or 0):
            return _continuity_abandon_callback(plan, "Alloy reserved count below callback minimum")
        ...
        all_keys = [continuity_option_key(obs, option) for option in obs.select.option]
        plan["choice"] = _continuity_choice(
            selected, "ALLOY_RESERVED_SERIALS", exact_count=wanted,
            violations=[key for key in all_keys if key not in selected],
        )
        plan["objective"] = "RECOVER_PRIMARY_ATTACK" if wanted else "PRESERVE_RESOURCES"
        plan["reason"] = f"ALLOY_EXACT_RESERVED_{reserved}"
        plan["pending_transaction"] = transaction
        plan["transaction_update"] = transaction
        return True
...
def _continuity_turbo_callback_legacy(obs, plan, bench_slots, pending):
    global _CONTINUITY_PENDING_EVENT
    transaction = dict(pending) if pending else None
    slot = _continuity_slot_by_line(bench_slots, transaction["line_key"]) if transaction else None
    active = active_pokemon(obs)
    effect = getattr(obs.select, "effect", None)
    standalone_serial_valid = bool(
        pending is not None
        or (
            active is not None
    ...
    certified, deficit = (
        _continuity_turbo_target(obs, bench_slots, plan)
        if standalone_serial_valid else (None, 0)
    )
    if not standalone_serial_valid:
        plan["pending_event"] = {
            "event": "ABANDON",
            "reason": "unbound Turbo effect serial",
        }
    if slot is None or certified is None or slot["line_key"] != certified["line_key"]:
        slot = certified
        transaction = (
            _continuity_transaction_base(
                obs, "TURBO", slot["line_key"],
                source_active_line=continuity_lineage_key(active, obs.current.yourIndex),
            ) if slot else None
        )
        if transaction is not None:
            transaction["effect_serial"] = getattr(active, "serial", None)
    elif slot is not None:
        deficit = _continuity_primary_deficit(slot) or 0
    plan["pending_transaction"] = transaction
    if pending is not None and transaction is None:
        _continuity_clear_pending("Turbo target no longer publicly safe")
    ...
    if obs.select.context == SelectContext.ATTACH_TO:
        ...
        queue = ([{"line_key": slot["line_key"], "role": "H1_after_KO",
                   "count": min(3, deficit), "deficit": deficit}] if slot else [])
        wanted = min(sum(item["count"] for item in queue), len(rows), obs.select.maxCount)
        if transaction is not None:
            transaction = _continuity_prepare_assignments(
                obs, plan, transaction, rows, queue, wanted
            )
            if transaction is None:
                return False
        selected = [row[0] for row in rows[:wanted]]
        all_keys = [continuity_option_key(obs, option) for option in obs.select.option]
        plan["choice"] = _continuity_choice(
            selected, "TURBO_EXACT_COUNT", exact_count=wanted,
            violations=[key for key in all_keys if key not in selected],
        )
        plan["objective"] = "BUILD_PRIMARY_SUCCESSOR" if wanted else "PRESERVE_DECK_ENERGY"
        plan["reason"] = (
            f"TURBO_EXACT_{wanted}_FOR_{transaction['line_key']}"
            if transaction else "TURBO_NO_CERTIFIED_SAFE_H1"
        )
        if transaction:
            plan["transaction_update"] = transaction
        return True
    ...
    if obs.select.context == SelectContext.ATTACH_FROM:
        if transaction is None:
            plan.setdefault(
                "pending_event", {"event": "UNKNOWN", "reason": "no safe Turbo target"}
            )
            return False
...
def _continuity_turbo_callback(obs, plan, bench_slots, pending):
    ...
    if pending is not None:
        slot = _continuity_slot_by_line(bench_slots, transaction.get("line_key"))
        if slot is not None:
            deficit = _continuity_primary_deficit(slot) or 0
            if exposed_cards and deficit > 0:
        ...
        if certificate is None or certificate.get("deficit", 0) <= 0:
            _continuity_abandon_callback(plan, "Turbo bound target disappeared or became unsafe")
            transaction = None
            slot = None
            certificate = None
    else:
        standalone_serial_valid = bool(
            active is not None
            and active.id == CINDERACE
            and effect is not None
            and effect.id == CINDERACE
            and effect.serial == active.serial
        )
        if standalone_serial_valid:
            slot, deficit = _continuity_turbo_target(obs, bench_slots, plan)
            if slot is not None:
                transaction = _continuity_energy_transition(
                    obs,
                    slot,
            ...
            plan["pending_event"] = {
                "event": "ABANDON", "reason": "unbound Turbo effect serial",
            }

    if slot is not None and certificate is not None:
        role = _continuity_role(
            certificate["execution_slot"],
            certificate["route"],
            "READY_AFTER_TURBO",
            "TURBO_EXACT_TARGET_COMPLETES_CERTIFIED_H1",
        )
        role["response_envelope"] = certificate["response_envelope"]
        role["h1_primary_gate"] = certificate["h1_primary_gate"]
        role["requirements"] = []
    ...
    if obs.select.context == SelectContext.ATTACH_TO:
        ...
        deficit = certificate.get("deficit", 0) if certificate else 0
        queue = ([{
            "line_key": slot["line_key"],
            "role": "H1_after_KO",
            "count": min(3, deficit),
            "deficit": deficit,
        }] if slot is not None and certificate is not None else [])
        wanted = min(
            sum(item["count"] for item in queue),
            len(rows),
            int(getattr(obs.select, "maxCount", 0) or 0),
        )
        if transaction is not None:
            transaction = _continuity_prepare_assignments(
                obs, plan, transaction, rows, queue, wanted
            ...
            if transaction is None:
                return _continuity_abandon_callback(plan, "Turbo ledger ownership conflict")
        if wanted < int(getattr(obs.select, "minCount", 0) or 0):
            # Mandatory callbacks cannot be claimed without the bound target.
            return False
        selected = [row[0] for row in rows[:wanted]]
        all_keys = [continuity_option_key(obs, option) for option in obs.select.option]
        plan["choice"] = _continuity_choice(
            selected, "TURBO_EXACT_BOUND_H1", exact_count=wanted,
            violations=[key for key in all_keys if key not in selected],
        )
        plan["objective"] = "BUILD_PRIMARY_SUCCESSOR" if wanted else "PRESERVE_DECK_ENERGY"
        plan["reason"] = (
...
def _continuity_phase_b_augment(obs, plan, slots, active_slot, bench_slots):
    ...
    if (
        ...
        and "callback effect" in mismatch_event.get("reason", "")
    ...
    if plan.get("choice") is None and active_slot and active_slot["card_id"] == DURALUDON:
        ...
        if (
            ...
            if attack_available:
                ...
                if exact and successor_slot is not None:
                    if successor_count > 0:
                        ...
                        successor_execution_slot = (
                            _continuity_project_energy_transaction(
                                obs, successor_slot, successor_transition,
                                successor_count,
                            )
                        )
                    elif successor_deficit == 0:
                        successor_execution_slot = successor_slot
                    if successor_execution_slot is not None:
                        successor_execution_route = _continuity_attack_route(
                            obs,
                            successor_execution_slot,
                            legal_attack_ids=None,
                            primary_only=True,
                            check_target=False,
                        )
                        successor_h1_gate = _continuity_h1_primary_gate(
                            obs, successor_execution_slot, projected_envelope
                        )
                    if (
                        successor_execution_slot is None
                        or successor_execution_route.get("readiness") != "READY"
                        or successor_h1_gate["state"] != "READY"
                    ):
                        successor_count = 0
                        successor_execution_slot = None
                        successor_execution_route = None
                ...
                if exact and successor_count:
                    _continuity_release_role(ledger, "H1_after_KO")
                exact = bool(
                    exact and _continuity_reserve_many(ledger, claims)
                )
        ...
        for row in _continuity_sorted_options(
            ...
            h1_gate = (
                _continuity_h1_primary_gate(
                    obs, projected_slot, plan["response_envelope"]
                ) if projected_slot and projected_route.get("attack_id") is not None
                else {"state": "UNAVAILABLE", "reason": "NO_PROJECTED_H1"}
            )
            if (
                slot
                and 0 < needed <= 2
                ...
                and projected_route.get("readiness") == "READY"
                and projected_route.get("attack_id") == METAL_DEFENDER
                and _continuity_bench_threat(
                    obs, plan["response_envelope"], projected_slot
                ) < projected_slot["hp"]
                and h1_gate["state"] == "READY"
            ):
    ...
    if plan.get("choice") is None and active_slot and active_slot["card_id"] == CINDERACE:
        ...
        turbo_ko_rows = _continuity_sorted_options(
            ...
            lambda option: (
                ...
                and _continuity_outgoing_block(active_slot["pokemon"], target, obs) is None
                and _continuity_outgoing_damage(
                    obs, active_slot["pokemon"], turbo, target
                ) >= target.hp
            ),
        )
    ...
    turbo_target, turbo_deficit = _continuity_turbo_target(obs, bench_slots, plan)
...
def _continuity_commit_transaction(obs, plan, selected):
    ...
    if plan is None:
        return
    selected_keys = {
        json.dumps(continuity_option_key(obs, obs.select.option[index]), separators=(",", ":"))
    ...
    for field in ("transaction_start", "passive_transaction_start", "transaction_update"):
        ...
        if field != "transaction_update" and triggers and not any(
            json.dumps(key, separators=(",", ":")) in selected_keys for key in triggers
        ):
            continue
        _CONTINUITY_PENDING = dict(transaction)
        return
