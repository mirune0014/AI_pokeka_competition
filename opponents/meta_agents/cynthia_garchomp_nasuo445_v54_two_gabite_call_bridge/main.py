...
def two_gabite_call_target_index(obs) -> int | None:
    """Return the first legal Gabite target for the scoped first Call."""
    if (
        obs.select.context != SelectContext.TO_HAND
        or getattr(obs.select.effect, "id", None) != GABITE
        or obs.select.maxCount != 1
        or has_in_play(obs, GARCHOMP_EX)
        or count_in_play(obs, GABITE) != 1
        or not any(
            pokemon
            and pokemon.id == GIBLE
            and getattr(pokemon, "appearThisTurn", None) is False
            for pokemon in (me(obs).bench or [])
        )
    ):
        return None

    for index, option in enumerate(obs.select.option):
        if getattr(option_card(obs, option), "id", None) == GABITE:
            return index
    return None


def old_gible_to_gabite_bridge_index(obs) -> int | None:
    """Return the best scoped old-Gible evolution that creates a second Call."""
    if (
        obs.select.context != SelectContext.MAIN
        or has_in_play(obs, GARCHOMP_EX)
        or count_in_play(obs, GABITE) != 1
        or GABITE not in hand_ids(obs)
    ):
        return None
    ...
    for index, option in enumerate(obs.select.option):
        ...
        if (
            getattr(evolution, "id", None) == GABITE
            and getattr(target, "id", None) == GIBLE
            and getattr(target, "appearThisTurn", None) is False
        ):
            eligible.append((score_evolve(obs, option)[0], index))
