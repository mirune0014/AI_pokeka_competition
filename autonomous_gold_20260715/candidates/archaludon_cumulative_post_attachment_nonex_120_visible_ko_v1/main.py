...
# ---- CUMULATIVE_PUBLIC_HIERARCHY_MEGA_BRAVE_LOCK_VETO_V2 ----
#
...
_CUM_RULE_ID = (
    "CUMULATIVE_PUBLIC_HIERARCHY_MEGA_BRAVE_VETO_BOSS_LEDGER_V1"
)
_CUM_INTEGRATION_CONTRACT_SHA = (
    "20E440642D3178259EB18EBDD03749C7F807EFCE5BCF98F19B4298EA3DAFDBB9"
)
_CUM_ADMISSION_SHA = (
...
_CUM_RULES = (
    ...
    },
    {
        "rule_id": PUBLIC_BOSS_LEDGER_RULE_ID,
        "rank": 11,
        "prefix": "__passive_boss_proposal__",
        "transaction": "__passive_public_boss_transaction__",
        "source_hash": (
...
def _cum_resolve_owner_collisions(
    ...
    for proposal in proposals:
        ...
        if (
            ...
            or proposal["rule_id"] == PUBLIC_BOSS_LEDGER_RULE_ID
        ):
...
def _cum_commit_proposal(proposal, snapshot_id, suppressed):
    ...
    if rule["rule_id"] == PUBLIC_BOSS_LEDGER_RULE_ID:
        ...
        _ledger_event("GUARD_EMITTED", **copy.deepcopy(commit_state["event"]))
    else:
...
def _cum_boss_telemetry_state():
    ...
    return {
        "ready": _cum_boss_ledger_ready,
        "confirmed_now": _cum_boss_confirmed_now,
        "boss_access": boss_rows,
        "public_discarded_boss_serials": [
            row["serial"]
            for row in boss_rows
            if row["zone"] == "PUBLIC_DISCARD"
        ],
        "held_boss_serials": [
            row["serial"] for row in boss_rows if row["zone"] == "HAND"
        ],
        "transaction": transaction,
        "last_reset_reason": _public_boss_ledger.get("last_reset_reason"),
    }
...
def _cum_emit_telemetry(
    ...
    *,
    ...
    row = {
        ...
        "boss_ledger": _cum_boss_telemetry_state(),
    }
...
def _cum_resume_owner(obs, parent_action, parent_semantic, snapshot_id):
    ...
    if rule is None or _cum_rule_transaction(rule) is None:
        _cum_clear_all_and_ledger("stale_owner")
    ...
    if caught is not None:
        _cum_clear_all_and_ledger("owner_exception")
        _cum_emit_telemetry(
            obs,
            snapshot_id=snapshot_id,
    ...
    if collision_failure is not None:
        ...
        _cum_clear_all_and_ledger("new_different_post_irreversible")
        _cum_emit_telemetry(
            obs,
            snapshot_id=snapshot_id,
            parent_semantic=parent_semantic,
    ...
    if bound is None:
        _cum_clear_all_and_ledger("owner_option_binding_failed")
        _cum_emit_telemetry(
            obs,
            snapshot_id=snapshot_id,
            parent_semantic=parent_semantic,
...
def choose_options(obs):
    ...
    try:
        ...
        reason = "boss_prepass_fail_closed:" + type(error).__name__
        ...
        _cum_emit_telemetry(
            ...
            owner_after=None,
            winner="exact_historical_silver",
            reason="rank12_boss_prepass_fail_closed",
            final_semantic=parent_semantic,
            attribution="exact_historical_silver",
            suppressed=[],
    ...
    proposals = _cum_evaluate_all_clear(obs, parent_action)
    caught = [
        proposal["caught_exception"]
        for proposal in proposals
        if proposal["caught_exception"] is not None
    ]
    if caught:
        _cum_clear_all_and_ledger("component_exception")
        _cum_emit_telemetry(
            obs,
            snapshot_id=snapshot_id,
            parent_semantic=parent_semantic,
            ...
            owner_after=None,
            winner="exact_historical_silver",
            reason="rank12_component_exception_fail_closed",
            final_semantic=parent_semantic,
            attribution="exact_historical_silver",
            suppressed=[],
            rollback_reason="component_exception",
            state_clear_result="ALL_CLEAR",
    ...
    if winner_id == "exact_historical_silver":
        if resolution["unknown"]:
            _cum_clear_all_and_ledger("unknown_clear_collision")
        _cum_emit_telemetry(
            obs,
            snapshot_id=snapshot_id,
            parent_semantic=parent_semantic,
    ...
    if bound is None:
        _cum_clear_all_and_ledger("winning_option_binding_failed")
        _cum_emit_telemetry(
            obs,
            snapshot_id=snapshot_id,
            parent_semantic=parent_semantic,
        ...
        _cum_cache_final(snapshot_id, parent_semantic, "exact_historical_silver")
        return parent_action
    if winner_id == PUBLIC_BOSS_LEDGER_RULE_ID:
        commit_state = winner["state_after"]
        _set_boss_guard_resolution(
            obs,
            owner=PUBLIC_BOSS_LEDGER_RULE_ID,
...
_NMR_CUMULATIVE_ID = "CUMULATIVE_NORMAL_MODE_TWO_PRIZE_ACTIVE_RACE_V1"
_NMR_CONTRACT_SHA = (
    "54E4571236FD636D9C646F5906204B3922E4CD5DF8B758EB852AF71EC9C7547F"
)
_NMR_DIRECT_PARENT_SHA = (
...
_nmr_parent_choose_options = choose_options
...
def _nmr_exact_archaludon_active(obs, pokemon, player_index):
    ...
    return (
        ...
        and _nmr_visible_serial_unique(
            obs, _nmr_serial(pre_evolution[0])
        )
...
def _nmr_build_certificate(obs, parent_action):
    ...
    parent_telemetry = _cum_last_telemetry or {}
    if (
        parent_telemetry.get("winning_rule_id") != "exact_historical_silver"
        or parent_telemetry.get("eligible_rule_ids")
    ):
        return None, "HIGHER_OR_PARENT_RULE_ELIGIBLE"
    mine = my_state(obs)
    opponent = opp_state(obs)
    if (
        mine is None
...
def _nmr_shift_parent_telemetry(row):
    for proposal in row.get("proposals", ()):
        rank = proposal.get("precedence_rank")
        if isinstance(rank, int) and rank >= 6:
            proposal["precedence_rank"] = rank + 1
    reason = row.get("precedence_reason")
    if isinstance(reason, str):
        for rank in range(11, 5, -1):
            prefix = f"rank{rank}_"
            if reason.startswith(prefix):
                reason = f"rank{rank + 1}_" + reason[len(prefix):]
                break
        if reason.startswith("rank12_"):
            reason = "rank13_" + reason[len("rank12_"):]
    row["precedence_reason"] = reason
    return row


def _nmr_record_telemetry(obs, parent_action, action, certificate, rejection):
    global _cum_last_telemetry
    row = _nmr_shift_parent_telemetry(
        copy.deepcopy(_cum_last_telemetry or {})
    )
    parent_semantic = _cum_action_semantic(obs, parent_action)
    final_semantic = _cum_action_semantic(obs, action)
    eligible = certificate is not None
    proposal = {
        "rule_id": _NMR_RULE_ID,
        "source_hash": _NMR_DIRECT_PARENT_SHA,
        "contract_hash": _NMR_CONTRACT_SHA,
        "eligible": eligible,
        "rejection_reason": None if eligible else rejection,
        "desired_action": final_semantic if eligible else None,
        "precedence_rank": 6,
        "certificate_digest": (
            _cum_digest(certificate) if eligible else None
        ),
        "transaction_id": None,
        "stage_before": "CLEAR",
        "stage_after": "CLEAR",
    ...
    proposals.insert(3, proposal)
    row.update(
        {
            "rule_id": _NMR_CUMULATIVE_ID,
            "integration_contract_hash": _NMR_CONTRACT_SHA,
            "direct_parent_hash": _NMR_DIRECT_PARENT_SHA,
            "proposals": proposals,
            "exact_parent_action": _cum_jsonable(parent_semantic),
            "final_action": _cum_jsonable(final_semantic),
            "normal_mode_certificate": _cum_jsonable(certificate),
        }
    )
    if eligible:
        row.update(
            {
                "eligible_rule_ids": [_NMR_RULE_ID],
                "proposed_actions_by_rule": {
                    _NMR_RULE_ID: _cum_jsonable(final_semantic)
                },
                "winning_rule_id": _NMR_RULE_ID,
                "collision_set": [_NMR_RULE_ID],
                "suppressed_rule_ids": [],
                "precedence_reason": "rank6_normal_mode_attack_first",
                "attribution_owner": _NMR_RULE_ID,
    ...
    _cum_last_telemetry = row
    if _cum_telemetry_pending:
        _cum_telemetry_pending[-1] = copy.deepcopy(row)


def choose_options(obs):
    parent_action = _nmr_parent_choose_options(obs)
    proposal, rejection = _nmr_build_certificate(obs, parent_action)
    if proposal is None:
        _nmr_record_telemetry(
            obs, parent_action, parent_action, None, rejection
        )
        return parent_action
    ...
    if not _cum_valid_action(obs, action):
        _nmr_record_telemetry(
            ...
            parent_action,
            None,
            "ATTACK_BINDING_INVALID",
        )
        return parent_action
    _nmr_record_telemetry(
        obs, parent_action, action, certificate, None
    )
    return action
...
# ---- Final wrapper: post-attachment non-ex 120 visible-KO safe successor ----

_PAN_RULE_ID = (
    "POST_ATTACHMENT_NONEX_120_VISIBLE_KO_SAFE_SUCCESSOR_ENVELOPE_V1"
)
_PAN_CUMULATIVE_ID = (
    "CUMULATIVE_POST_ATTACHMENT_NONEX_120_VISIBLE_KO_V1"
)
_PAN_DIRECT_PARENT_SHA = (
...
_PAN_SUPPORTED_SUCCESSOR_ATTACKS = {
    ...
    742: (1071,),  # Kadabra: Super Psy Bolt.
}

_pan_parent_choose_options = choose_options
_pan_parent_agent = agent
_pan_transaction = None
_pan_attachment_evidence = None
_pan_previous_public = None
_pan_last_snapshot_id = None
_pan_last_emission_semantic = None
        ...
        for card in pokemon:
            ...
            for tool in tools:
                if (
                    ...
                    or getattr(tool, "playerIndex", None)
                    != player_index
...
def _pan_direct_damage(obs, attacker, attack_id):
    ...
    if text:
        if (
            ...
            return None
        damage = getattr(attacker, "maxHp", 0) - getattr(
            attacker, "hp", 0
        )
...
def _pan_attachment_valid(obs, active):
    ...
    return (
        ...
        and evidence["observed_action_count"]
        <= obs.current.turnActionCount
        and bool(obs.current.energyAttached)
    )


def _pan_parent_clear():
    row = _cum_last_telemetry or {}
    return (
        _cum_active_transaction_owner is None
        and _cum_owner_meta is None
        and row.get("winning_rule_id") == "exact_historical_silver"
        and not row.get("eligible_rule_ids")
        and row.get("active_transaction_owner") is None
        and row.get("active_owner_after") is None
        and row.get("precedence_reason") == "rank13_exact_parent"
        and all(
            _cum_rule_transaction(rule) is None
            for rule in _CUM_RULES
        )
    )
...
def _pan_build_certificate(obs, parent_action):
    ...
    active_energy = _pan_energy_rows(
        obs, active, obs.current.yourIndex
    )
    ...
    inherited_routes = tuple(
        (
            attack_id,
            _pan_source_target_damage(
                obs, DURALUDON, attack_id, target
            ),
        )
    ...
    active_evolutions = [
        ...
        if (
            ...
            and option.inPlayArea == AreaType.ACTIVE
            and _pan_serial(option_target(obs, option))
            == active.serial
        )
    ]
    if (
    ...
    evolution_card = option_card(
        obs, obs.select.option[evolution_position]
    )
    if (
    ...
    evolution_semantic = _cum_action_semantic(
        obs, [evolution_position]
    )
    parent_semantic = _cum_action_semantic(obs, parent_action)
    ...
    return {
        ...
        "energy_attached": bool(obs.current.energyAttached),
        "retreated": bool(obs.current.retreated),
        "attachment_evidence": copy.deepcopy(
            _pan_attachment_evidence
        ),
        "evolution_serial": evolution_serial,
...
def _pan_pre_evolution_valid(obs, parent_action, transaction):
    ...
    active = active_pokemon(obs)
    target = opp_active_pokemon(obs)
    successor = _pan_successor_envelope(
        obs, transaction["projected_hp"]
    )
    return (
        _pan_main_context(obs)
        and _pan_parent_clear()
        ...
        and transaction["turn_action_count"]
        == obs.current.turnActionCount
        and (
            len(mine.prize or ()),
            len(opponent.prize or ()),
        ...
        and _h5v2_hand_fingerprint(mine)
        == transaction["hand"]
        and _h5v2_stadium_fingerprint(obs)
        == transaction["stadium"]
        and _h5v2_public_modifiers(obs)
        == transaction["public_modifiers"]
        and bool(obs.current.supporterPlayed)
        ...
        and _pan_attachment_evidence
        == transaction["attachment_evidence"]
        and _h5v2_parent_is_inherited_raging(obs, parent_action)
        and successor == transaction["successor_envelope"]
        and _cum_bind_semantic(
            obs, transaction["evolution_semantic"]
        )
        is not None
    )
...
def _pan_evolved_valid(obs, transaction):
    ...
    if (
        ...
        or _h5v2_hand_fingerprint(mine)
        != transaction["expected_post_evolution_hand"]
        or _h5v2_stadium_fingerprint(obs)
        != transaction["stadium"]
        or _h5v2_public_modifiers(obs)
        != transaction["public_modifiers"]
        or bool(obs.current.supporterPlayed)
        ...
        != transaction["energy_attached"]
        or bool(obs.current.retreated) != transaction["retreated"]
        or getattr(active, "maxHp", None)
        != transaction["projected_max_hp"]
    ...
    if (
        ...
        or getattr(prior[0], "playerIndex", None)
        != transaction["seat"]
        or _pan_attachment_evidence
        != transaction["attachment_evidence"]
    ):
        return False
    successor = _pan_successor_envelope(
...
def _pan_clear(reason, clear_attachment=False):
    ...
    global _pan_last_snapshot_id, _pan_last_emission_semantic
    _pan_transaction = None
    _pan_last_snapshot_id = None
    _pan_last_emission_semantic = None
    if clear_attachment:
...
def _pan_reset_all(reason):
    global _pan_previous_public, _pan_last_telemetry
    _pan_clear(reason, clear_attachment=True)
    _pan_previous_public = None
    _pan_last_telemetry = {
        "rule_id": _PAN_CUMULATIVE_ID,
        "reason": reason,
        "stage_after": "CLEAR",
    }
...
def _pan_record(
    ...
    _pan_last_telemetry = {
        "rule_id": _PAN_CUMULATIVE_ID,
        "mechanism": _PAN_RULE_ID,
        "direct_parent_sha256": _PAN_DIRECT_PARENT_SHA,
        "contract_sha256": _PAN_CONTRACT_SHA,
        "amendment_sha256": _PAN_AMENDMENT_SHA,
        "source_replay_sha256": _PAN_SOURCE_REPLAY_SHA,
        "seat": obs.current.yourIndex,
        "turn": obs.current.turn,
        "turn_action_count": obs.current.turnActionCount,
        "reason": reason,
        ...
        "parent_action": _cum_jsonable(
            _cum_action_semantic(obs, parent_action)
        ),
        "final_action": _cum_jsonable(
            _cum_action_semantic(obs, final_action)
        ),
        "successor_envelope": _cum_jsonable(
            (
                certificate.get("successor_envelope")
...
def _pan_cache_emission(obs, action):
    global _pan_last_snapshot_id, _pan_last_emission_semantic
    _pan_last_snapshot_id = _cum_public_snapshot_id(obs)
    _pan_last_emission_semantic = _cum_action_semantic(obs, action)


def _pan_retry(obs):
    if (
        _pan_transaction is None
        or _pan_last_snapshot_id != _cum_public_snapshot_id(obs)
        or _pan_last_emission_semantic is None
    ):
        return None
    return _cum_bind_semantic(obs, _pan_last_emission_semantic)
...
def _pan_call_parent(obs):
    global _pan_parent_call_count
    _pan_parent_call_count += 1
    return _pan_parent_choose_options(copy.deepcopy(obs))


def _pan_resume(obs, parent_action):
    global _pan_transaction
    transaction = _pan_transaction
        ...
        if (
            ...
            if not _pan_evolved_valid(obs, transaction):
                ...
                _pan_record(
                    ...
                    parent_action,
                    parent_action,
                    "post_evolution_actual_state_failure",
                    stage_before,
                    "CLEAR",
                )
                return parent_action
            transaction["stage"] = "EVOLVED_CONFIRMED"
            attack = _cum_bind_semantic(
                obs,
                _cum_action_semantic(
                    obs,
                    [
                        _h5v2_attack_options(
                            obs, COATED_ATTACK
                        )[0][0]
                    ],
                ),
            ...
            _pan_cache_emission(obs, attack)
            _pan_record(
                obs,
                parent_action,
                attack,
                "coated_attack_emitted",
                stage_before,
                "ATTACK_EMITTED",
                transaction,
            )
            return attack
        if not _pan_pre_evolution_valid(
            obs, parent_action, transaction
        ):
            ...
            _pan_record(
                ...
                parent_action,
                parent_action,
                "pre_evolution_rollback",
                stage_before,
                "CLEAR",
            )
            return parent_action
        action = _cum_bind_semantic(
            obs, transaction["evolution_semantic"]
        )
        if action is None:
            _pan_clear("pre_evolution_binding_failure")
            return parent_action
        _pan_cache_emission(obs, action)
        _pan_record(
            obs,
            parent_action,
            action,
    ...
    if stage_before == "ATTACK_EMITTED":
        ...
        _pan_record(
            ...
            parent_action,
            parent_action,
            "novel_callback_after_attack_emission",
            stage_before,
            "CLEAR",
        )
        return parent_action
...
def choose_options(obs):
    ...
    parent_action = None
    try:
        parent_action = _pan_call_parent(obs)
        if not _cum_valid_action(obs, parent_action):
            _pan_clear("invalid_direct_parent", clear_attachment=True)
            action = _cum_emergency_action(obs)
        elif _pan_transaction is not None:
            action = _pan_resume(obs, parent_action)
        else:
            certificate, rejection = _pan_build_certificate(
                obs, parent_action
            )
            if certificate is None:
                ...
                action = _cum_bind_semantic(
                    obs, certificate["evolution_semantic"]
                )
                if action is None:
                    action = parent_action
                    _pan_record(
        ...
        if not _cum_valid_action(obs, action):
            _pan_clear("invalid_final_action", clear_attachment=True)
            action = parent_action
            if not _cum_valid_action(obs, action):
                action = _cum_emergency_action(obs)
        return action
    except Exception as error:
        _pan_clear("wrapper_exception", clear_attachment=True)
        if parent_action is None:
            try:
                parent_action = _pan_call_parent(obs)
            except Exception:
                parent_action = _cum_emergency_action(obs)
        if not _cum_valid_action(obs, parent_action):
            parent_action = _cum_emergency_action(obs)
        _pan_record(
            obs,
            parent_action,
            parent_action,
            "wrapper_exception:" + type(error).__name__,
            (
                _pan_transaction["stage"]
...
def agent(obs_dict):
    select = (
        ...
        else getattr(obs_dict, "select", None)
    )
    if select is None:
        _pan_reset_all("deck_request")
    return _pan_parent_agent(obs_dict)


# Re-register the final entry point so insertion-order loaders select it last.
_pan_final_agent = agent
del agent
agent = _pan_final_agent
del _pan_final_agent
