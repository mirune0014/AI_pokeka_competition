...
BASICS = {IMP, MUNKIDORI, SNORUNT, DUNSPARCE, FEZANDIPITI_EX, BUDEW, YVELTAL, CORNERSTONE_OGERPON_EX}
...
def setup_score(obs, opt) -> tuple[int, str]:
    ...
    if ctx == SelectContext.SETUP_BENCH_POKEMON:
        ...
        if cid == CORNERSTONE_OGERPON_EX:
            return -250, "hold Cornerstone Ogerpon until Archaludon seen"
...
def score_play(obs, opt) -> tuple[int, str]:
    ...
    if cid == CORNERSTONE_OGERPON_EX:
        if facing(obs, ARCHALUDON_MARKERS):
            return 4600 if count_in_play(obs, CORNERSTONE_OGERPON_EX) < 1 else -500, "bench one Cornerstone Ogerpon"
        return -600, "hold Cornerstone Ogerpon outside Archaludon"
