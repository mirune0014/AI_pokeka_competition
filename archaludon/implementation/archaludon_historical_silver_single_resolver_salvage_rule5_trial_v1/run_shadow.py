...
baseline = load(BASELINE, "rule5_shadow_rule4")
sys.modules.pop("_historical_silver_parent", None)
candidate = load(CANDIDATE, "rule5_shadow_candidate")
        ...
        try:
            ...
            for step, obs, _recorded in decisions:
                ...
                if (
                    ...
                    and proposal.get("rule_id") == candidate._RULE5_ID
                    and (proposal.get("transaction") or {}).get("stage") == "BOSS_EMITTED"
                ):
                    natural_starts += 1
                if telemetry.get("rejection_reason") == "boss_attack_dispatched":
...
summary = {
    ...
    "all_differences_rule5": all(row["classification"] in allowed for row in differences),
