...
def _direct_final_prize_powerful_hand_action(
    obs: Observation,
) -> list[int] | None:
    ...
    if (
        ...
        or active_data.hp <= 0
        or active.maxHp != active_data.hp
        or getattr(active, "playerIndex", None) != my_index
        or not _draw_survival_known_stack_is_exact(active, my_index)
        or target_data is None
        or target_data.hp <= 0
        or target.maxHp != target_data.hp
        or getattr(target, "playerIndex", None) != 1 - my_index
...
def _source_transition_v2_parent_agent(obs_dict: dict) -> list[int]:
    ...
    select = obs.select
    context = select.context
    direct_final_prize_owner_at_entry = bool(
    ...
    direct_final_prize_action = _direct_final_prize_powerful_hand_action(obs)
    if (
        not direct_final_prize_owner_at_entry
        and direct_final_prize_action is not None
    ):
        return _remember_action(decision_signature, direct_final_prize_action)
