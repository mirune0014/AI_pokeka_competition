...
_NAC_ROTATION_RULE_ID = "HEALTHY_READY_PREATTACK_ROTATION_V1"
...
# -- 4. Healthy ready preattack rotation ------------------------------------
...
def _nacr_build(obs, parent_action):
    ...
    if (
        ...
        or parent_option.attackId != METAL_DEFENDER
    ):
        return _nac_reject("_nacr", "parent_not_metal_defender")
    active = active_pokemon(obs)
    opponent = opp_active_pokemon(obs)
    if (
        active is None
...
def _nacr_choose(obs, parent_action):
    if _nacr_transaction is not None:
        return _nacr_resume(obs)
    return _nacr_build(obs, parent_action)
