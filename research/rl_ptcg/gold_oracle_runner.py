...
def _implementation_paths(cli_path: Path, workspace: Path) -> list[Path]:
    ...
    paths = (
        Path(__file__),
        cli_path,
        root / "research" / "rl_ptcg" / "rollout_expert.py",
        root / "research" / "rl_ptcg" / "belief.py",
        root / "research" / "rl_ptcg" / "canonical_actions.py",
        root / "research" / "rl_ptcg" / "replay_records.py",
        root / "research" / "rl_ptcg" / "replay_reconstruction.py",
        root / "research" / "rl_ptcg" / "teacher_statistics.py",
        root / "research" / "rl_ptcg" / "gold_oracle_statistics.py",
        root / "research" / "rl_ptcg" / "gold_oracle_states.py",
        root / "research" / "rl_ptcg" / "gold_upper_tier_states.py",
        root / "research" / "rl_ptcg" / "gold_candidate_selection.py",
        root / "research" / "rl_ptcg" / "probe_search.py",
        root / "infrastructure" / "tools" / "ptcg_common.py",
        root / "infrastructure" / "tools" / "build_gold_candidate_selection.py",
    )
...
def _load_tools():
    root = Path(__file__).resolve().parents[2]
    tools_dir = root / "infrastructure" / "tools"
    if str(root) not in sys.path:
        sys.path.insert(0, str(root))
    if str(tools_dir) not in sys.path:
        sys.path.insert(0, str(tools_dir))
    from infrastructure.tools.ptcg_common import dataclass_to_dict, load_agent, pushd
