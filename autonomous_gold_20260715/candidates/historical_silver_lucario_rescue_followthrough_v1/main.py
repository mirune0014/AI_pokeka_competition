...
def score_target(obs, opt):
    ...
    if ctx == SelectContext.ATTACH_FROM:
        ...
        active = active_pokemon(obs)
        opponent_active = opp_active_pokemon(obs)
        raging_hammer_ko = bool(
            active
            and opponent_active
            and energy_count(active) + 2 >= 3
            and effective_damage(
                80 + damage_on(active) // 10 * 10, opponent_active
            ) >= opponent_active.hp
        )
        if (
            effect
            and effect.id == ARCHALUDON_EX
            and detect_matchup(obs) == "lucario"
            and active
            and active.id == DURALUDON
            and active.hp <= 70
            and card
            and cid == ARCHALUDON_EX
            and opt.area == AreaType.BENCH
            and energy_count(card) < 3
            and not any(
                pokemon
                and pokemon.id in {DURALUDON, ARCHALUDON, ARCHALUDON_EX}
                and energy_count(pokemon) >= 3
            ...
            and not raging_hammer_ko
        ):
            return 33000 + energy_count(card), "Lucario: complete rescued bench attacker"
