...
def fighting_gong_optional_readiness_index(
    obs,
    scored: list[tuple[int, int, str]],
) -> int | None:
    """Return Basic Fighting only for the exact optional Gong readiness state."""
    try:
        ...
        if (
            selection is None
            or current is None
            or getattr(selection, "context", None) != SelectContext.TO_HAND
            or getattr(getattr(selection, "effect", None), "id", None) != FIGHTING_GONG
            or getattr(selection, "minCount", None) != 0
            or getattr(selection, "maxCount", None) != 1
            or getattr(current, "energyAttached", None) is not False
        ):
            return None
        ...
        options = getattr(selection, "option", None)
        deck = getattr(selection, "deck", None)
        if options is None or deck is None or len(scored) != len(options) or not scored:
            return None
        ...
        if sorted(scored_indices) != list(range(len(options))):
            return None
        ...
        for score, index, _reason in baseline_rows:
            if len(baseline_selected) >= selection.maxCount:
                break
            if score < 0 and len(baseline_selected) >= selection.minCount:
                continue
            baseline_selected.append(index)
        if len(baseline_selected) < selection.minCount:
            baseline_selected = [
                index
                for _score, index, _reason in baseline_rows[: selection.minCount]
            ]

        gible_indices: list[int] = []
        basic_fighting_indices: list[int] = []
        for index, option in enumerate(options):
            option_type = getattr(option, "type", None)
            if option is None or option_type is None:
                return None
            if option_type != OptionType.CARD or getattr(option, "area", None) != AreaType.DECK:
                continue
            deck_index = getattr(option, "index", None)
            if type(deck_index) is not int or not 0 <= deck_index < len(deck):
                return None
            card = deck[deck_index]
            card_id = getattr(card, "id", None)
            if card is None or type(card_id) is not int:
                return None
            if card_id == GIBLE:
                gible_indices.append(index)
            elif card_id == BASIC_FIGHTING:
                basic_fighting_indices.append(index)
        if (
            len(baseline_selected) != 1
            or baseline_selected[0] not in gible_indices
            or not gible_indices
            or not basic_fighting_indices
        ):
            return None
        ...
        if (
            type(your_index) is not int
            or players is None
            or not 0 <= your_index < len(players)
            or players[your_index] is None
        ):
            return None
        ...
        if active is None or bench is None or len(active) != 1 or active[0] is None:
            return None
        in_play = list(active) + list(bench)
        if any(
            pokemon is None or type(getattr(pokemon, "id", None)) is not int
            for pokemon in in_play
        ):
            return None
        main_line = [pokemon for pokemon in in_play if pokemon.id in MAIN_LINE]
        if not main_line or active[0] not in main_line:
            return None
        for pokemon in main_line:
            energy_cards = getattr(pokemon, "energyCards", None)
            energy_types = getattr(pokemon, "energies", None)
            if (
                energy_cards is None
                or energy_types is None
                or len(energy_cards) != 0
                or len(energy_types) != 0
            ):
                return None

        return min(basic_fighting_indices)
    except Exception:
        return None
...
def choose_options(obs):
    ...
    gong_index = fighting_gong_optional_readiness_index(obs, scored)
    if gong_index is not None:
        return [gong_index]
