...
CANDIDATE = (
    AUTO
    / "candidates"
    / "archaludon_historical_silver_replay_repair_v1"
)
...
class Rule5AttackTransactionTests(unittest.TestCase):
    ...
    def test_unknown_one_prize_bench_is_ignored_when_unique_ex_is_exact(self):
        start, _boss, _attacker, _current, _target = boss_start(0, 21, 99)
        start["current"]["players"][1]["bench"].append(pokemon(345, 401, 1))
        self.assertEqual(self.call(start, [0]), [1])
        self.assertEqual(main._materialization_owner["target_serial"], 400)

    def test_unknown_higher_prize_bench_fails_closed(self):
        start, _boss, _attacker, _current, _target = boss_start(0, 21, 99)
        start["current"]["players"][1]["bench"].append(pokemon(117, 401, 1))
        parent_action = [0]
        self.assertIs(self.call(start, parent_action), parent_action)
        self.assertIsNone(main._materialization_owner)
        self.assertEqual(
            main._last_telemetry["rejection_reason"],
    ...
    def test_irrelevant_bench_skill_and_tool_do_not_block_exact_boss(self):
        start, _boss, _attacker, _current, _target = boss_start(0, 21, 99)
        unrelated = pokemon(120, 401, 1)
        unrelated["tools"] = [card(main._parent.HERO_CAPE, 800, 1)]
        start["current"]["players"][1]["bench"].append(unrelated)
        self.assertEqual(self.call(start, [0]), [1])
        self.assertEqual(main._materialization_owner["target_serial"], 400)

    def test_target_skill_scope_for_crustle_ogerpon_and_benign_allowlist(self):
        crustle_start, _boss, attacker, _current, crustle = boss_start(0, 21, 345)
        crustle_obs = main._to_observation_class(copy.deepcopy(crustle_start))
        self.assertIsNone(
            main._exact_damage_and_take(
                crustle_obs,
                crustle_obs.current.players[0].active[0],
                crustle_obs.current.players[1].bench[0],
                253,
            )
        )

        nonex_start, _boss, _attacker, _current, _target = boss_start(0, 21, 345)
        nonex_start["current"]["players"][0]["active"][0] = pokemon(main._DURALUDON, 10, 0)
        nonex_obs = main._to_observation_class(copy.deepcopy(nonex_start))
        nonex_result = main._exact_damage_and_take(
            nonex_obs,
            nonex_obs.current.players[0].active[0],
            nonex_obs.current.players[1].bench[0],
            224,
        )
        self.assertIsNotNone(nonex_result)

        ogerpon_start, _boss, _attacker, _current, _target = boss_start(0, 21, 117)
        ogerpon_obs = main._to_observation_class(copy.deepcopy(ogerpon_start))
        self.assertIsNone(
            main._exact_damage_and_take(
                ogerpon_obs,
                ogerpon_obs.current.players[0].active[0],
                ogerpon_obs.current.players[1].bench[0],
                253,
            )
        )

        benign_start, _boss, _attacker, _current, _target = boss_start(0, 21, 140)
        benign_obs = main._to_observation_class(copy.deepcopy(benign_start))
        benign_result = main._exact_damage_and_take(
            benign_obs,
            benign_obs.current.players[0].active[0],
            benign_obs.current.players[1].bench[0],
            253,
        )
        self.assertIsNotNone(benign_result)
