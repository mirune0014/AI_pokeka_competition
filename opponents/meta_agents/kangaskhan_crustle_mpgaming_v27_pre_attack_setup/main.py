...
for path in (ROOT, Path.cwd(), Path("/kaggle_simulations/agent")):
...
NONLETHAL_ATTACK_SCORE, PRE_ATTACK_SETUP_SCORE, HIGHEST_ATTACK_SCORE = 10000, 10001, 10002
...
def needs_setup_board(me):
    """Whether the public board lacks a basic continuity role."""
    return (len(field(me)) < 2
            or count(me, KANGASKHAN) == 0
            or count(me, DWEBBLE) + count(me, CRUSTLE) == 0)


def useful_pre_attack_evolution(obs, me, opt):
    """Evolve only a live Dwebble line that improves attacker continuity."""
    source = card(obs, opt.area, opt.index, obs.current.yourIndex)
    target = card(obs, opt.inPlayArea, opt.inPlayIndex, obs.current.yourIndex)
    return (source is not None and source.id == CRUSTLE
            and target is not None and target.id == DWEBBLE
            and (opt.inPlayArea == AreaType.ACTIVE
                 or bool(target.energies)
                 or count(me, CRUSTLE) == 0))


def useful_backup_attachment(obs, opt):
    """Spend the manual attachment on an unready bench attacker or Grow line."""
    source = card(obs, opt.area, opt.index, obs.current.yourIndex)
    target = card(obs, opt.inPlayArea, opt.inPlayIndex, obs.current.yourIndex)
    if (source is None or source.id not in ENERGY or target is None
            or opt.inPlayArea != AreaType.BENCH):
        return False
    if target.id == KANGASKHAN:
        return len(target.energies) < 3
    if target.id == DWEBBLE:
        return len(target.energies) < 3
    return False


def useful_setup_play(obs, me, opt):
    """Use only setup cards that address a thin board or missing core role."""
    c = card(obs, AreaType.HAND, opt.index, obs.current.yourIndex)
    if c is None or not needs_setup_board(me):
        return False
    if c.id == POFFIN:
        return True
    return c.id in (HILDA, LILLIE, POKEGEAR) and me.handCount <= 3


def public_fixed_damage_ko(opp, opt):
    """Recognize only a certain KO from the public attack and Active HP."""
    opp_active = active(opp)
    attack = ATTACK_DB.get(opt.attackId)
    damage = attack.damage if attack is not None else None
    return (opp_active is not None and type(damage) is int
            and damage >= opp_active.hp)


def close_race_attack_conversion_guard(sel, me, opp, me_active):
    """Convert a heal only when the active's missing HP is below the role threshold."""
    opp_active = active(opp)
    if (opp.deckCount <= 3 or abs(len(me.prize) - len(opp.prize)) > 1
    ...
    try:
        ...
        for opt in sel.option:
            ...
            if sel.context == SelectContext.MAIN:
                if opt.type == OptionType.ATTACK:
                    score = (HIGHEST_ATTACK_SCORE
                             if opt.attackId == ASCENSION or public_fixed_damage_ko(opp, opt)
                             else NONLETHAL_ATTACK_SCORE if opt.attackId in (SCISSORS, COMBO)
                             else 100)
                elif opt.type == OptionType.EVOLVE:
                    score = PRE_ATTACK_SETUP_SCORE if useful_pre_attack_evolution(obs, me, opt) else 9000
                elif opt.type == OptionType.ATTACH:
                    score = (PRE_ATTACK_SETUP_SCORE if useful_backup_attachment(obs, opt)
                             else backup_kang_grow_line_score(obs, me, opp, opt) or 7000)
                elif opt.type == OptionType.PLAY:
                    c = card(obs, AreaType.HAND, opt.index, obs.current.yourIndex)
                    score = (11000 if (c is not None and c.id == JUMBO_ICE_CREAM
                                       and heal_first)
                             else (PRE_ATTACK_SETUP_SCORE if useful_setup_play(obs, me, opt)
                                   else {POFFIN: 8500, HILDA: 8000, LILLIE: 7000, POKEGEAR: 6000,
                                         XEROSIC: 8250 if xerosic_gate else 5000, BOSS: 3000}.get(c.id if c else -1, 1000)))
