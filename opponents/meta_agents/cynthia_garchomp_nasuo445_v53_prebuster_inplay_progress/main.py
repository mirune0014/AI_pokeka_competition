...
def prebuster_inplay_progress_index(obs) -> int | None:
    """Finish one existing bench Garchomp line before an approved top Buster."""
    ctx = obs.select.context
    if ctx not in {SelectContext.MAIN, SelectContext.TO_HAND}:
        return None
    options = getattr(obs.select, "option", None) or []
    player = me(obs)
    bench = [pokemon for pokemon in (getattr(player, "bench", None) or []) if pokemon]
    active = active_pokemon(obs)

    if ctx == SelectContext.TO_HAND:
        effect = getattr(obs.select, "effect", None)
        if (
            getattr(effect, "id", effect) != GABITE
    ...
    public_guard = bool(
        ...
        and energy_count(active) >= 2
        and has_in_play(obs, ROSERADE)
        and not any(getattr(pokemon, "id", None) == GARCHOMP_EX for pokemon in bench)
        and not (
            getattr(obs.current, "energyAttached", False)
            and has_energized_bench_main_line(obs)
        )
        and is_approved_buster_conversion(obs)
    )
    ...
    if ctx == SelectContext.TO_HAND:
        ...
        bench_gabites = [pokemon for pokemon in bench if getattr(pokemon, "id", None) == GABITE]
        eligible_gabites = [
            pokemon for pokemon in bench_gabites
            if getattr(pokemon, "appearThisTurn", True) is False
        ]
        effect_serial = getattr(effect, "serial", None)
        if effect_serial is None:
            if len(bench_gabites) != 1 or len(eligible_gabites) != 1:
                return None
        elif not any(getattr(pokemon, "serial", None) == effect_serial for pokemon in eligible_gabites):
            return None

        garchomp_indices = [
            index for index, option in enumerate(options)
            if option.type == OptionType.CARD
            and getattr(option_card(obs, option), "id", None) == GARCHOMP_EX
        ]
        return min(garchomp_indices) if garchomp_indices else None

    if ctx != SelectContext.MAIN or not public_guard:
        return None

    scored: list[tuple[int, int, str]] = []
    for index, option in enumerate(options):
        try:
            score, reason = score_option_with_champions_call_order(obs, option)
            call_before_evolve_score = matching_champions_call_evolve_score(obs, option)
    ...
    if development_index is not None:
        corkscrew_scores = [
            ...
            if options[index].type == OptionType.ATTACK
            and options[index].attackId == CORKSCREW_DIVE
        ]
        if corkscrew_scores:
            corkscrew_score = max(corkscrew_scores)
    ...
    if top_option.type != OptionType.ATTACK or top_option.attackId != DRACONIC_BUSTER:
        return None
    ...
    for score, index, _reason in scored:
        ...
        if (
            ...
            and option.playerIndex in {None, obs.current.yourIndex}
        ):
            source = option_card(obs, option)
            if (
                getattr(source, "id", None) == GABITE
                and getattr(source, "appearThisTurn", True) is False
            ):
                eligible_calls.append((score, index))
    if eligible_calls:
        return max(eligible_calls, key=lambda row: (row[0], -row[1]))[1]
    ...
    for index, option in enumerate(options):
        ...
        pair = (getattr(evolution, "id", None), getattr(target, "id", None))
        if pair in {(GARCHOMP_EX, GABITE), (GABITE, GIBLE)}:
            eligible_evolutions.append((score_evolve(obs, option)[0], index))
    return max(eligible_evolutions, key=lambda row: (row[0], -row[1]))[1] if eligible_evolutions else None
