...
LAST_V4_POFFIN_ZERO_VETO_TRACE: dict | None = None
...
def agent(obs_dict: dict) -> list[int]:
    ...
    global LAST_V4_POFFIN_ZERO_VETO_TRACE, LAST_STAGED_POLICY_TRACE
    if isinstance(obs_dict, dict) and obs_dict.get("select") is None:
        _deck_v1.reset()
        return _parent.agent(obs_dict)
    ...
    LAST_V4_POFFIN_ZERO_VETO_TRACE = dict(
        _deck_v1.LAST_V4_POFFIN_ZERO_VETO_TRACE
    )
    parent_trace = {
