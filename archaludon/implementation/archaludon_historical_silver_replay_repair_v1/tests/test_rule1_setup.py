...
class Rule1SetupTests(unittest.TestCase):
    ...
    def test_unsupported_or_duraludon_active_commit(self) -> None:
        for active_id in (main._DURALUDON, 57, 9999):
            with self.subTest(active_id=active_id):
                ...
                parent_action = []
                action = self.call_parent(raw, parent_action)
                if active_id == main._DURALUDON:
                    self.assertEqual(self.selected_serial(raw, action), 11)
                    self.assertEqual(
                        main._last_proposal["exact_proof"]["active_card_id"],
                        main._DURALUDON,
                    )
                else:
                    self.assertIs(action, parent_action)
                    self.assertEqual(
                        ...
                        "missing_active_commit",
                    )

    def test_visible_active_mismatch_existing_backup_and_full_bench_reject(self) -> None:
        hand = [card(main._CINDERACE, 30, 0), card(main._DURALUDON, 11, 0)]
        cases = (
            ({"active": [pokemon(main._DURALUDON, 99)]}, "visible_active_mismatch"),
            ({"bench": [pokemon(main._DURALUDON, 99)]}, "backup_duraludon_already_present"),
            (
                {
    ...
    def test_duraludon_start_places_a_distinct_backup(self) -> None:
        hand = [
            card(main._DURALUDON, 40, 0),
            card(main._DURALUDON, 11, 0),
        ]
        self.commit_active(hand, selected_hand_index=0)
        raw = self.bench_raw(
            hand,
            active=[pokemon(main._DURALUDON, 40)],
        )
        action = self.call_parent(raw, [])
        self.assertEqual(self.selected_serial(raw, action), 11)
        self.assertEqual(main._last_proposal["exact_proof"]["active_card_id"], main._DURALUDON)
        self.assertEqual(main._setup_ledger["active_serial"], 40)

    def test_duraludon_start_without_backup_preserves_parent_empty_action(self) -> None:
        hand = [card(main._DURALUDON, 40, 0)]
        self.commit_active(hand, selected_hand_index=0)
        raw = self.bench_raw(hand, active=[pokemon(main._DURALUDON, 40)])
        parent_action = []
        self.assertIs(self.call_parent(raw, parent_action), parent_action)
        self.assertEqual(main._last_telemetry["rejection_reason"], "no_duraludon_option")

    def test_turn_zero_intermediate_context_preserves_setup_ledger(self) -> None:
        hand = [card(main._CINDERACE, 30, 0), card(main._DURALUDON, 11, 0)]
        self.commit_active(hand)
        ...
        parent_action = [7]
        self.assertIs(self.call_parent(intermediate, parent_action), parent_action)
        self.assertEqual(main._last_telemetry["rejection_reason"], "setup_intermediate_passthrough")
        self.assertIsInstance(main._setup_ledger, dict)
        bench = self.bench_raw(hand)
        action = self.call_parent(bench, [])
        self.assertEqual(self.selected_serial(bench, action), 11)

    def test_invalid_setup_state_clears_ledger(self) -> None:
        hand = [card(main._CINDERACE, 30, 0), card(main._DURALUDON, 11, 0)]
        for kwargs, expected in (
            ({"turn": 1}, "bench_turn_mismatch"),
            ({"result": 0}, "bench_result_mismatch"),
        ...
        main._setup_ledger = None
        self.commit_active(hand, seat=0)
        other_seat_hand = [card(main._CINDERACE, 30, 1), card(main._DURALUDON, 11, 1)]
        raw = self.bench_raw(other_seat_hand, seat=1)
        parent_action = []
        self.assertIs(self.call_parent(raw, parent_action), parent_action)
        self.assertEqual(main._last_telemetry["rejection_reason"], "seat_mismatch")
        self.assertIsNone(main._setup_ledger)
