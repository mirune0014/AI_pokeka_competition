...
OPENING_WIDTH_SETUP_IDS = {GIBLE, ROSELIA, SPIRITOMB, BUDDY_POFFIN, FIGHTING_GONG, POKE_PAD}
OPENING_FIRST_RESOURCE_IDS = {ROCK_FIGHTING, POWER_WEIGHT}


def opening_width_before_first_resource_applies(obs) -> bool:
    """Delay Rock/Weight only while the acting player's main line is still bare."""
    main_line = [pokemon for pokemon in my_pokemon(obs) if pokemon.id in MAIN_LINE]
    opposing_pokemon = opponent_pokemon(obs)
    crustle_pressure_visible = (
        any(pokemon.id in CRUSTLE_PRESSURE_IDS for pokemon in opposing_pokemon)
        and any(energy_count(pokemon) for pokemon in opposing_pokemon)
    )
    return (
        int(getattr(obs.current, "turn", 0) or 0) <= 2
        and len(main_line) < 2
        and not any(energy_count(pokemon) for pokemon in main_line)
        and not crustle_pressure_visible
    )


def cap_opening_first_resource_after_width(obs, scored: list[tuple[int, int, str]]) -> list[tuple[int, int, str]]:
    """Put legal width/search plays ahead of the first Rock Fighting or Power Weight."""
    if not opening_width_before_first_resource_applies(obs):
        return scored
    eligible_scores = [
        score for score, index, _reason in scored
        if (
            obs.select.option[index].type == OptionType.PLAY
            and getattr(option_card(obs, obs.select.option[index]), "id", None) in OPENING_WIDTH_SETUP_IDS
        )
    ]
    if not eligible_scores:
        return scored
    cap = max(eligible_scores) - 1
    return [
        (min(score, cap), index, reason)
        if (
            obs.select.option[index].type == OptionType.ATTACH
            and getattr(option_card(obs, obs.select.option[index]), "id", None) in OPENING_FIRST_RESOURCE_IDS
        )
...
def choose_options(obs):
    ...
    scored = cap_opening_first_resource_after_width(obs, scored)
