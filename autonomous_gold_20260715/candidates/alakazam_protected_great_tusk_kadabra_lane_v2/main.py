...
def _has_psychic_energy(pokemon):
    return bool(
        pokemon is not None
        and any(card.id in PSYCHIC_ENERGY_IDS for card in pokemon.energyCards)
    )


def _protecting_energy_count(pokemon):
    if pokemon is None:
        return 0
    return sum(
        card.id in (Mist_Energy, Rock_Fighting_Energy)
        for card in pokemon.energyCards
    )
...
def _same_field_target(option, area, index):
    return option.inPlayArea == area and option.inPlayIndex == index


def _reserved_bypass_lane(obs, my_index):
    """Reconstruct the single public Kadabra lane; no cross-call memory."""
    my_state = obs.current.players[my_index]
    active = my_state.active[0] if my_state.active else None
    if active is not None and active.id == Kadabra and _has_psychic_energy(active):
        return {
            "kind": "active_ready", "area": AreaType.ACTIVE, "index": 0,
            "serial": active.serial, "pokemon": active, "evolve_options": [],
        }

    ready_bench = sorted(
        (
            (bench_index + 1, pokemon.serial, bench_index, pokemon)
            for bench_index, pokemon in enumerate(my_state.bench)
            if pokemon.id == Kadabra and _has_psychic_energy(pokemon)
        ),
        key=lambda item: (item[0], item[1]),
    )
    if ready_bench:
        _, _, bench_index, pokemon = ready_bench[0]
        return {
            "kind": "bench_ready", "area": AreaType.BENCH,
            "index": bench_index, "serial": pokemon.serial,
            "pokemon": pokemon, "evolve_options": [],
        }

    unready_bench = sorted(
        (
            (bench_index + 1, pokemon.serial, bench_index, pokemon)
            for bench_index, pokemon in enumerate(my_state.bench)
            if pokemon.id == Kadabra and not _has_psychic_energy(pokemon)
        ),
        key=lambda item: (item[0], item[1]),
    )
    if unready_bench:
        _, _, bench_index, pokemon = unready_bench[0]
        return {
            "kind": "bench_unready", "area": AreaType.BENCH,
            "index": bench_index, "serial": pokemon.serial,
            "pokemon": pokemon, "evolve_options": [],
        }
    ...
    for option_index, option in enumerate(obs.select.option):
        if option.type != OptionType.EVOLVE:
            continue
        card = _main_option_card(obs, option, my_index)
        if card is None or card.id != Kadabra:
            continue
        pokemon = get_card(obs, option.inPlayArea, option.inPlayIndex, my_index)
        if pokemon is None or pokemon.id != Abra:
            continue
        key = (option.inPlayArea, option.inPlayIndex, pokemon.serial)
        abra_targets.setdefault(key, []).append(option_index)
    if not abra_targets:
        return None

    def field_position(key):
        area, index, serial = key
        position = 0 if area == AreaType.ACTIVE else index + 1
        return position, serial

    area, index, serial = min(abra_targets, key=field_position)
    pokemon = get_card(obs, area, index, my_index)
    return {
        "kind": "creatable_abra", "area": area, "index": index,
        "serial": serial, "pokemon": pokemon,
        "evolve_options": abra_targets[(area, index, serial)],
    }


def _certified_fixed_active_ko(obs, my_index, parent_order):
    """Return a legal fixed-damage attack index that publicly KOs Active."""
    my_state = obs.current.players[my_index]
    ...
    if active is None or target is None:
        return None
    attacker_data = card_table.get(active.id)
    ...
    for option_index, option in enumerate(obs.select.option):
        ...
        if attack is None or attack.damage <= 0:
            continue
        if attack.text.strip() and attack.attackId != ATTACK_TELEPORTATION:
            continue
        damage = attack.damage
        if (target_data.weakness is not None
                and attacker_data.energyType == target_data.weakness):
            damage *= 2
        if (target_data.resistance is not None
                and attacker_data.energyType == target_data.resistance):
            damage = max(0, damage - 30)
        if damage >= target.hp:
            certified.append((parent_rank[option_index], option_index))
    return min(certified)[1] if certified else None


def _certified_alakazam_route(obs, my_index, hand_counts, target_pokemon,
                               target_use_boss):
    """Certify the parent's target using only an actual public Alakazam path."""
    if obs.select.context != SelectContext.MAIN or target_pokemon is None:
        return None
    state = obs.current
    ...
    evolution_indices = []
    evolution_cost = 0
    if active.id == Alakazam and _has_psychic_energy(active) and legal_powerful_hand:
        pass
    elif (active.id == Kadabra and _has_psychic_energy(active)
          and not my_state.asleep and not my_state.paralyzed
          and not my_state.confused and state.turn >= 2):
        evolution_indices = [
            option_index
            for option_index, option in enumerate(obs.select.option)
            if (option.type == OptionType.EVOLVE
                and _same_field_target(option, AreaType.ACTIVE, 0)
                and (_main_option_card(obs, option, my_index) is not None)
                and _main_option_card(obs, option, my_index).id == Alakazam)
        ]
        if not evolution_indices:
            return None
        evolution_cost = 1
    else:
        return None
    ...
    for option_index, option in enumerate(obs.select.option):
        ...
        card = _main_option_card(obs, option, my_index)
        if card is not None:
            play_indices[card.id].append(option_index)

    boss_cost = 0
    boss_indices = []
    if target_use_boss:
        if state.supporterPlayed or hand_counts[Boss_Orders] < 1:
            return None
        boss_indices = play_indices[Boss_Orders]
        if not boss_indices:
            return None
        boss_cost = 1

    protection_cost = _protecting_energy_count(target_pokemon)
    hammer_indices = []
    if protection_cost:
        hammer_indices = play_indices[Enhanced_Hammer]
        if (hand_counts[Enhanced_Hammer] < protection_cost
                or len(hammer_indices) < protection_cost):
            return None

    hand_after_costs = (
        len(my_state.hand or []) - evolution_cost - boss_cost - protection_cost
    )
    if hand_after_costs < 0 or hand_after_costs * 20 < target_pokemon.hp:
        return None

    necessary = evolution_indices + boss_indices + hammer_indices
    route_kind = "complete_hammer" if protection_cost else "unprotected"
    return {
        "kind": route_kind,
        "winning": prize_count(target_pokemon) >= len(my_state.prize),
        "necessary": set(necessary),
    }


def _protected_tusk_order(obs, parent_order, my_index, hand_counts,
                           target_pokemon, target_use_boss):
    """Narrow protected-Great-Tusk overlay; parent order is the final tie-break."""
    state = obs.current
    my_state = state.players[my_index]
    op_state = state.players[1 - my_index]
    opponent_active = op_state.active[0] if op_state.active else None
    if (opponent_active is None or opponent_active.id != Great_Tusk
            or _protecting_energy_count(opponent_active) == 0):
        return parent_order

    context = obs.select.context
    lane = _reserved_bypass_lane(obs, my_index)

    if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
        if lane is None or lane["kind"] != "bench_ready":
            return parent_order
        chosen = []
        for option_index, option in enumerate(obs.select.option):
            if option.type != OptionType.CARD or option.playerIndex != my_index:
                continue
            card = get_card(obs, option.area, option.index, my_index)
            if (card is not None and card.id == Kadabra
                    and card.serial == lane["serial"]):
                chosen.append(option_index)
        if not chosen:
            return parent_order
        chosen_set = set(chosen)
        return sorted(parent_order, key=lambda index: 0 if index in chosen_set else 1)

    if context != SelectContext.MAIN:
        return parent_order

    fixed_ko_index = _certified_fixed_active_ko(obs, my_index, parent_order)
    if (fixed_ko_index is not None
            and prize_count(opponent_active) >= len(my_state.prize)):
        return sorted(
            parent_order, key=lambda index: 0 if index == fixed_ko_index else 1,
        )

    higher_route = _certified_alakazam_route(
        obs, my_index, hand_counts, target_pokemon, target_use_boss,
    )
    if higher_route is not None:
        necessary = higher_route["necessary"]
        return sorted(parent_order, key=lambda index: 0 if index in necessary else 1)
    ...
    preferred = []
    if fixed_ko_index is not None:
        preferred.append((-1, parent_rank[fixed_ko_index], fixed_ko_index))
    elif lane["kind"] == "active_ready":
        preferred.extend(
            (0, parent_rank[option_index], option_index)
            for option_index, option in enumerate(obs.select.option)
            if (option.type == OptionType.ATTACK
                and option.attackId == ATTACK_SUPER_PSY_BOLT)
        )
    elif lane["kind"] == "bench_ready" and fixed_ko_index is None:
        preferred.extend(
            (0, parent_rank[option_index], option_index)
            for option_index, option in enumerate(obs.select.option)
            if option.type == OptionType.RETREAT
        )
    elif lane["kind"] == "bench_unready":
        if len(lane["pokemon"].energyCards) == 0:
            for option_index, option in enumerate(obs.select.option):
                if option.type != OptionType.ATTACH:
                    continue
                card = _main_option_card(obs, option, my_index)
                if (card is not None and card.id in PSYCHIC_ENERGY_IDS
                        and _same_field_target(option, lane["area"], lane["index"])):
                    subtype = 0 if card.id == Basic_Psychic_Energy else 1
                    preferred.append((subtype, parent_rank[option_index], option_index))
    elif lane["kind"] == "creatable_abra":
        preferred.extend(
            (0, parent_rank[option_index], option_index)
            for option_index in lane["evolve_options"]
        )
        if len(lane["pokemon"].energyCards) == 0:
            for option_index, option in enumerate(obs.select.option):
                if option.type != OptionType.ATTACH:
                ...
                if (card is not None and card.id in PSYCHIC_ENERGY_IDS
                        and _same_field_target(option, lane["area"], lane["index"])):
                    subtype = 1 if card.id == Basic_Psychic_Energy else 2
                    preferred.append((subtype, parent_rank[option_index], option_index))
    preferred.sort()

    forbidden = set()
    if lane["kind"] in ("active_ready", "bench_ready", "bench_unready"):
        for option_index, option in enumerate(obs.select.option):
            if option.type != OptionType.EVOLVE:
                continue
            card = _main_option_card(obs, option, my_index)
            if (card is not None and card.id == Alakazam
                    and _same_field_target(option, lane["area"], lane["index"])):
                forbidden.add(option_index)
    if (lane["kind"] in (
            "active_ready", "bench_ready", "bench_unready", "creatable_abra",
        ) and len(lane["pokemon"].energyCards) >= 1):
        for option_index, option in enumerate(obs.select.option):
            if (option.type == OptionType.ATTACH
                    and _same_field_target(
                        option, lane["area"], lane["index"],
                    )):
                # The bypass attacker needs exactly one Energy. Spending the
                # once-per-turn attachment on it again cannot advance the lane.
                forbidden.add(option_index)

    top_index = parent_order[0]
    top_option = obs.select.option[top_index]
    # Parent PLAY/ABILITY/search work is deliberately allowed to resolve. The
    # lane only replaces an immediately conflicting field action.
    if top_option.type in (OptionType.PLAY, OptionType.ABILITY):
        return parent_order

    conflict = top_index in forbidden
    if top_option.type == OptionType.ATTACK:
        conflict = not (
            top_index == fixed_ko_index
            or (lane["kind"] == "active_ready"
                and top_option.attackId == ATTACK_SUPER_PSY_BOLT)
        )
    elif top_option.type == OptionType.RETREAT:
        conflict = fixed_ko_index is not None or lane["kind"] != "bench_ready"
    elif top_option.type == OptionType.ATTACH:
        preferred_indices = {item[2] for item in preferred}
        if (top_index in preferred_indices
                and top_index == preferred[0][2]):
            return parent_order
        conflict = (
            top_index in forbidden
            or lane["kind"] in ("bench_unready", "creatable_abra")
        )
        if (_same_field_target(top_option, lane["area"], lane["index"])
                and lane["kind"] in ("active_ready", "bench_ready")):
            conflict = True
    elif top_option.type == OptionType.EVOLVE and top_index not in forbidden:
        return parent_order

    if conflict and preferred:
        chosen = preferred[0][2]
        return [chosen] + [index for index in parent_order if index != chosen]
    if top_index in forbidden:
        return sorted(
            parent_order,
            key=lambda index: (
                2 if index in forbidden
                else 1 if obs.select.option[index].type == OptionType.END
                else 0
            ),
        )
    return parent_order
...
def agent(obs_dict: dict) -> list[int]:
    ...
    desc_indices = _protected_tusk_order(
        obs,
        desc_indices,
        my_index,
        hand_counts,
