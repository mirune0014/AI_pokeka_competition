...
from .gold_teacher_labels import verify_teacher_labels
...
def _record_archetype(record: Mapping[str, Any]) -> str:
    top = record.get("own_archetype")
    metadata = record.get("current_metadata")
    nested = metadata.get("own_archetype") if isinstance(metadata, Mapping) else None
    if top is not None and nested is not None and str(top) != str(nested):
        raise ValueError("record archetype binding drift")
    value = nested if top is None else top
...
def build_examples(records: Iterable[Mapping[str, Any]], split_by_id: Mapping[str, str], *, archetype: str,
                   ...
                   include_public_history: bool = True,
                   target_overrides: Mapping[str, Mapping[str, Any]] | None = None) -> list[PromptExample]:
    """Build examples using recorded actions unless a verified caller supplies a target.

    ``target_overrides`` is deliberately keyed by decision ID and contains a
    single actor-visible canonical option.  It affects only the semantic
    target; option ordering and feature construction remain unchanged.
    """
    ...
    for record in records:
        ...
        if split not in requested:
            continue
        if _record_archetype(record) != str(archetype):
            continue
        observed_target = _one_selection_target(record)
        override = None if target_overrides is None else target_overrides.get(decision_id)
        if override is not None and not isinstance(override, Mapping):
            raise ValueError("target override must be a canonical option object")
        target = override if override is not None else observed_target
        ...
        if target_id not in by_id:
            if override is not None:
                raise ValueError("target override is not an actor-view legal option")
            continue
...
def load_teacher_examples(
    corpus_dir: str | Path,
    teacher_label_dirs: Sequence[str | Path],
    ...
    *,
...
) -> tuple[list[PromptExample], dict[str, str]]:
    """Load split-bound rollout targets from the upper-tier state corpus."""
    workspace = Path(workspace_root).resolve()
    corpus = Path(corpus_dir)
    corpus = (corpus if corpus.is_absolute() else workspace / corpus).resolve()
    split_path = Path(teacher_split_path)
    split_path = (split_path if split_path.is_absolute() else workspace / split_path).resolve()
    for path in (corpus, split_path):
        try:
            path.relative_to(workspace)
        except ValueError as error:
            raise ValueError("teacher input escapes workspace: %s" % path) from error
    if not teacher_label_dirs:
        raise ValueError("at least one teacher label artifact is required")
    verify_gold_upper_tier_states(corpus, workspace)
    split_result = verify_teacher_state_split(split_path, workspace)
    split_by_id = dict(split_result["split_by_decision_id"])
    split_file_hash = _sha256_file(split_path)
    overrides: dict[str, Mapping[str, Any]] = {}
    hashes = {
        "teacher_corpus_manifest_file_sha256": _sha256_file(corpus / "manifest.json"),
        "teacher_corpus_states_sha256": _sha256_file(corpus / "states.jsonl"),
        "teacher_split_file_sha256": split_file_hash,
        "teacher_split_manifest_sha256": str(split_result["manifest_sha256"]),
    }
    resolved_labels = []
    for raw in teacher_label_dirs:
        path = Path(raw)
        path = (path if path.is_absolute() else workspace / path).resolve()
        try:
            path.relative_to(workspace)
        except ValueError as error:
            raise ValueError("teacher label artifact escapes workspace: %s" % path) from error
        resolved_labels.append(path)
    for index, label_dir in enumerate(sorted(set(resolved_labels), key=str)):
        verified = verify_teacher_labels(label_dir, workspace)
        manifest_path = label_dir / "manifest.json"
        manifest = json.loads(manifest_path.read_text(encoding="ascii"))
        split_binding = manifest.get("inputs", {}).get("teacher_split")
        if (
            not isinstance(split_binding, Mapping)
            or split_binding.get("sha256") != split_file_hash
            or split_binding.get("manifest_sha256") != split_result["manifest_sha256"]
        ):
            raise ValueError("teacher label artifact is not bound to the selected teacher split")
        hashes["teacher_label_%03d_manifest_file_sha256" % index] = _sha256_file(manifest_path)
        hashes["teacher_label_%03d_manifest_sha256" % index] = str(verified["manifest_sha256"])
        for decision_id, target in verified["target_overrides"].items():
            previous = overrides.get(decision_id)
            if previous is not None and previous != target:
                raise ValueError("teacher label artifacts conflict for decision: %s" % decision_id)
    ...
    with (corpus / "states.jsonl").open(encoding="ascii") as handle:
        for line in handle:
            ...
            if not isinstance(value, dict) or not isinstance(value.get("decision_id"), str):
                raise ValueError("teacher corpus contains an invalid state row")
            decision_id = value["decision_id"]
            if decision_id in seen_decisions:
                raise ValueError("teacher corpus contains duplicate decisions")
    ...
    consumed = {example.decision_id for example in examples}
    if consumed != set(overrides):
        raise ValueError("teacher targets were not consumed exactly by the selected corpus and splits")
    return examples, dict(sorted(hashes.items()))
