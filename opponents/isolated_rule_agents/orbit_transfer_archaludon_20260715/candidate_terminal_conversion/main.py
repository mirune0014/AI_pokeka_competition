...
_TERMINAL_ATTACKS_BY_ACTIVE = {
    DURALUDON: {223, RAGING_HAMMER},
    ARCHALUDON: {COATED_ATTACK},
    ARCHALUDON_EX: {METAL_DEFENDER},
}
_TERMINAL_CONVERSION_SCORE = 1000000000
_PUBLIC_UNCERTAINTY_MARKERS = ("effect", "protect", "prevent", "reduction", "status", "condition")
_RISKY_VISIBLE_TEXT = (
    "prevent all damage",
    "prevent damage",
    "less damage",
...
def _normalized_public_type(value):
    value = getattr(value, "value", getattr(value, "id", value))
    return value if isinstance(value, int) and not isinstance(value, bool) else None


def _visible_prize_value(pokemon):
...
def _card_effect_text(card):
    data = CARD_DB.get(getattr(card, "id", getattr(card, "cardId", None)))
    return " ".join(
...
def _has_unresolved_public_fields(subject):
    if subject is None:
        return True
    for name, value in getattr(subject, "__dict__", {}).items():
        lowered = name.lower()
        if any(marker in lowered for marker in _PUBLIC_UNCERTAINTY_MARKERS):
            if value not in (None, False, 0, "", (), [], {}):
                return True
    return False


def _visible_effect_cards(obs):
    cards = list(getattr(obs.current, "stadium", None) or [])
    for player in getattr(obs.current, "players", None) or []:
        for pokemon in (getattr(player, "active", None) or []) + (getattr(player, "bench", None) or []):
            if pokemon is None:
                continue
...
def _terminal_damage_state_is_public(obs, target):
    player = my_state(obs)
    active = active_pokemon(obs)
    for status in ("confused", "asleep", "paralyzed"):
        if getattr(player, status, None) is not False:
            return False
    active_data = CARD_DB.get(getattr(active, "id", None))
    if _opp_last_attack_id == COATED_ATTACK and getattr(active_data, "basic", False):
        return False
    if getattr(obs.select, "effect", None) is not None:
        return False
    if any(
        _has_unresolved_public_fields(subject)
        for subject in (obs.current, player, opp_state(obs), active, target)
    ):
        return False

    for card in _visible_effect_cards(obs):
        card_id = getattr(card, "id", getattr(card, "cardId", None))
        if card_id == FULL_METAL_LAB:
            continue
        text = _card_effect_text(card)
        if any(phrase in text for phrase in _RISKY_VISIBLE_TEXT):
            return False
    return True


def conservative_terminal_damage(obs, target, attack_id):
    """Return a public lower bound, ignoring Weakness and failing closed."""
    active = active_pokemon(obs)
    if (
        active is None
        or attack_id not in _TERMINAL_ATTACKS_BY_ACTIVE.get(active.id, set())
        or not _terminal_damage_state_is_public(obs, target)
    ):
        return None
    ...
    if active_data is None or target_data is None:
        return None
    attack_type = _normalized_public_type(getattr(active_data, "energyType", None))
    raw_resistance = getattr(target_data, "resistance", None)
    target_resistance = _normalized_public_type(raw_resistance)
    target_type = _normalized_public_type(getattr(target_data, "energyType", None))
    if attack_type is None or target_type is None or (raw_resistance is not None and target_resistance is None):
        return None
    if attack_type is not None and target_resistance == attack_type:
        damage = max(0, damage - 30)

    full_metal_lab_visible = any(
        getattr(card, "id", getattr(card, "cardId", None)) == FULL_METAL_LAB
        for card in (getattr(obs.current, "stadium", None) or [])
        if card is not None
    )
    if full_metal_lab_visible and target_type == METAL_ENERGY:
        damage = max(0, damage - 30)
    return damage


def terminal_conversion_surplus(obs, opt):
    """Certify a legal attack that visibly takes every remaining Prize."""
    if obs.select.context != SelectContext.MAIN or opt.type != OptionType.ATTACK:
        return None
    if not any(candidate is opt for candidate in (obs.select.option or [])):
        return None
    target = opp_active_pokemon(obs)
    if target is None:
        return None
    remaining_prizes = len(getattr(my_state(obs), "prize", None) or [])
    target_prizes = _visible_prize_value(target)
    if remaining_prizes < 1 or target_prizes is None or remaining_prizes > target_prizes:
        return None
    damage = conservative_terminal_damage(obs, target, getattr(opt, "attackId", None))
    target_hp = getattr(target, "hp", None)
    if damage is None or not isinstance(target_hp, int) or target_hp <= 0 or damage < target_hp:
        return None
...
def score_option(obs, opt):
    ...
    if ctx == SelectContext.MAIN:
        ...
        if fn:
            ...
            surplus = terminal_conversion_surplus(obs, opt)
            if surplus is None:
                score, reason = best_attack_damage(obs, opt.attackId), "attack"
            else:
                score = _TERMINAL_CONVERSION_SCORE + surplus
                reason = "terminal conversion"
