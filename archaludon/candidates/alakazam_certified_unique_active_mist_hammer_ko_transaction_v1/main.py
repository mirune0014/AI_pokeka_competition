...
from cg.api import AreaType, CardType, EnergyType, LogType, Observation, SelectContext, OptionType, Card, Pokemon, all_attack, all_card_data, to_observation_class
...
def _unique_mist_hammer_parent_agent(obs_dict: dict) -> list[int]:
    ...
    if guarded_teleportation_action is not None:
        return _remember_action(
            _decision_signature(obs), guarded_teleportation_action
        )
    ...
    if (
        ...
        if teleportation_action is not None:
            return _remember_action(
                _decision_signature(obs), teleportation_action
            )
...
# ---------------------------------------------------------------------------
# Certified unique-Active-Mist Hammer -> Powerful Hand transaction v1.
#
# This overlay is intentionally post-parent.  It cannot make Enhanced Hammer
# top-ranked: it starts only after the exact guarded-Teleportation parent has
# already returned one concrete Hammer PLAY option.  Every continuation is
# tied to physical public serials and fails closed to a clean parent call.
# ---------------------------------------------------------------------------

_unique_mist_hammer_latch = {}
_unique_mist_hammer_last_signature = None
_unique_mist_hammer_last_action = None
_unique_mist_hammer_last_abort_reason = None
_unique_mist_prior_attack_witness = {}
_unique_mist_hammer_telemetry = {
    "starts": 0,
    "targets": 0,
    "attacks": 0,
    "resolutions": 0,
...
_UNIQUE_MIST_SELF_EFFECT_SHIELD_ATTACKS = (
    75,
    244,
    505,
    595,
...
_UNIQUE_MIST_PROXY_ATTACKS = (213, 403, 528, 612, 665, 886, 1378)
_UNIQUE_MIST_ATTACKER_DENIAL_ATTACKS = (
    112,
    139,
    653,
    716,
...
_UNIQUE_MIST_SAFE_PRIOR_ATTACKS = (479,)

_UNIQUE_MIST_LOG_SCHEMA_FIELDS = (
    "type",
    "playerIndex",
    "hasBasicPokemon",
    "cardId",
...
# This is deliberately a closed certificate, not a text-pattern allowlist.
# Any catalog drift makes the one safe public-history route unavailable.
_UNIQUE_MIST_SAFE_479_CATALOG_FINGERPRINT = (
    345,
    "Crustle",
    int(CardType.POKEMON),
    150,
    False,
    False,
    False,
...
_UNIQUE_MIST_PARENT_DICT_NAMES = (
    "_hilda_source_latch",
    "_enriching_reserve_latch",
    "_fez_ko_bridge_latch",
    "_active_psychic_ko_latch",
...
def _unique_mist_clear_transaction(*, clear_duplicate: bool = True) -> None:
    global _unique_mist_hammer_last_signature
    global _unique_mist_hammer_last_action
    _unique_mist_hammer_latch.clear()
    if clear_duplicate:
        _unique_mist_hammer_last_signature = None
        _unique_mist_hammer_last_action = None


def _unique_mist_clear_prior_attack_witness() -> None:
    _unique_mist_prior_attack_witness.clear()


def _unique_mist_abort(reason: str) -> None:
    global _unique_mist_hammer_last_abort_reason
    _unique_mist_hammer_last_abort_reason = reason
    _unique_mist_hammer_telemetry["aborts"] += 1
    reasons = _unique_mist_hammer_telemetry["abort_reasons"]
    reasons[reason] = reasons.get(reason, 0) + 1
    _unique_mist_clear_transaction()


def _unique_mist_parent_snapshot() -> dict:
    return {
        "pre_turn": pre_turn,
        "ability_used_dudunsparce": ability_used_dudunsparce,
        "ability_used_fezandipiti": ability_used_fezandipiti,
        "_last_decision_signature": copy.deepcopy(_last_decision_signature),
        "_last_decision_action": copy.deepcopy(_last_decision_action),
        "dicts": {
            name: copy.deepcopy(globals()[name])
            for name in _UNIQUE_MIST_PARENT_DICT_NAMES
        },
    }


def _unique_mist_restore_parent(snapshot: dict) -> None:
    global pre_turn, ability_used_dudunsparce, ability_used_fezandipiti
    global _last_decision_signature, _last_decision_action
    ...
    _last_decision_signature = copy.deepcopy(
        snapshot["_last_decision_signature"]
    )
    _last_decision_action = copy.deepcopy(snapshot["_last_decision_action"])
    for name, value in snapshot["dicts"].items():
        globals()[name].clear()
        globals()[name].update(copy.deepcopy(value))


def _unique_mist_cards_fingerprint(cards) -> tuple | None:
    rows = []
    for card in cards or []:
        if (
            card is None
            or not isinstance(card.id, int)
            or not isinstance(card.serial, int)
            or card.id <= 0
            or card.serial <= 0
            or card_table.get(card.id) is None
        ):
...
def _unique_mist_pokemon_group_fingerprint(
    pokemon_rows, expected_player: int
) -> tuple | None:
    rows = []
    protected = []
    for pokemon in pokemon_rows or []:
        if not _bridge_pokemon_is_publicly_complete(
            pokemon, expected_player
        ):
            return None
        rows.append(_bridge_pokemon_fingerprint(pokemon))
...
def _unique_mist_statuses(state) -> tuple:
    return tuple(
        value
        for player in state.players
        for value in (
...
def _unique_mist_attacker_is_status_clear(state) -> bool:
    if state.yourIndex not in (0, 1):
        return False
    mine = state.players[state.yourIndex]
    return not any(
        (
...
_UNIQUE_MIST_OPTION_SCHEMA_FIELDS = (
    "type",
    "number",
    "area",
    "index",
...
def _unique_mist_option_signature(option) -> tuple:
    return tuple(
        getattr(option, name, None)
        for name in _UNIQUE_MIST_OPTION_SCHEMA_FIELDS
    )


def _unique_mist_strict_int(value, *, minimum: int | None = None) -> bool:
    return (
...
def _unique_mist_turn_player(state) -> int | None:
    if (
        not _unique_mist_strict_int(state.turn, minimum=1)
        or not _unique_mist_strict_int(state.firstPlayer, minimum=0)
...
def _unique_mist_log_signature(log) -> tuple | None:
    values = vars(log)
    if set(values) != set(_UNIQUE_MIST_LOG_SCHEMA_FIELDS):
        return None
    row = []
    for name in _UNIQUE_MIST_LOG_SCHEMA_FIELDS:
        value = values[name]
...
def _unique_mist_log_has_exact_non_null_fields(
    log, allowed: set[str]
) -> bool:
    values = vars(log)
    return (
        set(values) == set(_UNIQUE_MIST_LOG_SCHEMA_FIELDS)
        and {
...
def _unique_mist_boundary_log_fingerprint(
    log, expected_type: LogType
) -> tuple | None:
    allowed = {"type", "playerIndex"}
    if (
        log.type != expected_type
        or not _unique_mist_strict_int(log.playerIndex, minimum=0)
        or log.playerIndex not in (0, 1)
        or not _unique_mist_log_has_exact_non_null_fields(log, allowed)
    ):
        return None
    return (int(expected_type), log.playerIndex)


def _unique_mist_trailing_own_draw_log_fingerprint(
    log, my_index: int
) -> tuple | None:
    """Allow only the canonical turn draw bundled after our TURN_START."""
    allowed = {"type", "playerIndex", "cardId", "serial"}
    if (
        log.type != LogType.DRAW
        or not _unique_mist_strict_int(log.playerIndex, minimum=0)
        or log.playerIndex != my_index
        or not _unique_mist_strict_int(log.cardId, minimum=1)
        or card_table.get(log.cardId) is None
        or not _unique_mist_strict_int(log.serial, minimum=1)
        or not _unique_mist_log_has_exact_non_null_fields(log, allowed)
    ):
        return None
    return (int(LogType.DRAW), my_index, log.cardId, log.serial)


def _unique_mist_logs_fingerprint(logs) -> tuple:
    rows = tuple(_unique_mist_log_signature(log) for log in logs or [])
    if any(row is None for row in rows):
        return ("malformed_log_schema",)
    return rows


def _unique_mist_attack_log_fingerprint(log) -> tuple | None:
    allowed = {"type", "playerIndex", "cardId", "serial", "attackId"}
    if (
        ...
        or not _unique_mist_strict_int(log.cardId, minimum=1)
        or not _unique_mist_strict_int(log.serial, minimum=1)
        or not _unique_mist_strict_int(log.attackId, minimum=1)
        or card_table.get(log.cardId) is None
        or attack_table.get(log.attackId) is None
        or not _unique_mist_log_has_exact_non_null_fields(log, allowed)
    ):
        return None
    return (log.playerIndex, log.cardId, log.serial, log.attackId)


def _unique_mist_safe_479_catalog_fingerprint() -> tuple | None:
    data = card_table.get(345)
    attack = attack_table.get(479)
    ...
    fingerprint = (
        ...
        tuple(
            (
                skill.name.strip(),
                " ".join(_normalized_skill_text(skill.text).split()),
            )
            for skill in data.skills or []
        ),
        ...
        attack.name,
        attack.damage,
        tuple(int(value) for value in attack.energies or []),
        " ".join(_normalized_skill_text(attack.text).split()),
    )
    if fingerprint != _UNIQUE_MIST_SAFE_479_CATALOG_FINGERPRINT:
        return None
    return fingerprint


def _unique_mist_prior_attack_witness_fingerprint() -> tuple:
    witness = _unique_mist_prior_attack_witness
    return tuple(
        witness.get(name)
        for name in (
            "status",
            ...
            "attacker_serial",
            "attack_id",
            "source_active_card_id",
            "source_active_serial",
            "boundary_fingerprint",
            "post_close_fingerprint",
            "catalog_fingerprint",
        )
    )


def _unique_mist_mark_history_ambiguous(
    state, *, prior_turn: int | None = None
) -> None:
    current = _unique_mist_prior_attack_witness
    player = state.yourIndex if state.yourIndex in (0, 1) else None
    if prior_turn is None:
        prior_turn = current.get("prior_turn")
    current.clear()
    current.update(
        status="ambiguous",
        player=player,
        opponent=None if player is None else 1 - player,
        first_player=state.firstPlayer,
        prior_turn=prior_turn,
        certified_turn=None if prior_turn is None else prior_turn + 1,
        attacker_card_id=None,
        attacker_serial=None,
        attack_id=None,
        source_active_card_id=None,
        source_active_serial=None,
        boundary_fingerprint=(),
        post_close_fingerprint=(),
...
def _unique_mist_current_source_matches(
    state, card_id: int, serial: int
) -> bool:
    if state.yourIndex not in (0, 1):
        return False
    ...
    return (
        len(active) == 1
        and _bridge_pokemon_is_publicly_complete(active[0], opponent)
        and active[0].id == card_id
        and active[0].serial == serial
    )


def _unique_mist_begin_pending_history(state) -> None:
    prior_turn = (
        state.turn
        if _unique_mist_turn_player(state) == 1 - state.yourIndex
        else state.turn - 1
    )
    _unique_mist_prior_attack_witness.clear()
    _unique_mist_prior_attack_witness.update(
        status="pending",
        player=state.yourIndex,
        opponent=1 - state.yourIndex,
        first_player=state.firstPlayer,
        prior_turn=prior_turn,
        certified_turn=prior_turn + 1,
        attacker_card_id=None,
        attacker_serial=None,
        attack_id=None,
        source_active_card_id=None,
...
def _unique_mist_history_batch_key(obs: Observation) -> tuple | None:
    if not _unique_mist_strict_int(obs.current.turnActionCount, minimum=0):
        return None
    logs = _unique_mist_logs_fingerprint(obs.logs)
    if logs == ("malformed_log_schema",):
        return None
    return (obs.current.turn, obs.current.turnActionCount, logs)


def _unique_mist_finalize_prior_attack_history(state) -> None:
    current = _unique_mist_prior_attack_witness
    opponent = current.get("opponent")
    card_id = current.get("attacker_card_id")
    serial = current.get("attacker_serial")
    attack_id = current.get("attack_id")
    boundary = current.get("boundary_fingerprint")
    catalog = _unique_mist_safe_479_catalog_fingerprint()
    exact_boundary = (
        (int(LogType.TURN_START), opponent),
        (int(LogType.ATTACK), opponent, card_id, serial, attack_id),
        (int(LogType.TURN_END), opponent),
        (int(LogType.TURN_START), current.get("player")),
    )
    if (
        current.get("saw_start") is not True
        or current.get("saw_end") is not True
        or current.get("saw_own_start") is not True
        or current.get("attack_count") != 1
        or boundary != exact_boundary
        or card_id != 345
        or attack_id != 479
        or attack_id not in _UNIQUE_MIST_SAFE_PRIOR_ATTACKS
        or not _unique_mist_strict_int(serial, minimum=1)
        or catalog is None
        or state.turn != current.get("certified_turn")
        or _unique_mist_turn_player(state) != current.get("player")
        or not _unique_mist_current_source_matches(state, card_id, serial)
    ):
        _unique_mist_mark_history_ambiguous(
            state, prior_turn=current.get("prior_turn")
        )
        return
    current.update(
        status="safe_complete",
        source_active_card_id=card_id,
        source_active_serial=serial,
        catalog_fingerprint=catalog,
    )


def _unique_mist_observe_public_attack_history(obs: Observation) -> None:
    """Persist one closed, complete immediately-prior-turn certificate."""
    state = obs.current
    my_index = state.yourIndex
    turn_player = _unique_mist_turn_player(state)
    batch_key = _unique_mist_history_batch_key(obs)
    if my_index not in (0, 1) or turn_player is None or batch_key is None:
        _unique_mist_clear_prior_attack_witness()
        return
    opponent = 1 - my_index
    current = _unique_mist_prior_attack_witness
    if current and batch_key in current.get("processed_batches", ()):
        return
    if current and (
        current.get("player") != my_index
        or current.get("opponent") != opponent
        or current.get("first_player") != state.firstPlayer
        or state.turn < current.get("last_observed_turn", state.turn)
    ):
        _unique_mist_clear_prior_attack_witness()
        return
    opponent_start_positions = [
        index
        for index, log in enumerate(obs.logs)
        if log.type == LogType.TURN_START and log.playerIndex == opponent
    ]
    if current:
        jump = state.turn - current.get("last_observed_turn", state.turn)
        if (
            current.get("status") == "pending" and jump > 1
        ) or (
            current.get("status") != "pending" and jump > 2
        ):
            _unique_mist_mark_history_ambiguous(state)
            return
        if current.get("status") != "pending" and len(
            opponent_start_positions
        ) == 1:
            proposed_prior = (
                state.turn if turn_player == opponent else state.turn - 1
            )
            previous_prior = current.get("prior_turn")
            if (
                previous_prior is not None
                and proposed_prior <= previous_prior
            ):
                _unique_mist_mark_history_ambiguous(
                    state, prior_turn=previous_prior
                )
                return

    if (
        current.get("status") == "safe_complete"
        and not opponent_start_positions
    ):
        if (
            state.turn != current.get("certified_turn")
            or turn_player != my_index
            or not _unique_mist_current_source_matches(
                state,
                current.get("source_active_card_id"),
                current.get("source_active_serial"),
            )
            or any(
                log.type
                in (LogType.ATTACK, LogType.TURN_END, LogType.TURN_START)
                and log.playerIndex == opponent
                for log in obs.logs
            )
        ):
            _unique_mist_mark_history_ambiguous(
    ...
    if current.get("status") != "pending":
        if len(opponent_start_positions) != 1:
            if any(
                log.type in (LogType.ATTACK, LogType.TURN_END)
                and log.playerIndex == opponent
                for log in obs.logs
            ):
                _unique_mist_mark_history_ambiguous(state)
            return
        _unique_mist_begin_pending_history(state)
        current = _unique_mist_prior_attack_witness
        start_index = opponent_start_positions[0]
        if any(
            log.type in (LogType.ATTACK, LogType.TURN_END)
            and log.playerIndex == opponent
            for log in obs.logs[:start_index]
        ):
            _unique_mist_mark_history_ambiguous(
                state, prior_turn=current.get("prior_turn")
            )
            return
        logs = obs.logs[start_index:]
    else:
        logs = obs.logs
        if state.turn not in (
            current.get("prior_turn"),
            current.get("certified_turn"),
        ):
            _unique_mist_mark_history_ambiguous(
                state, prior_turn=current.get("prior_turn")
            )
    ...
    current["processed_batches"] = current.get(
        "processed_batches", ()
    ) + (batch_key,)
    current["last_observed_turn"] = state.turn
    for log in logs:
        if current.get("saw_own_start"):
            draw_row = _unique_mist_trailing_own_draw_log_fingerprint(
                log, my_index
            )
            if draw_row is None or current.get("saw_own_draw"):
                _unique_mist_mark_history_ambiguous(
                    state, prior_turn=current.get("prior_turn")
                )
                return
            current["saw_own_draw"] = True
            current["post_close_fingerprint"] += (draw_row,)
            continue
        if log.type == LogType.TURN_START:
            boundary_row = _unique_mist_boundary_log_fingerprint(
                log, LogType.TURN_START
            )
            if boundary_row is None:
                ...
                return
            if log.playerIndex == opponent:
                if current.get("saw_start"):
                    _unique_mist_mark_history_ambiguous(
                        state, prior_turn=current.get("prior_turn")
                    )
                    return
            ...
            elif log.playerIndex == my_index:
                if (
                    not current.get("saw_start")
                    or not current.get("saw_end")
                    or current.get("saw_own_start")
                ):
                    _unique_mist_mark_history_ambiguous(
                        state, prior_turn=current.get("prior_turn")
                    )
                    return
                current["saw_own_start"] = True
                current["boundary_fingerprint"] += (
                    boundary_row,
                )
            else:
            ...
            boundary_row = _unique_mist_boundary_log_fingerprint(
                log, LogType.TURN_END
            )
            if (
                boundary_row is None
                or log.playerIndex != opponent
                or not current.get("saw_start")
                or current.get("saw_end")
            ):
                _unique_mist_mark_history_ambiguous(
                    state, prior_turn=current.get("prior_turn")
                )
            ...
            row = _unique_mist_attack_log_fingerprint(log)
            if (
                log.playerIndex != opponent
                or not current.get("saw_start")
                or current.get("saw_end")
                or current.get("attack_count") != 0
                or row is None
            ):
                _unique_mist_mark_history_ambiguous(
                    state, prior_turn=current.get("prior_turn")
                ...
                return
            _, card_id, serial, attack_id = row
            current.update(
                attack_count=1,
    ...
    if current.get("saw_own_start"):
        _unique_mist_finalize_prior_attack_history(state)


def _unique_mist_prior_attack_is_certified_safe(
    state, target: Pokemon
) -> bool:
    witness = _unique_mist_prior_attack_witness
    my_index = state.yourIndex
    if (
        _unique_mist_turn_player(state) != my_index
        or witness.get("status") != "safe_complete"
        or witness.get("player") != my_index
        or witness.get("opponent") != 1 - my_index
        or witness.get("first_player") != state.firstPlayer
        or witness.get("prior_turn") != state.turn - 1
        or witness.get("certified_turn") != state.turn
    ):
        return False
    attack_id = witness.get("attack_id")
    card_id = witness.get("attacker_card_id")
    attacker_serial = witness.get("attacker_serial")
    if (
        not _unique_mist_strict_int(attack_id, minimum=1)
        or not _unique_mist_strict_int(card_id, minimum=1)
        or not _unique_mist_strict_int(attacker_serial, minimum=1)
        or attack_table.get(attack_id) is None
        or card_table.get(card_id) is None
    ):
        return False
    if attack_id != 479 or attack_id not in _UNIQUE_MIST_SAFE_PRIOR_ATTACKS:
        return False
    return (
        card_id == 345
        and target.id == 345
        and attacker_serial == target.serial
        and witness.get("source_active_card_id") == card_id
        and witness.get("source_active_serial") == attacker_serial
        and _unique_mist_current_source_matches(
            state, card_id, attacker_serial
        )
        and witness.get("catalog_fingerprint")
        == _UNIQUE_MIST_SAFE_479_CATALOG_FINGERPRINT
        and _unique_mist_safe_479_catalog_fingerprint()
        == _UNIQUE_MIST_SAFE_479_CATALOG_FINGERPRINT
    )


def _unique_mist_unused_option_fields_are_none(
    option, *, used: tuple[str, ...]
) -> bool:
    used_fields = set(used)
    return all(
        name in used_fields or getattr(option, name, None) is None
        for name in _UNIQUE_MIST_OPTION_SCHEMA_FIELDS
    )


def _unique_mist_resolve_play_option(
    obs: Observation, option, player_index: int
) -> Card | None:
    if (
        option.type != OptionType.PLAY
        or not _unique_mist_strict_int(option.index, minimum=0)
        or not _unique_mist_unused_option_fields_are_none(
            option, used=("type", "index")
        )
    ):
        return None
    card = _safe_get_card(obs, AreaType.HAND, option.index, player_index)
    if (
...
def _unique_mist_canonical_play_rows(
    obs: Observation, player_index: int
) -> list[tuple[int, Card]] | None:
    rows = []
    for option_index, option in enumerate(obs.select.option):
        if option.type != OptionType.PLAY:
            continue
        card = _unique_mist_resolve_play_option(
            obs, option, player_index
        )
        if card is None:
            return None
        rows.append((option_index, card))
    return rows


def _unique_mist_observation_signature(obs: Observation) -> tuple:
    state = obs.current
    select = obs.select
    players = []
    for player_index, player in enumerate(state.players):
        players.append(
            (
                ...
                player.deckCount,
                len(player.prize),
                _unique_mist_cards_fingerprint(player.hand),
                _unique_mist_cards_fingerprint(player.discard),
                _unique_mist_pokemon_group_fingerprint(
                    player.active, player_index
                ),
                _unique_mist_pokemon_group_fingerprint(
                    player.bench, player_index
                ),
                player.benchMax,
                player.poisoned,
                player.burned,
    ...
    return (
        _unique_mist_prior_attack_witness_fingerprint(),
        _unique_mist_logs_fingerprint(obs.logs),
        state.turn,
        state.turnActionCount,
        state.yourIndex,
        state.result,
        ...
        state.retreated,
        tuple(players),
        _unique_mist_cards_fingerprint(state.stadium),
        int(select.context),
        select.minCount,
        select.maxCount,
        getattr(select.effect, "id", None),
        getattr(select.effect, "serial", None),
        getattr(select.effect, "playerIndex", None),
        getattr(select.contextCard, "id", None),
        getattr(select.contextCard, "serial", None),
        tuple(_unique_mist_option_signature(option) for option in select.option),
    )


def _unique_mist_cache_action(
    obs: Observation, action: list[int]
) -> list[int]:
    global _unique_mist_hammer_last_signature
    global _unique_mist_hammer_last_action
    _unique_mist_hammer_last_signature = _unique_mist_observation_signature(
        obs
    )
    _unique_mist_hammer_last_action = tuple(action)
    return list(action)


def _unique_mist_remove_serial(rows: tuple, serial: int) -> tuple | None:
...
def _unique_mist_projected_pokemon_without_energy(
    pokemon: Pokemon, energy_serial: int
) -> tuple | None:
    fingerprint = _bridge_pokemon_fingerprint(pokemon)
    energy_cards = fingerprint[7]
    matches = [
        index for index, row in enumerate(energy_cards) if row[1] == energy_serial
    ]
    if len(matches) != 1 or len(fingerprint[6]) != len(energy_cards):
        return None
    index = matches[0]
    return fingerprint[:6] + (
        fingerprint[6][:index] + fingerprint[6][index + 1 :],
        energy_cards[:index] + energy_cards[index + 1 :],
    ) + fingerprint[8:]


def _unique_mist_energy_units_exact(pokemon: Pokemon) -> bool:
    if len(pokemon.energies) != len(pokemon.energyCards):
        return False
    expected = []
    for card in pokemon.energyCards:
        ...
        if unit is None:
            return False
        expected.append(unit)
    return tuple(int(value) for value in pokemon.energies) == tuple(expected)


def _unique_mist_skill_is_counter_safe(
    data, *, relation: str, source_kind: str
) -> bool:
    """Accept only effect text proven irrelevant to this counter placement."""
    for skill in data.skills or []:
        text = " ".join(_normalized_skill_text(skill.text).split())
        if not text:
            continue

        # Exact local effects in the positive Crustle family.
        if data.cardId == 345 and (
            "prevent all damage done to this pokemon by attacks from your "
            "opponent's pokemon {ex}"
        ) in text:
            if card_table[Alakazam].ex:
                return False
            continue
        if data.cardId == 18 and (
            "the {g} pokemon this card is attached to gets +20 hp" in text
        ):
            continue
        if data.cardId in (14, 1156) and (
            "is damaged by an attack" in text
            and "attacking pokemon" in text
        ):
            # Powerful Hand places counters and does not damage the target.
            continue
        if data.cardId == Battle_Cage and (
            "benched pokemon" in text
            and "damage from attacks is still taken" in text
        ):
            continue
        if data.cardId == Mist_Energy and relation != "target":
            # Its printed protection is attached-local to another Pokemon.
            continue
        if data.cardId == Rock_Fighting_Energy and relation != "target":
            continue

        # Pure Energy provision and already-resolved local search/HP clauses
        # cannot alter the current attack's damage-counter count.
        if source_kind == "energy" and text.startswith(
            "as long as this card is attached to a pokemon, it provides"
        ):
            dangerous = (
                "prevent all effects",
            ...
            if not any(marker in text for marker in dangerous):
                continue
        if (
            text.startswith("once during your turn")
            or text.startswith("when you play this pokemon")
        ) and not any(
            marker in text
            for marker in (
                "damage counter",
            ...
            continue
        if relation != "target" and (
            "this pokemon" in text
            or "pokemon this card is attached to" in text
        ) and not any(
            marker in text
            for marker in (
...
def _unique_mist_counter_environment_clear(
    state, target: Pokemon, removed_mist_serial: int | None
) -> bool:
    target_owner = 1 - state.yourIndex
    if not _bridge_pokemon_is_publicly_complete(target, target_owner):
        return False
    if not _unique_mist_energy_units_exact(target):
        return False
    target_data = card_table.get(target.id)
    if target_data is None or target_data.cardType != CardType.POKEMON:
        return False
    if not _unique_mist_skill_is_counter_safe(
        target_data, relation="target", source_kind="pokemon"
    ):
        return False
    ...
    for energy in target.energyCards:
        relation = "target"
        if energy.serial == removed_mist_serial:
            if energy.id != Mist_Energy:
                return False
            seen_removed += 1
            continue
        ...
        if energy.id in (Mist_Energy, Rock_Fighting_Energy):
            if energy.id == Mist_Energy or (
                target_data.energyType == EnergyType.FIGHTING
            ):
                return False
        if not _unique_mist_skill_is_counter_safe(
            data, relation=relation, source_kind="energy"
        ):
            return False
    if removed_mist_serial is not None and seen_removed != 1:
        return False
    if removed_mist_serial is None and seen_removed:
        return False

    for tool in target.tools:
        data = card_table.get(tool.id)
        if data is None or data.cardType != CardType.TOOL:
            return False
        if not _unique_mist_skill_is_counter_safe(
            data, relation="target", source_kind="tool"
        ):
            return False

    for stadium in state.stadium:
        data = card_table.get(stadium.id)
        if data is None or data.cardType != CardType.STADIUM:
            return False
        if not _unique_mist_skill_is_counter_safe(
            data, relation="global", source_kind="stadium"
        ):
            return False

    # Any visible non-target source with a genuinely global counter/attack
    # modifier remains uncertain.  Attached-local effects on another Pokemon
    # are explicitly separated by relation.
    for owner_index, owner in enumerate(state.players):
        for pokemon in list(owner.active) + list(owner.bench):
            if pokemon is None or pokemon.serial == target.serial:
                continue
            if not _bridge_pokemon_is_publicly_complete(
                pokemon, owner_index
            ):
                return False
            data = card_table.get(pokemon.id)
            if data is None or not _unique_mist_skill_is_counter_safe(
                data, relation="other", source_kind="pokemon"
            ):
                return False
            for kind, cards, card_type in (
                (
                    "energy",
                    pokemon.energyCards,
                    (CardType.BASIC_ENERGY, CardType.SPECIAL_ENERGY),
                ...
                for card in cards:
                    ...
                    if (
                        card_data is None
                        or card_data.cardType not in card_type
                        or not _unique_mist_skill_is_counter_safe(
                            card_data,
                            relation="other",
                            source_kind=kind,
                        )
...
def _unique_mist_powerful_hand_is_exact_and_payable(
    obs: Observation, active: Pokemon
) -> bool:
    if (
        active.id != Alakazam
        or not _bridge_pokemon_is_publicly_complete(
        ...
        or not _unique_mist_energy_units_exact(active)
    ):
        return False
    ...
    if (
        data is None
        or list(data.attacks or []) != [ATTACK_POWERFUL_HAND]
        or attack is None
        or attack.damage != 0
        or tuple(int(cost) for cost in attack.energies)
        != (int(EnergyType.PSYCHIC),)
    ):
        return False
    text = " ".join(_normalized_skill_text(attack.text).split())
    if (
        "place 2 damage counters on your opponent's active pokemon for each "
        "card in your hand"
    ) not in text:
        return False
    units = _bridge_retaliation_energy_units(active)
    if units is None or _bridge_retaliation_can_pay(
...
def _unique_mist_has_safe_count_setup(
    obs: Observation, hand_count: int, target_hp: int
) -> bool:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    draw_by_source = {Dudunsparce: 3, Fezandipiti_ex: 3}
    draw_by_evolution = {Kadabra: 2, Alakazam: 3, Dudunsparce: 3}
    for option in obs.select.option:
        if option.type == OptionType.ABILITY:
            ...
            draws = draw_by_source.get(getattr(source, "id", None))
            post_hammer = hand_count + (draws or 0) - 1
        elif option.type == OptionType.EVOLVE:
            if option.area != AreaType.HAND:
                continue
            evolution = _safe_get_card(
                obs, AreaType.HAND, option.index, my_index
            )
            draws = draw_by_evolution.get(getattr(evolution, "id", None))
            post_hammer = hand_count - 1 + (draws or 0) - 1
        else:
            continue
        if (
            draws is not None
            and mine.deckCount >= draws
            and post_hammer >= 0
            and 20 * post_hammer >= target_hp
        ):
            return True
    return False


def _unique_mist_better_boss_exists(
    obs: Observation,
    *,
    hand_count_after_hammer: int,
    ...
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    play_rows = _unique_mist_canonical_play_rows(obs, my_index)
    if play_rows is None:
        return False
    boss_options = [
        (option_index, card.serial)
        for option_index, card in play_rows
        if card.id == Boss_Orders
    ]
    if not boss_options or state.supporterPlayed:
        return False
    boss_hand = hand_count_after_hammer - 1
    if boss_hand < 0:
        return False
    active_prizes = prize_count(active_target)
    active_terminal = active_prizes >= len(mine.prize)
    for target in theirs.bench:
        if (
            not _bridge_pokemon_is_publicly_complete(target, 1 - my_index)
            or not _unique_mist_counter_environment_clear(
                state, target, None
            )
            or 20 * boss_hand < target.hp
        ):
            continue
        target_prizes = prize_count(target)
        if target_prizes > active_prizes or (
            target_prizes >= len(mine.prize) and not active_terminal
        ):
            return True
    return False


def _unique_mist_resolve_energy_option(
    obs: Observation, option
) -> tuple[int, AreaType, int, Pokemon, Card] | None:
    if (
        option.type != OptionType.ENERGY
        or option.playerIndex not in (0, 1)
        or option.area not in (AreaType.ACTIVE, AreaType.BENCH)
        or not _unique_mist_strict_int(option.playerIndex, minimum=0)
        or not _unique_mist_strict_int(option.index, minimum=0)
        or not _unique_mist_strict_int(option.energyIndex, minimum=0)
        or not _unique_mist_strict_int(option.count, minimum=1)
        or option.count != 1
        or not _unique_mist_unused_option_fields_are_none(
            option,
            used=(
    ...
    if (
        not isinstance(pokemon, Pokemon)
        or option.energyIndex < 0
        or option.energyIndex >= len(pokemon.energyCards)
        or len(pokemon.energyCards) != len(pokemon.energies)
    ):
        return None
    ...
    if (
        ...
        or data.cardType != CardType.SPECIAL_ENERGY
        or energy.serial <= 0
        or getattr(energy, "playerIndex", None) != option.playerIndex
    ):
        return None
    return option.playerIndex, option.area, option.index, pokemon, energy


def _unique_mist_start(
    ...
    if (
        ...
        or parent_action[0] < 0
        or parent_action[0] >= len(select.option)
        or any(globals()[name] for name in _UNIQUE_MIST_PARENT_DICT_NAMES)
        or active is None
        or target is None
        or not _unique_mist_attacker_is_status_clear(state)
        or not _unique_mist_prior_attack_is_certified_safe(state, target)
        or mine.hand is None
        or len(mine.hand) != mine.handCount
        or mine.handCount < 2
        or len(mine.prize) <= 0
        or len(theirs.prize) <= 0
    ):
        return None
    selected_index = parent_action[0]
    play_rows = _unique_mist_canonical_play_rows(obs, my_index)
    if play_rows is None:
        return None
    play_by_option = dict(play_rows)
    hammer = play_by_option.get(selected_index)
    if hammer is None or hammer.id != Enhanced_Hammer:
        return None
    exact_hammer_options = [
        option_index
        for option_index, card in play_rows
        if card.serial == hammer.serial
    ]
    if exact_hammer_options != [selected_index]:
        return None
    hand_before = _unique_mist_cards_fingerprint(mine.hand)
    mine_discard_before = _unique_mist_cards_fingerprint(mine.discard)
    opponent_discard_before = _unique_mist_cards_fingerprint(theirs.discard)
    stadium = _unique_mist_cards_fingerprint(state.stadium)
    mine_bench = _unique_mist_pokemon_group_fingerprint(
        mine.bench, my_index
    )
    opponent_bench = _unique_mist_pokemon_group_fingerprint(
        theirs.bench, 1 - my_index
    )
    if None in (
        hand_before,
        mine_discard_before,
        opponent_discard_before,
        stadium,
        mine_bench,
        opponent_bench,
    ):
        return None
    hand_after = _unique_mist_remove_serial(hand_before, hammer.serial)
    if hand_after is None:
        return None

    mist_matches = [
        (index, energy)
        for index, energy in enumerate(target.energyCards)
        if energy.id == Mist_Energy
    ]
    if len(mist_matches) != 1:
        return None
    mist_index, mist = mist_matches[0]
    target_after = _unique_mist_projected_pokemon_without_energy(
        target, mist.serial
    )
    hammer_fp = _bridge_card_fingerprint(hammer)
    mist_fp = _bridge_card_fingerprint(mist)
    if (
        target_after is None
        or not _unique_mist_powerful_hand_is_exact_and_payable(obs, active)
        or not _unique_mist_counter_environment_clear(
            state, target, mist.serial
        )
    ):
        return None
    ...
    if (
        20 * h1 < target.hp
        or 20 * (h1 - 1) >= target.hp
        or _unique_mist_has_safe_count_setup(obs, mine.handCount, target.hp)
        or _unique_mist_better_boss_exists(
            obs,
            hand_count_after_hammer=h1,
    ...
    _unique_mist_hammer_latch.update(
        ...
        own_prize_count=len(mine.prize),
        opponent_prize_count=len(theirs.prize),
        statuses=_unique_mist_statuses(state),
        stadium=stadium,
        own_bench=mine_bench,
        opponent_bench=opponent_bench,
        attacker_serial=active.serial,
        attacker_fingerprint=_bridge_pokemon_fingerprint(active),
        target_serial=target.serial,
        target_id=target.id,
        target_hp=target.hp,
        target_prizes=prize_count(target),
        target_before=_bridge_pokemon_fingerprint(target),
        target_after=target_after,
        hammer_serial=hammer.serial,
        hammer_fingerprint=hammer_fp,
        mist_serial=mist.serial,
        mist_id=mist.id,
        mist_fingerprint=mist_fp,
        hand_before=hand_before,
        hand_after_hammer=hand_after,
        hand_count_after_hammer=h1,
        mine_discard_before=mine_discard_before,
        mine_discard_after=mine_discard_before + (hammer_fp,),
        opponent_discard_before=opponent_discard_before,
        opponent_discard_after=opponent_discard_before + (mist_fp,),
        expected_damage=20 * h1,
        hammer_action=tuple(parent_action),
        prior_attack_witness=_unique_mist_prior_attack_witness_fingerprint(),
    )
    _unique_mist_hammer_telemetry["starts"] += 1
    return list(parent_action)


def _unique_mist_static_state_matches(obs: Observation, latch: dict) -> bool:
    state = obs.current
    my_index = state.yourIndex
    mine = state.players[my_index]
    theirs = state.players[1 - my_index]
    return (
        ...
        and _unique_mist_attacker_is_status_clear(state)
        and state.energyAttached == latch.get("energy_attached")
        ...
        and mine.deckCount == latch.get("own_deck_count")
        ...
        and _unique_mist_statuses(state) == latch.get("statuses")
        and _unique_mist_cards_fingerprint(state.stadium)
        == latch.get("stadium")
        and _unique_mist_pokemon_group_fingerprint(mine.bench, my_index)
        == latch.get("own_bench")
        and _unique_mist_pokemon_group_fingerprint(
            theirs.bench, 1 - my_index
        )
        == latch.get("opponent_bench")
        and _unique_mist_prior_attack_witness_fingerprint()
        == latch.get("prior_attack_witness")
    )


def _unique_mist_advance(obs: Observation) -> tuple[list[int] | None, str]:
    """Return (action, outcome); outcome is action, resolved, or a reason."""
    latch = _unique_mist_hammer_latch
    state = obs.current
    select = obs.select
    my_index = state.yourIndex
    mine = state.players[my_index]
    ...
    if stage == "await_resolution":
        target_gone = target is None or target.serial != latch.get("target_serial")
        prize_taken = len(mine.prize) < latch.get("own_prize_count")
        if state.result != -1 or target_gone or prize_taken:
            _unique_mist_hammer_telemetry["resolutions"] += 1
            _unique_mist_clear_transaction()
            return None, "resolved"
        return None, "resolution_not_observed"

    if not _unique_mist_static_state_matches(obs, latch):
        return None, "static_state_mismatch"
    if active is None or target is None:
        return None, "missing_active"
    if (
        active.serial != latch.get("attacker_serial")
        or _bridge_pokemon_fingerprint(active)
        != latch.get("attacker_fingerprint")
    ):
        return None, "attacker_mismatch"

    if stage == "await_hammer_target":
        if (
            state.turnActionCount != latch.get("turn_action_count") + 1
            or _bridge_pokemon_fingerprint(target)
            != latch.get("target_before")
            or _unique_mist_cards_fingerprint(mine.hand)
            != latch.get("hand_after_hammer")
            or mine.handCount != latch.get("hand_count_after_hammer")
            or _unique_mist_cards_fingerprint(mine.discard)
            != latch.get("mine_discard_before")
            or _unique_mist_cards_fingerprint(theirs.discard)
            != latch.get("opponent_discard_before")
            or select.context != SelectContext.DISCARD_ENERGY
            or getattr(select.effect, "id", None) != Enhanced_Hammer
            or getattr(select.effect, "serial", None)
            != latch.get("hammer_serial")
            or getattr(select.effect, "playerIndex", None) != my_index
            or select.minCount != 1
            or select.maxCount != 1
        ):
            return None, "hammer_target_callback_mismatch"
        resolved = []
        for option_index, option in enumerate(select.option):
            row = _unique_mist_resolve_energy_option(obs, option)
            if row is None:
                return None, "malformed_energy_option"
            resolved.append((option_index, row))
        matches = [
            option_index
            for option_index, (owner, area, _, pokemon, energy) in resolved
            if owner == 1 - my_index
            and area == AreaType.ACTIVE
            and pokemon.serial == latch.get("target_serial")
            and energy.serial == latch.get("mist_serial")
            and energy.id == latch.get("mist_id")
        ]
        if len(matches) != 1:
            return None, "unique_active_mist_option_missing"
        latch["stage"] = "await_attack"
        _unique_mist_hammer_telemetry["targets"] += 1
    ...
    if stage == "await_attack":
        if (
            ...
            or _bridge_pokemon_fingerprint(target)
            != latch.get("target_after")
            or _unique_mist_cards_fingerprint(mine.hand)
            != latch.get("hand_after_hammer")
            or mine.handCount != latch.get("hand_count_after_hammer")
            or _unique_mist_cards_fingerprint(mine.discard)
            != latch.get("mine_discard_after")
            or _unique_mist_cards_fingerprint(theirs.discard)
            != latch.get("opponent_discard_after")
            or select.context != SelectContext.MAIN
            or select.minCount != 1
            or select.maxCount != 1
            or not _unique_mist_powerful_hand_is_exact_and_payable(obs, active)
            or not _unique_mist_counter_environment_clear(state, target, None)
            or 20 * mine.handCount != latch.get("expected_damage")
            or 20 * mine.handCount < target.hp
            or 20 * (mine.handCount - 1) >= target.hp
            or prize_count(target) != latch.get("target_prizes")
        ):
            return None, "post_hammer_attack_mismatch"
        matches = [
            option_index
            for option_index, option in enumerate(select.option)
            if option.type == OptionType.ATTACK
        ...
        if len(matches) != 1:
            return None, "unique_powerful_hand_missing"
        latch["stage"] = "await_resolution"
        _unique_mist_hammer_telemetry["attacks"] += 1
        return [matches[0]], "action"
...
def agent(obs_dict: dict) -> list[int]:
    """Final public entry point; remain the last callable insertion."""
    if obs_dict.get("select") is None:
        _unique_mist_clear_transaction()
        _unique_mist_clear_prior_attack_witness()
        return _unique_mist_hammer_parent_agent(obs_dict)
    ...
    if (
        ...
        _unique_mist_observe_public_attack_history(obs)

    if _unique_mist_hammer_latch:
        signature = _unique_mist_observation_signature(obs)
        if (
            signature == _unique_mist_hammer_last_signature
            and _unique_mist_hammer_last_action is not None
        ):
            return list(_unique_mist_hammer_last_action)
        parent_snapshot = _unique_mist_parent_snapshot()
        action, outcome = _unique_mist_advance(obs)
        if action is not None:
            return _unique_mist_cache_action(obs, action)
        if outcome == "resolved":
            return _unique_mist_hammer_parent_agent(obs_dict)
        _unique_mist_restore_parent(parent_snapshot)
        _unique_mist_abort(outcome)
        return _unique_mist_hammer_parent_agent(obs_dict)

    parent_action = _unique_mist_hammer_parent_agent(obs_dict)
    transaction_action = _unique_mist_start(obs, parent_action)
    if transaction_action is not None:
        return _unique_mist_cache_action(obs, transaction_action)
